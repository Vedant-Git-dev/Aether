...
export async function resolveUpdatedNodeRuntime(
  ...
  if (env.AETHER_NODE_UPDATE_RESPAWNED === "1") {
    return null;
  }
  ...
  process.stderr.write(
    "Install a compatible Node.js for Aether only and retry this command.\n" +
  ...
  if (result.status !== 0 || !isUsableNode(nodePath, { env, trustedRoot: recoveryRoot })) {
    process.stderr.write(
      "aether: Node.js update failed; install a compatible Node.js manually.\n",
    );
    return null;
  }
  process.stderr.write("aether: Node.js updated. Retrying your command.\n");
  return nodePath;
}
