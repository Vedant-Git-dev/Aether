...
export const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    ...
    process.stderr.write(
      `[aether] Failed to respawn launcher: ${
...
function readWindowsServiceScript(filename) {
  ...
  if (buffer[0] === 0xff && buffer[1] === 0xfe) {
    ...
    const marker = /^@rem aether-launcher-encoding=(\S+)\s*$/.exec(
      buffer.subarray(0, end < 0 ? buffer.length : end).toString("latin1"),
    );
    if (marker) {
      if (!preamble) {
  ...
  process.stderr.write(
    `aether: service script uses code page ${Number.isFinite(codePage) ? codePage : "unknown"}; not decodable here\n`,
  );
  return null;
}
...
function managedServiceNode(homeDir, env) {
  let profile = env.AETHER_PROFILE?.trim();
  ...
  if (process.platform === "darwin") {
    ...
    const label = env.AETHER_LAUNCHD_LABEL?.trim() || `ai.aether.${suffix || "gateway"}`;
    ...
    const name =
      env.AETHER_SYSTEMD_UNIT?.trim() || `aether-gateway${suffix ? `-${suffix}` : ""}`;
    ...
    const scriptName = env.AETHER_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
    if (/[/\\]|\.\./.test(scriptName)) {
      return null;
    }
    const stateDir = resolveRecoveryPath(
      env.AETHER_STATE_DIR?.trim() ||
        (homeDir && path.join(homeDir, `.aether${suffix ? `-${suffix}` : ""}`)),
      homeDir,
    );
    const filename = resolveRecoveryPath(
      env.AETHER_TASK_SCRIPT?.trim() || (stateDir && path.join(stateDir, scriptName)),
      homeDir,
    );
...
export async function recoverNodeRuntime({
  ...
  if (
    ...
    env.AETHER_NODE_UPDATE_RESPAWNED === "1" ||
    !process.argv[1] ||
  ...
  const recoveryHome = resolveRecoveryPath(
    homeDir ?? (env.AETHER_HOME?.trim() || osHome),
    osHome,
    { allowMissing: true, allowCwd: true },
  );
  const recoveryPath = recoveryHome && path.join(recoveryHome, ".aether");
  const recoveryRoot =
    recoveryPath &&
  ...
  let nodePath = recoveryRoot
    ...
    : null;
  let reason = "cached Aether runtime";
  ...
  if (!nodePath && allowInstall && recoveryRoot) {
    ...
    reason = "private Aether runtime";
  }
  if (!nodePath) {
    return false;
  }
  process.stderr.write(
    `aether: Retrying with ${JSON.stringify(nodePath)} (${reason}; current Node failed runtime admission).\n`,
  );
  runRespawnedChild(nodePath, [...process.execArgv, process.argv[1], ...process.argv.slice(2)], {
    ...env,
    AETHER_NODE_UPDATE_RESPAWNED: "1",
  });
