...
export function enforceSupportedNodeRuntime(
  ...
  const requirement = engine
    ? `this Aether release requires Node ${engine}.`
    : "could not read this Aether release's Node requirement.";
  reportError(
    [
      `[aether] error: ${requirement}`,
      `[aether] detected Node ${detectedRuntime?.version ?? "missing"} (exec: ${detectedRuntime?.execPath || "unknown"}).`,
      "[aether] install Node: https://nodejs.org/en/download",
      "[aether] upgrade Node, then retry the Aether update.",
    ].join("\n"),
  );
  return false;
}
...
export function removeLegacyPackageInstallGuard(
  ...
  try {
    ...
    reportError(
      `[aether] error: could not remove the legacy package install guard: ${
...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[aether] warning: detected ${normalizedPackageManager} for install lifecycle.`,
    "[aether] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[aether] prefer: corepack pnpm install",
  ].join("\n");
}
