#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports Aether repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const aetherMeta =
    ...
    "aether" in source.packageJson
      ? (source.packageJson.aether as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    aetherMeta &&
    typeof aetherMeta.channel === "object" &&
    aetherMeta.channel &&
    !Array.isArray(aetherMeta.channel)
      ? (aetherMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
