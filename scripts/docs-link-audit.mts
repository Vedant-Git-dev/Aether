...
export function prepareMirroredDocsDir(
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aether-docs-link-audit-"));
...
function parseAuditUrl(
  ...
  try {
    ...
    return ok({
      ...
      pathname:
        url.hostname === "docs.aether.ai" ? decodeURIComponent(url.pathname) : url.pathname,
      hash: url.hash,
    });
  } catch (error) {
...
export function auditDocsLinks(
  ...
  if (options.anchors) {
    ...
    for (const record of records) {
      ...
      if (destination.hostname !== "docs.aether.ai") {
        continue;
      }
  ...
  for (const abs of index.markdownFiles) {
    ...
    for (const { href: raw, line } of document.links) {
      ...
      const urlResult = parseAuditUrl(raw, `https://docs.aether.ai${pageRoute(rel)}`);
      if (urlResult.ok && urlResult.value.hostname !== "docs.aether.ai") {
        continue;
      }
      checked++;
      if (!urlResult.ok) {
  ...
  try {
    ...
    if (args.includes("--anchors")) {
      ...
      if (!mirroredDocsDir.mirroredClawHub) {
        console.error(
          `docs:check-links: ClawHub docs are authored in aether/clawhub and are absent from this checkout. ` +
