...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "AETHER_COMPILE_CACHE_DISABLED_RESPAWNED";

const ensureSupportedRuntimeVersion = async () => {
  if (process.versions.bun) {
    // Bun >=1.4 (Rust rewrite) ships node:sqlite; feature-probe instead of
    // rejecting Bun outright so capable Bun builds can run Aether.
    let hasNodeSqlite;
    try {
    ...
    process.stderr.write(
      "aether: this Bun runtime is unsupported because it does not provide node:sqlite.\n" +
  ...
  const canRunDiagnostics = canRunAetherNodeDiagnostics(process.versions.node, probe.available);
  ...
  if (!diagnosticExemption) {
    process.stderr.write(`aether: ${failure}\n`);
  }
  if (diagnosticExemption) {
    return false;
  }
  ...
  if (unsupportedCommand === "update" && canRunDiagnostics) {
    ...
    process.env.AETHER_NODE_UPDATE_RESPAWNED = "1";
    return false;
  }
  return process.exit(1);
};
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.AETHER_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    AETHER_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["aether: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:aether/aether#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g aether@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.AETHER_CONTAINER)) {
    return true;
  }
...
const isHelpFastPathDisabled = () =>
  process.env.AETHER_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.AETHER_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.AETHER_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.AETHER_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "aether.json"), path.join(stateDir, "aether.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".aether", "aether.json"),
    path.join(homeDir, ".aether", "aether.json"),
    path.join(homeDir, ".aether", "aether.json"),
    path.join(homeDir, ".aether", "aether.json"),
  ];
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.AETHER_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.AETHER_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(commit ? `Aether ${version} (${commit})\n` : `Aether ${version}\n`);
    return true;
  } catch {
    return false;
  }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.AETHER_BUNDLED_VERSION) ?? "0.0.0";
}
...
if (!waitingForNodeUpdateRespawn) {
  ...
  if (
    ...
    (existsSync(new URL("./.aether-lifecycle-pending", import.meta.url)) ||
      existsSync(new URL("./dist/aether-install-guard", import.meta.url)))
  ) {
    try {
      ...
      process.stderr.write(
        `aether: package lifecycle is incomplete. Reinstall with package scripts enabled, then retry. ${error instanceof Error ? error.message : String(error)}\n`,
      );
      process.exit(1);
    }
  }
