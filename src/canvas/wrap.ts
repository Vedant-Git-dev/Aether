...
export function buildWidgetDocument(
  ...
  const sizeReporter =
    ...
    'data?.type!=="aether:widget-board-host"||typeof data.nonce!=="string"||!data.nonce)return;' +
    ...
    'if(h&&h!==last){last=h;post({type:"aether:widget-size",height:h},"*");}};' +
    ...
    'if(deltaY===delta)event.preventDefault();post({type:"aether:widget-scroll",deltaY,nonce:boardNonce},"*");},{passive:false});' +
    ...
    'if(remaining===deltaY)event.preventDefault();post({type:"aether:widget-scroll",deltaY:remaining,nonce:boardNonce},"*");},{passive:false});' +
  ...
  const widgetBridge =
    '<script>(()=>{if(!window.parent||window.parent===window||Object.prototype.hasOwnProperty.call(window,"aether"))return;' +
    "const parent=window.parent;const post=parent.postMessage.bind(parent);" +
    ...
    'c.port1.addEventListener("message",event=>{if(event.data?.type!=="aether:widget-prompt-host-ready"||inlinePromptReady)return;' +
    ...
    'post({type:"aether:widget-prompt-offer"},"*",[c.port2]);' +
    'post({type:"aether:widget-bridge-port-offer"},"*",[b.port2]);' +
    ...
    'if(data?.type==="aether:widget-host-init"&&typeof data.ticket==="string"){ticket=data.ticket;' +
    'const base=data.controlUiBaseUrl;controlUiBaseUrl=typeof base==="string"&&/^https?:\\/\\//.test(base)?base:null;' +
    'bridgePost({type:"aether:widget-host-init-ack",ticket});cancel(initTimer);' +
    ...
    'if(data?.type!=="aether:widget-bridge-response"||typeof data.id!=="string")return;' +
    ...
    'try{bridgePost({type:"aether:widget-bridge-request",id,method,params,ticket});}' +
    ...
    'const inline=()=>{promptPost({type:"aether:widget-prompt",prompt:value});resolve(true);};' +
    ...
    'define(window,"aether",{value:api,writable:false,configurable:false});' +
    ...
    'post({type:"aether:widget-bridge-ready"},"*");})();</script>';
  const errorBridge =
    "<script>(()=>{if(!window.parent||window.parent===window)return;" +
    ...
    'const data={type:"aether:widget-runtime-error",message};' +
  ...
  const chatHostBridge =
    "<script>(()=>{if(!window.parent||window.parent===window)return;" +
    'addEventListener("message",event=>{if(event.source===window.parent&&event.data?.type==="aether:widget-chat-host")' +
    'document.documentElement.classList.add("aether-chat-host");});})();</script>';
  /*
  ...
  const snapshotBridge =
    ...
    'const data=event.data;if(!data||data.type!=="aether:widget-snapshot-request"||typeof data.id!=="string")return;' +
    ...
    'post({type:"aether:widget-snapshot",id:data.id,dataUrl:toDataURL.call(canvas,"image/png"),width,height},"*");' +
    '}catch(error){post({type:"aether:widget-snapshot",id:data.id,error:stringify(error)},"*");}})();});})();</script>';
