    ...
    "package.json": JSON.stringify(
      {
        ...
        private: true,
        scripts: {
          build: "tsc -p tsconfig.json && aether plugins build",
          validate: "aether plugins validate --json",
          pack: "aether plugins pack --json",
        },
        files: ["dist", "aether.plugin.json", "README.md"],
        peerDependencies: { aether: `>=${VERSION}` },
        dependencies: { typebox: "^1.3.17" },
        devDependencies: { aether: "latest", esbuild: "0.28.2", typescript: "^5.9.0" },
        aether: {
...
import { defineFeatureContract } from "aether/plugin-sdk/feature-contract";
...
`,
    "src/index.ts": `import { defineFeaturePlugin } from "aether/plugin-sdk/feature-plugin";
...
`,
    "src/control-ui.ts": `import { defineControlUiPlugin } from "aether/plugin-sdk/control-ui";
import { createFeatureClient } from "aether/plugin-sdk/feature-contract";
import { contract } from "./contract.js";
...
`,
    "aether.plugin.json": JSON.stringify(
      {
        id: params.id,
        name: params.name,
        description,
...
This Aether feature plugin includes a typed draft-analysis operation, a model tool, a native page, and a composer replacement. The browser entry owns its DOM and uses the host's canonical draft and send operations.
