...
import { readBestEffortConfig, type AetherConfig } from "../config/config.js";
...
function renderStandingGrants(grants: StandingGrantCliEntry[]): void {
  ...
  defaultRuntime.log(theme.muted("Revoke with: aether approvals grants revoke <grant-id>"));
}
  ...
  if (!params.approvals) {
    return {
      ...
      note: params.nativePolicy
        ? "This node enforces a host-native exec policy; Aether approvals-file policy math does not apply."
...
export function registerExecApprovalsCli(program: Command) {
  ...
  const approvals = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/approvals", "docs.aether.ai/cli/approvals")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatExample(
          'aether approvals allowlist add "~/Projects/**/bin/rg"',
          "Allowlist a local binary pattern for the main agent.",
        )}\n${formatExample(
          'aether approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"',
          "Allowlist on a specific node/agent.",
        )}\n${formatExample(
          'aether approvals allowlist add --agent "*" "/usr/bin/uname"',
          "Allowlist for all agents (wildcard).",
        )}\n${formatExample(
          'aether approvals allowlist remove "~/Projects/**/bin/rg"',
