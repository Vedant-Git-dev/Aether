// `aether plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `aether plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!targetPlugin) {
    ...
    if (id === "skill-workshop") {
      ...
      const lines = [
        "Skill Workshop is built into Aether, not a plugin; configure it under skills.workshop.",
      ];
  ...
  if (output === undefined) {
    failPluginInspect(
      formatMissingPluginMessage({ id, listCommand: "aether plugins list --json" }),
      opts.json,
    );
  } else if (opts.json) {
    defaultRuntime.writeStdout(output);
...
function formatPluginInspection(
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "aether"}`);
