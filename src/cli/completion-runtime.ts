...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "AETHER_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Aether Completion";
}
 ...
 * `[[ -f "${HOME}/.aether/completions/aether.bash" ]] && source "${HOME}/.aether/completions/aether.bash"`.
...
/** Resolves the shell startup profile path that should contain the Aether completion block. */
...
/** Returns whether a shell profile already contains an Aether completion block or source line. */
 ...
 * Returns true if profile has `source <(aether completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "aether") {
  ...
  try {
    ...
    await publishOutputFileAtomically({
      ...
      tempPrefix: ".aether-completion-profile",
      durable: true,
