...
const moduleLocationImport = "aether-plugin-bundle:module-location";
const runtimeFilesImport = "aether-plugin-bundle:runtime-files";
...
function isPluginBundleHostImport(specifier: string): boolean {
  return isBuiltin(specifier) || specifier === "aether" || specifier.startsWith("aether/");
}
...
export async function buildPluginBundle(options: PluginBundleOptions) {
  ...
  const recovery = backend
    ...
    : "Use literal browser imports, or provide prebuilt browser assets without package.json aether.controlUi.";
      ...
      plugins: [
        {
          ...
          setup(build: PluginBuild) {
            build.onResolve({ filter: /^aether-plugin-bundle:module-location$/ }, () => ({
            ...
            build.onResolve({ filter: /^aether-plugin-bundle:runtime-files$/ }, () => ({
