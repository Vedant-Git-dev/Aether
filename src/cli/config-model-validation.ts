...
function collectTextModelRefs(config: AetherConfig): TouchedModelRef[] {
...
function resolveCanonicalPrimaryRef(
  ...
  const validationConfig: AetherConfig = {
    ...config,
    agents: {
      ...config.agents,
      defaults: {
...
function formatModelRefError(
  ...
  return `Cannot set model reference "${authoredValue}" at ${ref.path}: ${detail} Run ${formatCliCommand("aether models list")} to list available models.`;
}
...
export async function checkTouchedTextModelRefs(params: {
  ...
  const config = hasAgentRosterProperty(params.config)
    ? params.config
    : (migratePersistedImplicitMainRoster(params.config).config as AetherConfig);
  const previousConfig =
    params.previousConfig && !hasAgentRosterProperty(params.previousConfig)
      ? (migratePersistedImplicitMainRoster(params.previousConfig).config as AetherConfig)
  ...
  let validationConfig: AetherConfig;
  let validationPreviousConfig: AetherConfig | undefined;
  ...
  const validationRosterConfig = hasAgentRosterProperty(validationConfig)
    ? validationConfig
    : (migratePersistedImplicitMainRoster(validationConfig).config as AetherConfig);
  const validationPreviousRosterConfig =
    validationPreviousConfig && !hasAgentRosterProperty(validationPreviousConfig)
      ? (migratePersistedImplicitMainRoster(validationPreviousConfig).config as AetherConfig)
