...
import { AETHER_WRAPPER_ENV_KEY, resolveAetherWrapperPath } from "../../daemon/program-args.js";
...
function resolveGatewayInstallBindMode(cfg: AetherConfig): GatewayBindMode {
...
function formatNoAuthNonLoopbackInstallBlock(params: {
  ...
  if (normalizeOptionalString(auth.token)) {
    hints.push(
      `This config already has gateway.auth.token; run ${formatCliCommand("aether config set gateway.auth.mode token")} and then rerun ${formatCliCommand("aether gateway install --force")}.`,
    );
  } else if (normalizeOptionalString(auth.password)) {
    hints.push(
      `This config already has gateway.auth.password; run ${formatCliCommand("aether config set gateway.auth.mode password")} and then rerun ${formatCliCommand("aether gateway install --force")}.`,
    );
  } else {
    hints.push(
      `Configure token/password auth, use trusted-proxy auth, or set ${formatCliCommand("aether config set gateway.bind loopback")} before installing the managed service.`,
    );
  }
  return hints.join(" ");
}
...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === AETHER_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[normalizeInstallEnvKey(AETHER_WRAPPER_ENV_KEY)] = value;
      }
      continue;
    }
    if (
...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (opts.wrapper !== undefined) {
    try {
      wrapperPath = await resolveAetherWrapperPath(opts.wrapper);
      if (!wrapperPath) {
  ...
  if (!wrapperPath) {
    try {
      wrapperPath = await resolveAetherWrapperPath(installEnv[AETHER_WRAPPER_ENV_KEY]);
    } catch (err) {
      fail(`Invalid ${AETHER_WRAPPER_ENV_KEY}: ${String(err)}`);
      return;
    }
  }
  ...
  if (runtimeRaw === "node" && !wrapperPath && recordedNode && isNodeRuntime(recordedNode)) {
    ...
    if (replacement) {
      try {
        ...
        if (!runtimePath) {
          fail(
            `No supported Node runtime is available. Install Node ${SUPPORTED_NODE_VERSIONS}, then rerun aether gateway install.`,
          );
          return;
        }
      } catch (error) {
      ...
      fail(
        `${recordedRuntime.error.message} Reinstall with: ${formatCliCommand("aether gateway install --force")}.`,
      );
      return;
    }
  }
  ...
  if (loaded && !opts.force && !autoRefreshMessage && !opts.deferActivation) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("aether gateway install --force")}`);
    }
    return;
  }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.AETHER_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service AETHER_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[AETHER_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await getPlannedInstall();
      if (
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[AETHER_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[AETHER_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${AETHER_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }
