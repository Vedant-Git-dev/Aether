...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  const installCommand = formatCliCommand("aether gateway install");
  const reinstallCommand = formatCliCommand("aether gateway install --force");
  ...
  if (service.configAudit?.issues.length) {
    ...
    const recommendation =
      installBlock ??
      `Recommendation: run "${formatCliCommand("aether vitals")}" interactively for guided checks, or reinstall with "${reinstallCommand}".`;
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      const recovery =
        installBlock ??
        `Fix: rerun \`${reinstallCommand}\` from the same --profile / AETHER_STATE_DIR you expect.`;
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this Aether command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `aether --version`, `which aether`, and `aether gateway status --deep`; if this mismatch is unexpected, update PATH so `aether` points to the version you want, or reinstall the Gateway service from that same Aether install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("aether gateway restart")} and re-check with ${formatCliCommand("aether gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale Aether client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (serviceInspectionDetail) {
    ...
    defaultRuntime.error(errorText(`Retry: ${formatCliCommand("aether gateway status --deep")}`));
    spacer();
  }
  ...
  if (service.runtime?.missingUnit) {
    ...
    for (const hint of buildGatewayRuntimeRecoveryHints({
      ...
      restartCommand: formatCliCommand("aether gateway restart", env),
      env,
  ...
  if (service.runtime?.cachedLabel) {
    const env = service.command?.environment ?? process.env;
    const labelValue = resolveGatewayLaunchAgentLabel(env.AETHER_PROFILE);
    const recovery =
      installBlock ??
  ...
  if (service.foreignLaunchdJobs?.length) {
    ...
    if (shouldWarn) {
      ...
      defaultRuntime.log(infoText("Other Aether launchd jobs (macOS)"));
    ...
    if (shouldWarn && service.foreignLaunchdJobs.some((job) => job.safeToRemove)) {
      defaultRuntime.error(
        warnText(
          `Remove confirmed stray Gateway lifecycle jobs with ${formatCliCommand("aether vitals --fix")}.`,
        ),
      );
    }
    spacer();
  ...
  if (staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale Aether updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("aether gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.AETHER_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      if (updateCommands.length === 1 && unresolvedRepairs.length === 0) {
        defaultRuntime.log(
          `${label("Fix:")} ${updateCommands[0]} && ${formatCliCommand("aether gateway restart")}.`,
        );
        ...
        if (unresolvedRepairs.length === 0) {
          defaultRuntime.log(`Then run ${formatCliCommand("aether gateway restart")}.`);
        }
      }
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("aether gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("aether status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.aether.ai/troubleshooting`);
}
