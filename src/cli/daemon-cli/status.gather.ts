...
async function readFastStatusConfig(configPath: string): Promise<StatusConfigRead | null> {
  ...
  const cfg: AetherConfig = asNonArrayRecord(parsed);
...
export type DaemonStatus = {
   ...
   * can happen after `npm install -g aether@<v>` updates the gateway binary
   * without a corresponding `aether plugins update`.
   */
          ...
          if (hasOfficialPluginVersionCandidates({ installRecords, config: daemonCfg })) {
            if (!targetServiceCommand) {
              pluginVersionRestartReadiness = {
                status: "unresolved",
                reason:
                  "Gateway service command is unavailable, so the post-restart Aether version is unknown.",
              ...
              if (!layout?.packageVersion) {
                pluginVersionRestartReadiness = {
                  status: "unresolved",
                  reason:
                    "Gateway service package version is unavailable, so the post-restart Aether version is unknown.",
                  ...(runningGatewayVersion ? { runningGatewayVersion } : {}),
                };
              } else {
  ...
  return {
    ...
    service: {
      ...
      runtime: runtime?.inspectionFailure
        ? {
            ...runtime,
            detail: `${runtime.detail}; retry with aether gateway status --deep`,
          }
        : runtime,
      configAudit,
      ...(command
