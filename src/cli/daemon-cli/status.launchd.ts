...
import type { StaleAetherUpdateLaunchdJob } from "../../daemon/launchd.js";
import {
...
export type LaunchdJobDiagnostics = {
  staleUpdateLaunchdJobs?: StaleAetherUpdateLaunchdJob[];
...
export async function gatherLaunchdJobDiagnostics(
  ...
  const stale = deep
    ? await import("../../daemon/launchd.js")
        .then(({ findStaleAetherUpdateLaunchdJobs }) => findStaleAetherUpdateLaunchdJobs(env))
        .catch(() => [])
    : [];
  if (stale.length) {
