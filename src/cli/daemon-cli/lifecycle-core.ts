...
export async function runServiceStart(params: {
  ...
  const json = Boolean(params.opts?.json);
  const serviceCommand = formatCliCommand(`aether ${params.serviceNoun.toLowerCase()}`);
...
export async function runServiceRestart(params: {
        ...
        if (!handledRepair) {
          ...
          fail(
            `${params.serviceNoun} service needs repair before restart: ${issues
              .map((issue) => issue.message)
              .join("; ")}`,
            [formatCliCommand("aether gateway install --force")],
          );
          return false;
        }
        appendServiceLifecycleRepairAudit({
  ...
  if (loaded && params.checkTokenDrift) {
    ...
    try {
      ...
      const serviceToken = command?.environment?.AETHER_GATEWAY_TOKEN;
      ...
      if (driftIssue) {
        const recovery =
          ...
          `Run \`${formatCliCommand("aether gateway install --force")}\` to refresh the service token source.`;
