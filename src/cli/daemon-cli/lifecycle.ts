...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token: normalizeOptionalString(process.env.AETHER_GATEWAY_TOKEN),
      password: normalizeOptionalString(process.env.AETHER_GATEWAY_PASSWORD),
    },
    timeoutMs: 1_000,
  }).catch(() => null);
...
async function stopGatewayWithoutServiceManager(
  ...
  if (pids.length === 0) {
    ...
    if (portUsage !== "free") {
      throw new Error(
        portUsage === "busy"
          ? `Port ${port} is in use but the owning process could not be identified. Run ${formatCliCommand("aether gateway status --deep")} to diagnose.`
          : `Could not determine whether port ${port} is still in use, so the gateway cannot be confirmed stopped. Run ${formatCliCommand("aether gateway status --deep")} to diagnose.`,
      );
    }
    return null;
  }
...
async function signalGatewayRestart(
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "aether gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout, warn, activationAccepted: accepted }) => {
      ...
      if (restartedWithoutServiceManager) {
        ...
        fail(
          ...
          [formatCliCommand("aether gateway status --deep"), formatCliCommand("aether vitals")],
