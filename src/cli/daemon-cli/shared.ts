...
export function resolveDaemonInstallBlockMessage(
  ...
  if (process.platform === "linux" && hasSudoToRootSystemdUserManagerMismatch(env)) {
    return (
      ...
      "Aether state and service files would belong to root while systemctl targets the " +
      ...
      "See https://docs.aether.ai/cli/gateway#install-identity."
    );
  }
  return undefined;
}
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const installHint =
    ...
    formatCliCommand("aether gateway install", env);
  return buildPlatformServiceStartHints({
    installHint,
    startCommand: formatCliCommand("aether gateway start", env),
