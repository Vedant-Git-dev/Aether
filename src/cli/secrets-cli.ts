...
async function readPlanFile(pathname: string): Promise<SecretsApplyPlan> {
  ...
  if (!isSecretsApplyPlan(parsed)) {
    throw new Error(
      `Invalid secrets plan file: ${pathname}. Generate a fresh plan with ${formatCliCommand("aether secrets configure --plan-out <path>")}.`,
    );
  }
  return parsed;
}
...
export function registerSecretsCli(program: Command): void {
  const secrets = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/gateway/security", "docs.aether.ai/gateway/security")}\n`,
    );
        ...
        defaultRuntime.error(
          danger(
            formatGatewayCommandFailure({
              ...
              error: err,
              inspectCommand: "aether gateway status --deep",
            }),
          ),
        );
      },
    ...
    .action(async (opts: SecretsAuditOptions) => {
      const { report, exitCode } = await runSecretsCommand(
        ...
        (err) => {
          defaultRuntime.error(
            danger(
              `Secrets audit failed: ${formatErrorMessage(err)}. Run ${formatCliCommand("aether vitals")} to inspect config and credential state.`,
            ),
          );
        },
        2,
    ...
    .action(async (opts: SecretsConfigureOptions) => {
      await runSecretsCommand(
        ...
        (err) => {
          defaultRuntime.error(
            danger(
              `Secrets configure failed: ${formatErrorMessage(err)}. Re-run ${formatCliCommand("aether secrets audit")} before applying changes.`,
            ),
          );
        },
        1,
    ...
    .action(async (opts: SecretsApplyOptions) => {
      const result = await runSecretsCommand(
        ...
        (err) => {
          ...
          defaultRuntime.error(
            danger(
              `Secrets apply failed: ${message}. Re-run ${formatCliCommand("aether secrets apply --from <path> --dry-run")} to inspect the plan without writing.`,
            ),
          );
        },
        1,
