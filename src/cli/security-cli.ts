...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether security audit", "Run a local security audit."],
          [
            "aether security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["aether security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "aether security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "aether security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["aether security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["aether security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.aether.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Aether security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("aether security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("aether security audit --fix")}`));
        if (!fixResult) {
