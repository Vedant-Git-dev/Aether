...
export function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("aether ignite")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  return [
    params.mode === undefined
      ? [
          ...
          `Re-run \`${formatCliCommand("aether awaken --mode local")}\` or \`${formatCliCommand("aether ignite")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
        ].join(" ")
...
function applyInvocationDestructiveOverride(value: string | undefined): void {
  if (process.env.AETHER_SERVICE_MARKER?.trim() || value === undefined) {
    delete process.env[ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV];
  } else {
...
async function guardGatewayRunSelectedConfig(
  ...
  for (;;) {
    ...
    if (!snapshot.valid && params.opts.reset) {
      ...
      // off-root $include self-authorize AETHER_INCLUDE_ROOTS on the next read. Only explicit dev
    ...
    shouldBootstrap && lastGuardedGatewayRunSnapshot
      ? {
          ...
          dev:
            ...
            normalizeOptionalLowercaseString(process.env.AETHER_PROFILE) === "dev",
        }
      : undefined;
