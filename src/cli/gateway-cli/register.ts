...
import { formatByteSize } from "@aether/normalization-core";
      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["aether gateway run", "Run the gateway in the foreground."],
            ["aether gateway status", "Show service status plus connectivity/capability."],
            ["aether gateway auth-token --show", "Reveal the shared token interactively."],
            ["aether gateway discover", "Find local and wide-area gateway beacons."],
            ["aether gateway stability", "Show recent stability diagnostics."],
            ["aether gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.aether.ai/cli/gateway")}\n`,
      ),
  );
