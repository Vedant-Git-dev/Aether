...
import { printOrbBanner, type OrbBannerResult } from "../orb-banner.js";
...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.AETHER_GATEWAY_STARTUP_TRACE);
...
function warnInlinePasswordFlag() {
  defaultRuntime.error(
    "Warning: --password can be exposed via process listings. Prefer --password-file or AETHER_GATEWAY_PASSWORD.",
  );
}
...
function createConfiguredGatewayHealthProbe(cfg: AetherConfig) {
...
async function runGatewayCommandOnce(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.AETHER_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.AETHER_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.AETHER_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.AETHER_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  const bannerDone: Promise<OrbBannerResult> = process.stdout.isTTY
    ? printOrbBanner(defaultRuntime, { settleWhen: rawServerImport })
  ...
  if (snapshot) {
    ...
    const finalConfigEnteredServiceMode = Boolean(process.env.AETHER_SERVICE_MARKER?.trim());
  ...
  if (process.env.AETHER_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    ...
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("aether gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (opts.token) {
    ...
    if (token) {
      process.env.AETHER_GATEWAY_TOKEN = token;
    }
  }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or AETHER_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set AETHER_GATEWAY_TOKEN or AETHER_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or AETHER_GATEWAY_TOKEN/AETHER_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  const envSidecarStartupMode =
    isTruthyEnvValue(process.env.AETHER_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.AETHER_SKIP_PROVIDERS)
      ? "defer"
      : "start";
  ...
  const triageStartupFailure = async (error: unknown, signal?: AbortSignal) => {
    ...
    if (
      (supervisor || process.env.AETHER_SERVICE_MARKER) &&
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("aether gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("aether gateway status --deep")} for diagnostics.`,
    );
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.gatewayRunTestApi")] = testing;
}
