...
export async function runGatewayLoop(params: {
  ...
  if (process.title === "aether") {
    process.title = "aether-gateway";
  }
  ...
  // (e.g. `npm install -g aether@latest` triggered via update.run),
  ...
  const handleRestartAfterServerClose = async (
    ...
    if (respawn.mode === "failed") {
      ...
      gatewayLog.info(
        `restart mode: in-process restart (${respawn.detail ?? "AETHER_NO_RESPAWN"})`,
      );
    }
        ...
        if (!isGatewaySigusr1RestartExternallyAllowed()) {
          ...
          gatewayLog.warn(
            "An unauthorized SIGUSR1 restart signal was received and ignored. " +
              "If a pending gateway restart needs to be applied, run `aether gateway restart` " +
