...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<AetherConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.AETHER_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).AETHER_SERVICE_MARKER?.trim()
        : undefined,
    );
