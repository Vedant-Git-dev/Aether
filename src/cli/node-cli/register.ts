...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "aether node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["aether node status", "Check node host service status."],
          ["aether node install", "Install the node host service."],
          ["aether node start", "Start the installed node host service."],
          ["aether node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.aether.ai/cli/node")}\n`,
    );
  ...
  node
    ...
    .option("--context-path <path>", "Gateway WebSocket context path (e.g. /aether-gw)")
