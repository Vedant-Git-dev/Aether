// `aether plugins list`: builds registry reports and defers terminal-only formatting modules.
    ...
    const message =
      opts.enabled && report.plugins.length > 0
        ? `${
          ...
          } Run ${formatCliCommand("aether plugins list")} to inspect installed plugins.`
        : `No plugins found. Run ${formatCliCommand("aether plugins install <plugin>")} to add one, or ${formatCliCommand("aether plugins list --json")} to inspect raw discovery state.`;
