...
const CONFIG_SET_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("aether config set gateway.port 19001 --strict-json"),
  formatCliCommand(
    "aether config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
  ),
  formatCliCommand(
    "aether config set secrets.providers.vault --provider-source file --provider-path /etc/aether/secrets.json --provider-mode json",
  ),
  formatCliCommand("aether config set --batch-file ./config-set.batch.json --dry-run"),
].join("\n");
...
const CONFIG_PATCH_DESCRIPTION = [
  ...
  "Examples:",
  formatCliCommand("aether config patch --file ./aether.patch.json5 --dry-run"),
  formatCliCommand("aether config patch --stdin"),
].join("\n");
  ...
  try {
    ...
    if (!res.found) {
      const message = isConfigSchemaPath(schema, parsedPath)
        ? `Config path is valid but unset: ${opts.path}. The runtime default applies until you set an authored value with ${formatCliCommand(`aether config set ${quoteCliArg(opts.path)} <value>`)}.`
        : `Unknown config path: ${opts.path}. Run ${formatCliCommand("aether config schema")} to inspect valid paths.`;
      if (opts.json) {
...
async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "aether.json";
  try {
    const read = await readConfigFileSnapshotWithPluginMetadata({ observe: false });
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("aether awaken")} or run ${formatCliCommand("aether vitals --fix")}.`,
        );
      }
      exitCliAfterOutput(runtime, 1);
    }
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        writeRuntimeJson(runtime, {
          ...formatCliJsonFailure(`Aether config is invalid: ${shortPath}`),
          valid: false,
          path: outputPath,
          issues,
        });
      } else {
        runtime.error(danger(`Aether config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("aether config validate")}.`);
      }
      exitCliAfterOutput(runtime, 1);
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.aether.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only", ({ argv }) => isConfigSetJsonParseOnly(argv))
    ...
    .option(
      "--dry-run",
      "Validate changes without writing aether.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing aether.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for aether.json")
    .option("--json", "Output JSON", false)
