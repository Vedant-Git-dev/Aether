// Shared Aether banner: the dot-matrix orb mascot beside the AETHER
...
// wordmark starts on mascot row 3, keeping the orb's crown above the text line.
const MASCOT_ART = [
  "      .:●●:.",
  "    .:●●●●●●:.",
  "   :●●/●●●●\\●●:",
...
// Orb with the facet glints brightened; swapping the top two rows in and out
// produces the "pulse".
const MASCOT_PULSE_ROWS = ["     .:●●●●:.", "   .:◆●●●●◆:."] as const;
...
const GAP = 3;
const BANNER_WIDTH = MASCOT_WIDTH + GAP + 35;
const ROWS = MASCOT_ART.length;

type OrbBannerOptions = {
  columns?: number;
  isTty?: boolean;
  rich?: boolean;
...
export type OrbBannerResult = "static" | "completed" | "settled";
...
function plainTitleLine(): string {
  ...
  return supportsDecorativeEmoji() && icon ? `${icon} AETHER ${icon}` : "AETHER";
}
...
// shimmer passes and pulse count a little so back-to-back runs don't feel
...
async function animateBanner(opts: {
  ...
  sleep: (ms: number) => Promise<void>;
  write: (chunk: string) => void;
}): Promise<Exclude<OrbBannerResult, "static">> {
  ...
  const onSignal = (signal: "SIGINT" | "SIGTERM") => {
    restoreTerminalState(`orb banner ${signal}`);
  ...
  try {
    ...
    // Pulse: the orb flares and settles once, sometimes twice.
    const pulses = rng() < 0.4 ? 2 : 1;
    for (let pulse = 0; pulse < pulses; pulse++) {
      draw(composeFrame({ mascotRows: [...MASCOT_PULSE_ROWS, ...MASCOT_ART.slice(2)] }));
      if (!(await pause(95))) {
        return "settled";
      }
...
/**
 * Prints the Aether banner: animated on rich interactive terminals, static
 * otherwise, plain title on terminals too narrow for the art.
 */
export async function printOrbBanner(
  runtime: RuntimeEnv,
  options: OrbBannerOptions = {},
): Promise<OrbBannerResult> {
