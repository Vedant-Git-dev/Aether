// Runtime implementations for `aether plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "aether"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "aether vitals --fix" to install ${installSpec}, or install it manually with "aether plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
    ...
    return await withPluginLifecycleLease({}, async () => {
      ...
      if (inspection.state !== "fresh") {
        ...
        const message = [
          ...
          "Stop plugin package changes, then run `aether plugins registry --refresh` again.",
        ].join("\n");
        if (opts.json) {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("aether plugins registry --refresh")}`);
  }
    ...
    (report) => {
      ...
      const pluginConfigWarnings = new Set([
        ...
        ...collectStalePluginConfigWarnings({
          ...
          doctorFixCommand: "aether vitals --fix",
        ...
        return JSON.stringify(
          {
            ...
            sourceShadowing: shadowed.map((entry) => {
              ...
              return {
                ...
                repair: [
                  `aether plugins inspect ${entry.pluginId ?? "<plugin-id>"}`,
                  "edit or remove the config-selected plugin source",
                  "aether plugins registry --refresh",
                  "aether gateway restart --force",
                ],
              };
            }),
      ...
      const healthyMessage =
        ...
        'Run "aether health" to check the running Gateway, including runtime quarantines and fallbacks.';
      ...
      if (shadowed.length > 0) {
        ...
        for (const diag of shadowed) {
          ...
          lines.push("  repair:");
          lines.push("    aether plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
          lines.push("    edit or remove the config-selected plugin source");
          lines.push("    aether plugins registry --refresh");
          lines.push("    aether gateway restart --force");
        }
      }
      ...
      const docs = formatDocsLink("/plugin", "docs.aether.ai/plugin");
      lines.push("");
...
const MARKETPLACE_GATEWAY_RESTART_GUIDANCE =
  'The running Gateway could not refresh its marketplace catalog. Run "aether gateway restart" to apply the current catalog state.';

/** List entries from the configured Aether marketplace feed. */
...
/** Refresh the configured Aether marketplace feed snapshot. */
