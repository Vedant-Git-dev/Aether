...
const mockPersistedConfigs = new Map<string, AetherConfig>();
...
export const replaceConfigFileMock: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: AetherConfig }) => await configWriteMock(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/aether-state");
...
const writePersistedInstalledPluginIndexInstallRecords: Mock<WritePersistedInstalledPluginIndexInstallRecordsFn> =
  vi.fn<WritePersistedInstalledPluginIndexInstallRecordsFn>(async (records) => {
    ...
    return "/tmp/aether-state/aether.sqlite";
  });
...
const writeMockInstalledIndexWithLease: WritePersistedInstalledPluginIndexInstallRecordsWithLeaseFn =
  async (records) => {
    ...
    return {
      ...
      mutation: {
        databasePath: "/tmp/aether-state/aether.sqlite",
        before,
        after: row(
...
vi.mock("../config/config.js", () => ({
  assertConfigWriteAllowedInCurrentMode: () => {
    if (process.env.AETHER_NIX_MODE === "1") {
      throw new Error(
        [
          "Config is managed by Nix (`AETHER_NIX_MODE=1`), so Aether treats aether.json as immutable.",
          "Do not run setup, onboarding, aether update, plugin install/update/uninstall/enable, doctor repair/token-generation, or config set against this file.",
          "Agent-first Nix setup: https://github.com/aether/nix-aether#quick-start",
          "Aether Nix overview: https://docs.aether.ai/install/nix",
        ].join("\n"),
      );
    }
  },
  ...
  writeConfigFile: ((config: AetherConfig) =>
    invokeMock<
      [AetherConfig],
...
vi.mock("../hooks/install.js", () => ({
  HOOK_INSTALL_ERROR_CODE: {
    MISSING_AETHER_HOOKS: "missing_aether_hooks",
    EMPTY_AETHER_HOOKS: "empty_aether_hooks",
  },
...
export function resetPluginsCliTestState() {
  ...
  pluginCliConfigMock.mockReturnValue({} as AetherConfig);
  readConfigFileSnapshotMock.mockImplementation(async () => {
    const config = getRuntimeConfig();
    return {
      path: "/tmp/aether-config.json5",
      exists: true,
      raw: "{}",
      parsed: config,
      resolved: config,
  ...
  replaceConfigFileMock.mockImplementation((async (
    ...
    const configPath = params.writeOptions?.ownedConfigPathForWrite ?? "/tmp/aether-config.json5";
  ...
  resolveStateDir.mockReturnValue("/tmp/aether-state");
  resolveMarketplaceInstallShortcutMock.mockResolvedValue(null);
  installPluginFromMarketplaceMock.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfigMock.mockImplementation(((cfg: AetherConfig, pluginId: string) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })) as (...args: unknown[]) => unknown);
  recordPluginInstallMock.mockImplementation(
    ((cfg: AetherConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelectionMock.mockImplementation((({ config }: { config: AetherConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
