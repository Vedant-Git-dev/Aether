  ...
  try {
    ...
    defaultRuntime.error(
      [
        ...
        "To enable automatic runs, set `cron.enabled: true` (or remove `cron.enabled: false`), remove `AETHER_SKIP_CRON=1` from the Gateway's launch environment, and restart the Gateway.",
        store ? `store: ${store}` : "",
      ]
        .filter(Boolean)
        .join("\n"),
