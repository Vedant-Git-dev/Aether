...
function isSafeGatewayProbeTarget(target: GatewayProbeTarget): boolean {
  ...
  return isSecureWebSocketUrl(target.url, {
    allowPrivateWs: process.env.AETHER_ALLOW_INSECURE_PRIVATE_WS === "1",
  });
}
...
function isSafeRemoteGatewayProbeUrl(url: string): boolean {
  ...
  return (
    process.env.AETHER_ALLOW_INSECURE_PRIVATE_WS === "1" &&
...
async function resolveLocalGatewayProbeTargets(
  ...
  const hasExplicitPort = Boolean(normalizeOptionalString(process.env.AETHER_GATEWAY_PORT));
...
function isDebugProxyCaptureEnvEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    isTruthyEnvValue(env.AETHER_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.AETHER_DEBUG_PROXY_REQUIRE)
  );
}
...
function createAllowlistAgnosticCliLookupConfig(config: AetherConfig): AetherConfig {
...
async function runCliWithPreparedOutputMode(
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.AETHER_CONTAINER) ?? null;
  ...
  if (
    ...
    process.env.AETHER_UPDATE_IN_PROGRESS === "1"
  ) {
  ...
  if (shouldEnsureCliPath(normalizedArgv)) {
    const { ensureAetherCliOnPath } = await import("../infra/path-env.js");
    ensureAetherCliOnPath();
  }
  ...
  let bestEffortConfigPromise: Promise<AetherConfig> | null = null;
  ...
  const readBestEffortCliConfig = async (): Promise<AetherConfig> => {
  ...
  const replaceStartedProxy = async (config: AetherConfig["proxy"]) => {
  ...
  try {
    ...
    if (bareSessionInvocation) {
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          "Aether TUI needs an interactive TTY. Use `aether agent --local ...` for automation.",
        );
        process.exitCode = 1;
        return;
      }
    ...
    // `aether <typo> --help` surfaces the same Unknown command error as
    // `aether <typo>` instead of silently showing generic top-level help.
    ...
    if (bareRootLaunchTarget) {
      if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
        if (!process.stdin.isTTY || !process.stdout.isTTY) {
          console.error(
            "Remote Gateway inference setup needs an interactive TTY. Re-run `aether` in a terminal connected to this Gateway.",
          );
          process.exitCode = 1;
          return;
        }
        ...
        if (!process.stdin.isTTY || !process.stdout.isTTY) {
          console.error(
            bareRootLaunchTarget.classic
              ? "Aether config is invalid. Run `aether vitals --fix` before onboarding."
              : "Onboarding needs an interactive TTY. Use `aether awaken --non-interactive --accept-risk ...` for automation.",
          );
          process.exitCode = 1;
          return;
        }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[aether] Non-fatal uncaught exception (continuing):",
        ...
        for (const line of formatCliFailureLines({
          title: "Aether hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
