...
const CLAW_DEV_RESULT_SCHEMA_VERSION = "aether.aetherev.v1" as const;
...
async function prepareDev(projectPath: string, opts: ClawsDevOptions): Promise<PreparedDev> {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "aether-claw-dev-"));
  try {
    ...
    try {
      ...
      if (!result.ok) {
        throw new ClawProjectError(
          ...
          formatAetheriagnostics(result.diagnostics),
        );
      }
      ...
      if (!configSnapshot.valid) {
        throw new ClawProjectError(
          "config_unavailable",
          "Aether config is invalid; fix it before previewing a Claw project.",
        );
      }
...
export async function runClawsValidateCommand(
  ...
  if (opts.json) {
    writeRuntimeJson(runtime, {
      ...
      ...(result.claw.aetherProfile ? { aetherProfile: result.claw.aetherProfile } : {}),
