...
export function rejectAgentScopedModelCommand(
  ...
  throw new Error(
    `aether models ${commandName} does not support --agent; it is global and never agent-scoped. Remove --agent, or run ${formatCliCommand("aether agents list")} and set the per-agent model in agent config.`,
  );
}
