...
/** Options accepted by `aether plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: ${formatCliCommand(`aether plugins install clawhub:${pkg.name}`)}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    const message = "Usage: aether plugins search <query>";
