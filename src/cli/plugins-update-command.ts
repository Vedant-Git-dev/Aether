// `aether plugins update` command implementation for tracked npm plugins and hook packs.
...
function projectUpdaterResultOntoSourceConfig(params: {
  runtimeBase: AetherConfig;
  sourceBase: AetherConfig;
  updatedConfig: AetherConfig;
}): AetherConfig {
  const updatePatch = createMergePatch(params.runtimeBase, params.updatedConfig);
  return applyMergePatch(params.sourceBase, updatePatch) as AetherConfig;
}
...
async function runPluginUpdateCommandUnlocked(
  ...
  if (pluginSelection.pluginIds.length === 0 && hookSelection.hookIds.length === 0) {
    ...
    defaultRuntime.error(
      params.id
        ? `No tracked plugin or hook pack found for "${params.id}". Run "${formatCliCommand("aether plugins list")}" or "${formatCliCommand("aether hooks list")}" to inspect installed packages.`
