...
async function writeCompletionCache(params: {
  ...
  for (const shell of params.shells) {
    ...
    await publishOutputFileAtomically({
      ...
      tempPrefix: ".aether-completion-cache",
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.aether.ai/cli/completion")}\n`,
    )
    .addOption(
      new Option(
        "-s, --shell <shell>",
    ...
    .option(
      "--write-state",
      "Write completion scripts to $AETHER_STATE_DIR/completions (no stdout)",
    )
