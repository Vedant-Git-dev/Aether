...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by Aether and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the Aether version and timestamp that produced the file.",
  ].join("\n");
}
...
function configApplyHintForOperations(
  operations: readonly ConfigSetOperation[],
  beforeConfig: AetherConfig,
  afterConfig: AetherConfig,
): string {
...
export async function runConfigOperations(params: {
  ...
  let nextConfig = normalizeConfigMutationModelRefs(next as AetherConfig);
