...
import { openExistingAetherStateDatabaseReadOnly } from "../state/aether-state-db.js";
import {
  emitClawFailure,
...
export async function runClawsUpdateCommand(
  ...
  if (!source) {
    const database = await openExistingAetherStateDatabaseReadOnly();
  ...
  if (!loaded.ok) {
    ...
    emitClawFailure(runtime, opts.json, formatAetheriagnostics(diagnostics), {
      schemaVersion: CLAW_UPDATE_PLAN_SCHEMA_VERSION,
      stability: CLAW_OUTPUT_STABILITY,
      dryRun: true,
      mutationAllowed: false,
  ...
  const plan = await buildClawUpdatePlan({
    ...
    targetAetherProfile: loaded.aetherProfile,
