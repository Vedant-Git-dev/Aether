import { isSensitiveUrlQueryParamName } from "@aether/net-policy/redact-sensitive-url";
import {
  type ControlUiSessionPathTarget,
  parseControlUiSessionPath,
} from "@aether/session-url-contract/parse";
...
function bareSessionOptionError(flag: string): Error {
  return new Error(
    `Unsupported bare session URL option: ${sanitizeTerminalText(flag)}. Use \`aether tui <url> --help\` for the full option list.`,
  );
}
...
export function parseBareSessionInvocation(argv: readonly string[]): BareSessionInvocation | null {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    if (!arg.startsWith("-")) {
      ...
      throw new Error(
        "Unexpected extra argument for bare session URL. Use `aether tui <url> --help` for the full option list.",
      );
    }
