...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.AETHER_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.AETHER_PROFILE)}.service`;
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.AETHER_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.AETHER_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `aether-restart-${timestamp}.sh`;
...
${logSetup}
${userBusRepair}
printf '[%s] aether restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] aether restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
    printf '[%s] aether restart failed source=update status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
  fi
elif systemctl is-active --quiet '${escaped}' || systemctl is-enabled --quiet '${escaped}'; then
  status=78
  printf '[%s] system-scoped aether gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
status=0
if [ -n "$aether_system_launchd_conflict" ]; then
  status=78
  printf '[%s] aether restart blocked source=update reason=%s\n' "$(date -u +%FT%TZ)" "$aether_system_launchd_detail" >&2
...
# Self-cleanup (log is retained under the Aether state logs directory).
script_dir=$(dirname "$0")
rm -f "$0"
rmdir "$script_dir" 2>/dev/null || true
exit "$status"
`;
      ...
      filename = `aether-restart-${timestamp}.cmd`;
...
setlocal
set "AETHER_RESTART_SCRIPT_DIR=%~dp0."
...
findstr /x /c:"AETHER_RESTART_COMPLETE" "%~dpn0.out" >nul 2>&1
...
del "%~f0.vbs" >nul 2>&1
del "%~f0" >nul 2>&1
rmdir "%AETHER_RESTART_SCRIPT_DIR%" >nul 2>&1
exit %status%
)
`;
      scriptContent = `
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] aether restart log initialized"
} catch {
...
function Join-AetherProcessArguments {
...
function Invoke-AetherSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-AetherProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "aether restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "aether restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-AetherScheduledTaskState {
...
# AETHER_RESTART_KILL_POLICY_BEGIN
function Split-AetherWindowsCommandLine {
...
function Get-AetherListenerSnapshot {
  param([int]$Port)
  ...
  try {
    ...
    Write-RestartLog "aether restart Get-NetTCPConnection query failed source=update error=$($_.Exception.Message)"
  }
  ...
  try {
    ...
    Write-RestartLog "aether restart netstat query failed source=update error=$($_.Exception.Message)"
    return [pscustomobject]@{ Known = $false; Pids = @() }
  }
}

function Get-AetherProcessFacts {
  param([int]$ProcessId)
  ...
  try {
    ...
    return [pscustomobject]@{
      ...
      Argv = @(Split-AetherWindowsCommandLine -CommandLine ([string]$process.CommandLine))
    }
  } catch {
    Write-RestartLog "aether restart process query failed source=update pid=$ProcessId error=$($_.Exception.Message)"
    return $null
  }
}

function Test-AetherArgvEqual {
...
function Test-AetherSameProcess {
  param($Expected, $Actual)
  return (
    $null -ne $Actual -and
    $Actual.ProcessId -eq $Expected.ProcessId -and
    $Actual.CreationTimeFileTime -eq $Expected.CreationTimeFileTime -and
    (Test-AetherArgvEqual -Actual $Actual.Argv -Expected $Expected.Argv)
  )
}

function Get-AetherListenerKillDecision {
  param(
    [int]$CandidatePid,
    [string[]]$ExpectedArgv,
    $ObservedProcess,
  ...
  if (-not (Test-AetherArgvEqual -Actual $ObservedProcess.Argv -Expected $ExpectedArgv)) {
  ...
  if (-not (Test-AetherSameProcess -Expected $ObservedProcess -Actual $RecheckedProcess)) {
    return "process-replaced"
  }
  return "kill"
}

function Invoke-AetherVerifiedListenerKill {
  param(
    [int]$ProcessId,
    [int]$Port,
    [string[]]$ExpectedArgv,
    [scriptblock]$ProcessQuery = { param([int]$QueryPid) Get-AetherProcessFacts -ProcessId $QueryPid },
    [scriptblock]$ListenerQuery = { param([int]$QueryPort) Get-AetherListenerSnapshot -Port $QueryPort },
  ...
  if ($null -eq $observedProcess) {
    Write-RestartLog "aether restart skipped listener source=update pid=$ProcessId decision=process-unavailable"
    return
  }
  if ($ExpectedArgv.Count -eq 0) {
    Write-RestartLog "aether restart skipped listener source=update pid=$ProcessId decision=expected-command-unavailable"
    return
  }
  if (-not (Test-AetherArgvEqual -Actual $observedProcess.Argv -Expected $ExpectedArgv)) {
    Write-RestartLog "aether restart skipped listener source=update pid=$ProcessId decision=command-mismatch"
    return
  }
  ...
  try {
    ...
    if ($null -eq $lease) {
      Write-RestartLog "aether restart skipped listener source=update pid=$ProcessId decision=process-handle-unavailable"
      return
    }
    ...
    $decision = Get-AetherListenerKillDecision @decisionParams
    if ($decision -ne "kill") {
      Write-RestartLog "aether restart skipped listener source=update pid=$ProcessId decision=$decision"
      return
    }

    $lease.Kill()
    Write-RestartLog "aether restart killed stale listener source=update pid=$ProcessId"
  } catch {
    Write-RestartLog "aether restart ownership verification failed source=update pid=$ProcessId error=$($_.Exception.Message)"
  } finally {
    if ($null -ne $lease) {
      $lease.Dispose()
    }
...
# AETHER_RESTART_KILL_POLICY_END

function Invoke-AetherStartupLauncher {
  param([string]$LauncherPath)
  $launcherPath = $LauncherPath
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "aether restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "aether restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "aether restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
Write-RestartLog "aether restart attempt source=update target=$taskName"

$taskState = Get-AetherScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-AetherSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "aether restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "aether restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listenerSnapshot = Get-AetherListenerSnapshot -Port $port
  if (-not $listenerSnapshot.Known) {
    if ($attempt -eq 10) {
      Write-RestartLog "aether restart listener ownership unavailable source=update; refusing force-kill"
      break
    }
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      Invoke-AetherVerifiedListenerKill -ProcessId $listenerPid -Port $port -ExpectedArgv $expectedGatewayArgv
    }
    break
  }
...
$status = Invoke-AetherSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-AetherStartupLauncher -LauncherPath $gatewayScriptPath
}
if ($status -eq 0) {
  Write-RestartLog "aether restart done source=update"
} else {
  Write-RestartLog "aether restart failed source=update status=$status"
}

[Console]::Out.WriteLine("AETHER_RESTART_COMPLETE")
exit $status
`;
    } else {
      return null;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-restart-"));
