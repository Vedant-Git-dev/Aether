...
export async function rollbackFailedUpdate(params: {
  ...
  previousSchemaVersions?: AetherSchemaVersions;
  ...
  if (!opts.recovery) {
    try {
      ...
      if (
        opts.run &&
        resolveAetherStateSqlitePath(opts.run.env) !== resolveAetherStateSqlitePath(env)
      ) {
  ...
  const stateUnchanged = async () => {
    ...
    assertCurrent();
    const sharedPath = resolveAetherStateSqlitePath(env);
    if (
      baseline === undefined ||
