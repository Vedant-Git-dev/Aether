...
export async function hasLoadedLaunchdKeepAliveSupervisor(params: {
  ...
  // Aether's loaded LaunchAgent has canonical KeepAlive policy. Read this once before
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = formatCliCommand("aether gateway restart");
  const installCommand = formatCliCommand("aether gateway install --force");
  const statusCommand = formatCliCommand("aether gateway status --deep");
  if (platform === "darwin") {
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Aether ${beforeVersion} with the same package manager, then rerun \`${formatCliCommand("aether gateway install --force")}\`.`,
    );
  }
  return lines;
}
...
export async function maybeRestartServiceAfterFailedMutableUpdate(params: {
  ...
  if (params.recovery?.serviceRestartSafe !== true || !params.recovery.version) {
    defaultRuntime.error(
      "Managed gateway remains stopped: update safety is unverified. Run `aether vitals` and inspect the update failure before restarting.",
    );
    return "failed";
  }
  try {
    ...
    defaultRuntime.error(
      `Failed to restart managed gateway service after failed update: ${String(err)}. Run \`aether gateway status --deep\` before restarting it manually.`,
    );
    return "failed";
  }
}
