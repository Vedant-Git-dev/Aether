...
import {
  ...
  type IncompatibleAetherDatabase,
  type IndeterminateAetherDatabase,
...
export function formatSchemaRefusalLines(
  schemas: {
    incompatible: readonly IncompatibleAetherDatabase[];
    indeterminate: readonly IndeterminateAetherDatabase[];
  },
  dryRun = false,
): string[] {
...
async function checkTargetDatabaseSchemas(
  ...
  return preflightAetherDatabaseSchemas({
    env: context.env,
    supportedVersions,
...
export async function checkTargetDatabaseSchemasForContexts(
  supportedVersions: AetherSchemaVersions | undefined,
  contexts: readonly TargetDatabaseSchemaContext[],
): Promise<AetherDatabaseSchemaPreflight> {
  if (!supportedVersions) {
    return { incompatible: [], indeterminate: [] };
  }
  const incompatible = new Map<string, IncompatibleAetherDatabase>();
  const indeterminate = new Map<string, IndeterminateAetherDatabase>();
...
export function hasSchemaRefusal(schemas: AetherDatabaseSchemaPreflight): boolean {
