...
export async function convergeUpdatePlugins(params: {
  ...
  const preUpdateConfig = params.configSnapshot.valid
    ? {
        ...
        authoredConfig: isRecord(params.configSnapshot.parsed)
          ? (params.configSnapshot.parsed as AetherConfig) // SAFETY: valid snapshot validated this authored record.
  ...
  return await withOwnedManagedUpdateEnv(params.ownedManagedUpdateEnv, async () => {
    const previousCompatibilityHostVersion = process.env.AETHER_COMPATIBILITY_HOST_VERSION;
    ...
    if (compatibilityDowngradeTarget) {
      ...
      process.env.AETHER_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          delete process.env.AETHER_COMPATIBILITY_HOST_VERSION;
        } else {
          process.env.AETHER_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  });
