// `aether update status`: combines install metadata, configured channel, and remote update checks.
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Aether update status"));
  defaultRuntime.log("");
  ...
  if ("runStatusError" in runStatus) {
    ...
    if (run) {
      ...
      if (abandonedRun) {
        defaultRuntime.log(
          "Abandoned update detected; the Gateway will reconcile its recorded outcome. Run aether update repair to reconcile it now.",
        );
      }
