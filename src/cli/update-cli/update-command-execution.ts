...
import {
  parsePackageAetherSchemaVersions,
...
export async function executeMutableUpdate(
  ...
  const recheckSchemas = async (versions: AetherSchemaVersions | undefined) => {
    if (!admission) {
      throw new UpdatePreMutationError(
  ...
  let previousSchemaVersions: AetherSchemaVersions | undefined;
  ...
  let validatedConfigSnapshot: { config: AetherConfig; hash?: string | null } | undefined;
    ...
    if (shouldBlockMutableUpdateFromGatewayServiceEnv({ preManagedServiceStop })) {
      ...
      throw new UpdatePreMutationError(
        ...
        [
          ...
          "That path replaces the active Aether dist tree while the live gateway may still lazy-load old chunks.",
          `Run \`${formatCliCommand("aether update")}\` from a terminal outside the gateway service.`,
        ].join("\n"),
      );
    }
  ...
  const beforeActivate = async (roots: readonly string[] = [params.root]) => {
    ...
    previousSchemaVersions = parsePackageAetherSchemaVersions(
