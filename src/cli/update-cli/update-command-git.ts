...
import { AETHER_DATABASE_SCHEMA_DOCS_URL } from "../../state/aether-database-preflight.js";
...
export async function retireStandaloneGitWrapper(params: {
  ...
  const wrapperName = platform === "win32" ? "aether.cmd" : "aether";
...
}): Promise<{ schemaVersions?: AetherSchemaVersions; metadataUnreadable?: string }> {
...
export function createBeforeGitMutation(params: {
  ...
  checkTargetSchemas: (versions: AetherSchemaVersions | undefined) => Promise<void>;
  ...
  return async (target) => {
    if (target?.metadataUnreadable) {
      throw new UpdatePreMutationError(
        ...
        `Update refused: could not inspect the target's schema support (${target.metadataUnreadable}). Retry, or see ${AETHER_DATABASE_SCHEMA_DOCS_URL}.`,
      );
    }
