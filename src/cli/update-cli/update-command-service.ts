  ...
  try {
    ...
    if (!status.profileInstalled && !opts.skipPrompt) {
      ...
      if (isCancel(shouldInstall) || !shouldInstall) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${formatCliCommand("aether completion --install")}\` later to enable.`,
          ),
        );
        return;
      }
    ...
    defaultRuntime.log(
      theme.warn(
        `Shell completion refresh failed: ${message}. Update will continue. Resolve the reported error before retrying: ${formatCliCommand("aether completion --write-state --install")}`,
      ),
    );
  }
}
...
export async function maybeRestartService(params: {
    ...
    try {
      ...
      defaultRuntime.error(
        `Gateway: restart failed: ${String(err)}. Code update remains installed; a service stopped for update may still be stopped. ` +
          "Run `aether gateway status --deep` and ask its service owner to restart it manually.",
      );
  ...
  if (!activation.opts.json) {
    ...
    if (activation.result.mode === "npm" || activation.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("aether vitals")}\`, then \`${formatCliCommand("aether gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("aether gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
