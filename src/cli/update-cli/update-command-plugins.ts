...
const POST_UPDATE_PLUGIN_REPAIR_GUIDANCE =
  "Run aether update repair to retry post-update plugin repair.";
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run aether plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as AetherConfig["channels"],
      };
    }
