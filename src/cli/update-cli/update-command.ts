...
import { disableCurrentAetherUpdateLaunchdJob } from "../../daemon/launchd.js";
...
async function updateCommandInternal(
  ...
  if ((channel === "extended-stable" && explicitTag) || unsupportedMainTag) {
    await refuseUpdate(
      ...
      unsupportedMainTag
        ? "`--tag main` cannot update a package install. Run `aether update --channel dev` to switch to the supported Git checkout and build flow."
        : undefined,
    );
    return;
  }
  ...
  let packageTargetSchemaVersions: AetherSchemaVersions | undefined;
    ...
    if (targetVersion) {
      ...
      if (targetMetadata.error || targetMetadata.version !== targetVersion) {
        await refuseUpdate(
          ...
          `Update refused: could not inspect exact package target aether@${targetVersion}: ${targetMetadata.error ?? `registry returned version ${targetMetadata.version ?? "unknown"}`}.`,
        );
        return;
      }
  ...
  if (updateInstallKind === "package") {
    ...
    if (runtimeSelection.replacedNodeRunner && !opts.json) {
      defaultRuntime.log(
        theme.warn(
          `Managed gateway service Node (${runtimeSelection.replacedNodeRunner}) cannot run aether@${runtimeSelection.targetVersion ?? tag}.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
  ...
  const prepareMutableUpdate = async (env?: NodeJS.ProcessEnv) => {
    ...
    await withOwnedManagedUpdateEnv(env, async () => {
      ...
      fence.assertCurrent();
      await disableCurrentAetherUpdateLaunchdJob().catch(() => undefined);
      fence.assertCurrent();
