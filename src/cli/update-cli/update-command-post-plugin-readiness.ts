  ...
  return {
    ...
    warnings: [
      ...(pluginUpdate.warnings ?? []),
      {
        ...
        guidance: ["Run `aether update repair` to retry post-update readiness checks."],
      },
    ],
  };
}
...
export async function applyPostPluginUpdateReadiness(params: {
  ...
  if (!entryPath) {
    return createPostPluginReadinessExecutionFailure(
      params.pluginUpdate,
      "Updated Aether entrypoint not found for post-plugin readiness checks",
    );
  }
    ...
    warnings: [
      ...
      ...report.findings.map((finding) => {
        const warning: NonNullable<PostCorePluginUpdateResult["warnings"]>[number] = {
          ...
          guidance: [
            finding.fixHint ??
              `Resolve this finding, then rerun \`aether vitals --lint --only ${finding.checkId}\`.`,
          ],
        };
        if (finding.source) {
