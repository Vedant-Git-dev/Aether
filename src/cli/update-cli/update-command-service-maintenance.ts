...
import { stableStringify } from "@aether/normalization-core/stable-stringify";
...
const GATEWAY_SERVICE_INSPECTION_BLOCK_MESSAGE =
  "Gateway service inspection is unavailable. Refusing to mutate code because managed service ownership cannot be verified. Run `aether gateway status --deep` and retry when service access is restored.";
...
function matchesStoppedService(
  ...
  return Boolean(
    ...
    resolveGatewayProfileSuffix(before.serviceEnv.AETHER_PROFILE) ===
      resolveGatewayProfileSuffix(state.env.AETHER_PROFILE) &&
  ...
  if (serviceUpdateVerdict.kind === "foreign") {
    return {
      ...
      serviceMutationSkipMessage:
        "Gateway service management skipped: the service belongs to a different Aether installation and was left untouched.",
    };
  }
