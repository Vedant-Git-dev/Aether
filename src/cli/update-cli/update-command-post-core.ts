...
export async function readPostCorePluginInstallRecordsFile(
  ...
  try {
    ...
    throw new Error(
      `Unable to read plugin install records file: ${filePath}. Run aether vitals to inspect and repair plugin installation state.`,
      { cause: err },
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new Error(
      `Malformed JSON in plugin install records file: ${filePath}. Run aether vitals to inspect and repair plugin installation state.`,
      { cause: err },
    );
  }
  try {
    ...
    throw new Error(
      `Invalid plugin install records in handoff file: ${filePath}. Run aether vitals to inspect and repair plugin installation state.`,
      { cause: err },
    );
  }
}
...
export async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-update-post-core-"));
