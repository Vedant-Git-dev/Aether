...
import { err as resultError, ok, type Result } from "@aether/normalization-core/result";
...
export function assertGatewayServiceAdmissionUnchanged(
  ...
  if (expectedVerdict && expectedVerdict.kind !== serviceUpdateVerdict.kind) {
    throw new GatewayServiceUpdateOwnershipError(
      "Gateway service ownership changed after database admission; run `aether gateway status --deep` and retry.",
      undefined,
    );
  }
  if (
...
export async function resolvePackageRuntimePreflight(params: {
  ...
  if (!target && params.installedRoot) {
    ...
    if (!version) {
      return resultError(
        "Cannot inspect the installed Aether runtime requirement; repair its package.json before retrying aether update.",
      );
    }
    target = {
      version,
  ...
  return resultError(
    [
      `${runtimeLabel} is incompatible with aether@${targetVersion}.`,
      ...(runtime.failure ? [runtime.failure] : []),
      `The requested package requires ${target.nodeEngine}.`,
      runtime.nodeRunner
        ? "Use a compatible version of the Node runtime that owns the managed Gateway service, then rerun `aether update`."
        : "Use a Node runtime that satisfies the engine range above, then rerun `aether update`.",
      "Bare `npm i -g aether` can silently install an older compatible release.",
      "After switching Node versions, use `npm i -g aether@latest`.",
    ].join("\n"),
  );
}
...
export function formatManagedServicePackageUpdatePlan(params: {
  ...
  if (rootRedirect) {
    return [
      ...
      {
        level: "warn",
        message: `Shell Aether root differs from the managed gateway service root: ${rootRedirect.previousRoot}`,
      },
      {
        level: "muted",
