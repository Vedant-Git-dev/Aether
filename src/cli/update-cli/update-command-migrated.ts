...
export async function inspectActivatedUpdateState(
  ...
  > & {
    ...
    candidateSchemaVersions?: AetherSchemaVersions;
  },
  ...
  try {
    ...
    const shared = current.find((entry) => entry.path === resolveAetherStateSqlitePath(env));
    ...
    return updateStateSchemaVersionsMatch(schemaVersions, current, {
      sharedPath: resolveAetherStateSqlitePath(env),
      candidateSchemaVersions,
    })
      ? undefined
...
export async function continueMigratedUpdateInFreshProcess(
  ...
  const scratchDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-update-migrated-"));
  try {
