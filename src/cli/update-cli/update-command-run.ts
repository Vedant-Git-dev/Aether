      ...
      : await service
          ...
          .catch((cause: unknown) => {
            throw new GatewayServiceUpdateOwnershipError(
              "Gateway service inspection is unavailable before update admission. Run `aether gateway status --deep` from the service's owning account and retry when service access is restored.",
              cause,
            );
          });
    if (command) {
...
export async function admitUpdateCommandRun(params: {
  ...
  await assertAetherStateWriteAllowedAtPath({
    databasePath: resolveAetherStateSqlitePath(env),
    env,
  ...
  if (
    !env[UPDATE_RUN_ID_ENV] &&
    env.AETHER_UPDATE_RUN_HANDOFF !== "1" &&
