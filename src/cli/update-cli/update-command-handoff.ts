...
const UPDATE_CHAT_HANDOFF_GUIDANCE =
  "From chat, the Aether owner can start the update with the gateway update action or /update, which hands it to a managed helper.";
...
export async function handoffUpdateFromGateway(params: {
  ...
  if (
    process.env.AETHER_UPDATE_RUN_HANDOFF === "1" ||
  ...
  if (!argv1) {
    throw new UpdatePreMutationError(
      ...
      "Cannot locate the installed updater; run `aether vitals` before retrying.",
    );
  }
  ...
  if (started.status === "joined") {
    throw new UpdatePreMutationError(
      ...
      "Another managed update is already running. Inspect `aether status --all` before retrying.",
    );
  }
  const identity = {
  ...
  const statusCommand = formatInstallationTargetCommand(["aether", "status", "--all"], target, {
    env,
  });
