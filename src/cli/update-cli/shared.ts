...
const UPSTREAM_REPOSITORY_URL = "https://github.com/aether/aether.git";
...
export const DEFAULT_PACKAGE_NAME = "aether";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

/** Normalize a CLI tag/version/spec into the npm target form accepted by update flows. */
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["aether", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.AETHER_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "aether");
  }
  return path.join(home, "aether");
}
...
/** Locate the installed Aether package root that should receive update operations. */
export async function resolveUpdateRoot(): Promise<string> {
  ...
  const invocationRoot = process.argv[1]
    ? await resolveAetherPackageRoot({ cwd: path.dirname(path.resolve(process.argv[1])) })
    : null;
  return (
    invocationRoot ??
    (await resolveAetherPackageRoot({ moduleUrl: import.meta.url, cwd: process.cwd() })) ??
    process.cwd()
  );
}
...
async function cloneGitCheckoutTransactionally(params: {
  ...
  const stagingDir = await fs.mkdtemp(path.join(stagingParent, ".aether-clone-"));
  ...
  try {
    ...
    const publish = async (): Promise<string> => {
      ...
      if (!preserveDir) {
        throw new Error(
          `AETHER_GIT_DIR appeared while cloning: ${params.dir}. The existing path was left unchanged; move it or choose another AETHER_GIT_DIR, then retry.`,
        );
      }
...
/** Ensure the configured source-update directory exists and points at an Aether checkout. */
export async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new UpdatePreMutationError(
        ...
        `AETHER_GIT_DIR points at a non-git directory: ${params.dir}. Set AETHER_GIT_DIR to an empty folder or an aether checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new UpdatePreMutationError(
      "invalid-git-directory",
      `AETHER_GIT_DIR does not look like a core checkout: ${params.dir}.`,
    );
  }
...
/** Detect the package manager that owns a global/package Aether install. */
export async function resolveGlobalManager(params: {
  ...
  if (params.installKind === "package") {
    ...
    if (!detected) {
      throw new Error(
        "Update refused: package manager owner is unknown; no changes were made. Run this Aether install through its active npm, pnpm, or Bun global shim, or reinstall it with that package manager, then retry.",
      );
    }
    return detected;
  }
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: aether completion --write-state";
...
export async function tryWriteCompletionCache(
  ...
  const binPath = path.join(root, "aether.mjs");
