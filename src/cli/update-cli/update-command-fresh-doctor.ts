...
import type { ConfigFileSnapshot } from "../../config/types.aether.js";
...
async function withNormalConfigValidation<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.AETHER_UPDATE_IN_PROGRESS;
  process.env.AETHER_UPDATE_IN_PROGRESS = "0";
  try {
    return await run();
  } finally {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.AETHER_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
  }
}
  ...
  return {
    ...
    warnings: [
      ...(pluginUpdate.warnings ?? []),
      {
        ...
        guidance: ["Run `aether update repair` to retry post-update plugin repair."],
      },
    ],
  };
}
...
export async function runUpdateFinalizationDoctorInFreshProcess(params: {
  ...
  if (!entryPath) {
    throw new Error("Updated Aether entrypoint not found for post-plugin doctor");
  }
  const args = [
    entryPath,
    "doctor",
  ...
  try {
    await runExec(
      ...
      [params.entryPath, "config", "validate", "--json"],
      {
        ...
        env: { AETHER_UPDATE_IN_PROGRESS: "0" },
      },
    );
    return true;
  } catch {
