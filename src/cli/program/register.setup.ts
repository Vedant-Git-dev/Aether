...
function addSystemAgentOptions(command: Command): Command {
  return command
    .option("-m, --message <text>", "Run one Aether request")
  ...
  const command = program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("aether ignite")}\n` +
        `    ${theme.muted("Chat with Aether, or onboard when setup is incomplete.")}\n` +
        `  ${theme.command('aether ignite -m "status"')}\n` +
        `    ${theme.muted("Run one system-agent request.")}\n` +
        `  ${theme.command("aether ignite --wizard")}\n` +
        `    ${theme.muted("Run full onboarding.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.aether.ai/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
