...
function hasLegacyStateMigrationInputs(): boolean {
  ...
  if (
    !process.env.AETHER_STATE_DIR?.trim() &&
  ...
  return (
    [
      ...
      path.join(stateDir, "state", "aether.sqlite"),
...
export async function ensureConfigReady(
  ...
  params.runtime.error(heading("Aether config is invalid"));
  ...
  if (isPluginPackagingFailure || isReadOnlyConfig || !shouldOfferRecovery) {
    const fixHint = isPluginPackagingFailure
      ...
      : isReadOnlyConfig
        ...
        : commandText(formatCliCommand("aether vitals --fix"));
    params.runtime.error(`${muted("Fix:")} ${fixHint}`);
  }
  params.runtime.error(
    `${muted("Inspect:")} ${commandText(formatCliCommand("aether config validate"))}`,
  );
  params.runtime.error(
    muted(
