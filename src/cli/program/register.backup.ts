...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "docs.aether.ai/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether backup create", "Create a timestamped backup in the current directory."],
          [
            "aether backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "aether backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "aether backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "aether backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["aether backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "aether backup verify ./2026-03-09T08-00-00.000+08-00-aether-backup.tar.gz",
            "Check that the archive structure and manifest are intact.",
          ],
          [
            "aether backup verify ~/Backups/latest.tar.gz --json",
  ...
  backup
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "aether backup restore ~/Backups/latest.tar.gz --target ./restored-aether",
            "Verify, then extract the whole archive into a fresh staging directory.",
          ],
          [
            "aether backup restore ~/Backups/latest.tar.gz --target ./restored-aether --json",
...
function registerBackupGitCommands(backup: Command): void {
  ...
  git
    .command("create")
    .description("Dump selected Aether databases and commit one Git revision")
    .requiredOption("--repository <path>", "Git backup repository directory")
    .option("--all", "Back up the shared database and every registered agent database", false)
    .option("--global", "Back up the shared Aether state database", false)
...
function registerBackupSqliteCommands(backup: Command): void {
  ...
  sqlite
    .command("create")
    .description("Create a compact, verified snapshot of an Aether SQLite database")
    .option("--global", "Snapshot the shared Aether state database", false)
    .option("--agent <id>", "Snapshot one per-agent Aether database")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "aether backup sqlite create --global --repository ~/Backups/aether-sqlite",
            "Snapshot the shared state database.",
          ],
          [
            "aether backup sqlite create --agent main --repository ~/Backups/aether-sqlite",
