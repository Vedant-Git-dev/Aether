...
import { parseStrictPositiveInteger } from "@aether/normalization-core/number-coercion";
...
function registerSessionsLifecycleCommand(
  ...
  const examples: Array<[string, string]> = destructive
    ? [
        ['aether sessions delete "agent:main:scratch-1"', "Delete with confirmation."],
        [
          'aether sessions delete "agent:main:scratch-1" "agent:main:scratch-2" --yes',
          "Delete several sessions non-interactively.",
        ],
        [
          'aether sessions delete "agent:work:scratch-1" --agent work --dry-run',
          "Preview an agent-scoped delete.",
        ],
      ]
    : [
        ['aether sessions archive "agent:main:scratch-1"', "Archive one session."],
        [
          'aether sessions archive "agent:main:scratch-1" "agent:main:scratch-2"',
          "Archive several sessions.",
        ],
        [
          'aether sessions archive "agent:work:scratch-1" --agent work --dry-run',
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples(examples)}${
          destructive
            ? `\n\n${theme.muted(
                "Deletion uses the Control UI lifecycle operation, including transcript archival and runtime cleanup. Retained deleted-session archives can remain eligible for memory search. To remove indexed memories, run `aether memory forget --agent <agent-id> --session <id-or-key>` on the Gateway host or container using its state and configuration. Use the agent that owned the deleted session, including for global keys. Memory cleanup runs locally; --url does not forward it to a remote Gateway.",
              )}`
            : ""
        }`,
    )
...
export function registerStatusHealthSessionsCommands(program: Command) {
  program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether status", "Show channel health + session summary."],
          ["aether status --all", "Full diagnosis (read-only)."],
          ["aether status --json", "Machine-readable output."],
          ["aether status --usage", "Show model provider usage/quota snapshots."],
          [
            "aether status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["aether status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.aether.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.aether.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether sessions", "List all sessions."],
          ["aether sessions --agent work", "List sessions for one agent."],
          ["aether sessions --all-agents", "Aggregate sessions across agents."],
          ["aether sessions --active 120", "Only last 2 hours."],
          ["aether sessions --limit 25", "Show the newest 25 sessions."],
          ["aether sessions --json", "Machine-readable output."],
          ["aether sessions --store ./tmp/sessions.sqlite", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.aether.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "aether sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          [
            "aether sessions cleanup --dry-run --fix-dm-scope",
            "Preview stale direct-DM rows after returning dmScope to main.",
          ],
          ["aether sessions cleanup --enforce", "Apply maintenance now."],
          ["aether sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["aether sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "aether sessions cleanup --enforce --store ./tmp/sessions.sqlite",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .aether/trajectory-exports")
  ...
  addSessionsGatewayOptions(sessionsCmd.command("compact <key>"))
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            'aether sessions compact "agent:main:main"',
            "LLM-summarize a session to reclaim context budget.",
          ],
          [
            'aether sessions compact "agent:main:main" --max-lines 200',
            "Truncate to the last 200 transcript lines instead.",
          ],
          [
            'aether sessions compact "agent:work:main" --agent work --json',
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `aether sessions --agent work compact <key>`. Merge those
