...
function outputParentHelpWithoutStartupBanner(parent: Command): void {
  const previous = process.env.AETHER_SUPPRESS_HELP_BANNER;
  process.env.AETHER_SUPPRESS_HELP_BANNER = "1";
  try {
    parent.outputHelp();
  } finally {
    if (previous === undefined) {
      delete process.env.AETHER_SUPPRESS_HELP_BANNER;
    } else {
      process.env.AETHER_SUPPRESS_HELP_BANNER = previous;
    }
  }
}
