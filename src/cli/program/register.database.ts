...
import {
  ...
  preflightAetherStateDatabasePath,
  preflightAetherAgentDatabasePath,
} from "../../state/aether-database-preflight.js";
import { resolveDatabasePath } from "../../state/aether-state-db-maintenance.js";
import { claimAetherStateOwnership } from "../../state/aether-state-ownership-operations.js";
import { inspectAetherStateOwnershipAtPath } from "../../state/aether-state-ownership.js";
...
async function runDatabasePreflight(
  ...
  const result =
    options.agentId === undefined
      ? await preflightAetherStateDatabasePath(databasePath)
      : await preflightAetherAgentDatabasePath(databasePath, options.agentId);
  if (options.json) {
  ...
  try {
    ...
    const ownership =
      options.manager !== undefined
        ? claimAetherStateOwnership(options.manager, {
            path: databasePath,
            env: process.env,
          })
        : inspectAetherStateOwnershipAtPath(databasePath);
...
export function registerDatabaseCommand(program: Command): void {
  const database = program
    ...
    .addHelpText("after", `\nDocs: ${AETHER_DATABASE_SCHEMA_DOCS_URL}\n`);
