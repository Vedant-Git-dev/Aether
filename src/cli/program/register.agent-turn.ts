...
/** Register `aether agent` for one Gateway-backed agent turn. */
...
${formatHelpExamples([
  ['aether agent --to +15555550123 --message "status update"', "Start a new session."],
  ['aether agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  ["aether agent --agent ops --message-file ./task.md", "Read a multiline message file."],
  [
    'aether agent --session-key agent:ops:incident-42 --message "Summarize status"',
    "Target an exact session key.",
  ],
  [
    'aether agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'aether agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['aether agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'aether agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.aether.ai/cli/agent")}`,
    )
  ...
  agent
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ['aether agent exec "Fix the failing test"', "Run in the current directory."],
          [
            "aether agent exec --message-file task.md --cwd ./repo",
            "Read a prompt file and set the workspace.",
          ],
          [
            'aether agent exec "Summarize this repo" --model openai/gpt-5.6-sol --fallback anthropic/claude-sonnet-4-6 --json',
            "Use an explicit fallback chain and JSON output.",
          ],
          [
            'aether agent exec "Inspect this repo" --model ollama/qwen3.5:9b --code-mode code --local-model-lean --json',
            "Force Code Mode with the lean local-model tool surface.",
          ],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agent#agent-exec", "docs.aether.ai/cli/agent#agent-exec")}`,
    )
