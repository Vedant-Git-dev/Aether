...
async function runStateStoreGuard(commandPath: string[]): Promise<void> {
  ...
  try {
    ...
    outcome = await checkCliGatewayStateDir({ command: `aether ${commandPath.join(" ")}` });
  } catch (error) {
...
export function registerPreActionHooks(program: Command, programVersion: string) {
  program.hook("preAction", async (_thisCommand, actionCommand) => {
    ...
    if (cliLogLevel) {
      process.env.AETHER_LOG_LEVEL = cliLogLevel;
    }
    if (!verbose) {
      ...
      beforeStateMigrations = async (snapshot) => {
        if (snapshot) {
          const { isValidAgentId, normalizeAgentId } =
            await import("@aether/normalization-core/agent-id");
