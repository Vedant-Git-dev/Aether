...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration Aether backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether migrate list", "Show available migration providers."],
          ["aether migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["aether migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "aether migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "aether migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing Aether state"),
