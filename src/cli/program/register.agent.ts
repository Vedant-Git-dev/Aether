...
export function registerAgentsCommands(program: Command): void {
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.aether.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ['aether agents set-identity --agent main --name "Aether" --emoji "🔮"', "Set name + emoji."],
  ["aether agents set-identity --agent main --avatar avatars/aether.png", "Set avatar path."],
  [
    "aether agents set-identity --workspace ~/.aether/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "aether agents set-identity --identity-file ~/.aether/workspace/IDENTITY.md --agent main",
