...
function resolveHelpCommand(
  ...
  if (commandPath.length === 0) {
    return formatCliCommand("aether --help");
  }
  return formatCliCommand(`aether ${commandPath.join(" ")} --help`);
}
...
function formatDocsHint(): string {
  return `${theme.muted("Docs:")} ${formatDocsLink("/cli", "docs.aether.ai/cli")}`;
}

function formatCliMachineOutput(humanOutput: string): string {
  const docs = `Docs: ${formatDocsLink("/cli", "docs.aether.ai/cli", { force: false })}`;
...
function formatUnknownCommandMessage(command: string, commandPath: readonly string[]): string {
  return commandPath.length > 0
    ? `Aether ${commandPath.join(" ")} has no command ${quote(command)}.`
    : `Aether does not know the command ${quote(command)}.`;
}
...
function formatCliUnknownCommandOutput(
  ...
  return lines(
    ...
    hasParentCommand
      ? undefined
      : `${theme.muted("Plugin command?")} ${theme.command(formatCliCommand("aether plugins list"))}`,
    formatDocsHint(),
  );
}
...
/** Convert Commander parse errors into Aether-specific help and docs guidance. */
export function formatCliParseErrorOutput(
  ...
  if (unknownOption) {
    ...
    const output = `Aether does not recognize option ${quote(option)}.`;
    return lines(
      theme.error(output),
  ...
  const output = `Aether could not parse this command: ${message}`;
  return lines(
    theme.error(output),
    formatHelpHint(options.argv, { commandPath: options.commandPath }),
  );
