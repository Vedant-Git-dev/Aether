...
export function registerOnboardGatewayOptions(command: Command): Command {
  return command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. AETHER_GATEWAY_TOKEN)",
    )
...
export function registerOnboardCommand(program: Command): void {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.aether.ai/cli/onboard")}\n`,
    )
    .option(
      "--workspace <dir>",
        ...
        if (unsupportedOptions.length > 0) {
          rejectOption(
            [
              ...
              "Run those setup options without --modern, or remove them to open Aether.",
            ].join("\n"),
          );
          return;
        }
        if (opts.nonInteractive && opts.acceptRisk !== true) {
          rejectOption(
            [
              ...
              `Re-run with: ${formatCliCommand("aether awaken --modern --non-interactive --accept-risk ...")}`,
            ].join("\n"),
          );
          return;
        }
