// Formatting layer for `aether skills` commands; keeps discovery data separate from terminal UI.
...
function appendClawHubHint(output: string, json?: boolean): string {
  ...
  const command = formatCliCommand("aether skills");
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("aether skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawHubHint(
      `Skill "${safeRequestedName}" not found. Run \`${formatCliCommand("aether skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`aether config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("$AETHER_CONFIG_PATH")} ${theme.muted("(default: ~/.aether/aether.json)")}`,
    );
  }
