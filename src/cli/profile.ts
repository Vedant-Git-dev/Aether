...
export function applyCliProfileEnv(params: {
  ...
  const inheritedProfile = normalizeOptionalString(env.AETHER_PROFILE) ?? "default";
  const existingStateDir = normalizeOptionalString(env.AETHER_STATE_DIR);
  const existingConfigPath = normalizeOptionalString(env.AETHER_CONFIG_PATH);
  ...
  const inheritedServiceSelectors = {
    AETHER_LAUNCHD_LABEL: [resolveGatewayLaunchAgentLabel(inheritedProfile)],
    AETHER_SYSTEMD_UNIT: [inheritedSystemdServiceName, `${inheritedSystemdServiceName}.service`],
    AETHER_WINDOWS_TASK_NAME: [resolveGatewayWindowsTaskName(inheritedProfile)],
  };
  ...
  const replacesInheritedProfileConfig = Boolean(
    ...
    resolveHomeRelativePath(existingConfigPath, {
      env: env as NodeJS.ProcessEnv,
      homedir,
    }) === path.join(inheritedProfileStateDir, "aether.json"),
  );
  ...
  if (
    ...
    env.AETHER_CONFIG_PATH = path.join(stateDir, "aether.json");
  }
  ...
  if (profile === "dev" && !env.AETHER_GATEWAY_PORT?.trim()) {
    env.AETHER_GATEWAY_PORT = "19001";
  }
}
