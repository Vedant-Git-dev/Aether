...
function resolveHooksReportTarget(config: AetherConfig, rawAgentId?: string): HooksReportTarget {
...
function buildHooksReport(config: AetherConfig, target: HooksReportTarget): HookStatusReport {
...
async function setHookEnabled(hookName: string, enabled: boolean, agentId?: string): Promise<void> {
  const committed = await transformConfigFile({
    transform: (config) => {
      ...
      if (!hook) {
        throw new Error(
          `Hook "${hookName}" not found. Run \`${formatCliCommand("aether hooks list")}\` to see available hooks.`,
        );
      }
      ...
      if (enabled && !hook.requirementsSatisfied) {
        ...
        const installHint = hook.install.length
          ...
          : "";
        throw new Error(
          `Hook "${hookName}" is not eligible; missing ${missing}.${installHint} Run \`${formatCliCommand(`aether hooks info ${hookName}`)}\` for details.`,
        );
      }
      const entries = { ...config.hooks?.internal?.entries };
      entries[hook.hookKey] = { ...entries[hook.hookKey], enabled };
      const nextConfig: AetherConfig = {
        ...config,
        hooks: {
          ...config.hooks,
          internal: {
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.aether.ai/cli/hooks")}\n`,
    );
  ...
  hooks.hook("preAction", (_thisCommand, actionCommand) => {
    ...
    if (
      ...
      throw new Error(
        `aether hooks ${actionCommand.name()} does not support --agent; the option only selects an owner for read-only hook reports.`,
      );
    }
  });
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `aether plugins install`")
    ...
    .action(async (raw: string, opts: HooksInstallOptions) => {
      defaultRuntime.log(
        theme.warn("`aether hooks install` is deprecated; use `aether plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `aether plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`aether hooks update` is deprecated; use `aether plugins update`."),
      );
