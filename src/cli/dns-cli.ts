...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "docs.aether.ai/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. aether.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($AETHER_CONFIG_PATH, default ~/.aether/aether.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by aether dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active Aether config ($AETHER_CONFIG_PATH, default ~/.aether/aether.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
