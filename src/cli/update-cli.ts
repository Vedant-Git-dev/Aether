// Commander wiring for `aether update`, its status/finalize subcommands, and help text.
  ...
  return async (opts: Record<string, unknown>, command: Command) => {
    try {
      if (inheritOptionFromParent<boolean>(command, "reapplyLocalOverrides")) {
        throw new Error(
          `--reapply-local-overrides is not supported for aether update ${command.name()}. Use it with aether update.`,
        );
      }
      if (!options.supportsDryRun && inheritOptionFromParent<boolean>(command, "dryRun")) {
        throw new Error(
          `--dry-run is not supported for \`aether update ${command.name()}\`. Run \`aether update --dry-run\` instead.`,
        );
      }
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether update repair", "Reconcile abandoned runs or repair post-update state."],
          [
            "aether update repair --accept-capabilities",
            "Accept reviewed plugin capability changes during repair.",
          ],
          ["aether update repair --channel beta", "Repair against the beta update channel."],
          ["aether update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.aether.ai/cli/update")}`,
    )
    .action(
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Aether and inspect update channel status");
  ...
  update
    .addHelpText("after", () => {
      const examples = [
        ["aether update", "Update a source checkout (git)"],
        [
          "aether update --channel extended-stable",
          "Switch to the monthly supported npm channel",
        ],
        ["aether update --channel beta", "Switch to beta channel (git + npm)"],
        ["aether update --channel dev", "Switch to dev channel (git + npm)"],
        ["aether update --tag beta", "One-off update to a dist-tag or version"],
        ["aether update --dry-run", "Preview actions without changing anything"],
        ["aether update --no-restart", "Update without restarting the service"],
        ["aether update --json", "Output result as JSON"],
        ["aether update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aether update --accept-capabilities", "Accept reviewed plugin capability changes"],
        ["aether update repair", "Repair stranded post-update plugin state"],
        ["aether update wizard", "Interactive update wizard"],
        ["aether --update", "Shorthand for aether update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aether update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.aether.ai/cli/update")}`;
    })
        ...
        async (opts, command) => {
          for (const key of ["channel", "tag", "timeout", "restart", "acceptCapabilities"]) {
            if (
              ...
              throw new Error(
                `--${key === "restart" ? "no-restart" : key === "acceptCapabilities" ? "accept-capabilities" : key} is not supported for aether update cleanup.`,
              );
            }
          }
  ...
  update
    ...
    .requiredOption("--snapshot-config <path>", "Copied Aether config")
    .requiredOption("--snapshot-state <path>", "Copied Aether state directory")
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether update status", "Show channel + version status."],
          ["aether update status --json", "JSON output."],
          ["aether update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
