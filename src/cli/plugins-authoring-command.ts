...
import { jsonSchemaValuesEqual } from "@aether/normalization-core/json-schema";
...
export function buildToolPluginPackageManifest(params: {
  ...
  const aether =
    params.packageManifest.aether &&
    typeof params.packageManifest.aether === "object" &&
    !Array.isArray(params.packageManifest.aether)
      ? { ...(params.packageManifest.aether as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(aether.extensions)
    ? aether.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (!jsonSchemaValuesEqual(params.manifest, expectedManifest)) {
    errors.push("aether.plugin.json generated metadata is stale. Run aether plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `aether.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("aether.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`aether.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `aether.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include aether.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json aether.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run aether plugins build.");
...
export async function collectPluginsValidationResult(
  ...
  if (browserSource) {
    ...
    if (!jsonSchemaValuesEqual(manifest.controlUi, declaration)) {
      errors.push("Control UI manifest is stale. Run aether plugins build.");
    }
  }
  if (errors.length > 0) {
...
function writeToolPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageManifest = {
    name: `aether-plugin-${params.id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && aether plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && aether plugins validate --entry ./dist/index.js",
      test: "vitest run --config ./vitest.config.ts",
    },
    files: ["dist", "aether.plugin.json", "README.md"],
    peerDependencies: {
      aether: TOOL_PLUGIN_API_RANGE,
    },
    dependencies: {
      typebox: "^1.1.38",
    },
...
import { getToolPluginMetadata } from "aether/plugin-sdk/tool-plugin";
...
function writeProviderPluginScaffold(params: { rootDir: string; id: string; name: string }): void {
  const packageName = `aether-plugin-${params.id}`;
  ...
  const description = `Add ${params.name} models to Aether.`;
  const packageManifest = {
    name: packageName,
    version: "0.1.0",
    description: `Aether provider plugin for ${params.name}.`,
    type: "module",
    scripts: {
      build: "tsc -p tsconfig.json",
      test: "vitest run --config ./vitest.config.ts",
  ...
  const indexSource = `import { definePluginEntry } from "aether/plugin-sdk/plugin-entry";
import { createProviderApiKeyAuthMethod } from "aether/plugin-sdk/provider-auth";
...
`;
  const testSource = `import { describe, expect, it } from "vitest";
import type { AetherPluginApi, ProviderPlugin } from "aether/plugin-sdk/plugin-entry";
import entry from "./index.js";
...
describe(${idLiteral}, () => {
  it("registers the provider and resolves its authenticated catalog", async () => {
    ...
    } as Partial<AetherPluginApi>;

    entry.register(api as AetherPluginApi);
...
Aether provider plugin for ${params.name}.
...
jobs:
  publish:
    permissions:
      ...
      contents: read
      id-token: write
    uses: aether/clawhub/.github/workflows/package-publish.yml@${CLAWHUB_PACKAGE_PUBLISH_WORKFLOW_REF}
    with:
