...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.aether.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Aether hooks")
    ...
    .option("--hook-url <url>", "Aether hook URL")
    .option("--hook-token <token>", "Aether hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("aether webhooks gmail setup --account default")}.`,
    );
  }
