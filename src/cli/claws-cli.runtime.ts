...
export async function runClawsInspectCommand(
  ...
  if (!result.ok) {
    emitClawFailure(runtime, opts.json, formatAetheriagnostics(result.diagnostics), {
  ...
  const extensionPlan = await planClawExtensions({
    extensions: result.aetherProfile?.extensions ?? [],
  ...
  const payload = {
    ...
    ...(result.aetherProfile ? { aetherProfile: result.aetherProfile } : {}),
  ...
  if (!valid) {
    runtime.error(formatAetheriagnostics(diagnostics));
    runtime.exit(1);
  }
}
...
export async function runClawsAddCommand(
  ...
  let plan = await buildClawAddPlan({
    ...
    aetherProfile: result.aetherProfile,
    source: result.source,
    diagnostics: result.diagnostics,
    context: basePlanContext,
  });
  let legacyResumePlan = result.legacyAetherProfile
    ? await buildClawAddPlan({
        manifest: result.manifest,
        clawMarkdownBody: result.clawMarkdownBody,
        packageBootstrap: result.packageBootstrap,
        aetherProfile: result.legacyAetherProfile,
  ...
  if (result.legacyAetherProfile && !resumeState) {
    plan = {
      ...plan,
      blockers: [
        ...plan.blockers,
  ...
  if (plan.blockers.length > 0) {
    if (opts.json) {
      ...
      runtime.error(formatAetheriagnostics(plan.blockers));
    }
    runtime.exit(1);
    return;
  }
