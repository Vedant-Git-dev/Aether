...
export async function writeManifestFile(
  ...
  const dir = tempDirs.make("aether-claws-cli-");
  const path = join(dir, "aether.claw.json");
...
export async function writePackageFixture(tempDirs: {
  ...
  const root = tempDirs.make("aether-claws-cli-package-");
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      version: "1.2.3",
      aether: { claw: "aether.claw.json" },
    }),
    "utf8",
  );
  await writeFile(
    join(root, "aether.claw.json"),
    JSON.stringify({
      schemaVersion: 1,
      agent: { id: "demo-agent", name: "Demo Agent" },
      workspace: {
