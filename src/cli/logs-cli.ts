...
function resolveLogsSystemdUnitName(runtime: LogsCliRuntimeModule, env: NodeJS.ProcessEnv): string {
  const override = env.AETHER_SYSTEMD_UNIT?.trim();
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${runtime.resolveGatewaySystemdServiceName(env.AETHER_PROFILE)}.service`;
}
...
function createLogWriters(onOutputClosed?: () => void) {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `aether logs: output ${target} closed (${code}). Stopping tail.`;
      try {
...
async function emitGatewayError(
  ...
  const hint = `Hint: run \`${formatCliCommand("aether vitals")}\`.`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.aether.ai/cli/logs")}\n`,
    );
