...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: AetherConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(mode: ChannelAuthMode): string {
  ...
  if (configured.length === 0) {
    throw new Error(
      `No configured channel supports ${mode}. Run ${formatCliCommand("aether channels status")} to inspect channels or ${formatCliCommand("aether channels add --channel <channel>")} to add one.`,
    );
  }
...
async function resolveChannelPluginForMode(
  ...
  if (!channelId) {
    throw new Error(
      `Unsupported channel "${channelInput}". Run ${formatCliCommand("aether channels list")} to see available channels.`,
    );
  }
  ...
  if (!plugin || !supportsChannelAuthMode(plugin, mode)) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        channel: channelId,
        action: mode,
        inspectCommand: "aether channels status --channel " + channelId,
      }),
    );
  }
...
async function reconcileGatewayRuntimeAfterLocalLogin(params: {
  ...
  try {
    ...
    if (result.outcome && result.outcome.status !== "handed-off") {
      params.runtime.log(
        `Local login saved auth for ${params.channelId}/${params.accountId}. Gateway start: ${result.outcome.reason}. Check ${formatCliCommand(`aether channels status --channel ${params.channelId} --probe`)}.`,
      );
    }
  } catch (error) {
  ...
  if (!login) {
    throw new Error(
      formatUnsupportedChannelActionMessage({
        ...
        action: "login",
        inspectCommand: "aether channels status --channel " + channelInput,
      }),
    );
  }
