...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether channels list", "List configured channels."],
          ["aether channels list --all", "Show configured, bundled, and installable channels."],
          ["aether channels add", "Open guided channel setup."],
          ["aether channels status --probe", "Run channel status checks and probes."],
          [
            "aether channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["aether channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.aether.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether channels add", "Open guided setup for available chat channels."],
          [
            "aether channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["aether channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
