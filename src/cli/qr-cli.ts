...
function readDevicePairPublicUrlFromConfig(cfg: AetherConfig): string | undefined {
...
function shouldResolveLocalGatewayPasswordSecret(
  ...
  if (trimToUndefined(env.AETHER_GATEWAY_PASSWORD)) {
    return false;
  }
  ...
  const envToken = trimToUndefined(env.AETHER_GATEWAY_TOKEN);
...
async function resolveLocalGatewayPasswordSecretIfNeeded(cfg: AetherConfig): Promise<void> {
...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.aether.ai/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        ...
        const lines: string[] = [
          ...
          "Scan or paste this setup code in the Aether companion app onboarding.",
          "",
        ];
        ...
        lines.push(
          ...
          `  ${theme.command("aether devices list")}`,
          `  ${theme.command("aether devices approve <requestId>")}`,
        );
