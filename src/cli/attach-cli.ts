...
export function writeClaudeMcpConfig(mcpConfig: AttachGrant["mcpConfig"]): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "aether-attach-"));
...
export async function registerAttachCli(program: Command, _argv: string[] = process.argv) {
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n  aether attach                       Attach Claude Code to the main session\n  aether attach movies-a1166b81       Attach to a short session reference\n  aether attach --session agent:main:telegram:123 --ttl 600000\n  aether attach --print-config        Set up the grant + config and print how to launch it yourself\n",
    )
    .action(
      async (
