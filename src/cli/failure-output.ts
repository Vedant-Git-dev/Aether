...
} from "../state/aether-update-schema-refusal.js";
...
function isAgentSelectionCliError(error: unknown): error is Error {
  ...
  // stack trace and `aether vitals` for a missing flag.
...
function isImmutableConfigCliError(error: unknown): error is Error {
  ...
  // `aether vitals`, which cannot lift an externally managed config.
  return (
...
function shouldShowDebugDetails(
  ...
  return hasDebugArg(argv) || isTruthyEnvValue(env.AETHER_DEBUG);
}
...
export function formatCliFailureLines(options: FormatCliFailureOptions): string[] {
  ...
  const lines = [
    `[aether] ${options.title}`,
    `[aether] Reason: ${formatCliOperatorError(options.error, {
      argv: options.argv,
      env,
    })}`,
  ];
  ...
  if (showDebugDetails) {
    lines.push("[aether] Stack:");
    pushPrefixed(lines, formatUncaughtError(options.error));
  } else {
    lines.push("[aether] Debug: set AETHER_DEBUG=1 to include the stack trace.");
  }

  if (options.includeDoctorHint !== false) {
    lines.push(`[aether] Try: ${formatCliCommand("aether vitals", env)}`);
  }
  lines.push(`[aether] Help: ${formatCliCommand("aether --help", env)}`);
  return lines;
}
