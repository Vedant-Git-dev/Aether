...
async function packFeaturePlugin(opts: PluginsPackOptions) {
  ...
  if (!isRecord(packageManifest) || !isRecord(packageManifest.aether)) {
    throw new Error("Plugin package metadata is missing. Run aether plugins build.");
  }
  ...
  const staging = await fs.mkdtemp(path.join(os.tmpdir(), "aether-plugin-pack-"));
  try {
    ...
    const packedPackage = {
      ...
      typeof packageManifest.peerDependencies.aether === "string"
        ? { peerDependencies: { aether: packageManifest.peerDependencies.aether } }
        : {}),
      aether: {
        ...aether,
    ...
    await destination.create(
      "aether.plugin.json",
      await source.readBytes("aether.plugin.json"),
    );
