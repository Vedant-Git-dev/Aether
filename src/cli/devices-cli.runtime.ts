...
import { coerceErrorMessage as normalizeErrorMessage } from "@aether/normalization-core/error-coercion";
import {
...
function buildFallbackStateMismatchError(
  ...
  const guidance = currentRequestId
    ...
    : [
        "The running gateway may be using a different AETHER_PROFILE or AETHER_STATE_DIR than this CLI.",
...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("aether devices list")} to choose a paired device.`,
  );
...
export async function runDevicesJoinCodeCommand(opts: DevicesRpcOpts): Promise<void> {
  ...
  const command = `npx aether connect ${quoteCliArg(joinUrl)}`;
  if (opts.json) {
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("aether devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
...
export async function runDevicesApproveCommand(
  ...
  if (usingImplicitSelection) {
    ...
    switch (approval.kind) {
      ...
      case "re-approval":
        defaultRuntime.log(
          "  Note:   Already paired. Approval-bound device details changed, so Aether created a fresh request instead of silently reusing the old approval.",
        );
        break;
      case "new-pairing":
  ...
  if (!result) {
    defaultRuntime.error(
      `No pending device request matches ${sanitizeForLog(resolvedRequestId)}. Run ${formatCliCommand("aether devices list")} and retry with the current request ID.`,
    );
...
export async function runDevicesRejectCommand(
  ...
  if (!normalizedRequestId) {
    defaultRuntime.error(
      `requestId is required. Run ${formatCliCommand("aether devices list")} to choose a pending request.`,
    );
    defaultRuntime.exit(1);
    return;
  }
...
export async function runDevicesRenameCommand(opts: DevicesRpcOpts): Promise<void> {
  ...
  if (!deviceId || !label) {
    defaultRuntime.error(
      `--device and --name are required. Run ${formatCliCommand("aether devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
