...
export const GATEWAY_SERVICE_PATHS_UNVERIFIED =
  "Installed Gateway service state and config paths could not be verified. Inspect the service environment with `aether gateway status --deep` before repairing plugin state.";
...
> {
  try {
    ...
    delete serviceEnv.AETHER_STATE_DIR;
    delete serviceEnv.AETHER_CONFIG_PATH;
    delete serviceEnv.AETHER_HOME;
    ...
    if (
      !environment ||
      !["AETHER_STATE_DIR", "AETHER_HOME", "HOME", "USERPROFILE"].some((key) =>
...
export function compareCliGatewayStateDirs(params: {
  ...
  const gatewayConfigPath = resolveIdentityPathViaExistingAncestorSync(
    params.gatewayConfigPath ?? path.join(gatewayStateDir, "aether.json"),
  );
  const differences = [
  ...
  return {
    ...
    message: [
      ...
      `Fix: run AETHER_STATE_DIR=${quoteCliArg(gatewayStateDir)} AETHER_CONFIG_PATH=${quoteCliArg(gatewayConfigPath)} ${params.command ?? "aether tune"}.`,
