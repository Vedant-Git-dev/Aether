import { isRecord as isPlainRecord } from "@aether/normalization-core/record-coerce";
...
export function formatConfigUnsetMissingPathMessage(params: {
  ...
  if (params.runtimeOnly) {
    return `Config path not found in authored config: ${params.path}. It only exists after runtime defaults are applied, so there is nothing for config unset to remove. Use ${formatCliCommand("aether config set <path> <value>")} to override the inherited value.`;
  }
  return `Config path not found: ${params.path}. Nothing was changed. Run ${formatCliCommand("aether config get <path>")} first if you are unsure of the path.`;
}
