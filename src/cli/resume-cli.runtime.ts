...
const RESUME_INTERACTIVE_TERMINAL_GUIDANCE =
  "Attaching to a session requires an interactive terminal. Re-run `aether resume [query]` from an interactive terminal.";
...
async function promptResumeSession(
  ...
  if (choices.length === 0) {
    throw new Error(
      "No recent sessions found. Run `aether sessions` to inspect sessions or `aether tui` to start one.",
    );
  }
...
function reportResumeFailure(
  ...
  defaultRuntime.error(
    "Run `aether resume` to choose from recent sessions or `aether sessions` to inspect all sessions.",
  );
}
