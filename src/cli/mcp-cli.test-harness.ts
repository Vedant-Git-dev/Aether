...
export const serveAetherChannelMcp = mocks.serveAetherChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveAetherChannelMcp: mocks.serveAetherChannelMcp,
}));
...
export async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
