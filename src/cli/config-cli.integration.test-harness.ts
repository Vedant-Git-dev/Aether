  ...
  return {
    ...
    resolvePluginMetadataSnapshot: (
      ...
      snapshot ??= actual.loadPluginMetadataSnapshot({
        ...params,
        pluginIds: ["codex", "discord", "aether-mem0"],
...
export function useConfigCliIntegrationHarness() {
  ...
  async function withConfigFileHarness(
    ...
    const envSnapshot = captureEnv(["AETHER_CONFIG_PATH", "AETHER_TEST_FAST"]);
    try {
      fs.writeFileSync(configPath, raw, "utf8");
      setTestEnvValue("AETHER_TEST_FAST", "1");
      setTestEnvValue("AETHER_CONFIG_PATH", configPath);
      clearConfigCache();
