...
import { parseStrictFiniteNumber } from "@aether/normalization-core/number-coercion";
import { asNullableRecord as asRecord } from "@aether/normalization-core/record-coerce";
import {
        ...
        if (asRecord(server.oauth)?.identity !== "per-requester") {
          ...
          if (authStatus.state === "requires-authorization") {
            issues.push(
              issue(
                "warning",
                `OAuth credentials require additional authorization; run ${formatCliCommand(`aether mcp login ${name}`)}`,
              ),
            );
            ...
            issues.push(
              issue(
                "warning",
                `OAuth credentials are not authorized; run ${formatCliCommand(`aether mcp login ${name}`)}`,
              ),
            );
          }
        }
...
async function probeMcpServerIssues(params: {
  ...
  const runtime = await createSessionMcpRuntime({
    sessionId: "aether-cli-mcp-doctor",
...
async function probeMcpServersOrFail(params: {
  ...
  const runtime = await createSessionMcpRuntime({
    sessionId: "aether-cli-mcp-probe",
    workspaceDir: process.cwd(),
...
const AETHER_MCP_REGISTRY_SCOPE_NOTE =
  "Note: this command only shows Aether-managed mcp.servers entries and does not include mcporter servers from config/mcporter.json.";

export function registerMcpCli(program: Command) {
  const mcp = program
    .command("mcp")
    .description("Manage Aether mcp.servers config and channel bridge");

  mcp
    .command("serve")
    .description("Expose Aether channels over MCP stdio")
    ...
    .action(async (opts) => {
      try {
        ...
        const { serveAetherChannelMcp } = await import("../mcp/channel-server.js");
        await serveAetherChannelMcp({
        ...
        defaultRuntime.error(
          `MCP server failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("aether gateway status --deep --require-rpc")} to inspect Gateway health.`,
        );
        defaultRuntime.exit(1);
      }
    });
  ...
  mcp
    .command("list")
    .description("List Aether-managed MCP servers from mcp.servers")
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No Aether-managed MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('aether mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        defaultRuntime.log(AETHER_MCP_REGISTRY_SCOPE_NOTE);
        return;
      }
      defaultRuntime.log(`Aether-managed MCP servers (${loaded.path}):`);
  ...
  mcp
    .command("show")
    .description("Show one Aether-managed MCP server or the full mcp.servers config")
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && !value) {
        fail(
          `No MCP server named "${name}" in ${loaded.path}. Run ${formatCliCommand("aether mcp list")} to see configured servers.`,
          opts.json,
        );
      }
      if (opts.json) {
      ...
      if (name) {
        defaultRuntime.log(`Aether-managed MCP server "${name}" (${loaded.path}):`);
      } else {
        defaultRuntime.log(`Aether-managed MCP servers (${loaded.path}):`);
      }
      printJson(value ?? {});
  ...
  mcp
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name && loaded.mcpServers[name]?.enabled === false) {
        fail(
          `MCP server "${name}" is disabled in ${loaded.path}. Run ${formatCliCommand(`aether mcp configure ${name} --enable`)} before probing it.`,
          opts.json,
        );
      }
      ...
      if (!opts.json && Object.keys(servers).length === 0) {
        defaultRuntime.log(
          `No MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand("aether mcp add <name> --command <command>")}.`,
        );
        return;
      }
      const runtime = await createSessionMcpRuntime({
  ...
  mcp
    ...
    .action(
      async (
        ...
        if (server.auth === "oauth") {
          defaultRuntime.log(
            `Run ${formatCliCommand(`aether mcp login ${name}`)} to authorize this MCP server.`,
          );
        }
      },
    );
  ...
  mcp
    .command("set")
    .description("Set one Aether-managed MCP server from a JSON object")
    .argument("<name>", "MCP server name")
  ...
  mcp
    ...
    .action(async (name: string, opts: { include?: string; exclude?: string; clear?: boolean }) => {
      ...
      if (!result.updated) {
        fail(
          `No MCP server named "${name}" in ${result.path}. Run ${formatCliCommand("aether mcp list")} to see configured servers.`,
        );
      }
  ...
  mcp
    ...
    .action(async (name: string, opts: { code?: string }) => {
      ...
      const manualCommand = formatCliCommand(`aether mcp login ${name} --code <code>`);
      try {
        ...
        if (callbackServer) {
          defaultRuntime.log("Waiting for the browser to return to Aether...");
  ...
  mcp
    .command("unset")
    .description("Remove one Aether-managed MCP server")
    .argument("<name>", "MCP server name")
    .action(async (name: string) => {
      const result = await unsetConfiguredMcpServer({ name });
      if (!result.ok) {
