...
function hasDiagnosticsTimelinePath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.AETHER_DIAGNOSTICS_TIMELINE_PATH?.trim());
}
...
export function createGatewayDispatchStartupTrace(
  ...
  configureDiagnosticsTimeline(config: AetherConfig): Promise<void>;
  ...
  const enabled =
    isTruthyEnvValue(process.env.AETHER_GATEWAY_STARTUP_TRACE) &&
  ...
  let timelineActivation: "unknown" | "enabled" | "disabled" = timelineModule
    ? "unknown"
    : "disabled";
  let timelineConfig: AetherConfig | undefined;
