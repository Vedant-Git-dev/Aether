...
async function fetchJoinPayload(
  ...
  try {
    const guarded = await fetchWithSsrFGuard({
      url: target.toString(),
      auditContext: "aether-connect-join",
      maxRedirects: 0,
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether connect oc-pair://<setup-code>", "Connect in the foreground."],
          [
            "aether connect https://gateway.example/j/<code> --service",
            "Install the node host service.",
          ],
          [
            "aether connect https://gateway.example/j/<code> --service --session-host",
            "Install a worker-session host service.",
          ],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/connect", "docs.aether.ai/cli/connect")}\n`,
    )
