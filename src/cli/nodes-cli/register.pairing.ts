...
function buildUnknownNodePairRequestIdMessage(
  ...
  lines.push(`Run ${formatCliCommand("aether nodes pending")} to inspect current requests.`);
    ...
    nodes
      ...
      .action(async (opts: NodesRpcOpts) => {
        await runNodesCommand("rename", async () => {
          ...
          if (!name) {
            throw new Error(
              `--name must not be empty. Run ${formatCliCommand("aether nodes list")} to see paired nodes, then rerun with --name <displayName>.`,
            );
          }
          const nodeId = await resolveCliNodeId(opts, normalizeOptionalString(opts.node) ?? "");
