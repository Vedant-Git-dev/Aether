    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether nodes status", "List known nodes with live status."],
          ["aether nodes pending", "Show pending node pairing requests."],
          ["aether nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'aether nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["aether nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.aether.ai/cli/nodes")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `aether nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
