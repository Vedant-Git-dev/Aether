...
import { basenameFromAnyPath, extnameFromAnyPath } from "@aether/media-core/file-name";
import {
  detectMime,
  extensionForMime,
  getFileExtension,
...
type OutboundProvenanceDatabase = Pick<AetherStateKyselyDatabase, "outbound_media_provenance">;

async function getTrustedGeneratedHtmlMarker(
  resolvedFilePath: string,
): Promise<{ sha256: string; size: number } | undefined> {
  try {
    const { db } = openAetherStateDatabase();
...
async function resolveTrustedGeneratedHostReadHtml(
  ...
  const [resolvedFilePath, tmpRoot, outboundRoot] = await Promise.all([
    ...
    realpath(resolvePreferredAetherTmpDir()).catch(() => undefined),
...
export async function pruneStaleTrustedGeneratedHtmlMarkers(): Promise<void> {
  ...
  runAetherStateWriteTransaction(({ db: writeDb }) => {
