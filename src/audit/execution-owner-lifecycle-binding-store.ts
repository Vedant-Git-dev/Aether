...
function ensureExecutionOwnerLifecycleBindingSchema(db: DatabaseSync): void {
  ...
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(SCHEMA_START);
  const end = start < 0 ? -1 : AETHER_STATE_SCHEMA_SQL.indexOf(SCHEMA_END, start);
  if (start < 0 || end < start) {
    throw new Error("Aether execution owner lifecycle binding schema marker is missing.");
  }
  // sqlite-allow-raw -- Canonical feature-local additive DDL only; metadata rows use Kysely.
  db.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + SCHEMA_END.length));
}
