...
import {
  ...
  runWithAetherStateBusyTimeout,
} from "../state/aether-state-db.js";
import { isAetherStateWriteContentionError } from "../state/aether-state-ownership.js";
...
const AUDIT_WRITER_SHUTDOWN_TIMEOUT_MS = AETHER_SQLITE_BUSY_TIMEOUT_MS + 5_000;
...
export function createAuditEventWriter(
  ...
  const database = {
    env: { ...process.env, AETHER_STATE_DIR: options.stateDir ?? resolveStateDir(process.env) },
  };
  ...
  const runWithoutBusyWait = <T>(operation: () => T): T =>
    runWithAetherStateBusyTimeout(() => operation(), database, 0);
  ...
  const reportMaintenance = (): AuditMaintenanceAttempt => {
    ...
    for (const maintenance of [
      ...
      try {
        ...
        if (isAetherStateWriteContentionError(error)) {
