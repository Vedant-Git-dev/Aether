...
type OwnerLifecycleDatabase = {
  cron_run_receipts: WithSqliteRowId<AetherStateDatabase["cron_run_receipts"]>;
  execution_owner_lifecycle_bindings: AetherStateDatabase["execution_owner_lifecycle_bindings"];
  flow_runs: WithSqliteRowId<AetherStateDatabase["flow_runs"]>;
  task_runs: WithSqliteRowId<AetherStateDatabase["task_runs"]>;
};
...
export function pageOwnerLifecycleReceipts(params: {
  ...
  const retainedRows = withExistingAetherStateDatabaseReadOnly(
    ({ db }) =>
