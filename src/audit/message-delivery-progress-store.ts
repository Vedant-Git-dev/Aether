...
import { ensureColumn, tableExists } from "../state/aether-state-db-schema-helpers.js";
...
type ProgressTable = AetherStateKyselyDatabase["outbound_message_progress"];
type ProgressDatabase = Pick<AetherStateKyselyDatabase, "outbound_message_progress">;
...
function progressSchemaSql(): string {
  ...
  const finalIndex = AETHER_STATE_SCHEMA_SQL.indexOf(
    "CREATE INDEX IF NOT EXISTS outbound_message_progress_run_occurred_idx",
    start,
  );
  const end = finalIndex >= 0 ? AETHER_STATE_SCHEMA_SQL.indexOf(";", finalIndex) : -1;
  if (start < 0 || end < 0) {
    throw new Error("canonical outbound message progress schema is missing");
  }
  return AETHER_STATE_SCHEMA_SQL.slice(start, end + 1);
}
...
function ensureProgressSchema(options: AetherStateDatabaseOptions): void {
...
export function pruneExpiredOutboundMessageProgress(
  params: { now?: number; database?: AetherStateDatabaseOptions } = {},
): number {
  const database = openAetherStateDatabase(params.database);
