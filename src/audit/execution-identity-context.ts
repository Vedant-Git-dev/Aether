...
>;
type ExecutionIdentityRow = Selectable<AetherStateKyselyDatabase["execution_identity_contexts"]>;
...
const ensureExecutionIdentityContextSchema = createAetherStateSchemaEnsurer({
...
type ExecutionIdentityStoreOptions = AetherStateDatabaseOptions & {
  now?: number;
  limits?: {
    maxRows: number;
...
type ExecutionIdentityReadOptions = AetherStateDatabaseOptions & {
  now?: number;
};
...
function persistExecutionIdentityAdmissionEnvelope(
  ...
  const opened = openAetherStateDatabase(options);
...
function verifyExecutionIdentityAdmissionRetry(
  ...
  const { db } = openAetherStateDatabase(options);
...
function readExecutionIdentityContextByExecutionId(
  ...
  return (
    withExistingAetherStateDatabaseReadOnly(({ db }) => {
      if (!tableExists(db, "execution_identity_contexts")) {
      ...
      remediation: [
        {
          ...
          text: "Run aether vitals and inspect the shared state database before trusting this execution.",
        },
      ],
    });
  }
...
function inspectRunSelector(
  ...
  const inspected = withExistingAetherStateDatabaseReadOnly<
      ...
      try {
        ...
        return unavailableResult({
          ...
          remediation: [
            {
              code: "inspect_state_integrity",
              text: "Run aether vitals and inspect the shared state database before trusting this run.",
            },
          ],
        });
      }
          ...
          remediation: [
            {
              ...
              text: "Select one candidate with aether audit --execution <id> --explain.",
            },
          ],
        },
        decisions: [],
    ...
    try {
      ...
      return unavailableResult({
        ...
        remediation: [
          {
            code: "inspect_state_integrity",
            text: "Run aether vitals and retry the run inspection.",
          },
        ],
      });
    }
