...
import { normalizeOptionalString as readTtsResultString } from "@aether/normalization-core/string-coerce";
import type { AetherConfig, ResolvedTtsPersona, TtsProvider } from "../config/types.js";
...
): { cfg: AetherConfig; config: ResolvedTtsConfig; prefsPath: string } | { error: string } {
  ...
  try {
    const setup = await queries.run(async () => {
      ...
      const prepareProviderRegistry = async (): Promise<TtsProviderRegistry> => {
        ...
        const selectRegistry = (queryConfig: AetherConfig | undefined) => {
