...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: AetherConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: AetherConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildAliasIndex = (cfg?: AetherConfig) =>
    buildCapabilityProviderIndex(resolver.listProviders(cfg), "aliases");

  const listProviders = (cfg?: AetherConfig): SpeechProviderPlugin[] => [
