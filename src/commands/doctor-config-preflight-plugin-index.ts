...
function throwPluginRegistryPersistenceFailed(
  reason: string,
  repair = 'Run "aether vitals --fix" and retry.',
): never {
  throw new Error(
    `Aether refreshed the plugin registry but could not verify the persisted replacement (${reason}); refusing to write the migration checkpoint. ${repair}`,
  );
}
...
export async function persistRefreshedPluginIndex(params: {
  ...
  if (persistedPluginMetadataSnapshot?.registrySource !== "persisted") {
    ...
    throwPluginRegistryPersistenceFailed(
      ...
      'Stop plugin package changes, run "aether plugins registry --refresh", then retry.',
    );
  }
