...
function isTempBackedAetherRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const aetherIndex = lower.lastIndexOf("aether");
  if (aetherIndex < 1) {
    return false;
  }
  return lower[aetherIndex - 1] === "tmp" || lower[aetherIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("aether@"),
    ) || isTempBackedAetherRoot(beforeSkillRoot)
  );
}
...
function resolveExpectedBundledSkillPath(params: {
  ...
  // Snapshot paths use shell `~` semantics. AETHER_HOME may point at an isolated
...
export function sessionSnapshotIssueToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "To clean up the advisory artifact, run `aether vitals --fix` to rewrite stale cached session metadata paths, or start a fresh session after confirming history can be retired.",
  };
}
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("aether.doctorSessionSnapshotsTestApi")
  ] = {
