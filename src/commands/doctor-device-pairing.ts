...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["aether", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["aether", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["aether", "devices", "remove", pending.deviceId]),
    };
  }
...
async function collectLegacyPairingStoreFindings(cfg: AetherConfig): Promise<HealthFinding[]> {
...
export async function collectDevicePairingHealthFindings(params: {
  ...
  if (deviceAuth.sourcePresent) {
    const fixCommand = formatCliCommand("aether vitals --fix", params.env);
