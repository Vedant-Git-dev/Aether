...
function renderBlockingSystemGatewayServices(services: ExtraGatewayService[]): string {
  return [
    "System-level Aether gateway service detected while the user gateway service is not installed.",
    ...services.map((svc) => `- ${svc.label} (${svc.detail})`),
    "Aether will not install a second user-level gateway service automatically.",
    "Run `aether gateway status --deep` or `aether vitals --deep` to inspect duplicate services.",
...
function renderEstablishedGatewayConnections(connections: PortConnection[]): string {
  return [
    ...
    "If logs show protocol mismatch after rollback, stop stale Aether client processes listed here and rerun doctor.",
  ].join("\n");
}
...
async function noteGatewayPortDiagnostics(cfg: AetherConfig, deep: boolean): Promise<boolean> {
...
async function noteGatewayServiceInspectionFailure(
  ...
  lines.push(`Run ${formatCliCommand("aether gateway status --deep")} and retry doctor.`);
  note(lines.join("\n"), "Gateway");
}
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (!loaded) {
    ...
    if (!install) {
      note(
        `Run ${formatCliCommand("aether gateway install")} when you want to install the gateway service.`,
        "Gateway",
      );
    }
    if (install) {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.AETHER_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("aether gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
