...
export async function triageCommand(
  ...
  // Packaged Aether/Bun hosts cannot interpret npm shim entrypoints. Reuse the
  ...
  try {
    ...
    const file = path.join(outputDir, `aether-triage-prompt-${now}-${process.pid}.md`);
  ...
  const agentLabel = runEmbedded
    ? "the embedded Aether agent using your configured model"
    : handoff?.agent;
    ...
    try {
      if (automatic) {
        ...
        const result = await runUtf8CommandWithTimeout(
          ...
          {
            input: prompt,
            env: { ...targetEnv, AETHER_SHELL: "exec" },
            ...agentOptions,
  ...
  const installRoot = await resolveAetherPackageRoot({
  ...
  if (!installRoot) {
    throw new Error("Cannot locate the Aether installation; use a suggested handoff command.");
  }
  const failedResult =
  ...
  const result = await runUpdateRepairLoop({
    ...
    validate: async (signal): Promise<UpdateRepairValidation> => {
      try {
        ...
        if (!entrypoint) {
          throw new Error("The installed Aether entrypoint is unavailable.");
        }
  ...
  if (result.status === "unavailable") {
    if (result.reason === "exec-denied-by-policy") {
      throw new Error(
        "The operator's policy denies unattended repair (exec-denied-by-policy). Use `aether triage` for an external handoff.",
      );
    }
    throw new Error(
      `Embedded agent unavailable: ${result.reason}. Run \`aether awaken\` or use a suggested handoff command.`,
    );
  }
