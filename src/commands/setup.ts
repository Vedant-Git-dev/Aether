...
export async function setupCommand(
  ...
  if (snapshot.exists && !snapshot.valid) {
    if (opts?.json) {
      ...
      writeRuntimeJson(runtime, {
        ...formatCliJsonFailure(`Aether config is invalid: ${shortenHomePath(configPath)}`),
        issues: normalizeConfigIssues(snapshot.issues),
      });
    }
    const formatConfigFilePath = deps.formatConfigFilePath ?? formatDefaultConfigPath;
    runtime.error(
      `Config invalid at ${await formatConfigFilePath(configPath)}. Run \`${formatCliCommand("aether vitals")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  const cfg = shouldPersistRoster
    ? (migratePersistedImplicitMainRoster(snapshot.sourceConfig).config as AetherConfig)
  ...
  let next: AetherConfig = snapshot.exists ? resolvedConfig : cfg;
  ...
  runtime.log(`Next guided path: ${formatCliCommand("aether awaken")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("aether tune")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("aether channels add")}.`);
}
