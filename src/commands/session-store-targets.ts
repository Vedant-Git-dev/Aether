...
export function resolveExplicitSessionStorePath(params: {
  ...
  try {
    ...
    if (
      ...
      throw new Error("the SQLite file has application tables but no Aether schema metadata");
    }
  } catch (error) {
  ...
  if (databaseFailure) {
    throw new Error(
      `Session store target is not a session store: ${displayTarget}. ${formatErrorMessage(databaseFailure.error)}. Pass a legacy store selector or SQLite target reported by aether sessions or aether status.`,
    );
  }
  return storePath;
}
