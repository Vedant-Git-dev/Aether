...
function listTuiLastSessionStateRows(database: DatabaseSync) {
  const db =
    getNodeSqliteKysely<Pick<AetherStateKyselyDatabase, "config_machine_state">>(database);
...
export function readRepairJournal(database: DatabaseSync): ReservedKeyRename[] {
  const db = getNodeSqliteKysely<Pick<AetherStateKyselyDatabase, "state_leases">>(database);
...
export function readRepairJournalReadOnly(env: NodeJS.ProcessEnv): ReservedKeyRename[] {
  return (
    withExistingAetherStateDatabaseReadOnly((database) => readRepairJournal(database.db), {
      env,
    }) ?? []
  );
}
