...
import { decodeMountInfoPath } from "@aether/normalization-core/mountinfo-path";
import { asNullableObjectRecord } from "@aether/normalization-core/record-coerce";
...
function listOrphanAgentDirs(cfg: AetherConfig, stateDir: string): OrphanAgentDir[] {
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for AETHER_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move AETHER_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // AETHER_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.AETHER_OAUTH_DIR?.trim()) {
    return true;
  }
  ...
  switch (issue.kind) {
    case "mac-cloud-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Move AETHER_STATE_DIR to local non-synced storage such as ~/.aether.",
      };
    case "linux-sd-state-dir":
      return {
        ...
        fixHint: "Move AETHER_STATE_DIR to SSD/NVMe-backed storage.",
      };
    case "linux-volatile-state-dir":
      return {
        ...
        path: issue.path,
        target: issue.mountPoint,
        fixHint: "Move AETHER_STATE_DIR to persistent local storage.",
      };
    case "missing-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to create the state directory.",
      };
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to repair runtime state directory permissions.",
      };
  }
...
export async function noteStateIntegrity(
  ...
  const stateDir = resolveStateDir(env, homedir);
  const defaultStateDir = path.join(homedir(), ".aether");
  const oauthDir = resolveOAuthDir(env, stateDir);
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.aether).",
        `  Set locally: AETHER_STATE_DIR=~/.aether ${formatCliCommand("aether vitals")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
      ...
      if (wedgedSubagentSessions.length > 0) {
        ...
        warnings.push(
          [
            ...
            "  Aether will not auto-resume these child sessions on restart; reconcile their task records instead.",
            `  Examples: ${wedgedSubagentSessions
              .slice(0, 3)
              .map(({ key }) => key)
              .join(", ")}`,
            `  Fix: ${formatCliCommand("aether tasks maintenance --apply")}`,
          ].join("\n"),
        );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return "- Tip: back up the agent workspace in a private git repo; keep ~/.aether out of git (credentials, sessions). Details: /concepts/agent-workspace#git-backup-recommended-private";
}
