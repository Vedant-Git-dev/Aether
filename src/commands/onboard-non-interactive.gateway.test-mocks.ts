...
vi.mock("../plugins/plugin-lifecycle-lease.js", () => ({
  withPluginLifecycleLease: async (
    ...
    try {
      return await run({
        databasePath: path.join(path.dirname(resolveTestConfigPath()), "aether.sqlite"),
...
vi.mock("../config/config.js", async (importActual) => {
  ...
  return {
    ...
    resolveGatewayPort: (cfg: AetherConfig) => cfg.gateway?.port ?? 18789,
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/aether-workspace",
