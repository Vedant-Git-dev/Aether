...
import { readNonBlankString, readStringValue } from "@aether/normalization-core/string-coerce";
...
export async function exportTrajectoryCommand(
  ...
  if (!sessionKey) {
    throwTrajectoryExportError(
      `--session-key is required. Run ${formatCliCommand("aether sessions")} to choose a session.`,
    );
  }
  ...
  if (!entry?.sessionId) {
    throwTrajectoryExportError(
      `Session not found: ${sessionKey}. Run ${formatCliCommand("aether sessions")} to see available sessions.`,
    );
  }
