...
export function withoutProviderModelPolicy(
  patch: Partial<AetherConfig>,
  config: AetherConfig,
): Partial<AetherConfig> {
...
function snapshotPolicy(config: AetherConfig, agentId: string) {
...
export async function completeProviderModelAccess(params: {
  ...
  const message = application.claimed
    ...
    : "Model access saved. Application by the running Gateway is not confirmed. Run `aether gateway restart` to apply it.";
