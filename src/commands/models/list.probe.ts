...
import { disposeAetherAgentDatabaseByPath } from "../../state/aether-agent-db.js";
...
function resolveProbeSecretRef(profile: ProfileEntry, cfg: AetherConfig) {
...
function withoutProfileFallback(cfg: AetherConfig, provider: string): AetherConfig {
  const next: AetherConfig = {
    ...cfg,
    auth: {
      ...cfg.auth,
...
async function probeTarget(params: {
  ...
  try {
    ...
    if (target.boundValue || target.useRuntimeAuth) {
      ...
      // disposeAetherAgentDatabaseByPath's exact-path guard would otherwise
      // skip the registry row, leaking an agent_databases entry per probe.
      isolatedAgentDir = await fs.realpath(
        await fs.mkdtemp(path.join(os.tmpdir(), "aether-auth-probe-")),
      );
    }
    ...
    await work.settle(async () => {
      ...
      if (isolatedAgentDir) {
        ...
        cleanups.push(
          ...
          () => {
            disposeAetherAgentDatabaseByPath(resolveAuthProfileDatabasePath(ownedDir));
          },
...
function formatActiveGatewayModelsProbeRefusal(identity: GatewayLockIdentity): string {
  return `A Gateway is running for this state directory (pid ${identity.pid}, port ${identity.port}). Stop the Gateway first (${formatCliCommand("aether gateway stop")}), then rerun models status --probe.`;
}
