...
async function connectAccount(
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        "Could not confirm sign-in cancellation. The connection is closing; run `aether models accounts list` to check whether an account was saved.",
        { cause: cancelError },
      );
    }
    if (
    ...
    throw new Error("Sign-in did not complete. Re-run `aether models accounts login`.", {
      cause: error,
    });
  } finally {
    await retirePrompt();
...
export async function modelsAccountsListCommand(
  ...
  await withModelsAccountsGateway(options, "read", runtime, async ({ client, signal }) => {
    ...
    if (result.accounts.length === 0) {
      runtime.log("No saved accounts on this page. Use `aether models accounts login`.");
    }
    ...
    if (result.nextCursor) {
      runtime.log(
        `Next page: aether models accounts list --cursor ${sanitizeTerminalText(result.nextCursor)}`,
      );
    }
  });
}
