...
export async function refreshRunningGatewayAuthState(
  ...
  runtime.error(
    localTarget === true
      ? `Warning: Model auth changes were saved, but the ${gatewayConnected ? "running" : "local"} Gateway could not refresh them. Run \`aether gateway restart\` to apply the saved changes.`
