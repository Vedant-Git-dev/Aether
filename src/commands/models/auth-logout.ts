...
function restoreCredentialConfigMutation(params: {
  current: AetherConfig;
  before: AetherConfig;
  after: AetherConfig;
}): AetherConfig {
  // SAFETY: Restoration selects or rebuilds fields from three validated AetherConfig values.
  return restoreConfigMutationValue(params.current, params.before, params.after) as AetherConfig;
}

function restoreSurvivingProfileOrder(params: {
  desired: AetherConfig;
  before: AetherConfig;
  after: AetherConfig;
...
export async function modelsAuthLogoutCommand(
  ...
  if (!profileId) {
    throw new Error(
      `Missing profile id. Run ${formatCliCommand("aether models auth list")} to see saved profile ids.`,
    );
  }
  ...
  if (!credential) {
    throw new Error(
      `Auth profile "${profileId}" not found for agent "${agentId}". Run ${formatCliCommand(`aether models auth list --agent ${agentId}`)} to see saved profile ids.`,
    );
  }
  ...
  if (remaining.length === 0) {
    runtime.log(
      `No auth profiles remain for ${credential.provider}. Run ${formatCliCommand(`aether models auth login --provider ${credential.provider}`)} to sign in again.`,
    );
  }
}
