...
import { readByteStreamWithLimit } from "@aether/media-core/read-byte-stream-with-limit";
import { expectDefined } from "@aether/normalization-core";
import { resolveExpiresAtMsFromDurationMs } from "@aether/normalization-core/number-coercion";
import {
...
async function resolveModelsAuthAgent(rawAgentId?: string | null, config?: AetherConfig) {
...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("aether plugins list --json")}\`.`,
  );
}
  ...
  try {
    for (const candidate of profiles) {
      const persisted = await persistProviderAuthProfilesAfterLogin({
        ...
        ...(params.env?.AETHER_STATE_DIR ? { stateDir: params.env.AETHER_STATE_DIR } : {}),
      });
    ...
    if (defaultModel) {
      params.runtime.log(
        params.setDefault
          ...
          : `Default model available: ${defaultModel} (current default unchanged; run ${formatCliCommand(`aether models set ${defaultModel}`)} to apply)`,
      );
    }
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (!process.stdin.isTTY) {
    throw new Error(
      `setup-token requires an interactive TTY. In automation, use ${formatCliCommand("aether models auth paste-token --provider <provider>")} instead.`,
    );
  }
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("aether plugins install")}\`.`,
    );
  }
  ...
  if (!provider) {
    throw new Error(
      `No token-capable provider is available. Run ${formatCliCommand("aether plugins list")} to verify provider plugins are installed.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (!rawProvider) {
    throw new Error(
      `Missing --provider. Run ${formatCliCommand("aether models status")} or ${formatCliCommand("aether plugins list")} to choose a provider.`,
    );
  }
  ...
  const validateTokenInput = (value: string | undefined): string | undefined => {
    ...
    if (isOpenAIProvider(provider) && looksLikeOpenAIApiKey(trimmed)) {
      return `That looks like an OpenAI API key. Use ${formatCliCommand("aether models auth paste-api-key --provider openai")} for API-key auth.`;
    }
    return undefined;
  };
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in Aether.");
    runtime.log("Aether prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this Aether path is allowed again.");
  }
}
...
export async function modelsAuthAddCommand(opts: { agent?: string }, runtime: RuntimeEnv) {
  ...
  if (providerPlugin) {
    ...
    if (methodId !== "paste") {
      ...
      if (!method) {
        throw new Error(
          `Unknown token auth method "${methodId}". Run ${formatCliCommand("aether models auth login --provider " + providerPlugin.id)} to choose interactively.`,
        );
      }
      await runProviderAuthMethod({
        config,
...
function maybeLogOpenAICodexNativeSearchTip(runtime: RuntimeEnv, providerId: string) {
  ...
  runtime.log(
    `Tip: Codex-capable models can use native Codex web search. Configure the \`web_search\` tool with \`${formatCliCommand("aether tune --section web")}\`. Docs: https://docs.aether.ai/tools/web`,
  );
}
...
export async function runModelsAuthLoginFlowCore(
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("aether plugins install")}\`.`,
    );
  }
  if (useProviderPicker) {
    await prompter.note(
      `Provider "${requestedProviderId}" uses its own CLI login. Select a provider with an Aether auth flow.`,
      "Provider auth",
    );
  ...
  await prompter.note(
    [
      ...
      "Location: the machine running Aether",
      `For personal model accounts on a Gateway, run ${formatCliCommand("aether models accounts login --help")}.`,
    ].join("\n"),
    "Provider sign-in",
  );
  ...
  if (!selectedProvider) {
    throw new Error(
      `Unknown provider. Run ${formatCliCommand("aether models status")} or ${formatCliCommand("aether plugins list")} to see available provider plugins.`,
    );
  }
  ...
  if (!chosenMethod) {
    throw new Error(
      `Unknown auth method. Run ${formatCliCommand("aether models auth login --provider " + selectedProvider.id)} without --method to choose interactively.`,
    );
  }
  ...
  if (opts.force) {
    ...
    try {
      ...
      if (!clearedStore) {
        throw new Error(
          "auth store is busy; close other Aether commands using this state directory and retry",
        );
      }
      opts.runtime.log(
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  if (!process.stdin.isTTY) {
    throw new Error(
      `models auth login requires an interactive TTY. In automation, use ${formatCliCommand("aether models auth paste-token --provider <provider>")} when token auth is available.`,
    );
  }
