...
export async function withModelsAccountsGateway<T>(
  ...
  const client = new GatewayClient({
    ...
    clientDisplayName: "aether models accounts",
      ...
      if (error instanceof GatewayClientRequestError && error.gatewayCode === "FORBIDDEN") {
        throw new Error(
          [
            ...
            "For proxy client sign-in, see https://docs.aether.ai/gateway/remote#gateway-behind-an-identity-aware-proxy. Browser sign-in and device pairing alone do not identify this CLI; ask an administrator if access is denied.",
            "For shared or agent-local credentials instead, use `aether models auth login`.",
          ].join("\n"),
          { cause: error },
        );
      }
