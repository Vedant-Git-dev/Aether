...
function resolveModelInput(params: { raw: string; cfg: AetherConfig }) {
...
export function resolveModelTarget(params: { raw: string; cfg: AetherConfig }): {
  provider: string;
  model: string;
} {
...
function resolveKnownAgentId(cfg: AetherConfig, rawAgentId: string): string {
  const agentId = normalizeAgentId(rawAgentId);
  if (!listAgentIds(cfg).includes(agentId)) {
    throw new Error(
      `Unknown agent id "${rawAgentId}". Use "${formatCliCommand("aether agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
...
export async function updateDefaultModelPrimaryConfig(params: {
  modelRaw: string;
  field: "model" | "imageModel";
}): Promise<{ updated: AetherConfig; warning?: string }> {
