...
function getFallbacks(cfg: AetherConfig, key: DefaultsFallbackKey): string[] {
  ...
  const updated = await updateConfig(
    (cfg, { runtimeConfig }) => {
      ...
      if (filtered.length === existing.length) {
        throw new Error(
          `${params.notFoundLabel} not found: ${targetKey}. Run ${formatCliCommand(`aether ${listCommandForFallbackKey(params.key)}`)} to see configured fallbacks.`,
        );
      }
