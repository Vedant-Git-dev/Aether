/** Implementation of `aether models status`. */
import path from "node:path";
import { stripSelfProviderModelPrefix } from "@aether/model-catalog-core/provider-model-id-normalization";
import {
...
function resolveEnvAgentDirOverride(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.AETHER_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
