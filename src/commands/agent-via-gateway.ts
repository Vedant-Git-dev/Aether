...
type AgentDispatchOpts = Omit<AgentCliOpts, "messageFile"> & {
  message: string;
  gatewayDispatchConfig?: AetherConfig;
...
function resolveImplicitCliAgentId(cfg: AetherConfig, remote?: RemoteGatewayRoster): string {
  const migratedConfig = remote
    ? cfg
    : (migratePersistedImplicitMainRoster(cfg).config as AetherConfig);
...
const replyPayloadModuleLoader = createLazyPromiseLoader(
  () => import("aether/plugin-sdk/reply-payload"),
  { cacheRejections: true },
);
...
async function loadRuntimeConfig(): Promise<AetherConfig> {
  const { getRuntimeConfig } = await runtimeConfigModuleLoader.load();
  return getRuntimeConfig();
}

function usesRemoteGateway(cfg: AetherConfig): boolean {
  return Boolean(
    cfg.gateway?.mode === "remote" || normalizeOptionalString(process.env.AETHER_GATEWAY_URL),
  );
}

async function loadRemoteGatewayRoster(cfg: AetherConfig): Promise<RemoteGatewayRoster> {
...
async function loadRemoteGatewayRosterWithShellEnvFallback(
  cfg: AetherConfig,
): Promise<{ config: AetherConfig; roster: RemoteGatewayRoster }> {
  try {
...
function formatActiveGatewayLocalRefusal(identity: GatewayLockIdentity): string {
  return `A Gateway is running for this state directory (pid ${identity.pid}, port ${identity.port}). Run without --local to use it, or stop the Gateway first (${formatCliCommand("aether gateway stop")}).`;
}
...
function missingAgentMessageError(): Error {
  return new Error(
    `Missing message. Use ${formatCliCommand('aether agent --message "..." --agent <id>')} or ${formatCliCommand("aether agent --message-file <path> --agent <id>")}.`,
  );
}
...
function parseTimeoutSeconds(opts: { cfg: AetherConfig; timeout?: string }) {
  const raw =
...
function formatGatewayAgentTransportLossHint(err: unknown): string | undefined {
  ...
  return (
    ...
    "Check `aether gateway status` and the session transcript before retrying or rerunning with --local, so the turn does not execute twice."
  );
}
...
async function normalizeSessionKeyOptsForDispatch(
  ...
  let selectionCfg: AetherConfig | undefined;
...
async function agentViaGatewayCommand(
  ...
  let cfg: AetherConfig = opts.gatewayDispatchConfig ?? readGatewayDispatchConfig();
  ...
  if (
    ...
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("aether agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (knownAgents && !knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("aether agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  const dispatchGatewayAgentCall = async (activeCfg: AetherConfig) =>
    await withProgress(
      {
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(messageOpts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: aether sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
