...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(aether completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `aether vitals --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`aether completion --write-state\` or \`aether vitals --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
