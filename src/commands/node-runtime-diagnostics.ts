  ...
  return {
    ...
    fixHint: [
      ...
      ...(source === "gateway-service"
        ? [
            "After switching Node, refresh a managed Gateway with `aether gateway install --force`; for an externally managed service, have its deployment owner update the launcher.",
          ]
        : []),
    ].join("\n"),
  };
...
async function collectServiceNodeRuntimeFindings(
  ...
  try {
    ...
    findings.push({
      ...
      fixHint: "Run `aether gateway status --deep` and check access to its recorded executable.",
    });
  }
  return findings;
}
