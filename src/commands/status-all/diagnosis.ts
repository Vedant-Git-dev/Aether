// Appends the read-only diagnosis section for `aether status --all`.
...
export async function appendStatusAllDiagnosis(params: {
  ...
  if (params.portUsage) {
    ...
    if (!portOk) {
      ...
      if (gatewayPidCount > 1) {
        lines.push(
          `  ${muted(`${gatewayPidCount} Aether gateway processes appear to be listening on port ${params.port}; stop stale gateway processes before trusting channel health.`)}`,
        );
      }
      ...
      lines.push(`  ${muted("Detected Aether Gateway listener on the configured port.")}`);
    }
  }
  ...
  if (!params.nodeOnlyGateway && params.exporterDiagnostics) {
    if (params.exporterDiagnostics.ok) {
      ...
      emitUnavailableDiagnostics({
        ...
        retry: "aether gateway stability --type telemetry.exporter",
      });
    }
  }
  ...
  if (!params.nodeOnlyGateway && params.deliveryDiagnostics?.ok) {
    if (isDeliveryDiagnosticsLike(params.deliveryDiagnostics.value)) {
      ...
      emitUnavailableDiagnostics({
        ...
        retry: "aether gateway stability",
      });
    }
  } else if (
  ...
  lines.push("Troubleshooting: https://docs.aether.ai/troubleshooting");
  lines.push("");
}
