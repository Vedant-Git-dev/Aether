...
function formatStatusServiceValue(params: StatusManagedService): string {
  ...
  const installedPrefix = params.managedByAether ? "installed · " : "";
...
export function buildStatusOverviewSurfaceRows(params: {
  cfg: Pick<AetherConfig, "update" | "gateway" | "telemetry">;
  update: StatusUpdateLike;
  tailscaleMode: string;
...
/** Builds the stable gateway object used by `aether status --json`. */
