...
type BrowserHatchHandoffDeps = {
  ...
  resolveTarget?: (config: AetherConfig, env: NodeJS.ProcessEnv) => Promise<BrowserHatchTarget>;
  probePresence?: (
...
async function probeDashboardPresence(
  ...
  try {
    // Read presence over the same trusted local CLI path every `aether`
    ...
    const presence = await callGateway<SystemPresence[]>({
      ...
      timeoutMs,
      // Connect as a CLI-mode loopback client (what every `aether` command
