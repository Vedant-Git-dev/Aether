// Builds overview table rows for `aether status` and `aether status --all`.
...
/** Builds the default `aether status` overview rows from scan, health, memory, and session inputs. */
export function buildStatusCommandOverviewRows(
  ...
  const updatesDisabled =
    ...
    isTruthyEnvValue(params.env.AETHER_NO_AUTO_UPDATE) ||
...
export function buildStatusAllOverviewRows(params: {
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRows ?? []),
      { Item: "Security", Value: `Run: ${formatCliCommand("aether security audit --deep")}` },
