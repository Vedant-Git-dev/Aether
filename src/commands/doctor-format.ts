...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.AETHER_PROFILE);
    hints.push(
      `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`,
    );
    hints.push(`Then reinstall: ${formatCliCommand("aether gateway install", env)}`);
  }
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("aether gateway install", env)}`);
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
    return hints;
  ...
  if (missingGuiSession || runtime.status === "stopped") {
    if (!missingGuiSession && platform === "linux" && isSystemdStartLimitHit(runtime)) {
      ...
      hints.push(
        ...
        `Recover with: ${formatCliCommand("aether gateway restart", env)}, then inspect logs if it keeps crashing.`,
      );
  ...
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    const unit =
      runtime.systemd?.unit ?? `${resolveGatewaySystemdServiceName(env.AETHER_PROFILE)}.service`;
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("aether gateway restart", env)}`,
      );
    }
  }
  return hints;
