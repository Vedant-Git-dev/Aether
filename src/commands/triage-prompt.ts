...
function renderTriageTail(bundle: TriageBundle, redaction: SupportRedactionContext): string[] {
  ...
  return [
    ...
    "## Privacy",
    "",
    "The diagnostics archive excludes secrets, tokens, raw chat payloads, and raw logs. Failed-update excerpts are sanitized and byte-bounded; local paths are relative to `~` or `$AETHER_STATE_DIR`.",
    "",
  ];
}
...
export function renderTriagePrompt(params: {
  ...
  const lines = [
    "You are repairing THIS machine's Aether installation. Diagnose the root cause, apply the repair autonomously within your existing permissions, and verify the result. Preserve configuration, history, and databases. Use local `aether vitals`, `aether vitals --fix`, `aether status --all`, and `aether logs` as needed. Product documentation: https://docs.aether.ai.",
    "",
    "## Environment",
    "",
    `- Aether: ${VERSION}`,
    `- Platform: ${process.platform}`,
    `- Node.js: ${process.versions.node} (the runtime executing Aether, which may differ from the shell default)`,
    "- Local shell commands inherit `AETHER_STATE_DIR`, `AETHER_CONFIG_PATH`, and `AETHER_WORKSPACE_DIR` for the diagnosed installation and its default workspace; expand archive references in that shell. In embedded triage, in-process config and session tools use temporary agent run state. The execution cwd is separate from the installation's default workspace. Do not substitute a remote or sandbox installation for this local target.",
  ];
  ...
  lines.push(
    ...
    "## Completion goal",
    "",
    "Diagnose and repair the original symptom using existing repair commands, including `aether vitals --fix` and, for unfinished updates, `aether update repair`. Respect installation ownership, locks, schema and capability approval refusals. If maintenance refuses to stop the Gateway from this fixing subtree, use read-only diagnosis or safe offline artifact repair and atomic restart, or report that an independent operator must run maintenance outside triage. Do not bypass the refusal.",
    failure?.gateway === "preserve"
      ...
      : "Only activate a Gateway intended to run. For managed recovery, use atomic `aether gateway restart` when needed, never stop then start: an explicit stop after native scope attachment cancels this recovery and its children. Preserve later operator stops and report cancellation or infeasibility instead of claiming recovery.",
    "For a running Gateway, verify this installation with `aether health --json` AND `aether status --all` or `aether gateway status --deep`. Verify the running version matches the expected version above when supplied, and reproduce the original symptom to confirm it is resolved.",
  ...
  if (omitted > 0) {
    lines.push(`${omitted} more findings omitted; run \`aether vitals\` for the full list.`);
  }
