// Builds the data model for the standard `aether status` text report.
...
export async function buildStatusCommandReportData(params: {
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        theme.muted(
          `Skipped in fast status. Full report: ${formatCliCommand("aether security audit")}`,
        ),
        theme.muted(`Deep probe: ${formatCliCommand("aether status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${formatCliCommand("aether tasks maintenance --apply")}`,
