...
function createDoctorCronService(storePath: string, cfg: AetherConfig): CronService {
  const noop = () => {};
...
function cadenceFinding(params: {
  ...
  return {
    ...
    fixHint: `Run ${formatCliCommand("aether vitals --fix")} to materialize heartbeat cadence in cron.`,
  };
}
    ...
    return [
      {
        ...
        fixHint: `Run ${formatCliCommand("aether vitals --fix")} after resolving the cron store error.`,
      },
    ];
  }
}
