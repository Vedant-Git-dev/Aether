...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.AETHER_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.AETHER_GATEWAY_TOKEN);
}
