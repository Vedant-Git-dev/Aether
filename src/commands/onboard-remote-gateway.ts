...
function bindGatewayConfig(target: RemoteGatewayInferenceTarget): AetherConfig {
  return {
    ...target.config,
    gateway: {
 ...
 * Gateway's Aether finish setup before handing off to its normal TUI.
...
export async function runRemoteGatewayInferenceOnboarding(
  ...
  const activate = async (
    ...
    try {
      result = await request<WizardStartResult>({
        method: "aether.setup.activate.start",
        params: {
          sessionId,
          kind: params.kind,
  ...
  await runGuidedOnboarding({}, runtime, {
    ...
    runSystemAgentChat: async () => {
      ...
      try {
        for (;;) {
          await prompter.note(reply.reply, "Aether");
          if (reply.action === "exit") {
            await prompter.outro("Aether setup finished.");
            return;
          }
        ...
        if (error instanceof WizardCancelledError) {
          await prompter.outro("Aether setup paused.");
          return;
        }
        throw error;
      }
