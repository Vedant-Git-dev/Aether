...
import { coerceErrorMessage as formatPackageReadFailure } from "@aether/normalization-core/error-coercion";
...
import {
  auditAetherPeerDependenciesInManagedNpmRoot,
  type AetherPeerLinkAuditIssue,
  reconcileRegisteredAetherHostLinks,
  relinkAetherPeerDependenciesInManagedNpmRoot,
...
type PluginHostLinkAudit = {
  peerLinkIssues: AetherPeerLinkAuditIssue[];
  packageReadFailures: PluginPackageReadFailure[];
  registeredPeerLinkIssues: AetherPeerLinkAuditIssue[];
...
export async function listPluginAetherHostLinkIssues(
  params: InstalledPluginIndexRecordStoreOptions,
  ...
  const registeredAudit = await reconcileRegisteredAetherHostLinks({
...
/** Relinks npm-owned plugin packages to the current Aether host package. */
export async function maybeRepairPluginAetherHostLinks(
  params: PluginHostLinkDoctorParams,
): Promise<boolean> {
  const npmRoots = resolveDoctorPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audit = await listPluginAetherHostLinkIssues(params);
    if (audit.peerLinkIssues.length > 0) {
      note(
        [
          "Managed npm Aether host peer links need repair:",
          ...audit.peerLinkIssues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("aether vitals --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
    ...
    if (audit.registeredPeerLinkIssues.length > 0) {
      note(
        [
          "Registered npm plugin Aether host links need repair:",
          ...audit.registeredPeerLinkIssues.map(
            (issue) => `- ${issue.packageName}: ${issue.reason}`,
          ),
          `Repair with ${formatCliCommand("aether vitals --fix")} to relink registered npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  const registeredRepair = await reconcileRegisteredAetherHostLinks({
    installRecords: await loadInstalledPluginIndexInstallRecords(params),
    extensionsDir: resolveRegisteredPluginExtensionsRoot(params),
    env: params.env,
    mode: "repair",
  ...
  if (repaired > 0) {
    note(
      `Repaired Aether host peer link(s) for ${repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  if (registeredRepair.repaired > 0) {
    note(
      `Repaired Aether host peer link(s) for ${registeredRepair.repaired} registered npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Aether host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
