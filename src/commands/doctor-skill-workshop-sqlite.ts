...
import { truncateWithMarker } from "@aether/normalization-core/utf16-slice";
...
export async function inspectLegacySkillWorkshopMigration(params: {
  ...
  try {
    if (database && tableExists(database.db, "skill_workshop_proposals")) {
      const kysely = getNodeSqliteKysely<Pick<AetherStateDatabase, "skill_workshop_proposals">>(
        database.db,
      );
...
async function relocateLegacyWorkshopTargets(
  ...
  const database = openAetherStateDatabase({ env });
  const kysely = getNodeSqliteKysely<
    Pick<AetherStateDatabase, "skill_workshop_proposals" | "skill_workshop_proposal_rollbacks">
  >(database.db);
