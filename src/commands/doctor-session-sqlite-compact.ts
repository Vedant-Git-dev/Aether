...
import {
  ...
  clearAetherAgentDatabaseOpenFailure,
  ensureAetherAgentDatabasePermissions,
  isAetherAgentDatabaseOpen,
...
export async function compactDoctorSessionSqliteTarget(
  ...
  const sqlitePath = resolveAetherAgentSqlitePath(databaseOptions);
  ...
  if (!stat.isFile()) {
    throw new Error(`Aether agent database is not a regular file: ${sqlitePath}`);
  }
  if (isAetherAgentDatabaseOpen(sqlitePath)) {
    throw new Error(
      `Aether agent database ${sqlitePath} is already open in this process. Stop Aether and retry.`,
    );
  }
  const requireQuarantineCleared = () => {
    if (!clearAetherAgentDatabaseOpenFailure(sqlitePath, { env: options.env })) {
      throw new Error(
        `Aether agent database ${sqlitePath} was repaired, but its persisted quarantine record could not be cleared. Rerun aether vitals --fix so the database is not refused again.`,
      );
    }
  };
  const compactTarget = () => {
    const compact = compactDoctorSqliteFile({
      ...
      afterSuccess: () => {
        ...
        ensureAetherAgentDatabasePermissions(sqlitePath, databaseOptions);
      },
      sqlitePath,
  ...
  return options.operation === "import-finalize"
    ? withAgentDatabaseMaintenanceLease({ env: databaseOptions.env }, async (maintenance) => {
        await migrateAetherAgentDatabaseForMaintenance(
