...
export const resolveAetherPackageRoot = defineMockFn(vi.fn().mockResolvedValue(null));
...
function createLegacyStateMigrationDetectionResult(params?: {
  ...
  return {
    ...
    sharedAuthStore: {
      sourcePath: "/tmp/state/agents/main/agent/aether-agent.sqlite",
      hasLegacy: false,
    },
    ...
    rescuePending: {
      sourcePaths: ["/tmp/state/crestodian/rescue-pending", "/tmp/state/aether/rescue-pending"],
      hasLegacy: false,
    },
    channelPairing: {
      sourceDir: "/tmp/oauth",
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadAetherPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/aether-root.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../infra/aether-root.js")>();
  return {
    ...actual,
    resolveAetherPackageRoot,
  };
});
...
vi.mock("./doctor-browser.js", () => ({
  detectLegacyAetherBrowserProfileResidue: vi.fn().mockResolvedValue(null),
  maybeArchiveLegacyAetherBrowserProfileResidue: vi.fn().mockResolvedValue({
    changes: [],
    warnings: [],
  }),
...
beforeEach(() => {
  ...
  resolveAetherPackageRoot.mockReset().mockResolvedValue(null);
  ...
  originalStateDir = process.env.AETHER_STATE_DIR;
  originalUpdateInProgress = process.env.AETHER_UPDATE_IN_PROGRESS;
  process.env.AETHER_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "aether-doctor-state-"));
  process.env.AETHER_STATE_DIR = tempStateDir;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    delete process.env.AETHER_STATE_DIR;
  } else {
    process.env.AETHER_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.AETHER_UPDATE_IN_PROGRESS;
  } else {
    process.env.AETHER_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
