...
export async function triageAfterFailure(
  ...
  const previousShell = process.env.AETHER_SHELL;
  process.env.AETHER_SHELL = "exec";
  ...
  try {
    await withConsoleLogsRoutedToStderr(async () => {
      const resolvedRoot =
        failure.installationRoot ?? (await resolveAetherPackageRoot({ argv1: process.argv[1] }));
      if (!resolvedRoot) {
        throw new Error("installed CLI root is unavailable; run aether triage manually");
      }
      ...
      const supervisor =
        failure.kind === "gateway-startup"
          ? detectRespawnSupervisor(process.env, process.platform, {
              includeLinuxAetherGatewayServiceMarker: true,
            })
          : null;
    ...
    runtime.error(
      `Automatic triage could not complete: ${reason}. Run \`aether triage\` manually.`,
    );
    if (managedStartup && !cancellation.aborted) {
      try {
        ...
        runtime.error(
          "Managed triage diagnostics could not complete; retain the original failure and run aether triage manually.",
        );
      }
    }
    if (failure.kind === "update" && failure.installationRoot && !cancellation.aborted) {
      ...
      const promptPath = path.join(
        outputDir,
        `aether-triage-failure-${Date.now()}-${process.pid}.md`,
      );
      try {
        ...
        runtime.error(
          `Saved failure diagnostics: ${promptPath}. Run aether triage manually after repairing the installed CLI.`,
        );
      } catch {
        runtime.error(
          "Failure diagnostics could not be saved; retain the original update error and run aether triage manually.",
        );
      }
    }
  } finally {
    ...
    if (previousShell === undefined) {
      delete process.env.AETHER_SHELL;
    } else {
      process.env.AETHER_SHELL = previousShell;
    }
  }
  runtime.error(
