...
import { parseStrictNonNegativeInteger } from "@aether/normalization-core/number-coercion";
...
function setAgentExecEnvironment(params: { stateDir: string; cwd: string }): () => void {
  const previousStateDir = process.env.AETHER_STATE_DIR;
  ...
  const previousConfigPath = process.env.AETHER_CONFIG_PATH;
  const previousWorkspaceDir = process.env.AETHER_WORKSPACE_DIR;
  process.env.AETHER_STATE_DIR = params.stateDir;
  delete process.env.AETHER_CONFIG_PATH;
  process.env.AETHER_WORKSPACE_DIR = params.cwd;
  return () => {
    if (previousStateDir === undefined) {
      delete process.env.AETHER_STATE_DIR;
    } else {
      process.env.AETHER_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.AETHER_CONFIG_PATH;
    } else {
      process.env.AETHER_CONFIG_PATH = previousConfigPath;
    }
    if (previousWorkspaceDir === undefined) {
      delete process.env.AETHER_WORKSPACE_DIR;
    } else {
      process.env.AETHER_WORKSPACE_DIR = previousWorkspaceDir;
    }
  };
}

function formatActiveGatewayExecRefusal(identity: GatewayLockIdentity): string {
  return `A Gateway is running for this state directory (pid ${identity.pid}, port ${identity.port}). Omit --state-dir to use isolated temporary state, or stop the Gateway first (${formatCliCommand("aether gateway stop")}).`;
}
...
export async function agentExecCommand(
  ...
  try {
    ...
    const stateDir = opts.stateDir
      ...
      : await fs.mkdtemp(path.join(os.tmpdir(), "aether-agent-exec-"));
    ...
    // AETHER_STATE_DIR moves so discovery and the installed-index DB agree.
    ...
    // AETHER_CONFIG_PATH would only feed this same snapshot, while writing
