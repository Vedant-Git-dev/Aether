...
import { resolveAetherReleaseCohortVersion } from "../infra/npm-registry-spec.js";
...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("aether tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("aether tasks flow cancel <flow-id>")}`,
    ]
...
function pluginVersionDriftToHealthFindings(
    ...
    return [
      {
        ...
        severity: "warning",
        message: `Active official plugins match post-restart Aether ${drift.gatewayVersion}, but the running Gateway is ${runningGatewayVersion}.`,
        path: "plugins",
        requirement: "plugin-version-gateway-restart",
        fixHint: formatCliCommand("aether gateway restart"),
      },
    ];
  }
  return drift.drifts.map((entry) => {
    ...
    return {
      checkId: WORKSPACE_STATUS_CHECK_ID,
      severity: "warning",
      message: `Plugin ${entry.pluginId} is ${entry.installedVersion}, but a Gateway restart will load Aether ${drift.gatewayVersion}.${runningGatewayVersion ? ` The running Gateway is ${runningGatewayVersion}.` : ""}${updateCommand ? "" : ` Repair target resolution failed: ${targetError}.`}`,
      path: `plugins.entries.${entry.pluginId}`,
      target: entry.pluginId,
      requirement: "plugin-version-drift",
      fixHint: updateCommand
        ? `${formatCliCommand(updateCommand)} && ${formatCliCommand("aether gateway restart")}`
        : `No install command generated; retry aether vitals after checking registry availability (${targetError}).`,
    };
  });
}
...
function isGatewayRestartPending(
  ...
  return Boolean(
    ...
    resolveAetherReleaseCohortVersion(runningGatewayVersion) !==
      resolveAetherReleaseCohortVersion(drift.gatewayVersion),
  );
}
  ...
  return [
    {
      ...
      fixHint:
        "Repair the Gateway service installation, then rerun aether vitals before restarting.",
    },
  ];
}
...
function taskFlowRecoveryToHealthFinding(finding: TaskFlowRecoveryFinding): HealthFinding {
  return {
    ...
    fixHint: [
      formatCliCommand(`aether tasks flow show ${finding.flowId}`),
      formatCliCommand(`aether tasks flow cancel ${finding.flowId}`),
    ].join(" or "),
  };
}
...
function notePluginVersionReadiness(readiness: PluginVersionRestartReadiness | undefined) {
  ...
  if (readiness.status === "unresolved") {
    const running = readiness.runningGatewayVersion
      ? `\nRunning Gateway: Aether ${readiness.runningGatewayVersion}`
      : "";
    note(
  ...
  if (drift.drifts.length === 0) {
    ...
    note(
      [
        `Running Gateway: Aether ${readiness.runningGatewayVersion}`,
        `Active official plugins match post-restart Aether ${drift.gatewayVersion}.`,
        `Fix: ${formatCliCommand("aether gateway restart")}.`,
      ].join("\n"),
      "Plugin restart readiness",
    );
    return;
  ...
  const lines = [
    ...
    } not on post-restart Aether ${drift.gatewayVersion}`,
    ...
    singleDrift && updateCommands.length === 1
      ? `Fix: ${updateCommands[0]} && ${formatCliCommand("aether gateway restart")}.`
      : updateCommands.length > 0
        ? [
            "Fix each drifted plugin:",
            ...updateCommands.map((command) => `- ${command}`),
            ...(unresolvedRepairs.length === 0
              ? [`Then run ${formatCliCommand("aether gateway restart")}.`]
              : []),
          ].join("\n")
        : null,
  ];
...
export function noteWorkspaceStatus(cfg: AetherConfig, options: NoteWorkspaceStatusOptions = {}) {
