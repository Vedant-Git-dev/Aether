...
import { replaceConfigFile, type AetherConfig } from "../config/config.js";
...
import { AETHER_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function isOperatorOwnedEnvironmentIssue(
  ...
  switch (issue.code) {
    ...
    case SERVICE_AUDIT_CODES.gatewayTokenDrift:
      return hasGatewayServiceEnvironmentOverride(command, ["AETHER_GATEWAY_TOKEN"], {
        environmentValueSources,
      });
    case SERVICE_AUDIT_CODES.gatewayPasswordEmbedded:
      return hasGatewayServiceEnvironmentOverride(command, ["AETHER_GATEWAY_PASSWORD"], {
        environmentValueSources,
      });
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "aether-gateway.service";
}
...
async function resolveSystemdServiceRewriteBlock(
  ...
  return `Gateway service ${unitName} is running; skipped command/entrypoint rewrites and leaving supervisor metadata unchanged. Stop the service first or use \`aether gateway install --force\` when you want to replace the active launcher.`;
}
...
export function extraGatewayServiceToHealthFinding(service: ExtraGatewayService): HealthFinding {
  return {
    ...
    fixHint:
      service.legacy === true
        ? "Run `aether vitals` interactively to review legacy gateway services and confirm supported cleanup."
...
export async function maybeRepairGatewayServiceConfig(
  ...
  const managedWrapperPath = managedDefinition.environment?.[AETHER_WRAPPER_ENV_KEY]?.trim();
  const serviceInstallEnv =
    managedWrapperPath && !Object.hasOwn(process.env, AETHER_WRAPPER_ENV_KEY)
      ? { ...process.env, [AETHER_WRAPPER_ENV_KEY]: managedWrapperPath }
      : process.env;
  const serviceWrapperPath = normalizeOptionalString(
    command.environment?.[AETHER_WRAPPER_ENV_KEY],
  );
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${AETHER_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  const sourceCheckoutWarning = serviceLayout?.entrypointSourceCheckout
    ? [
        ...
        "Run `aether gateway install --force` from the intended package install to replace the gateway service definition.",
      ].join("\n")
    : null;
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service AETHER_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `aether gateway install --force` when you want Aether to rewrite its managed unit; operator-owned drop-ins remain unchanged.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `aether gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return cfg;
  ...
  const installedWindowsTaskName =
    managedDefinition.environment?.AETHER_WINDOWS_TASK_NAME?.trim();
  const serviceRepairEnv =
    updateRepairWillRewriteWindowsTask && installedWindowsTaskName
      ? {
          ...serviceInstallEnv,
          AETHER_WINDOWS_TASK_NAME: installedWindowsTaskName,
        }
      : serviceInstallEnv;
  ...
  try {
    ...
    if (
      ...
      if (installedWindowsTaskName) {
        ...
        restartEnv.AETHER_WINDOWS_TASK_NAME = installedWindowsTaskName;
      }
...
export async function maybeResolveDuelingSystemdGatewayScopes(
  ...
  note(
    [
      "Both a user-scope and a system-scope Aether gateway unit are installed:",
      `- user:   ${user.unitPath}`,
