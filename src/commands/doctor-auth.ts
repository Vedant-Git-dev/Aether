...
export function noteCopilotAmbientToken(cfg: AetherConfig, env = process.env): void {
  if (
  ...
  if (claimed) {
    note(
      "GitHub Copilot is no longer enabled by GH_TOKEN/GITHUB_TOKEN. To use Copilot, run `aether models auth login --provider github-copilot` or set COPILOT_GITHUB_TOKEN.",
      "GitHub Copilot",
    );
  }
}
...
export function noteSharedAuthStoreStatus(env: NodeJS.ProcessEnv = process.env): void {
  ...
  note(
    "Shared auth profiles still live in the main agent database. Run `aether vitals --fix` to move them into shared SQLite state and make the main agent deletable.",
    "Shared auth store",
  );
}

function hasConfiguredCodexOAuthProfile(cfg: AetherConfig): boolean {
...
export function noteLegacyCodexProviderOverride(cfg: AetherConfig): void {
...
function listAuthProfileHealthTargets(cfg: AetherConfig): AuthProfileHealthTarget[] {
...
async function resolveAuthIssueHint(
  ...
  if (issue.reasonCode === "malformed_api_key") {
    return "Paste the API key value, not an Aether onboarding command.";
  }
  ...
  return {
    ...
    fixHint:
      ...
      (params.issue.status === "expiring"
        ? "Run `aether vitals --fix` to refresh expiring OAuth profiles, or re-authenticate static tokens."
        : "Run `aether vitals --fix` to refresh OAuth profiles, or re-authenticate this provider."),
  };
}
