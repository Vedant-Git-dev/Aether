...
async function detectAetherGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
/** Offers to update Aether before doctor when running interactively from an updatable install. */
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.AETHER_UPDATE_IN_PROGRESS);
  ...
  const git = await detectAetherGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update Aether from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
    ...
    let candidateSchemaVersions: AetherSchemaVersions | undefined;
    ...
    if (externallyManaged) {
      ...
      try {
        ...
        if (activated !== "ok") {
          throw new Error(
            "Gateway restart was not verified; run `aether gateway status --deep` before restarting manually.",
          );
        }
        note("Restarted the running gateway service after updating Aether.", "Update");
      } catch (err) {
        restartSafe = false;
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("aether update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
