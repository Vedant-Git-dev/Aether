...
import { resolveAetherPackageRootsSync } from "../infra/aether-root.js";
...
function resolveSandboxScript(
  ...
  // Scan every aether package root the shared resolver finds (symlinked launcher via realpath,
  ...
  const roots = resolveAetherPackageRootsSync({
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.doctorSandboxTestApi")] = {
...
function codexBwrapNeedsNetworkNamespaceProbe(cfg: AetherConfig): boolean {
  const network = cfg.agents?.defaults?.sandbox?.docker?.network?.trim().toLowerCase();
  return network === undefined || network === "" || network === "none";
}

async function probeCodexBwrapNamespaces(cfg: AetherConfig): Promise<CodexBwrapNamespaceProbe> {
...
async function noteCodexBwrapNamespaceWarning(
  ...
  const lines = [
    ...
    "",
    "Fix the host namespace policy for the Aether service user, then restart the gateway.",
    "Prefer an AppArmor profile that grants the required namespaces to the Aether service process.",
...
function resolveSandboxDockerImage(cfg: AetherConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBackend(cfg: AetherConfig): string {
  const backend = cfg.agents?.defaults?.sandbox?.backend?.trim();
  return (backend || "docker").toLowerCase();
}

function resolveSandboxBrowserImage(cfg: AetherConfig): string {
    ...
    const lines =
      containerEngine.id === "docker"
        ? [
            ...
            "- Disable sandbox mode: aether config set agents.defaults.sandbox.mode off",
          ]
        : [
...
${formatLegacyRegistryInspectionLine(file)}`,
    path: file.path,
    fixHint: `Run ${formatCliCommand("aether vitals --fix")} to migrate valid entries to SQLite.`,
  };
}
  ...
  if (!prompter.shouldRepair) {
    note(
      [
        ...
        `Run ${formatCliCommand("aether vitals --fix")} to migrate them to SQLite.`,
      ].join("\n"),
      "Sandbox",
    );
    return;
...
export function noteSandboxScopeWarnings(cfg: AetherConfig) {
