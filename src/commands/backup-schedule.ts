...
const BACKUP_CRON_JOB_NAME = "aether-backup-scheduled";
const LOCAL_GATEWAY_REQUIRED_ERROR =
  "backup enable manages backups on the Gateway host and currently requires a local Gateway. Create the cron job manually with aether cron add for remote Gateways.";
...
export async function backupEnableCommand(
  ...
  if (options.push) {
    ...
    if (origin.code !== 0) {
      throw new Error(
        `--push requires an origin remote. Run: aether backup git init --repository ${shortenHomePath(repositoryPath)} --remote <url>`,
      );
    }
    if (!redactSecrets) {
