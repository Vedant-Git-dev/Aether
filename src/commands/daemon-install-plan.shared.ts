...
function isAetherCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "aether") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "aether.cmd" || basename === "aether.ps1" || basename === "aether.exe"
    );
  }
  return false;
}
...
/** Resolve the Aether CLI binary directory from argv/PATH for daemon PATH. */
function resolveDaemonAetherBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isAetherCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "aether.cmd" : "aether");
...
/** Merge runtime and Aether binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonAetherBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
