...
type RuntimePluginInstallDescriptor = {
  ...
  npmSpec: string;
  warningLabel: string;
  /** Keep this official runtime package on the same release cohort as Aether. */
  versionBoundToAether?: boolean;
};

type RuntimePluginInstallResult =
  | { ok: true; cfg: AetherConfig; required: boolean }
  | { ok: false; status: "skipped" | "failed" | "timed_out"; message: string };

type ModelSelectionRuntimePluginsResult =
  | { ok: true; cfg: AetherConfig; codexInstalled: boolean }
...
async function ensureRuntimePluginForModelSelection(
  ...
  const result = await ensureOnboardingPluginInstalled({
    ...
    entry: {
      ...
      ...(params.descriptor.versionBoundToAether ? { versionBoundToAether: true } : {}),
    },
