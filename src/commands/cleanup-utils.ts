...
import { acquireAetherStateDatabaseFileExclusion } from "../state/aether-state-db-cache.js";
...
/**
 * Allowed Trash roots for Aether-owned paths: each declared path's own parent, plus the
...
function collectWorkspaceDirs(cfg: AetherConfig | undefined): string[] {
...
async function acquireStateCleanupOwnership(cleanup: CleanupResolvedPaths) {
  const env = {
    ...process.env,
    AETHER_CONFIG_PATH: cleanup.configPath,
    AETHER_STATE_DIR: cleanup.stateDir,
  };
  ...
  try {
    lock = await acquireGatewayLock({
      ...
      // Shipped readers validate this role as any live Aether process. A new
    ...
    if (error instanceof GatewayLockError) {
      throw new Error(
        "Cannot remove Aether state while the Gateway or another state maintenance command owns this state directory. Stop the Gateway and retry.",
        { cause: error },
      );
    }
    throw error;
  ...
  if (!lock) {
    throw new Error("Cannot remove Aether state without exclusive state ownership.");
  }
  return lock;
}
...
async function detachStateLockDirectory(
  ...
  try {
    ...
    const message = `Failed to finalize Aether state cleanup because the lock directory changed: ${String(error)}`;
...
export async function removeStateAndLinkedPaths(
  ...
  let lockHeld = true;
  let stateCoordinator: ReturnType<typeof acquireAetherStateDatabaseFileExclusion> | undefined;
  ...
  try {
    ...
    if (!isPathWithin(lockDir, stateDir)) {
      throw new Error("Cannot remove Aether state because its active lock is outside state.");
    }
    const databasePath = resolveAetherStateSqlitePath({
      ...process.env,
      AETHER_STATE_DIR: stateDir,
    });
    stateCoordinator = acquireAetherStateDatabaseFileExclusion(databasePath);
    ...
    if (overlappingPreservePath) {
      throw new Error(
        `Cannot remove Aether state while preserving ${shortenHomeInString(overlappingPreservePath)} because it overlaps the active state lock. Move the workspace outside the lock directory and retry.`,
      );
    }
    ...
    if (!stateRemoval.ok) {
      throw new Error("Failed to remove non-preserved Aether state while ownership was held.");
    }
    ...
    if (newStateOperationStarted) {
      throw new Error(
        "Aether state cleanup was interrupted by a new state operation. Stop other Aether commands and retry.",
      );
    }
    if (stateDirRemoved) {
