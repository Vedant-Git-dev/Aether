...
function result(config: AetherConfig, warnings: string[] = []): DoctorTailscaleMigrationResult {
...
export async function prepareTailscaleConfigMigration(params: {
  ...
  if (managed) {
    return result(
      ...
      inspection.urls.some((url) => !new URL(url).port)
        ? [
            "The predecessor Tailscale route will be adopted from a previous Aether release when the Gateway starts.",
          ]
        : [],
    );
  }
  ...
  return result(config, [
    `Legacy Tailscale Serve still targets Gateway port ${gatewayPort}, but Doctor cannot prove that Aether owns the existing route; configuration was not changed. If you confirm the route belongs to the current Tailscale hostname and is stale from an older Aether release, remove only its root handler with ${cleanup}, then configure gateway.bind="loopback" and gateway.tailscale.mode="serve" manually and restart the Gateway. If another service owns the route, leave managed Tailscale ingress off and configure gateway.trustedProxies for that proxy instead.`,
  ]);
}
