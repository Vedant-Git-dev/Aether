...
export type GuidedOnboardingDeps = {
  ...
  persistRiskAcknowledgement?: (config: AetherConfig) => Promise<string | void>;
   ...
   * "chat" preserves the legacy handoff into the Aether system-agent chat —
...
async function runGuidedOnboardingFlow(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      t("wizard.guided.invalidConfigRepair", {
        fixCommand: formatCliCommand("aether vitals --fix"),
        inspectCommand: formatCliCommand("aether config validate"),
      }),
    );
    runtime.exit(1);
    return null;
  ...
  const assertLocalSetupOwner = (config: AetherConfig) => {
    if (
      localSetup?.status === "pending" &&
  ...
  // Inference is the only prerequisite for Aether. Use the caller's or
  // current default workspace as isolated probe context; Aether owns any
  ...
  const claimLocalSetup = (sourceConfig: AetherConfig) => {
    if (!localOnboarding) {
      return;
    }
  ...
  if (alreadyConfigured) {
    ...
    if (workspaceConflict) {
      await prompter.note(
        t("wizard.guided.workspaceConflictClassic", {
          command: formatCliCommand("aether awaken --classic"),
        }),
    ...
    // would, then hand off to the hatch instead of parking in the Aether chat.
    ...
    try {
      ...
      if (!appliedSnapshot.valid) {
        throw new Error("Setup wrote an invalid Aether config.");
      }
