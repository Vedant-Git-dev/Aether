        ...
        [
          ...
          ...(hasResolvedRemoteAuth
            ...
            : [
                ...
                `Fix the SecretRef, then run \`${formatCliCommand("aether health")}\` again.`,
              ]),
        ].join("\n"),
        "Gateway auth",
      );
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from aether.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<AetherConfig["tools"]>["web"]>["search"];
    ...
    if (codexRelevant) {
      note(
        [
          ...
          "If you do not choose one, Aether can select a provider from available credentials; otherwise other models may not have web search.",
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Aether update wizard" : "Aether configure");
    ...
    const baseConfig: AetherConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.aether.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("aether vitals")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.aether.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        "Docs: https://docs.aether.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
