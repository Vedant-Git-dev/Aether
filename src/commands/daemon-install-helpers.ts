...
import {
  AETHER_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveAetherWrapperPath,
...
function configContainsSecretRef(config: AetherConfig | undefined): boolean {
  if (!config) {
    return false;
  }
...
// they share the AETHER_ prefix that is otherwise stripped from preserved
...
const PRESERVED_AETHER_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "AETHER_CLI_CONTAINER_BYPASS",
  "AETHER_CONFIG_READONLY",
  "AETHER_CONTAINER_HINT",
]);
...
function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    // Like AETHER_SQLITE_LIBRARY, HOMEBREW_PREFIX must regenerate from each install/repair invocation.
    if (
      upper === "HOME" ||
      upper === "PATH" ||
      upper === "TMPDIR" ||
      upper === "HOMEBREW_PREFIX" ||
      (upper.startsWith("AETHER_") && !PRESERVED_AETHER_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[AETHER_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${AETHER_WRAPPER_ENV_KEY} because it points to the Windows task script; using the Aether gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveAetherWrapperPath(wrapperInput);
  ...
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [AETHER_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, AETHER_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.AETHER_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("aether gateway install")}\` after fixing the error.`;
}
