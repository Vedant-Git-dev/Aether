...
export async function migrateListCommand(runtime: RuntimeEnv, opts: { json?: boolean } = {}) {
  ...
  return await withPluginMigrationProviders({ cfg }, async (registered) => {
    ...
    if (providers.length === 0) {
      runtime.log(
        `No migration providers found. Run ${formatCliCommand("aether plugins list")} to verify provider plugins are installed and enabled.`,
      );
      return;
    }
    runtime.log(
...
export async function migratePlanCommand(
  ...
  if (!providerId) {
    throw new Error(
      `Migration provider is required. Run ${formatCliCommand("aether migrate list")} to choose one.`,
    );
  }
...
export async function migrateApplyCommand(
  ...
  if (!opts.yes && !process.stdin.isTTY) {
    throw new Error(
      `aether migrate apply requires --yes in non-interactive mode. Preview first with ${formatCliCommand(`aether migrate plan '${providerId.replaceAll("'", "'\\''")}'`)}.`,
    );
  }
  if (!provider) {
    return await withMigrationProvider(
