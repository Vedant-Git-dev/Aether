...
export function writeSessionSqliteMigrationFailureReports(
  ...
  const payload = {
    ...
    recoveryCommand: "aether vitals --session-sqlite recover --github-issue",
...
export function createSessionSqliteMigrationFailureIssue(
  ...
  const body = [
    "Aether doctor generated this sanitized report from a local session SQLite migration recovery.",
    "",
    reportBody,
  ].join("\n");
...
function renderFailureMarkdown(payload: {
  ...
  const lines = [
    ...
    `- Aether version: ${payload.version}`,
