...
import { listAetherPluginManifestMetadata } from "../../../plugins/manifest-metadata-scan.js";
...
function getModelCompatCatalogPlugins() {
  modelCompatCatalogPlugins ??= listAetherPluginManifestMetadata().flatMap(({ manifest }) => {
...
export const MODEL_COMPAT_CATALOG_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'nativeWebSearchTool and requiresMistralToolIds are unused and retired; run "aether vitals --fix" to remove them.',
    match: (value) => inspectModelCompatOverrides(value).dead > 0,
  },
  {
    path: ["models", "providers"],
    message:
      'Catalog-known model compat values are provider-owned; run "aether vitals --fix" to remove matching config overrides.',
