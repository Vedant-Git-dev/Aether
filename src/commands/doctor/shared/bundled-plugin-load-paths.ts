...
function resolveBundledWorkspaceDir(cfg: AetherConfig): string | undefined {
  const defaultAgentId = tryResolveDefaultAgentId(cfg);
  return defaultAgentId ? resolveAgentWorkspaceDir(cfg, defaultAgentId) : undefined;
}

function isAetherNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "aether" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by Aether. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Aether package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isAetherNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Aether loads the packaged bundled plugin from "${hit.toPath}".`,
  );
