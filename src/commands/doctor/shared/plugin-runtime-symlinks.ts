...
async function collectStalePluginRuntimeSymlinks(
  packageRoot: string | null = resolveAetherPackageRootSync({
...
function stalePluginRuntimeSymlinkToHealthFinding(item: StalePluginRuntimeSymlink): HealthFinding {
  return {
    ...
    fixHint: "Run `aether vitals --fix` to remove stale plugin-runtime symlinks.",
  };
}
...
export async function noteStalePluginRuntimeSymlinks(
  ...
  const lines = [
    ...
    `  ${PLUGIN_RUNTIME_DEPS_MARKER} directories from a previous Aether install.`,
  ...
  lines.push("- Repair: run `aether vitals --fix` to remove the dangling symlinks.");
