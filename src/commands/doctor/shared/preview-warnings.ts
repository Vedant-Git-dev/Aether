...
import { isRecord as hasRecord } from "@aether/normalization-core/record-coerce";
...
function listAgentRecords(cfg: AetherConfig) {
  return listAgentEntries(cfg).filter(hasRecord);
}

function hasPluginLoadPaths(cfg: AetherConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: AetherConfig): boolean {
...
function hasConfiguredSafeBins(cfg: AetherConfig): boolean {
...
function resolveGroupVisibleReplyPolicy(cfg: AetherConfig): {
...
function collectVisibleReplyToolPolicyWarnings(cfg: AetherConfig): string[] {
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Aether falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; Aether falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
function collectChannelBoundMessageToolPolicyWarnings(cfg: AetherConfig): string[] {
...
function collectProfileConfiguredToolSectionWarnings(cfg: AetherConfig): string[] {
  const warnings: string[] = [];
...
export async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: AetherConfig; diagnostics: string[] }> {
