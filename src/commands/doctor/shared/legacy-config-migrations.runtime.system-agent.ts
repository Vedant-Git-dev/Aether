...
export function findLegacySystemAgentOwnerIssue(raw: unknown) {
  ...
  return root && resolveMissingLegacySystemAgent(root)
    ? {
        path: "agents",
        message:
          'Legacy ambient operations have no system-agent owner; run "aether vitals --fix" to set agents.defaults.systemAgent.agentId from the default agent.',
      }
    : undefined;
}
...
const LEGACY_SYSTEM_AGENT_CONFIG_RULE: LegacyConfigRule = {
  path: ["crestodian"],
  message:
    'crestodian config was retired; system-agent rescue now uses built-in policy. Run "aether vitals --fix" to remove it.',
};
