...
function clearRepairedCodexSessionHarness(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "aether") {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("aether.codexRouteSessionRepairTestApi")
        ...
        ...(stale.length > 0
          ? [
              [
                ...
                "- Run `aether vitals --fix` to migrate legacy bindings and stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : []),
      ],
