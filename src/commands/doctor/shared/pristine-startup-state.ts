...
import { isRecord, isStringRecord } from "@aether/normalization-core/record-coerce";
import {
  resolveConfigPath,
...
export function planPristineStartupConfigMigrations(
  ...
  return {
    skipAllStateMigrations:
      ...
      !configMayRequireStartupPluginConvergence({ config: config as AetherConfig, env }),
    skipCoreStateMigrations,
  };
}
...
export function planPristineStartupStateMigrations(
  ...
  const explicitStateDir = env.AETHER_STATE_DIR?.trim();
