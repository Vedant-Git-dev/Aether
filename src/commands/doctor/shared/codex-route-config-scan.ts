import { AGENT_MODEL_CONFIG_KEYS } from "@aether/model-catalog-core/configured-model-refs";
...
}): { cfg: AetherConfig; changes: string[] } {
...
function codexPluginIsBlockedOutsideEntry(cfg: AetherConfig): boolean {
  return cfg.plugins?.enabled === false || pluginIdListIncludes(cfg.plugins?.deny, "codex");
}

export function codexPluginRepairIsBlocked(cfg: AetherConfig): boolean {
  return (
...
function isCodexPluginUnavailableByConfig(cfg: AetherConfig): boolean {
