...
function isPluginsGloballyDisabled(cfg: AetherConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: AetherConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: AetherConfig): string[] {
...
function isPluginEntryDisabled(cfg: AetherConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: AetherConfig, channelId: string): boolean {
  const channels = asNullableRecord(cfg.channels);
  const entry = asNullableRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: AetherConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: AetherConfig): string[] {
  const entries = asNullableRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: AetherConfig): string[] {
...
function collectConfiguredChannelIds(cfg: AetherConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectWebSearchPluginIds(cfg: AetherConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: AetherConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: AetherConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: AetherConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: AetherConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: AetherConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: AetherConfig): string[] {
...
function addEligiblePluginId(cfg: AetherConfig, pluginIds: Set<string>, pluginId: string): void {
...
function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareAetherVersions(
  ...
  const touchedComparedToRelease = compareAetherVersions(params.touchedVersion, releaseVersion);
