...
import {
  ...
  resolveAetherMcpTransportAlias,
...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use Aether transport names; CLI-native type aliases are legacy here. Run "aether vitals --fix".',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_DISABLED_RULES: LegacyConfigRule[] = [
  ...
  message:
    ...
    'Run "aether vitals --fix" to migrate it.',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_TIMEOUT_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} timeout aliases were retired; use connectionTimeoutMs and requestTimeoutMs. Run "aether vitals --fix".`,
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} legacy aliases were retired; use camelCase spellings and cwd. Run "aether vitals --fix".`,
  match: (value) =>
    isRecord(value) &&
  ...
  defineLegacyConfigMigration({
    ...
    apply: (raw, changes) => {
      ...
      for (const [serverName, rawServer] of Object.entries(servers)) {
        ...
        const alias = resolveAetherMcpTransportAlias(rawServer.type);
