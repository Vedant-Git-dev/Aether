...
const LEGACY_TTS_PROVIDER_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message:
      'tts legacy provider aliases/keys are legacy; use provider: "microsoft" and tts.providers.<provider>. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsProviderKeys(value),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts legacy provider aliases/keys are legacy; use provider: "microsoft" and plugins.entries.voice-call.config.tts.providers.<provider>. Run "aether vitals --fix".',
    match: (value) =>
...
const LEGACY_TTS_ENABLED_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message: 'tts.enabled is legacy; use tts.auto. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsEnabled(value),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.tts.enabled is legacy; use agents.entries.*.tts.auto. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsInLocations({ agents: value }, hasLegacyTtsEnabled),
  },
  {
    path: ["channels"],
    message:
      'supported channel TTS enabled fields are legacy; use the same TTS block auto field. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsInLocations({ channels: value }, hasLegacyTtsEnabled),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts.enabled is legacy; use plugins.entries.voice-call.config.tts.auto. Run "aether vitals --fix".',
...
const LEGACY_TTS_SPEAKER_SELECTION_RULES: LegacyConfigRule[] = [
  {
    path: ["tts"],
    message:
      'tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsSpeakerSelection(value),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsInLocations({ agents: value }, hasLegacyTtsSpeakerSelection),
  },
  {
    path: ["channels"],
    message:
      'supported channel TTS speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "aether vitals --fix".',
    match: (value) => hasLegacyTtsInLocations({ channels: value }, hasLegacyTtsSpeakerSelection),
  },
  {
    path: ["plugins", "entries"],
    message:
      'plugins.entries.voice-call.config.tts speaker selection fields voice/voiceName/voiceId are legacy; use speakerVoice or speakerVoiceId. Run "aether vitals --fix".',
    match: (value) =>
...
export const LEGACY_CONFIG_MIGRATIONS_RUNTIME_TTS: LegacyConfigMigrationSpec[] = [
  defineLegacyConfigMigration({
    ...
    legacyRules: [
      {
        path: ["messages", "tts"],
        message: 'messages.tts moved to top-level tts. Run "aether vitals --fix".',
      },
    ],
    apply: (raw, changes) => {
