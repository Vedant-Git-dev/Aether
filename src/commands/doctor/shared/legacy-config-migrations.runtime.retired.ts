...
const rule = (
  ...
  path,
  message: `${message} Run "aether vitals --fix".`,
  ...(match ? { match } : {}),
});
    ...
    legacyRules: [
      {
        ...
        message:
          'hooks.internal.handlers is retired. Move each module to a managed/workspace hook directory with HOOK.md + handler file before running "aether vitals --fix"; the fix removes retired registrations and does not materialize executable files.',
      },
    ],
    apply: (raw, changes) => {
