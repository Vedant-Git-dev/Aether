...
const LEGACY_DEFAULT_MODEL_MIGRATION = defineLegacyConfigMigration({
  ...
  legacyRules: [
    {
      path: ["defaultModel"],
      message: 'defaultModel moved to agents.defaults.model. Run "aether vitals --fix".',
    },
  ],
  apply: (raw, changes) => {
    ...
    legacyRules: [
      {
        path: ["models", "pricing"],
        message:
          'models.pricing is retired because pricing ships with the hosted catalog; run "aether vitals --fix" to remove it.',
      },
    ],
    apply: (raw, changes) => {
    ...
    legacyRules: [
      {
        ...
        message:
          'models.providers.codex and models.providers.openai-codex are legacy; run "aether vitals --fix" to move them to models.providers.openai.',
        match: (value, root) => codex.hasAutoFixableLegacyOpenAICodexProvider(value, root),
      },
      {
        path: ["models", "providers"],
        message:
          'openai-codex-responses is legacy; run "aether vitals --fix" to use openai-chatgpt-responses.',
        match: (value) => {
    ...
    legacyRules: [
      {
        ...
        message:
          'Legacy agents.defaults.models restricts model overrides; run "aether vitals --fix" to migrate valid refs to agents.defaults.modelPolicy.allow.',
