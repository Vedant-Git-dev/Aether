...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as AetherConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as AetherConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<AetherConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<AetherConfig["models"]>>;
...
function normalizeLegacyCodexCliProviderRuntimePins(
  cfg: AetherConfig,
  changes: string[],
): { config: AetherConfig; changed: boolean } {
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<AetherConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const rewriteRemainingSlots = (config: AetherConfig): AetherConfig =>
    rewriteModelRefs(config, "config", changes, (modelRef) => {
      ...
      return migrated &&
        ...
        ? migrated.ref
        : null;
    }).value as AetherConfig; // SAFETY: Rewriting model-ref strings and map keys preserves the config's value and container types.
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as AetherConfig["agents"],
      }
    : cfgWithProviders;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<AetherConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as AetherConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<AetherConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
