...
function readValidConfiguredAuthOrder(cfg: AetherConfig): Record<string, string[]> | undefined {
...
function hasValidConfiguredAuthProfiles(cfg: AetherConfig): boolean {
...
function hasNonemptyConfiguredAuthOrder(cfg: AetherConfig): boolean {
...
function loadConfiguredAgentAuthStores(
  ...
  const envAgentDir =
    env.AETHER_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim() || undefined;
  if (envAgentDir) {
  ...
  if (sharedOwnership.location === "legacy-main") {
    ...
    const owner =
      availability === "present"
        ? inspectAetherAgentDatabaseOwner(sharedDatabasePath)
        : undefined;
    if (
  ...
  for (const agentDir of agentDirs) {
    ...
    const owner =
      availability === "present" ? inspectAetherAgentDatabaseOwner(databasePath) : undefined;
    if (owner) {
      if (
        owner.status === "unreadable" ||
        (expectedAgentIds && owner.status === "owned" && !expectedAgentIds.has(owner.agentId))
  ...
  try {
    const registryEntries = listAetherRegisteredAgentDatabases({ env });
  ...
  for (const [databasePath, owners] of registeredOwnersByPath) {
    ...
    const owner = inspectAetherAgentDatabaseOwner(databasePath);
...
function removeAuthOrderKeys(cfg: AetherConfig, providers: ReadonlySet<string>): AetherConfig {
...
function repairStaleConfiguredAuthOrders(params: {
  ...
  activeStores?: readonly AuthProfileStore[];
  runtimeProfileIds?: ReadonlySet<string>;
}): { config: AetherConfig; changes: string[] } {
...
export function maybeRepairStaleConfiguredAuthOrders(params: {
  cfg: AetherConfig;
  env?: NodeJS.ProcessEnv;
}): { config: AetherConfig; changes: string[]; warnings?: string[] } {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.staleAuthOrderTestApi")] = {
