...
import {
  ...
  AETHER_EMBEDDED_CONTEXT_ENGINE_HOST,
...
function runtimeHostCandidate(params: {
  ...
  if (runtimeId === "aether" || runtimeId === "auto") {
    return { runtimeId, host: AETHER_EMBEDDED_CONTEXT_ENGINE_HOST, paths: params.paths };
  }
  if (runtimeId === "codex") {
...
function selectedContextEngineSlotId(cfg: AetherConfig): string {
...
export async function maybeRepairContextEngineHostCompatibility(params: {
  ...
  doctorFixCommand: string;
  env?: NodeJS.ProcessEnv;
}): Promise<{ config: AetherConfig; changes: string[]; warnings?: string[] }> {
  const resolved = await resolveSelectedContextEngineInfo(params);
  if (!resolved.info) {
