...
const MEMORY_SEARCH_RULE: LegacyConfigRule = {
  path: ["memorySearch"],
  message:
    'top-level memorySearch was moved; use memory.search instead. Run "aether vitals --fix".',
};

const AGENT_MEMORY_SEARCH_OWNER_RULES: LegacyConfigRule[] = [
  {
    path: ["agents", "defaults", "memorySearch"],
    message: 'agents.defaults.memorySearch moved to memory.search. Run "aether vitals --fix".',
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.memorySearch moved to agents.entries.*.memory.search. Run "aether vitals --fix".',
...
const LEGACY_MEMORY_SEARCH_AUTO_PROVIDER_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'memorySearch.provider = "auto" is legacy; use "openai" explicitly. Run "aether vitals --fix".',
    match: isLegacyMemorySearchAutoProvider,
  },
  {
    path: ["memory", "search", "provider"],
    message:
      'memory.search.provider = "auto" is legacy; use "openai" explicitly. Run "aether vitals --fix".',
    match: isLegacyMemorySearchAutoProvider,
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.memorySearch.provider = "auto" is legacy; use "openai" explicitly. Run "aether vitals --fix".',
    match: (value) =>
...
const LEGACY_MEMORY_SEARCH_STORE_PATH_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'memorySearch.store.path is legacy; memory indexes now live in each agent database. Run "aether vitals --fix".',
  },
  {
    path: ["memory", "search", "store", "path"],
    message:
      'memory.search.store.path is legacy; memory indexes now live in each agent database. Run "aether vitals --fix".',
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.memorySearch.store.path is legacy; memory indexes now live in each agent database. Run "aether vitals --fix".',
    match: (value) =>
...
const LEGACY_MEMORY_SEARCH_FLAT_KEY_RULES: LegacyConfigRule[] = [
  {
    path: ["memory", "search"],
    message:
      'memory.search uses legacy flat chunkSize, chunkOverlap, or maxResults fields. Run "aether vitals --fix".',
    match: hasLegacyMemorySearchFlatKeys,
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.memorySearch uses legacy flat chunkSize, chunkOverlap, or maxResults fields. Run "aether vitals --fix".',
    match: (value) =>
      someAgentEntry(value, (agent) =>
...
const LEGACY_SANDBOX_SCOPE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.perSession is legacy; use agents.defaults.sandbox.scope instead. Run "aether vitals --fix".',
    match: (value) => hasLegacySandboxPerSession(value),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.sandbox.perSession is legacy; use agents.entries.*.sandbox.scope instead. Run "aether vitals --fix".',
...
const UNSUPPORTED_SANDBOX_BROWSER_NETWORK_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.sandbox.browser.network = "none" cannot expose the browser control port. Run "aether vitals --fix" to disable the sidecar and restore the dedicated browser network.',
    match: isUnsupportedSandboxBrowserNetwork,
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.sandbox.browser.network = "none" cannot expose the browser control port. Run "aether vitals --fix" to disable the affected sidecar and restore the dedicated browser network.',
    match: (value) =>
      someAgentEntry(value, (agent) =>
...
const LEGACY_AGENT_RUNTIME_POLICY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.agentRuntime is ignored; set models.providers.<provider>.agentRuntime or a model-scoped agentRuntime instead. Run "aether vitals --fix".',
  },
  {
    path: ["agents", "defaults", "embeddedHarness"],
    message:
      'agents.defaults.embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "aether vitals --fix".',
  ...
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.agentRuntime is ignored; set provider/model runtime policy instead. Run "aether vitals --fix".',
    match: (value) => someAgentEntry(value, (agent) => getRecord(agent.agentRuntime) !== null),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.embeddedHarness is legacy and ignored; set provider/model runtime policy instead. Run "aether vitals --fix".',
...
const DEPRECATED_EMBEDDED_AGENT_KEY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.embeddedPi is legacy; use agents.defaults.embeddedAgent instead. Run "aether vitals --fix".',
    match: (value) => getRecord(value) !== null,
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.embeddedPi is legacy; use agents.entries.*.embeddedAgent instead. Run "aether vitals --fix".',
...
const LEGACY_AGENT_LLM_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.llm is legacy; use models.providers.<id>.timeoutSeconds for slow model/provider timeouts. Run "aether vitals --fix".',
    match: (value) => getRecord(value) !== null,
  },
];
...
const IGNORED_AGENT_MODEL_TIMEOUT_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.model.timeoutMs is ignored; agent model config only selects primary/fallback models. Run "aether vitals --fix" to remove it.',
    match: (value) => hasOwnTimeoutMs(value),
  },
  {
    path: ["agents", "defaults", "subagents", "model"],
    message:
      'agents.defaults.subagents.model.timeoutMs is ignored; subagent model config only selects primary/fallback models. Run "aether vitals --fix" to remove it.',
    match: (value) => hasOwnTimeoutMs(value),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.model.timeoutMs and agents.entries.*.subagents.model.timeoutMs are ignored; agent model config only selects primary/fallback models. Run "aether vitals --fix" to remove them.',
    match: (value) =>
      someAgentEntry(
        value,
        (agent) =>
...
const PROFILE_CONFIGURED_TOOL_SECTION_RULES: LegacyConfigRule[] = [
  {
    path: ["tools"],
    message:
      'tools.profile filters explicit configured-section tool grants; run "aether vitals --fix" to rewrite the explicit grants into a valid allowlist.',
    match: (value) => toolProfileConfiguredSectionsNeedExplicitRepair(value),
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.tools.profile filters explicit configured-section tool grants; run "aether vitals --fix" to rewrite the explicit grants into a valid allowlist.',
    match: (value, root) => {
...
const SILENT_REPLY_LEGACY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.silentReplyRewrite was removed; exact NO_REPLY is no longer rewritten to visible fallback text. Run "aether vitals --fix" to remove it.',
  },
  {
    path: ["agents", "defaults", "silentReply"],
    message:
      'agents.defaults.silentReply.direct was removed; direct chats never receive NO_REPLY prompt guidance. Run "aether vitals --fix" to remove it.',
    match: (value) => Object.hasOwn(getRecord(value) ?? {}, "direct"),
  },
  {
    path: ["surfaces"],
    message:
      'surfaces.*.silentReplyRewrite was removed; exact NO_REPLY is no longer rewritten to visible fallback text. Run "aether vitals --fix" to remove it.',
    match: (value) => hasSurfaceSilentReplyRewrite(value),
  },
  {
    path: ["surfaces"],
    message:
      'surfaces.*.silentReply.direct was removed; direct chats never receive NO_REPLY prompt guidance. Run "aether vitals --fix" to remove it.',
...
const SYSTEM_PROMPT_OVERRIDE_LEGACY_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'agents.defaults.systemPromptOverride was removed; Aether owns the generated system prompt. Run "aether vitals --fix" to remove it.',
  },
  {
    path: ["agents"],
    message:
      'agents.entries.*.systemPromptOverride was removed; Aether owns the generated system prompt. Run "aether vitals --fix" to remove it.',
    match: (value) =>
...
const BINDING_DM_PEER_KIND_RULE: LegacyConfigRule = {
  path: ["bindings"],
  message:
    'bindings[].match.peer.kind uses the retired "dm" alias; use "direct". Run "aether vitals --fix".',
  match: (value) =>
    ...
    legacyRules: [
      {
        ...
        message:
          'session.typingMode moved to agents.defaults.typingMode. Run "aether vitals --fix".',
      },
    ],
    apply: (raw, changes) => {
