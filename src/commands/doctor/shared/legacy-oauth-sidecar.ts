...
const LEGACY_OAUTH_SECRET_KEY_ENV = "AETHER_AUTH_PROFILE_SECRET_KEY";
const LEGACY_OAUTH_SECRET_KEYCHAIN_SERVICE = "Aether Auth Profile Secrets";
...
function buildLegacyOAuthSecretKey(seed: string): Buffer {
  ...
  return hash("sha256", `aether:auth-profile-oauth:${seed}`, "buffer");
}
...
function resolveLegacyOAuthSecretKeyFileCandidates(env: NodeJS.ProcessEnv): string[] {
  if (process.platform === "win32") {
    ...
    return uniquePaths([
      root ? path.join(root, "Aether", LEGACY_OAUTH_SECRET_KEY_FILE_NAME) : undefined,
      home
        ? path.join(home, ".aether-auth-profile-secrets", LEGACY_OAUTH_SECRET_KEY_FILE_NAME)
        : undefined,
    ]);
  }
...
function resolveLegacyOAuthSecretKeySeeds(env: NodeJS.ProcessEnv): string[] {
  ...
  if (env.NODE_ENV === "test" && env.VITEST === "true") {
    addSeed("aether-test-oauth-profile-secret-key");
  }
...
function emitKeychainOnlyMigrationHintOnce(profileId: string): void {
  ...
  authProfilesLog.warn(
    "Legacy Codex OAuth credentials are stored only in macOS Keychain on this host. " +
      "Headless paths cannot prompt for Keychain access; run `aether vitals --fix` " +
