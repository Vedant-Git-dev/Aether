...
export function createRetiredModelRefRepairResolver(params: {
  ...
  const owners = new Map(
    agents.map((agentId) => {
      ...
      >();
      const prepareModelResolver = (cfg: AetherConfig) => {
        const modelOptions = {
          cfg,
          agentId,
  ...
  const resolveForOwner = (
    ...
    const validatePolicy = (repair: ModelRefRepair): ModelRefRepair => {
      ...
      warn(
        `Retained model reference "${canonical}" for agent "${agentId}": "${replacement}" is not permitted. Allow "${replacement}" in ${policyPath} and rerun aether vitals --fix, or choose an allowed model override.`,
      );
      return { kind: "unchanged" };
    };
    ...
    if (!rule?.retirement) {
      ...
      if (!baseUrl) {
        warn(
          `Retained ${canonical} for agent "${agentId}": its exact authentication route is unavailable. Restore that provider account and rerun aether vitals --fix, or choose a current model explicitly.`,
        );
        return validatePolicy(preserved);
      }
  ...
  return (modelRef: string, path: string): string | null | undefined => {
    ...
    if (
      ...
      params.warnings?.push(
        `Retained retired ${path} "${modelRef}": no provider successor is declared and this global default has no agent default to inherit. Choose a supported default with aether models set.`,
      );
      return undefined;
    }
    params.changes.push(
...
export function repairRetiredSessionModelRef(
  ...
  if (
    ...
    const warning = `Retained retired ${decision.modelRef} for agent "${agentId}": clearing this session override would still select it with the same pinned account. Choose a supported default or an allowed model override, then rerun aether vitals --fix.`;
