...
/** Repair missing installs inferred from the current Aether config. */
...
async function repairMissingPluginInstallsWithLease(
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "aether vitals --fix" after the update completes.`;
      changes.push(detail);
