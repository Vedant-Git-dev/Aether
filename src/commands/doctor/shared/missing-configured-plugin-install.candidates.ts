...
import { compareAetherReleaseVersions } from "../../../infra/npm-registry-spec.js";
import {
...
export function collectDownloadableInstallCandidates(params: {
  ...
  for (const entry of CONFIGURED_RUNTIME_PLUGIN_INSTALL_CANDIDATES) {
    ...
    if (existing && entry.versionBoundToAether) {
      candidates.set(entry.pluginId, { ...existing, versionBoundToAether: true });
    } else if (!existing) {
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareAetherReleaseVersions(params.installedVersion, params.currentVersion);
