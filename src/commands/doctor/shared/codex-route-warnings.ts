...
import { asOptionalRecord as asMutableRecord } from "@aether/normalization-core/record-coerce";
import {
  normalizeFastMode,
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores Aether compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `aether vitals --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.repairBlocked
    ? "- Enable plugins.entries.codex and plugin loading, and remove `codex` from plugins.deny; or set the affected OpenAI models to an Aether runtime policy."
    : "- Run `aether vitals --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an Aether runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: AetherConfig): string[] {
  ...
  return [
    ...(inlineArgs
      ...
      : []),
    [
      "- Custom Codex app-server command bypasses Aether's managed exact-version binary.",
      "- plugins.entries.codex.config.appServer.command: Doctor did not execute, inspect, or rewrite this command.",
      "- Remove the override to use managed Codex startup, or verify the custom binary matches the Codex version bundled with this Aether release.",
    ].join("\n"),
  ];
}
...
function modelUsesCodexForEveryAgent(cfg: AetherConfig, modelRef: string): boolean {
...
function formatCodexModelParamWarning(hits: readonly CodexModelParamHit[]): string {
  const fixHint = hits.some((hit) => hit.removable)
    ? '- Run `aether vitals --fix` to remove only redundant priority service-tier params; remove any remaining params or set the affected route\'s agentRuntime.id to "aether".'
    : '- Remove these params or set the affected route\'s agentRuntime.id to "aether"; Doctor cannot migrate them without changing behavior.';
  return [
...
function repairRedundantCodexServiceTiers(cfg: AetherConfig, env?: NodeJS.ProcessEnv) {
...
function collectCodexComputerUseWarnings(cfg: AetherConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  const codex = asMutableRecord(entries?.codex);
  const config = asMutableRecord(codex?.config);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `aether vitals --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `aether vitals --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
export function maybeRepairCodexRoutes(params: {
  ...
  blockedProviderPlan?: BlockedLegacyOpenAICodexProviderPlan;
}): { cfg: AetherConfig; warnings: string[]; changes: string[] } {
  const env = params.env ?? process.env;
  const blockedProviderPlan =
    params.blockedProviderPlan ?? collectBlockedLegacyOpenAICodexProviderPlan(params.cfg);
  const blockedModelIdentities = new Set(blockedProviderPlan.blockedModelIdentities);
