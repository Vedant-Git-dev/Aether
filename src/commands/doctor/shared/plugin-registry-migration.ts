...
function invalidPersistedInstallRecordMessage(filePath: string): string {
  return [
    ...
    "Stop the Gateway, back up this database, delete only the config_machine_state row with state_key='plugins.installedIndex' using SQLite tooling, then rerun `aether vitals --fix` to rebuild it.",
  ].join(" ");
}

const INVALID_CONFIG_INSTALL_RECORD_MESSAGE =
  "plugins.installs contains invalid records. Back up aether.json, correct or remove the invalid retired plugins.installs record, then rerun `aether vitals --fix`.";
...
export type PluginRegistryDoctorMigrationParams = LoadInstalledPluginIndexParams &
  InstalledPluginIndexStoreOptions & {
    ...
    readConfig?: () => Promise<AetherConfig> | AetherConfig;
  };
