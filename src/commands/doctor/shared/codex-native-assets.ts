...
import { normalizeOptionalLowercaseString as normalizeString } from "@aether/normalization-core/string-coerce";
...
function isCodexRuntimeConfigured(cfg: AetherConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: AetherConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      ...
      "- To review or promote them: install the Codex plugin (aether plugins install npm:@aether/codex), then run aether migrate plan codex.",
    ].join("\n"),
  ];
}

if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.codexNativeAssetsTestApi")] = {
