...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("aether automations list")} and ${formatCliCommand("aether automations show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("aether automations list")} and ${formatCliCommand("aether automations show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.aether/bin/ensure-whatsapp.sh` is not maintained by current Aether and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("aether channels status --probe")}, ${formatCliCommand("aether vitals")}, and ${formatCliCommand("aether gateway status")} for current health checks.`,
