...
type CronMigrationDatabase = Pick<AetherStateDatabase, "migration_sources">;
...
export function hasLegacyCronMigrationReceipt(source: LegacyCronMigrationSource): boolean {
  return hasLegacyCronMigrationReceiptInDatabase(openAetherStateDatabase().db, source);
}
...
export function hasLegacyCronMigrationReceiptReadOnly(source: LegacyCronMigrationSource): boolean {
  const statePath = resolveAetherStateSqlitePath(process.env);
