...
export function formatUnresolvedCommandPromptAdvisory(names: string[]): string | null {
  ...
  return [
    ...
    '- Recreate it as a command automation (`aether automations add ... --command "<shell>"`) or grant explicit shell/process tool access before relying on it.',
  ].join("\n");
}
...
export function formatUnresolvedShellPromptAdvisory(names: string[]): string | null {
  ...
  return [
    ...
    '- For a deterministic run, recreate it as a command automation (`aether automations add ... --command "<shell>"`).',
  ].join("\n");
}
...
export function formatScheduledToolPolicyAdvisory(params: {
  ...
  lines.push(
    ...
    "- Reauthorize with an exact explicit cap: `aether cron edit <id> --tools <tool,...>`.",
  );
  return lines.join("\n");
}
...
export function formatIncompleteInheritedAuthorityAdvisory(names: string[]): string | null {
  ...
  return [
    ...
    "- If the job uses Codex configured MCP, reauthorize in place with an exact explicit list: `aether automations edit <id> --tools <tool,...>`.",
  ].join("\n");
}
