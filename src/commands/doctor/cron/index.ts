...
function readLegacyCronStorePath(cfg: AetherConfig): string | undefined {
  return (cfg.cron as (NonNullable<AetherConfig["cron"]> & { store?: string }) | undefined)
    ?.store;
}
...
function legacyCronStoreFinding(params: {
  ...
  return {
    ...
    fixHint:
      params.fixHint ??
      `Run ${formatCliCommand("aether vitals --fix")} to normalize legacy cron storage.`,
  };
}
...
export async function collectLegacyCronStoreHealthFindings(params: {
  ...
  try {
    ...
    return [
      legacyCronStoreFinding({
        ...
        fixHint: [
          `Fix the file's permissions or contents and re-run ${formatCliCommand("aether vitals")}.`,
  ...
  const sqliteStorePath = resolveAetherStateSqlitePath();
    ...
    if (names.length > 0) {
      findings.push(
        legacyCronStoreFinding({
          ...
          path: sqliteStorePath,
          requirement,
          fixHint: `Review with ${formatCliCommand("aether automations list --all")} and reauthorize with ${formatCliCommand("aether automations edit <id> --tools <tool,...>")}.`,
        }),
      );
    }
  }
  ...
  if (normalized.legacyGatewayExecJobs.length > 0) {
    findings.push(
      legacyCronStoreFinding({
        ...
        fixHint:
          "Review the affected jobs with `aether automations list --all`, then recreate each one from a fresh authenticated creator turn or explicitly reauthorize its complete tool cap from a trusted operator shell.",
      }),
    );
  }
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("aether vitals")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        noteHeading,
        ...previewLines,
        `Repair with ${formatCliCommand("aether vitals --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  if (inFlightCount > 0) {
    ...
    note(
      [
        ...
        `- Review with ${formatCliCommand("aether automations list --all")} or ${formatCliCommand("aether automations show <id>")}.`,
      ].join("\n"),
      "Cron",
    );
  }
  ...
  if (chronicFailureCount > 0) {
    note(
      [
        ...
        `- Review with ${formatCliCommand("aether automations list")} or ${formatCliCommand("aether automations show <id>")}.`,
      ].join("\n"),
      "Cron",
    );
  }
    ...
    note(
      [
        ...
        ...autoDisabledJobs.map(
          (job) =>
            `- ${job.name} (${job.id}): recorded reason \`${job.reason}\` after ${job.consecutiveErrors} consecutive errors. Fix the cause, then re-enable with ${formatCliCommand(`aether automations enable ${job.id}`)}.`,
        ),
      ].join("\n"),
      "Cron",
    );
  ...
  const noteHeading = legacyStoreDetected
    ...
    : `Cron store issues detected at ${shortenHomePath(resolveAetherStateSqlitePath())}.`;

  note(
    [
      noteHeading,
      ...previewLines,
      `Repair with ${formatCliCommand("aether vitals --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
