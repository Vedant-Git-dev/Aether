...
export async function applyLegacyCronStoreRepair(params: {
  cfg: AetherConfig;
  retiredModelRefConfig?: Pick<AetherConfig, "agents" | "models">;
  ...
  if (resolveRetired) {
    for (const job of state.rawJobs) {
      ...
      if (!agentId) {
        warnings.push(
          `Skipped retired model repair for cron job "${jobId}": select its owning agent, then rerun aether vitals --fix.`,
        );
        continue;
      }
      ...
      if (retirementChanges.length > beforeChanges && asOptionalRecord(job.state)?.autoDisabled) {
        ...
        retirementChanges.push(
          `Automation "${jobName}" remains auto-disabled. Run aether automations enable ${jobId} to resume it after this repair.`,
        );
      }
    }
  }
    ...
    try {
      ...
      const failure =
        err instanceof CronJobsStoreChangedError
          ? `Cron store at ${shortenHomePath(state.storePath)} changed while doctor was waiting, so no rows were rewritten; re-run ${formatCliCommand("aether vitals --fix")} to repair from a fresh snapshot.`
  ...
  if (state.legacyQuarantine) {
    ...
    if (archiveResult.ok) {
      ...
      warnings.push(
        `Migrated quarantined automations to SQLite but could not archive the legacy cron file at ${shortenHomePath(state.legacyQuarantine.path)}: ${archiveResult.reason}. Remove it manually or rerun ${formatCliCommand("aether vitals --fix")} to retry.`,
      );
    }
  }
  ...
  if (state.legacyStoreDetected) {
    ...
    if (archiveResult.ok) {
      ...
      for (const failure of archiveResult.failures) {
        warnings.push(
          `Migrated automations to SQLite but could not archive the legacy cron file at ${shortenHomePath(failure.path)}: ${failure.reason}. Remove it manually or rerun ${formatCliCommand("aether vitals --fix")} to retry.`,
        );
      }
    }
