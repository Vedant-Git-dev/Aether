...
import {
  ...
  listPluginAetherHostLinkIssues,
  maybeRepairPluginAetherHostLinks,
...
type PluginRegistryHealthIssue =
  ...
  | {
      kind: "managed-npm-aether-peer-link";
      packageName: string;
      packageDir: string;
      reason: string;
    }
  | {
      kind: "registered-npm-aether-host-link";
      packageName: string;
      packageDir: string;
      reason: string;
    }
...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "aether.plugin.json"));
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("aether vitals --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return null;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("aether vitals --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
export async function detectPluginRegistryHealthIssues(
  ...
  const hostLinkAudit = await listPluginAetherHostLinkIssues(params);
  ...
  switch (issue.kind) {
    case "registry-missing-or-stale":
      return {
        ...
        path: issue.path,
        fixHint: "Run `aether vitals --fix` to rebuild the plugin registry from enabled plugins.",
      };
    case "stale-managed-npm-bundled-plugin":
      return {
        ...
        fixHint:
          "Run `aether vitals --fix` to remove stale managed npm packages and rebuild the plugin registry.",
      };
    case "stale-local-bundled-plugin-install-record":
      return {
        ...
        target: issue.pluginId,
        fixHint:
          "Run `aether vitals --fix` to remove stale local install records and rebuild the plugin registry.",
      };
    case "managed-npm-aether-peer-link":
      return {
        checkId: PLUGIN_REGISTRY_CHECK_ID,
        severity: "warning",
        message: `Managed npm package ${issue.packageName} has a broken Aether peer link: ${issue.reason}.`,
        path: issue.packageDir,
        target: issue.packageName,
        fixHint: "Run `aether vitals --fix` to relink managed npm plugin packages.",
      };
    case "registered-npm-aether-host-link":
      return {
        checkId: PLUGIN_REGISTRY_CHECK_ID,
        severity: "warning",
        message: `Registered npm plugin ${issue.packageName} has a broken Aether host link: ${issue.reason}.`,
        path: issue.packageDir,
        target: issue.packageName,
        fixHint: "Run `aether vitals --fix` to relink the installed npm plugin package.",
      };
    case "managed-npm-package-unreadable":
      return {
        ...
        path: issue.packageDir,
        fixHint: "Restore access to the package files, then run `aether vitals` again.",
      };
...
export function pluginRegistryIssueToRepairEffect(
  ...
  switch (issue.kind) {
    ...
    case "managed-npm-aether-peer-link":
      return {
        kind: "package",
        action: "would-relink-managed-npm-aether-peer",
        target: issue.packageDir,
        dryRunSafe: false,
      };
    case "registered-npm-aether-host-link":
      return {
        kind: "package",
        action: "would-relink-registered-npm-aether-host",
        target: issue.packageDir,
        dryRunSafe: false,
      };
    case "managed-npm-package-unreadable":
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedPluginAetherHostLinks = await maybeRepairPluginAetherHostLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("aether vitals --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  return {
    config: params.config,
    ...(indexChanged || repairedPluginAetherHostLinks
