// Implements `aether agents add`, including config mutation, workspace setup, auth copy, and route binding setup.
...
export async function agentsAddCommand(
  ...
  if (!nonInteractive && !isTerminalInteractive(wizardOutput)) {
    failAgentsAdd(
      `Agent creation needs an interactive TTY. Use \`${formatCliCommand("aether agents add <id> --non-interactive --workspace <dir>")}\` for automation.`,
    );
  }
  ...
  if (nonInteractive) {
    if (!workspaceFlag) {
      failAgentsAdd(
        `Non-interactive agent creation requires --workspace. Re-run ${formatCliCommand("aether agents add <id> --workspace <path>")} or omit flags to use the wizard.`,
      );
    }
    if (!nameInput) {
      failAgentsAdd(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("aether agents add <id> --workspace <path>")}.`,
      );
    }
    ...
    if (!validation.ok) {
      failAgentsAdd(
        validation.reason === "reserved-id"
          ? `"${validation.agentId}" is reserved. Choose another name, or run ${formatCliCommand("aether agents list")} to inspect configured agents.`
          : validation.message,
      );
    }
    ...
    if (created.status === "error") {
      failAgentsAdd(
        created.reason === "reserved-id"
          ? `"${created.agentId}" is reserved. Choose another name, or run ${formatCliCommand("aether agents list")} to inspect configured agents.`
  ...
  const wizardRuntime: RuntimeEnv = opts.json
    ...
    : runtime;
  try {
    await prompter.intro("Add Aether agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
      ...
      if (wantsBindings) {
        ...
        await prompter.note(
          [
            ...
            "Docs: https://docs.aether.ai/concepts/multi-agent",
          ].join("\n"),
          "Routing",
        );
      }
