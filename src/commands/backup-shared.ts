...
export function recordBackupOutcomeBestEffort(
  ...
  try {
    ...
    assertNotUpdateCapturePath(resolveAetherStateSqlitePath(), resolveStateDir());
...
export function buildBackupArchiveRoot(nowMs = Date.now()): string {
  return `${formatBackupArchiveTimestamp(nowMs)}-aether-backup`;
}
...
export async function resolveBackupAgentRoot(
  ...
  return {
    agentId,
    sourcePath,
    databasePath: path.join(sourcePath, "aether-agent.sqlite"),
  };
}

/** Resolve configured agent storage roots and their canonical database paths for backup ownership. */
export async function resolveBackupAgentRoots(config: AetherConfig): Promise<BackupAgentRoot[]> {
  return await Promise.all(
    listAgentIds(config).map((agentId) => resolveBackupAgentRoot(config, agentId)),
  );
}

/** Resolve the backup plan from the current Aether state/config/workspace paths on disk. */
export async function resolveBackupPlanFromDisk(
  ...
  if (includeWorkspace && discoverySnapshot.exists && !discoverySnapshot.valid) {
    throw new Error(
      `Config invalid at ${shortenHomePath(discoverySnapshot.path)}. Aether cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`,
    );
  }
