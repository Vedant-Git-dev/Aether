...
function addPluginLoadPath(cfg: AetherConfig, pluginPath: string): AetherConfig {
...
async function finishOnboardingPluginInstall(params: {
  ...
  prepareConfig?: (cfg: AetherConfig) => AetherConfig | Promise<AetherConfig>;
  ...
  const clawhubSpecs = clawhubSpec
    ? resolveClawHubInstallSpecsForUpdateChannel({
        ...
        officialPackageName: entry.trustedSourceLinkedOfficialInstall
          ...
          : undefined,
        coreVersion: VERSION,
        versionBoundToCore: entry.versionBoundToAether,
      })
    : null;
