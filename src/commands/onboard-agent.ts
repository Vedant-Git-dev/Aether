...
function isInjectedMainRoster(config: AetherConfig): boolean {
...
function mergeOnboardingCandidate(params: {
  ...
  const merged = applyMergePatch(params.currentRuntime, proposalPatch) as AetherConfig;
...
export async function ensureOnboardingAgent(params: {
  ...
  if (before?.exists && !before.valid) {
    throw new Error("Cannot create the first agent from an invalid Aether config.");
  }
  if (before && (resolveConfigSnapshotHash(before) ?? null) !== params.expectedConfigHash) {
    throw new Error("Aether config changed before first-agent creation. Retry setup.");
  }
  ...
  if (!after.valid) {
    throw new Error("Agent creation wrote an invalid Aether config.");
  }
  ...
  const sessionMigrationWarnings =
    sessionMigration.armed && !sessionMigration.complete
      ? [
          `Legacy main-agent session history migration is incomplete${sessionMigration.warnings.length > 0 ? `: ${sessionMigration.warnings.join("; ")}` : ""}. Run \`aether vitals --fix\`; Aether will also retry at next startup.`,
        ]
      : [];
  return {
    config,
