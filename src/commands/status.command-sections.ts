...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("aether security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("aether security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      reasonLine,
      clearLine,
      "  Docs: https://docs.aether.ai/concepts/models#selection-source-and-fallback-strictness",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.aether.ai/faq",
    "Troubleshooting: https://docs.aether.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("aether status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("aether logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("aether node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("aether status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("aether gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`aether devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("aether devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("aether devices list")}`),
  ];
}
