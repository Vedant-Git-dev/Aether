...
export async function retireSessionSqliteRecovery(params: {
  ...
  readConfig(): Promise<AetherConfig>;
  ...
  await assertAetherStateWriteAllowedAtPath({
    databasePath: path.join(params.preview.stateDir, "state", "aether.sqlite"),
    env: params.env,
  ...
  return withDoctorSqliteMaintenanceLock({
    ...
    run: async (authority) => {
      ...
      await assertAetherStateWriteAllowedAtPath({
        databasePath: path.join(report.stateDir, "state", "aether.sqlite"),
        env: params.env,
        recoverOrphanedSidecars: false,
      });
