...
export function sessionTranscriptIssueToHealthFinding(
  ...
  return {
    ...
    path: issue.filePath,
    fixHint:
      "Run `aether vitals --fix` to repair legacy transcripts during their staged import into SQLite.",
  };
}
...
async function noteSessionSqliteMigrationHealth(params: {
  ...
  try {
    ...
    note(
      `- Skipped: Gateway or another SQLite maintenance command owns the state directory. Stop the Gateway, then run "${formatCliCommand("aether vitals --fix", params.env)}" for session-store maintenance.`,
      "Session SQLite",
    );
    recordPostSessionRefusal({
  ...
  if (reservedKeyReport.found > 0) {
    note(
      params.shouldRepair
        ...
        : `- Found ${reservedKeyReport.found} durable session key(s) that collide with the reserved incognito namespace. Run "aether vitals --fix" to rename them.`,
      "Session SQLite",
    );
  }
  if (canonicalKeyReport.foundGroups > 0) {
    note(
      params.shouldRepair
        ...
        : `- Found ${canonicalKeyReport.foundGroups} non-canonical or duplicate session-key group(s). Run "aether vitals --fix" to preserve their history and canonicalize the rows.`,
      "Session SQLite",
    );
  }
  if (deliveryReport.found > 0) {
    note(
      params.shouldRepair
        ...
        : `- Found ${deliveryReport.found} durable session row(s) with legacy delivery fields. Run "aether vitals --fix" to canonicalize them.`,
      "Session SQLite",
    );
  }
  if (resolvedSkillsReport.found > 0) {
    note(
      params.shouldRepair
        ? `- Stripped the runtime-only skills catalog from ${resolvedSkillsReport.repaired} durable session row(s). Logical SQLite pages are freed; shrinking the on-disk database requires "aether vitals --session-sqlite compact --session-sqlite-all-agents".`
        : `- Found ${resolvedSkillsReport.found} durable session row(s) carrying a runtime-only skills catalog. Run "aether vitals --fix" to strip it.`,
      "Session SQLite",
    );
  }
  if (
  ...
  if (report.totals.issues > 0) {
    lines.push(
      `- Found ${report.totals.issues} session SQLite issue(s). Inspect with "${formatCliCommand("aether vitals --session-sqlite dry-run --session-sqlite-all-agents", params.env)}".`,
    );
  }
  if (!params.shouldRepair) {
    lines.push(
      '- Run "aether vitals --fix" to migrate legacy session metadata/transcripts to SQLite.',
    );
  }
  if (params.shouldRepair && report.migrationRun && report.totals.archivedTranscriptFiles > 0) {
    lines.push(
      `- After verifying the upgrade, preview rollback retirement with "${formatCliCommand("aether update cleanup --dry-run", params.env)}" for state ${resolveStateDir(params.env)}. Keep the same AETHER_STATE_DIR and AETHER_CONFIG_PATH overrides.`,
    );
  }
  note(lines.join("\n"), "Session SQLite");
  return postSessionPluginReceipt;
