...
import { resolveAetherAgentSqlitePath } from "../state/aether-agent-db.js";
...
function assertTranscriptFileUnchanged(
  ...
  if (
    ...
    throw new Error(
      "Legacy transcript changed during import; stop active session writers and rerun `aether vitals --fix`.",
    );
  }
}
...
export function resolveTargetSqlitePath(
  ...
  return resolveAetherAgentSqlitePath(resolveTargetSqliteOptions(target, env));
}
