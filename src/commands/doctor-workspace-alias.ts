...
function configuredWorkspaceDirs(cfg: AetherConfig): string[] {
...
const REPAIR_HINT =
  "If the same workspace moved, run `aether vitals --fix` and confirm the move, or use `aether vitals --fix --force`. Otherwise restore the original link or configure the intended workspace directly.";
...
export function createWorkspaceAliasMigrationRepair(
  ...
  beforePrompt: () => void,
): ((cfg: AetherConfig) => Promise<void>) | undefined {
