 ...
 * Mirrors the real `aether config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("aether config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("aether config get <path>")}          read a config entry`,
  `  ${formatCliCommand("aether config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("aether config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.map((section) => section || '""').join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("aether tune")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }

  // Fail closed once at the shared entry: both `aether tune` and the
  // no-subcommand `aether config` route here, so a single guard keeps them
