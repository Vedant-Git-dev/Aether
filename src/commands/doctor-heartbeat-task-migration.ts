...
function resolveHeartbeatTaskMigrationAgents(cfg: AetherConfig) {
...
function migrationFinding(params: {
  ...
  return {
    ...
    fixHint: `Run ${formatCliCommand("aether vitals --fix")} to convert heartbeat tasks into automations.`,
  };
}
...
function taskJobInput(params: {
  ...
  return {
    ...
    description: "Migrated from heartbeat monitor scratch by aether vitals.",
...
async function loadCronPlanningSnapshot(
  ...
  const rows = loadCronRows(openAetherStateDatabase({ env }).db, cronStoreKey(storePath));
