...
/** CLI options accepted by `aether reset`. */
type ResetOptions = {
  scope?: ResetScope;
  yes?: boolean;
...
async function stopGatewayIfRunning(runtime: RuntimeEnv): Promise<boolean> {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside Aether-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("aether awaken --install-daemon")}`);
    return;
  }
