...
import {
  findStaleAetherUpdateLaunchdJobs,
...
export async function noteMacDisabledGatewayLaunchAgent(env: NodeJS.ProcessEnv = process.env) {
  ...
  const labelEnv = env.AETHER_LAUNCHD_LABEL?.trim() ? `AETHER_LAUNCHD_LABEL=${label} ` : "";
  note(
    [
      ...
      `After verifying the installation is safe to run, use ${labelEnv}${formatCliCommand("aether gateway start", env)} to re-enable and start it. Keep the same state/config overrides.`,
      `If an update was interrupted or installation safety is uncertain, run ${formatCliCommand("aether update", env)} or ${formatCliCommand("aether vitals", env)} and ${formatCliCommand("aether triage", env)} before starting it.`,
    ].join("\n"),
    "Gateway (macOS)",
  );
}

/** Returns a warning for stale Aether updater launchd jobs left after interrupted updates. */
async function collectMacStaleAetherUpdateLaunchdJobsWarning(): Promise<string | null> {
  if (process.platform !== "darwin") {
    return null;
  }
  const scanEnv = await resolveGatewayServiceEnvForPlatformNotes();
  const jobs = await findStaleAetherUpdateLaunchdJobs(scanEnv).catch(() => []);
  if (jobs.length === 0) {
    return null;
  }

  return [
    "- Stale Aether updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("aether gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleAetherUpdateLaunchdJobs() {
  const warning = await collectMacStaleAetherUpdateLaunchdJobsWarning();
  if (warning) {
    note(warning, "Gateway (macOS)");
  }
}
...
function hasConfigGatewayCreds(cfg: AetherConfig): boolean {
...
async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const envToken = await launchctlGetenv("AETHER_GATEWAY_TOKEN");
  const envPassword = await launchctlGetenv("AETHER_GATEWAY_PASSWORD");
  ...
  return [
    ...
    envToken
      ? "- `AETHER_GATEWAY_TOKEN` is set; explicit environment URL or node-host targets can use a different token than gateway.auth.token."
      : undefined,
    envPassword
      ? "- `AETHER_GATEWAY_PASSWORD` is set; explicit environment URL or node-host targets can use a different password than gateway.auth.password."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv AETHER_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv AETHER_GATEWAY_PASSWORD" : undefined,
  ]
...
export async function noteMacLaunchctlGatewayEnvOverrides(cfg: AetherConfig) {
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const staleUpdateWarning = await collectMacStaleAetherUpdateLaunchdJobsWarning();
...
export function noteStartupOptimizationHints(env: NodeJS.ProcessEnv = process.env) {
  ...
  const noRespawn = normalizeOptionalString(env.AETHER_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- AETHER_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/aether-compile-cache",
    "  mkdir -p /var/tmp/aether-compile-cache",
    "  export AETHER_NO_RESPAWN=1",
