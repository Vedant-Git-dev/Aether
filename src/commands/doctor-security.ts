...
import type { AetherConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AetherConfig): SecurityAuditFinding[] {
...
function collectExecPolicyConflictWarnings(
  ...
  const findings: SecurityAuditFinding[] = [];
  const defaultRequestedSecuritySource = "Aether default (full)";
  const defaultRequestedAskSource = "Aether default (off)";
  ...
  const maybeWarn = (params: {
    ...
    findings.push({
      ...
      remediation: [
        ...
        "Headless runs like isolated cron cannot answer approval prompts; align both files, or keep the Control UI or a connected desktop app available so gateway automation runs can raise approval cards.",
        `Inspect with: ${formatCliCommand("aether approvals get --gateway")}`,
      ].join("\n"),
    });
  };
  ...
  return [
    {
      ...
      remediation: [
        `Run ${formatCliCommand("aether vitals --fix")} to remove the inactive entries.`,
...
function collectExecFilesystemPolicyWarnings(cfg: AetherConfig): SecurityAuditFinding[] {
...
function collectPlaintextConfigSecretWarnings(cfg: AetherConfig): SecurityAuditFinding[] {
  ...
  return [
    {
      ...
      severity: "warn",
      title: "WARNING",
      detail: "aether.json contains plaintext secret-bearing config fields.",
      remediation: [
        `Paths: ${pathLine}`,
        "Agents or workspace tools that can read config files may see these API keys/tokens.",
        `Migrate them to SecretRefs with ${formatCliCommand("aether secrets configure")} or ${formatCliCommand("aether secrets apply")}, then verify with ${formatCliCommand("aether secrets audit --check")}.`,
      ].join("\n"),
    },
  ];
}
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    findings.push({
      ...
      remediation: [
        ...
        `Check local policy with: ${formatCliCommand("aether approvals get --gateway")}`,
      ].join("\n"),
    });
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `Fix: ${formatCliCommand("aether tune")} to set a password`,
              `Or switch to token: ${formatCliCommand("aether config set gateway.auth.mode token")}`,
            ]
          : [
              `Fix: ${formatCliCommand("aether vitals --fix")} to generate a token`,
              `Or set token directly: ${formatCliCommand(
                "aether config set gateway.auth.mode token",
              )}`,
            ];
      findings.push({
        ...
        remediation: [
          `Fix: ${formatCliCommand("aether config set gateway.bind loopback")}`,
...
export async function noteSecurityWarnings(cfg: AetherConfig) {
  const findings = await collectSecurityWarnings(cfg);
  if (findings.length > 0) {
    const lines = findings.flatMap(renderSecurityFindingLines);
    lines.push(`- Run: ${formatCliCommand("aether security audit --deep")}`);
    note(lines.join("\n"), "Security");
  }
}
