...
function collectUnsupportedInternalHookEntryWarnings(cfg: AetherConfig): string[] {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  const runWithCurrentPluginMetadata = <T>(config: AetherConfig, run: () => T): T => {
  ...
  let retiredModelRefConfig: Pick<AetherConfig, "agents" | "models"> | undefined;
  const doctorFixCommand = formatCliCommand("aether vitals --fix");
    ...
    const rosterRepair = {
      config: {
        ...
        agents: {
          ...
          entries: migratedEntries as NonNullable<AetherConfig["agents"]>["entries"],
        },
      },
      changes: [
        ...new Set(
