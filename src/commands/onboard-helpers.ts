...
import { printOrbBanner } from "../cli/orb-banner.js";
...
export function summarizeExistingConfig(config: AetherConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: AetherConfig): string | null {
...
/** Prints the onboarding banner: pixel mascot beside the AETHER wordmark. */
export async function printWizardHeader(runtime: RuntimeEnv): Promise<void> {
  await printOrbBanner(runtime);
}
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.aether.ai/gateway/remote",
    "https://docs.aether.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
  ...
  if (
    ...
    throw new Error(
      "Full reset workspace overlaps the active onboarding lock directory. " +
        "Choose a workspace outside the Aether state migration directory or use a narrower reset scope.",
    );
  }
}
...
export async function probeGatewayConfiguredModel(
  ...
  try {
    const config = configCandidate as AetherConfig;
