...
import { withArtifactPreservingStateReads } from "../state/aether-state-db-readonly.js";
import { resolveAetherStateSqlitePath } from "../state/aether-state-db.paths.js";
import { assertAetherStateWriteAllowedAtPath } from "../state/aether-state-ownership.js";
...
import { maybeRepairPluginAetherHostLinks } from "./doctor-plugin-host-links.js";
...
export function shouldSkipPluginValidationForDoctorConfigPreflight(
  ...
  return isTruthyEnvValue(env.AETHER_UPDATE_IN_PROGRESS);
}
...
export async function runDoctorConfigPreflight(
  ...
  if (stateMigrationsRequested) {
    await assertAetherStateWriteAllowedAtPath({
      databasePath: resolveAetherStateSqlitePath(process.env),
      env: process.env,
      ...
      try {
        ...
        startupMigrationHeartbeatError =
          error instanceof Error
            ? error
            : new Error("Aether startup migration lease heartbeat failed.");
      }
    }, 60_000);
  ...
  const migratePluginDoctorState = async (config: AetherConfig) => {
  ...
  try {
    ...
    if (options.repairPrefixedConfig === true && snapshot.exists && !snapshot.valid) {
      ...
      if (!activeConfigRepair && (await recoverConfigFromJsonRootSuffix(snapshot))) {
        note("Removed non-JSON prefix from aether.json.", "Config");
        configRepaired = true;
      } else if (
        !activeConfigRepair &&
        ...
        note(
          "Restored aether.json from last-known-good; original saved as .clobbered.*.",
          "Config",
        );
        configRepaired = true;
      }
      ...
      if (
        ...
        throw new Error(
          `Config at ${snapshot.path} is not parseable and cannot be repaired automatically. The file remains unchanged. Inspect the exact parse error with ${formatCliCommand("aether config validate")}, then hand-edit the file; or move it aside and run ${formatCliCommand("aether awaken")} to generate a fresh config.`,
        );
      }
    }
    ...
    if (automaticConfigRepair && stateMigrationsAllowed && freshConfigGuardAllowed) {
      ...
      note(
        `Migrated legacy config keys${activeConfigRepair ? " in the active aether.json" : " at startup"}:\n${automaticConfigRepair.changes.map((entry) => `- ${entry}`).join("\n")}`,
        "Doctor changes",
      );
      configSnapshotRead = await readConfigSnapshotForPreflight(false);
      snapshot = configSnapshotRead.snapshot;
    ...
    if (
      ...
      if (
        ...
        throw new Error(
          'Aether config identity changed while persisting the refreshed plugin registry; refusing to write the migration checkpoint. Run "aether vitals --fix" and retry.',
        );
      }
