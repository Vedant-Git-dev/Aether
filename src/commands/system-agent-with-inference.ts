// Aether command gate: prove inference before starting conversational setup.
...
/**
 * Start Aether only after the configured default model completes a real
...
export async function runSystemAgentWithInference(
  ...
  if (opts.yes && !opts.message?.trim()) {
    failOneShotExecution(
      opts,
      runtime,
      new Error("Aether --yes requires --message so approval is limited to one request."),
    );
    return;
  }
  const oneShot = isOneShotRequest(opts);
  if (!oneShot && !hasInteractiveTty(opts)) {
    runtime.error("Aether needs an interactive TTY. Use --message for one command.");
    runtime.exit(1);
    return;
  }
  ...
  if (oneShot) {
    const guidance = "Run `aether awaken` to connect and live-test AI first.";
    if (opts.json) {
      writeRuntimeJson(runtime, {
        ok: false,
        status: inference.status,
        error: `Aether requires working inference: ${inference.error}`,
        guidance,
      });
    } else {
      runtime.error(
        [`Aether requires working inference: ${inference.error}`, guidance].join("\n"),
      );
    }
    if (!requestExitAfterOneShotOutput(runtime, 1)) {
      runtime.exit(1);
  ...
  runtime.log("Aether requires working inference. Starting guided AI setup…");
