...
async function collectInstallPolicyHealthLines(
  ...
  if (!options.deep) {
    lines.push(
      `- Static checks passed. Run ${formatCliCommand("aether vitals --deep")} to execute a synthetic policy probe.`,
    );
    return lines;
  }

  const probeDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-install-policy-probe-"));
  try {
