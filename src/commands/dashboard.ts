// Implements `aether dashboard` URL resolution, readiness check, clipboard, and browser launch.
...
async function resolveDashboardTarget() {
  ...
  if (snapshot.exists && !snapshot.valid) {
    throw new Error(
      `Aether config is invalid: ${snapshot.path}. Run \`aether vitals --fix\` or \`aether config validate\`.`,
    );
  }
...
export async function dashboardCommand(
  ...
  if (!(await hasVerifiedControlUiLoopbackAlias(target))) {
    ...
    runtime.log("Restart the Gateway, then run `aether gateway status --deep` for details.");
    return;
  }
  const document = await waitForControlUiDocument({
    url: target.documentUrl,
  ...
  if (!document.ready) {
    ...
    runtime.log("Run `aether gateway status --deep` for details.");
    runtime.exit(1);
    return;
  }
  ...
  try {
    ...
    runtime.log("Run `aether vitals`, then retry `aether dashboard`.");
    return;
  }
  ...
  if (opened) {
    runtime.log("Opened in your browser. Keep that tab to control Aether.");
  ...
  if (fallbackToManualAuth) {
    runtime.log(
      "Token auto-auth not delivered. Append your gateway token (from AETHER_GATEWAY_TOKEN or gateway.auth.token) as a URL fragment with key `token` to authenticate.",
    );
  } else if (fallbackToJsonHandoff) {
    runtime.log(
      "One-time pairing URL not delivered. Run `aether dashboard --json` and open its `browserUrl` within ten minutes.",
    );
  }
}
