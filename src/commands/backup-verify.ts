...
function assertCanonicalSqlitePathCasing(relativePath: string, archivePath: string): void {
  ...
  const portablePath = resolvePortableArchivePathKey(relativePath);
  const isGlobalAlias =
    portablePath === "state/aether.sqlite" && relativePath !== "state/aether.sqlite";
  const isAgentAlias =
    segments.length === 4 &&
    segments[0]?.toLowerCase() === "agents" &&
    Boolean(segments[1]) &&
    segments[2]?.toLowerCase() === "agent" &&
    segments[3]?.toLowerCase() === "aether-agent.sqlite" &&
    (segments[0] !== "agents" ||
      segments[2] !== "agent" ||
...
function listSqliteSnapshotEntries(
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      resolvePortableArchivePathKey(relativePath) === "aether-agent.sqlite" &&
      relativePath !== "aether-agent.sqlite"
    ) {
...
function resolveExpectedSqliteRole(entry: SqliteSnapshotEntry): ExpectedSqliteRole | undefined {
  ...
  if (entry.agentId) {
    return relativePath === "aether-agent.sqlite" ? "agent" : undefined;
  }
...
function resolveExpectedSqliteRoleFromRelativePath(
  ...
  if (relativePath === "state/aether.sqlite") {
    return "global";
  }
  const segments = relativePath.split("/");
  if (
...
async function verifySqliteSnapshots(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "aether-backup-verify-sqlite-"));
  try {
    ...
    for (const entry of sqliteEntries) {
      ...
      try {
        ...
        if (entry.agentId) {
          assertAetherAgentDatabaseOwner(database, {
...
async function verifyResolvedBackupArchive(archivePath: string): Promise<PreparedBackupArchive> {
  ...
  try {
    ...
    if (hasErrnoCode(error, "ENOENT")) {
      throw new Error(
        "Archive does not exist. Check the path and run `aether backup verify <archive>` again.",
        { cause: error },
      );
    }
    throw new Error(
  ...
  if (!archiveStat.isFile()) {
    throw new Error(
      "Archive must be a regular file. Choose a backup archive created by `aether backup create` and try again.",
    );
  }
  ...
  if (listing.invalidReason) {
    throw new Error(
      `Archive is not a valid Aether backup. ${listing.invalidReason.replace(/[.!?]*$/u, ".")} Choose another archive or create a new one with \`aether backup create\`.`,
    );
  }
