...
function findProviderSecretRefProfiles(store: AuthProfileStore, cfg: AetherConfig) {
...
export async function maybeMigrateModelCatalogCredentials(params: {
  ...
  if (detected > 0) {
    note(
      `Found ${detected} plaintext model credential${detected === 1 ? "" : "s"}. Run aether vitals --fix to copy and verify them in agent SQLite before plaintext catalog authentication is retired.`,
      "Model catalog credentials",
    );
  }
  if (removable > 0) {
    note(
      `Found ${removable} stored unresolved model credential marker${removable === 1 ? "" : "s"}. Run aether vitals --fix to remove them from agent SQLite.`,
      "Model catalog credentials",
    );
  }
  const shouldRepair =
