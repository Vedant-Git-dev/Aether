...
export async function resolveNonInteractiveApiKey(params: {
  ...
  const returnOperatorKey = (key: string, source: "flag" | "env", envVarName?: string) => {
    ...
    return reject(`Paste the API key value, not an Aether onboarding command.${envHint}`);
  };
  ...
  return reject(
    `Missing ${params.flagName} (or ${params.envVar} in env${profileHint}). Export ${params.envVar}, pass ${params.flagName}, or run ${formatCliCommand("aether awaken")} for interactive setup.`,
  );
}
