...
async function resolveGatewayHealthProbeToken(
  ...
  if (nextConfig.gateway?.auth?.mode === "password") {
    ...
    const resolved = await resolveConfiguredSecretInputWithFallback({
      ...
      readFallback: () => process.env.AETHER_GATEWAY_PASSWORD,
    });
    return {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("aether.onboardNonInteractiveLocalTestApi")
  ] = {
...
export async function runNonInteractiveLocalSetup(params: {
  ...
  sourceConfigBeforeMigrations: AetherConfig;
  baseHash?: string;
}) {
  ...
  if (workspaceConflict) {
    runtime.error(
      [
        ...
        `Run \`${formatCliCommand("aether awaken --classic")}\` to confirm moving the existing agent fleet.`,
      ].join("\n"),
    );
  }

  let nextConfig: AetherConfig = applyLocalSetupWorkspaceConfig(
    baseConfig,
    requestedWorkspaceDir,
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `aether gateway run` or `aether agent --local`.",
              ]
            : [`Run \`${formatCliCommand("aether gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
    ...
    if (!probe.ok) {
      ...
      if (!explicitlySkippedAbsentGateway || !opts.json) {
        logNonInteractiveOnboardingFailure({
          ...
          hints: !opts.installDaemon
            ? [
                "Non-interactive local setup only waits for an already-running gateway unless you pass `--install-daemon` to `aether awaken`.",
                `Fix: start \`${formatCliCommand("aether gateway run")}\`, re-run \`${formatCliCommand("aether awaken --install-daemon")}\`, or use \`${formatCliCommand("aether awaken --skip-health")}\`.`,
      ...
      try {
        ...
        logNonInteractiveOnboardingFailure({
          ...
          daemonInstall: daemonInstallStatus,
          daemonRuntime: opts.installDaemon ? daemonRuntimeRaw : undefined,
          hints: [`Run \`${formatCliCommand("aether health")}\` for full diagnostics.`],
        });
        runtime.exit(1);
        return;
      }
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("aether tune --section web")}\` to store your Brave API key for web_search. Docs: https://docs.aether.ai/tools/web`,
    );
  }
}
