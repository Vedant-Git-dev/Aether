...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
    case "module-missing":
      return `Fix: run \`${formatCliCommand("aether vitals --fix")}\`.`;
    case "service-missing":
      return `Fix: run \`${formatCliCommand("aether gateway install --force")}\`.`;
    case "service-stopped":
      return `Fix: run \`${formatCliCommand("aether gateway restart")}\`.`;
    case "startup-blocked":
      return `Fix: run \`${formatCliCommand("aether gateway status --deep")}\`.`;
    case "not-listening":
      return `Fix: start \`${formatCliCommand("aether gateway run")}\`, or run \`${formatCliCommand("aether gateway restart")}\` for a managed gateway.`;
    default:
      return undefined;
  }
}
