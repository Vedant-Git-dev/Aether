 ...
 */
import { validateDottedDecimalIPv4Input } from "@aether/net-policy/ipv4";
...
function resolveGeneratedTokenInput(params: {
  ...
  if (params.ambientEnvOnly) {
    return createGatewayEnvSecretRef(params.config, "AETHER_GATEWAY_TOKEN");
  }
...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  if (bind === "custom") {
    ...
    if (customBindHostIssue) {
      const setCommand = formatCliCommand("aether config set gateway.customBindHost <ipv4>");
      const interactiveCommand = formatCliCommand("aether awaken");
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.AETHER_GATEWAY_TOKEN);
  ...
  // plaintext > ambient AETHER_GATEWAY_TOKEN > randomToken(). Ambient env
    ...
    if (gatewayTokenRefEnv) {
      ...
      if (!resolvedFromEnv) {
        rejectOnboardingOption(
          opts,
          runtime,
          `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("aether awaken --non-interactive")}.`,
        );
        return null;
      }
      nextConfig = {
      ...
      // branch, an ambient AETHER_GATEWAY_TOKEN (or randomToken() fallback)
      ...
      // AETHER_GATEWAY_TOKEN keeps its env ref so a later rotation still wins;
    ...
    const password =
      input === undefined
        ? (nextConfig.gateway?.auth?.password ??
          normalizeOptionalString(process.env.AETHER_GATEWAY_PASSWORD))
            ...
            ? {
                password:
                  opts.secretInputMode === "ref"
                    ? createGatewayEnvSecretRef(nextConfig, "AETHER_GATEWAY_PASSWORD")
                    : password,
              }
            : {}),
        },
