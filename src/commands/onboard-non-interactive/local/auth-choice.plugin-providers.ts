...
export async function applyNonInteractivePluginProviderChoice(params: {
...
}): Promise<AetherConfig | null | undefined> {
  ...
  const providerConfig = agentScopedModels
    ...
    : enableResult.config;
  const projectProviderResult = (updated: AetherConfig) =>
    agentScopedModels
  ...
  if (
    ...
    const stagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aether-setup-credential-"));
    ...
    try {
      ...
      closeAetherAgentDatabases(stagingRoot);
