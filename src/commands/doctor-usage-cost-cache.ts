...
import { listAetherRegisteredAgentDatabases } from "../state/aether-agent-db.js";
...
async function maybeRemoveLegacyUsageCostCacheFiles(params: {
  ...
  const files = await detectLegacyUsageCostCacheFiles(params).catch((error: unknown) => {
    const command = params.shouldRepair ? "aether vitals --fix" : "aether vitals";
  ...
  if (!params.shouldRepair) {
    note(
      `${files.length} rebuildable usage-cost cache ${files.length === 1 ? "file remains" : "files remain"}. Run \`aether vitals --fix\` to remove ${files.length === 1 ? "it" : "them"}.`,
      "Usage cost cache",
    );
    return;
  }
...
async function maybeRemoveLegacySkillUploadTree(params: {
  ...
  if (!params.shouldRepair) {
    note(
      "Legacy skill-upload staging remains. Run `aether vitals --fix` to discard it; active uploads now live in SQLite and must be retried.",
      "Skill uploads",
    );
    return;
  }
...
export async function maybeRepairLegacyRuntimeFiles(
  ...
  if (shouldRepair) {
    for (const entry of listAetherRegisteredAgentDatabases({ env })) {
