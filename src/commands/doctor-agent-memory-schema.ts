...
function inspectAgentMemoryRecallMetadataMigration(
  ...
  if (!stat.isFile()) {
    throw new Error(`Aether agent database is not a regular file: ${pathname}`);
  }
...
async function repairDoctorAgentMemorySchemas(
  ...
  let registered: ReturnType<typeof listAetherRegisteredAgentDatabases>;
  try {
    registered = listAetherRegisteredAgentDatabases({
      env,
      includeIncompatibleSchemaVersions: true,
    });
  } catch (error) {
  ...
  for (const entry of registered) {
    ...
    try {
      ...
      closeAetherAgentDatabaseByPath(entry.path);
