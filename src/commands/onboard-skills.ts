...
function isTrustedAutoInstallableSkill(skill: { bundled: boolean; source: string }): boolean {
  ...
  return skill.bundled && skill.source === "aether-bundled";
}
...
export async function setupSkills(
  ...
  let next: AetherConfig = cfg;
  if (candidateInstallable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("aether skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("aether skills check")}`,
      ].join("\n"),
  ...
  if (candidateInstallable.length > 0) {
    ...
    for (const target of selectedReadySkills) {
      ...
      // visible through `aether skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("aether vitals")}\` to review skills + requirements.`,
      );
