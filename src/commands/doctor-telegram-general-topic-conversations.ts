...
function resolveRepairScopes(cfg: AetherConfig, env: NodeJS.ProcessEnv) {
...
function repairLegacyRow(database: AetherAgentDatabase, legacyConversationId: string): boolean {
...
export async function repairTelegramGeneralTopicConversations(params: {
  ...
  for (const { scope } of resolveRepairScopes(params.cfg, env)) {
    await runExclusiveSqliteSessionWrite(
      scope,
      async () => {
        repaired += runAetherAgentWriteTransaction(
          (database) => {
