...
import { readNonBlankString as readNonEmptyString } from "@aether/normalization-core/string-coerce";
...
function coerceLegacyConfigAuthProfileStore(cfg: AetherConfig): AuthProfileStore | null {
...
export async function maybeMigrateAuthProfileJsonStoresToSqlite(params: {
  ...
  note(
    [
      ...
      `- ${formatCliCommand("aether vitals --fix")} imports legacy auth profile JSON into SQLite, verifies it, records a receipt, and archives the original bytes.`,
    ].join("\n"),
  ...
  if (hasLegacyOAuth && sharedMainCredentialSourceRemains) {
    result.warnings.push(
      `Deferred shared legacy OAuth migration until higher-priority shared-main credential sources are resolved by ${formatCliCommand("aether vitals --fix")}.`,
    );
...
function ensureConfigAuthProfiles(config: AetherConfig): Record<string, AuthProfileConfig> {
