...
export function uiProtocolFreshnessIssueToHealthFinding(
  ...
  return {
    ...
    fixHint: issue.canBuild
      ? issue.kind === "missing-assets"
        ? "Run `aether vitals --fix` to build Control UI assets."
        : `Run \`aether vitals --fix --force\` to rebuild Control UI assets, or run \`${formatControlUiSourceCommand(issue.root, "build")}\`.`
      : "Reinstall Aether to restore bundled Control UI assets.",
  };
}
