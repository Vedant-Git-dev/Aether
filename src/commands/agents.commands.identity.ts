...
export async function agentsSetIdentityCommand(
  ...
  if (normalizedAgent && !normalizedAgent.ok) {
    failAgentIdentity(`Agent "${opts.agent}" not found. Create it with \`aether agents add\`.`);
  }
  ...
  if (!resolvedAgentIds.includes(resolvedAgentId)) {
    failAgentIdentity(
      `Agent "${resolvedAgentId}" not found. Create it with \`aether agents add\`.`,
    );
  }
  ...
  if (locatorDiffers && workspaceLocatorDir) {
    ...
    runtime.log(
      `Stored workspace unchanged. Relocate with ${formatCliCommand(`aether config set agents.entries.${resolvedAgentId}.workspace ${quoteCliArg(workspaceLocatorDir)}`)}.`,
    );
