...
async function resolveHeartbeatScratchMigrationOwners(cfg: AetherConfig) {
...
async function readHeartbeatSource(
  ...
  try {
    ...
    if (!options?.recoverClaims) {
      throw new Error(
        `an interrupted migration claim exists at ${staleClaim}; run aether vitals --fix to restore it`,
        { cause: error },
      );
    }
...
function migrationFinding(params: {
  ...
  return {
    ...
    fixHint: `Run ${formatCliCommand("aether vitals --fix")} to migrate HEARTBEAT.md into cron scratch.`,
  };
}
