...
export function compactDoctorSqliteFile(
  ...
  try {
    database.exec(
      `PRAGMA busy_timeout = ${options.busyTimeoutMs ?? AETHER_SQLITE_BUSY_TIMEOUT_MS};`,
    );
...
export function checkpointDoctorSqliteFile(database: DatabaseSync, sqlitePath: string): void {
  ...
  if (busy !== 0) {
    throw new Error(`SQLite checkpoint remained busy for ${sqlitePath}. Stop Aether and retry.`);
  }
}
