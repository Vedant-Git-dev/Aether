  ...
  return await withArtifactPreservingStateReads(async () => {
    ...
    try {
      ...
      if (!read.snapshot.valid && !repair) {
        throw new Error('Aether config is invalid; run "aether vitals --fix" before startup.');
      }
...
async function assertStartupStateMigrationReady(params: {
  ...
  const { assertAetherDatabasesReady } = await import("../state/aether-database-preflight.js");
  await assertAetherDatabasesReady({
    env: params.env,
    config: params.cfg,
  ...
  const { inspectAetherRegisteredAgentDatabases } =
    await import("../state/aether-agent-db-registry.js");
...
export async function completeStartupMigrationPreflight(params: {
  ...
  if (
    ...
    if (!params.migrationCheckpoint) {
      throw new Error("Aether state migration checkpoint module was not loaded.");
    }
  ...
  if (params.shouldRecordStartupCheckpoint && params.startupMigrationWarnings.length === 0) {
    if (!params.migrationCheckpoint) {
      throw new Error("Aether startup migration checkpoint module was not loaded.");
    }
