...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "aether-tui") {
    return true;
  }
  return executable === "aether" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local Aether TUI processes without inferring their Gateway or activity. */
...
function hasWhatsappEnabled(cfg: AetherConfig): boolean {
  ...
  return [
    {
      ...
      fixHint: `Inspect Gateway diagnostics with ${formatCliCommand(
        "aether gateway diagnostics export",
