...
export function migrateLegacyCommandOwners(cfg: AetherConfig, changes: string[]): AetherConfig {
...
function resolveConfiguredCommandOwners(cfg: AetherConfig): string[] {
  const owners = cfg.commands?.ownerAllowFrom;
  if (!Array.isArray(owners)) {
    return [];
  }
...
export function hasConfiguredCommandOwners(cfg: AetherConfig): boolean {
...
export function formatCommandOwnerHint(params: {
  ...
  const command = formatCliCommand("aether config set commands.ownerAllowFrom");
...
export function noteCommandOwnerHealth(cfg: AetherConfig): void {
  ...
  note(
    [
      ...
      `Fix: set commands.ownerAllowFrom to your channel user id, for example ${formatCliCommand("aether config set commands.ownerAllowFrom '[\"telegram:123456789\"]'")}`,
