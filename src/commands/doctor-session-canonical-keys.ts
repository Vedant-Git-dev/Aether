...
function selectCanonicalSessionCandidate(
  ...
  params: { cfg: AetherConfig; env: NodeJS.ProcessEnv },
) {
...
async function repairCanonicalSessionGroup(
  ...
  setCanonicalSqliteSessionMainKey(
    openAetherAgentDatabase({ agentId: destination.agentId, path: destination.sqlitePath }),
...
export async function repairCanonicalSessionKeys(params: {
  ...
  if (params.apply) {
    for (const store of stores) {
      setCanonicalSqliteSessionMainKey(
        openAetherAgentDatabase({ agentId: store.agentId, path: store.sqlitePath }),
        params.cfg.session?.mainKey,
      );
    }
  }
