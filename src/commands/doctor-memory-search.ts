...
import {
  ...
  normalizeProviderId,
} from "@aether/model-catalog-core/provider-id";
...
function resolveMemoryDoctorAgentScopes(cfg: AetherConfig): MemoryDoctorAgentScope[] {
  ...
  switch (reason) {
    case "plugins-disabled":
      return {
        ...
        fix: `Fix: ${formatCliCommand("aether config set plugins.enabled true --strict-json")}, or select another memory provider.`,
      };
    ...
    case "plugin-disabled":
      return {
        ...
        fix: `Fix: Enable it: ${formatCliCommand(`aether plugins enable ${pluginId} --accept-capabilities`)}, or select another memory provider.`,
      };
...
function hasConfiguredAwsSdkAuthForProvider(provider: string, cfg: AetherConfig): boolean {
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: AetherConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: AetherConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("aether vitals --fix")} or ${formatCliCommand("aether memory status --fix")}`
    : `Verify: ${formatCliCommand("aether memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: AetherConfig): Promise<void> {
...
function hasActiveAlternateMemoryPluginSlot(cfg: AetherConfig): boolean {
...
function isActiveMemoryPluginAvailable(cfg: AetherConfig): boolean {
  const plugins = normalizePluginsConfig(cfg.plugins);
...
function resolveActiveMemoryConversationRecallSupport(cfg: AetherConfig): {
 ...
 * Runs as part of `aether vitals` using config-only checks where possible.
 */
...
async function noteMemorySearchHealthForAgent(
  ...
  if (provider === "local") {
    ...
    const updateFix =
      !ownerPolicyBlock && !inspectSetup
        ? `Fix: Update the installed plugin: ${formatCliCommand(`aether plugins update ${installedOwner.id}`)}`
        : null;
  ...
  if (
    ...
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("aether config set memory.search.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("aether memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("aether config set memory.search.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("aether memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `aether vitals` without
  ...
  noteFn(
    [
      ...
      `- Configure credentials: ${formatCliCommand("aether tune --section model")}`,
      `- To disable: ${formatCliCommand("aether config set memory.search.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("aether memory status --deep")}`,
    ].join("\n"),
    "Memory search",
