...
function listHint(agent?: string): string {
  ...
  return formatCliCommand(`aether sessions list${agentFlag} --json`);
}
...
const WORKTREE_PRESERVATION_REASON_COPY = {
  ...
  "foreign-lock": "Git reports a lock owned outside Aether",
  "snapshot-failed": "Aether could not create a safety snapshot",
      ...
      switch (result.status) {
        ...
        case "deleted": {
          ...
          if (archivedTranscripts.length > 0) {
            ...
            runtime.log(
              agentId
                ? `To remove indexed memories for this session, run aether memory forget --agent ${quoteCliArg(agentId)} --session ${quoteCliArg(result.key)} on the Gateway host or container using its state and configuration.`
                : "Run aether memory forget on the Gateway host or container using its state and configuration; select the owning agent with --agent and this session with --session.",
            );
          }
          if (result.worktreePreserved) {
            const preserved = result.worktreePreserved;
            runtime.error(
              `Worktree ${preserved.branch} at ${preserved.path} needs attention: ${WORKTREE_PRESERVATION_REASON_COPY[preserved.reason]}. Inspect it with ${formatCliCommand("aether worktrees list")}.`,
            );
          }
          break;
        }
