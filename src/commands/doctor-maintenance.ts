...
function assertDoctorMaintenanceInspection(
  ...
  throw new Error(
    kind === "owned" && inspection.blockMessage
      ? inspection.blockMessage
      : `Gateway service ownership or shutdown could not be verified. Run ${formatCliCommand("aether gateway status --deep", env)} and stop it through its service owner before retrying.`,
  );
}
...
export async function beginDoctorMaintenance(params: {
  ...
  root: string | null;
  runtime: RuntimeEnv;
}): Promise<{ release(): Promise<void>; finish(cfg: AetherConfig): Promise<void> } | undefined> {
  ...
  const release = async () => {
    try {
      if (repairStoresMayBeOpen) {
        ...
        const [{ closeAetherAgentDatabasesAsync }, { closeAetherStateDatabaseByPath }] =
          await Promise.all([
            import("../state/aether-agent-db.js"),
            import("../state/aether-state-db.js"),
          ]);
        ...
        await closeAetherAgentDatabasesAsync();
        closeAetherStateDatabaseByPath(resolveAetherStateSqlitePath(env));
      }
    } finally {
  ...
  try {
    ...
    const databasePath = path.resolve(resolveAetherStateSqlitePath(env));
    ...
    const { assertNoAetherAgentDatabaseLeasesReadOnly, AetherAgentDatabaseLeaseActiveError } =
      await import("../state/aether-agent-db-lease.js");
    try {
      assertNoAetherAgentDatabaseLeasesReadOnly({ env });
    } catch (error) {
      if (error instanceof AetherAgentDatabaseLeaseActiveError) {
        throw error;
      }
      // Classify unreadable state under the held owners without opening a writer.
      const { preflightAetherDatabaseSchemas } =
        await import("../state/aether-database-preflight.js");
      const { AETHER_STATE_SCHEMA_VERSION } =
        await import("../state/aether-state-db-contract.js");
      const { AETHER_AGENT_SCHEMA_VERSION } =
        await import("../state/aether-agent-db-contract.js");
    ...
    throw new Error(
      `Doctor could not enter maintenance. ${String(error)} Stop the Gateway service and other Aether processes using this state, then run ${formatCliCommand("aether vitals --fix", env)} from an independent shell.`,
      { cause: error },
    );
  }
  return {
    ...
    async finish(cfg) {
      ...
      if (!health.healthy) {
        throw new Error(
          `Doctor repaired state, but the managed Gateway did not become ready: ${renderRestartDiagnostics(health).join(" ")}. Run ${formatCliCommand("aether gateway status --deep", env)}.`,
        );
      }
