...
const LEGACY_COLLECTION_BACKUP_SCHEMA = "aether.skill-collection-backup.v1";
...
function readLegacyCollectionBackupManifest(
  ...
  return {
    ...
    sourceDirs,
    manifest: {
      schema: "aether.skill-collection-backup.v2",
      id: backupId,
  ...
  const newerBackups = await Promise.all(
    entries
      ...
      .map(async (entry) => {
        ...
        return (
          record?.schema === "aether.skill-collection-backup.v2" &&
