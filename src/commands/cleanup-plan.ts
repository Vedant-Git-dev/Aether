// Resolves cleanup inputs from current Aether config and state paths.
import {
...
function buildCleanupPlanForConfig(cfg: AetherConfig) {
...
export async function resolveCleanupPlanForRemoval(runtime: RuntimeEnv) {
  ...
  if (!snapshot.valid || workspaceWarnings.length > 0) {
    ...
    runtime.error(
      `Cannot safely remove Aether state because workspace configuration could not be resolved: ${issueSummary}. Fix the configuration and retry.`,
    );
    return undefined;
  }
