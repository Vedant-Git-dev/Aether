...
export async function messageCommand(
  ...
  const agentId = resolveAmbientOwnerAgentId(cfg, undefined, {
    surface: "message CLI",
    hint: `Run ${formatCliCommand("aether config set agents.defaults.systemAgent.agentId <id>")} with a configured agent ID.`,
  });
  ...
  if (!actionMatch) {
    throw new Error(
      `Unknown message action "${actionInput}". Use one of ${CHANNEL_MESSAGE_ACTION_NAMES.join(
        ", ",
      )}. Example: ${formatCliCommand("aether message send --channel <channel> --target <id> --text <message>")}.`,
    );
  }
