...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is Aether ${VERSION}; the running Gateway is Aether ${gatewayVersion}.`,
      "Check `aether --version`, `which aether`, and `aether gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `aether` points to the version you want, or reinstall the Gateway service from that same Aether install.",
    ].join("\n"),
    "Aether version mismatch",
  );
}
...
function noteGatewayStateDirectory(
  ...
  if (comparison.kind === "warn") {
    note(
      `${comparison.message}\nRun plugin inspection and doctor --fix with the Gateway's AETHER_STATE_DIR and AETHER_CONFIG_PATH. To change the managed service, run \`aether gateway install --force\` from the intended profile and review operator-owned service overrides.`,
      "Gateway state directory mismatch",
    );
  }
}

async function noteInstalledGatewayStateDirectory(cfg: AetherConfig, timeoutMs: number) {
    ...
    if (channelsResult.status === "fulfilled") {
      ...
      note(
        [
          ...
          `Retry: ${formatCliCommand("aether channels status --probe")}`,
        ].join("\n"),
        "Channel warnings",
      );
    }
    if (exporterResult.status === "fulfilled") {
      ...
      note(
        [
          ...
          `Retry: ${formatCliCommand("aether gateway stability --type telemetry.exporter")}`,
        ].join("\n"),
        "Telemetry exporters",
      );
    }
