...
import type { AetherConfig, GatewayAuthConfig } from "../config/config.js";
...
function hasConfiguredProviderModels(cfg: AetherConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
...
function hasStaticManifestCatalogRows(cfg: AetherConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
  return (
...
function listConfiguredModelProviders(cfg: AetherConfig): string[] {
  return Object.entries(cfg.models?.providers ?? {})
    .filter(([, provider]) => (provider.models?.length ?? 0) > 0)
    .map(([provider]) => provider);
}

function resolveSingleConfiguredProvider(cfg: AetherConfig): string | undefined {
...
export function buildGatewayAuthConfig(params: {
  ...
  if (params.mode === "trusted-proxy") {
    if (!params.trustedProxy) {
      throw new Error(
        `trustedProxy config is required when mode is trusted-proxy. Run ${formatCliCommand("aether tune --section gateway")} to configure Gateway auth interactively.`,
      );
    }
