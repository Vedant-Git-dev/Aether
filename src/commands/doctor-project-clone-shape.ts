    ...
    return [
      {
        ...
        fixHint: "Restore access to the project registry and rerun aether vitals.",
      },
    ];
  }
      ...
      findings.push({
        ...
        fixHint: [
          ...
          ...keys
            ...
            .flatMap(unset),
          "git repack -a -d",
          "Rerun aether vitals. If history or objects remain missing, recover them from the original repository.",
        ].join("\n"),
      });
    } catch {
      findings.push({
        ...
        fixHint:
          "Check the clone path, permissions, and Git installation, then rerun aether vitals.",
      });
    }
  }
  return findings;
...
export async function noteProjectCloneShape(cfg: AetherConfig): Promise<void> {
