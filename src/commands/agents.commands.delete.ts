...
export async function agentsDeleteCommand(
  ...
  if (!input) {
    failAgentsDelete(
      opts,
      runtime,
      `Agent id is required. Run ${formatCliCommand("aether agents list")} to choose one.`,
    );
    return;
  }
  ...
  if (!normalized.ok) {
    failAgentsDelete(
      opts,
      runtime,
      `Agent "${input}" not found. Run ${formatCliCommand("aether agents list")} to see configured agents.`,
    );
    return;
  }
  ...
  if (!configured && (!existingJournal || existingJournal.cleanupCompleted)) {
    failAgentsDelete(
      opts,
      runtime,
      `Agent "${agentId}" not found. Run ${formatCliCommand("aether agents list")} to see configured agents.`,
    );
    return;
  }
  ...
  return await withAgentDeletion(agentId, async (begin) => {
    ...
    if (failed.length === 0 && !purgeFailed) {
      if (deleteFiles) {
        ...
        unregisterAetherAgentDatabases({ agentId });
      }
      deletion.finish();
    }
