...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: AetherConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/aether-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: AetherConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = AetherConfig>(
  overrides: T = {} as T,
): AetherConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): AetherConfig {
