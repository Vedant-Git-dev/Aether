  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("aether vitals --fix")} to retire stale generations after the gateway restarts.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
...
export function staleManagedNpmInstallGenerationToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "Run `aether vitals --fix` to retire the stale generation for pruning after the gateway restarts.",
  };
}
