/**
 * Top-level `aether awaken` command entrypoint.
 *
...
function validatePreflightOptions(opts: OnboardOptions, runtime: RuntimeEnv): boolean {
  if (opts.mode !== undefined && opts.mode !== "local" && opts.mode !== "remote") {
    return rejectOption(
      opts,
      runtime,
      `Invalid --mode "${String(opts.mode)}". Use "local" or "remote", or run ${formatCliCommand("aether awaken")} for interactive setup.`,
    );
  }
  ...
  if (opts.nonInteractive && opts.secretInputMode === "ref") {
    const gatewayCredentials = [
      ["--gateway-password", opts.gatewayPassword, "AETHER_GATEWAY_PASSWORD"],
      ["--remote-token", opts.remoteToken, "AETHER_GATEWAY_TOKEN"],
      ["--remote-password", opts.remotePassword, "AETHER_GATEWAY_PASSWORD"],
    ] as const;
  ...
  if (opts.gatewayTokenRefEnv !== undefined) {
    ...
    if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
      return rejectOption(
        opts,
        runtime,
        "Invalid --gateway-token-ref-env. Use an environment variable name like AETHER_GATEWAY_TOKEN.",
      );
    }
    ...
    if (!process.env[gatewayTokenRefEnv]?.trim()) {
      return rejectOption(
        opts,
        runtime,
        `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("aether awaken")}.`,
      );
    }
  }
  if (opts.nonInteractive && opts.mode === "remote" && !opts.remoteUrl?.trim()) {
    return rejectOption(
      opts,
      runtime,
      `Missing --remote-url for remote mode. Example: ${formatCliCommand("aether awaken --non-interactive --accept-risk --mode remote --remote-url ws://127.0.0.1:3000")}.`,
    );
  }
  if (opts.nonInteractive && opts.mode === "remote" && opts.remoteUrl?.trim()) {
  ...
  if (
    ...
    return rejectOption(
      opts,
      runtime,
      `--import-from is required for non-interactive migration import. Run ${formatCliCommand("aether migrate list")} to choose a provider.`,
    );
  }
  return true;
}
...
async function validateResetAuthChoice(params: {
  ...
  if (!availableChoices.has(authChoice)) {
    return rejectOption(
      params.opts,
      params.runtime,
      `Auth choice "${authChoice}" was not matched to a provider setup flow. Run ${formatCliCommand("aether awaken")} to choose interactively.`,
    );
  }
 ...
 * routes it through the inference-gated Aether.
 */
...
export async function setupWizardCommand(
  ...
  if (
    ...
    rejectOption(
      normalizedOpts,
      runtime,
      `Invalid --secret-input-mode. Use "plaintext" or "ref", or run ${formatCliCommand("aether awaken")} for the interactive setup.`,
    );
    return;
  }
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    rejectOption(
      normalizedOpts,
      runtime,
      `Invalid --reset-scope. Use "config", "config+creds+sessions", or "full". Run ${formatCliCommand("aether awaken --reset --reset-scope config")} for a config-only reset.`,
    );
    return;
  }
  if (normalizedOpts.resetScope && !normalizedOpts.reset) {
    rejectOption(
      normalizedOpts,
      runtime,
      `--reset-scope requires --reset. Re-run with ${formatCliCommand(`aether awaken --reset --reset-scope ${normalizedOpts.resetScope}`)}.`,
    );
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    ...
    rejectOption(
      ...
      [
        ...
        "Read: https://docs.aether.ai/security",
        `Re-run with: ${formatCliCommand("aether awaken --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    return;
  }
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected - Aether runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.aether.ai/windows",
      ].join("\n"),
    );
  }
  ...
  const runSetupAfterOptionalReset = async () => {
    if (normalizedOpts.reset) {
      ...
      const setupBaseConfig: AetherConfig = {};
