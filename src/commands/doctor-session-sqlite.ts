...
function resolveDoctorSessionSqliteConfig(options: DoctorSessionSqliteOptions): AetherConfig {
  if (options.cfg) {
    return options.cfg;
  }
...
async function compactSqliteDatabase(
  ...
  try {
    if (options.operation === "import-finalize") {
      closeAetherAgentDatabaseByPath(resolveTargetSqlitePath(target));
    }
