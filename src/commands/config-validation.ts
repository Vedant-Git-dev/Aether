...
function validateConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Aether config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
      isPluginPackagingRuntimeOutputInvalidConfigSnapshot(snapshot)
        ? `Fix: ${formatPluginPackagingRuntimeOutputRecoveryHint()}`
        : `Fix: ${formatCliCommand("aether vitals --fix")}`,
    );
    runtime.error(`Inspect: ${formatCliCommand("aether config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid Aether config, or null after reporting validation errors. */
