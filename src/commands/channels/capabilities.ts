// Implements `aether channels capabilities` account capability/probe reporting.
import {
...
async function resolveCapabilitiesRuntimeConfig(config: AetherConfig, runtime: RuntimeEnv) {
  return (
...
export async function channelsCapabilitiesCommand(
  ...
  if ((!rawChannel || rawChannel === "all") && (opts.account || rawTarget)) {
    ...
    const message = `${option} requires a specific --channel. Run ${formatCliCommand("aether channels list")} to choose one.`;
