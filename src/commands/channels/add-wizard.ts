// Guided channel-setup wizard flow shared by `aether channels add` (clack
...
import { readConfigFileSnapshotForWrite, type AetherConfig } from "../../config/config.js";
...
export async function runChannelsAddWizardFlow(params: ChannelsAddWizardFlowParams): Promise<void> {
  ...
  const commitWizardConfig = async (config: AetherConfig) => {
...
export async function runChannelsSetupWizard(
  ...
  if (snapshot.exists && !snapshot.valid) {
    throw new Error(
      "Aether config is invalid; run `aether vitals --fix`, then retry channel setup.",
    );
  }
