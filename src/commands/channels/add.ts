...
// Implements guided and non-interactive `aether channels add` account setup.
...
async function channelsAddCommandImpl(
  ...
  if (useWizard) {
    ...
    if (!isTerminalInteractive()) {
      runtime.error(
        "Interactive channel setup requires a TTY. Use `aether channels add --channel <id> --use-env` or pass the channel's credential flags for non-interactive setup.",
      );
      runtime.exit(1);
      return;
    }
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run aether vitals --fix, then retry aether channels add.`
  ...
  if (!plugin) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("aether channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
