// Implements `aether channels list` across runtime accounts, local config, and catalog-only entries.
...
export async function channelsListCommand(
  ...
  // there are no configured channels even though aether.json has one.
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `aether status` or `aether models list`.",
    ),
  );
