...
const DIRECT_PROBE_HOST = "docs.aether.ai";
...
async function collectWebFetchProxyDiagnostic(params: {
  ...
  return [
    ...
    `  ${formatCliCommand("aether config set tools.web.fetch.useTrustedEnvProxy true")}`,
    "- Keep the opt-in disabled for untrusted proxies; enabling it lets the proxy resolve DNS after Aether's hostname checks.",
  ].join("\n");
}
