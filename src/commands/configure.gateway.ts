...
import { parseIpAddressOrCidr } from "@aether/net-policy/ip";
...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "AETHER_GATEWAY_TOKEN",
          placeholder: "AETHER_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like AETHER_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Aether will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Aether trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.aether.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
