...
export function throwStartupMigrationGuardRejected(): never {
  throw new Error(
    "Aether startup migrations were skipped because the selected config changed during startup; refusing to report the gateway ready. Retry startup so the new config can be validated.",
  );
}

export function throwStartupMigrationIdentityChanged(): never {
  throwStartupMigrationRefusal(
    "Aether migration inputs changed during startup; refusing to report the gateway ready. Restart Aether so state migrations run against the final config and plugin inventory.",
  );
}
...
async function describeLiveGatewayOwnerStartupBlocker(
  ...
  return `Another gateway (pid ${activeGateway.pid}) already owns this state directory; refusing to run automatic startup migrations or report the gateway ready. Stop it with "aether gateway stop" (or select a different AETHER_STATE_DIR), then retry startup.`;
}
