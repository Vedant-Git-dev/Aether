...
export function repairReservedIncognitoSessionKeys(params: {
  ...
  const sharedDatabase = params.apply ? openAetherStateDatabase({ env: params.env }) : undefined;
  ...
  for (const { target, databaseOptions } of targets) {
    const wasOpen = isAetherAgentDatabaseOpen(target.sqlitePath);
    try {
      runAetherAgentWriteTransaction(
      ...
      if (!wasOpen) {
        closeAetherAgentDatabaseByPath(target.sqlitePath);
      }
    }
  }
  runAetherStateWriteTransaction(
...
function listReservedIncognitoKeys(database: DatabaseSync): string[] {
  const db = getNodeSqliteKysely<AetherAgentKyselyDatabase>(database);
