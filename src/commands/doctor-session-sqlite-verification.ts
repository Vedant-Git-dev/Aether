...
import { inspectAetherAgentDatabaseOwner } from "../state/aether-agent-db.js";
import {
...
export function createRecoveryDestinationVerifier(stateDir: string) {
  ...
  return (refs: Iterable<{ target: SessionSqliteMigrationTargetManifest }>) => {
    for (const { target } of refs) {
      ...
      if (!expected) {
        const owner = inspectAetherAgentDatabaseOwner(target.sqlitePath);
...
export function verifyHistoricalMigrationArtifact(params: {
  ...
  const verified = withAetherAgentDatabaseReadOnly(
    (database) => {
