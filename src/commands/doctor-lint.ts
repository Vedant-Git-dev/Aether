...
import { closeAetherStateDatabaseByPath } from "../state/aether-state-db-cache.js";
...
async function withReadOnlyPluginStateSnapshot<T>(
  ...
  const sourceDatabasePath = resolveAetherStateSqlitePath(sourceEnv);
  let cleanup: () => boolean;
  ...
  try {
    if (fs.existsSync(sourceDatabasePath)) {
      ...
      privateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "aether-doctor-lint-state-"));
      cleanup = () => {
        try {
  ...
  const privateStateDir = path.join(privateRoot, "aether-state");
  const privateDatabasePath = resolveAetherStateSqlitePath({
    ...sourceEnv,
    AETHER_STATE_DIR: privateStateDir,
  });
  ...
  try {
    ...
    const privateEnv = {
      ...sourceEnv,
      AETHER_CONFIG_PATH: sourceConfigPath,
      AETHER_STATE_DIR: privateStateDir,
    };
  ...
  try {
    ...
    closeAetherStateDatabaseByPath(privateDatabasePath);
    if (!cleanup()) {
...
async function withDoctorLintStateEnv<T>(
  ...
  const overrides = {
    AETHER_CONFIG_PATH: resolveConfigPath(env, stateDir),
