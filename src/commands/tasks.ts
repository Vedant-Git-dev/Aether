...
export async function tasksListCommand(
  ...
  if (tasks.length === 0) {
    runtime.log(
      `No background tasks found. Run ${formatCliCommand("aether tasks audit")} to check for stale task state.`,
    );
    return;
  }
  const rich = isRich();
...
export async function tasksMaintenanceCommand(
  ...
  if (!opts.apply) {
    runtime.log("Dry run only. Re-run with `aether tasks maintenance --apply` to write changes.");
  }
}
