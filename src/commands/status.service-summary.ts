...
function normalizeServiceWrapperPath(
  ...
  const wrapperPath = command?.environment?.[AETHER_WRAPPER_ENV_KEY]?.trim();
...
export async function readServiceStatusSummary(
  ...
  try {
    ...
    const managedByAether = state.installed;
    // A running unmanaged process still counts as installed for status display.
    const externallyManaged = !managedByAether && state.running;
    const installed = managedByAether || externallyManaged;
