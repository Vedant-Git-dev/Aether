...
import { AETHER_AGENT_RUNTIME_ID } from "../agents/agent-runtime-id.js";
...
function usesIdentityHeadersWithoutMachineCredentials(cfg: AetherConfig): boolean {
...
function lacksNodeOnboardingUrl(cfg: AetherConfig): boolean {
...
function lacksNodeOnboardingPlugin(cfg: AetherConfig): boolean {
...
function lacksDeviceCapableRuntimeRoute(cfg: AetherConfig): boolean {
  ...
  return listAgentIds(cfg).every((agentId) => {
    ...
    if (runtime === AETHER_AGENT_RUNTIME_ID) {
      return false;
    }
...
export function collectNodeHostingPreconditionFindings(
  ...
  if (lacksNodeOnboardingPlugin(cfg)) {
    findings.push({
      ...
      severity: "warning",
      message:
        "The device-pair plugin is not enabled; node onboarding join codes and aether connect are unavailable.",
  ...
  if (lacksNodeOnboardingUrl(cfg)) {
    findings.push({
      ...
      fixHint:
        "If an edge proxy fronts node onboarding, allow /j/* and /__aether__/worker without edge identity auth, and preserve WebSocket upgrade on /__aether__/worker. Both routes enforce their own credentials.",
    });
  }
  return findings;
}
