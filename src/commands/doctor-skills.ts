...
function formatUnavailableSkillDoctorLines(
  ...
  if (includeDisableHint) {
    lines.push(`Disable unused skills: ${formatCliCommand("aether vitals --fix")}`);
  }
  lines.push(
    `Inspect details: ${formatCliCommand("aether skills check --agent <id>")} or ${formatCliCommand("aether skills info <name> --agent <id>")}`,
  );
  return lines;
}
