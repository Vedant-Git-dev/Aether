...
// Implements docs link/search output for `aether docs`.
import { readResponseWithLimit } from "../infra/http-body.js";
import { type RuntimeEnv, writeRuntimeJson } from "../runtime.js";

const SEARCH_API = "https://docs.aether.ai/api/search";
  ...
  if (!query) {
    if (options.json) {
      writeRuntimeJson(runtime, {
        query: null,
        url: "https://docs.aether.ai/",
        results: [],
      });
      return;
    }
    const docs = formatDocsLink("/", "docs.aether.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('aether docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.aether.ai/");
      runtime.log(`Search: ${formatCliCommand('aether docs "your query"')}`);
    }
    return;
  }
