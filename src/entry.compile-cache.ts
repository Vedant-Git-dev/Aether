...
function shouldEnableAetherCompileCache(params: {
...
function resolveAetherCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
type AetherCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
...
type AetherCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void | Promise<void>;
};

function buildAetherCompileCacheRespawnPlan(params: {
  currentFile: string;
  installRoot: string;
  compileCacheDir?: string;
  env?: NodeJS.ProcessEnv;
}): AetherCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
...
export async function respawnWithoutAetherCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
  env?: NodeJS.ProcessEnv;
  prepareWriteError?: () => Promise<(message: string) => void | Promise<void>>;
}): Promise<boolean> {
  const plan = buildAetherCompileCacheRespawnPlan({
  ...
  runAetherCompileCacheRespawnPlan(
    plan,
    writeError
      ? {
          spawn,
...
function runAetherCompileCacheRespawnPlan(
  plan: AetherCompileCacheRespawnPlan,
  runtime: AetherCompileCacheRespawnRuntime = {
    spawn,
    attachChildProcessBridge,
    exit: process.exit.bind(process) as (code?: number) => never,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      return runtime.writeError(
        `[aether] Failed to respawn CLI without compile cache: ${
...
export function enableAetherCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableAetherCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveAetherCompileCacheDirectory(params));
  } catch {
