...
export function resolveSessionMutationAuthorization(params: {
  ...
  let cachedCfg: AetherConfig | undefined;
  const getCfg = (): AetherConfig => (cachedCfg ??= params.context.getRuntimeConfig());
  ...
  return {
    ...
    authorization: (() => {
      ...
      const assertTalkTargetCurrent = (cfg: AetherConfig) => {
