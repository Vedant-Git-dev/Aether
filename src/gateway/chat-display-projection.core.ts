...
import { normalizeLowercaseStringOrEmpty as normalizeErrorSignal } from "@aether/normalization-core/string-coerce";
...
export function projectChatDisplayMessagesWithState(
  ...
  const projectedActivity = messages.map((message) => {
    ...
    return {
      ...
      __aether: {
        ...asOptionalRecord(asOptionalRecord(message)?.["__aether"]),
        runId: activity.details.runId,
      },
