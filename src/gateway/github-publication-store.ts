...
import { ensureGitHubPublicationSchema } from "../state/aether-state-db-schema-additive.js";
...
export function ensureGitHubPublicationStore(): void {
  ensureGitHubPublicationSchema(openAetherStateDatabase().db);
}

export function hasGitHubPublicationStore(): boolean {
  return tableExists(openAetherStateDatabase().db, "github_publication_requests");
}
...
export function listGitHubPublicationsForClaim(
  ...
  const db = openAetherStateDatabase().db;
