...
function resolveGatewayUrlOverrideSource(urlSource: string): "cli" | "env" | undefined {
  ...
  if (urlSource === "env AETHER_GATEWAY_URL") {
    return "env";
  }
  return undefined;
}
...
export function ensureExplicitGatewayAuth(params: {
  ...
  const sourceHint =
    params.urlOverrideSource === "env"
      ? "Set AETHER_GATEWAY_TOKEN or AETHER_GATEWAY_PASSWORD alongside AETHER_GATEWAY_URL; config credentials are intentionally not reused."
...
export function resolveGatewayUrlOverride(params: {
  ...
  const envUrl = trimToUndefined((params.env ?? process.env).AETHER_GATEWAY_URL);
