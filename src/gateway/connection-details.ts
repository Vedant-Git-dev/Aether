...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: AetherConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    ...
    options.localPortOverride !== undefined
      ? undefined
      : normalizeOptionalString(process.env.AETHER_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.AETHER_ALLOW_INSECURE_PRIVATE_WS === "1";
