...
const pendingAuthorizations = resolveGlobalMap<string, PendingAuthorization>(
  Symbol.for("aether.providerBrowserAuthorizations"),
  (pending) => {
...
function respond(res: ServerResponse, status: number, message: string): void {
  ...
  res.end(
    `<!doctype html><html lang="en"><meta charset="utf-8"><title>Provider sign-in</title><body><main><h1>${message}</h1><p>Return to Aether for the sign-in result.</p></main></body></html>`,
  );
}
