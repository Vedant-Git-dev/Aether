...
import { finiteSecondsToTimerSafeMilliseconds } from "@aether/normalization-core/number-coercion";
...
async function finalizeCronCompletionAnnouncement(params: {
  ...
  deps: CliDeps;
  resolveCronAgent: (requested?: string | null) => { agentId: string; cfg: AetherConfig };
...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = env.AETHER_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;
  ...
  const findAgentEntry = (cfg: AetherConfig, agentId: string) =>
    listAgentEntries(cfg).find((entry) => normalizeAgentId(entry.id) === agentId);

  const hasConfiguredAgent = (cfg: AetherConfig, agentId: string) =>
