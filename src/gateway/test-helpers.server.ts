...
import { rawDataToString } from "@aether/gateway-client/websocket-data";
import {
...
import { closeAetherAgentDatabasesForTest } from "../state/aether-agent-db.js";
import {
...
function publishGatewayTestConfig(
  config: AetherConfig = loadGatewayTestConfig(),
): AetherConfig {
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.AETHER_CONFIG_PATH) {
    configPaths.add(process.env.AETHER_CONFIG_PATH);
  }
  if (process.env.AETHER_STATE_DIR) {
    configPaths.add(path.join(process.env.AETHER_STATE_DIR, "aether.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aether-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AETHER_STATE_DIR = path.join(tempHome, ".aether");
  delete process.env.AETHER_CONFIG_PATH;
  delete process.env.AETHER_AGENT_DIR;
}

function applyGatewaySkipEnv() {
  process.env.AETHER_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.AETHER_SKIP_GMAIL_WATCHER = "1";
  process.env.AETHER_SKIP_CANVAS_HOST = "1";
  process.env.AETHER_SKIP_CHANNELS = "1";
  process.env.AETHER_SKIP_PROVIDERS = "1";
  process.env.AETHER_SKIP_CRON = "1";
  process.env.AETHER_TEST_MINIMAL_GATEWAY = "1";
  process.env.AETHER_DISABLE_BUNDLED_PLUGINS = "1";
  process.env.AETHER_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "aether-test-no-bundled-extensions")
    : "aether-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  delete process.env.AETHER_GATEWAY_TOKEN;
  resetTaskRegistryForTests({ persist: false });
  resetTaskFlowRegistryForTests({ persist: false });
  const stateDir = process.env.AETHER_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".aether-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".aether-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".aether-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>aether-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (tempHome) {
    ...
    closeAetherAgentDatabasesForTest(tempHome);
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__aetherTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__aetherTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  gatewayFixtureLifetime.assertAdmission();
  const envSnapshot = captureEnv(["AETHER_GATEWAY_TOKEN"]);
  const prev = process.env.AETHER_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AETHER_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.AETHER_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
