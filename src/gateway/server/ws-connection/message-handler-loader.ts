    ...
    .catch((error: unknown) => {
      ...
      if (staleInstall) {
        ...
        params.logWsControl.error(
          `failed to load ws message handler because the Aether installation changed while the Gateway was running conn=${params.connId}; run: ${staleInstall.restartCommand}; error: ${formattedError}`,
        );
