...
type CallGatewayBaseOptions = {
   ...
   * Bypasses AETHER_GATEWAY_URL and AETHER_GATEWAY_PORT for this call only.
   */
  localPortOverride?: number;
  /** Keep a caller-supplied config target authoritative over AETHER_GATEWAY_URL. */
...
const defaultGetRuntimeConfig = async (): Promise<AetherConfig> =>
...
async function loadGatewayConfig(): Promise<AetherConfig> {
 ...
 */
async function loadGatewayConfigForExplicitConnection(): Promise<AetherConfig> {
  try {
...
function loadGatewayConfigForConnectionDetails(): AetherConfig {
...
function resolveGatewayPortValue(config?: AetherConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: AetherConfig) {
...
function resolveGatewayCallTimeout(timeoutValue: unknown): {
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.AETHER_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(isVitestRuntimeEnv() && process.env.AETHER_TEST_HANDSHAKE_TIMEOUT_MS);
...
function createGatewayUnreachableTransportError(params: {
  ...
  return new GatewayTransportError({
    ...
    message: [
      ...
      "Start it with `aether gateway run` or check `aether gateway status`.",
    ...
    if (!storedAuth?.token && deviceAuthScope) {
      throw new GatewayStoredDeviceAuthUnavailableError(
        [
          ...
          `Run \`aether tui --url ${deviceAuthScope}\` to send a pairing request, approve it in that gateway's Control UI (Settings -> Devices) or run \`aether devices approve --latest\` on the gateway host, then retry.`,
        ].join("\n"),
      );
    }
  }
