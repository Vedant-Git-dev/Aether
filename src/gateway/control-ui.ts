...
import { detectMime, kindFromMime } from "@aether/media-core/mime";
import {
  asDateTimestampMs,
...
const CONTROL_UI_NAMESPACE_PREFIX = "/__aether__/";
...
function respondControlUiAssetsUnavailable(
  ...
  const message = options?.preparing
    ...
    : options?.failed
      ? "Control UI assets could not be prepared. Check the Gateway logs or run `aether vitals --fix`."
...
// `/__aether__/`, so a browser that opens the default entry infers
// `/__aether__` as its base path (see `inferBasePathFromPathname`) and fetches
// `/__aether__/control-ui-config.json`. Accept that namespaced alias so the
...
// Single-underscore `/__aether` prefix used by the pre-base-path-relative
...
// `/__aether/control-ui-config.json`, so current main and the v2026.6.1
// release serve and document that exact path under an empty base path.
const LEGACY_CONTROL_UI_NAMESPACE_PREFIX = "/__aether";

// The old documented no-base-path bootstrap endpoint
// (`/__aether/control-ui-config.json`, single underscore). It is derived from
// the legacy `/__aether` namespace joined with the canonical config constant
 ...
 * single-underscore suffix `${basePath}/__aether/control-ui-config.json` that
 ...
 * the default-namespace alias `/__aether__/control-ui-config.json`, which is
 * what the default `/__aether__/` entry requests after inferring its base path
...
function matchesControlUiBootstrapConfigPath(pathname: string, basePath: string): boolean {
  ...
  // `/__aether` namespace, so joining it with the base path yields
  // `${basePath}/__aether/control-ui-config.json` (or the bare legacy path
  ...
  // The default `/__aether__/` namespace alias only applies when no base path
