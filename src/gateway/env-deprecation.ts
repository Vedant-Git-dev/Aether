...
// Emits a one-shot notice for ignored pre-Aether environment prefixes.
...
const LEGACY_ENV_PREFIXES = ["AETHER_", "MOLTBOT_"] as const;
type LegacyEnvPrefix = (typeof LEGACY_ENV_PREFIXES)[number];

let warned = false;

/** Emits a one-time warning when ignored legacy AETHER_/MOLTBOT_ env vars are present. */
export function warnLegacyAetherEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but Aether only reads AETHER_* names now.`,
      "Rename them by replacing the legacy prefix with AETHER_; the old names are ignored.",
    ].join("\n"),
    { code: "AETHER_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}
