...
import { parseControlUiSessionPath } from "@aether/session-url-contract/parse";
...
export function serveControlUiShareDocument(
  ...
  const title = session.namespace === "dashboard" ? "Aether dashboard" : "Aether session";
  const description = "Open this shared link in Aether. Access to the session is required.";
...
<meta property="og:image:alt" content="Aether logo and lobster mascot">
...
<img src="${image}" width="1200" height="630" alt="Aether logo and lobster mascot">
<h1>${title}</h1><p>${description}</p><a href="${target}">Open ${title === "Aether dashboard" ? "dashboard" : "session"}</a>
</main></body></html>`;
  res.statusCode = 200;
