...
/** Attach Aether metadata to a transcript message without dropping existing metadata. */
export function attachAetherTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__aether"] &&
    typeof record["__aether"] === "object" &&
    !Array.isArray(record["__aether"])
      ? (record["__aether"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __aether: {
...
function readTranscriptMessageSenderIsOwner(message: unknown): boolean | undefined {
  const aether = asOptionalRecord(asOptionalRecord(message)?.["__aether"]);
  const value = aether?.senderIsOwner;
...
export function projectSessionMessagePayload(params: {
  ...
  const rawMessage = attachAetherTranscriptMeta(params.message, {
...
export function projectTranscriptEntryMessage(
  ...
  if (record.message) {
    ...
    return attachAetherTranscriptMeta(record.message, {
  ...
  if (record.type === "custom_message") {
    return attachAetherTranscriptMeta(
      {
        role: "custom",
  ...
  const compactionIdentity =
    kind === "compaction" ? asOptionalRecord(record["__aether"]) : undefined;
  return {
    role: "system",
