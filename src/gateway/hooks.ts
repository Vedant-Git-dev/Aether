...
export function resolveHooksConfig(cfg: AetherConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: AetherConfig, defaultAgentId?: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-aether-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-aether-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-aether-idempotency-key"]) ||
