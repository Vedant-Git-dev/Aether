...
function projectChatHistoryMediaReference(value: unknown): string | undefined {
  ...
  if (/^\/(?:api\/chat\/media\/outgoing|media|__aether__)\//u.test(reference)) {
...
export function sanitizeChatHistoryContentBlock(
  ...
  if ("aetherReasoningReplay" in entry) {
    delete entry.aetherReasoningReplay;
    changed = true;
  }
...
function projectAssistantCommentaryFallbacks(message: unknown, maxChars: number): unknown[] {
  ...
  const transcriptMeta = readRecord(entry["__aether"]);
    ...
    return [
      {
        ...
        aetherStreamFallback: {
          replacementText: projected.text,
          source: "segment",
          itemId,
        },
        ...(projectedMeta ? { __aether: projectedMeta } : {}),
      },
    ];
  });
}
...
function projectWorkspaceConflictDetails(
  ...
  if (
    ...
    !/^refs\/aether\/worker-results\/[A-Za-z0-9-]+$/u.test(details.stagedResultRef) ||
...
export function sanitizeChatHistoryMessage(
  ...
  const aetherMeta = readRecord(entry["__aether"]);
  if (aetherMeta && ("upstreamUserText" in aetherMeta || "media" in aetherMeta)) {
    ...
    const projectedMeta = { ...aetherMeta };
    ...
    if (Object.keys(projectedMeta).length > 0) {
      entry["__aether"] = projectedMeta;
    } else {
      delete entry["__aether"];
    }
    changed = true;
  }
  ...
  if (truncated) {
    ...
    const meta = readRecord(entry["__aether"]);
    entry["__aether"] = {
      ...meta,
      truncated: true,
