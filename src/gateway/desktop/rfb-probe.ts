...
/** Maps standard RFB security numbers into the credential UX supported by Aether. */
export function classifyRfbSecurity(
  ...
  for (const securityType of securityTypes) {
    ...
    if ([6, 16, 19, 22, 113].includes(securityType)) {
      // noVNC supports these schemes and stops here, but Aether has no matching credential UX.
