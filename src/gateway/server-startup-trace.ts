...
export function createGatewayStartupTrace(log: GatewayLogger, startedAt = performance.now()) {
  const logEnabled = isTruthyEnvValue(process.env.AETHER_GATEWAY_STARTUP_TRACE);
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.AETHER_DIAGNOSTICS_EVENT_LOOP);
