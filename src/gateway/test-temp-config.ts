...
/** Writes a temp Aether config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.AETHER_CONFIG_PATH;

  const testConfig = canonicalizeTempConfigForTest(params.cfg) as AetherConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "aether-test-config-"));
  const configPath = path.join(dir, "aether.json");

  process.env.AETHER_CONFIG_PATH = configPath;
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.AETHER_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
