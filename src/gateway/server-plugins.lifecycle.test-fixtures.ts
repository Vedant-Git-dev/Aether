...
export const INSTANCE_BINDING_PROBE_KEY = Symbol.for("aether.test.gatewayInstanceBindingProbe");
...
export async function writeInstanceBindingProbePlugin(bundledRoot: string): Promise<void> {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({
      ...
      type: "commonjs",
      main: "index.js",
      aether: { extensions: ["./index.js"] },
      peerDependencies: { aether: ">=2026.1.1" },
    })}\n`,
  );
  await fs.writeFile(
  ...
  id: "instance-binding-probe",
  register(api) {
    const coordinator = globalThis[Symbol.for("aether.test.gatewayInstanceBindingProbe")];
