...
// Translates OpenAI chat requests to Aether agent runs and SSE/JSON responses.
import { randomUUID } from "node:crypto";
import type { IncomingMessage, ServerResponse } from "node:http";
import { STREAM_ERROR_FALLBACK_TEXT } from "@aether/ai/internal/shared";
...
import { avoidTrailingHighSurrogateBreak } from "@aether/normalization-core/utf16-slice";
import { z } from "zod";
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "aether";
  ...
  if (!stream) {
    try {
      ...
      const content = resolveAssistantResultText(result) || "No response from Aether.";
  ...
  const maybeFinalize = () => {
    ...
    queueMicrotask(() => {
      ...
      const text = resolveAssistantTextCompletion({
        ...
        fallbackText: finalToolCalls ? "" : "No response from Aether.",
      });
