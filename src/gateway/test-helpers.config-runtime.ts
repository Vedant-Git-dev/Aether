...
import type { ConfigFileSnapshot, AetherConfig } from "../config/types.js";
...
const resolveConfigPath = () => path.join(testConfigRoot.value, "aether.json");
...
const composeTestConfig = (baseConfig: Record<string, unknown>) => {
  ...
  const defaults = {
    ...
    workspace: path.join(os.tmpdir(), "aether-gateway-test"),
    ...fileDefaults,
  ...
  return migratePersistedImplicitMainRoster(composed).config as AetherConfig;
};

export function loadGatewayTestConfig(): AetherConfig {
...
export function createGatewayConfigOverrides(actual: GatewayConfigRuntime): GatewayConfigOverrides {
  ...
  return {
    ...
    applyConfigOverrides: (cfg: AetherConfig) =>
