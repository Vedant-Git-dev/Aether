...
// Bridges /v1/embeddings requests to configured Aether memory providers.
...
import {
  ...
  isAetherAgentModelId,
  isUnknownGatewayAgentError,
  resolveAgentIdForRequest,
  resolveOpenAiCompatibleHttpOperatorScopes,
} from "./http-utils.js";

// OpenAI-compatible `/v1/embeddings` bridge. It maps Aether agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (!isAetherAgentModelId(requestModel)) {
    sendInvalidRequest(res, "Invalid `model`. Use `aether` or `aether/<agentId>`.");
    return true;
  }
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-aether-model")) ||
