    ...
    if (preservedFunnel) {
      params.logTailscale.warn(
        `external Tailscale Funnel for port ${params.port} remains active only for plugin-authenticated webhook routes; Gateway-authenticated routes reject its unattributable ingress. ` +
          "First configure a durable gateway password (gateway.auth.password or AETHER_GATEWAY_PASSWORD) and set gateway.auth.mode=password, " +
          "then run `aether config set gateway.tailscale.mode funnel` and `aether config unset gateway.tailscale.preserveFunnel`; " +
          "see https://docs.aether.ai/gateway/tailscale#public-internet-funnel-%2B-shared-password",
      );
      return null;
    }
  }
