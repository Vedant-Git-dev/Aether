...
import type { loadAetherPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "AETHER_SKIP_STARTUP_MODEL_PREWARM";
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadAetherPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  cfg: AetherConfig;
  getConfig?: () => AetherConfig | Promise<AetherConfig>;
  isCurrent?: () => boolean;
...
type StartupExternalAuthHydrationDeps = {
  listAgentIds: (cfg: AetherConfig) => string[];
  resolveAgentDir: (cfg: AetherConfig, agentId: string) => string;
  collectConfiguredRefs: (cfg: AetherConfig, agentId: string) => readonly { value: string }[];
  hydrate: (cfg: AetherConfig, agentDir: string, providers: readonly string[]) => void;
};
...
async function hydrateConfiguredExternalCliAuth(params: {
  ...
  const deps: StartupExternalAuthHydrationDeps =
    ...
    (await Promise.all([
      ...
      hydrate: (cfg: AetherConfig, agentDir: string, providers: readonly string[]) => {
...
export async function startGatewaySidecars(params: {
  cfg: AetherConfig;
  getModelRuntimeConfig?: () => AetherConfig;
  pluginMetadataSnapshot?: PluginMetadataSnapshot;
  pluginRegistry: ReturnType<typeof loadAetherPlugins>;
    ...
    const channelStart = skipChannels
      ? measureStartup(params.startupTrace, "sidecars.channel-skip", () =>
          params.logChannels.info(
            "skipping channel start (AETHER_SKIP_CHANNELS=1 or AETHER_SKIP_PROVIDERS=1)",
          ),
        )
      : shouldStartChannels
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@aether/acp-core/runtime/session-identifiers"),
          ]);
        if (params.shouldCreatePostReadySidecars?.() === false) {
          return;
        }
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: AetherConfig;
    activationSourceConfig: AetherConfig;
    ...
    getCurrentActivationSourceConfig?: () => AetherConfig | null;
    getCronService?: () => PluginServiceCronHost | null | undefined;
    onChannelsStarted?: () => Awaitable<void>;
    onPluginServices?: (pluginServices: PluginServicesHandle | null) => void;
