...
export async function startGatewayDiscovery(params: {
  ...
  const advertise = async (generation: DiscoveryGeneration) => {
    ...
    const localEnabled =
      generation.mode !== "off" &&
      !isTruthyEnvValue(process.env.AETHER_DISABLE_BONJOUR) &&
    ...
    const context = {
      ...
      sshPort: minimal ? undefined : (parseTcpPort(process.env.AETHER_SSH_PORT) ?? undefined),
      tailnetDns,
    ...
    const advertiseTimeoutMs =
      clampTimerTimeoutMs(
        parseStrictPositiveInteger(
          process.env.AETHER_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim(),
        ),
      ) ?? 5_000;
    for (;;) {
