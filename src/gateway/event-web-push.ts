...
import { normalizeOptionalString } from "@aether/normalization-core";
...
import { isTranscriptOnlyAetherAssistantMessage } from "../shared/transcript-only-aether-assistant.js";
...
function resolveEventWebPushNotification(
  ...
  if (event === "question.requested") {
    ...
    return {
      ...
      title: "Aether needs an answer",
      body: "An agent has a question for you.",
      tag: `aether-question-${id}`,
...
function preferenceFor(target: CurrentWebPushTarget, stateDir?: string) {
  ...
  const user = profileId
    ? getUserPreferences(
        profileId,
        [WEB_PUSH_USER_PREFERENCES_KEY],
        stateDir ? { env: { ...process.env, AETHER_STATE_DIR: stateDir } } : {},
      )[WEB_PUSH_USER_PREFERENCES_KEY]
    : undefined;
