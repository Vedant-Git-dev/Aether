...
export const AETHER_MODEL_ID = "aether";
/** Default OpenAI-compatible model alias that targets the default Aether agent. */
export const AETHER_DEFAULT_MODEL_ID = "aether/default";
...
class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-aether-session-key` cannot use reserved internal session namespaces.");
...
class InvalidGatewayModelError extends Error {
  constructor() {
    super("Invalid `model`. Use `aether` or `aether/<agentId>`.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-aether-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-aether-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === AETHER_MODEL_ID || lowered === AETHER_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^aether[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Checks Aether routing-model syntax without resolving fleet ownership. */
export function isAetherAgentModelId(model: string | undefined): boolean {
  const raw = model?.trim();
  if (!raw) {
    return false;
  }
  ...
  return (
    /^aether[:/][a-z0-9][a-z0-9_-]{0,63}$/i.test(raw) ||
    /^agent:[a-z0-9][a-z0-9_-]{0,63}$/i.test(raw)
  );
}

/** Validates and resolves the `x-aether-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !isAetherAgentModelId(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `aether` or `aether/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-aether-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-aether-model`." };
  }
...
export function resolveAgentIdForRequest(params: {
  ...
  const cfg = getRuntimeConfig();
  if (params.model?.trim() && !isAetherAgentModelId(params.model)) {
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-aether-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-aether-message-channel")) ??
