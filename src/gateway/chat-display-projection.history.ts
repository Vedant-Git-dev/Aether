...
import { asOptionalRecord as readRecord } from "@aether/normalization-core/record-coerce";
...
import { isAetherDeliveryMirrorAssistantMessage } from "../shared/transcript-only-aether-assistant.js";
...
function readTtsSupplementMarker(
  ...
  const marker = readRecord(message.aetherTtsSupplement);
  if (!marker) {
    return undefined;
  }
  const textSha256 =
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__aether"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function attachProjectedTurnBoundary(message: Record<string, unknown>): Record<string, unknown> {
  const metadata = readRecord(message["__aether"]);
...
function aetherAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "aether" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    aetherAssistantModel(previousVisible) !== "acp-runtime" ||
    aetherAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
...
function isDuplicateChannelFinalDeliveryMirror(
  ...
  if (!previousVisible || !isAetherDeliveryMirrorAssistantMessage(current)) {
    return false;
  }
  const deliveryMirror = readRecord(current.aetherDeliveryMirror);
  ...
  if (isAetherDeliveryMirrorAssistantMessage(previousVisible)) {
    return false;
  }
  if (isProjectedSessionsSendForwardedMessage(previousVisible)) {
    return false;
  }
  const previousMeta = readRecord(previousVisible["__aether"]);
