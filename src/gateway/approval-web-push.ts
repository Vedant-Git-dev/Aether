...
function approvalPreferences(params: {
  ...
  const storedUser = profileId
    ? getUserPreferences(
        profileId,
        [WEB_PUSH_USER_PREFERENCES_KEY],
        params.stateDir ? { env: { ...process.env, AETHER_STATE_DIR: params.stateDir } } : {},
      )[WEB_PUSH_USER_PREFERENCES_KEY]
    : undefined;
...
function approvalNotificationCopy(params: {
  ...
  if (params.terminal) {
    return {
      title: `${label}Aether approval updated`,
      body:
  ...
  return {
    title: `${label}Aether approval requested`,
    body:
      params.preferences.detailLevel === "private"
        ? "Open Aether to review this request."
        : `Open Aether to review an approval${agent}.`,
  };
}
...
function approvalWebPushTag(approvalId: string): string {
  return `aether-approval-${approvalId}`;
}

function approvalWebPushTopic(approvalId: string): string {
  return createHash("sha256")
    .update(`aether-approval:${approvalId}`)
    .digest("base64url")
    .slice(0, 32);
}
