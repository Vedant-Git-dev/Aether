...
import { asPositiveSafeInteger } from "@aether/normalization-core/number-coercion";
...
function buildMessageToolVisibleReplyMirror(
  ...
  const sourceMessageSeq = asPositiveSafeInteger(readRecord(pending.anchor["__aether"])?.seq);
  const deliveryMirror = [pending.deliveryMirrorAnchor, pending.completionAnchor].find((message) =>
    isAetherDeliveryMirrorAssistantMessage(message),
  );
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__aether"]);
  if (transcriptMeta) {
    mirror["__aether"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readMessageToolDeliveryMirrorText(message: Record<string, unknown>): string | undefined {
  ...
  if (!isAetherDeliveryMirrorAssistantMessage(message)) {
    return undefined;
  }
...
function readMessageToolDeliveryMirrorCallId(message: Record<string, unknown>): string | undefined {
  ...
  return normalizeOptionalString(readRecord(message.aetherDeliveryMirror)?.toolCallId);
}
