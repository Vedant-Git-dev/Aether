...
// Deduplicates external history messages against local Aether transcripts.
...
import { isAetherCliImageCachePath } from "../agents/embedded-agent-runner/run/images.media-refs.js";
...
function stripTrailingCliImageMentions(text: string): {
  ...
  while (end > 0) {
    ...
    if (!line.startsWith("@") || !isAetherCliImageCachePath(line.slice(1))) {
      break;
    }
    end -= 1;
  }
...
function isClaudeCliImportedUserMessage(message: unknown, role: string | undefined): boolean {
  ...
  const meta = asOptionalRecord(asOptionalRecord(message)?.["__aether"]);
...
function resolveImportedExternalIdentityKey(message: unknown): string | undefined {
  ...
  const rawMeta = (message as { __aether?: unknown })["__aether"];
...
function projectImportedIdentity(localMessage: unknown, importedMessage: unknown): unknown {
  ...
  const importedMeta = asOptionalRecord(imported?.["__aether"]);
  if (!local || !importedMeta) {
    return localMessage;
  }
  const localMeta = asOptionalRecord(local["__aether"]);
  ...
  return changed ? { ...local, __aether: nextMeta } : localMessage;
}
...
export function mergeImportedChatHistoryMessages(params: {
  ...
  for (const entry of merged) {
    ...
    const localMeta = asOptionalRecord(asOptionalRecord(entry.message)?.["__aether"]);
  ...
  for (const message of params.importedMessages) {
    ...
    if (!imported.hasCliImageMentions) {
      ...
      // before comparing the text after an Aether-generated note.
