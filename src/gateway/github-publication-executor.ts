...
const PUBLICATION_MARKER = "Aether-Publication";
...
export async function executeGitHubPublication<Row extends PublicationRow>(params: {
  ...
  try {
    ...
    const pullRequestMarker = `<!-- aether-publication:${row.request_id} -->`;
    ...
    if (!pullRequestUrl) {
      ...
      const footer = sessionUrl?.startsWith("https://")
        ? `\n\n---\n[View the Aether team session](${sessionUrl})`
        : "";
