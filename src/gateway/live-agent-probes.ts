...
const AETHER_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const AETHER_CLI_CHILD_TIMEOUT_MS = AETHER_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function resolveAetherCliProcessArgs(
  args: readonly string[],
  hasBuildOutput: boolean,
): string[] {
  return [hasBuildOutput ? "aether.mjs" : "scripts/run-app.mjs", ...args];
}
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the Aether MCP automations tool from server `aether`. " +
      "If it is not already visible, search/load MCP tools for `aether automations` or `automations`, " +
      "then call the matching Aether MCP tool; Claude-style names may appear as `mcp__aether__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Aether proof. " +
      ...
      "Do the actual tool call; I will verify externally with the Aether cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the Aether MCP automations tool from server `aether` now. " +
      "If it is not already visible, search/load MCP tools for `aether automations` or `automations`, " +
      "then call the matching Aether MCP tool; Claude-style names may appear as `mcp__aether__automations`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not Aether proof. " +
      ...
      "I will verify externally with the Aether cron CLI."
    );
  }
  return (
    "Your previous Aether automations MCP tool call was cancelled before the job was created. " +
    "Retry the Aether MCP automations tool from server `aether` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__aether__automations`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runAetherCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(AETHER_CLI_GATEWAY_TIMEOUT_MS)];
  ...
  const { stdout, stderr } = await runExec(
    process.execPath,
    resolveAetherCliProcessArgs(cliArgs, hasBuildOutput),
    {
      baseEnv: childEnv,
      cwd: process.cwd(),
      logOutput: false,
      maxBuffer: 1024 * 1024,
      timeoutMs: AETHER_CLI_CHILD_TIMEOUT_MS,
    },
  );
  ...
  if (!trimmed) {
    throw new Error(
      [
        `aether ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `aether ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runAetherCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
