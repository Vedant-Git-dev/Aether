...
function resolveMcpLoopbackTools(
  ...
  // Restricted CLI grants use Aether's implementations for coding tools;
...
export class McpLoopbackToolCache {
  ...
  #grantConfigScopes = new Map<string, Set<AetherConfig>>();
  #epoch = 0;
  ...
  async resolve(input: McpLoopbackScopeParams): Promise<CachedScopedTools> {
    ...
    if (params.grantToken) {
      const scopes = this.#grantConfigScopes.get(params.grantToken) ?? new Set<AetherConfig>();
