...
export async function finishGatewayStartup(params: {
  ...
  if (!minimalTestGateway) {
    const { startAetherDatabaseIntegrityVerifier } =
      await import("../state/aether-database-verify.js");
    kernel.addGatewayLifetimeSidecar(startAetherDatabaseIntegrityVerifier({ env: process.env }));
  }
  postAttachRuntimeReturned = true;
  activateScheduledServicesWhenReady();

  const { startManagedGatewayConfigReloader } = await import("./server-reload-managed.js");
  const assertRuntimeSecurityConfig = (cfg: AetherConfig, env?: NodeJS.ProcessEnv) => {
