...
function restoreGatewayToken(prevToken: string | undefined) {
  if (prevToken === undefined) {
    ...
    process.env.AETHER_GATEWAY_TOKEN = prevToken;
  }
}
...
function resolveGatewayTokenOrEnv(): string {
  ...
  if (typeof token !== "string") {
    throw new Error("expected gateway token in test state or AETHER_GATEWAY_TOKEN");
  }
  return token;
}
...
async function startRateLimitedTokenServerWithPairedDeviceToken() {
  ...
  const deviceIdentityPath = nextAuthIdentityPath("aether-auth-rate-limit");
  try {
...
async function ensurePairedDeviceTokenForCurrentIdentity(ws: WebSocket): Promise<{
  ...
  const deviceIdentityPath = nextAuthIdentityPath("aether-auth-device");
