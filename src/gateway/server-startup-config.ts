...
export function createRuntimeSecretsActivator(params: {
  ...
  let activeDegradationConfig: AetherConfig | null = null;
...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: AetherConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: AetherConfig) => {
