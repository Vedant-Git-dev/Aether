...
type GatewayInstallAuthMode = NonNullable<NonNullable<AetherConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: AetherConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.AETHER_GATEWAY_PASSWORD) ||
    normalizeOptionalString(durableServiceEnv.AETHER_GATEWAY_PASSWORD),
  );
}
