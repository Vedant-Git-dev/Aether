...
function usesLegacyAetherCompaction(params: GatewaySessionCompactionParams): boolean {
  ...
  return (
    (!persistedRuntime || persistedRuntime === "aether") &&
...
export async function preflightGatewaySessionCompaction(
  ...
  if (!usesLegacyAetherCompaction(params)) {
    return undefined;
  }
  try {
