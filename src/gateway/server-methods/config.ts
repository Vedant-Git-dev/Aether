...
): { config: AetherConfig; writeConfig: AetherConfig; schema: ConfigSchemaResponse } | null {
...
function listExplicitAgentRosterIds(config: AetherConfig): string[] {
  ...
  params.respond(
    ...
    errorShape(
      ...
      `config.set would remove existing agent entries: ${droppedIds.join(", ")}. ` +
        "Use the agents.delete RPC or `aether agents delete <id>` for intentional deletion.",
    ),
  );
  return true;
}
...
function validateSubmittedConfigOrRespond(params: {
  ...
  modelIdNormalizationPolicies: Parameters<typeof normalizeSubmittedConfigModelRefs>[1];
  respond: RespondFn;
}): { validationCandidate: AetherConfig; config: AetherConfig } | null {
  const validationCandidate = normalizeSubmittedConfigModelRefs(
    params.candidate as AetherConfig,
  ...
  return { validationCandidate: validationCandidate as AetherConfig, config: validated.config };
}
...
function shouldDisconnectSharedAuthClientsForConfigWrite(params: {
  prevConfig: AetherConfig;
  prevSourceConfig: AetherConfig;
  nextConfig: AetherConfig;
  preparedSecretsSnapshot: PreparedSecretsRuntimeSnapshot;
}): boolean {
  return (
...
export const configHandlers: GatewayRequestHandlers = {
  ...
  "config.patch": async ({ params, respond, client, context }) => {
    ...
    const normalizedPatch = normalizeSubmittedConfigModelRefs(
      parsedRes.parsed as AetherConfig,
      modelIdNormalizationPolicies,
    );
