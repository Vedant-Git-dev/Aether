// Aether gateway methods host the setup/repair conversation for clients.
import {
...
export const systemAgentHandlers: GatewayRequestHandlers = {
  "aether.approval.list": async ({ respond, client, context }) => {
  ...
  "aether.chat.history": ({ params, respond }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentChatHistoryParams,
  ...
  "aether.setup.detect": async ({ params, respond }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupDetectParams,
  ...
  "aether.setup.verify": async ({ params, respond, context }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupVerifyParams,
  ...
  "aether.setup.auth.start": async ({ params, respond, context, client }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupAuthStartParams,
        "aether.setup.auth.start",
        respond,
      )
    ) {
      return;
  ...
  "aether.setup.activate.start": async ({ params, respond, context }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupActivateStartParams,
        "aether.setup.activate.start",
        respond,
      )
    ) {
      return;
  ...
  "aether.setup.prepare.start": async ({ params, respond, context }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupAuthStartParams,
        "aether.setup.prepare.start",
        respond,
      )
    ) {
      return;
        ...
        new WizardSession(
          async (prompter, signal, runnerSession) => {
            await runSystemAgentGatewayTask(async () => {
              ...
              // so setup never writes resolved runtime defaults into aether.json.
   ...
   */
  "aether.setup.activate": async ({ params, respond }) => {
    if (
      !assertValidParams(
        params,
        validateSystemAgentSetupActivateParams,
        "aether.setup.activate",
        respond,
      )
    ) {
      return;
  ...
  "aether.chat": async ({ params: rawParams, respond, client, context }) => {
    const params = sanitizeSystemAgentChatParams(rawParams);
    if (!assertValidParams(params, validateSystemAgentChatParams, "aether.chat", respond)) {
      return;
    }
    ...
    const pending = await runSystemAgentGatewayTask(async () => {
      ...
      if (!ownerKey) {
        ...
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, "Aether caller identity unavailable."),
        );
        return undefined;
      }
      ...
      if (boundSession && boundSession.ownerKey !== ownerKey) {
        ...
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, "Aether session belongs to another caller.", {
        ...
        respond(
          ...
          errorShape(
            ...
            params.wizardCancel !== undefined
              ? "No active Aether chat session is awaiting that wizard cancel."
              : "No active Aether chat session is awaiting that wizard answer.",
            { details: buildSystemAgentSessionInvalidatedErrorDetails() },
          ),
        );
        return undefined;
        ...
        if (!inference.ok) {
          respond(
            ...
            errorShape(
              ...
              `Aether requires working inference: ${inference.error}`,
              {
      ...
      try {
        ...
        if (!turnReply) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.INVALID_REQUEST, "Aether chat input is missing."),
          );
          return undefined;
        }
        reply = turnReply;
