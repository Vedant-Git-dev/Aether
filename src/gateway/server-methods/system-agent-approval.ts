...
export async function prepareDelegatedSystemAgentApproval(params: {
  ...
  if (!approvalAuthority) {
    throw new Error("delegated Aether approval requires an active run authority");
  }
  ...
  const assertLiveApprovalAuthority = () => {
    if (!isAuthorityActive() || params.sessions.get(params.sessionId) !== params.session) {
      throw new Error(
        "Aether change cancelled: system-agent approval authority is no longer active. Retry the request if it is still needed.",
      );
    }
  };
  ...
  return async (proposal) => {
    ...
    return await withProposalFailureCleanup(async (): ReturnType<DelegatedProposalResolver> => {
      ...
      if (params.session.pendingApproval) {
        ...
        throw new Error("Aether change is no longer pending. Retry the request.");
      }
      ...
      if (!manager) {
        throw new Error("Aether approval registry unavailable");
      }
      ...
      const cancelledReply = {
        text: "Aether change cancelled. No change. Retry the request if it is still needed.",
        action: "none" as const,
        applied: false,
      };
      const failedReply = {
        text: "Aether change failed to complete. Check the current settings and Aether status before retrying.",
        action: "none" as const,
        applied: false,
      };
      ...
      void handlePendingApprovalRequest({
        ...
        respond: () => undefined,
        context: params.context,
        requestEventName: "aether.approval.requested",
        requestEvent,
        twoPhase: true,
        approvalKind: "system-agent",
        ...
        afterDecisionErrorLabel: "Aether approval apply failed",
      }).catch((error: unknown) => {
