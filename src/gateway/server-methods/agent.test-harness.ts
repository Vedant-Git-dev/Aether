...
import { toErrorObject as toLintErrorObject } from "@aether/normalization-core/error-coercion";
...
const envSnapshot = captureEnv(["AETHER_STATE_DIR"]);
...
const mocks = vi.hoisted(() => ({
  ...
  loadConfigReturn: {} as AetherConfig,
...
function resolveAgentTestConfig(cfg: AetherConfig = mocks.loadConfigReturn): AetherConfig {
  if (cfg.agents?.list) {
    return cfg;
  }
...
export function useTestStateDir(root: string): void {
  setTestEnvValue("AETHER_STATE_DIR", root);
}
