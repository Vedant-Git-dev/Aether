...
async function validateDispatchExecutionMode(params: {
  ...
  respondInvalidWorkerSession(
    params.respond,
    `runtime ${params.sessionRuntime} requires a cloud worker provider that supports ${params.executionMode}; choose a compatible provider, or select an agent/model route with agentRuntime.id "aether"`,
  );
  return false;
}
