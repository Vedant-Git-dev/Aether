...
beforeEach(async () => {
  ledgerHome = await createTempHomeEnv("aether-update-rpc-");
});
afterEach(async () => {
...
>(async (params) => ({
  status: "started",
  pid: 12345,
  command: "aether update --yes --timeout 1800",
  logPath: "/tmp/aether-update-run-handoff/handoff.log",
  handoffId: params?.handoffId ?? "handoff-default",
  installRoot: params?.root ?? "/tmp/aether",
}));
...
export async function withTransferredUpdateHandoff(
  ...
  const tempRoot = await import("../../infra/tmp-aether-dir.js");
  const tmp = vi.spyOn(tempRoot, "resolvePreferredAetherTmpDir").mockReturnValue(root);
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as AetherConfig,
    sourceConfig: {} as AetherConfig,
    valid: true,
    config: {} as AetherConfig,
    runtimeConfig: {} as AetherConfig,
    issues: [],
    warnings: [],
    legacyIssues: [],
  });
  ...
  initializeGatewayUpdateStatusMock.mockResolvedValue({
    root: "/tmp/aether",
    status: { root: "/tmp/aether", installKind: "git", packageManager: "pnpm" },
    installReceipt: null,
  });
