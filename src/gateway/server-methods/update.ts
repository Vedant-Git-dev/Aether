...
export const updateHandlers: GatewayRequestHandlers = {
  ...
  "update.run": async ({ params, respond, client, context }) => {
    ...
    const ownerRequiredMessage = () =>
      `Only the Aether owner can start an update from chat. ${formatCommandOwnerHint({ cfg: context.getRuntimeConfig(), channel: params.requester?.channel, id: params.requester?.senderId })}`;
    ...
    try {
      ...
      if (targetFailureReason) {
        ...
        result = refusedUpdate("error", "not-aether-root");
      ...
      try {
        ...
        if (result.status === "ok" && handoff?.status !== "started") {
          noticeFailureMessage =
            "The update was installed, but its restart notice could not be saved. Run aether update status after the gateway restarts.";
