  ...
  if (value.event === "config.external") {
    ...
    return {
      entry: {
        ...
        kind: "external-edit",
        source: "external",
        summary: summarizePaths("Configuration edited outside Aether", changedPaths),
...
export const systemChangesHandlers: GatewayRequestHandlers = {
  "aether.changes.list": ({ params, respond }) => {
    if (
      !assertValidParams(params, validateSystemChangesListParams, "aether.changes.list", respond)
    ) {
      return;
    }
    try {
