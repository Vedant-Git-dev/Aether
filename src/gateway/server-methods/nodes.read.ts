  ...
  "node.runnerInventory.update": async ({ params, respond, client, context }) => {
    ...
    if (!connId || !pairingGeneration) {
      ...
      respondRunnerInventoryRetry(
        respond,
        pendingSurface
          ? `node capability surface is awaiting operator approval; run \`aether nodes approve ${pendingSurface.requestId}\` (see \`aether nodes pending\`), then this node retries automatically`
