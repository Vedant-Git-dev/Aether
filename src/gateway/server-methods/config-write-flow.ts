...
export function didSharedGatewayAuthChange(prev: AetherConfig, next: AetherConfig): boolean {
...
export function didActiveSharedGatewayAuthChange(params: {
  fallbackPrev: AetherConfig;
  fallbackSource?: AetherConfig;
  next: AetherConfig;
}): boolean {
  ...
  const selectOwnedGatewayValue = <Key extends "auth" | "tailscale" | "trustedProxies">(
    key: Key,
  ): NonNullable<AetherConfig["gateway"]>[Key] =>
    currentSourceGateway && Object.hasOwn(currentSourceGateway, key)
      ? (projectAuthoredValuesOntoRuntimeOverlay({
        ...
        }) as NonNullable<AetherConfig["gateway"]>[Key])
      : fallbackGateway?.[key];
  const activeSharedAuthConfig: AetherConfig = {
    ...params.fallbackPrev,
    gateway: {
      ...fallbackGateway,
