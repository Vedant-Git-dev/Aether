...
export async function appendInjectedAssistantMessageToTranscript(params: {
  ...
  const rawDeliveryFacts = preparedDisplayMessage.aetherDelivery;
  ...
  const messageBody: AppendMessageArg & Record<string, unknown> = applyAssistantDeliveryDirectives({
    ...
    // keep their replayable default and carry cancellation in aetherAbort.
    ...
    ...(params.ttsSupplement ? { aetherTtsSupplement: params.ttsSupplement } : {}),
    ...(params.abortMeta
      ? {
          aetherAbort: {
            aborted: true,
  ...
  if (rawDeliveryFacts && messageBody.aetherDelivery === undefined) {
    messageBody.aetherDelivery = rawDeliveryFacts;
  }
