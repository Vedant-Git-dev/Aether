...
function getVoiceCallRealtimeConfig(config: AetherConfig): {
  provider?: string;
...
function getVoiceCallStreamingConfig(config: AetherConfig): {
  provider?: string;
...
type RealtimeProviderWithConfig<TConfig extends Record<string, unknown>> = VoiceModelProvider & {
  resolveConfig?: (ctx: { cfg: AetherConfig; rawConfig: TConfig }) => TConfig;
  isConfigured: (ctx: { cfg: AetherConfig; providerConfig: TConfig }) => boolean;
};
...
const DEFAULT_REALTIME_INSTRUCTIONS = [
  "You are Aether's realtime voice interface. Keep spoken replies concise.",
  `If the user asks for code, repository state, files, current Aether context, tool-backed actions, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`,
  `Do not claim you cannot use tools, perform actions, or reach Aether unless ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} returns that failure.`,
  `When ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} is in progress, speak one brief acknowledgement such as "Let me check that for you", then wait for the final Aether result before answering with the actual result.`,
  `If Aether is already working through ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and the user asks in any language for progress, cancellation, a redirect/change, or a follow-up, call ${REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME} with the semantic mode.`,
  "For greetings and casual chatter while Aether is working, answer naturally and do not redirect the active work.",
].join(" ");
