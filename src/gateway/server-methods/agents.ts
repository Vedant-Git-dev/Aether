...
import { normalizeOptionalString as resolveOptionalStringParam } from "@aether/normalization-core/string-coerce";
import {
  GATEWAY_CLIENT_CAPS,
  GATEWAY_CLIENT_IDS,
  hasGatewayClientCap,
...
import { unregisterAetherAgentDatabase } from "../../state/aether-agent-db-registry.js";
...
function resolveAgentIdOrError(agentIdRaw: string, cfg: AetherConfig) {
...
function agentOwnsSharedAuthStore(cfg: AetherConfig, agentId: string): boolean {
...
function unregisterAgentDeleteDatabases(agentId: string, databasePaths: string[]): void {
  for (const databasePath of databasePaths) {
    unregisterAetherAgentDatabase({ agentId, path: databasePath });
  }
}
