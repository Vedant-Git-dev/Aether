...
import { asOptionalRecord as transcriptEventRecord } from "@aether/normalization-core/record-coerce";
...
function findSourceReplyTranscriptMirrorByIdempotencyKeyInEvents(
  ...
  if (found?.message.provider !== "aether" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
