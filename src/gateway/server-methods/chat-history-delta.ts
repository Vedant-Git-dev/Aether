...
export function readChatHistoryDelta(params: {
  ...
  for (const row of result.events) {
    ...
    if (
      isAetherDeliveryMirrorAssistantMessage(entryMessage) &&
      asOptionalRecord(asOptionalRecord(entryMessage)?.aetherDeliveryMirror)?.kind ===
        "channel-final"
    ) {
    ...
    if (
      historyProjection.messages.some(
        (message) => asOptionalRecord(message.aetherStreamFallback)?.source === "segment",
      )
    ) {
