...
import { asDateTimestampMs } from "@aether/normalization-core/number-coercion";
...
function resolveAuthRefreshScope(cfg: AetherConfig): {
  providerIds: string[];
  profileIds?: string[];
} {
...
async function refreshAfterCredentialMutation(
  ...
  try {
    ...
    return "Model auth changes were saved, but the Gateway could not refresh them. Run `aether gateway restart` to apply the saved changes.";
  }
}
...
export const modelsAuthStatusHandlers: GatewayRequestHandlers = {
  ...
  "models.authStatus": async ({ params, respond, context, client }) => {
    ...
    const resolveScope = (cfg: AetherConfig) =>
