...
const RESTART_SAFE_CHAT_REQUEST_VERIFIER_DOMAIN = "aether.chat.restart-retry.v1";
...
function hasRestartUnsafeMessageSemantics(rawMessage: string, cfg: AetherConfig): boolean {
  if (
...
function isRestartSafeChatSession(params: {
  ...
  return Boolean(
    ...
    (entry.agentHarnessId === undefined || entry.agentHarnessId === AETHER_AGENT_RUNTIME_ID) &&
