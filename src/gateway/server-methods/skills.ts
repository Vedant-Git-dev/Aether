...
import {
  ...
  fetchAetherSkillSecurityVerdicts,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      ...
      if (targets.length === 0) {
        respond(true, { schema: "aether.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchAetherSkillSecurityVerdicts(targets);
      respond(true, { schema: "aether.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
  ...
  "skills.skillCard": ({ params, respond, context }) => {
    ...
    respond(
      true,
      {
        schema: "aether.skills.skill-card.v1",
      ...
      if (requested.requestedReference) {
        ...
        respond(
          ...
          errorShape(
            ...
            `ClawHub cannot return details for ${requested.requestedReference}; external skill sources are install-only. Install it directly, or run "aether skills install ${requested.requestedReference}".`,
          ),
        );
        return;
      }
