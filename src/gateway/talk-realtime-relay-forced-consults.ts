...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with Aether.",
    "Do not answer the request yet. Wait for the Aether result before giving the actual answer.",
  ].join(" ");
}

function buildForcedConsultSpeechPrompt(text: string): string {
  return [
    "Aether finished checking. Speak this result naturally and concisely.",
  ...
  session.harness.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking aether_agent_consult, so Aether is forcing the consult for realtime Talk.",
...
export function submitForcedTalkRealtimeRelayToolResult(
  ...
  if (cancelled) {
    const providerResult = buildRealtimeVoiceAgentCancelProviderResult(
      "Aether cancelled this consult before completion. Do not restart it.",
    );
