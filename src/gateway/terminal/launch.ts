...
type TerminalLaunchPolicy = {
  ...
  isEnabled: () => boolean;
  prepareConfig: (config: AetherConfig, options: { restartPending: boolean }) => void;
  commitConfig: () => void;
...
export function createTerminalLaunchPolicy(initialConfig: AetherConfig): TerminalLaunchPolicy {
  let activeConfig = initialConfig;
  let hasPendingRestart = false;
  let preparedConfig: AetherConfig | null = null;
  let appliedConfigWhileRestartPending: AetherConfig | null = null;
  ...
  const resolveForConfig = (config: AetherConfig, agentId?: string, shellConfig = config) => {
...
export function buildTerminalEnv(baseEnv: NodeJS.ProcessEnv): Record<string, string> {
  ...
  // Lets shells and prompts detect that they are inside an Aether terminal.
  env.AETHER_TERMINAL = "1";
  return env;
}
