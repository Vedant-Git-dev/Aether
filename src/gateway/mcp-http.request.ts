...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.AETHER_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.AETHER_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: AetherConfig, rawSessionKey: string | undefined): string {
...
function resolveMcpSender(params: {
  ...
  const captureKey = normalizeOptionalString(getHeader(params.req, "x-aether-cli-capture-key"));
  const clientGrant =
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("AETHER_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}
...
export function resolveMcpCliCaptureKey(
  ...
  return normalizeOptionalString(getHeader(req, "x-aether-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-aether-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-aether-message-channel")) ?? undefined,
    ...
    clientCaps: normalizeMcpClientCapsHeader(getHeader(req, "x-aether-client-caps")),
    currentChannelId: normalizeOptionalString(getHeader(req, "x-aether-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-aether-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-aether-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-aether-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-aether-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-aether-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-aether-source-reply-delivery-mode"),
    ),
    taskSuggestionDeliveryMode: normalizeMcpTaskSuggestionDeliveryMode(
      getHeader(req, "x-aether-task-suggestion-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-aether-require-explicit-message-target"),
    ),
    senderIsOwner: auth.senderIsOwner,
  };
}
