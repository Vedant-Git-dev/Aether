...
/** Formats the Bonjour instance name while preserving user-provided Aether names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "Aether";
  }
  if (/aether/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Aether)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.AETHER_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "aether");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.AETHER_TAILNET_DNS?.trim();
