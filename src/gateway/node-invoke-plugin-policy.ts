...
import type {
  ...
  AetherPluginNodeInvokeTransportResult,
...
function validateRiskClassification(
  value: NonNullable<AetherPluginNodeInvokePolicyContext["risk"]>,
): NonNullable<AetherPluginNodeInvokePolicyContext["risk"]> | null {
...
export async function applyPluginNodeInvokePolicy(params: {
...
}): Promise<AetherPluginNodeInvokePolicyResult | null> {
  ...
  let risk: AetherPluginNodeInvokePolicyContext["risk"];
  ...
  const dispatchNode = async (
    override: Parameters<AetherPluginNodeInvokePolicyContext["invokeNode"]>[0] = {},
    sessionAuthority?: { assertCurrent: () => void; signal: AbortSignal },
  ): Promise<AetherPluginNodeInvokeTransportResult> => {
    const deny = (
      reasonCode: string,
      result: AetherPluginNodeInvokeTransportResult,
    ): AetherPluginNodeInvokeTransportResult => {
  ...
  const invokeOwned = (
    ...
    override: NonNullable<Parameters<AetherPluginNodeInvokePolicyContext["invokeNode"]>[0]>,
    ...
    ownedInvocation && override.workspace
      ...
      : undefined;
  const invokeNode: AetherPluginNodeInvokePolicyContext["invokeNode"] = async (override = {}) => {
  ...
  let result: AetherPluginNodeInvokePolicyResult;
  try {
  ...
  if (!nodeCommandDispatched && !nodeGateDecisionRecorded && isCallerRuntimeAuthorityActive()) {
    recordNodeDecision({
      ...
      summary: result.ok
        ? "The plugin policy returned without invoking the expected Aether node callback."
        ...
        ? [
            {
              ...
              text: "Route the native action through the provided Aether node callback.",
            },
          ]
        : [],
    });
