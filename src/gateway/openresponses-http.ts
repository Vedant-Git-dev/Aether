 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for Aether Gateway.
 *
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-aether-session-key"),
  });
}
    ...
    try {
      ...
      const response = createResponseResource({
        ...
        output: [
          createAssistantOutputItem({
            id: outputItemId,
            text: assistantText || "No response from Aether.",
            phase: "final_answer",
            status,
          }),
        ],
