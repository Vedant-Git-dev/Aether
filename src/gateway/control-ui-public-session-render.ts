...
export function renderPublicSessionDocument(params: {
  ...
  for (const value of params.messages.slice(-MAX_MESSAGES).toReversed()) {
    ...
    rows.push(
      `<article class="message ${message.role}" aria-label="${message.role === "user" ? "User" : "Assistant"} message"><h2>${message.role === "user" ? "User" : "Aether"}</h2><div class="content">${markdown.render(text)}${clipped ? '<p class="omitted">Message shortened for this public view.</p>' : ""}</div></article>`,
    );
  }
  const description = "A public, read-only Aether conversation. No login required.";
...
<title>${title} · Aether</title>${canonicalMetadata}
<meta property="og:type" content="website"><meta property="og:site_name" content="Aether">
...
<header><div class="masthead"><div class="brand"><span aria-hidden="true">✳</span>Aether</div><span class="badge">Public · Read-only</span></div>
