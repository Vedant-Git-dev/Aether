...
const REMOTE_WORKSPACE_ROOT = ".aether-worker/workspaces";
const REMOTE_GIT_PACK_NAME = ".aether-base.pack";
...
> {
  ...
  const syncWorkspaceImpl = async (
    ...
    const temporaryDirectory = await fs.mkdtemp(
      path.join(resolvePreferredAetherTmpDir(), "aether-worker-workspace-sync-"),
    );
    try {
  ...
  const reconcileWorkspaceRun = async (
    ...
    const temporaryDirectory = await fs.mkdtemp(
      path.join(resolvePreferredAetherTmpDir(), "aether-worker-workspace-reconcile-"),
    );
    ...
    try {
      ...
      const baseManifestTransfer = await runBoundedInboundRsync({
        ...
        argv: (rsyncSsh) => [
          ...
          rsyncSsh,
          "--",
          `${prepared.scpTarget}:.aether-worker/manifests/${baseDigest}.json`,
          baseManifestPath,
        ],
      ...
      if (changed) {
        ...
        const currentManifestTransfer = await runBoundedInboundRsync({
          ...
          argv: (rsyncSsh) => [
            ...
            rsyncSsh,
            "--",
            `${prepared.scpTarget}:.aether-worker/manifests/${currentDigest}.json`,
            currentManifestPath,
          ],
          destinationRoot: manifestRoot,
          entryLimit: 1,
