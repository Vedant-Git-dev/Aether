...
export function createWorkerSessionPlacementStore(
  ...
  const path = (options.database ?? openAetherStateDatabase()).path;
  const now = options.now ?? Date.now;
  const runtime: PlacementStoreRuntime = {
    path,
    instanceId: randomUUID(),
    now,
    read: () => openAetherStateDatabase({ path }).db,
    write: (operation) => runAetherStateWriteTransaction(({ db }) => operation(db), { path }),
  };
  ...
  const store = {
    ...
    recordWorkspaceResultConflict(
      ...
      if (
        paths.length === 0 ||
        !/^refs\/aether\/worker-results\/[A-Za-z0-9-]+$/u.test(stagedResultRef)
      ) {
