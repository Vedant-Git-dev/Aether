...
const turnClaimReleaseWaiters = resolveGlobalMap<string, Map<string, Set<TurnClaimReleaseWaiter>>>(
  Symbol.for("aether.turnClaimReleaseWaiters"),
  (waitersByPath) => {
...
>(Symbol.for("aether.workerTurnClaimClosedHandlers"), (handlersByPath) => {
...
const workerTurnOwners = resolveGlobalMap<string, Map<string, BoundWorkerTurnOwner>>(
  Symbol.for("aether.workerTurnExecutionIdentities"),
  (ownersByPath) => {
