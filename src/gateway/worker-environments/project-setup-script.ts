...
const runSetup = (script, workspaceDir, homeDir) => {
  ...
  return new Promise((resolve, reject) => {
    child = spawn(script, [], {
      cwd: workspaceDir,
      env: { PATH: process.env.PATH, HOME: homeDir, LANG: "C.UTF-8", AETHER_SOURCE_TREE_PATH: workspaceDir, AETHER_WORKTREE_PATH: workspaceDir },
      detached: true,
  ...
  const workerRoot = ownedDirectory(machineHome, ".aether-worker");
  ...
  if (fs.existsSync(existing)) {
    ...
    const manifestRoot = ownedDirectory(ownedDirectory(homeDir, ".aether-worker"), "manifests");
  ...
  const recipe = git(seed, ["ls-tree", input.baseCommit, "--", ".aether/worktree-setup.sh"]);
  const expectedRecipe = input.setupRecipe ? "100755 blob " + input.setupRecipe + "\\t.aether/worktree-setup.sh" : null;
  ...
  const workspaceDir = ownedDirectory(directory, "workspace");
  const homeDir = ownedDirectory(directory, "home");
  const manifestRoot = ownedDirectory(ownedDirectory(homeDir, ".aether-worker", fresh), "manifests", fresh);
  ...
  if ((fresh || changed) && input.setupRecipe && input.runSetupScript !== false) {
    const script = path.join(ownedDirectory(workspaceDir, ".aether"), "worktree-setup.sh");
  ...
  ownedDirectory(ownedDirectory(homeDir, ".aether-worker"), "manifests");
