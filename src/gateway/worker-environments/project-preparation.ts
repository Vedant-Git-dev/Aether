...
export async function readWorkerProjectSetupRecipe(
  ...
  const tree = await requireGit(
    project.root,
    ["ls-tree", "-z", project.baseCommit, "--", ".aether/worktree-setup.sh"],
    { signal, timeoutMs: 30_000 },
  );
  return /^100755 blob ([a-f0-9]{40}(?:[a-f0-9]{24})?)\t\.aether\/worktree-setup\.sh\0$/u.exec(
    tree,
  )?.[1];
}
...
export function createWorkerProjectPreparation(params: {
  ...
  const readPreparedWorkspace = (prepared: unknown) => {
    ...
    const suffix = `/.aether-worker/prepared/${params.namespace}/${preparation.cacheKey}`;
    if (
      !isRecord(prepared) ||
  ...
  const prepareSeed: ProjectPreparation["prepare"] = async (transport) => {
    ...
    if (
      ...
      !directory.includes(`/.aether-worker/git-seeds/${params.namespace}/`) ||
    ...
    const temporaryRoot = await fsp.mkdtemp(
      path.join(resolvePreferredAetherTmpDir(), "aether-project-base-"),
    );
    try {
      requireCurrent();
