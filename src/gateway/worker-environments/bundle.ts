...
const AETHER_NPM_REGISTRY = "https://registry.npmjs.org/";
...
function resolvePackageRoot(packageRoot: string | undefined): string {
  ...
  const resolved = resolveAetherPackageRootSync({
    moduleUrl: import.meta.url,
    argv1: process.argv[1],
    cwd: process.cwd(),
  });
  if (!resolved) {
    throw new Error("Unable to locate the running Aether package root for worker bundling");
  }
  return resolved;
}
...
async function verifyPublishedNpmRelease(params: {
  ...
  const temporaryRoot = await fs.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-worker-npm-proof-"),
  );
  try {
      ...
      unwrapNpmJsonEntry(
        await runNpmProofCommand({
          argv: [
            "npm",
            "view",
            `aether@${params.version}`,
            "name",
            "version",
            "dist.integrity",
            "--json",
            `--registry=${AETHER_NPM_REGISTRY}`,
          ],
          cwd: temporaryRoot,
          failureMessage: `Aether ${params.version} is not published; use the worker bundle install`,
          runCommand,
        }),
      ),
    );
    if (
      published?.name !== "aether" ||
      published.version !== params.version ||
      !NPM_SHA512_INTEGRITY_PATTERN.test(published.integrity)
    ) {
      throw new Error(
        `Cannot verify exact public npm release aether@${params.version}; use the worker bundle install`,
      );
    }
    const packedValue = await runNpmProofCommand({
      ...
      cwd: temporaryRoot,
      failureMessage:
        "Unable to verify the installed Aether package; use the worker bundle install",
      runCommand,
    });
    ...
    if (
      ...
      throw new Error(
        `Installed Aether ${params.version} does not match the published package; use the worker bundle install`,
      );
    }
    ...
    const packedBundle = await prepareWorkerBundle({
      ...
      aetherVersion: params.version,
    });
    if (packedBundle.bundleHash !== params.bundleHash) {
      throw new Error(
        `Published Aether ${params.version} does not match the prepared worker bundle; use the worker bundle install`,
      );
    }
...
async function prepareWorkerBundle(
  ...
  const aetherVersion = (options.aetherVersion ?? VERSION).trim();
  if (!aetherVersion) {
    throw new Error("Worker bundle requires a non-empty Aether version");
  }
...
export async function resolveWorkerNpmInstallationArtifact(params: {
  ...
  const version = params.bundle.aetherVersion.trim();
  ...
  return {
    install: "npm",
    bundleHash: params.bundle.bundleHash,
    aetherVersion: version,
    packageIntegrity,
    protocolFeatures: params.bundle.protocolFeatures,
    packageSpec: `aether@${version}`,
  };
}
