...
type NodePackageManifest = DistributionPackageManifest & {
  ...
  aether?: { extensions?: string[]; runtimeExtensions?: string[] };
};
  ...
  return await Promise.all(
    options.plugins.map(async ({ id, root }) => {
      ...
      const manifest = JSON.parse(
        await fs.readFile(path.join(builtRoot, "aether.plugin.json"), "utf8"),
      ...
      const entries = packageJson.aether?.runtimeExtensions ?? packageJson.aether?.extensions;
      if (!entries?.length) {
...
async function prepareNodeBootstrapArtifact(
  ...
  if (sourcePackage.name !== "aether") {
    throw new Error("Node bootstrap requires the running Aether package root");
  }
  ...
  return Object.freeze({
    ...
    tarballBytes,
    aetherVersion: packageJson.version,
    buildId,
      ...
      prepared ??= Promise.resolve().then(async () => {
        try {
          temporaryRoot = await fs.mkdtemp(
            path.join(resolvePreferredAetherTmpDir(), "aether-node-runtime-"),
          );
          if (closed) {
            throw new Error("Node bootstrap artifact provider is closed");
          }
