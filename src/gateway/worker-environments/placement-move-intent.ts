...
function moveSchemaSql(): string {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(MOVE_SCHEMA_START);
  const endMarkerStart = AETHER_STATE_SCHEMA_SQL.indexOf(MOVE_SCHEMA_END, start);
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Worker placement move schema marker is missing");
  }
  return AETHER_STATE_SCHEMA_SQL.slice(start, endMarkerStart + MOVE_SCHEMA_END.length);
}
