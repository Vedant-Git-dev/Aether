...
import { normalizeSortedUniqueTrimmedStringList } from "@aether/normalization-core/string-normalization";
...
import { ensureWorkerEnvironmentNodeEnrollmentSchema } from "../../state/aether-state-db-schema-additive.js";
import type {
  DB as StateDatabase,
...
function normalizeBootstrapReceipt(value: {
  ...
  return {
    bundleHash,
    aetherVersion: required(value.aetherVersion, "bootstrap Aether version"),
...
function bootstrapReceiptFrom(row: Row): WorkerEnvironmentBootstrapReceipt | null {
  const {
    ...
    bootstrap_aether_version: aetherVersion,
    bootstrap_protocol_features_json: encodedFeatures,
    bootstrap_install_kind: installKind,
  } = row;
  if (bundleHash === null && aetherVersion === null && encodedFeatures === null) {
    return null;
  }
  if (bundleHash === null || aetherVersion === null || encodedFeatures === null) {
...
export function createWorkerEnvironmentStore(
  options: { database?: AetherStateDatabase; now?: () => number } = {},
) {
  const database = options.database ?? openAetherStateDatabase();
  ...
  const read = () => openAetherStateDatabase({ path }).db;
  let inventoryVersion = 0;
  const write = <T>(operation: (db: DatabaseSync) => T): T => {
    const result = runAetherStateWriteTransaction(({ db }) => operation(db), { path });
    ...
    refreshBootstrapReceipt(input: BootstrapRefreshInput): WorkerEnvironmentRecord {
      ...
      return write((db) => {
        ...
        updateRow(db, environmentId, current.state, {
          ...
          bootstrap_aether_version: receipt.aetherVersion,
    ...
    transition(input: TransitionInput): WorkerEnvironmentRecord {
      ...
      return write((db) => {
        ...
        updateRow(db, environmentId, from, {
          ...
          bootstrap_aether_version: bootstrapReceipt?.aetherVersion ?? null,
