...
export function createNodeWorkerWorkspaceActions(params: {
  ...
  const transfer = async (
    ...
    const result = await exec({
      ...command,
      argv: ["aether-internal-workspace-transfer"],
      transfer: input,
...
const script = path.join(root, ".aether", "worktree-setup.sh");
const stat = fs.statSync(script, { throwIfNoEntry: false });
if (stat?.isFile() && (stat.mode & 0o111)) {
  const run = spawnSync(script, [], {
    cwd: root,
    env: { ...process.env, AETHER_SOURCE_TREE_PATH: root, AETHER_WORKTREE_PATH: root },
    stdio: "inherit",
  });
