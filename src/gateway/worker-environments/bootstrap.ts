...
const BOOTSTRAP_ROOT = ".aether-worker";
...
const NODE_MISSING_MARKER = "AETHER_WORKER_NODE_MISSING";
const NODE_UNSUPPORTED_MARKER = "AETHER_WORKER_NODE_UNSUPPORTED";
const NPM_MISSING_MARKER = "AETHER_WORKER_NPM_MISSING";
const BOOTSTRAP_OUTPUT_TAG = "AETHER_WORKER_BOOTSTRAP_V1";
...
try {
  ...
  const shapeMatches =
    Object.keys(actual).sort().join(",") === "bundleHash,aetherVersion,protocolFeatures";
  const featuresMatch =
  ...
  process.exit(
    shapeMatches &&
      ...
      actual.aetherVersion === expected.aetherVersion &&
      featuresMatch
      ? 0
      : 1,
  );
...
find "$incoming" -type f -name 'aether-upload-*.tgz.*' -mmin +60 -exec rm -f -- {} + 2>/dev/null || true
upload=$incoming/aether-upload-$hash.tgz.$operation_token
...
function normalizeHandshake(artifact: WorkerInstallationArtifact): WorkerAdmissionHandshake {
  ...
  const aetherVersion = artifact.aetherVersion.trim();
  ...
  if (!aetherVersion) {
    throw new Error("Worker Aether version must be non-empty");
  }
  if (
  ...
  if (artifact.install === "npm") {
    if (
      !isExactSemverVersion(aetherVersion) ||
      artifact.packageSpec !== `aether@${aetherVersion}`
    ) {
      throw new Error(`Worker npm install must use exact package aether@${aetherVersion}`);
    }
  ...
  return { bundleHash, aetherVersion, protocolFeatures };
}
...
function parseReceiptJson(
  ...
  if (
    ...
    parsed.aetherVersion !== expected.aetherVersion ||
...
function workerUploadFilename(bundleHash: string, operationToken: string): string {
  return `aether-upload-${bundleHash}.tgz.${operationToken}`;
}
...
incoming=$(cd "$incoming" && pwd -P)
rm -f -- "$incoming/aether-upload-$hash.tgz.$operation_token"
`;
...
export async function bootstrapWorker(
  ...
  const prepared = await prepareWorkerSsh({
    ...
    temporaryDirectoryPrefix: "aether-worker-bootstrap-",
  });
  try {
