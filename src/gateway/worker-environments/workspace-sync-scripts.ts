...
git update-ref refs/heads/aether-worker "$base"
git symbolic-ref HEAD refs/heads/aether-worker
git read-tree "$base"
...
function eligiblePaths() {
  ...
  removeSelected(".aether-base.pack");
  ...
  for (const priorManifestDigest of priorManifestDigests) {
    ...
    const priorPath = path.join(process.env.HOME, ".aether-worker", "manifests", priorManifestDigest + ".json");
    ...
    for (const entry of prior.entries) {
      ...
      if (entry.path !== ".aether-base.pack" && !isDerivedWorkspacePath(entry.path, isStagedInput(entry.path))) {
...
async function main() {
  const workerRoot = path.join(process.env.HOME, ".aether-worker");
