...
export const WORKER_WORKSPACE_RSYNC_DESTINATION = "aether-rsync-destination";
...
export function workerAcceptedWorkspaceRsyncReceiverPath(params: {
  ...
  const workspaceRootMarker = "/.aether-worker/workspaces/";
...
export function workerWorkspaceRsyncReceiverEntryPath(bundleHash: string): string {
  ...
  return `.aether-worker/${bundleHash}/${WORKER_BUNDLE_RSYNC_RECEIVER_PATH}`;
}
...
export function parseRemoteWorkspaceSetup(
  ...
  if (
    record?.tag !== "aether-workspace-setup-v1" ||
