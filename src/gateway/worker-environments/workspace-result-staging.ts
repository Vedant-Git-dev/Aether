...
// Match managed-worktree refs/aether/snapshots: deleting the owning ref is
// sufficient; unreachable objects may remain until normal Git GC.
const WORKER_RESULT_REF_PREFIX = "refs/aether/worker-results";
const WORKER_RESULT_CANDIDATE_REF_PREFIX = "refs/aether/worker-result-candidates";
const WORKER_RESULT_CLEANUP_REF_PREFIX = "refs/aether/worker-result-cleanup";
const WORKER_RESULT_CLAIM_ID_PATTERN = /^[A-Za-z0-9-]+$/u;
const STAGED_RESULT_MESSAGE = "Aether worker workspace result";
...
async function stageWorkerWorkspaceResult(params: {
  ...
  chunks.push(
    Buffer.from(
      `commit ${stagedResultRef}\nauthor Aether <aether@localhost> 0 +0000\ncommitter Aether <aether@localhost> 0 +0000\ndata ${message.byteLength}\n`,
    ),
    message,
    Buffer.from("\ndeleteall\n"),
  );
...
async function withMaterializedWorkerWorkspaceResult<T>(
  ...
  const stagingRoot = await fs.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-checkpoint-payload-"),
  );
  try {
