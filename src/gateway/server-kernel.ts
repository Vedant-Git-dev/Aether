...
import { clearGatewayAgentCliShim } from "../infra/aether-cli-shim.js";
import { ensureAetherCliOnPath } from "../infra/path-env.js";
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`aether config set gateway.auth.mode token\` and \`aether config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed Aether config and rebuild.",
    "For the first-party Nix flow, see https://github.com/aether/nix-aether#quick-start and https://docs.aether.ai/install/nix.",
  ].join(" ");
}
