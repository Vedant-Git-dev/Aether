...
import { createAetherCodingTools } from "../agents/agent-tools.js";
...
import { createAetherTools } from "../agents/aether-tools.js";
import { filterRequesterYieldTools } from "../agents/aether-tools.requester-yield.js";
...
export function resolveGatewayScopedTools(
  ...
  const aetherTools = createAetherTools({
  ...
  const baseTools = nodeExecSurface
    ? aetherTools.filter((tool) => tool.name.trim().toLowerCase() !== "exec")
    : aetherTools;
  ...
  const allTools = nodeExecDefaults
    ? [
        ...
        createLazyExecTool(
          ...
          {
            description:
              "Execute a shell command on a connected Aether node. This tool is node-only; use the CLI native shell for Gateway-local commands when it is available. Commands run synchronously. The sole connected node that can execute commands is selected automatically; set node when several can.",
