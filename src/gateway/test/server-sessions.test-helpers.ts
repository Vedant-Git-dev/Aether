...
import type { AssistantMessage, UserMessage } from "aether/plugin-sdk/llm";
...
function createGatewaySessionsTestHarness(startServer: boolean, setup?: GatewaySessionsSuiteSetup) {
  ...
  async function createConfiguredGlobalAgentSessionStore({
    ...
    const configPath = process.env.AETHER_CONFIG_PATH;
    if (!configPath) {
      throw new Error("AETHER_CONFIG_PATH is required");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
