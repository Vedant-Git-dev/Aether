...
import { closeAetherStateDatabaseForTest } from "../../state/aether-state-db.js";
import { createAetherTestState } from "../../test-utils/aether-test-state.js";
...
async function initializeRemoteBackedGitSeed(root: string): Promise<void> {
  ...
  await execFileAsync("git", ["-C", workspace, "config", "user.name", "Aether Test"]);
  await execFileAsync("git", [
    "-C",
    workspace,
    "config",
    "user.email",
    "aether-test@example.invalid",
  ]);
...
export function setupGatewaySessionsWorktreeTestHarness() {
  ...
  const { createSessionStoreDir } = setupGatewaySessionsHandlerTestHarness(async (makeTempDir) => {
    seedRoot = makeTempDir("aether-worktree-seed-");
  ...
  async function createArchiveWorktreeFixture() {
    const state = await createAetherTestState({
      layout: "state-only",
      prefix: "aether-archive-worktree-",
    });
