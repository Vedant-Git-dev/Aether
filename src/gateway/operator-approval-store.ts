      ...
      return [
        {
          ...
          text: "Run aether vitals and inspect the shared state database before requesting the action again.",
        },
      ];
    default:
      return [
...
function projectCorruptOperatorApprovalReceipt(
  ...
  return {
    ...
    remediation: [
      {
        code: "inspect_state_integrity",
        text: "Run aether vitals and inspect the shared state database before trusting this approval.",
      },
    ],
  };
}
...
export function hasOperatorApprovalReceiptsForRun(params: {
  runId: string;
  nowMs?: number;
  databaseOptions?: AetherStateDatabaseOptions;
}): boolean {
  return (
...
function selectOperatorApprovalRow(
  database: ReturnType<typeof openAetherStateDatabase>,
  id: string,
...
export function insertOperatorApproval(params: {
  ...
  return runAetherStateWriteTransaction((database) => {
    const stateDb = getNodeSqliteKysely<OperatorApprovalDatabase>(database.db);
    executeSqliteQuerySync(
      database.db,
      stateDb
...
export function listTerminalOperatorApprovals(
  ...
  const database = openAetherStateDatabase(params.databaseOptions);
  const stateDb = getNodeSqliteKysely<OperatorApprovalDatabase>(database.db);
  const records: OperatorApprovalRecord[] = [];
