...
import { ensureRepositoryGitHubPublicationSchema } from "../state/aether-state-db-schema-additive.js";
...
export function claimRepositoryGitHubPublication(
  ...
  const claimed = runAetherStateWriteTransaction(
    ({ db }) => {
      assertCurrent();
...
export function markRepositoryGitHubPublicationReported(requestId: string): void {
  const database = openAetherStateDatabase().db;
