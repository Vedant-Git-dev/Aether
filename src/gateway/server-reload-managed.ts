...
export function startManagedGatewayConfigReloader(
  ...
  const applyRuntimeConfigOverrides = (config: AetherConfig): AetherConfig => {
  ...
  const {
    ...
    assertRestartReady: () =>
      import("../state/aether-database-preflight.js").then(({ assertAetherDatabasesReady }) =>
        assertAetherDatabasesReady({ env: process.env, operation: "gateway-restart" }),
      ),
  ...
  let lastCommittedRuntimeConfig: AetherConfig | undefined;
