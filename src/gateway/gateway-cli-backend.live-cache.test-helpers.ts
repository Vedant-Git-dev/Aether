...
import { loadAetherPlugins } from "../plugins/loader.js";
...
export async function initializeCacheProbeGitWorkspace(workspaceDir: string): Promise<void> {
  ...
  await execFileAsync("git", ["-C", workspaceDir, "config", "user.name", "Aether Tests"]);
  await execFileAsync("git", [
    "-C",
    workspaceDir,
    "config",
    "user.email",
    "aether-tests@localhost",
  ]);
...
export function prepareClaudeCacheProbeBackend(params: {
  ...
  return {
    ...
    ...(registration.builtWithAetherVersion
      ? { builtWithAetherVersion: registration.builtWithAetherVersion }
      : {}),
  };
}
...
export async function verifyCliBackendAnnounceOrdering({
  ...
  const announceRequest = client.request(
    "agent",
    {
      ...
      timeout: 240,
      message: [
        "Run this exact Aether CLI-backed completion announcement scenario. Use tool calls, not prose.",
