...
function stateDatabaseOptions(stateDir?: string): AetherStateDatabaseOptions {
  return stateDir
...
export function readManagedImageRecord(
  ...
  const database = openAetherStateDatabase(stateDatabaseOptions(stateDir));
...
export function listManagedImageRecordEntries(params: {
  ...
  const database = openAetherStateDatabase(stateDatabaseOptions(params.stateDir));
