...
import { asNullableRecord as asLoopbackSchemaRecord } from "@aether/normalization-core/record-coerce";
...
async function callLoopbackJsonRpc(params: {
  ...
  if (params.messageProvider) {
    headers["x-aether-message-channel"] = params.messageProvider;
  }
  if (params.accountId) {
    headers["x-aether-account-id"] = params.accountId;
  }
  const timeoutMs = parsePositiveInt(
    params.env?.AETHER_MCP_LOOPBACK_PROBE_TIMEOUT_MS,
    CLI_CRON_MCP_LOOPBACK_REQUEST_TIMEOUT_MS,
    "AETHER_MCP_LOOPBACK_PROBE_TIMEOUT_MS",
  );
  const maxBodyBytes = parsePositiveInt(
    params.env?.AETHER_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES,
    CLI_CRON_MCP_LOOPBACK_MAX_BODY_BYTES,
    "AETHER_MCP_LOOPBACK_PROBE_MAX_BODY_BYTES",
  );
