...
import { maxBytesForKind, mediaKindFromMime, type MediaKind } from "@aether/media-core/constants";
import { mimeTypeFromFilePath, normalizeMimeType } from "@aether/media-core/mime";
...
async function recordMatchesTranscriptMessage(
  ...
  const env = stateDir ? { ...process.env, AETHER_STATE_DIR: stateDir } : process.env;
  ...
  for (const message of messages) {
    const meta = (message as { __aether?: { id?: string } } | null)?.["__aether"];
