// OpenAI-compatible `/v1/models` HTTP route backed by configured Aether agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([AETHER_MODEL_ID, AETHER_DEFAULT_MODEL_ID]);
  const compatibilityAgentId = tryResolveLegacyCompatibilityAgentId(cfg);
  if (compatibilityAgentId) {
    ids.add(`aether/${compatibilityAgentId}`);
  }
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`aether/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (!isAetherAgentModelId(decodedId)) {
    sendInvalidRequest(res, "Invalid model id.");
    return true;
  }

  const normalizedModelId = decodedId.trim().toLowerCase();
  if (normalizedModelId !== AETHER_MODEL_ID && normalizedModelId !== AETHER_DEFAULT_MODEL_ID) {
    const cfg = getRuntimeConfig();
