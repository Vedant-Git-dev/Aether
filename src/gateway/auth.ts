...
export function assertGatewayAuthConfigured(
  ...
  if (auth.mode === "token" && isInvalidGatewaySecret(auth.token)) {
    throw new Error(
      "Gateway token must not be blank or the literal string undefined/null. Run `aether vitals --fix --generate-gateway-token` for an inline token, or rotate its external secret source.",
    );
  }
  ...
  if (auth.mode === "password" && !auth.password) {
    if (
      ...
      throw new Error(
        "gateway auth mode is password, but gateway.auth.password contains a provider reference object instead of a resolved string — bootstrap secrets (gateway.auth.password) must be plaintext strings or set via the AETHER_GATEWAY_PASSWORD environment variable because the secrets provider system has not initialised yet at gateway startup", // pragma: allowlist secret
      );
    }
    throw new Error(
  ...
  if (auth.mode === "trusted-proxy") {
    ...
    if (auth.token) {
      throw new Error(
        "gateway auth mode is trusted-proxy, but a shared token is also configured; remove gateway.auth.token / AETHER_GATEWAY_TOKEN because trusted-proxy and token auth are mutually exclusive",
      );
    }
  }
}
