...
import { prepareGatewayAgentCliShim } from "../infra/aether-cli-shim.js";
...
export async function prepareGatewayServerBootstrap(input: {
  ...
  const [
    {
      ...
      AetherDatabaseSchemaPreflightError,
    ...
    Promise.all([
      import("../state/aether-database-preflight.js"),
      import("../state/aether-agent-db.js"),
      import("../state/aether-state-db-contract.js"),
    ]),
  );
  const inspectDatabaseSchemas = (signal?: AbortSignal) =>
    preflightAetherDatabaseSchemas({
      ...
      env: process.env,
      supportedVersions: {
        state: stateDatabase.AETHER_STATE_SCHEMA_VERSION,
        agent: agentDatabase.AETHER_AGENT_SCHEMA_VERSION,
      },
    });
  ...
  if (databaseSchemas.incompatible.length > 0) {
    for (const database of databaseSchemas.incompatible) {
      log.error("database schema preflight rejected newer schema", {
        ...
        docsUrl: AETHER_DATABASE_SCHEMA_DOCS_URL,
      });
    }
    throw new AetherDatabaseSchemaPreflightError(databaseSchemas.incompatible);
  }
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.AETHER_TEST_MINIMAL_GATEWAY === "1";
  const ambientEnvTriggers = opts.ambientEnvTriggers ?? "suppress";

  logAcceptedEnvOption({
    key: "AETHER_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "AETHER_RAW_STREAM_PATH",
  ...
  const seededControlUiAllowedOrigins = controlUiSeed.seededAllowedOrigins
    ...
    : undefined;
  const applyFixedGatewayOverlays = (config: AetherConfig): AetherConfig => {
  ...
  const applyReloadableGatewayAuthRefs = (config: AetherConfig): AetherConfig => {
  ...
  const prepareReloadCandidate = (params: {
    ...
    const reapplyCompareOverlays = (config: AetherConfig): AetherConfig => {
    ...
    const reapplyRuntimeOverlays = (config: AetherConfig): AetherConfig =>
