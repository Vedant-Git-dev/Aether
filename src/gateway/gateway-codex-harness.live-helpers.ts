...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current Aether session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsAetherStatus =
    normalized.includes("aether is running on") ||
    /aether\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("aether status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsAetherStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from aether status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live aether config shows") ||
