...
const HOOKS_GATEWAY_AUTH_REUSE_WARNING =
  "Security warning: hooks.token matches active Gateway shared-secret auth. Startup continues for compatibility; rotate hooks.token or Gateway auth. Run aether security audit for a full report, and run aether doctor --fix when the reused hooks.token is persisted in config.";
...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken = trimToUndefined(params.env.AETHER_GATEWAY_TOKEN);
  if (envToken) {
    return true;
  }
  if (
...
export async function ensureGatewayStartupAuth(params: {
  ...
  const [resolvedTokenRefValue, resolvedPasswordRefValue] = await Promise.all([
    resolveGatewayTokenSecretRefValue({
      ...
      hasTokenFallback: Boolean(trimToUndefined(env.AETHER_GATEWAY_TOKEN)),
    ...
    resolveGatewayPasswordSecretRefValue({
      ...
      hasTokenOverride:
        hasGatewayTokenOverrideCandidate({ authOverride: params.authOverride }) ||
        tokenAlreadySubstituted,
      hasPasswordFallback: Boolean(trimToUndefined(env.AETHER_GATEWAY_PASSWORD)),
