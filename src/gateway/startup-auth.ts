...
const HOOKS_GATEWAY_AUTH_REUSE_WARNING =
  "Security warning: hooks.token matches active Gateway shared-secret auth. Startup continues for compatibility; rotate hooks.token or Gateway auth. Run aether security audit for a full report, and run aether vitals --fix when the reused hooks.token is persisted in config.";
