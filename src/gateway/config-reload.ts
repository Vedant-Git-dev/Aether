...
export type GatewayConfigReloadTransactionOwnership = {
  isCurrent: () => boolean;
  markRuntimeCommitted: (runtimeConfig: AetherConfig, plan: GatewayReloadPlan) => void;
  commitRuntimeEnv: () => void;
  publishRuntimeEnv: () => void;
  rollbackRuntimeEnv: () => void;
  reapplyRuntimeOverlays: (config: AetherConfig) => AetherConfig;
  runtimeEnv?: NonNullable<ConfigWriteNotification["preparedCandidate"]>["runtimeEnv"];
  runtimeRefresh?: RuntimeConfigSnapshotRefreshOptions;
};

type PreparedGatewayConfigCandidate = {
  runtimeConfig: AetherConfig;
  compareConfig: AetherConfig;
  runtimeEnv?: NonNullable<ConfigWriteNotification["preparedCandidate"]>["runtimeEnv"];
  reapplyRuntimeOverlays?: (config: AetherConfig) => AetherConfig;
  reapplyCompareOverlays?: (config: AetherConfig) => AetherConfig;
};

function asPluginInstallConfig(records: PluginInstallRecords): AetherConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  initialConfig: AetherConfig;
  initialCompareConfig?: AetherConfig;
  ...
  prepareConfigCandidate?: (params: {
    ...
    previousSourceConfig: AetherConfig;
  }) => PreparedGatewayConfigCandidate;
  initialInternalWriteHash?: string | null;
  readSnapshot: (activeSourceConfig: AetherConfig) => Promise<ConfigFileSnapshot>;
  /** Pauses restart emission synchronously when a matching disk candidate is observed. */
  onConfigCandidateObserved?: () => void;
  onConfigChange?: (plan: GatewayReloadPlan, nextConfig: AetherConfig) => void | Promise<void>;
  /** Publishes runtime state after a hot or no-op config transaction. */
  onConfigApplied?: (plan: GatewayReloadPlan, nextConfig: AetherConfig) => void | Promise<void>;
  /** Runs synchronously when a config transaction publishes its runtime state. */
  onRuntimeConfigCommitted?: (plan: GatewayReloadPlan, nextConfig: AetherConfig) => void;
  ...
  let currentReapplyRuntimeOverlays =
    initialCandidate?.reapplyRuntimeOverlays ?? ((config: AetherConfig) => config);
  let currentRuntimeRefresh: RuntimeConfigSnapshotRefreshOptions | undefined;
  const resolveSettings = (config: AetherConfig) => {
  ...
  let lastSourceOnlyReapplyRuntimeOverlays: ((config: AetherConfig) => AetherConfig) | null =
    null;
  let lastSourceOnlyRuntimeRefresh: RuntimeConfigSnapshotRefreshOptions | undefined;
  let lastSourceOnlyRuntimeConfig: AetherConfig | null = null;
  let lastSourceOnlySourceConfig: AetherConfig | null = null;
  ...
  const applySnapshot = async (
    candidateRuntimeConfig: AetherConfig,
    nextSourceConfig: AetherConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
    ...
    let committedRuntimeConfig: AetherConfig | null = null;
