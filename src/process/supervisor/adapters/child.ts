...
function resolveChildInvocation(params: {
  ...
  const candidate = resolveWindowsSpawnProgramCandidate({
    command,
    env: params.env,
    // npm shims invoke `node` from PATH; process.execPath may be a packaged Aether executable.
    execPath:
...
const WORKER_START_MESSAGE = { type: "aether-worker-start-v1" } as const;
...
export async function createChildAdapter(params: ChildAdapterInput): Promise<WorkerChildAdapter> {
  ...
  if (
    ...
    (params.ownProcessTree === true || process.env.AETHER_SERVICE_MARKER?.trim())
  ) {
    return await createServiceChildRelayAdapter({
      assertCurrent: params.assertCurrent,
