...
type AgentLookupCache = {
  agentsRef: AetherConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackSoleAgentId?: string;
};

const agentLookupCacheByCfg = new WeakMap<AetherConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AetherConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AetherConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AetherConfig["bindings"];
  byChannel: Map<string, EvaluatedBindingsByChannel>;
  byChannelAccount: Map<string, EvaluatedBindingsEntry>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<AetherConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AetherConfig,
  {
    bindingsRef: AetherConfig["bindings"];
    agentsRef: AetherConfig["agents"];
    sessionRef: AetherConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AetherConfig): Map<string, ResolvedAgentRoute> {
...
export function listEffectiveGroupRouteBindings(cfg: AetherConfig) {
