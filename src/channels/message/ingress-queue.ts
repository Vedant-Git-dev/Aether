 ...
 *
 * Stores, claims, completes, and tombstones inbound channel events in Aether state.
 */
...
type ChannelIngressDatabase = Pick<AetherStateKyselyDatabase, "channel_ingress_events">;
...
const FAILED_NULL_PAYLOAD_SENTINEL = "AETHER_CHANNEL_INGRESS_FAILED_NULL_V1";
...
export function openChannelIngressDatabase(stateDir?: string) {
  return openAetherStateDatabase({
...
async function openChannelIngressDatabaseForListing(
  ...
  const database = await openExistingAetherStateDatabaseReadOnly({ env });
  if (!database) {
    return undefined;
  }
  return {
...
/** Creates a durable channel/account-scoped ingress queue backed by the Aether state database. */
  ...
  >["recoverStaleClaims"] = async (recoverOptions) => {
    ...
    for (const row of claimedRows) {
      ...
      if (claimRec === null) {
        ...
        const tombstoned = runAetherStateWriteTransaction(
          (tx) =>
            tombstoneCorruptRow({
              db: tx.db,
              row,
