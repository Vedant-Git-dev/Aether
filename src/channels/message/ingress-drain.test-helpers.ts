...
export async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-ingress-drain-"),
  );
  try {
...
export function seedPendingBacklog(stateDir: string, total: number): void {
  const database = openAetherStateDatabase({ env: { AETHER_STATE_DIR: stateDir } });
  const kysely = getNodeSqliteKysely<Pick<AetherStateKyselyDatabase, "channel_ingress_events">>(
    database.db,
  );
  for (let offset = 0; offset < total; offset += 500) {
    const rows: Array<Insertable<AetherStateKyselyDatabase["channel_ingress_events"]>> = [];
