...
const ENV_CATALOG_PATHS = ["AETHER_PLUGIN_CATALOG_PATHS", "AETHER_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveAetherPackageRootSync({ cwd: process.cwd() }),
      resolveAetherPackageRootSync({ moduleUrl: import.meta.url }),
