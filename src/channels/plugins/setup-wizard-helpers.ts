...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: AetherConfig) => Array<string | number> | undefined;
}): AetherConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: AetherConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: AetherConfig) => Array<string | number> | undefined;
}): (cfg: AetherConfig, dmPolicy: DmPolicy) => AetherConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: AetherConfig, allowFrom: string[]) => AetherConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: AetherConfig, groupPolicy: "open" | "allowlist" | "disabled") => AetherConfig {
  ...
  const promptToken = async (): Promise<string> =>
    (
      await params.prompter.text({
        ...
        // Credential input: masked in terminal prompts, and the Aether
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: AetherConfig) => AetherConfig | Promise<AetherConfig>;
  applySet?: (
    cfg: AetherConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Aether config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "AETHER_SECRET",
      envVarFormatError:
        'Use an env var name like "AETHER_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends AetherConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends AetherConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  channel: string;
  defaultAccountId: string | ((cfg: AetherConfig) => string);
  enabled?: boolean;
  noteTitle?: string;
  noteLines?: string[];
  message: string;
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: AetherConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
  ...
  const sharedParams = {
    ...
    placeholder: params.placeholder,
    parseEntries: params.parseEntries,
    getExistingAllowFrom: ({ cfg }: { cfg: AetherConfig }) =>
      params.getExistingAllowFrom?.(cfg) ??
      (cfg.channels?.[params.channel] as { allowFrom?: Array<string | number> } | undefined)
        ?.allowFrom ??
      [],
    ...(params.mergeEntries ? { mergeEntries: params.mergeEntries } : {}),
    applyAllowFrom: ({ cfg, allowFrom }: { cfg: AetherConfig; allowFrom: string[] }) =>
