...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: AetherConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: AetherConfig }>(
  collector: WarningCollector<{ cfg: AetherConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: AetherConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: AetherConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: AetherConfig },
>(
  projectCfg: (cfg: AetherConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: AetherConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: AetherConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: AetherConfig },
>(params: {
  providerConfigPresent: (cfg: AetherConfig) => boolean;
