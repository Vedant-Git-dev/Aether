...
function resolveScopedChannelMediaMaxBytes(params: {
  cfg: AetherConfig;
  accountId?: string | null;
  resolveChannelLimitMb: (params: { cfg: AetherConfig; accountId: string }) => number | undefined;
}): number | undefined {
...
export function createScopedChannelMediaMaxBytesResolver(channel: string) {
  return (params: { cfg: AetherConfig; accountId?: string | null }) =>
...
>(params: {
  ...
  const outbound: ChannelOutboundAdapter = {
    ...
    sendPayload: async (ctx) => {
      const { sendTextMediaPayload } = await import("aether/plugin-sdk/reply-payload");
