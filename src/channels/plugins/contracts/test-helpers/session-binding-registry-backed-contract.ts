...
export function describeSessionBindingRegistryBackedContract(id: string) {
  ...
  describe(`${entry.id} session binding contract`, () => {
    let testState: AetherTestState | undefined;
    ...
    beforeEach(async () => {
      testState = await createAetherTestState({
      ...
      // plugin boundaries, so never fall back to the developer's real ~/.aether/aether.json here.
