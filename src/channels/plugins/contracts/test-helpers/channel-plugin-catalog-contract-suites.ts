    ...
    JSON.stringify({
      ...(richManifest
        ? {
            ...
            schemaVersion: 1,
            description:
              "Extension manifest. Declares plugin packages that Aether can discover during onboarding and install on demand via `aether plugins install`.",
          }
        : {}),
      entries: [entry],
    }),
...
function createTemporaryCatalogFile(
  ...
  const directory = fs.mkdtempSync(path.join(resolvePreferredAetherTmpDir(), prefix));
...
const [richNpmCatalogFixture, clawhubCatalogFixture, yuanbaoCatalogFixture] = [
  {
    ...
    prefix: "aether-catalog-rich-",
    channelId: "wecom",
    entry: {
      name: "@wecom/wecom-aether-plugin",
      description:
        "Aether WeCom (企业微信) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      aether: {
        channel: {
        ...
        install: {
          npmSpec: "@wecom/wecom-aether-plugin@1.2.3",
          defaultChoice: "npm",
    ...
    expected: {
      ...
      installSource: {
        defaultChoice: "npm",
        npm: {
          spec: "@wecom/wecom-aether-plugin@1.2.3",
          packageName: "@wecom/wecom-aether-plugin",
          selector: "1.2.3",
  ...
  },
  {
    ...
    entry: {
      ...
      aether: {
        ...
        install: {
          clawhubSpec: "clawhub:aether/clawhub-chat@2026.5.2",
    ...
    expected: {
      ...
      installSource: {
        defaultChoice: "clawhub",
        clawhub: {
          spec: "clawhub:aether/clawhub-chat@2026.5.2",
          packageName: "aether/clawhub-chat",
          version: "2026.5.2",
          exactVersion: true,
        },
        warnings: [],
  ...
  },
  {
    ...
    prefix: "aether-catalog-yuanbao-",
    channelId: "aether-plugin-yuanbao",
    entry: {
      name: "aether-plugin-yuanbao",
      description:
        "Aether Yuanbao (元宝) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      aether: {
        channel: {
          id: "aether-plugin-yuanbao",
          label: "Yuanbao",
        ...
        install: {
          npmSpec: "aether-plugin-yuanbao@1.0.0",
          defaultChoice: "npm",
          minHostVersion: ">=2026.4.10",
...
export function describeChannelPluginCatalogEntriesContract() {
  const cases: ChannelCatalogContractCase[] = [
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(
          path.join(resolvePreferredAetherTmpDir(), "aether-channel-catalog-state-"),
        );
    ...
    },
    {
      ...
      setup: () => {
        const stateDir = fs.mkdtempSync(
          path.join(resolvePreferredAetherTmpDir(), "aether-catalog-state-"),
        );
    ...
    createRichExternalCatalogCase(richNpmCatalogFixture),
    {
      ...
      setup: () => ({
        ...
        catalogPaths: [
          createTemporaryCatalogFile("aether-catalog-prerelease-", {
            ...createCatalogEntry({
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredAetherTmpDir(), "aether-catalog-home-"),
          );
          writeCatalogFile(
          ...
          return {
            env: {
              ...process.env,
              AETHER_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              AETHER_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          return {
            env: { ...process.env, AETHER_STATE_DIR: stateDir },
