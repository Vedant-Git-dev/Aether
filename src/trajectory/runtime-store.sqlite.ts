...
import { parseDateStringTimestampMs } from "@aether/normalization-core/number-coercion";
import {
...
const lastGlobalSweepAtByDatabase = new WeakMap<AetherAgentDatabase, number>();
...
export function appendSqliteTrajectoryRuntimeEvents(
  ...
  let sweptDatabase: AetherAgentDatabase | undefined;
...
export function loadSqliteTrajectoryRuntimeEventRowsSync(
  ...
  const read = withAetherAgentDatabaseReadOnly(
    (database) => {
      const db = getTrajectoryKysely(database.db);
      const tailEvents =
...
function readSqliteTrajectoryRuntimeRuns(database: AetherAgentDatabase): TrajectoryRuntimeRun[] {
  const db = getTrajectoryKysely(database.db);
...
function readNextTrajectorySeq(database: AetherAgentDatabase, sessionId: string): number {
  const db = getTrajectoryKysely(database.db);
