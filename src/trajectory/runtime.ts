...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with AETHER_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.AETHER_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
