...
const AETHER_NODE_OPTIONS_READY = "AETHER_NODE_OPTIONS_READY";
const AETHER_NODE_EXTRA_CA_CERTS_READY = "AETHER_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (
    ...
    isTruthyEnvValue(env.AETHER_NO_RESPAWN)
  ) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[AETHER_NODE_EXTRA_CA_CERTS_READY]) &&
    !childEnv.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[AETHER_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !shouldSkipRespawnForArgv(argv, platform) &&
    !isTruthyEnvValue(env[AETHER_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[AETHER_NODE_OPTIONS_READY] = "1";
