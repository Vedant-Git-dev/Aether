#!/usr/bin/env node
// Boots the Aether CLI entry point under Node.
// CLI process entrypoint for Aether command execution.
...
import {
  ...
  respawnWithoutAetherCompileCacheIfNeeded,
...
import { ensureAetherExecMarkerOnProcess } from "./infra/aether-exec-env.js";
...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "aether.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "aether.mjs", entryBasename: "entry.mjs" },
  { wrapperBasename: "aether.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "aether";
  ensureAetherExecMarkerOnProcess();
  ...
  const waitingForCompileCacheRespawn = await respawnWithoutAetherCompileCacheIfNeeded({
  ...
  if (!waitingForCompileCacheRespawn) {
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.AETHER_AUTH_STORE_READONLY = "1";
    }
...
export async function tryHandleRootHelpFastPath(
  ...
  if (
    env.AETHER_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1" ||
  ...
  const handleError =
    ...
    (async (error: unknown) => {
      ...
      const writeError = await prepareCliDiagnosticBlockWriter();
      await writeError(`[aether] Failed to display help: ${detail}\n`);
      process.exit(1);
    });
  try {
