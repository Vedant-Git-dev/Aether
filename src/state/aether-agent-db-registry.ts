...
import { invalidateRegisteredAgentDatabasesMemo } from "./aether-agent-db-registry-listing.js";
import {
  invalidateAetherAgentDatabaseValidation,
  invalidateAetherAgentDatabaseValidationsForAgent,
} from "./aether-agent-db-validation-cache.js";
import type { AetherStateDatabase } from "./aether-state-db-contract.js";
import type { DB as AetherStateKyselyDatabase } from "./aether-state-db.generated.js";
import { runAetherStateWriteTransaction } from "./aether-state-db.js";
import { resolveAetherAgentDatabaseStoredPath } from "./aether-state-db.paths.js";

export {
  inspectAetherRegisteredAgentDatabases,
  listAetherRegisteredAgentDatabases,
  readAetherAgentDatabaseRegistryToken,
} from "./aether-agent-db-registry-listing.js";
...
export function createAetherAgentDatabasePathMatcher(): (left: string, right: string) => boolean {
...
export function isSameAetherAgentDatabasePath(left: string, right: string): boolean {
...
export function registerAetherAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
  schemaVersion?: number;
}): void {
  if (!isPersistentAetherAgentDatabasePath(params.path, params.env)) {
    return;
  }
  ...
  runAetherStateWriteTransaction(
    (database) => {
      ...
      const storedPath = resolveAetherAgentDatabaseStoredPath(database.path, params.path);
      const db = getNodeSqliteKysely<AetherAgentRegistryDatabase>(database.db);
      executeSqliteQuerySync(
        ...
        db
          ...
          .values({
            ...
            path: storedPath,
            schema_version: params.schemaVersion ?? AETHER_AGENT_SCHEMA_VERSION,
  ...
  invalidateAetherAgentDatabaseValidation(params.path);
...
export function isPersistentAetherAgentDatabasePath(
  pathname: string,
...
export function unregisterAetherAgentDatabase(params: {
  agentId: string;
  path: string;
  env?: NodeJS.ProcessEnv;
}): void {
...
export function unregisterAetherAgentDatabases(params: {
  agentId: string;
  env?: NodeJS.ProcessEnv;
  ...
  invalidateAetherAgentDatabaseValidationsForAgent(params.agentId);
