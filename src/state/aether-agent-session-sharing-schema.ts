import { withLegacySessionParticipantsSchema } from "./aether-agent-participants-migration.js";
import { AGENT_SCHEMA_WITHOUT_PROGRESS_CARD_SQL } from "./aether-agent-progress-card-schema.js";
...
function splitSessionSharingSchema(sql: string): { sharing: string; withoutSharing: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Aether agent session-sharing schema markers are missing.");
  }
  return {
...
if (sessionSuggestionsStart === -1) {
  throw new Error("Aether agent session-suggestions schema marker is missing.");
}
