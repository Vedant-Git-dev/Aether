...
import { repairLegacySubagentRetainedResults } from "./aether-state-db-legacy-backfills.js";
import { tableExists, tableHasColumn } from "./aether-state-db-schema-helpers.js";
...
function reprojectLegacyCronJson(db: DatabaseSync): void {
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Aether v12 cron job row is not canonical");
    }
...
function rebuildJsonCanonicalTable(db: DatabaseSync, tableName: string): void {
  ...
  if (tableExists(db, migrationTable)) {
    throw new Error(`Aether v13 migration table already exists: ${migrationTable}`);
  }
  const startMarker = `CREATE TABLE IF NOT EXISTS ${tableName} (`;
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(startMarker);
  const endMarker = "\n) STRICT;";
  ...
  const migrationSchema = AETHER_STATE_SCHEMA_SQL.slice(start, end + endMarker.length).replace(
    startMarker,
