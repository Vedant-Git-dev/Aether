...
} from "./aether-state-db-additive-columns.js";
import {
...
} from "./aether-state-db-legacy-backfills.js";
import { ensureColumn, tableHasColumn } from "./aether-state-db-schema-helpers.js";
...
export function ensureSessionRepositoryWorkspaceSchema(database: DatabaseSync): void {
  ...
  database.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + marker.length)); // sqlite-allow-raw -- Canonical first-use DDL; workspace rows use Kysely.
}
...
export function ensureRepositoryGitHubPublicationSchema(database: DatabaseSync): void {
  ...
  database.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + marker.length)); // sqlite-allow-raw -- Canonical first-use DDL; publication rows use Kysely.
}
...
export function ensureGitHubPublicationSessionLifecycleSchema(database: DatabaseSync): void {
  ...
  database.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + marker.length)); // sqlite-allow-raw -- Canonical first-use DDL; bindings use Kysely.
}

function secretStoreSchemaSql(): string {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_START);
  const endMarkerStart = AETHER_STATE_SCHEMA_SQL.indexOf(SECRET_STORE_SCHEMA_END, start);
  const hasBoundedSchema = start >= 0 && endMarkerStart >= start;
  if (!hasBoundedSchema) {
    throw new Error("Aether secret store schema marker is missing.");
  }
  return AETHER_STATE_SCHEMA_SQL.slice(start, endMarkerStart + SECRET_STORE_SCHEMA_END.length);
}
...
export function ensureMcpOAuthPendingSchema(database: DatabaseSync): void {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_START);
  const endMarkerStart = AETHER_STATE_SCHEMA_SQL.indexOf(MCP_OAUTH_PENDING_SCHEMA_END, start);
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Aether MCP OAuth pending schema marker is missing.");
  }
  database.exec(
    AETHER_STATE_SCHEMA_SQL.slice(start, endMarkerStart + MCP_OAUTH_PENDING_SCHEMA_END.length),
...
export function ensureDevicePairingJoinCodeSchema(database: DatabaseSync): void {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(DEVICE_PAIRING_JOIN_CODE_SCHEMA_START);
  const endMarkerStart = AETHER_STATE_SCHEMA_SQL.indexOf(
    DEVICE_PAIRING_JOIN_CODE_SCHEMA_END,
    start,
  );
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Aether device pairing join-code schema marker is missing.");
  }
  database.exec(
    AETHER_STATE_SCHEMA_SQL.slice(
      start,
...
export function ensureConfigRevisionKeySchema(database: DatabaseSync): void {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(CONFIG_REVISION_KEY_SCHEMA_START);
  const endMarkerStart = AETHER_STATE_SCHEMA_SQL.indexOf(CONFIG_REVISION_KEY_SCHEMA_END, start);
  if (start < 0 || endMarkerStart < start) {
    throw new Error("Aether config revision key schema marker is missing.");
  }
  database.exec(
    AETHER_STATE_SCHEMA_SQL.slice(start, endMarkerStart + CONFIG_REVISION_KEY_SCHEMA_END.length),
...
export function ensurePersonalGitHubPublicationSchema(db: DatabaseSync): void {
  ...
  db.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + marker.length)); // sqlite-allow-raw -- Canonical lazy additive DDL only.
}
