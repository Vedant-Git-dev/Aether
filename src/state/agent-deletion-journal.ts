...
import { ensureAgentDeletionJournalSchema } from "./aether-state-db-schema-additive.js";
...
function assertAgentDeletionIdentityClaimAllowed(
  ...
  if (deletedAgentId && normalizeAgentId(claimAgentId) === normalizeAgentId(deletedAgentId)) {
    throw new Error(
      `Aether agent database is unavailable while agent ${normalizeAgentId(deletedAgentId)} is deleted.`,
    );
  }
}
...
export function assertAgentDeletionPathFence(
  ...
  for (const row of journalRows) {
    ...
    for (const fence of fences) {
      ...
      if (blockedPath) {
        throw new Error(
          `Aether agent database ${blockedPath} is unavailable while agent ${row.agent_id} deletion owns ${fence.path}.`,
        );
      }
    }
  }
  ...
  runAetherStateWriteTransaction((database) => {
    ...
    const registeredDatabasePaths = executeSqliteQuerySync(
      ...
      resolveSqliteDatabaseFilePaths(
        resolveAetherRegisteredAgentDatabasePath(database.path, row.path),
      ),
    );
    const databasePaths = [
      ...new Set(
