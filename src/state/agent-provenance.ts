...
import { createAetherStateSchemaEnsurer } from "./aether-state-feature-schema.js";
...
type AgentProvenanceDatabase = Pick<AetherStateKyselyDatabase, "agent_provenance">;
type AgentProvenanceOptions = AetherStateDatabaseOptions & { nowMs?: number };

export const ensureAgentProvenanceSchema = createAetherStateSchemaEnsurer({
...
export function listAgentProvenance(options: AetherStateDatabaseOptions = {}): AgentProvenance[] {
  ensureAgentProvenanceSchema(options);
  const database = openAetherStateDatabase(options);
  const db = getNodeSqliteKysely<AgentProvenanceDatabase>(database.db);
