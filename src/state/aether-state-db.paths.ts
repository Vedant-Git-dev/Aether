// State database path helpers resolve shared Aether state DB paths.
import path from "node:path";
import { resolveStateDir } from "../config/paths.js";

/** Resolve the directory that contains the shared state SQLite file. */
export function resolveAetherStateSqliteDir(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveStateDir(env), "state");
}

/** Resolve the shared state SQLite file path. */
export function resolveAetherStateSqlitePath(env: NodeJS.ProcessEnv = process.env): string {
  return path.join(resolveAetherStateSqliteDir(env), "aether.sqlite");
}

/** Resolve the state owner directory for a canonical or explicit shared database path. */
export function resolveAetherStateDirForDatabasePath(databasePath: string): string {
...
export function resolveAetherAgentDatabaseStoredPath(
  registryDatabasePath: string,
  agentDatabasePath: string,
): string {
  const stateDir = resolveAetherStateDirForDatabasePath(registryDatabasePath);
...
export function resolveAetherRegisteredAgentDatabasePath(
  registryDatabasePath: string,
  storedPath: string,
): string {
  return path.isAbsolute(storedPath)
    ? storedPath
    : `${resolveAetherStateDirForDatabasePath(registryDatabasePath)}${path.sep}${storedPath}`;
}
