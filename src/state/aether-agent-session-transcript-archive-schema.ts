...
function sessionTranscriptArchiveSchemaSql(): string {
  const start = AETHER_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_START);
  const end = AETHER_AGENT_SCHEMA_SQL.indexOf(ARCHIVE_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Aether session transcript archive schema markers are missing.");
  }
