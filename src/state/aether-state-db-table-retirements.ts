...
function assertRecognizedRetiredCommitmentsSchema(db: DatabaseSync): void {
  ...
  assertSqliteSchemaContains(
    db,
    "retired Aether commitments schema",
    RETIRED_COMMITMENTS_SCHEMA_SQL,
    deriveRetiredCommitmentsContract().compatibility,
  );
  throw new Error(
    "Retired Aether commitments schema has unsupported additional indexes; refusing destructive migration.",
  );
}
...
function assertNoRetiredCommitmentsForeignKeys(db: DatabaseSync): void {
  ...
  for (const table of tables) {
    ...
    if (
      ...
      throw new Error(
        `Retired Aether commitments schema is referenced by table ${table.name}; refusing destructive migration.`,
      );
    }
  }
}
...
function assertNoRetiredCommitmentsSchemaDependencies(db: DatabaseSync): void {
  const probeTable = "__aether_retired_commitments_probe";
  if (tableExists(db, probeTable)) {
    throw new Error(
      `Aether state database already contains ${probeTable}; refusing destructive migration.`,
    );
  }
  const before = collectRetainedSchemaSql(db);
  const savepoint = "aether_probe_commitments_dependencies";
  ...
  if (changedObject) {
    ...
    throw new Error(
      `Retired Aether commitments schema is referenced by ${type} ${name}; refusing destructive migration.`,
    );
  }
}
...
function migrateRetiredCommitmentsSchema(db: DatabaseSync, previousVersion: number): boolean {
  ...
  const savepoint = "aether_retire_commitments_v7";
  db.exec(`SAVEPOINT ${savepoint};`);
  try {
