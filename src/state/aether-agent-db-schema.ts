...
import { safeParseJsonRecord } from "@aether/normalization-core";
...
import { normalizeNullableString as migratedText } from "@aether/normalization-core/string-coerce";
...
import { ensureAetherAgentBoardSchemaInTransaction } from "./aether-agent-board-schema.js";
import {
  AGENT_MEDIA_SCHEMA_VERSION,
  AETHER_AGENT_SCHEMA_VERSION,
  type AetherAgentDatabaseOptions,
} from "./aether-agent-db-contract.js";
import * as maintenanceAuthority from "./aether-agent-db-lease.js";
import { ensureAetherAgentDatabasePermissions } from "./aether-agent-db-permissions.js";
import { registerAetherAgentDatabase } from "./aether-agent-db-registry.js";
import {
  ...
  repairAndAssertAetherAgentV14SchemaForMigration,
...
import { migrateSessionNodesAndWindows } from "./aether-agent-db-session-nodes-migration.js";
import {
  addSessionProvenanceColumns,
  backfillSessionEntryProvenance,
  backfillTranscriptMutationWatermarks,
} from "./aether-agent-db-session-provenance.js";
import type { DB as AetherAgentKyselyDatabase } from "./aether-agent-db.generated.js";
...
} from "./aether-agent-participants-migration.js";
import { hasPendingInputConsumptionColumnMigration } from "./aether-agent-pending-inputs-schema.js";
import { AETHER_AGENT_SCHEMA_SQL } from "./aether-agent-schema.js";
import { AETHER_SQLITE_BUSY_TIMEOUT_MS } from "./aether-state-db.js";

type AetherAgentMetadataDatabase = Pick<AetherAgentKyselyDatabase, "schema_meta">;
...
function migrateAetherAgentSchema(db: DatabaseSync): void {
...
function backfillAetherAgentSchema(db: DatabaseSync, previousVersion: number): void {
...
export function* agentDatabaseIntegrityBeforeMutationSteps(
  ...
  if (userVersion === AETHER_AGENT_SCHEMA_VERSION && !hasPendingCurrentVersionMigration) {
    yield* verifyAndRepairCanonicalSqliteIndexSteps(database, pathname, AETHER_AGENT_SCHEMA_SQL, {
      allowMissingColumns: true,
      validateAfterRepair: () =>
        assertAetherAgentCurrentRuntimeSchema(database, { agentId, pathname }),
      diagnostics,
    });
    assertAetherAgentCurrentRuntimeSchema(database, { agentId, pathname });
  } else if (
...
function persistAgentSchemaMetadata(
  ...
  executeSqliteQuerySync(
    db,
    getNodeSqliteKysely<AetherAgentMetadataDatabase>(db)
...
function ensureAgentSchema(
  ...
  const schemaSql =
    targetVersion < 18
      ? withLegacySessionParticipantsSchema(AETHER_AGENT_SCHEMA_SQL)
      : AETHER_AGENT_SCHEMA_SQL;
  ...
  try {
    runSqliteImmediateTransactionSync(db, () => {
      ...
      if (previousVersion > targetVersion) {
        throw new Error(
          `Aether agent database ${pathname} uses schema version ${previousVersion}; expected at most ${targetVersion} for this migration.`,
        );
      }
      if (previousVersion === AGENT_MEDIA_SCHEMA_VERSION) {
        const legacySql = withLegacySessionParticipantsSchema(AETHER_AGENT_SCHEMA_SQL);
      ...
      if (previousVersion === targetVersion) {
        ...
        repairAndAssertAetherAgentV14SchemaForMigration(db, { agentId, pathname });
      }
      ...
      migrateAetherAgentSchema(db);
      migrateConversationDeliveryTargetColumn(db);
      backfillAetherAgentSchema(db, previousVersion);
      ...
      migrateMemoryChunkMetadataSchema(db);
      if (previousVersion < targetVersion) {
        ensureAetherAgentBoardSchemaInTransaction(db);
      }
...
export function ensureAetherAgentDatabaseSchema(
  db: DatabaseSync,
  options: AetherAgentDatabaseOptions & { register?: boolean },
): void {
  runSqliteIntegrityOperationSync(ensureAetherAgentDatabaseSchemaSteps(db, options));
}

/** Share one schema sequence between synchronous callers and leased maintenance. */
export function* ensureAetherAgentDatabaseSchemaSteps(
  db: DatabaseSync,
  options: AetherAgentDatabaseOptions & { register?: boolean },
  ...
  if (databaseOptions.register === true) {
    registerAetherAgentDatabase({ agentId, path: pathname, env: databaseOptions.env });
  }
}

/** Upgrade older owned databases to the structural schema required by the media cutover. */
export function migrateAetherAgentDatabaseToMediaPrerequisiteSchema(
  db: DatabaseSync,
...
export { ensureAgentSchema as ensureAetherAgentSchema };
