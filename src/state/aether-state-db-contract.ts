...
export const AETHER_STATE_SCHEMA_VERSION = 17;
export const AETHER_STATE_STRICT_SCHEMA_VERSION = 3;
...
export const AETHER_SQLITE_BUSY_TIMEOUT_MS = 5_000;
/** User-facing guide for schema refusals; lives here so error sites avoid import cycles. */
export const AETHER_DATABASE_SCHEMA_DOCS_URL =
  "https://docs.aether.ai/reference/database-schemas";

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type AetherStateDatabase = {
  db: DatabaseSync;
  path: string;
  walMaintenance: SqliteWalMaintenance;
};
/** Options for resolving or overriding the shared state database path. */
export type AetherStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
  database?: AetherStateDatabase;
  readOnly?: boolean;
};
export type AetherStateDatabaseSchemaMigration = {
  kind:
