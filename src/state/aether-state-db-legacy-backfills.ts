...
import { asFiniteNumber, asSafeIntegerInRange } from "@aether/normalization-core/number-coercion";
...
import { compactLegacyDeliveryQueueFailures } from "./aether-state-db-delivery-queue-backfill.js";
...
export function backfillAcpReplayEstimatedBytes(db: DatabaseSync): void {
  ...
  const replayDb =
    getNodeSqliteKysely<
      Pick<AetherStateKyselyDatabase, "acp_replay_events" | "acp_replay_sessions">
    >(db);
