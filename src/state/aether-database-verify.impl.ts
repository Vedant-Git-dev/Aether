...
import { toStructuredErrorObject } from "@aether/normalization-core/error-coercion";
import { runtimeProcessEntrypoints } from "../infra/runtime-process-entrypoints.js";
import { resolveRuntimeWorkerUrl } from "../infra/runtime-worker-url.js";
import { createSubsystemLogger } from "../logging/subsystem.js";
import {
  confirmAetherAgentDatabaseIntegrity,
  listAetherRegisteredAgentDatabases,
  recordAetherAgentDatabaseOpenFailure,
} from "./aether-agent-db.js";
import type {
  AetherDatabaseVerifyResult,
  AetherDatabaseVerifyTarget,
} from "./aether-database-verify.worker.js";
import { recordAetherDatabaseQuarantine } from "./aether-quarantine-store.js";
import {
...
export const AETHER_DATABASE_VERIFY_INITIAL_DELAY_MS = 5 * 60_000;
export const AETHER_DATABASE_VERIFY_INTERVAL_MS = 24 * 60 * 60_000;

const log = createSubsystemLogger("state/database-verify");
const DATABASE_VERIFY_CHILD_ARG = "--aether-database-verify-child";

function isVerifyResult(value: unknown): value is AetherDatabaseVerifyResult {
...
export function runDatabaseVerifyWorker(
  targets: readonly AetherDatabaseVerifyTarget[],
  options: { onWorker?: (worker: ChildProcess | undefined) => void; workerUrl?: URL } = {},
): Promise<AetherDatabaseVerifyResult[]> {
  const workerUrl =
  ...
  let result: AetherDatabaseVerifyResult[] | undefined;
...
export function collectAetherDatabaseVerifyTargets(options: {
  env: NodeJS.ProcessEnv;
}): AetherDatabaseVerifyTarget[] {
  const targets = new Map<string, AetherDatabaseVerifyTarget>();
  const statePath = path.resolve(resolveAetherStateSqlitePath(options.env));
  if (existsSync(statePath)) {
    targets.set(statePath, { kind: "state", label: "Aether state database", path: statePath });
  }
  let registeredDatabases: ReturnType<typeof listAetherRegisteredAgentDatabases> = [];
  try {
    registeredDatabases = listAetherRegisteredAgentDatabases({ env: options.env });
  } catch (error) {
  ...
  for (const registered of registeredDatabases) {
    ...
    targets.set(agentPath, {
      kind: "agent",
      label: `Aether agent database ${registered.agentId}`,
      path: agentPath,
    });
  }
...
export function applyAetherDatabaseVerificationResults(options: {
  env: NodeJS.ProcessEnv;
  results: readonly AetherDatabaseVerifyResult[];
  targets: readonly AetherDatabaseVerifyTarget[];
}): void {
  ...
  for (const result of options.results) {
    ...
    const confirmation =
      target.kind === "state"
        ? confirmAetherStateDatabaseIntegrity(result.path)
        : confirmAetherAgentDatabaseIntegrity(result.path);
    ...
    const recorded = recordAetherDatabaseQuarantine({
      env: options.env,
