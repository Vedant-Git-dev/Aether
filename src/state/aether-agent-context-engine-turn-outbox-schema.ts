...
function contextEngineTurnOutboxSchemaSql(): string {
  const start = AETHER_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_START);
  const end = AETHER_AGENT_SCHEMA_SQL.indexOf(OUTBOX_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Aether context-engine turn outbox schema markers are missing.");
  }
