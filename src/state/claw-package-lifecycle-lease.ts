...
type ClawPackageLifecycleDatabase = Pick<AetherStateKyselyDatabase, "state_leases">;
...
type ClawPackageLifecycleLeaseOptions = AetherStateDatabaseOptions & {
  nowMs?: number;
  owner?: string;
  required?: boolean;
};
...
export function acquireClawPackageLifecycleLease(
  ...
  const databasePath = options.path ?? resolveAetherStateSqlitePath(env);
      ...
      ({ db }) => {
        ...
        if (existing) {
          throw new ClawPackageLifecycleBusyError(
            `Package ${artifact.ref} is being changed by another Aether lifecycle; retry after ${new Date(existing.expires_at ?? expiresAt).toISOString()}.`,
          );
        }
        executeSqliteQuerySync(
          db,
