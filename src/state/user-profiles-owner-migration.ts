...
export function repairMergedGatewayOwnerProfile(
  options: AetherStateDatabaseOptions & { shouldRepair: boolean },
): { repaired: boolean; changes: string[]; warnings: string[] } {
  const unchanged = { repaired: false, changes: [], warnings: [] };
  if (!withExistingAetherStateDatabaseReadOnly(({ db }) => ownerRepairRequired(db), options)) {
    return unchanged;
  }
  if (!options.shouldRepair) {
    return {
      ...unchanged,
      warnings: [
        "The shared gateway owner profile requires repair. Run aether vitals --fix, then reconnect.",
      ],
    };
  }
