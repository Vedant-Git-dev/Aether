...
let aetherStateCanonicalNamedIndexSet: ReadonlySet<string> | undefined;

function getAetherStateCanonicalNamedIndexSet(): ReadonlySet<string> {
  aetherStateCanonicalNamedIndexSet ??= new Set(
    getCanonicalSqliteNamedIndexContracts(AETHER_STATE_SCHEMA_SQL).map((index) => index.name),
  );
  return aetherStateCanonicalNamedIndexSet;
}

/** Project canonical SQL to the tables the shared runtime may create during this open. */
export function getAetherStateRuntimeSchema(options: {
  includeVersionLazyAdditiveTables: boolean;
}): string {
  let schema = AETHER_STATE_SCHEMA_SQL;
...
export const AETHER_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY: SqliteSchemaCompatibility = {
...
export function isAetherStateStartupRepairableSchemaIssue(issue: SqliteSchemaIssue): boolean {
  ...
  return (
    ...
    getAetherStateCanonicalNamedIndexSet().has(issue.objectName)
  );
}

/** Identify compatible schema differences repaired only by their feature owner. */
export function isAetherStateFirstUseSchemaIssue(issue: SqliteSchemaIssue): boolean {
  return (
