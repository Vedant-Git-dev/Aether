...
import { AETHER_AGENT_SCHEMA_WITHOUT_BOARD_SQL } from "./aether-agent-board-schema.js";
...
function splitProgressCardSchema(sql: string): {
  ...
  if (start === -1 || end === -1) {
    throw new Error("Aether agent progress-card schema markers are missing.");
  }
  return {
...
const progressCardSchema = splitProgressCardSchema(AETHER_AGENT_SCHEMA_WITHOUT_BOARD_SQL);
...
export function ensureAetherAgentProgressCardSchemaInTransaction(db: DatabaseSync): void {
  if (!db.isTransaction) {
