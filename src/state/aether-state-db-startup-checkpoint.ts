...
import { ensureColumn } from "./aether-state-db-schema-helpers.js";
...
export function withAetherStateStartupCheckpointConnection<T>(
...
export function initializeNativeAetherStateConnection(
  options: AetherStateDatabaseOptions,
  initializeCanonicalSchema: (db: DatabaseSync, pathname: string, env: NodeJS.ProcessEnv) => void,
): void {
  if (
  ...
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
  runWithAetherStateWriteAccess({ databasePath: pathname, env }, "native state bootstrap", () => {
    const db = openNodeSqliteDatabase(pathname);
    try {
      if (!isUninitializedNativeStartupDatabase(db)) {
        return;
