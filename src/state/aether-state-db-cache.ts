...
import { closeTrackedStateDatabase } from "./aether-state-db-handle.js";
import { assertSupportedStateSchemaVersion } from "./aether-state-db-schema-version.js";

const cachedDatabases = new Map<string, AetherStateDatabase>();
type StateDatabaseHandle = Pick<AetherStateDatabase, "db" | "path"> &
  Partial<Pick<AetherStateDatabase, "walMaintenance">>;
type AetherStateDatabaseCloseOptions = NonNullable<
  Parameters<AetherStateDatabase["walMaintenance"]["close"]>[0]
...
type AetherStateDatabaseLifecycleEvent =
  | { kind: "opened"; database: AetherStateDatabase }
  | { kind: "closed"; path: string }
  | { kind: "open-error"; path: string; error: unknown };
const databaseLifecycleListeners = new Set<(event: AetherStateDatabaseLifecycleEvent) => void>();

function notifyAetherStateDatabaseLifecycle(event: AetherStateDatabaseLifecycleEvent): void {
  for (const listener of databaseLifecycleListeners) {
    listener(event);
  }
}

function readSqliteDataVersion(database: AetherStateDatabase): number {
...
export function registerAetherStateDatabaseLifecycleListener(
  listener: (event: AetherStateDatabaseLifecycleEvent) => void,
): () => void {
...
function closeAetherStateDatabaseHandle(
  database: StateDatabaseHandle,
  options?: Parameters<AetherStateDatabase["walMaintenance"]["close"]>[0],
): unknown[] {
  ...
  if (database.db.isOpen) {
    ...
    unregisterRetainedExitClose ??= registerSqliteCacheExitClose(closeAetherStateDatabase);
  } else {
...
function evictCachedAetherStateDatabase(database: AetherStateDatabase): boolean {
  ...
  cachedDatabases.delete(database.path);
  notifyAetherStateDatabaseLifecycle({ kind: "closed", path: database.path });
  ...
  closeAetherStateDatabaseHandle(database, { checkpointMode: "PASSIVE" });
  return true;
}

/** Evict an exact cached shared-state owner after a proven corruption read. */
function evictAetherStateDatabaseAfterCorruption(
  database: AetherStateDatabase,
  error: unknown,
): boolean {
  return isSqliteCorruptionError(error) && evictCachedAetherStateDatabase(database);
}
...
const terminalOpenLatch = createSqliteTerminalOpenLatch({
  closeByPath: (pathname) => {
    ...
    if (cached) {
      evictCachedAetherStateDatabase(cached);
    }
  },
});

/** Publish a fully opened handle and bind query corruption to its exact cache owner. */
function publishAetherStateDatabase(database: AetherStateDatabase): AetherStateDatabase {
  const { db, path: pathname } = database;
  cachedDataVersions.set(db, readSqliteDataVersion(database));
  cachedDatabases.set(pathname, database);
  notifyAetherStateDatabaseLifecycle({ kind: "opened", database });
  registerNodeSqliteKyselyQueryErrorHandler(db, (error) => {
    ...
    if (!db.isTransaction && isSqliteCorruptionError(error)) {
      evictCachedAetherStateDatabase(database);
    }
  });
...
function getAetherStateDatabaseRuntimeFailure(pathname: string): Error | undefined {
  ...
  try {
    ...
    if (isSqliteSchemaVersionError(failure)) {
      ...
      notifyAetherStateDatabaseLifecycle({
        kind: "open-error",
        path: resolvedPath,
        error: failure,
      });
...
function getCachedAetherStateDatabase(pathname: string): AetherStateDatabase | undefined {
  const runtimeFailure = getAetherStateDatabaseRuntimeFailure(pathname);
  if (runtimeFailure) {
...
function getAetherStateDatabaseIfOpenAtPath(pathname: string): AetherStateDatabase | undefined {
  const cached = getCachedAetherStateDatabase(pathname);
  return cached?.db.isOpen ? cached : undefined;
}

/** Remove a closed cached owner while fresh-open access is held. */
function closeStaleCachedAetherStateDatabase(database: AetherStateDatabase): void {
  if (cachedDatabases.get(database.path) !== database) {
    return;
  }
  const errors = closeAetherStateDatabaseHandle(database);
  notifyAetherStateDatabaseLifecycle({ kind: "closed", path: database.path });
  throwStateDatabaseCleanupErrors(
    errors,
    `Stale Aether state database cleanup failed for ${database.path}.`,
  );
}

/** Latch background verification damage so later opens fail without rescanning. */
export function recordAetherStateDatabaseOpenFailure(
  pathname: string,
  error: Error,
...
export function clearAetherStateDatabaseOpenFailure(pathname: string): void {
  terminalOpenLatch.clear(pathname);
}

/** Reject shared-state access after a process-local terminal failure. */
function assertAetherStateDatabaseOpenAllowed(pathname: string): void {
...
function recordAetherStateDatabaseLifecycleOpenError(pathname: string, error: unknown): void {
  notifyAetherStateDatabaseLifecycle({ kind: "open-error", path: path.resolve(pathname), error });
}

/** Reject a fresh shared-state open after known corruption until repair clears it. */
function assertAetherStateDatabaseFreshOpenAllowedAtPath(
  pathname: string,
  env: NodeJS.ProcessEnv,
): void {
  assertAetherStateDatabaseOpenAllowed(pathname);
  let quarantineFailure: Error | undefined;
  try {
    const quarantine = readAetherDatabaseQuarantine(pathname, { env });
    if (quarantine) {
      quarantineFailure = createAetherDatabaseVerificationError(
        "state",
        pathname,
        quarantine.reason,
      );
...
function retireAetherStateDatabaseHandle(
  database: StateDatabaseHandle,
  options?: AetherStateDatabaseCloseOptions,
): void {
  const { busyTimeoutMs = AETHER_SQLITE_BUSY_TIMEOUT_MS, ...closeOptions } = options ?? {};
  ...
  runWithSqliteCoordinator(coordinator, "state database retirement", () => {
    ...
    const errors = closeAetherStateDatabaseHandle(database, closeOptions);
    if (wasCached) {
      try {
        notifyAetherStateDatabaseLifecycle({ kind: "closed", path: database.path });
      } catch (error) {
    ...
    throwStateDatabaseCleanupErrors(
      errors,
      `Aether state database cleanup failed for ${database.path}.`,
    );
  });
}
...
function retireAetherStateDatabaseHandles(
  pathname?: string,
  options?: AetherStateDatabaseCloseOptions,
): boolean {
  ...
  for (const database of databases) {
    ...
    found = true;
    try {
      retireAetherStateDatabaseHandle(database, options);
    } catch (error) {
      errors.push(error);
    }
  }
  throwStateDatabaseCleanupErrors(errors, "Aether state database cleanup failed.");
  return found;
}

/** Close one cached shared state database handle by exact pathname. */
export function closeAetherStateDatabaseByPath(
  pathname: string,
  options?: AetherStateDatabaseCloseOptions,
): boolean {
  return retireAetherStateDatabaseHandles(path.resolve(pathname), options);
}

/** Close all cached shared state database handles. */
export function closeAetherStateDatabase(options?: AetherStateDatabaseCloseOptions): void {
  retireAetherStateDatabaseHandles(undefined, options);
}

/** Test whether a cached shared state database handle is still open, optionally at one path. */
export function isAetherStateDatabaseOpen(pathname?: string): boolean {
...
export function closeAetherStateDatabaseForTest(): void {
...
export const aetherStateDatabaseCache = {
  assertAetherStateDatabaseFreshOpenAllowedAtPath,
  assertAetherStateDatabaseOpenAllowed,
  clearAetherStateDatabaseOpenFailure,
  closeAetherStateDatabase,
  closeAetherStateDatabaseByPath,
  closeAetherStateDatabaseForTest,
  closeAetherStateDatabaseHandle,
  closeStaleCachedAetherStateDatabase,
  evictCachedAetherStateDatabase,
  evictAetherStateDatabaseAfterCorruption,
  getCachedAetherStateDatabase,
  getAetherStateDatabaseRuntimeFailure,
  getAetherStateDatabaseIfOpenAtPath,
  isAetherStateDatabaseOpen,
  publishAetherStateDatabase,
  recordAetherStateDatabaseOpenFailure,
  recordAetherStateDatabaseLifecycleOpenError,
};

/** Drain local cached owners before excluding participating foreign handles for file removal. */
export function acquireAetherStateDatabaseFileExclusion(pathname: string) {
  ...
  try {
    closeAetherStateDatabaseByPath(databasePath);
        ...
        outcome = {
          value: await handles.runWithCanonicalMutation(
            ...
            async (assertInspection) => {
              assertInspection();
              const opened = getAetherStateDatabaseIfOpenAtPath(databasePath);
              if (opened) {
                return await prepareSqliteReadOnlyLocationFromOwnedDatabase(opened.db, () => {
                  assertInspection();
                  if (getAetherStateDatabaseIfOpenAtPath(databasePath) !== opened) {
      ...
      if (database) {
        try {
          ...
          handles.runWithCanonicalWrites(handles.assertCurrent, () => {
            errors.push(...closeAetherStateDatabaseHandle(database));
          });
          notifyAetherStateDatabaseLifecycle({ kind: "closed", path: databasePath });
        } catch (error) {
          errors.push(error);
        }
      }
...
export function confirmAetherStateDatabaseIntegrity(
  pathname: string,
): SqliteIntegrityConfirmation {
  const resolvedPath = path.resolve(pathname);
  closeAetherStateDatabaseByPath(resolvedPath);
