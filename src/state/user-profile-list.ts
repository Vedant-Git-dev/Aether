...
export function listProfiles(options: AetherStateDatabaseOptions = {}) {
...
export function readUserProfileAliases(
  ...
  const opened = aetherStateDatabaseCache.getAetherStateDatabaseIfOpenAtPath(
...
export function getUserProfileDisplay(
  ...
  const profile = withExistingAetherStateDatabaseReadOnly(({ db }) => {
    if (!tableExists(db, "user_profiles")) {
      return undefined;
    }
...
export function resolveUserProfileReference(
  reference: string,
  options: AetherStateDatabaseOptions & { allowedProfileIds?: ReadonlySet<string> } = {},
  ...
  return (
    withExistingAetherStateDatabaseReadOnly(({ db }): Result<string | undefined, "ambiguous"> => {
      if (!tableExists(db, "user_profiles")) {
        return ok(undefined);
      }
