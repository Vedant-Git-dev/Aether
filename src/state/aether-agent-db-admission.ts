...
import { assertAgentDatabaseMaintenanceAccess } from "./aether-agent-db-lease.js";
import {
...
export type AetherAgentDatabaseWriteAdmission = <T>(
...
function assertAgentDatabaseOpenAuthority(
  operation: SqliteIntegrityOperation<AetherAgentDatabase>,
  assertCurrent?: () => void,
): void {
  try {
    assertCurrent?.();
...
export function createAetherAgentDatabaseAdmissionOwner(
  openSteps: (
  ...
  function withAetherAgentDatabaseAsync<T>(
    inputOptions: AetherAgentDatabaseOptions,
    operation: (database: AetherAgentDatabase) => T | Promise<T>,
    ...
    if (existing?.controller.signal.aborted) {
      return existing.promise.then(
        () => withAetherAgentDatabaseAsync(options, operation, assertCurrent),
        () => withAetherAgentDatabaseAsync(options, operation, assertCurrent),
      );
    }
    const pending =
      existing ?? startAetherAgentDatabaseAdmission(options, agentId, pathname, assertCurrent);
  ...
  async function withAetherAgentDatabaseAdmission<T>(
    inputOptions: AetherAgentDatabaseOptions,
    withAdmission: AetherAgentDatabaseWriteAdmission,
    operation: (database: AetherAgentDatabase) => T | Promise<T>,
  ): Promise<T> {
    const options = { ...inputOptions, env: { ...(inputOptions.env ?? process.env) } };
    const agentId = normalizeAgentId(options.agentId);
    ...
    if (existing) {
      ...
      return withAetherAgentDatabaseAdmission(options, withAdmission, operation);
    }
    const admission = createAetherAgentDatabaseAdmission(agentId, pathname);
    ...
    try {
      while (true) {
        const outcome = await withAdmission(async (assertCurrent) => {
          try {
            assertCurrent();
            assertAetherAgentDatabaseAdmissionCurrent(options, pending, check?.database);
          } catch (error) {
  ...
  function createAetherAgentDatabaseAdmission(agentId: string, pathname: string) {
    const completion = createDeferredCore<AetherAgentDatabase>();
    ...
    return {
      pending,
      complete: (database: AetherAgentDatabase) => {
        retire();
        if (
  ...
  function assertAetherAgentDatabaseAdmissionCurrent(
    options: AetherAgentDatabaseOptions,
    pending: PendingAgentDatabaseOpen,
  ...
  function startAetherAgentDatabaseAdmission(
    options: AetherAgentDatabaseOptions,
    agentId: string,
    pathname: string,
    assertCurrent?: () => void,
    ...
    void (async () => {
      ...
      while (!step.done) {
        ...
        assertAgentDatabaseOpenAuthority(operation, () => {
          assertAetherAgentDatabaseAdmissionCurrent(options, pending, database);
          assertCurrent?.();
        });
  ...
  return { withAetherAgentDatabaseAsync, withAetherAgentDatabaseAdmission };
}
