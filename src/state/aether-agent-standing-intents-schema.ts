...
function standingIntentsSchemaSql(): string {
  const start = AETHER_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_START);
  const end = AETHER_AGENT_SCHEMA_SQL.indexOf(STANDING_INTENTS_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Aether standing-intents schema markers are missing.");
  }
  return AETHER_AGENT_SCHEMA_SQL.slice(start, end);
}
...
export function ensureAetherAgentStandingIntentsSchema(db: DatabaseSync): void {
  const ensure = () => {
