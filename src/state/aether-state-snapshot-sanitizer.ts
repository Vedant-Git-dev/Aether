// Removes transient runtime state from restorable Aether database snapshots.
...
type SnapshotSanitizerDatabase = Pick<AetherStateKyselyDatabase, "exec_approvals_config">;
...
export function sanitizeAetherStateLeaseRows(database: DatabaseSync): void {
...
export function sanitizeAetherGlobalStateSnapshot(database: DatabaseSync): void {
  ...
  sanitizeAetherStateLeaseRows(database);
