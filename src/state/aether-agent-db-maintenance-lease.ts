...
import {
  ...
  assertNoAetherAgentDatabaseLeases,
  runWithAgentDatabaseMaintenanceAuthority,
} from "./aether-agent-db-lease.js";
import { closeAetherAgentDatabasesAsync } from "./aether-agent-db-lifecycle.js";
...
import type { AetherStateMutationOperation } from "./aether-state-lease-context.js";
import { withAetherStateLease, type AetherStateLeaseContext } from "./aether-state-lease.js";

type MaintenanceScope = {
  databasePath: string;
  owner: AetherStateLeaseContext;
...
async function runMaintenanceScope<T>(
  ...
  const lease: AetherStateLeaseContext = {
    signal: owner.signal,
    ...
    ...(owner.withDatabaseFileMutation
      ? {
          withDatabaseFileMutation<Value, Captured>(
            operation: AetherStateMutationOperation<Value, Captured>,
          ) {
            assertAdmission();
            return track(
              owner.withDatabaseFileMutation!({
...
export function withAgentDatabaseMaintenanceLease<T>(
  options: Pick<AetherStateDatabaseOptions, "env"> & {
    schemaPolicy?: "existing";
    leaseMs?: number;
  },
  run: (maintenance: AetherStateLeaseContext) => Promise<T>,
): Promise<T> {
  const databasePath = path.resolve(resolveAetherStateSqlitePath(options.env));
  ...
  return withAetherStateLease(
    ...
    (maintenance) =>
      runMaintenanceScope(databasePath, maintenance, async (lease) => {
        ...
        assertNoAetherAgentDatabaseLeases(lease, options);
        return run(lease);
      }),
  );
}
