...
import { assertAetherAgentDatabaseForMaintenance } from "./aether-agent-db-maintenance.js";
import { isPersistentAetherAgentDatabasePath } from "./aether-agent-db-registry.js";
import {
  assertAetherAgentCurrentRuntimeSchema,
  assertCanonicalAgentPersistenceVersion,
  readExistingAgentSchemaMeta,
} from "./aether-agent-db-schema-helpers.js";
import type {
  DeferredStateSchemaPublication,
  IncompatibleAetherDatabase,
  AetherAgentSchemaPreflightResult,
  AetherDatabaseSchemaPreflight,
  AetherStateSchemaPreflightResult,
} from "./aether-database-preflight.types.js";
import type { AetherSchemaVersions } from "./aether-schema-versions.js";
import {
...
} from "./aether-state-db-dangling-workshop-index.js";
import {
  assertAetherStateDatabaseOwner,
  assertAetherStateDatabaseForMaintenance,
  aetherStateMigrationAssertions,
} from "./aether-state-db-maintenance.js";
import { assertCanonicalStateSchemaShape } from "./aether-state-db-schema-repair.js";
import {
...
import {
  ...
  isAetherStateFirstUseSchemaIssue,
  isAetherStateStartupRepairableSchemaIssue,
  AETHER_STATE_MAINTENANCE_SCHEMA_COMPATIBILITY,
  STATE_PERSISTENT_SCHEMA_COMPATIBILITY,
} from "./aether-state-schema-compatibility.js";
import { readStateSchemaPublicationBlocker } from "./aether-state-schema-publication.js";
import { AETHER_STATE_SCHEMA_SQL } from "./aether-state-schema.js";

export type {
  DeferredStateSchemaPublication,
  IncompatibleAetherDatabase,
  IndeterminateAetherDatabase,
  AetherDatabaseSchemaPreflight,
} from "./aether-database-preflight.types.js";

export { AETHER_DATABASE_SCHEMA_DOCS_URL } from "./aether-state-db.js";
...
type AgentRegistryDatabase = Pick<AetherStateKyselyDatabase, "agent_databases">;

type AetherDatabaseSchemaPreflightOperation = "doctor" | "gateway-restart" | "gateway-startup";

function formatDoctorIncompatibleDatabase(database: IncompatibleAetherDatabase): string {
...
export class AetherDatabaseSchemaPreflightError extends SqliteSchemaVersionError {
  constructor(
    readonly incompatibleDatabases: readonly IncompatibleAetherDatabase[],
    options: { operation?: AetherDatabaseSchemaPreflightOperation } = {},
  ) {
    ...
    const doctorGuidance =
      operation === "doctor"
        ? ` ${incompatibleDatabases.map(formatDoctorIncompatibleDatabase).join(" ")} Run Doctor with the Aether install that wrote this state (typically the active Gateway install), or another build that supports these schemas.`
        : "";
    super(
      `${prefix} because ${incompatibleDatabases.length} Aether database schema(s) are newer than this build. ` +
        `Refused by ${describeRunningAetherBuild()}.${doctorGuidance} See ${AETHER_DATABASE_SCHEMA_DOCS_URL}.`,
    );
    this.name = "AetherDatabaseSchemaPreflightError";
  }
}

/** Verify persisted runtime schemas before certifying repair or accepting restart. */
export async function assertAetherDatabasesReady(
  options: {
    ...
    | { operation: "gateway-restart" }
    | { operation: "gateway-startup"; config: AetherConfig }
  ),
): Promise<void> {
  const schemas = await preflightAetherDatabaseSchemas({
    env: options.env,
    supportedVersions: {
  ...
  if (schemas.incompatible.length > 0) {
    throw new AetherDatabaseSchemaPreflightError(schemas.incompatible, {
  ...
  const action =
    options.operation === "doctor"
      ...
      : options.operation === "gateway-startup"
        ...
        : "Gateway refused restart";
  throw new Error(
    `${action} because persisted database readiness could not be verified: ${shown.join("; ")}${omitted > 0 ? `; +${omitted} more` : ""}. ${options.operation === "doctor" ? "Stop Aether processes, then restore the affected database from a verified backup." : "Stop the Gateway and other Aether processes, run aether vitals --fix, then retry."}`,
  );
}
      ...
      ? [
          {
            ...
            path: resolveAetherRegisteredAgentDatabasePath(registryPath, row.path),
          },
        ]
      : [],
  );
...
function inspectCurrentStateStartupSchema(
  ...
  assertAetherStateDatabaseOwner(database, { pathname: databasePath });
  ...
  if (metadata?.schema_version !== foundVersion) {
    throw new Error(
      `Aether state database ${databasePath} metadata schema version ${typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid"} does not match ${foundVersion}.`,
    );
  }
  ...
  return {
    blockingIssues: issues.filter(
      (issue) =>
        !isAetherStateStartupRepairableSchemaIssue(issue) &&
        !isAetherStateFirstUseSchemaIssue(issue),
    ),
    startupRepairableIssues: issues.filter(isAetherStateStartupRepairableSchemaIssue),
  };
}

/** Compare one explicit SQLite file with this release's canonical shared-state schema. */
export async function preflightAetherStateDatabasePath(
  databasePath: string,
): Promise<AetherStateSchemaPreflightResult> {
  const resolvedPath = path.resolve(databasePath);
  const base = {
    schema: "aether.state-schema-preflight.v1",
    databasePath: resolvedPath,
    targetVersion: AETHER_STATE_SCHEMA_VERSION,
  } as const;
  ...
  let ownership: AetherExternalStateOwnership | null = null;
  const result = (
    status: AetherStateSchemaPreflightResult["status"],
  ...
  try {
    ...
    database.exec(
      `PRAGMA busy_timeout = ${AETHER_SQLITE_BUSY_TIMEOUT_MS}; PRAGMA query_only = ON; PRAGMA trusted_schema = OFF;`,
    );
    ...
    if (!Number.isSafeInteger(foundVersion) || foundVersion < 0) {
      throw new Error(
        `Aether state database ${resolvedPath} has invalid schema version metadata.`,
      );
    }
    contentVersion =
      foundVersion > AETHER_STATE_SCHEMA_VERSION
        ? foundVersion
        : readStateSchemaContentVersion(database);
    if (contentVersion > AETHER_STATE_SCHEMA_VERSION) {
      try {
        ownership = inspectAetherStateOwnershipFromDatabase(database, resolvedPath);
      } catch {
 ...
 */
export async function preflightAetherAgentDatabasePath(
  databasePath: string,
  agentId: string,
): Promise<AetherAgentSchemaPreflightResult> {
  const resolvedPath = path.resolve(databasePath);
  const base = {
    schema: "aether.agent-schema-preflight.v1" as const,
    databasePath: resolvedPath,
    agentId,
    targetVersion: AETHER_AGENT_SCHEMA_VERSION,
    requiresWrite: false,
    issues: [],
  };
  let database: DatabaseSync | undefined;
  ...
  try {
    ...
    foundVersion = readSqliteUserVersion(database);
    status = "incompatible";
    assertAetherAgentDatabaseForMaintenance(database, { agentId, pathname: resolvedPath });
    ...
    assertAetherAgentCurrentRuntimeSchema(database, { agentId, pathname: resolvedPath });
...
export async function preflightAetherDatabaseSchemas(options: {
  env: NodeJS.ProcessEnv;
  scope?: "state";
  signal?: AbortSignal;
  supportedVersions: AetherSchemaVersions;
...
}): Promise<AetherDatabaseSchemaPreflight> {
  options.signal?.throwIfAborted();
  const result: AetherDatabaseSchemaPreflight = { incompatible: [], indeterminate: [] };
  const statePath = path.resolve(resolveAetherStateSqlitePath(options.env));
  ...
  try {
    if (statePresence.status === "present") {
      ...
      stateDatabase.exec(`PRAGMA busy_timeout = ${AETHER_SQLITE_BUSY_TIMEOUT_MS};`);
      ...
      if (
        ...
        if (migrationVersion === AETHER_STATE_SCHEMA_VERSION) {
          ...
          if (blockingIssues.length > 0) {
            throw new Error(
              `Aether state database ${statePath} requires repair: ${blockingIssues.map((issue) => issue.message).join("; ")}; run aether vitals --fix.`,
            );
          }
        } else {
          aetherStateMigrationAssertions.get(migrationVersion)?.(stateDatabase, {
            pathname: statePath,
          });
        }
      } else if (
        options.verifyCurrentSchemaShape === true &&
        migrationVersion === AETHER_STATE_SCHEMA_VERSION
      ) {
        try {
          assertAetherStateDatabaseForMaintenance(stateDatabase, { pathname: statePath });
        } catch (error) {
  ...
  const inspectionTargets: Array<{ agentId?: string; path: string }> = [
    ...
    ...(options.configuredAgentDatabaseTargets !== undefined
      ? registeredDatabases.filter((database) =>
          isPersistentAetherAgentDatabasePath(database.path, options.env),
        )
      : []),
  ...
  for (const row of inspectionTargets) {
    ...
    try {
      ...
      agentDatabase.exec(`PRAGMA busy_timeout = ${AETHER_SQLITE_BUSY_TIMEOUT_MS};`);
      ...
      if (agentVersion <= options.supportedVersions.agent) {
        ...
        if (
          ...
          assertAetherAgentDatabaseForMaintenance(agentDatabase, {
            agentId,
            pathname: agentPath,
          });
        }
