...
function messageToolRunOutcomeSchemaSql(): string {
  const start = AETHER_AGENT_SCHEMA_SQL.indexOf(SCHEMA_START);
  const end = AETHER_AGENT_SCHEMA_SQL.indexOf(SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Aether message-tool run outcome schema markers are missing.");
  }
