...
import { MAX_TIMER_TIMEOUT_MS } from "@aether/normalization-core/number-coercion";
import { computeBackoff, sleepWithAbort } from "../infra/backoff.js";
import { createSqliteLifecycleAggregateError } from "../infra/sqlite-coordinator.js";
import { isSqliteLockError } from "../infra/sqlite-error-diagnostics.js";
import { StateDatabaseCoordinatorContentionError } from "../infra/state-database-coordinator.js";
import { loggingState } from "../logging/state.js";
import { withExistingAetherStateDatabaseArtifactPreservingReadOnly } from "./aether-state-db-readonly.js";
import type { AetherStateLeaseContext } from "./aether-state-lease-context.js";
import { createAetherStateLeaseExclusion } from "./aether-state-lease-exclusion.js";
import { startAetherStateLeaseHeartbeat } from "./aether-state-lease-heartbeat.js";
import {
  readLeaseDatabase,
  resolveLeaseDatabasePath,
  withLeaseWriteTransaction,
  type AetherStateLeaseDatabase,
} from "./aether-state-lease-storage.js";
import {
  acquireAetherStateLeaseInTransaction,
  readAetherStateLeaseExpiry,
  releaseAetherStateLeaseInTransaction,
  renewAetherStateLeaseInTransaction,
} from "./aether-state-lease-store.js";

type AetherStateLeaseOptions = {
  scope: string;
  key: string;
...
export type { AetherStateLeaseContext } from "./aether-state-lease-context.js";

type AetherStateLeaseErrorCode =
  | "AETHER_STATE_LEASE_INVALID_INPUT"
  | "AETHER_STATE_LEASE_TIMEOUT"
  | "AETHER_STATE_LEASE_ABORTED"
  | "AETHER_STATE_LEASE_LOST"
  | "AETHER_STATE_LEASE_STORAGE_FAILED";

export class AetherStateLeaseError extends Error {
  readonly code: AetherStateLeaseErrorCode;

  constructor(message: string, options: { code: AetherStateLeaseErrorCode; cause?: unknown }) {
    super(message, { cause: options.cause });
    this.name = "AetherStateLeaseError";
...
function leaseError(
  code: AetherStateLeaseErrorCode,
  message: string,
  cause?: unknown,
): AetherStateLeaseError {
  return new AetherStateLeaseError(message, {
    code,
    ...(cause === undefined ? {} : { cause }),
  });
}

function invalidInput(message: string): AetherStateLeaseError {
  return leaseError("AETHER_STATE_LEASE_INVALID_INPUT", message);
}
...
function validateOptions(options: AetherStateLeaseOptions) {
...
function tryAcquire(
  ...
  return withLeaseWriteTransaction(params.database, params.operationLabel, (db) =>
    acquireAetherStateLeaseInTransaction(db, params, params.leaseMs),
  );
}
...
function renew(
  ...
  return withLeaseWriteTransaction(params.database, params.operationLabel, (db) => {
    const expiresAt = renewAetherStateLeaseInTransaction(db, params, params.leaseMs);
...
function assertLeaseOwnedInDatabase(database: DatabaseSync, params: LeaseIdentity): number {
  const expiresAt = readAetherStateLeaseExpiry(database, params);
  if (expiresAt === undefined) {
    throw leaseError(
      "AETHER_STATE_LEASE_LOST",
      `${params.leaseLabel} ${params.scope}/${params.key} was lost`,
...
function verifyLeaseOwnership(
  params: LeaseIdentity & { database?: AetherStateLeaseDatabase; transaction?: DatabaseSync },
): number {
  try {
    ...
    if (error instanceof AetherStateLeaseError) {
      throw error;
    }
    throw leaseError(
      "AETHER_STATE_LEASE_STORAGE_FAILED",
...
function release(
  ...
  withLeaseWriteTransaction(params.database, params.operationLabel, (db) =>
    releaseAetherStateLeaseInTransaction(db, params),
  );
}
...
export async function withAetherStateLease<T>(
  options: AetherStateLeaseOptions,
  ...
  let closed = false;
  let workerHeartbeat: ReturnType<typeof startAetherStateLeaseHeartbeat> | undefined;
  let startingHeartbeat: ReturnType<typeof startAetherStateLeaseHeartbeat> | undefined;
  ...
  const abortLost = (cause?: unknown) => {
    if (!leaseLost.signal.aborted) {
      leaseLost.abort(
        cause instanceof AetherStateLeaseError
          ? cause
          : leaseError(
              "AETHER_STATE_LEASE_LOST",
  ...
  const renewFromTimer = () => {
    try {
      ...
      if (error instanceof AetherStateLeaseError && error.code === "AETHER_STATE_LEASE_LOST") {
        abortLost(error);
  ...
  const startWorker = async (expiresAt: number) => {
    const started = startAetherStateLeaseHeartbeat({
  ...
  const fileExclusion = createAetherStateLeaseExclusion({
    ...
    readMutationExpiry: (databasePath) => {
      const expiresAt = withExistingAetherStateDatabaseArtifactPreservingReadOnly(
  ...
  try {
    ...
    try {
      ...
      if (authorityError instanceof Error) {
        if (failure !== error && authorityError instanceof AetherStateLeaseError) {
