...
} from "./aether-state-db-audit-migration.js";
import {
  AETHER_STATE_STRICT_SCHEMA_VERSION,
  type AetherStateDatabaseOptions,
  type AetherStateDatabaseSchemaMigration,
} from "./aether-state-db-contract.js";
import { resolveDatabasePath } from "./aether-state-db-maintenance.js";
...
} from "./aether-state-db-schema-helpers.js";
import { AetherStateDatabaseSchemaMigrationRequiredError } from "./aether-state-db-schema-migration-required.js";
import { FOLDED_SINGLETON_STATE_TABLES_V12 } from "./aether-state-db-schema-v12-foldin.js";
import { readStateSchemaMigrationVersion } from "./aether-state-db-schema-version.js";
...
} from "./aether-state-db-table-retirements.js";
import {
  resolveAetherAgentDatabaseStoredPath,
  resolveAetherStateDirForDatabasePath,
...
export function migrateWorkerPlacementExecutionModeSchema(
  ...
  const endMarker = "\n) STRICT;";
  const end = start >= 0 ? AETHER_STATE_SCHEMA_SQL.indexOf(endMarker, start) : -1;
  if (start < 0 || end < 0) {
    throw new Error("Canonical worker placement schema block is missing");
  }
  const placementSchema = AETHER_STATE_SCHEMA_SQL.slice(start, end + endMarker.length);
  ...
  if (
    ...
    throw new Error("Aether v7 worker placement columns are not canonical");
  }
  ...
  if (unexpectedObjects.length > 0) {
    throw new Error("Aether v7 worker placement schema has unsupported attached objects");
  }
  const migrationTable = "worker_session_placements_migration_v8";
  if (tableExists(db, migrationTable)) {
    throw new Error(`Aether worker placement migration table already exists: ${migrationTable}`);
  }
...
function isDefaultAgentDatabasePath(pathname: string, agentId: string): boolean {
  ...
  return (
    path.basename(pathname) === "aether-agent.sqlite" &&
...
export function migrateAgentDatabaseRelativePaths(
  ...
  for (const row of rows) {
    ...
    if (typeof agentId !== "string" || typeof registeredPath !== "string") {
      throw new Error("Aether v8 agent database registry paths are not canonical");
    }
    if (!path.isAbsolute(registeredPath)) {
      continue;
    }
    const storedPath = resolveAetherAgentDatabaseStoredPath(databasePath, registeredPath);
    if (!path.isAbsolute(storedPath)) {
      updatePath.run(storedPath, agentId, registeredPath);
      relativized += 1;
    }
  }
  const stateDir = resolveAetherStateDirForDatabasePath(databasePath);
  for (const row of rows) {
    const agentId = row.agent_id;
    const registeredPath = row.path;
    if (
      ...
      !path.isAbsolute(resolveAetherAgentDatabaseStoredPath(databasePath, registeredPath))
    ) {
      continue;
    }
    ...
    if (isDefaultAgentDatabasePath(absolutePath, agentId)) {
      ...
      const counterpartStored = resolveAetherAgentDatabaseStoredPath(
        databasePath,
        counterpartAbsolute,
      );
...
export function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  if (!hasCanonicalAgentDatabasesPrimaryKey(db)) {
    ...
    throw new Error(
      `Aether state database ${pathname} has a noncanonical agent database registry schema that cannot be repaired automatically; restore the canonical agent_databases shape before retrying.`,
    );
  }
  if (!hasCanonicalAuditEventsSchema(db)) {
    if (canRepairLegacyAuditEventsSchema(db)) {
      throw new AetherStateDatabaseSchemaMigrationRequiredError("audit-events-v2", pathname);
    }
    throw new Error(
      `Aether state database ${pathname} has a noncanonical audit event schema that cannot be repaired automatically; restore the canonical audit_events shape before retrying.`,
    );
  }
}
export function detectAetherStateDatabaseSchemaMigrations(
  ...
  if (behavior.artifactPreservingReadOnly) {
    return (
      withExistingAetherStateDatabaseArtifactPreservingReadOnly(
        ({ db }) => detectAetherStateDatabaseSchemaMigrationsFromDatabase(db, pathname),
  ...
  try {
    return detectAetherStateDatabaseSchemaMigrationsFromDatabase(db, pathname);
  } finally {
    db.close();
  }
}
 ...
 */
export function detectAetherStateDatabaseSchemaMigrationsFromDatabase(
  db: DatabaseSync,
  pathname: string,
): AetherStateDatabaseSchemaMigration[] {
  const migrations: AetherStateDatabaseSchemaMigration[] = [];
  ...
  if (tableExists(db, "audit_events") && userVersion < AETHER_STATE_STRICT_SCHEMA_VERSION) {
