...
import { ensureColumn, tableExists, tableHasColumn } from "./aether-state-db-schema-helpers.js";
import { migrateJsonCanonicalWideRowsV13 } from "./aether-state-db-schema-v13-widerow.js";
import {
...
function admitStateDatabaseForSchemaRepair(
  ...
  const admit = () => {
    ...
    if (danglingWorkshopIndex) {
      assertAetherStateWriteAllowed({ database, databasePath: pathname, env });
    }
  };
  if (danglingWorkshopIndex) {
...
type AetherStateMigrationVersion = keyof typeof STATE_MIGRATION_ALLOWED_MISSING_TABLES;

/** Require canonical shared-state ownership without requiring the latest schema. */
export function assertAetherStateDatabaseOwner(
  database: DatabaseSync,
  ...
  if (metadata?.role !== "global") {
    ...
    throw new Error(
      `Aether state database ${options.pathname} has schema role ${role}; expected global.`,
    );
  }
}

/** Require the canonical shared-state owner and schema before offline file maintenance. */
export function assertAetherStateDatabaseForMaintenance(
  database: DatabaseSync,
  options: { pathname: string },
  readTable?: SqliteTableContractReader,
): void {
  const userVersion = assertSupportedStateSchemaVersion(database, options.pathname);
  if (readStateSchemaContentVersion(database) !== AETHER_STATE_SCHEMA_VERSION) {
    throw new Error(
      `Aether state database ${options.pathname} uses schema version ${userVersion}; run aether vitals --fix before compacting it.`,
    );
  }

  assertAetherStateDatabaseOwner(database, options);
  ...
  if (metadata?.schema_version !== userVersion) {
    ...
    throw new Error(
      `Aether state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${userVersion}; run aether vitals --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
...
function assertAetherStateDatabaseVersionForMigration(
  database: DatabaseSync,
  options: { pathname: string; version: AetherStateMigrationVersion },
): void {
  const userVersion = readSqliteUserVersion(database);
  if (readStateSchemaMigrationVersion(database) !== options.version) {
    throw new Error(
      `Aether state database ${options.pathname} uses schema version ${userVersion}; expected ${options.version} before migrating it.`,
    );
  }
  assertAetherStateDatabaseOwner(database, options);
  const metadata = database
  ...
  if (metadata?.schema_version !== userVersion) {
    const schemaVersion =
      typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `Aether state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${userVersion}; repair the ownership metadata before migrating it.`,
    );
  }
  assertSqliteSchemaTablesPresent(database, options.pathname, AETHER_STATE_SCHEMA_SQL, {
...
function assertAetherStateDatabaseV5ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 5 });
}

/** Require every stable v6 table before the v7 retirement migration can run. */
function assertAetherStateDatabaseV6ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 6 });
}

/** Require every stable v7 table before the v8 placement migration can run. */
function assertAetherStateDatabaseV7ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 7 });
}

/** Require every stable v8 table before the v9 registry migration can run. */
function assertAetherStateDatabaseV8ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 8 });
}

/** Require every stable v9 table before the v10 retirement migration can run. */
function assertAetherStateDatabaseV9ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 9 });
}

/** Require every stable v10 table before the v11 curator retirement can run. */
function assertAetherStateDatabaseV10ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 10 });
}

/** Require every stable v11 table before singleton state folds into the v12 store. */
function assertAetherStateDatabaseV11ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 11 });
}

/** Require every stable v12 table before wide rows become JSON-canonical. */
function assertAetherStateDatabaseV12ForMigration(
  database: DatabaseSync,
  options: { pathname: string },
): void {
  assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 12 });
}

/** Keep historical migration gates beside their version-specific ownership assertions. */
export const aetherStateMigrationAssertions = new Map([
  [5, assertAetherStateDatabaseV5ForMigration],
  [6, assertAetherStateDatabaseV6ForMigration],
  [7, assertAetherStateDatabaseV7ForMigration],
  [8, assertAetherStateDatabaseV8ForMigration],
  [9, assertAetherStateDatabaseV9ForMigration],
  [10, assertAetherStateDatabaseV10ForMigration],
  [11, assertAetherStateDatabaseV11ForMigration],
  [12, assertAetherStateDatabaseV12ForMigration],
  [
    13,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 13 }),
  ],
  [
    14,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 14 }),
  ],
  [
    15,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 15 }),
  ],
  [
    16,
    (database: DatabaseSync, options: { pathname: string }) =>
      assertAetherStateDatabaseVersionForMigration(database, { ...options, version: 16 }),
  ],
]);
...
export function resolveDatabasePath(options: AetherStateDatabaseOptions = {}): string {
...
function migratePreparedWorkerOwnership(db: DatabaseSync, previousVersion: number): boolean {
  ...
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(marker);
  const end = AETHER_STATE_SCHEMA_SQL.indexOf("\n) STRICT;", start);
  if (start < 0 || end < start) {
    throw new Error("Aether worker environment schema marker is missing.");
  }
  const columns = splitSqlList(AETHER_STATE_SCHEMA_SQL.slice(start + marker.length, end))
...
export function runStateSchemaMigrationTransaction<T>(
  ...
  return runSqliteImmediateTransactionSync(
    ...
    () => {
      ...
      const blocker =
        publishedVersion < AETHER_STATE_SCHEMA_VERSION
      ...
      try {
        ...
        if (cause instanceof AetherStateOwnershipError) {
          throw cause;
        }
          ...
          [
            {
              ...
              supportedVersion: AETHER_STATE_SCHEMA_VERSION,
            },
          ],
...
export function executeCanonicalStateSchema(
  ...
  database.exec(getAetherStateRuntimeSchema(options));
}
