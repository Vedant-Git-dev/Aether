...
const coordinatedStateTransactions = resolveGlobalSingleton(
  Symbol.for("aether.coordinatedStateTransactions"),
...
export function withSharedStateWriteCoordinator<T>(
  ...
  if (params.existing?.isTransaction && !coordinatedStateTransactions.has(params.existing)) {
    throw new Error(
      "Cannot join an uncoordinated shared-state transaction; enter through runAetherStateWriteTransaction before BEGIN.",
    );
  }
  ...
  const coordinator = acquireStateDatabaseCoordinator({
    ...
    busyTimeoutMs:
      ...
      (params.existing ? readSqliteBusyTimeout(params.existing) : AETHER_SQLITE_BUSY_TIMEOUT_MS),
  });
