...
import {
  ...
  ensureAetherAgentBoardSchemaInTransaction,
} from "./aether-agent-board-schema.js";
import { CONTEXT_ENGINE_TURN_OUTBOX_TABLE } from "./aether-agent-context-engine-turn-outbox-schema.js";
import { FIRST_USE_ADDITIVE_AGENT_COLUMN_DEFINITIONS } from "./aether-agent-db-additive-columns.js";
import {
  AGENT_MEDIA_SCHEMA_VERSION,
  AETHER_AGENT_SCHEMA_VERSION,
} from "./aether-agent-db-contract.js";
import { AetherAgentDatabaseMediaMigrationRequiredError } from "./aether-agent-db-migration-required.js";
import {
  ensureSessionAdditiveColumns,
  ensureSessionEntryValidityProjection,
} from "./aether-agent-db-session-migrations.js";
import { SESSION_GOAL_OPERATIONS_TABLE } from "./aether-agent-goal-operations-schema.js";
import { MESSAGE_TOOL_RUN_OUTCOMES_TABLE } from "./aether-agent-message-tool-outcome-schema.js";
import { LEGACY_PARTICIPANT_OPTIONAL_COLUMNS } from "./aether-agent-participants-migration.js";
import { SESSION_PENDING_INPUTS_TABLE } from "./aether-agent-pending-inputs-schema.js";
import {
  ensureAetherAgentProgressCardSchemaInTransaction,
  AGENT_PROGRESS_CARD_SCHEMA_SQL,
  SESSION_PROGRESS_CARDS_TABLE,
} from "./aether-agent-progress-card-schema.js";
import { AETHER_AGENT_SCHEMA_SQL } from "./aether-agent-schema.js";
import { SESSION_PARTICIPANTS_TABLE } from "./aether-agent-session-participants-schema.js";
import {
  AGENT_V14_ADDITIVE_SCHEMA_SQL,
  AGENT_V14_CORE_SCHEMA_SQL,
  AGENT_V14_SESSION_SHARING_SCHEMA_SQL,
} from "./aether-agent-session-sharing-schema.js";
import { SESSION_TRANSCRIPT_ARCHIVES_TABLE } from "./aether-agent-session-transcript-archive-schema.js";
import {
  STANDING_INTENTS_FTS_SHADOW_TABLES,
  STANDING_INTENTS_FTS_TABLE,
  STANDING_INTENTS_TABLE,
} from "./aether-agent-standing-intents-schema.js";
...
export function assertAetherAgentSchemaContains(
...
export function assertAetherAgentCurrentRuntimeSchema(
  database: DatabaseSync,
  ...
  if (metadata.schemaVersion !== AETHER_AGENT_SCHEMA_VERSION) {
    throw new Error(
      `Aether agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match ${AETHER_AGENT_SCHEMA_VERSION}; run aether vitals --fix before using it.`,
    );
  }
  if (hasRetiredAgentStateLeaseSchema(database)) {
    throw new Error(
      `Aether agent database ${options.pathname} retains retired state_leases storage; run aether vitals --fix before using it.`,
    );
  }
...
function repairAndAssertAgentSchemaGroup(
  ...
  assertAetherAgentSchemaContains(database, pathname, schemaSql, "legacy");
}
...
export function ensureSessionKeyContractSchemaInTransaction(db: DatabaseSync): void {
  const start = AETHER_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_START);
  const end = AETHER_AGENT_SCHEMA_SQL.indexOf(SESSION_KEY_CONTRACT_SCHEMA_END, start);
  if (start === -1 || end === -1) {
    throw new Error("Aether agent session-key contract schema markers are missing.");
  }
  db.exec(AETHER_AGENT_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- Idempotent additive lazy ensure.
}

export function repairAndAssertAetherAgentV14SchemaForMigration(
  database: DatabaseSync,
  options: { agentId: string; pathname: string },
): void {
  ...
  if (userVersion !== 14) {
    throw new Error(
      `Aether agent database ${options.pathname} uses schema version ${userVersion}; expected 14 before migrating it.`,
    );
  }
  const agentId = normalizeAgentId(options.agentId);
  const metadata = readExistingAgentSchemaMeta(database);
  ...
  if (metadata.schemaVersion !== 14) {
    throw new Error(
      `Aether agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 14; repair the ownership metadata before migrating it.`,
    );
  }
  ...
  if (hasAnyCanonicalTable(database, AGENT_V14_BOARD_SCHEMA_SQL)) {
    ...
    ensureAetherAgentBoardSchemaInTransaction(database);
  ...
  if (hasAnyCanonicalTable(database, AGENT_PROGRESS_CARD_SCHEMA_SQL)) {
    ...
    ensureAetherAgentProgressCardSchemaInTransaction(database);
...
export function assertCanonicalAgentPersistenceVersion(
  ...
  if (userVersion < AGENT_MEDIA_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new AetherAgentDatabaseMediaMigrationRequiredError(pathname, userVersion);
  }
  if (userVersion < AETHER_AGENT_SCHEMA_VERSION && !isNewUnownedDatabase) {
    throw new Error(
      `Aether agent database ${pathname} uses schema version ${userVersion}; stop active agents and run aether vitals --fix to migrate session identities before using it.`,
    );
  }
}
...
export function assertExistingAgentSchemaOwner(
  ...
  if (existing.role !== "agent") {
    throw new Error(
      `Aether agent database ${pathname} has schema role ${existing.role ?? "unknown"}; expected agent.`,
    );
  }
  if (!existing.agentId) {
    throw new Error(`Aether agent database ${pathname} has no agent owner.`);
  }
  if (normalizeAgentId(existing.agentId) !== agentId) {
    throw new Error(
      `Aether agent database ${pathname} belongs to agent ${existing.agentId}; requested agent ${agentId}.`,
    );
  }
}
...
export function assertAgentSchemaVersion(
  ...
  if (userVersion !== options.version || metadata?.schemaVersion !== options.version) {
    throw new Error(
      `Aether agent database ${options.pathname} did not converge on schema version ${options.version}.`,
    );
  }
