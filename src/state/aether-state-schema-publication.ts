...
import { CONTENT_VERSION_KEY } from "./aether-state-db-schema-version.js";
...
export function resolveStateSchemaVersionToPublish(db: DatabaseSync): number {
  ...
  if (published >= AETHER_STATE_SCHEMA_VERSION || !readStateSchemaPublicationBlocker(db)) {
    return AETHER_STATE_SCHEMA_VERSION;
  }
  ...
  executeSqliteQuerySync(
    ...
    getNodeSqliteKysely<PublicationDatabase>(db)
      ...
      .values({
        ...
        value_json: String(AETHER_STATE_SCHEMA_VERSION),
      ...
      .onConflict((conflict) =>
        conflict
          ...
          .where("config_machine_state.value_json", "!=", String(AETHER_STATE_SCHEMA_VERSION)),
      ),
  );
  return published;
}
