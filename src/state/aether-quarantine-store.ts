...
import { AETHER_DATABASE_SCHEMA_DOCS_URL } from "./aether-state-db-contract.js";
import { resolveAetherStateSqliteDir } from "./aether-state-db.paths.js";

const AETHER_QUARANTINE_SCHEMA_VERSION = 2;
const AETHER_QUARANTINE_BUSY_TIMEOUT_MS = 5_000;
const AETHER_QUARANTINE_DIR_MODE = 0o700;
const AETHER_QUARANTINE_FILE_MODE = 0o600;

type AetherDatabaseKind = "agent" | "state";

type AetherDatabaseQuarantine = {
  kind: AetherDatabaseKind;
  quarantinedAt: number;
  reason: string;
};

// Read admission needs this error without importing schema migrations.
export function createAetherDatabaseVerificationError(
  kind: "agent" | "state",
  pathname: string,
  ...
  const error = new Error(
    `Aether ${kind} database ${pathname} is quarantined after integrity verification failed: ${storedError ?? "unknown integrity error"}. Restore the database from a backup or repair it, then run aether vitals --fix to clear the quarantine. See ${AETHER_DATABASE_SCHEMA_DOCS_URL}.`,
  );
...
function resolveQuarantineStorePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveAetherStateSqliteDir(env), "aether-quarantine.sqlite");
}

function ensureQuarantineStoreDirectory(storePath: string): void {
  const dir = path.dirname(storePath);
  mkdirSync(dir, { recursive: true, mode: AETHER_QUARANTINE_DIR_MODE });
  applyPrivateModeSync(dir, AETHER_QUARANTINE_DIR_MODE);
}

function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  database.exec(`
    PRAGMA busy_timeout = ${AETHER_QUARANTINE_BUSY_TIMEOUT_MS};
    PRAGMA journal_mode = DELETE;
    PRAGMA synchronous = FULL;
  `);
  const userVersion = readQuarantineSchemaVersion(database, storePath);
  if (userVersion > AETHER_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Aether quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === AETHER_QUARANTINE_SCHEMA_VERSION) {
    return;
  ...
  if (userVersion === 1) {
    database.exec(`
      ...
      PRAGMA user_version = ${AETHER_QUARANTINE_SCHEMA_VERSION};
      COMMIT;
    `);
    return;
  }
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Aether quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
function withQuarantineWriter<T>(env: NodeJS.ProcessEnv, operation: (db: DatabaseSync) => T): T {
  ...
  try {
    if (!existed) {
      applyPrivateModeSync(storePath, AETHER_QUARANTINE_FILE_MODE);
    }
...
export function readAetherDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): AetherDatabaseQuarantine | undefined {
  ...
  const database = openNodeSqliteDatabase(storePath);
  try {
    database.exec(`PRAGMA busy_timeout = ${AETHER_QUARANTINE_BUSY_TIMEOUT_MS};`);
    const userVersion = readQuarantineSchemaVersion(database, storePath);
    if (userVersion === 0) {
      return undefined;
    }
    ...
    if (
      ...
      throw new Error(`Aether quarantine store ${storePath} contains an invalid row.`);
    }
...
export function recordAetherDatabaseQuarantine(options: {
...
export function clearAetherDatabaseQuarantine(
  pathname: string,
  options: { env?: NodeJS.ProcessEnv } = {},
): boolean {
