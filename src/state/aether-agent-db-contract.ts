...
export const AETHER_AGENT_SCHEMA_VERSION = 19;
export const AGENT_MEDIA_SCHEMA_VERSION = 17;

/** Open per-agent SQLite database handle plus lifecycle maintenance. */
export type AetherAgentDatabase = {
  agentId: string;
  db: DatabaseSync;
  path: string;
...
export type AetherAgentDatabaseOptions = AetherStateDatabaseOptions & {
  agentId: string;
};

/** Shared-state registry row describing an agent database seen by this process. */
export type AetherRegisteredAgentDatabase = {
  agentId: string;
  path: string;
...
export type AetherAgentDatabaseOwnerInspection =
