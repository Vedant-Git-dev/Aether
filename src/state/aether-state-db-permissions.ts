...
import {
  resolveAetherStateSqliteDir,
  resolveAetherStateSqlitePath,
} from "./aether-state-db.paths.js";

const AETHER_STATE_DIR_MODE = 0o700;
const AETHER_STATE_FILE_MODE = 0o600;
...
export function ensureAetherStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveAetherStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveAetherStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`Aether state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: AETHER_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, AETHER_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      try {
        bestEffortChmodSync(candidate, AETHER_STATE_FILE_MODE);
      } catch (error) {
