...
export type AetherDatabaseVerifyTarget = {
  path: string;
  kind: "agent" | "state";
  label: string;
};

export type AetherDatabaseVerifyResult = {
  path: string;
  ok: boolean;
  error?: string;
  terminal?: boolean;
...
function isVerifyTarget(value: unknown): value is AetherDatabaseVerifyTarget {
...
async function verifyAetherDatabase(
  target: AetherDatabaseVerifyTarget,
  ...
  let result = await (async (): Promise<AetherDatabaseVerifyResult> => {
    try {
...
export async function verifyAetherDatabases(
  targets: readonly AetherDatabaseVerifyTarget[],
): Promise<AetherDatabaseVerifyResult[]> {
  const results: AetherDatabaseVerifyResult[] = [];
  for (const target of targets) {
    results.push(await verifyAetherDatabase(target));
  }
  return results;
}
  ...
  process.once("message", (message: unknown) => {
    void (async () => {
      try {
        ...
        const results = await verifyAetherDatabases(targets);
