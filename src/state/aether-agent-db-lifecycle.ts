...
import type {
  AetherAgentDatabase,
  AetherAgentDatabaseOwnerInspection,
} from "./aether-agent-db-contract.js";
import { releaseAetherAgentDatabaseLease } from "./aether-agent-db-lease.js";
import {
...
export const AETHER_AGENT_DB_OPEN_HANDLE_CAP = 64;
const agentDbLog = createSubsystemLogger("state/agent-db");
const AETHER_AGENT_DB_SLOW_OPEN_MS = 1_000;
...
type AgentDatabaseLifecycle = {
  databases: Map<string, AetherAgentDatabase>;
  borrowers: WeakMap<DatabaseSync, Set<object>>;
  incognito: WeakSet<AetherAgentDatabase>;
  generation: number;
...
export type PendingAgentDatabaseOpen = {
  ...
  promise: Promise<AetherAgentDatabase>;
  assertHeld?: () => void;
  operations: number;
  releaseBorrow?: () => void;
};
...
const cache = resolveGlobalSingleton<AgentDatabaseLifecycle>(
  Symbol.for("aether.agentDatabaseLifecycle"),
  () => ({
    databases: new Map(),
    ...
    failures: new Map(),
    leases: new Map(),
    terminal: createSqliteTerminalOpenLatch({ closeByPath: closeAetherAgentDatabaseByPath }),
...
export function startAgentDatabaseOpenTiming(
  ...
  return (phase: keyof typeof phaseDurationsMs): void => {
    ...
    if (phase === "registration" && elapsedMs >= AETHER_AGENT_DB_SLOW_OPEN_MS) {
      agentDbLog.warn("slow Aether agent database open", {
        agentId,
        elapsedMs,
        path: pathname,
        pid: process.pid,
        ...
        phaseDurationsMs,
        ...diagnostics,
        thresholdMs: AETHER_AGENT_DB_SLOW_OPEN_MS,
      });
    }
  };
}
...
export function closeCachedAetherAgentDatabase(
  ...
  if (lease) {
    releaseAetherAgentDatabaseLease(lease.leaseId, { env: lease.env });
...
export function evictLruAgentDatabaseHandles(): void {
  ...
  while (cache.databases.size >= AETHER_AGENT_DB_OPEN_HANDLE_CAP) {
    let evicted = false;
    for (const [pathname, database] of cache.databases) {
      ...
      closeCachedAetherAgentDatabase(database, { eviction: true });
      cache.databases.delete(pathname);
      cache.failures.delete(pathname);
      if (cache.incognito.has(database)) {
        cache.generation += 1;
      }
      agentDbLog.debug("evicted Aether agent database handle", {
    ...
    if (!evicted) {
      ...
      agentDbLog.warn("agent database handle cap exceeded; all cached handles are retained", {
        cap: AETHER_AGENT_DB_OPEN_HANDLE_CAP,
        openHandles: cache.databases.size,
      });
      return;
    }
...
export function closeAetherAgentDatabaseByPath(
  pathname: string,
  ...
  closeCachedAetherAgentDatabase(database);
...
export type AetherAgentDatabaseWorkerCloseResult = {
  errors: Error[];
  settled: boolean;
};
 ...
 */
export function settleAetherAgentDatabaseWorkerClose(
  pathname: string,
): AetherAgentDatabaseWorkerCloseResult {
  const resolvedPath = path.resolve(pathname);
  const errors: Error[] = [];
  const database = cache.databases.get(resolvedPath);
  if (database) {
...
export function closeAetherAgentDatabases(rootPath?: string): void {
  ...
  for (const pathname of cache.databases.keys()) {
    if (rootPath === undefined || isPathInside(rootPath, pathname)) {
      closeAetherAgentDatabaseByPath(pathname);
    }
  }
}

/** Drain native opens before a lifecycle owner releases shared state or removes its root. */
export async function closeAetherAgentDatabasesAsync(rootPath?: string): Promise<void> {
  while (true) {
...
export function inspectAetherAgentDatabaseOwner(
  pathname: string,
): AetherAgentDatabaseOwnerInspection {
  ...
  try {
    ...
    setSqliteBusyTimeout(db, AETHER_SQLITE_BUSY_TIMEOUT_MS);
