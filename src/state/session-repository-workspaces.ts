...
import { ensureSessionRepositoryWorkspaceSchema } from "./aether-state-db-schema-additive.js";
import { tableExists } from "./aether-state-db-schema-helpers.js";
import type { DB, SessionRepositoryWorkspaces } from "./aether-state-db.generated.js";
import {
...
const resultRefPattern = /^refs\/aether\/worker-results\/[A-Za-z0-9-]+$/u;
...
export function createSessionRepositoryWorkspaceStore(
  ...
  const databasePath = (options.database ?? openAetherStateDatabase()).path;
  const now = options.now ?? Date.now;
  const read = () => openAetherStateDatabase({ path: databasePath }).db;
  const write = <T>(operation: (db: DatabaseSync) => T) =>
    runAetherStateWriteTransaction(({ db }) => operation(db), { path: databasePath });
  const ensure = () => {
    const db = read();
    if (!ensured.has(db)) {
        ...
        const inserted = executeSqliteQueryTakeFirstSync(
          ...
          query(db)
            ...
            .values({
              ...
              branch: branch ?? `aether/${workspaceId}`,
              checkpoint_ref: null,
              manifest_hash: null,
              revision: 0,
