...
export function createCommandHandlers(context: CommandHandlerContext) {
  ...
  const setAgent = async (id: string) => {
    ...
    chatLog.addSystem(`agent set to ${state.currentAgentId}; use /aether to return`);
  };
          ...
          if (model?.available === false) {
            const guidance =
              model.unavailableReason === "cooldown"
                ...
                : "Run aether models auth login or choose another model.";
            chatLog.addSystem(
  ...
  const commandHandlers = {
    ...
    aether: (args) => {
      chatLog.addSystem(
        args ? `returning to Aether with request: ${args}` : "returning to Aether",
      );
      requestExit({
      ...
      if (!isBtw) {
        ...
        reduceTuiSessionProjection(state, {
          ...
          message: {
            ...
            __aether: { idempotencyKey: `${runId}:user` },
          },
          runId,
          scope: sendScope,
        });
