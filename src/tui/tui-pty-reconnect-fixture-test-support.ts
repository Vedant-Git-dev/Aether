...
export const TUI_PTY_RECONNECT_FIXTURE = {
  variables: `
      const disconnectReason = process.env.AETHER_TUI_PTY_DISCONNECT_REASON;
      const reconnectOutcome = process.env.AETHER_TUI_PTY_RECONNECT_OUTCOME;
      const reconnectMembership = process.env.AETHER_TUI_PTY_RECONNECT_MEMBERSHIP;
      ...
      let disconnectPending = disconnectReason === undefined
        ? 0
        : Number(process.env.AETHER_TUI_PTY_DISCONNECT_COUNT ?? 1);
