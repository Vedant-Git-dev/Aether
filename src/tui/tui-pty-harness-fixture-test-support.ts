...
export async function startTuiFixture(
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "aether-tui-pty-"));
  ...
  const run = startPty(execPath, resolveRuntimeWorkerArgv(pathToFileURL(scriptPath), execPath), {
    ...
    env: {
      AETHER_THEME: "dark",
      AETHER_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
      AETHER_TUI_PTY_STARTUP_RELEASE_PATH: startupHistoryReleasePath,
    },
...
export async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.AETHER_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.AETHER_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      const startupDelayMs = Number(process.env.AETHER_TUI_PTY_STARTUP_DELAY_MS ?? 0);
      ${TUI_PTY_STARTUP_SESSION_FIXTURE.variables}
      const footerModel = process.env.AETHER_TUI_PTY_MODEL;
      const footerThinkingLevel = process.env.AETHER_TUI_PTY_THINKING_LEVEL;
      let verboseLevel = process.env.AETHER_TUI_PTY_VERBOSE_LEVEL;
      let modeTargetTraceLevel: string | undefined;
      const launchThinkingLevel = process.env.AETHER_TUI_PTY_LAUNCH_THINKING;
      const initialMessage = process.env.AETHER_TUI_PTY_INITIAL_MESSAGE;
      const inFlightRunText = process.env.AETHER_TUI_PTY_IN_FLIGHT_TEXT;
      const dynamicCommandDescription = process.env.AETHER_TUI_PTY_DYNAMIC_COMMAND_DESCRIPTION;
      const thinkingLabel = process.env.AETHER_TUI_PTY_THINKING_LABEL;
      const safeThinkingLabel = process.env.AETHER_TUI_PTY_SAFE_THINKING_LABEL;
      ...
      const enablePickerFixture = process.env.AETHER_TUI_PTY_PICKER_FIXTURE === "1";
      const pickerModelValue = process.env.AETHER_TUI_PTY_PICKER_MODEL_VALUE ?? "fixture-provider/fixture-model-2";
      const pickerModelName = process.env.AETHER_TUI_PTY_PICKER_MODEL_NAME ?? "Fixture 2";
      const pickerSessionKey = process.env.AETHER_TUI_PTY_PICKER_SESSION_KEY ?? "agent:main:picker-target";
      const pickerSessionTitle = process.env.AETHER_TUI_PTY_PICKER_SESSION_TITLE;
      const pickerSessionPreview = process.env.AETHER_TUI_PTY_PICKER_SESSION_PREVIEW;
      const pickerSessionDisplayName = process.env.AETHER_TUI_PTY_PICKER_SESSION_DISPLAY_NAME ?? "Picker target";
      const initialPluginApprovalSessionKey = process.env.AETHER_TUI_PTY_INITIAL_APPROVAL_SESSION_KEY;
      ...
      let fastMode = process.env.AETHER_TUI_PTY_FAST_MODE === "true";
      ...
      class FixtureBackend implements TuiBackend {
        ...
        async sendChat(opts: Parameters<TuiBackend["sendChat"]>[0]) {
          ...
          if (opts.message.startsWith("live reply dedupe proof: ")) {
            ...
            const assistantMessage = {
              ...
              __aether: { id: "live-assistant-" + assistantSequence, seq: assistantSequence },
            };
            ...
            queueMicrotask(() => {
              this.onEvent?.({
                ...
                payload: {
                  ...
                  message: userMessage,
                  messageId: userMessage.__aether.id,
              ...
              this.onEvent?.({
                ...
                payload: {
                  ...
                  messageId: assistantMessage.__aether.id,
            ...
            queueMicrotask(() => {
              ...
              this.onEvent?.({
                ...
                payload: {
                  ...
                  runId,
                  sessionKey: opts.sessionKey,
                  question: process.env.AETHER_TUI_PTY_BTW_QUESTION ?? "picker focus proof",
                  text: "PTY_SIDE_OK",
                },
              });
              this.onEvent?.({
        ...
        async patchSession(opts: Parameters<TuiBackend["patchSession"]>[0]) {
          ...
          const releasePath = process.env.AETHER_TUI_PTY_PATCH_RELEASE_PATH;
      ...
      async function main() {
        await runTui({
          ...
          deliver: process.env.AETHER_TUI_PTY_DELIVER === "1",
          session: process.env.AETHER_TUI_PTY_SESSION,
          thinking: launchThinkingLevel,
          message: initialMessage,
          historyLimit: 5,
          title: "aether tui pty fixture",
