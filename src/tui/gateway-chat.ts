...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set AETHER_GATEWAY_TOKEN/AETHER_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient implements TuiBackend {
  ...
  static async connectBound(
    opts: GatewayConnectionOptions & { config: AetherConfig; url: string },
  ...
  private notifyConnectError(error: Error) {
    ...
    if (
      ...
      error.message = [
        error.message,
        "Pairing request sent. Approve it in that gateway's Control UI (Settings -> Devices), or run `aether devices approve --latest` on the gateway host, then retry.",
      ].join("\n");
    }
...
async function resolveGatewayConnection(
  ...
  const bootstrap = await resolveGatewayClientBootstrap({
    ...
    overrideAuthErrorHint:
      "Fix: pass --token or --password once to request pairing, approve it in that gateway's Control UI (Settings -> Devices), then retry with the same credential so Aether can store the device token.",
