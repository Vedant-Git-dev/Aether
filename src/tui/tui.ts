...
import { getRuntimeConfig, type AetherConfig } from "../config/config.js";
...
export function resolveTuiLocalAuthCliInvocation(params: {
  ...
  return resolveCurrentAetherCliInvocation(
  ...
  return normalizeAgentId(
    params.fallbackAgentId ??
      ...
      resolveDefaultAgentId(params.cfg, {
        surface: "TUI startup",
        hint: `Pass an agent-scoped --session key (e.g., '${formatCliCommand("aether tui --session agent:agentname:main")}').`,
      }),
  );
}
...
export function scheduleProcessExitAfterTuiReturn(
  ...
  const timer = setTimeout(() => {
    try {
      process.stderr.write("aether tui forcing process exit after return\n");
    } catch {
...
function resolveEmptySessionInfoDefaults(config: AetherConfig): SessionInfo {
  return {
...
async function runTuiUnlocked(opts: RunTuiOptions): Promise<TuiResult> {
  ...
  const cliInvocation = resolveCurrentAetherCliInvocation([]);
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__aether_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    const title = opts.title ?? "aether tui";
  ...
  const forceExit = () => {
    try {
      process.stderr.write("aether tui forcing exit\n");
    } catch {
    ...
    beginTuiShutdown({
      ...
      onError: (err) => {
        if (!isTuiTerminalLossError(err)) {
          try {
            process.stderr.write(`aether tui shutdown failed: ${formatTuiErrorMessage(err)}\n`);
          } catch {
