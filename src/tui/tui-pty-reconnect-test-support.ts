  ...
  it.each(
    ...
    async ({ recovery, membership, activity }) => {
      const fixture = await startTuiFixture({
        env: {
          AETHER_TUI_PTY_DISCONNECT_REASON: "fixture transport loss",
          AETHER_TUI_PTY_RECONNECT_OUTCOME: recovery === "gap" ? "gap" : "replacement",
          AETHER_TUI_PTY_RECONNECT_MEMBERSHIP: membership,
        },
      });
      try {
  ...
  await Promise.all([
    ...RECONNECT_CASES.map(async ({ outcome, marker, activity }) => {
      const fixture = await startTuiFixture({
        env: {
          AETHER_TUI_PTY_DISCONNECT_REASON: "fixture transport loss",
          AETHER_TUI_PTY_RECONNECT_OUTCOME: outcome,
        },
      });
      try {
