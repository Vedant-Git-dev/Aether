import { asOptionalObjectRecord as asMessageRecord } from "@aether/normalization-core/record-coerce";
...
function extractPairingQrTerminalText(record: Record<string, unknown>): string {
  ...
  for (const block of content) {
    ...
    if (
      blockRecord.type === "aether_pairing_qr" &&
...
function extractUserAttachmentText(record: Record<string, unknown>): string {
  ...
  // their authoritative attachments in __aether.media instead.
