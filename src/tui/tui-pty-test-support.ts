...
async function writePtyInput(
  ...
  const delayMs = readPositiveIntegerEnv("AETHER_TUI_PTY_TYPE_DELAY_MS", env);
  if (!delayMs || opts.delay === false) {
    pty.write(data);
    return;
  }
  const chunkSize = readPositiveIntegerEnv("AETHER_TUI_PTY_TYPE_CHUNK_SIZE", env) ?? 1;
...
function mirrorPtyOutput(data: string) {
  const mirrorPath = process.env.AETHER_TUI_PTY_MIRROR_PATH;
  if (!mirrorPath) {
    return;
  }
...
export function startPty(
  ...
  const cols = readPtyDimensionEnv("AETHER_TUI_PTY_COLS", 100, ptyEnv);
  const rows = readPtyDimensionEnv("AETHER_TUI_PTY_ROWS", 30, ptyEnv);
