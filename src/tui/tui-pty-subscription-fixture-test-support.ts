...
export const TUI_PTY_SESSION_SUBSCRIPTION_FIXTURE_SCRIPT = `
  ...
  reconnectSessionSubscription() {
    if (process.env.AETHER_TUI_PTY_SUBSCRIBE_RECONNECT !== "1") {
      return;
    }
    process.env.AETHER_TUI_PTY_SUBSCRIBE_RECONNECT = "0";
  ...
  async subscribeSessionEvents() {
    ...
    const configuredFailures = Number(process.env.AETHER_TUI_PTY_SUBSCRIBE_FAILURES ?? 0);
