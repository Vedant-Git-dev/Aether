...
export const TUI_PTY_STARTUP_SESSION_FIXTURE = {
  variables: `
      const restoreDelayMs = Number(process.env.AETHER_TUI_PTY_RESTORE_DELAY_MS ?? 0);
      const restoreFailures = Number(process.env.AETHER_TUI_PTY_RESTORE_FAILURES ?? 0);
      const reconnectHistoryDelayMs = Number(
        process.env.AETHER_TUI_PTY_RECONNECT_HISTORY_DELAY_MS ?? 0,
      );
      let restoreAttempts = 0;
      let reconnectDuringRestore = process.env.AETHER_TUI_PTY_RECONNECT_DURING_RESTORE === "1";
  `,
  loadHistory: `
  ...
  `,
  historyBarrier: `const startupReleasePath = process.env.AETHER_TUI_PTY_STARTUP_RELEASE_PATH;
