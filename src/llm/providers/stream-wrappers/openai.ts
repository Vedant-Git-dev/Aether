...
} from "@aether/ai/internal/openai";
import {
...
} from "@aether/ai/transports";
import { isPromiseLike } from "@aether/normalization-core/promise-like";
...
type AetherSimpleStreamOptions = SimpleStreamOptions & {
  aetherCodeModeToolSurface?: boolean;
  aetherCodeModeAllowedHostedToolTypes?: Set<string>;
};
...
function isCodeModeEnabled(config?: AetherConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  ...
  return (model, context, options) => {
    ...
    const codeModeSurfaceFromOptions =
      (options as AetherSimpleStreamOptions | undefined)?.aetherCodeModeToolSurface === true;
    ...
    if (
      ...
      const allowedHostedToolTypes =
        (options as AetherSimpleStreamOptions | undefined)
          ?.aetherCodeModeAllowedHostedToolTypes ?? new Set<string>();
      const activation =
      ...
      const observeToolSurface =
        ...
        (codeModeDiagnosticsEnabled
          ...
          : undefined);
      const codeModeOptions: AetherSimpleStreamOptions = {
        ...options,
        aetherCodeModeToolSurface: true,
        aetherCodeModeAllowedHostedToolTypes: allowedHostedToolTypes,
        onPayload: (payload) => {
          if (activation?.state === "native_active") {
