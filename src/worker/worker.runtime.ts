...
export async function createWorkerRuntimeEnvironment(sessionId: string) {
  const stateDir = await mkdtemp(path.join(tmpdir(), "aether-worker-"));
  ...
  process.env.AETHER_CONFIG_PATH = path.join(stateDir, "aether.json");
    ...
    close: () =>
      (closing ??= (async () => {
        ...
        closeAetherStateDatabaseByPath(
          resolveAetherStateSqlitePath({ AETHER_STATE_DIR: stateDir }),
        );
