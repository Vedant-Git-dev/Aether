...
const FIXED_DESTINATION = "aether-rsync-destination";
...
const receiverTarget =
  receiverMode === "git-pack"
    ? path.posix.join(workspace, ".aether-base.pack")
    : receiverMode === "accepted-next"
      ? path.posix.join(
          path.posix.dirname(workspace),
          `.aether-accepted-${createHash("sha256").update(workspace).digest("hex")}-${receiverNonce}`,
          "next",
        )
      : workspace;
