...
import * as statePaths from "../state/aether-state-db.paths.js";
...
export function useIsolatedStateGuard(): void {
  const resolveStatePath = statePaths.resolveAetherStateSqlitePath;
  const openDatabase = nodeSqlite.openNodeSqliteDatabase;
  let restore = () => {};
  beforeEach(() => {
    const testHome = process.env.AETHER_TEST_HOME;
    if (!testHome) {
    ...
    const assertOwnedPath = (pathname: string) => {
      ...
      if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
        throw new Error(`Aether state escaped the isolated test home: ${pathname}`);
      }
    };
    // Check resolution too: a missing foreign DB would otherwise make the leak silently pass.
    const pathSpy = vi
      .spyOn(statePaths, "resolveAetherStateSqlitePath")
