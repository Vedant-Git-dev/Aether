...
function readPluginManifestId(pluginDir: string): string | undefined {
  try {
    const manifestPath = path.join(pluginDir, "aether.plugin.json");
...
function findBundledPluginMetadataFast(
  ...
  const rawRoots = [
    ...
    path.resolve(AETHER_PACKAGE_ROOT, "extensions"),
    path.resolve(AETHER_PACKAGE_ROOT, "dist-runtime", "extensions"),
    path.resolve(AETHER_PACKAGE_ROOT, "dist", "extensions"),
