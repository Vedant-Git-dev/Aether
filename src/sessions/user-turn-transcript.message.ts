...
import { mimeTypeFromFilePath } from "@aether/media-core/mime";
...
export function buildLateMediaAttachedProjection(message: AgentMessage): {
  ...
  const isLateMedia = readAetherMessageMeta(message)?.lateMedia === true;
...
function readAetherMessageMeta(message: AgentMessage): Record<string, unknown> | undefined {
  return asOptionalRecord(Reflect.get(message, "__aether"));
}
export function buildPersistedUserTurnMessage(params: UserTurnInput): PersistedUserTurnMessage {
  ...
  // the live turn) — see https://github.com/aether/aether/issues/3658.
  const aetherMeta = buildPersistedUserTurnMetadata(params, normalizedMedia);
  const message: PersistedUserTurnMessage = {
    ...
    ...(Object.keys(aetherMeta).length > 0 ? { __aether: aetherMeta } : {}),
  };
...
export function buildLateResolvedMediaMessage(params: {
  ...
  const metadata: Record<string, unknown> = {
    ...readAetherMessageMeta(params.resolvedMessage),
    lateMedia: true,
  };
...
function isBeforeAgentRunBlockedMessage(message: AgentMessage): boolean {
  const marker = readAetherMessageMeta(message)?.beforeAgentRunBlocked;
...
export function mergePreparedUserTurnMessageForRuntime(params: {
  ...
  const runtimeMeta = readAetherMessageMeta(params.runtimeMessage);
  const preparedMeta = readAetherMessageMeta(params.preparedMessage);
  return {
    ...params.runtimeMessage,
    ...params.preparedMessage,
    ...(preparedMeta ? { __aether: { ...runtimeMeta, ...preparedMeta } } : {}),
