...
export function attachSessionTranscriptRunId<T>(message: T, runId: string | null | undefined): T {
  ...
  const metadata = isRecord(message["__aether"]) ? message["__aether"] : {};
  if (metadata.runId === normalizedRunId) {
    return message;
  }
  return {
    ...message,
    __aether: { ...metadata, runId: normalizedRunId },
  };
}
