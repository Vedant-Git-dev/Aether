...
export function rewritePersistedSteerTargetRunId(
  ...
  const metadata = { ...message["__aether"] };
  delete metadata.steerTargetRunId;
  if (targetRunId) {
    metadata.steerTargetRunId = targetRunId;
  }
  const nextMessage = { ...message };
  delete nextMessage["__aether"];
  if (Object.keys(metadata).length > 0) {
    nextMessage["__aether"] = metadata;
  }
  return nextMessage;
}
...
>(params: { runtimeMessage: TMessage; preparedMessage?: PersistedUserTurnMessage }): TMessage {
  ...
  const preparedMeta = params.preparedMessage["__aether"];
  ...
  const nextMessage: TMessage & { display?: boolean; __aether?: Record<string, unknown> } = {
  ...
  const runtimeMeta = { ...nextMessage["__aether"] };
  ...
  if (Object.keys(runtimeMeta).length > 0) {
    nextMessage["__aether"] = runtimeMeta;
  }
  return nextMessage;
}
...
export function applyTranscriptSenderIdentityToWrite(
  ...
  const original = asOptionalRecord(Reflect.get(message, "__aether"));
  const identity =
    message.role === "user" ? readTranscriptSenderIdentity(original?.senderIdentity) : undefined;
  const senderId = original?.senderId;
  const next = write();
  const metadata = next && asOptionalRecord(Reflect.get(next, "__aether"));
  ...
  return Object.assign({ ...next }, { __aether: redacted });
}
...
export function preparePersistedUserTurnMessageForTranscriptWrite(
  ...
  const originalMeta = message["__aether"];
  ...
  delete protectedMessage["__aether"];
  if (Object.keys(protectedMeta).length > 0) {
    protectedMessage["__aether"] = protectedMeta;
  }
