...
>;
type SessionStateEventsTable = AetherStateKyselyDatabase["session_state_events"];
type SessionStateEventRow = Selectable<SessionStateEventsTable>;
type SessionWatchCursorRow = Selectable<AetherStateKyselyDatabase["session_watch_cursors"]>;
...
export function recordSessionStateEvent(
  input: SessionStateEventInput,
  options: AetherStateDatabaseOptions & { now?: number } = {},
  ...
  try {
    const event = runAetherStateWriteTransaction(({ db }) => {
...
export function registerMainSessionGroupWatch(
  ...
  const now = options.now ?? Date.now();
  try {
    const { db: readDb } = openAetherStateDatabase(options);
