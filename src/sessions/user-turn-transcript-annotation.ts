...
export function bindUserTurnTranscriptAnnotation(params: {
  ...
  const assertCurrent = () => {
    ...
    if (
      ...
      admittedMessage["__aether"]?.steerTargetRunId !== undefined ||
      ...
      sessionTranscriptIndexNeedsReconcile(
        openAetherAgentDatabase({ agentId: admission.agentId, path: admission.storePath }).db,
        admission.sessionId,
      ) ||
  ...
  return async (annotation) => {
    ...
    const rewritten = await rewriteTranscriptMessageAtAnchor(admission, (current) => {
      ...
      const metadata = admittedMessage["__aether"] ?? {};
      if (
      ...
      const next = { ...admittedMessage, __aether: { ...metadata, ...fields } };
