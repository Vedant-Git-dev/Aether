...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Aether. Do not edit while the gateway service is installed.",
...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to aether gateway install --wrapper <path> or AETHER_WRAPPER.`,
  ];
}
...
export async function publishLaunchAgentPlist(params: {
  ...
  const temporaryPath = `${params.plistPath}.aether-${randomUUID()}.tmp`;
    ...
    try {
      ...
      try {
        if (previousContents === null) {
          ...
          const rollbackPath = `${params.plistPath}.aether-${randomUUID()}.rollback`;
          try {
...
export async function rewriteLaunchAgentPlistForRestart({
  ...
  const canonicalEnvironment = {
    ...
    AETHER_SERVICE_VERSION: undefined,
  };
  const prepared = await prepareLaunchAgentProgramArguments({
    env,
    label,
