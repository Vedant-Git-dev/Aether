...
export async function launchFallbackTaskScript(
  ...
  if (command?.programArguments.length) {
    ...
    const programArguments =
      command.environment?.AETHER_SERVICE_KIND === "gateway"
  ...
  const scriptProbe = spawnSync(
    ...
    [
      ...
      "-EncodedCommand",
      Buffer.from(
        "$ErrorActionPreference='Stop'; [System.IO.File]::OpenRead($env:AETHER_TASK_SCRIPT).Dispose()",
        "utf16le",
      ).toString("base64"),
    ],
    {
      env: { ...resolveServiceManagerEnv(), AETHER_TASK_SCRIPT: scriptPath },
      stdio: "ignore",
      windowsHide: true,
    },
  );
  ...
  const { child } = await spawnWithFallback({
    ...
    argv: [getWindowsCmdExePath(), "/d", "/s", "/v:off", "/c", '""%AETHER_TASK_SCRIPT%""'],
    options: {
      detached: true,
      env: { ...process.env, AETHER_TASK_SCRIPT: scriptPath },
      stdio: "ignore",
      windowsHide: true,
