/** Inspects installed platform services for extra Aether or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["aether", "aether"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["aether_service_marker"];
  const kindKeys = ["aether_service_kind"];
...
function isAetherGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "aether") {
    return false;
  }
  return label.startsWith("ai.aether.");
}

function isAetherGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("aether-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isAetherGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  // Windows schtasks /Query returns task names prefixed with \ (e.g.
  // \Aether Gateway for root-folder tasks). Strip the leading
  ...
  return stripped === defaultName || /^aether gateway \(.+\)$/.test(stripped);
}
...
async function scanLaunchdDir(params: {
  ...
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".plist",
    isIgnoredName: params.includeManagedAether ? () => false : isIgnoredLaunchdLabel,
  });

  for (const { name: labelFromName, fullPath, contents } of candidates) {
    const nativeLabel = params.includeManagedAether
    ...
    const marker =
      ...
      executionMarker === "aether"
        ? "aether"
        : executionMarker === "aether" || legacyLabel
          ? "aether"
          : null;
    if (!marker) {
      continue;
    ...
    if (!params.includeManagedAether && isIgnoredLaunchdLabel(label)) {
      continue;
    }
    if (
      !params.includeManagedAether &&
      marker === "aether" &&
      isAetherGatewayLaunchdService(label, contents)
    ) {
      continue;
    }
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "aether" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedAether ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedAether &&
      marker === "aether" &&
      isAetherGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isAetherGatewayTaskName(name)) {
        continue;
      }
