...
export async function resolveLaunchAgentGatewayContext(env: GatewayServiceEnv): Promise<{
  ...
  return {
    port:
      ...
      parseTcpPort(command?.environment?.AETHER_GATEWAY_PORT ?? "") ??
      parseTcpPort(env.AETHER_GATEWAY_PORT ?? ""),
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.aether.ai/gateway",
  ].join("\n");
}
