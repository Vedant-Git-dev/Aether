  ...
  if (stat.isFile()) {
    ...
    for (const rawLine of splitSystemdLogicalLines(content)) {
      ...
      for (const { key, value } of parseSystemdEnvAssignments(line.slice(separator + 1).trim())) {
        ...
        if (
          value &&
          (normalizedKey === "AETHER_GATEWAY_TOKEN" ||
            normalizedKey === "AETHER_GATEWAY_PASSWORD")
        ) {
...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds AETHER_GATEWAY_TOKEN and should be reinstalled.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service AETHER_GATEWAY_TOKEN does not match gateway.auth.token in aether.json",
...
function auditGatewayPassword(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  if (
    !command?.environment?.AETHER_GATEWAY_PASSWORD?.trim() ||
    isEnvironmentFileOnlySource(command.environmentValueSources?.AETHER_GATEWAY_PASSWORD)
  ) {
    return;
  }
  issues.push({
    code: SERVICE_AUDIT_CODES.gatewayPasswordEmbedded,
    message: "Gateway service embeds AETHER_GATEWAY_PASSWORD and should be reinstalled.",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.AETHER_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.AETHER_GATEWAY_TOKEN);
}
