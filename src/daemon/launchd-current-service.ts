...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  // Detached update/restart handoffs keep AETHER_LAUNCHD_LABEL as the service
  ...
  return (
    normalizeOptionalString(env.AETHER_LAUNCHD_LABEL) === label &&
    normalizeOptionalString(env.AETHER_SERVICE_MARKER) === "aether" &&
    Boolean(normalizeOptionalString(env.AETHER_SERVICE_KIND))
  );
}
 ...
 * The detached update helper's recovery CLI inherits AETHER_LAUNCHD_LABEL but
