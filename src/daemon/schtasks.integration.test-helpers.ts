...
async function inspectNativeAclDenial(params: {
  ...
  const scriptEnv = { ...process.env, AETHER_TASK_SCRIPT: params.scriptPath };
  ...
  const dotnetOpenRead = inspectNativeAclCommand(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      '$ErrorActionPreference="Stop"; $stream=[System.IO.File]::OpenRead($env:AETHER_TASK_SCRIPT); try { [void]$stream.ReadByte() } finally { $stream.Dispose() }',
    ],
    scriptEnv,
  );
  const cmdType = inspectNativeAclCommand(
    getWindowsCmdExePath(),
    ["/d", "/s", "/v:off", "/c", '"type "%AETHER_TASK_SCRIPT%""'],
    scriptEnv,
    true,
  );
  ...
  const cmdBatchResult = inspectNativeAclCommand(
    getWindowsCmdExePath(),
    ["/d", "/s", "/v:off", "/c", '""%AETHER_TASK_SCRIPT%""'],
    scriptEnv,
    true,
  );
...
export async function proveNativeStartupFallbackLaunch(params: {
  ...
  const stateDir = path.join(proofRoot, "state & %AETHER_STARTUP_PROBE% !");
  const env: GatewayServiceEnv = {
    ...params.env,
    APPDATA: path.join(proofRoot, "appdata"),
    AETHER_CONFIG_PATH: path.join(stateDir, "aether.json"),
  ...
  await fs.writeFile(
    ...
    [
      ...
      `const env = ${JSON.stringify({
        APPDATA: env.APPDATA,
        AETHER_CONFIG_PATH: env.AETHER_CONFIG_PATH,
        AETHER_PROFILE: env.AETHER_PROFILE,
        AETHER_STATE_DIR: env.AETHER_STATE_DIR,
      })};`,
