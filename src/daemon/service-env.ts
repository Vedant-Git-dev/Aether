...
function readServiceSqliteEnvironment(
...
): { AETHER_SQLITE_LIBRARY?: string; HOMEBREW_PREFIX?: string } {
  ...
  if (
    platform !== "darwin" ||
    (runtime !== "bun" && !normalizeOptionalString(env.AETHER_WRAPPER))
  ) {
    return {};
  }
  const library = normalizeOptionalString(env.AETHER_SQLITE_LIBRARY);
  const prefix = normalizeOptionalString(env.HOMEBREW_PREFIX);
  return {
    ...(library ? { AETHER_SQLITE_LIBRARY: library } : {}),
...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical Aether proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.AETHER_PROXY_URL);
  return proxyUrl ? { AETHER_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.AETHER_PROFILE;
  const wrapperPath = normalizeOptionalString(env.AETHER_WRAPPER);
  ...
  return {
    ...
    ...(env.AETHER_CONFIG_READONLY !== undefined
      ? { AETHER_CONFIG_READONLY: env.AETHER_CONFIG_READONLY }
      : {}),
    AETHER_WRAPPER: wrapperPath,
    AETHER_GATEWAY_PORT: String(port),
    AETHER_LAUNCHD_LABEL: resolvedLaunchdLabel,
    AETHER_SYSTEMD_UNIT: systemdUnit,
    AETHER_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    AETHER_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    AETHER_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    AETHER_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.AETHER_GATEWAY_TOKEN);
  const gatewayPassword = normalizeOptionalString(env.AETHER_GATEWAY_PASSWORD);
  ...
  const allowInsecurePrivateWs = normalizeOptionalString(env.AETHER_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ...readServiceSqliteEnvironment(env, platform, params.runtime),
    AETHER_GATEWAY_TOKEN: gatewayToken,
    AETHER_GATEWAY_PASSWORD: gatewayPassword,
    CF_ACCESS_CLIENT_ID: cloudflareAccessClientId,
    CF_ACCESS_CLIENT_SECRET: cloudflareAccessClientSecret,
    AETHER_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    AETHER_STATE_DIR: sharedEnv.stateDir,
    AETHER_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.AETHER_STATE_DIR;
  const configPath = env.AETHER_CONFIG_PATH;
