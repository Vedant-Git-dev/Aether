  ...
  if (params.kind === "stopped") {
    hints.push(
      ...buildPlatformRuntimeLogHints({
        ...
        env: params.env,
        systemdServiceName: resolveGatewaySystemdServiceName(params.env.AETHER_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(params.env.AETHER_PROFILE),
      }),
    );
  }
  return hints;
...
export function buildPlatformServiceStartHints(params: {
  ...
  // Install guidance and the Aether start command stay first; native manager
