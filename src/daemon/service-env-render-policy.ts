...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  const launchAgent =
    ...
    Boolean(params.serviceEnvironment.AETHER_LAUNCHD_LABEL?.trim());
  writeManagedServiceEnvKeysToEnvironment(params.plan.environment, params.managedServiceEnvKeys);
  if (params.plan.environment.AETHER_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.AETHER_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    AETHER_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
