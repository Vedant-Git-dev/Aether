...
async function resolveScheduledTaskProcess(
  ...
  // Match full persisted argv so a same-port Aether process cannot impersonate this task.
...
export function shouldManageGatewayListenerPort(env: GatewayServiceEnv): boolean {
  return normalizeLowercaseStringOrEmpty(env.AETHER_SERVICE_KIND) !== NODE_SERVICE_KIND;
}
