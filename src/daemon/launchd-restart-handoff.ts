...
printf '[%s] aether restart attempt source=handoff mode=${mode} target=%s pid=%s interactive=0\\n' "$(date -u +%FT%TZ)" "$service_target" "$wait_pid" >&2
...
if [ -n "$aether_system_launchd_conflict" ]; then
  printf '[%s] aether restart blocked source=handoff mode=${mode} reason=%s interactive=0\n' "$(date -u +%FT%TZ)" "$aether_system_launchd_detail" >&2
  exit 78
fi
`;
...
if [ "$status" -eq 0 ]; then
  printf '[%s] aether service park done source=handoff interactive=0\\n' "$(date -u +%FT%TZ)" >&2
else
  printf '[%s] aether service park failed source=handoff status=%s interactive=0\\n' "$(date -u +%FT%TZ)" "$status" >&2
fi
exit "$status"
`;
  }
...
if [ "$status" -eq 0 ]; then
  printf '[%s] aether restart done source=handoff mode=${mode} interactive=0\\n' "$(date -u +%FT%TZ)" >&2
else
  printf '[%s] aether restart failed source=handoff mode=${mode} status=%s interactive=0\\n' "$(date -u +%FT%TZ)" "$status" >&2
fi
exit "$status"
`;
  }
  ...
  try {
    const child = spawn(
      ...
      [
        ...
        "aether-launchd-restart-handoff",
