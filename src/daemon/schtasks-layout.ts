...
export async function writeTaskXmlTempFile(xml: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-task-xml-"));
...
export function shouldUseHiddenWindowsTaskLauncher(env: GatewayServiceEnv): boolean {
  const value = normalizeLowercaseStringOrEmpty(env.AETHER_WINDOWS_TASK_HIDDEN_LAUNCHER);
...
export function buildTaskScript({
  ...
  const commandArguments =
    environment?.AETHER_SERVICE_KIND === "gateway"
