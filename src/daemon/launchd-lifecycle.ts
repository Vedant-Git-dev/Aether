...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      actionHint: "aether gateway start",
...
function formatLaunchAgentLeftUnloadedError(params: {
  ...
  return [
    ...
    `Fix: run \`aether gateway start\`, or \`launchctl bootstrap ${params.domain} ${params.plistPath}\`.`,
  ].join("\n");
}
...
export async function restartLaunchAgent({
  ...
  // `aether gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    try {
      await bootstrapLaunchAgentOrThrow({
        ...
        serviceTarget,
        plistPath,
        actionHint: "aether gateway restart",
        onMutation: reportMutation,
