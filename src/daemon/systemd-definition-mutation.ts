...
export async function readSystemdDefinitionMutationCapability(
  ...
  const names =
    selected === "aether-gateway.service" ? [selected, "aether.service"] : [selected];
  const budget =
...
export async function withSystemdDefinitionMutation<T>(
  ...
  const targets = lockedTargets
    .map((target) => path.join(path.dirname(target), `.aether-${sha256Hex(target)}`))
    .toSorted();
  const execute = async (): Promise<T> => {
    ...
    const publish = async (
      ...
      try {
        ...
        await refresh(true);
        // Locks coordinate Aether writers, not external editors: POSIX rename
