...
async function currentGatewayLaunchAgentLabel(
  ...
  const configuredCurrentLabel = process.env.AETHER_LAUNCHD_LABEL?.trim();
...
async function restoreLaunchAgentOwnedFile(params: {
  ...
  const temporaryPath = `${params.path}.aether-${randomUUID()}.rollback`;
  try {
...
async function activateLaunchAgent(params: {
  ...
  try {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "aether gateway install --force",
      retryPendingTeardown: true,
    });
  } catch (error) {
    try {
