/** Summarizes installed service command paths and Aether package layout. */
...
async function resolveAetherPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveAetherPackageRoot(entrypointReal) : undefined;
