...
function isCandidate(label: string, env: NodeJS.ProcessEnv): boolean {
  return (
    /^ai\.aether\.[A-Za-z0-9._-]+$/.test(label) &&
    !new Set([
      resolveGatewayLaunchAgentLabel(),
      resolveGatewayLaunchAgentLabel(env.AETHER_PROFILE),
...
function lifecycleInvocation(
  ...
  if (["node", "bun"].includes(path.basename(words[0] ?? ""))) {
    words = words.slice(1);
    if (path.basename(words[0] ?? "") !== "aether.mjs") {
      return undefined;
    }
  }
  if (
    !path.isAbsolute(words[0] ?? "") ||
    !["aether", "aether.mjs"].includes(path.basename(words[0] ?? ""))
  ) {
    return undefined;
  }
  words = words.slice(1);
...
const SCRIPT_HELPER_NAME = /(?:aether_|AETHER_)[A-Za-z0-9_]*/;
const SCRIPT_LITERAL_EXEC = String.raw`/(?:[A-Za-z0-9_.-]+/)*aether(?:\.mjs)?`;
...
// an absolute Aether path; inspectJob also excludes shell-altering job environments.
...
async function inspectJob(
  ...
  const hasServiceMarker =
    /^\t\tAETHER_SERVICE_MARKER => aether$/m.test(environment) &&
    /^\t\tAETHER_SERVICE_KIND => (gateway|node)$/m.test(environment);
  const generatedWrapper = args.some((arg) => arg.endsWith(`/service-env/${label}-env-wrapper.sh`));
  if (
    hasServiceMarker ||
    generatedWrapper ||
    /<key>Comment<\/key>\s*<string>Aether (Gateway|Node)\b/.test(plist ?? "")
  ) {
    return null;
  }
  ...
  const programScript =
    ...
    (shellEnvironmentDiagnostic ||
      !["aether", "aether.mjs", "node", "bun", "env"].includes(programName))
...
export async function findForeignLaunchdJobs(
  ...
  if (labels.length > MAX_JOBS) {
    throw new Error(
      `Too many Aether launchd jobs to inspect safely (${labels.length}; limit ${MAX_JOBS}).`,
    );
  }
  ...
  for (const label of labels) {
    if (Date.now() >= deadline) {
      throw new Error("Aether launchd job inspection exceeded its 10-second budget.");
    }
  ...
  return jobs
    .map((job) =>
      [
        ...
        job.safeToRemove
          ? "  Removable with aether vitals --fix."
