...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // values are also omitted after Aether moves them to the generated env file.
...
function removeLegacyGatewayVersionMetadata(content: string): string {
  const description =
    /^Description=Aether Gateway \((?:(profile: [^,)\r\n]+), )?v([^)\r\n]+)\)$/mu.exec(content);
  if (!description) {
    return content;
  }
  ...
  if (
    inlineEnvironment.get("AETHER_SERVICE_MARKER") !== GATEWAY_SERVICE_MARKER ||
    inlineEnvironment.get("AETHER_SERVICE_KIND") !== GATEWAY_SERVICE_KIND ||
    inlineEnvironment.get("AETHER_SERVICE_VERSION") !== description[2]
  ) {
    return content;
  }
  const replacement = description[1]
    ? `Description=Aether Gateway (${description[1]})`
    : "Description=Aether Gateway";
  const refreshed =
    content.slice(0, description.index) +
    replacement +
    content.slice(description.index + description[0].length);
  return removeSystemdInlineEnvironmentKeys(refreshed, new Set(["AETHER_SERVICE_VERSION"]));
}
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set(["AETHER_GATEWAY_TOKEN", "AETHER_GATEWAY_PASSWORD"]);
