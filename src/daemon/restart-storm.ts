...
const WARNING_PREFIX = "aether gateway restart-storm warning ";
  ...
  try {
    ...
    for (const line of tail.split("\n")) {
      ...
      if (body.startsWith(WARNING_PREFIX)) {
        ...
        /^aether gateway lifecycle source=cli action=restart mode=(?:kickstart|bootout)(?: |$)/.test(
          body,
        )
      ) {
...
export async function warnAboutGatewayRestartStorm(
  ...
  const message = [
    ...
    likelyJobs.length > 0
      ...
      : "Check for a stray keepalive launchd job invoking aether gateway restart.",
    "Run aether gateway status to inspect jobs and aether vitals --fix to remove eligible stray jobs.",
  ].join(" ");
  try {
    fs.appendFileSync(
      resolveGatewayRestartLogPath(env),
