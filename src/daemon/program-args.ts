...
export const AETHER_WRAPPER_ENV_KEY = "AETHER_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/aether -> .pnpm/aether@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // aether from node_modules/.bin points at the package root sibling.
...
export async function resolveAetherWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${AETHER_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveAetherWrapperPath(params.wrapperPath);
  if (wrapperPath) {
