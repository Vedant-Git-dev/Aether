// Generates setup codes used to pair external channels with Aether.
...
export function resolveConfiguredPairingPublicUrl(config: AetherConfig): string | undefined {
...
function describeSecurePairingFix(source?: string): string {
  ...
  return (
    ...
    "ws:// is only valid for localhost, private LAN addresses, or .local hosts."
  );
}

function normalizePairingHost(host: string): string {
...
function isPrivateLanHost(host: string): boolean {
  const normalized = normalizePairingHost(host);
...
function isPairingCleartextAllowedHost(host: string): boolean {
  const normalized = normalizePairingHost(host);
  return (
    normalized === "localhost" || isLoopbackIpAddress(normalized) || isPrivateLanHost(normalized)
  );
}
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.AETHER_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.AETHER_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenFallback: Boolean(normalizeOptionalString(env.AETHER_GATEWAY_TOKEN)),
    hasPasswordFallback: Boolean(normalizeOptionalString(env.AETHER_GATEWAY_PASSWORD)),
  });
  ...
  const pairingUrlError = validatePairingUrl(urlResult.url, urlResult.source);
  if (pairingUrlError) {
    return { ok: false, error: pairingUrlError };
  }
  ...
  const issuedBootstrapProfile = options.bootstrapProfile ?? PAIRING_SETUP_BOOTSTRAP_PROFILE;
