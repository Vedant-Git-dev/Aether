...
const SUPPORT_EXPORT_PREFIX = "aether-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.AETHER_DISABLE_BONJOUR?.trim();
  if (!raw) {
    return "unset";
  }
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$AETHER_STATE_DIR/logs/stability" };
  }
  try {
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/aether-sanitized.jsonl`: sanitized log summaries and metadata",
