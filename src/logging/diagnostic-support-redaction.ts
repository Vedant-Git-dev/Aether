...
// Public Aether package references must remain usable in repair commands.
const HANDLE_RE = /(^|[^\w:/])@(?!aether\/[a-z0-9])[A-Za-z0-9_]{5,}\b(?!\.)/gu;
...
function pathRedactionPrefixes(options: SupportRedactionContext): PathRedactionPrefix[] {
  ...
  addPathPrefixVariants(prefixes, options.stateDir, "$AETHER_STATE_DIR");
...
function redactServiceIdentifiersForSupport(value: string): string {
  ...
  return value
    ...
    .replace(MATRIX_EVENT_ID_RE, (eventId) =>
      eventId === "$AETHER_STATE_DIR" ? eventId : "<redacted-matrix-event>",
    );
}
