...
function formatPressureNextStep(
  ...
  return pressure.level === "critical"
    ? "nextStep=inspect latest stability bundle or run aether gateway diagnostics export; restart gateway if process is unstable"
    : "nextStep=run aether gateway status --deep and aether gateway diagnostics export; restart gateway if pressure persists";
}
