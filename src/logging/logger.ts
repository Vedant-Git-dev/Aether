...
import { DEFAULT_POSIX_TMP_ROOT } from "../infra/tmp-aether-dir.js";
...
export const DEFAULT_LOG_FILE = `${DEFAULT_LOG_DIR}/aether.log`; // legacy single-file path
...
type LoggerConfigLoader = () => AetherConfig["logging"] | undefined;
...
export function applyLoggingConfig(config: AetherConfig["logging"] | undefined): void {
...
export function readLoggerConfig(): AetherConfig["logging"] | undefined {
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.AETHER_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.AETHER_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedRuntimeSettings {
  ...
  const cfg: AetherConfig["logging"] | LoggerSettings | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.AETHER_TEST_FILE_LOG !== "1" ? "silent" : "info";
