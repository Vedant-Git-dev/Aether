...
} from "../infra/tmp-aether-dir.js";
import { canUseNodeFs, formatLocalDate, LOG_PREFIX, LOG_SUFFIX } from "./log-file-shared.js";

const ROLLING_LOG_FILE_RE = /^(aether(?:-[a-z0-9-]+)?)-(\d{4}-\d{2}-\d{2})\.log$/u;
...
export function resolveDefaultRollingLogFile(options?: {
  ...
  const logDir =
    options?.logDir ?? (canUseNodeFs() ? resolvePreferredAetherTmpDir() : DEFAULT_POSIX_TMP_ROOT);
...
/** Returns whether a path is one of Aether's dated rolling log files. */
