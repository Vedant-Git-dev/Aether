...
function buildDroppedMarker(target: FileLogQueueEntry, count: number): FileLogQueueEntry {
  const date = new Date();
  const message = `[aether] file log queue overflow; dropped ${count} oldest record${count === 1 ? "" : "s"}`;
  const record = {
    0: message,
    _meta: {
      date,
...
function warnAboutRotationFailure(entry: FileLogQueueEntry, synchronous: boolean): void {
  ...
  const message = `[aether] log file rotation failed; continuing writes file=${entry.file} maxFileBytes=${entry.maxFileBytes}`;
...
function warnAboutAppendFailure(entry: FileLogQueueEntry, synchronous: boolean): void {
  ...
  const message = saturated
    ? "[aether] log file append failure diagnostics saturated; suppressing new file targets"
    : `[aether] log file append failed; records dropped; check that the path is a writable regular file; file=${entry.file}`;
