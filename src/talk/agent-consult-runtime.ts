    ...
    return await sessionWorkAdmission.run(async () => {
      ...
      const runPromise = params.agentRuntime.runEmbeddedAgent({
        ...
        extraSystemPrompt:
          ...
          "You are the configured Aether agent receiving delegated requests from a live voice bridge. Act on behalf of the user, use available tools when appropriate, and return a brief speakable result.",
        agentDir,
        abortSignal,
      });
