/**
 * Runtime adapter for realtime voice control of active Aether agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  const noActiveRun = (): RealtimeVoiceAgentControlResult => ({
    ...
    message: `There is no active Aether run to ${mode === "cancel" ? "cancel" : "steer"}.`,
    ...controlResultPresentation,
  });
  ...
  if (mode === "cancel") {
    ...
    const message = aborted
      ? "Cancelled the active Aether run."
      : "Aether could not cancel the active run.";
    return {
      ok: aborted,
      ...controlResultContext,
      sessionId,
  ...
  const message = unconfirmed
    ? "Aether could not confirm that input. It was not sent again; check the conversation before retrying."
    : mode === "followup"
      ? "Queued that follow-up for the active Aether run."
