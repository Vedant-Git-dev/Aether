/**
 * Shared realtime voice controls for active Aether agent runs.
 *
 ...
 */
import { asNonArrayRecord } from "@aether/normalization-core/record-coerce";
import {
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "aether_agent_control";
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Control an active Aether tool-backed voice run. Use this when the caller asks in any language for status/progress, cancellation, a redirect/change to the active work, or a follow-up after the current work. Do not use this for ordinary greetings or chatter unless the caller is asking about the active work.",
  parameters: {
    type: "object",
    properties: {
      text: {
...
export const REALTIME_VOICE_AGENT_CONTROL_FAILURE_MESSAGE =
  "Aether could not process that voice control. Please try again.";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Aether voice control result.",
    "Do not delegate this message or call any tools.",
    "Speak this exact Aether status to the voice call, without adding, removing, or rephrasing words.",
...
export function buildRealtimeVoiceAgentCancelProviderResult(
  message = "Cancelled the active Aether run.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Aether is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Aether has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Aether could not queue that follow-up."
    : "Aether could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Aether finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Aether is starting ${name}.` : "Aether is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Aether finished ${name} and is continuing.`
        : "Aether finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Aether is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Aether is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Aether is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Aether is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Aether is working on the current voice request.";
  }
