...
export function readVoiceSessionRecord(
  ...
  return readVoiceSessionRecordInTransaction(
    openAetherAgentDatabase({ agentId }),
    voiceSessionId,
  );
}

function voiceSessionRowsQuery(database: AetherAgentDatabase) {
  return getNodeSqliteKysely<Pick<AetherAgentKyselyDatabase, "cache_entries">>(database.db)
...
export function readVoiceSessionRecordRows(agentId: string, updatedBefore?: number) {
  const database = openAetherAgentDatabase({ agentId });
...
export function writeVoiceSessionRecordInTransaction(
  ...
  const { compiled, bind } = compileSqliteQueryBindings<ClientVoiceSessionRecord>((p) =>
    getNodeSqliteKysely<Pick<AetherAgentKyselyDatabase, "cache_entries">>(database.db)
