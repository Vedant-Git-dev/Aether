// Implements maintenance commands for Aether-backed session cleanup.
...
export const handleSystemAgentCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    logVerbose(
      `Ignoring /aether from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
    );
  ...
  return {
    ...
    reply: {
      text:
        ...
        })) ?? "Aether did not find a rescue request.",
    },
  };
};
