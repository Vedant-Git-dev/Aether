...
import { buildCurrentAetherCliExecRequest } from "./commands-aether-cli.js";
import {
...
const DIAGNOSTICS_DOCS_URL = "https://docs.aether.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "aether gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonRequest() {
  return buildCurrentAetherCliExecRequest(["gateway", "diagnostics", "export", "--json"]);
}
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Aether session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include an Aether session file.",
    ) === true
  );
}
