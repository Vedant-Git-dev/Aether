...
import { type AetherConfig, getRuntimeConfig } from "../../config/config.js";
...
function hasExplicitAudioUnderstandingConfig(cfg: AetherConfig): boolean {
  ...
  try {
    ...
    const text =
      error instanceof WorkspaceAliasRepointedError
        ? "⚠️ This agent's workspace state needs repair: the configured workspace path no longer matches its stored identity. Ask the gateway operator to run `aether vitals --fix` and confirm the move only if the same workspace moved."
        : "⚠️ This agent's workspace is missing on the gateway host. Ask the operator to restore the workspace from backup and run `aether vitals`.";
