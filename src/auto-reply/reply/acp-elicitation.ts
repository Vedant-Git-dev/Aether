import type { AcpElicitationRequest } from "@aether/acp-core/runtime/types";
import {
...
export function parseAcpElicitationRequest(
  ...
  if (!isStructuredInputRecord(snapshot)) {
    return unsupported("Aether declined a malformed or over-limit ACP input request.");
  }
  ...
  if (mode !== "form") {
    return unsupported(
      `Aether does not support ACP elicitation mode ${JSON.stringify(mode ?? "unknown")}.`,
    );
  }
  return {
    correlation,
...
function readScope(
  ...
  if (hasSession === hasRequest) {
    return "Aether declined an ACP input request with an invalid or ambiguous scope.";
  }
  if (hasSession) {
    const normalizedSessionId = readCorrelationText(sessionId);
    if (!normalizedSessionId) {
      return "Aether declined an ACP input request with an invalid session id.";
    }
    ...
    if (toolCallId !== undefined && toolCallId !== null && !normalizedToolCallId) {
      return "Aether declined an ACP input request with an invalid tool-call id.";
    }
    return {
  ...
  return normalizedRequestId
    ...
    : "Aether declined an ACP input request with an invalid request scope.";
}
