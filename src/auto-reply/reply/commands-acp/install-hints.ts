...
export function resolveAcpInstallCommandHint(cfg: AetherConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `aether plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^aether plugins install /u, "");
    ...
    return "aether plugins install acpx";
  }
