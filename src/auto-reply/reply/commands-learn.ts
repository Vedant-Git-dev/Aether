...
import {
  agentHarnessExposesAetherTools,
  selectAgentHarness,
...
const SKILL_WORKSHOP_UNAVAILABLE_REPLY =
  "Skill workshop is not available on this agent. Use a non-sandboxed agent where the skill_workshop tool is available, or use the aether skills workshop CLI.";
const PERSONAL_WORKSHOP_LEARN_REPLY =
  "This turn cannot stage a pending workspace proposal, so /learn made no change. Ordinary explicit personal skill creation publishes a revision. Ask for that directly if intended, or use the existing administrator UI or aether skills workshop CLI for workspace proposal review.";
...
function resolveWorkshopSurface(
  ...
  try {
    ...
    if (cliProvider) {
      ...
      if (!agentHarnessExposesAetherTools(harness.id)) {
        return undefined;
      }
    }
