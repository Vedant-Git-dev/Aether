...
import { scheduleGatewaySigusr1Restart, triggerAetherRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = defineGatewayControlCommand(
  ...
  async (params) => {
    ...
    if (hasSigusr1Listener) {
      ...
      return sessionCommandReply(
        "⚙️ Restarting Aether in-process (SIGUSR1); back in a few seconds.",
      );
    }
    let sentinelWritten = false;
    try {
    ...
    const restartMethod = triggerAetherRestart();
    if (!restartMethod.ok) {
      if (sentinelWritten) {
        await clearRestartSentinel();
      }
    ...
    return sessionCommandReply(
      `⚙️ Restarting Aether via ${restartMethod.method}; give me a few seconds to come back online.`,
    );
  },
);
