...
import type { HumanMention } from "@aether/gateway-protocol";
import { expectDefined, stableStringify } from "@aether/normalization-core";
...
const FOLLOWUP_DRAIN_CALLBACKS_KEY = Symbol.for("aether.followupDrainCallbacks");
...
const QUEUED_ADMISSION_OWNER_STATE_KEY = Symbol.for("aether.queuedAdmissionOwnerState");
...
function buildCollectTranscriptInput(
  ...
  const text = buildCollectPrompt({
    ...
    renderItem: (item, index) => {
      ...
      for (const mention of message?.["__aether"]?.humanMentions ?? []) {
