...
function followupOwnsNativeCompaction(params: FollowupRuntimeParams, runtimeId: string): boolean {
  ...
  // compaction owner; Aether maintenance would corrupt that runtime state.
  return (
  ...
  visit((event) => {
    ...
    if (scanTaint && message) {
      if (message.role === "user") {
        ...
        const metadata = (message as { __aether?: unknown })["__aether"];
        if (metadata && typeof metadata === "object" && !Array.isArray(metadata)) {
          const aether = metadata as { resultContentSource?: unknown; turnTainted?: unknown };
          if (aether.turnTainted === true || aether.resultContentSource === "network") {
            tainted = true;
            scanTaint = false;
          }
        }
...
export async function runSessionCompactionIfNeeded(params: {
  ...
  if (isCodexRuntime && !shouldCompactByTranscriptBytes) {
    // Codex owns native-thread token pressure; Aether owns the host transcript byte fuse
