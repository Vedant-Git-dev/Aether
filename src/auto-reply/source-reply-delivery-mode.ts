...
export function buildHarnessVisibleReplyGuidance(params: {
  ...
  const deliveryGuidance = messageToolOwnsVisibleReply(params)
    ? params.messageToolAvailable
      ? "Visible source replies are not automatically delivered for this run. Use `message(action=send)` for user-visible source-channel output. For progress, set `final=false`. Set `final=true`, or omit it, for the completed reply to the current source conversation; Aether stops after confirming delivery. Do not repeat visible message content in your final answer."
    ...
    : params.messageToolAvailable
      ? "You can participate in the conversation throughout your work. Use `message` when you have something worth saying; you don’t need to wait until you’re finished, and sending a message doesn’t end your task. Aether delivers your final response automatically."
      : "For the current source conversation, reply normally in your final assistant message; Aether will deliver it through the active source conversation.";
