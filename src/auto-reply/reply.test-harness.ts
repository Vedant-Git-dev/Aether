...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".aether", "agents", "main", "sessions"), { recursive: true });
    const env: Record<string, string | undefined> = {
      HOME: home,
      USERPROFILE: home,
      AETHER_STATE_DIR: path.join(home, ".aether"),
      AETHER_AGENT_DIR: path.join(home, ".aether", "agent"),
    };
