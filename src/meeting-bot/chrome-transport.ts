...
>(
    ...
    if (audioBridge.external) {
      if (params.config.chrome.audioBridgeCommand) {
        if (params.mode === "agent") {
          throw new Error(
            "Chrome agent mode requires chrome.audioInputCommand and chrome.audioOutputCommand so Aether can run STT and regular TTS directly.",
          );
        }
  ...
  async function launchOnNode(params: MeetingChromeLaunchParams<Config, Mode>): Promise<{
    ...
    try {
      ...
      Reflect.set(
        transport,
        Symbol.for("aether.internal.meeting-node-output-generation.v1"),
