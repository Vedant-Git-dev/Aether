...
import type { AetherPluginConfigSchema } from "../plugins/plugin-config-schema.types.js";
import type { AetherPluginNodeInvokePolicy } from "../plugins/plugin-registration.types.js";
...
> = {
  cap: string;
  configSchema: AetherPluginConfigSchema & { parse(value: unknown): Config };
  createNodePolicy(config: Config): AetherPluginNodeInvokePolicy;
  createRuntime(params: { api: AetherPluginApi; config: Config }): Runtime;
  description: string;
  ...
  registerCli(api: AetherPluginApi, config: Config): void;
  registerNodeWhen(config: Config): boolean;
  resolveGatewayTimeoutMs(config: Config): number;
  resolveToolRuntime(
    api: AetherPluginApi,
    agentId: string | undefined,
  ): Promise<AetherPluginApi["runtime"] | undefined>;
