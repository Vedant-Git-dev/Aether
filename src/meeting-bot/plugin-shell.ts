...
import type { AetherPluginApi } from "../plugins/plugin-api.types.js";
import type { AetherPluginCliRootCommandDescriptor } from "../plugins/plugin-registration.types.js";
...
export function createMeetingPluginNodeHostHandler(options: MeetingPluginNodeHostOptions) {
  ...
  return createMeetingConfiguredNodeHost({
    ...
    browser: {
      ...
      openedNotes: [
        `${options.browserPageName} page control is handled by Aether browser automation when using chrome-node.`,
      ],
    },
  });
}
...
> & {
  cli: {
    descriptor: AetherPluginCliRootCommandDescriptor;
  ...
  runtime: new (params: {
    config: Config;
    fullConfig: AetherPluginApi["config"];
    logger: AetherPluginApi["logger"];
    runtime: AetherPluginApi["runtime"];
  }) => Runtime;
...
>(options: MeetingPluginShellEntryOptions<Config, Request, Runtime>) {
  ...
  return createMeetingPluginEntryOptions<Config, Request, Runtime>({
    ...
    toolDescription: `Join and manage ${options.browserGuestLabel} browser guests. Guest admission, tenant sign-in, and media permissions may require manual action in the Aether Chrome profile.`,
