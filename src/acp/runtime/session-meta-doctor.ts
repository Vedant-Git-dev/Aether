...
type DoctorAcpScope = { config: AetherConfig; env: NodeJS.ProcessEnv; pluginId: string };
...
> {
  ...
  try {
    const database = await openExistingAetherStateDatabaseReadOnly({ env: scope.env });
    if (!database) {
...
export function updateAcpSessionIdentityForDoctor(
  ...
  const updated = withAetherAgentDatabaseReadOnly((agentDatabase) => {
    runAetherStateWriteTransaction(
      (database) => {
