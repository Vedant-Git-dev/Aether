/** Interactive stdio ACP client used to connect a terminal session to an Aether ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "aether";
  ...
  try {
    ...
    await client.initialize({
      ...
      clientInfo: { name: "aether-acp-client", version: "1.0.0" },
    });
