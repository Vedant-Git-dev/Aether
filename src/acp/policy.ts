...
export function isAcpEnabledByPolicy(cfg: AetherConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: AetherConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: AetherConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: AetherConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: AetherConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: AetherConfig, agentId: string): boolean {
