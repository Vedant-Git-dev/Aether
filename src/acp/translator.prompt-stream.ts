...
import { readBool, readMetadataString, readNonNegativeInteger } from "@aether/acp-core/meta";
import type { AcpSessionStore } from "@aether/acp-core/session";
import type { AcpServerOptions } from "@aether/acp-core/types";
  ...
  private async finishPrompt(
    ...
    if (options.interruption) {
      ...
      await this.emitPromptChunk(
        pending,
        "agent_message_chunk",
        `[Aether interruption] ${options.interruption}`,
        false,
      );
    }
    ...
    try {
      if (options.recordDisconnectNotice) {
        const text = pending.sendAccepted
          ? "[Aether interruption] The Gateway disconnected after accepting this message, so its final outcome is unknown. Check the session before retrying."
          : "[Aether interruption] The Gateway disconnected before Aether could confirm whether this message was accepted, so its final outcome is unknown. Check the session before retrying.";
