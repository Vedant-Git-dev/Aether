#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the Aether Gateway. */
...
import { closeAetherStateDatabase } from "../state/aether-state-db.js";
  ...
  const gateway = new GatewayClient({
    ...
    onEvent: (evt) => {
      ...
      void agent?.handleGatewayEvent(evt).catch((err: unknown) => {
        process.stderr.write(`aether acp: gateway event ${evt.event} failed\n`);
        if (opts.verbose) {
          process.stderr.write(
            `aether acp: gateway event ${evt.event} error: ${formatErrorMessage(err)}\n`,
          );
        }
      });
    },
...
function printHelp(): void {
  console.log(`Usage: aether acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or AETHER_GATEWAY_TOKEN.",
    );
  }
