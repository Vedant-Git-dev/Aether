...
export function createSqliteAcpEventLedger(
  params: AetherStateDatabaseOptions & AcpLedgerOptions = {},
): AcpEventLedger {
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runAetherStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openAetherStateDatabase(dbOptions).db);
