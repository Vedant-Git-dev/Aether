...
type CollectDangerousConfigFlags = (cfg: AetherConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  if (cfg.mcp?.apps?.enabled === true) {
    findings.push({
      ...
      remediation:
        "Keep this enabled only for MCP servers you trust. Disable with `aether config set mcp.apps.enabled false --strict-json` when it is not needed.",
    });
  }
  ...
  for (const credential of ["token", "password"] as const) {
    ...
    if (isInvalidGatewaySecret(input)) {
      findings.push({
        ...
        remediation:
          credential === "token"
            ? "Run `aether vitals --fix --generate-gateway-token` for an inline token; otherwise rotate its external secret source. Restart the Gateway afterward."
            : "Generate a real secret (for example, `openssl rand -hex 32`) and update AETHER_GATEWAY_PASSWORD or gateway.auth.password (or its external source). Restart the Gateway afterward.",
      });
