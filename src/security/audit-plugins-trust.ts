...
function readChannelCommandSetting(
  ...
  const channelCfg = cfg.channels?.[channelId as keyof NonNullable<AetherConfig["channels"]>];
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = deps.isToolAllowedByPolicies("__aether_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `aether plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }

  const hookInstalls = readHookInstalls({
    env: { ...process.env, AETHER_STATE_DIR: params.stateDir },
  });
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `aether hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
