...
import type { AetherConfig, SecurityConfig } from "../config/types.aether.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "aether" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  if (!config) {
    try {
      ...
      return failClosed(`could not load Aether config (${formatErrorMessage(err)})`);
    }
  }
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    aetherVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
