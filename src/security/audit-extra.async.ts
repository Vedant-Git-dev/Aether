...
import type { AetherConfig, ConfigFileSnapshot } from "../config/config.js";
  ...
  try {
    const result = await withDockerProbeTimeout(params.timeoutMs, (signal) =>
      params.execDockerRawFn(
        ["ps", "-a", "--filter", "label=aether.sandboxBrowser=1", "--format", "{{.Names}}"],
    ...
    const result = await withDockerProbeTimeout(params.timeoutMs, (signal) =>
      params.execDockerRawFn(
        [
          "inspect",
          "-f",
          '{{ index .Config.Labels "aether.configHash" }}\t{{ index .Config.Labels "aether.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("aether sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected aether.browserConfigEpoch=${browserHashEpoch}.`,
      remediation: `${formatCliCommand("aether sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("aether sandbox recreate --browser --all")} (add --force to skip prompt), ` +
...
function buildSandboxBrowserDockerProbeTimeoutFinding(timeoutMs: number): SecurityAuditFinding {
  return {
    ...
    detail:
      ...
      "Aether skipped any remaining sandbox browser container drift checks for this status run.",
    remediation:
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `aether security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    try {
      ...
      findings.push({
        ...
        detail:
          ...
          "The extension entrypoint list is unavailable. Deep scan will cover the plugin directory but may miss entries declared via `aether.extensions`.",
        remediation:
          "Inspect the plugin package.json for syntax errors. If the plugin is untrusted, remove it from your Aether extensions state directory.",
      });
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all aether.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Aether extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  const env = { ...process.env, AETHER_STATE_DIR: params.stateDir };
  const reportWorkshopScanFailure = (filePath: string, error: unknown) => {
    findings.push({
      ...
      remediation:
        "Check file permissions and skill layout, then rerun `aether security audit --deep`.",
    });
  };
  ...
  for (const entry of entries) {
    if (resolveSkillSource(entry.skill) === "aether-bundled") {
      continue;
    }
