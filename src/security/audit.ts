...
import { redactSensitiveUrlLikeString } from "@aether/net-policy/redact-sensitive-url";
...
import type { ConfigFileSnapshot, AetherConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import { copyConfigResolutionFacts } from "../config/resolution-facts.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.aether.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenFallback: Boolean(normalizeOptionalString(params.env.AETHER_GATEWAY_TOKEN)),
    hasPasswordFallback: Boolean(normalizeOptionalString(params.env.AETHER_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: AetherConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Aether state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Aether state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectElevatedFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectAgentRosterFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const agents = listAgentEntries(cfg);
  ...
  return [
    {
      ...
      remediation: "Run `aether vitals --fix` to repair the authored agent roster.",
    },
  ];
}
...
function listConfiguredMcpServerNames(cfg: AetherConfig): string[] {
...
function collectAgentSkillMcpBoundaryScopes(cfg: AetherConfig): AgentSkillMcpBoundaryScope[] {
  const agents = listAgentEntries(cfg);
  ...
  findings.push({
    ...
    detail:
      ...
      `\nMCP server registries visible to the gateway configuration/state:\n${sources
        ...
        .join("\n")}\n` +
      "agents.*.skills filters Aether skill visibility and snapshots; it is not a shell-time authorization boundary. " +
...
function collectOpenExecSurfacePaths(cfg: AetherConfig): string[] {
