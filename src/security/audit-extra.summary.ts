...
function summarizeGroupPolicy(cfg: AetherConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AetherConfig): boolean {
...
function isBrowserEnabled(cfg: AetherConfig): boolean {
...
export function collectAttackSurfaceSummaryFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  ...
  const detail =
    ...
    `\n` +
    "trust model: personal assistant (one trusted operator boundary), not hostile multi-tenant on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.aether.ai/gateway/multi-tenant-hosting";
