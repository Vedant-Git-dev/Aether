...
function isGatewayRemotelyExposed(cfg: AetherConfig): boolean {
...
function formatHooksTokenReuseRemediation(reuse: GatewayAuthSharedSecretReuse): string {
  ...
  return `Run ${formatCliCommand("aether vitals --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
function listKnownNodeCommands(cfg: AetherConfig): Set<string> {
  const baseCfg: AetherConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function listOpenInboundPolicies(cfg: AetherConfig): string[] {
  const out: string[] = [];
...
export function listPotentialMultiUserSignals(cfg: AetherConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function listAuditAgentToolContexts(cfg: AetherConfig): AuditAgentToolContext[] {
...
function collectRiskyToolExposureContexts(cfg: AetherConfig): {
...
function collectControlPlaneToolExposureContexts(cfg: AetherConfig): string[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AETHER_STATE_DIR on a local-only volume and re-run "${formatCliCommand("aether security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (password && !hasUnresolvedConfigPath(cfg, "gateway.auth.password")) {
    findings.push({
      ...
      remediation:
        "Prefer AETHER_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-aether-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = listAgentEntries(cfg);
...
export function collectNodeDenyCommandPatternFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: AetherConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (mainGroupScopes.length > 0) {
    findings.push({
      ...
      remediation:
        'Use session.groupScope="per-group" globally and remove binding overrides, or reserve "main" for rooms whose members you trust: https://docs.aether.ai/channels/groups#session-keys',
    });
  }
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "Aether's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.aether.ai/gateway/multi-tenant-hosting",
    remediation:
