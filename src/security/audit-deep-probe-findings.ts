...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("aether status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("aether security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set AETHER_GATEWAY_TOKEN/AETHER_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("aether security audit --deep")}".`,
    });
  }
  return findings;
}
