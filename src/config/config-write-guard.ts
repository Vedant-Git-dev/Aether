...
const NIX_AETHER_AGENT_FIRST_URL = "https://github.com/aether/nix-aether#quick-start";
/** Public Aether Nix overview shown with immutable-config errors. */
const NIX_OVERVIEW_URL = "https://docs.aether.ai/install/nix";

/** Error thrown when external management disables config mutation. */
export class ConfigReadOnlyError extends Error {
  readonly code = "AETHER_CONFIG_READONLY";

  constructor(params: { configPath?: string } = {}) {
    super(
      [
        "Config is externally managed (`AETHER_CONFIG_READONLY=1`), so Aether treats aether.json as immutable.",
        ...(params.configPath ? [`Config path: ${params.configPath}`] : []),
        "Edit the config in your external deployment source, then redeploy or restart Aether as needed.",
      ].join("\n"),
    );
...
export class NixModeConfigMutationError extends Error {
  readonly code = "AETHER_NIX_MODE_CONFIG_IMMUTABLE";
...
function formatNixModeConfigMutationMessage(params: { configPath?: string } = {}): string {
  return [
    ...
    "This usually means nix-aether, the first-party Nix distribution, or another Nix-managed package set this mode.",
    ...(params.configPath ? [`Config path: ${params.configPath}`] : []),
    ...
    "Edit the Nix source for this install instead. For nix-aether, edit `programs.aether.config` or `instances.<name>.config`, then rebuild with Home Manager or NixOS.",
    `Agent-first Nix setup: ${NIX_AETHER_AGENT_FIRST_URL}`,
    `Aether Nix overview: ${NIX_OVERVIEW_URL}`,
  ].join("\n");
}
