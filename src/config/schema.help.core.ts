...
export const CORE_FIELD_HELP: Record<string, string> = {
  ...
  "meta.lastTouchedVersion": "Aether version that most recently wrote this config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Aether. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  secrets:
  ...
  "wizard.lastRunVersion": "Aether version used by the last wizard run.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Aether runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  "update.channel":
  ...
  "update.checkOnStart":
    "Checks the Aether update endpoint when the gateway starts, including read-only extended-stable hints (default: true). Set false to disable update checks and anonymous update pings.",
  "update.auto.enabled":
  ...
  "gateway.auth.trustedProxy.deviceAutoApprove":
    "Optional policy for automatically approving new browser and native UI operator devices and same-key scope upgrades after trusted-proxy authentication. Grants are capped by deviceAutoApprove.scopes and the proxy's x-aether-scopes header when present.",
  ...
  "gateway.tailscale.preserveFunnel":
    "Deprecated migration guard for mode='serve'. If an external Funnel still targets the ordinary Gateway listener, Aether leaves exposure unchanged and warns that only plugin-authenticated webhooks remain usable until password auth is configured and mode is migrated to 'funnel'.",
  "gateway.remote":
  ...
  "talk.realtime.instructions":
    "Additional system instructions appended to Aether's built-in realtime Talk prompt. Use this for voice style, tone, and other provider-facing realtime behavior while keeping agent-consult guidance intact.",
  ...
  "talk.realtime.consultRouting":
    "Gateway relay fallback for final user transcripts when the realtime provider skips aether_agent_consult. provider-direct preserves provider replies; force-agent-consult routes through Aether.",
  ...
  "talk.speechLocale":
    'BCP 47 locale id for Talk speech recognition on device nodes and the system-voice fallback, for example "ru-RU". Leave unset to use each device default.',
  ...
  "agents.ownership":
    'Durable multi-agent ownership generation marker. "explicit" means ambient channels, heartbeat, system-agent consults, Talk, cron, and bare CLI operations must resolve a surface-specific owner or fail closed. Aether stamps this automatically when creating or migrating a fleet; omit it for a sole agent.',
  "agents.defaults":
  ...
  "agents.entries.*.runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Aether execution or acp for external ACP harness defaults.",
  "agents.entries.*.runtime.type":
    'Runtime type for this agent: "embedded" (default Aether runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.entries.*.runtime.acp.agent":
    "Optional ACP harness agent id to use for this Aether agent (for example codex, claude, cursor, gemini, aether).",
  ...
  "agents.defaults.systemAgent":
    "Target settings for ambient Aether system-agent and Custodian inference plus selected unscoped operator reads.",
