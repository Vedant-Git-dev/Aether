// Defines the top-level Aether configuration type.
...
/** Top-level Aether config as read from user/project config files. */
export type AetherConfig = {
  ...
  $schema?: string;
  meta?: {
    /** Last Aether version that wrote this config. */
  ...
  ui?: {
    /** Accent color for Aether UI chrome (hex). */
    seamColor?: string;
    /**
...
export type AetherConfigInput = Omit<AetherConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const aetherConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = AetherConfig & {
  readonly [aetherConfigStateBrand]?: TState;
};
