...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.AETHER_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.AETHER_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.AETHER_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `aether vitals --fix` once the
