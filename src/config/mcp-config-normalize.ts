...
import { normalizeLowercaseStringOrEmpty as normalizeMcpString } from "@aether/normalization-core/string-coerce";
import { isRecord } from "../utils.js";

type ConfigMcpServers = Record<string, Record<string, unknown>>;
type AetherMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_AETHER_TRANSPORT: Record<string, AetherMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to Aether HTTP transport names. */
export function resolveAetherMcpTransportAlias(
  value: unknown,
): AetherMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_AETHER_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias Aether can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_AETHER_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveAetherMcpTransportAlias(next.type);
  // `transport` is Aether's canonical field; legacy `type` only fills a gap.
