...
function collectReferencedAgentIds(cfg: AetherConfig): string[] {
...
export function formatDuplicateAgentDirError(dups: DuplicateAgentDir[]): string {
  const lines: string[] = [
    ...
    "Auth profiles live in each agent's SQLite store, so a shared agentDir is not how credentials are shared: give each agent its own directory and either leave its store empty to inherit the main agent's profiles, or log it in with `aether models auth login`.",
  ];
  return lines.join("\n");
}
