...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Aether agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Aether, or the agent id plus Aether. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Aether agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Aether conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Aether Discord conversation. */
