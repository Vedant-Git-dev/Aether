...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "AETHER_INCLUDE_ROOTS" ||
    // Config cannot opt into or out of the host-selected read-only mode.
    key.toUpperCase() === "AETHER_CONFIG_READONLY" ||
...
function collectConfigEnvVarsByTarget(cfg?: AetherConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AetherConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: AetherConfig): Record<string, string> {
...
type PublishedConfigRuntimeEnvState = {
  ...
  sourceConfig: AetherConfig | null;
};
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.AETHER_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
