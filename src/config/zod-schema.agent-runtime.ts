...
const BLOCKED_WEB_SEARCH_KEYS_ISSUE_FIELD = "__aetherBlockedWebSearchKeys";
...
function addExecPolicyModeConflictIssue(
  ...
  ctx.addIssue({
    code: z.ZodIssueCode.custom,
    path: ["mode"],
    message: `mode cannot be combined with security or ask in the same exec object. Update the deploy script, template, or patch at this scope. ${repair} Doctor migrates supported legacy policies to mode; run "aether vitals --fix" only when the saved file still needs migration.`,
  });
}
