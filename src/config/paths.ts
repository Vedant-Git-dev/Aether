...
/**
 * Nix mode detection: When AETHER_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsConfigReadOnly(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.AETHER_CONFIG_READONLY === "1" || resolveIsNixMode(env);
}
const CONFIG_FILENAME = "aether.json";
const LEGACY_CONFIG_FILENAMES = ["aether.json"] as const;
...
/** Build a homedir thunk that respects AETHER_HOME for the given env. */
...
export function isDefaultStateDir(
  ...
  const override = env.AETHER_STATE_DIR?.trim();
  if (!override) {
...
export function isDefaultInstallIdentity(
  ...
  // AETHER_HOME isolated so an alternate state tree cannot adopt that service.
  if (env.AETHER_HOME?.trim()) {
    return false;
  }
  if (
  ...
  try {
    canonicalStateDir = resolveProfileStateDir(env.AETHER_PROFILE ?? "default", env, homedir);
  } catch {
  ...
  if (!isNamedProfile(env) && !env.AETHER_CONFIG_PATH?.trim()) {
    return true;
  }
  return (
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const aetherOverride = env.AETHER_STATE_DIR?.trim();
  if (aetherOverride) {
    env.AETHER_STATE_DIR = resolveUserPath(aetherOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `AETHER_INCLUDE_ROOTS` as a
 ...
 * directory containing `aether.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.AETHER_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via AETHER_CONFIG_PATH.
 * Default: ~/.aether/aether.json (or $AETHER_STATE_DIR/aether.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir?: string,
): string {
  const override = env.AETHER_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.AETHER_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.AETHER_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const aetherStateDir = env.AETHER_STATE_DIR?.trim();
  if (aetherStateDir) {
    const resolved = resolveUserPath(aetherStateDir, env, effectiveHomedir);
 ...
 * Default: $AETHER_STATE_DIR/tmp/aether-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(
  ...
  const suffix = uid != null ? `aether-${uid}` : "aether";
...
export function resolveOAuthDir(
  ...
  const override = env.AETHER_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.AETHER_GATEWAY_PORT?.trim();
  ...
  const profile = normalizeProfileName(env.AETHER_PROFILE);
  if (!profile) {
    return DEFAULT_GATEWAY_PORT;
  }
  // Keep byte-for-byte aligned with AppProfile.defaultGatewayPort in
  // apps/macos/Sources/Aether/AppProfile.swift so both surfaces connect to the same Gateway.
