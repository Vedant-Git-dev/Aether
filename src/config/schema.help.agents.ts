...
export const AGENT_FIELD_HELP: Record<string, string> = {
  ...
  "agents.entries.*.models.*.agentRuntime.id":
    'Per-agent model runtime id: "aether", "auto", a registered plugin harness id such as "codex", or a supported CLI backend alias such as "claude-cli".',
  "agents.entries.*.models.*.codeMode":
    "Aether Code Mode for this agent and exact provider/model: On forces it on, Off disables it, and Default inherits the agent activation setting, then the shared model override, then tools.codeMode.enabled. This does not change the selected runtime or Codex native Code Mode.",
  ...
  "agents.defaults.compaction.enabled":
    "Enable embedded proactive auto-compaction (default: true). Set false to stop threshold-driven embedded compaction while preserving Aether overflow recovery, preflight compaction, and manual /compact.",
  ...
  "agents.defaults.compaction.midTurnPrecheck":
    "Optional embedded Aether tool-loop precheck that detects context pressure after a tool result is appended and before the next model call. When enabled, Aether reuses existing precheck recovery to truncate tool results or compact before retrying.",
  "agents.defaults.compaction.midTurnPrecheck.enabled":
    "Enable structured mid-turn context pressure checks for embedded Aether tool loops. Default: false. Keep disabled unless long tool-heavy sessions hit context overflow before normal turn-end compaction can run.",
  ...
  "agents.defaults.embeddedAgent":
    "Embedded Aether runner hardening controls for how workspace-local agent settings are trusted and applied in Aether sessions.",
  "agents.defaults.embeddedAgent.projectSettingsPolicy":
    'How embedded Aether handles workspace-local `.aether/settings.json`: "sanitize" (default) strips shellPath/shellCommandPrefix, "ignore" disables project settings entirely, and "trusted" applies project settings as-is.',
  "agents.defaults.embeddedAgent.executionContract":
    'Embedded Aether execution contract: "default" keeps the standard runner behavior, while "strict-agentic" enables structured plan tracking and non-visible turn recovery for supported OpenAI/OpenAI Codex GPT-5-family runs.',
  "agents.entries.*.embeddedAgent":
    "Optional per-agent embedded Aether overrides. Use this to opt specific agents into stricter GPT-5 execution behavior without changing the global default.",
  "agents.entries.*.embeddedAgent.executionContract":
    'Optional per-agent embedded Aether execution contract override. Set "strict-agentic" to enable structured plan tracking and non-visible turn recovery for that agent on supported OpenAI/OpenAI Codex GPT-5-family runs, or "default" to inherit the standard runner behavior.',
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Aether MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Aether. Embedded Aether and other runtime adapters can consume these servers without storing them inside runtime-owned project settings.",
  "mcp.sessionIdleTtlMs":
    "Optional idle eviction for Aether session MCP runtimes in milliseconds. Unset or 0 keeps runtimes alive until session cleanup or Gateway shutdown. Positive values round down; active leases and pending acquisitions prevent eviction. Run-owned runtimes still retire when their one-shot run ends.",
  "mcp.servers":
    "Named MCP server definitions. Aether stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  "mcp.servers.*.codex":
    "Aether projection metadata for Codex app-server threads only. It does not affect ACP sessions or generic Codex harness config. Omit this block to keep the server available to every Codex app-server agent with Codex's default MCP approval behavior.",
  ...
  "mcp.servers.*.oauth.authProfileId":
    "Refresh-capable auth profile id used to inject the current bearer token into this remote MCP server. When set, Aether resolves and refreshes the profile at runtime and does not project refresh material downstream.",
  "mcp.servers.*.codex.agents":
    "Optional non-empty Aether agent ids that should receive this MCP server in Codex app-server thread config. Empty, blank, or invalid lists fail closed; when omitted, the server is projected for all Codex app-server agents.",
  ...
  "mcp.servers.*.codex.default_tools_approval_mode":
    "Codex-native spelling for the same per-server MCP tool approval mode. Prefer defaultToolsApprovalMode in Aether config.",
};
