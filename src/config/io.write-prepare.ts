...
import { asOptionalRecord, readStringField } from "@aether/normalization-core/record-coerce";
import {
...
function shouldPersistCanonicalAgentRoster(params: {
  ...
  const runtimeRoster = toAgentEntriesRecord(
    listAgentEntries(params.runtimeConfig as AetherConfig),
  );
  const sourceRoster = toAgentEntriesRecord(
    listAgentEntries(params.sourceConfig as AetherConfig),
  );
  const nextRoster = toAgentEntriesRecord(listAgentEntries(params.nextConfig as AetherConfig));
  return (
...
function assertCanonicalAgentRosterRetainsEntries(params: {
  ...
  const canonicalIds = new Set(
    listAgentEntries(params.canonicalConfig as AetherConfig).map((entry) =>
  ...
  const currentEntries =
    currentRoster?.kind === "list" && Array.isArray(currentRoster.value)
      ...
      : listAgentEntries(params.currentConfig as AetherConfig);
...
function indexAgentRosterForWrite(config: unknown, legacyIdsByIndex: ReadonlyMap<number, string>) {
  ...
  if (roster?.kind !== "list" || !Array.isArray(roster.value)) {
    return toAgentEntriesRecord(listAgentEntries(config as AetherConfig)) as Record<
      string,
      unknown
    >;
  }
...
function canonicalizeAgentRosterForExplicitWrite(params: {
  ...
  const nextEntries = toAgentEntriesRecord(
    listAgentEntries(params.nextConfig as AetherConfig),
  ...
  const explicitEntries =
    explicitRoster?.kind === "list" && Array.isArray(explicitRoster.value)
      ...
      : (toAgentEntriesRecord(listAgentEntries(params.valueSource as AetherConfig)) as Record<
          string,
          unknown
        >);
  ...
  const includeProjectionRootAuthoredConfig =
    persistCanonicalRoster && !hasAgentRosterProperty(projectedAuthoredRoster)
      ? setPathValueCreatingParents(
          ...
          ["agents", "entries"],
          toAgentEntriesRecord(listAgentEntries(params.sourceConfig as AetherConfig)),
        )
      : projectedAuthoredRoster;
