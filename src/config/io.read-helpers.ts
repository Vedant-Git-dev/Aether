...
import { asOptionalRecord, isRecord } from "@aether/normalization-core/record-coerce";
...
export function coerceConfig(value: unknown): AetherConfig {
  return (asOptionalRecord(value) ?? {}) as AetherConfig;
}
...
export function normalizeConfigIoDeps(overrides: ConfigIoDeps = {}): NormalizedConfigIoDeps {
  ...
  return {
    ...
    suppressFutureVersionWarning:
      ...
      (isTruthyEnvValue(env.AETHER_UPDATE_IN_PROGRESS) ||
        isTruthyEnvValue(env.AETHER_UPDATE_POST_CORE)),
...
export function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AetherConfig, env, { lowerPrecedenceEnv });
  }
