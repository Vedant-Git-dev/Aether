...
export function withSqliteSessionDatabase<T>(
  ...
  const admittedOperation = finishAdmission
    ...
    : operation;
  try {
    if (getAetherAgentDatabaseIfOpen(options)) {
      if (diagnostics) {
        diagnostics.admissionMode = "cached";
      }
      return admittedOperation(openAetherAgentDatabase(options));
    }
    if (diagnostics) {
      diagnostics.admissionMode = "async";
    }
    // The caller keeps its FIFO section while the existing owner joins the integrity child.
    const result = withAetherAgentDatabaseAsync(options, admittedOperation, assertCurrent);
...
export async function runExclusiveSqliteSessionWrite<T>(
  ...
  const storePath = resolveAetherAgentSqlitePath(databaseOptions);
...
export function resolveSqliteScope(
  ...
  const effectiveStorePath = incognitoAgentId
    ? resolveIncognitoAetherAgentSqlitePath({ agentId: incognitoAgentId, env: scope.env })
    : scope.storePath;
...
export function resolveSqliteTranscriptArchiveDirectory(
  ...
  const databasePath = resolveAetherAgentSqlitePath(toDatabaseOptions(scope));
...
export function readSqliteTranscriptStoreBatches<T>(
  ...
  readChunk: (
    database: Pick<AetherAgentDatabase, "db" | "path">,
  ...
  const groups = new Map<
    string,
    { indexes: Map<string, number[]>; options: AetherAgentDatabaseOptions }
  >();
  ...
  for (const [index, scope] of scopes.entries()) {
    ...
    const databasePath = resolveAetherAgentSqlitePath(options);
  ...
  for (const group of groups.values()) {
    withAetherAgentDatabaseReadOnly(
      (database) => {
...
export function formatLegacySqliteSessionMarkerForScope(scope: ResolvedTranscriptScope): string {
  return formatSqliteSessionFileMarker({
    agentId: scope.agentId,
    sessionId: scope.sessionId,
    storePath: scope.path ?? resolveAetherAgentSqlitePath(toDatabaseOptions(scope)),
  });
}
