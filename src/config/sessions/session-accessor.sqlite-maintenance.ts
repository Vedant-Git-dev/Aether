...
export async function refreshSqliteSessionPlannerStatisticsBestEffort(
  ...
  const storePath = resolveAetherAgentSqlitePath(toDatabaseOptions(scope));
  ...
  const completion = runExclusiveSqliteSessionWrite(
    ...
    async () => {
      ...
      const database = openAetherAgentDatabase(toDatabaseOptions(scope));
...
async function readSessionTranscriptJsonlBytes(
  ...
  for (let offset = 0; offset < sessionIds.length; offset += SESSION_TRANSCRIPT_BYTE_QUERY_BATCH) {
    ...
    const opened = withAetherAgentDatabaseReadOnly((database) => {
