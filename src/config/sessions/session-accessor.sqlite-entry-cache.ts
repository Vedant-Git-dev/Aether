...
import {
  deferAetherAgentPostCommitPublication,
...
type SessionEntryCacheDatabase = Pick<AetherAgentDatabase, "agentId" | "db">;
...
function ensureSessionNodesGenerationTracker(database: DatabaseSync): void {
  ...
  database.exec(`
    CREATE TEMP TABLE IF NOT EXISTS aether_session_nodes_cache_generation (id INTEGER NOT NULL PRIMARY KEY CHECK (id = 1), generation INTEGER NOT NULL) STRICT;
    INSERT OR IGNORE INTO aether_session_nodes_cache_generation (id, generation) VALUES (1, 0);
    ${trackedSchemaVersion === undefined ? "" : "UPDATE aether_session_nodes_cache_generation SET generation = generation + 1 WHERE id = 1;"}
    DROP TRIGGER IF EXISTS aether_session_nodes_cache_generation_insert;
    DROP TRIGGER IF EXISTS aether_session_nodes_cache_generation_update;
    DROP TRIGGER IF EXISTS aether_session_nodes_cache_generation_delete;
    CREATE TEMP TRIGGER aether_session_nodes_cache_generation_insert
      AFTER INSERT ON main.session_nodes BEGIN UPDATE aether_session_nodes_cache_generation SET generation = generation + 1 WHERE id = 1; END;
    CREATE TEMP TRIGGER aether_session_nodes_cache_generation_update
      AFTER UPDATE ON main.session_nodes BEGIN UPDATE aether_session_nodes_cache_generation SET generation = generation + 1 WHERE id = 1; END;
    CREATE TEMP TRIGGER aether_session_nodes_cache_generation_delete
      AFTER DELETE ON main.session_nodes BEGIN UPDATE aether_session_nodes_cache_generation SET generation = generation + 1 WHERE id = 1; END;
  `);
...
function readSessionNodesGeneration(database: DatabaseSync): number {
  ...
  const row = database
    .prepare("SELECT generation FROM temp.aether_session_nodes_cache_generation WHERE id = 1")
...
function publishTrackedCacheUpdate(database: AetherAgentDatabase, publish: () => void): void {
  if (deferAetherAgentPostCommitPublication(database, publish)) {
    return;
  }
  if (database.db.isTransaction) {
    throw new Error(
      "SQLite session entry writes must use runAetherAgentWriteTransaction for cache publication",
    );
  }
  publish();
}
