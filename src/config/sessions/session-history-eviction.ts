...
export async function inspectSqliteSessionHistoryDiskBudget(
  ...
  params.env.AETHER_STATE_DIR = resolveStateDir(params.env);
...
function readHistoricalSessionIds(params: {
  ...
  const database = openAetherAgentDatabase(params.databaseOptions);
...
export function kickSessionHistoryDiskBudgetMaintenance(input: {
  ...
  if (
    input.agentId &&
    isIncognitoAetherAgentSqlitePath(input.storePath, {
