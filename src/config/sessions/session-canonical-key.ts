...
} from "../../state/aether-agent-db-contract.js";
...
const SESSION_CANONICAL_KEY_REPAIR_COMMAND = "aether vitals --fix";
...
export function isCanonicalSqliteSessionMainKeyCurrent(
  ...
  const result = withAetherAgentDatabaseReadOnly((database) => {
    ...
    if (schema?.schema_version !== AETHER_AGENT_SCHEMA_VERSION) {
      return false;
    }
    return (
      executeSqliteQueryTakeFirstSync(
