...
import {
  ...
  AETHER_TRANSCRIPT_ARTIFACT_API,
  AETHER_TRANSCRIPT_ARTIFACT_PROVIDER,
  isTranscriptOnlyAetherAssistantMessage,
} from "../../shared/transcript-only-aether-assistant.js";
...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyAetherAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyAetherAssistant &&
    isTranscriptOnlyAetherAssistantMessage(message)
  ) {
    return undefined;
  }
...
function parseRecentConversationText(
  ...
  const deliveryMirror = message.aetherDeliveryMirror;
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyAetherAssistant: true,
      });
      if (assistantText) {
...
export async function appendAssistantMessageToSessionTranscript(params: {
  ...
  return appendExactAssistantMessageToSessionTranscript({
    ...
    message: {
      ...
      api: AETHER_TRANSCRIPT_ARTIFACT_API,
      provider: AETHER_TRANSCRIPT_ARTIFACT_PROVIDER,
      model: AETHER_DELIVERY_MIRROR_MODEL,
      usage: {
        input: 0,
        output: 0,
        cacheRead: 0,
      ...
      timestamp: Date.now(),
      ...(params.deliveryMirror ? { aetherDeliveryMirror: params.deliveryMirror } : {}),
    },
  });
}
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return (
    message.provider === AETHER_TRANSCRIPT_ARTIFACT_PROVIDER &&
    message.model === AETHER_DELIVERY_MIRROR_MODEL
  );
}
...
function isIdentifiedDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  const marker = (message as { aetherDeliveryMirror?: InternalSessionTranscriptDeliveryMirror })
    .aetherDeliveryMirror;
  return (
