...
export function recordAssistantManagedMediaUrls<T extends AssistantDirectiveMessage>(
  ...
  if (message.role === "assistant" && mediaUrls.length > 0) {
    Object.assign(message, {
      aetherDelivery: {
        ...(isRecord(message.aetherDelivery) ? message.aetherDelivery : {}),
        mediaUrls,
      },
    });
  }
...
// whole applier (plus its parser imports) can be deleted; aetherDelivery facts
...
export function applyAssistantDeliveryDirectives<T extends AssistantDirectiveMessage>(
  ...
  if (facts) {
    const currentFacts = isRecord(message.aetherDelivery) ? message.aetherDelivery : undefined;
    ...
    Object.assign(message, { aetherDelivery: mergedFacts });
  }
