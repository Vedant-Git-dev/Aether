...
import type { AetherAgentDatabase } from "../../state/aether-agent-db.js";
import type {
...
export function samePhysicalStore(left: PhysicalStore, right: PhysicalStore): boolean {
  return isSameAetherAgentDatabasePath(left.path, right.path);
}
...
export function warningForDivergence(
  ...
  return `session: ${kind} for ${canonicalKey}; preserved claims ${claimsText}. Run aether vitals --fix to quarantine the losing claims.`;
}
...
function mutateLegacySessionClaims<T>(
  ...
  commit: (database: AetherAgentDatabase) => T,
): Promise<T> {
  const scope = {
...
async function copyClaimCrossStore(params: {
  ...
  const destination = withAetherAgentDatabaseReadOnly(
...
export async function processIdenticalClaims(params: {
  ...
  if (!canonical) {
    ...
    const sourceAfter = withAetherAgentDatabaseReadOnly(
...
export async function repairDivergentClaims(params: {
  ...
  const canonicalResult = withAetherAgentDatabaseReadOnly(
    (database) => readClaim(database, params.destination, params.canonicalKey, params.canonicalKey),
    {
      agentId: params.destination.databaseAgentId,
      env: params.env,
