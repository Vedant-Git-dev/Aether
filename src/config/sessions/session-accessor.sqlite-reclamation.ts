...
import { readAetherAgentDatabaseIdentity } from "../../state/aether-agent-db-identity.js";
import { retainAetherAgentDatabaseReadOnly } from "../../state/aether-agent-db-readonly.js";
...
import {
  ...
  runAetherAgentWriteTransaction,
...
} from "../../state/aether-state-db.paths.js";
import {
...
type ReclamationDatabaseOptions = AetherAgentDatabaseOptions & {
...
function toWorkerDatabaseOptions(
  ...
  const sharedStatePath = options.database?.path ?? resolveAetherStateSqlitePath(sourceEnv);
  return {
    agentId: options.agentId,
    env: { AETHER_STATE_DIR: resolveAetherStateDirForDatabasePath(sharedStatePath) },
    path: resolveAetherAgentSqlitePath(options),
  };
}
...
export function reclaimSqliteSessionInTransaction(
  ...
  callbacks: {
    beforeMutation?: () => void;
    onCommit?: (database: AetherAgentDatabase) => void;
  } = {},
  ...
  const value = runAetherAgentWriteTransaction((transactionDb) => {
    callbacks.beforeMutation?.();
...
function reclaimSqliteFreePagesBestEffort(databaseOptions: ReclamationDatabaseOptions): void {
  try {
    const database = openAetherAgentDatabase(databaseOptions);
...
export async function runSqliteSessionReclamation(params: {
  ...
  onInProcessCommit?: (database: AetherAgentDatabase) => void;
  plan: SqliteSessionReclamationPlan;
  ...
  if (
    ...
    isIncognitoAetherAgentSqlitePath(params.plan.databaseOptions.path, {
      ...
      async () => {
        ...
        return reclaimSqliteSessionInTransaction(params.plan, {
          ...
          onCommit: (database) => {
            ...
            if (publish) {
              deferAetherAgentPostCommitPublication(database, publish);
            }
  ...
  const retained = await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      params.assertCommitAllowed?.();
      return retainAetherAgentDatabaseReadOnly(params.plan.databaseOptions);
    },
  ...
  try {
    ...
    const plan = {
      ...
      databaseOptions: {
        ...params.plan.databaseOptions,
        path: readAetherAgentDatabaseIdentity(claim.database).filename,
      },
    };
    ...
    if (workerResult.cleanupIncomplete) {
      reclamationLog.error(
        "SQLite session reclamation committed but Worker cleanup is incomplete",
        {
          ...
          path: params.plan.databaseOptions.path,
          recovery: "restart Aether before deleting the owning agent",
        },
      );
...
export function createSessionEntryReclamationPlan(params: {
  databaseOptions: AetherAgentDatabaseOptions;
