...
import {
  ...
  withAetherAgentDatabaseAdmission,
  type AetherAgentDatabaseWorkerCloseResult,
  type AetherAgentDatabaseWriteAdmission,
...
async function settleReclamationDatabase(
  ...
  let outcome: AetherAgentDatabaseWorkerCloseResult = { errors: [], settled: false };
  for (let attempt = 0; attempt < WORKER_CLOSE_MAX_ATTEMPTS; attempt += 1) {
    outcome = settleAetherAgentDatabaseWorkerClose(pathname);
...
export async function materializeTranscriptArchiveInWorker(
  ...
  try {
    const opened = withAetherAgentDatabaseReadOnly(
      (database) => {
...
async function runReclamationWorkerPort(
  ...
  const withAdmission: AetherAgentDatabaseWriteAdmission = async (run) => {
  ...
  try {
    result = await withAetherAgentDatabaseAdmission(
    ...
    if (cleanup.settled) {
      ...
      throw new AggregateError(
        ...
        "SQLite session reclamation failed and Worker cleanup is incomplete; restart Aether before deleting the owning agent",
        { cause: error },
      );
    }
    throw error;
