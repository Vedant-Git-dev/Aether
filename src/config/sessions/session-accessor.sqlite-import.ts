...
export async function importSqliteSessionRowsBatch(
  ...
  const databasePath = resolveAetherAgentSqlitePath(toDatabaseOptions(resolved));
  if (
    prepared.some(
      (row) => resolveAetherAgentSqlitePath(toDatabaseOptions(row.resolved)) !== databasePath,
    )
  ) {
