...
export async function applySessionEntryLifecycleMutation(params: {
  ...
  afterUpsertsInTransaction?: (database: AetherAgentDatabase) => void;
  ...
  const afterCount = readSessionEntryCount(openAetherAgentDatabase(toDatabaseOptions(resolved)));
    ...
    async () =>
      await runExclusiveSqliteSessionWrite(
        ...
        async () => {
          let archivedTranscripts: SessionLifecycleArchivedTranscript[] = [];
          const maintenancePlans: SessionEntryMaintenancePlan[] = [];
          const publishRemovals = runAetherAgentWriteTransaction((transactionDb) => {
