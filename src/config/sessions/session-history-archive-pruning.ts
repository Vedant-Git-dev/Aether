...
export function hasCanonicalSessionTranscriptArchives(
  ...
  // aether-agent-db.ts cache rule: LRU eviction closes idle handles across awaits.
  return hasCanonicalSessionTranscriptArchivesInDatabase(
    openAetherAgentDatabase(databaseOptions),
  );
}

function hasCanonicalSessionTranscriptArchivesInDatabase(database: AetherAgentDatabase): boolean {
