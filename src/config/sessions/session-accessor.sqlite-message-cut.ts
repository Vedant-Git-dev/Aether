...
async function mutateSqliteSessionAtMessage(
  ...
  const preparedEntry = readSessionEntryRow(
    openAetherAgentDatabase(toDatabaseOptions(resolved)),
    sourceKey,
  )?.entry;
  ...
  return await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      ...
      const { databasePath, result, publish } = runAetherAgentWriteTransaction((database) => {
        params.commitGuard?.();
...
function extractEditorMediaRefs(
  ...
  const media = asRecord(message["__aether"])?.media;
