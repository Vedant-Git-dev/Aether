...
export async function withSqliteSessionDeletions<T>(
  ...
  const repositories = createSessionRepositoryWorkspaceStore({
    database: openAetherStateDatabase({ env: scope.env }),
  });
      ...
      async () => {
        try {
          ...
          for (const workspace of repositoryWorkspaces) {
            const currentEntry = () =>
              readSessionEntryRow(
                openAetherAgentDatabase(toDatabaseOptions(scope)),
...
export function runSqliteSessionDeletionTransaction<T>(
  ...
  options: Parameters<typeof runAetherAgentWriteTransaction>[1],
  transactionOptions?: Parameters<typeof runAetherAgentWriteTransaction>[2],
): T {
  if (!deletions.getStore() || transactionMutations.getStore()) {
    return runAetherAgentWriteTransaction(operation, options, transactionOptions);
  }
