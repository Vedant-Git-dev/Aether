...
import { ensureAetherAgentProgressCardSchemaInTransaction } from "../../state/aether-agent-progress-card-schema.js";
import { ensureSessionParticipantsSchema } from "../../state/aether-agent-session-participants-schema.js";
import {
...
export function copySessionNodeArtifactsForRepair(
  source: AetherAgentDatabase,
  destination: AetherAgentDatabase,
  ...
  if (sourceTables.has("session_progress_cards")) {
    ...
    if (progressCards.length > 0 && !destinationTables.has("session_progress_cards")) {
      ensureAetherAgentProgressCardSchemaInTransaction(destination.db);
      destinationTables = readSessionNodeArtifactTables(destination);
    }
...
function readSessionNodeArtifactTables(database: AetherAgentDatabase): Set<string> {
  const db = getSessionKysely(database.db);
  return new Set(
    executeSqliteQuerySync(
      database.db,
