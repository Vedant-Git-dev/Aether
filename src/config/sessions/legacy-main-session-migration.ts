...
import { isSameAetherAgentDatabasePath } from "../../state/aether-agent-db-registry.js";
...
type LedgerDatabase = Pick<AetherStateKyselyDatabase, "migration_runs" | "migration_sources">;
...
function resolveArmingDecision(cfg: AetherConfig, legacyAgentId: string): ArmingDecision {
...
async function migrateLegacyMainSessionKeysInternal(
  ...
  if (!arming.armed) {
    ...
    return {
      ...
      warnings: unresolved
        ? [
            `session: legacy ${legacyAgentId} rows have no unambiguous configured owner; preserve them and run aether vitals after assigning agents.defaults.sessionStore.agentId`,
          ]
        : [],
    };
  }
  ...
  for (const pathname of resolved.jsonPaths) {
    warnings.push(
      `session: deferred legacy-main session migration for JSON store ${pathname}; run aether vitals --fix`,
    );
  }
  ...
  const destination: PhysicalStore = resolved.stores.find((store) =>
    isSameAetherAgentDatabasePath(store.path, destinationResolved.path),
  ) ?? {
