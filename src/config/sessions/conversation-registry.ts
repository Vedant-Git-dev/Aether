...
import {
  ...
  type AetherAgentReadOnlyDatabase,
} from "../../state/aether-agent-db-readonly.js";
import {
...
function selectConversationRows(
  ...
  const readRows = (database: AetherAgentReadOnlyDatabase): ConversationRecord[] => {
  ...
  const held = getAetherAgentDatabaseIfOpen(databaseOptions);
  ...
  const read = withAetherAgentDatabaseReadOnly(readRows, databaseOptions);
