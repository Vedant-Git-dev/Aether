...
import { ensureSessionTranscriptArchiveSchema } from "../../state/aether-agent-session-transcript-archive-schema.js";
import {
...
export async function prunePublishedSessionArchivesByRetention(params: {
  ...
  const candidates = await runExclusiveSqliteSessionWrite(
    params.scope,
    async () => {
      const database = openAetherAgentDatabase(toDatabaseOptions(params.scope));
      const db = getSessionKysely(database.db);
      const exists = executeSqliteQueryTakeFirstSync(
        database.db,
        db
