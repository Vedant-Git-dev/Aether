...
export async function replaceTranscriptEvents(
  ...
  await runExclusiveSqliteSessionWrite(
    resolved,
    async () => {
      runAetherAgentWriteTransaction((database) => {
...
export async function replaceSessionWithBranchedTranscript(
  ...
  const expectedLifecycleRevision = readSessionEntryRow(
    openAetherAgentDatabase(databaseOptions),
    resolved.sessionKey,
  ...
  await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      assertActive?.();
      const committed = runAetherAgentWriteTransaction((database) => {
        assertActive?.();
...
export function persistCompactionBoundaryWithSessionEntrySync(
  ...
  const fencedScope = withOwnedSessionTranscriptWriterFence(scope);
  const resolved = resolveSqliteTranscriptScope(fencedScope);
  return runAetherAgentWriteTransaction(
    (database) => {
      assertOwnedTranscriptWriteCommit(fencedScope);
...
export async function withTranscriptWriteLock<T>(
  ...
  return await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      ...
      return await run({
        readEvents: async () => {
          // aether-agent-db.ts cache rule: LRU eviction closes idle handles across caller awaits.
        ...
        // aether-agent-db.ts cache rule: never retain a handle across caller awaits; LRU may close it.
        readMessageFacts: async (params) =>
          readTranscriptMirrorFacts(openAetherAgentDatabase(databaseOptions), resolved, params),
        replaceEvents: async (events) => {
          if (transcriptSnapshot?.kind === "stale") {
            throw new SqliteTranscriptMutationConflictError(resolved.sessionId);
          }
          const expectedSnapshot = transcriptSnapshot?.rows;
          const nextSnapshot = runAetherAgentWriteTransaction((writeDatabase) => {
        ...
        appendMessage: async (options) => {
          ...
          runAetherAgentWriteTransaction((writeDatabase) => {
            assertLockedTranscriptWriteAllowed(writeDatabase, resolved, fencedScope);
        ...
        () =>
          run({
            ...
            storePath:
              resolved.path ??
              scope.storePath ??
              resolveAetherAgentSqlitePath({ agentId: resolved.agentId, env: resolved.env }),
          }),
        toDatabaseOptions(resolved),
