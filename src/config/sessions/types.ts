...
import type {
  ...
  SessionAcpIdentity,
  SessionAcpMeta,
} from "@aether/acp-core/types";
import { asNonNegativeFiniteNumber } from "@aether/normalization-core/number-coercion";
import { normalizeOptionalString, type FastMode } from "@aether/normalization-core/string-coerce";
import type {
...
type SessionEntryCore = SessionRestartRecoveryState &
  ...
  Pick<SessionRow, "permissionMode" | "sessionRoot"> & {
    ...
    model?: string;
    /**
     * Prevents Aether model changes and automatic maintenance eviction until
    ...
    displayName?: string;
    /** Canonical delivery state. Legacy delivery fields are migrated by `aether vitals --fix`. */
