...
import type { AetherRegisteredAgentDatabase } from "../../state/aether-agent-db-contract.js";
import {
...
type ResolveSqliteStoreTargetOptions = {
  ...
  registeredDatabases?: readonly Pick<AetherRegisteredAgentDatabase, "agentId" | "path">[];
...
function resolveDatabaseOwner(pathname: string): string | undefined {
  ...
  const owner = inspectAetherAgentDatabaseOwner(pathname);
...
function resolveCustomStoreSqlitePath(params: {
  ...
  const registeredDatabases =
    ...
    listAetherRegisteredAgentDatabases(params.options.env ? { env: params.options.env } : {});
  const isSameDatabasePath = params.options.isSameDatabasePath ?? isSameAetherAgentDatabasePath;
...
export function resolveUnsuffixedSqliteTargetFromSessionStorePath(
  ...
  if (path.basename(resolved) === "aether-agent.sqlite" || resolved.endsWith(".sqlite")) {
  ...
  if (path.basename(resolved) !== "sessions.json") {
    const sqliteBaseName = path.basename(resolved, path.extname(resolved)) || "aether-agent";
    return { path: path.join(sessionsDir, `${sqliteBaseName}.sqlite`) };
  }
  if (path.basename(sessionsDir) !== "sessions") {
    return { path: path.join(sessionsDir, "aether-agent.sqlite") };
  }
  ...
  return {
    ...
    path: path.join(agentDir, "agent", "aether-agent.sqlite"),
  };
}
...
export function resolveSqliteTargetFromSessionStorePath(
  ...
  if (
    requestedAgentId &&
    isIncognitoAetherAgentSqlitePath(unsuffixedTarget.path, {
  ...
  if (path.resolve(storePath).endsWith(".sqlite")) {
    const registeredDatabases =
      ...
      listAetherRegisteredAgentDatabases(options.env ? { env: options.env } : {});
    const registeredOwners = resolveRegisteredOwners(
      unsuffixedTarget.path,
      registeredDatabases,
      options.isSameDatabasePath ?? isSameAetherAgentDatabasePath,
    );
...
function resolveAgentIdFromSqliteDatabasePath(databasePath: string): string | undefined {
  if (path.basename(databasePath) !== "aether-agent.sqlite") {
    return undefined;
  }
