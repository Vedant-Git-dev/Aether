...
import { openAetherAgentDatabase } from "../../state/aether-agent-db.js";
import { ensureSessionGoalOperationsSchema } from "../../state/aether-agent-goal-operations-schema.js";
import {
...
import { runSqliteSessionDeletionTransaction as runAetherAgentWriteTransaction } from "./session-accessor.sqlite-deletion.js";
import {
...
export async function appendExpectedSessionTranscriptTurn(
  ...
  options: {
    atomicGroup?: boolean;
    config?: import("../types.aether.js").AetherConfig;
    cwd?: string;
  ...
  return await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      ...
      const preparedDatabase = openAetherAgentDatabase(toDatabaseOptions(resolved));
      if (mutation) {
        ensureSessionGoalOperationsSchema(preparedDatabase.db);
      }
      // aether-agent-db.ts cache rule: LRU can close idle handles during shouldAppend awaits.
      ...
      const publish = runAetherAgentWriteTransaction((transactionDb) => {
        mutation?.assertCurrent?.();
        ...
        for (const append of messages) {
          ...
          if (mutation && goal && isRecord(message) && message.role === "user") {
            message = {
              ...message,
              __aether: {
                ...(isRecord(message["__aether"]) ? message["__aether"] : {}),
                intent: {
                  kind:
