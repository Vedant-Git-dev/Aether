...
export function planSessionStateDeleteIfUnreferenced(params: {
  ...
  return {
    ...
    archiveTranscript:
      params.archiveTranscript !== false && !isIncognitoAetherAgentDatabase(params.database),
...
function deleteSqliteSessionStateRows(database: AetherAgentDatabase, sessionId: string): boolean {
  const db = getSessionKysely(database.db);
