  ...
  if (legacyStore) {
    throw new SessionStoreMigrationRequiredError(
      params.operation === "doctor"
        ...
        : `Legacy session store requires migration: ${legacyStore}. Run "${formatCliCommand("aether vitals --fix", env)}" against the same state/config before starting Aether.`,
    );
  }
}
...
export async function runSessionStartupMigration(params: {
  ...
  handoffDatabase?: (database: AetherAgentDatabaseOptions) => Promise<void>;
  deps?: {
  ...
  const registeredDatabases = new Set(
    listAetherRegisteredAgentDatabases({ env }).map((entry) => `${entry.agentId}\0${entry.path}`),
  );
  ...
  for (const target of targets) {
    ...
    const alreadyOpen = isAetherAgentDatabaseOpen(databasePath);
    let handedOff = false;
    try {
      try {
        ...
        if (
          ...
          await withAetherAgentDatabaseAsync(options, (database) =>
      ...
      if (!alreadyOpen && !handedOff && isAetherAgentDatabaseOpen(databasePath)) {
