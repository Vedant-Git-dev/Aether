...
} from "../../state/aether-agent-db-identity.js";
...
export function resolveSessionEntry(
  ...
  const read = (
    database: Pick<AetherAgentDatabase, "agentId" | "db" | "path">,
  ): ResolvedSqliteSessionEntry => {
  ...
  if (options.readOnly) {
    const result = withAetherAgentDatabaseReadOnly(read, toDatabaseOptions(resolved));
    return result.found
      ? result.value
      : { existing: undefined, legacyKeys: [], normalizedKey: resolved.sessionKey };
  }
  return read(openAetherAgentDatabase(toDatabaseOptions(resolved)));
}
...
export function loadSessionEntryWithDatabase(scope: SessionAccessScope): {
  ...
  databaseClaim: AetherAgentDatabaseClaim;
} {
  const resolved = resolveSqliteScope(scope);
  const options = toDatabaseOptions(resolved);
  const database = openAetherAgentDatabase(options);
  const borrowed = borrowAetherAgentDatabase(options);
  const databaseClaim = createAetherAgentDatabaseClaim(database, borrowed.release);
  try {
...
export function listSessionEntryKeysReadOnly(
  ...
  const result = withAetherAgentDatabaseReadOnly((database) => {
...
export function listSessionEntryRows(scope: SessionEntryListScope = {}): SessionEntrySummary[] {
  const resolved = resolveSqliteScope({ ...scope, sessionKey: "" });
  const database = openAetherAgentDatabase(toDatabaseOptions(resolved));
...
function listSqliteSessionEntriesFromDatabase(
  ...
  const cache = !isIncognitoAetherAgentSqlitePath(database.path, {
...
export function replaceSessionEntrySync(scope: SessionAccessScope, entry: SessionEntry): void {
  ...
  const publish = runAetherAgentWriteTransaction((database) => {
...
type SqliteSessionEntrySnapshotPatchParams = {
  ...
  options: SqliteSessionEntryPatchOptions;
  readSnapshot: (database: AetherAgentDatabase) => SqliteLifecycleTargetSnapshot;
  resolved: ResolvedSqliteScope;
  sessionKey: string;
  storePath: string;
  update: (
...
async function patchSqliteSessionEntrySnapshot(
  ...
  resolved.env.AETHER_STATE_DIR = resolveStateDir(resolved.env);
  const databaseOptions = toDatabaseOptions(resolved);
  const databasePath = resolveAetherAgentSqlitePath(databaseOptions);
  resolved.path = databasePath;
  databaseOptions.path = databasePath;
  const incognito = isIncognitoAetherAgentSqlitePath(databasePath, databaseOptions);
  const withDatabase = <T>(operation: () => T | Promise<T>) =>
    !incognito && !getAetherAgentDatabaseIfOpen(databaseOptions)
      ? withAetherAgentDatabaseAsync(databaseOptions, operation)
      : operation();
  let wrote = false;
    ...
    async () =>
      withDatabase(async () => {
        ...
        return withDatabase(() => {
          ...
          const publish = runAetherAgentWriteTransaction((writeDatabase) => {
