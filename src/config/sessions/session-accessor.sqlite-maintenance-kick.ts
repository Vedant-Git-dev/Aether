...
export function kickSessionEntryMaintenanceAfterWrite(
  ...
  const databasePath = resolveAetherAgentSqlitePath(toDatabaseOptions(params.scope));
  const database = getAetherAgentDatabaseIfOpen(toDatabaseOptions(params.scope));
  if (!database) {
    return;
  }
