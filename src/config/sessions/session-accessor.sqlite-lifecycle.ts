...
import {
  ...
  runSqliteSessionDeletionTransaction as runAetherAgentWriteTransaction,
...
function captureLifecycleDatabaseScope<T extends ResolvedSqliteReadScope>(scope: T): T {
  ...
  env.AETHER_STATE_DIR = resolveStateDir(env);
  return {
    ...scope,
    env,
    path: resolveAetherAgentSqlitePath(toDatabaseOptions({ ...scope, env })),
  };
}
...
async function withCommittedHistoryMaintenance<T>(
  ...
  run: (
    recordCommit: (database: AetherAgentDatabase) => void,
    markCommitted: () => void,
  ) => Promise<T>,
  ...
  try {
    return await run(
      (database) => {
        deferAetherAgentPostCommitPublication(database, () => {
          committed = true;
        });
      },
      () => {
...
export async function cleanupSessionLifecycleArtifactsCore(
  ...
  if (!withAetherAgentDatabaseReadOnly(() => true, databaseOptions).found) {
    return { removedEntries: 0, archivedTranscriptArtifacts: 0 };
  }
    ...
    async (recordCommit) =>
      runExclusiveSqliteSessionWrite(
        ...
        async () => {
          ...
          runAetherAgentWriteTransaction((transactionDb) => {
            params.commitGuard?.();
...
async function deleteSqliteSessionEntryLifecycleLocked(
  ...
  return await withSqliteSessionDeletions(
    ...
    async (assertCurrent) => {
      ...
      const matchesPreparedTarget = (database: AetherAgentDatabase) => {
        const targetSnapshot = readLifecycleTargetSnapshot(database, params.target);
        return (
