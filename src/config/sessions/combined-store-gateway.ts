...
function createSessionModelSources(cfg: AetherConfig, preparedAgentIds?: ReadonlySet<string>) {
...
function resolvePreparedConfiguredSessionStoreTargets(
  ...
  const registryToken = readAetherAgentDatabaseRegistryToken();
  ...
  const candidates = dedupeSessionStoreTargetsBySqliteTarget(
    [
      ...listAetherRegisteredAgentDatabases().map(({ agentId, path }) => ({
        agentId,
        storePath: path,
      })),
