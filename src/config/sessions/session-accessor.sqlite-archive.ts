      ...
      if (message.type === "commit-request") {
        ...
        if (!withWriteAdmission || admission || message.admissionId !== admissionId + 1) {
          workerError ??= new Error(
            "SQLite reclamation Worker requested invalid write admission; cleanup is uncertain, restart Aether before deleting the owning agent",
          );
          ...
          if (!exited && admission === requested) {
            try {
              ...
              workerError = new AggregateError(
                ...
                "SQLite reclamation admission failed and Worker cleanup is uncertain; restart Aether before deleting the owning agent",
                { cause: workerError },
              );
        ...
        if (!admission || message.admissionId !== admission.id) {
          workerError ??= new Error(
            "SQLite reclamation Worker released invalid write admission; cleanup is uncertain, restart Aether before deleting the owning agent",
          );
          void worker.terminate();
          return;
        }
