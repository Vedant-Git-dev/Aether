...
import { toStringifiedError } from "@aether/normalization-core/error-coercion";
...
import {
  ...
  isIncognitoAetherAgentDatabase,
...
type SessionTranscriptReconcileParams = AetherAgentDatabaseOptions & {
...
type ReconcileDatabaseOptions = AetherAgentDatabaseOptions & {
...
function reconcileKey(params: AetherAgentDatabaseOptions): string {
  return resolveAetherAgentSqlitePath(params);
}

function captureMemorySource(params: AetherAgentDatabaseOptions) {
  const database = getAetherAgentDatabaseIfOpen(params);
  return database && isIncognitoAetherAgentDatabase(database)
  ...
  return await runExclusiveSqliteSessionWrite(
    ...
    async () => {
      const write = () => {
        ...
        return runAetherAgentWriteTransaction(operation, databaseOptions, { operationLabel });
      };
      return !isIncognitoAetherAgentSqlitePath(databaseOptions.path, databaseOptions) &&
        !getAetherAgentDatabaseIfOpen(databaseOptions)
        ? withAetherAgentDatabaseAsync(databaseOptions, write)
        : write();
    },
    operationLabel,
  );
...
async function reconcilePreparedTranscriptIndexes(
  ...
  const databasePath = resolveAetherAgentSqlitePath(params);
  ...
  try {
    ...
    await runProjectionWrite(
      ...
      (database) => {
        ...
        if (sessionIds.length > 0) {
          ...
          releaseDatabase = borrowAetherAgentDatabase(databaseOptions).release;
          if (memorySource) {
    ...
    try {
      ...
      const failure = new Error(
        `Transcript lease cleanup incomplete; restart Aether before deleting this agent: ${toStringifiedError(error).message}`,
        { cause: error },
      );
      throw outcome.ok
        ? failure
...
export function isSessionTranscriptIndexReconcileRunning(
  params: AetherAgentDatabaseOptions,
): boolean {
...
export async function waitForSessionTranscriptProjection(
  ...
  while (isSessionTranscriptIndexReconcileRunning(databaseOptions)) {
    ...
    const pending = withAetherAgentDatabaseReadOnly(
