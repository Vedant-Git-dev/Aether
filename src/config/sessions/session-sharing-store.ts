...
type SessionMemberDatabase = Pick<AetherAgentKyselyDatabase, "session_members">;
...
function resolveDatabaseOptions(scope: SessionAccessScope): AetherAgentDatabaseOptions {
  return toDatabaseOptions(resolveSqliteScope(scope));
}

function getSessionMemberKysely(database: Pick<AetherAgentDatabase, "db">) {
...
function readSessionMembers<T>(
  ...
  fallback: T,
  operation: (database: Pick<AetherAgentDatabase, "db">) => T,
): T {
  const result = withAetherAgentDatabaseReadOnly(operation, resolveDatabaseOptions(scope), {
...
export function addSessionMember(
  ...
  const inserted = runAetherAgentWriteTransaction((database) => {
