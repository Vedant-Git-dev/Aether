...
import { resolveAetherAgentSqlitePath } from "../../state/aether-agent-db.paths.js";
import type {
...
export function createTranscriptIdentityReader(database: AetherAgentDatabase, sessionId: string) {
...
export function readTranscriptEventRows(
  database: Pick<AetherAgentDatabase, "db">,
  sessionId: string,
...
export function readTranscriptStatsBatchReadOnlySync(
  ...
  for (const group of groups.values()) {
    const read = withAetherAgentDatabaseReadOnly((database) => {
...
export function loadLatestAssistantText(
  scope: SessionTranscriptReadScope,
  options: { includeTranscriptOnlyAetherAssistant?: boolean } = {},
...
function parseLatestAssistantMessageEvent(
  ...
  if (
    !options.includeTranscriptOnlyAetherAssistant &&
    isTranscriptOnlyAetherAssistantModel(message.provider, message.model)
  ) {
    return undefined;
  }
  return {
