...
import { openAetherAgentDatabaseReadOnly } from "../../state/aether-agent-db-readonly.js";
...
function resolveLeaseEnvironment(owner: ReconcileWorkerOwner) {
  return {
    AETHER_STATE_DIR: owner.stateDir,
    ...(owner.externallySupervised ? { AETHER_SUPERVISOR_MODE: "external" } : {}),
  };
}

function releaseLease(owner: ReconcileWorkerOwner & { leaseId: string }): void {
  try {
    releaseAetherAgentDatabaseLease(owner.leaseId, { env: resolveLeaseEnvironment(owner) });
...
async function run(): Promise<void> {
  ...
  try {
    const database = (() => {
      ...
      claimAetherAgentDatabaseLease(options, reconcileInput.leaseId);
      const opened = openAetherAgentDatabaseReadOnly(options);
      if (!opened.found) {
