...
export function copySqliteSessionOwnedStateForCanonicalRepair(params: {
  canonicalKey: string;
  destinationDatabase: AetherAgentDatabase;
  ...
  const sourceDatabase = openAetherAgentDatabase(toDatabaseOptions(source));
...
export function listSqliteSessionGenerationIdsForCanonicalRepair(params: {
  ...
  const database = openAetherAgentDatabase(toDatabaseOptions(source));
