...
export function listConfiguredSessionStoreAgentIds(cfg: AetherConfig): string[] {
...
export function listKnownSessionStoreAgentIds(
  ...
  const isSameDatabasePath = createAetherAgentDatabasePathMatcher();
  ...
  if (!isPerAgentSessionStoreConfig(cfg.session?.store)) {
    ...
    if (durableTarget.shared && durableTarget.agentId && fsSync.existsSync(durableTarget.path)) {
      try {
        const logicalOwners = withAetherAgentDatabaseReadOnly(
          (database) =>
  ...
  for (const registered of listAetherRegisteredAgentDatabases({ env })) {
...
export function isConfiguredSessionStoreAgentId(cfg: AetherConfig, agentId: string): boolean {
