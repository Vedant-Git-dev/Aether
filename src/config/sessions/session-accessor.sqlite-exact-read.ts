...
export function loadExactSessionEntryCandidates(
  ...
  const read = (database: Pick<AetherAgentDatabase, "agentId" | "path" | "db">) => {
  ...
  if (!scope.readOnly) {
    return read(openAetherAgentDatabase(options));
  }
  const result = withAetherAgentDatabaseReadOnly(read, options);
...
export function loadExactSessionEntryCandidatesReadOnlyBatch(
  ...
  for (const [index, scope] of scopes.entries()) {
    ...
    try {
      ...
      const groupKey = [
        options.agentId,
        resolveAetherAgentSqlitePath(options),
  ...
  for (const group of groups.values()) {
    try {
      withAetherAgentDatabaseReadOnly((database) => {
