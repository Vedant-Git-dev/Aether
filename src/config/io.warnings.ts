...
export function warnIfConfigFromFuture(
  ...
  logger.warn(
    [
      `Your Aether config was written by version ${touched}, but this command is running ${VERSION}.`,
      "Check: `aether --version`, `which aether`, and `aether gateway status --deep`.",
      "If unexpected, update PATH so `aether` points to the version you want, or reinstall the Gateway service from that same Aether install.",
    ].join("\n"),
  );
}
