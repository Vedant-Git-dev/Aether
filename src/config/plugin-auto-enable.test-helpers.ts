...
export function makeTempDir(): string {
  return makeTrackedTempDir("aether-plugin-auto-enable", tempDirs);
}

export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const rootDir = makeTempDir();
  return {
    AETHER_STATE_DIR: path.join(rootDir, "state"),
    AETHER_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/aether.plugin.json`,
    })),
    diagnostics: [],
  };
}
