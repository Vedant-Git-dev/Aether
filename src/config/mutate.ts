...
import type { ConfigFileSnapshot, AetherConfig } from "./types.js";
...
export async function withConfigMutationExclusive<T>(
  fn: (config: AetherConfig) => Promise<T>,
): Promise<T> {
...
function resolveIncludeOwnedWriteCandidate(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): (IncludeWriteBoundary & { nextConfig: AetherConfig }) | null {
  ...
  const nextConfig = applyUnsetPathsForWrite(
    ...
    }) as AetherConfig, // SAFETY: Projection and path edits preserve the config object.
...
function createRootBoundBackupFs(
  ...
  return {
    ...
    // Full include-backup fencing is blocked by https://github.com/aether/fs-safe/issues/251.
...
async function tryWriteIncludeOwnedConfigMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: AetherConfig } | null> {
  const includeWrite = resolveIncludeOwnedWriteCandidate({
    snapshot: params.snapshot,
    nextConfig: params.nextConfig,
    writeOptions: params.writeOptions,
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: AetherConfig,
): { persistedHash: string | null; persistedConfig: AetherConfig } {
  if (result) {
    return {
...
export type ConfigReplaceInput =
  | { sourceConfig: AetherConfig; nextConfig?: never }
  | { nextConfig: AetherConfig; sourceConfig?: never };
...
type MutateConfigFileParams<T> = Omit<TransformConfigFileParams<T>, "transform" | "commit"> & {
  mutate: (draft: AetherConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
};
