...
export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "AETHER_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Aether binary (${currentVersion}) is older than the config last written by Aether ${touchedVersion}.`,
    hints: [
      "Run the newer aether binary on PATH, or reinstall the intended gateway service from the newer install.",
