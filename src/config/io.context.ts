...
type ValidationPluginMetadataSnapshotLoader = {
  load: (config: AetherConfig) => Pick<PluginMetadataSnapshot, "manifestRegistry">;
...
export type ConfigIoContext = {
  ...
  finalizeLoadedRuntimeConfig: (config: AetherConfig) => AetherConfig;
  ...
  resolveRuntimePreflightSourceConfig: (candidate: AetherConfig) => AetherConfig;
...
export function createConfigIoContext(options: ConfigIoFactoryOptions = {}): ConfigIoContext {
  ...
  // The normalized default homedir already applies AETHER_HOME. Path
  ...
  function finalizeLoadedRuntimeConfig(cfg: AetherConfig): AetherConfig {
  ...
  function resolveRuntimePreflightSourceConfig(candidate: AetherConfig): AetherConfig {
