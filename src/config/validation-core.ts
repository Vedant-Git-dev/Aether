...
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@aether/net-policy/ip";
...
import type { ConfigValidationIssue, AetherConfig } from "./types.js";
...
import { AetherSchema } from "./zod-schema.js";
import { McpServerNameSchema, NodeHostMcpServerNameSchema } from "./zod-schema.root-support.js";

export function collectHeartbeatOwnerWarnings(config: AetherConfig): ConfigValidationIssue[] {
...
function materializeBundledModelProviderOverlays(config: AetherConfig): AetherConfig {
...
function validateGatewayTailscaleBind(config: AetherConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: AetherConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
function collectModelPolicyAllowIssues(config: AetherConfig): ConfigValidationIssue[] {
  const issues: ConfigValidationIssue[] = [];
        ...
        withConfigIssuePath(
          {
            ...
            message:
              ...
              `${remediation} SSH/OpenShell backends may keep multiline values. Run aether vitals to report the invalid path; manual remediation is required.`,
          },
          pathSegments,
        ),
      );
...
export function validateConfigObjectRaw(
...
): { ok: true; config: AetherConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  const legacyDefaultAgentId = isRecord(raw)
    ? tryGetLegacyDefaultAgentId(raw as AetherConfig)
    : undefined;
  ...
  const validated = AetherSchema.safeParse(normalizedRaw);
  ...
  let parsedConfig = validated.data as AetherConfig;
