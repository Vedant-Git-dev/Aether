...
function isPluginExplicitlyDisabled(cfg: AetherConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AetherConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: AetherConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: AetherConfig, channelId: string): boolean {
  const channels = cfg.channels;
