...
function formatRemovedPluginConfigWarning(pluginId: string): string {
  if (pluginId === "skill-workshop") {
    return "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into Aether skills now. Use skills.workshop settings and aether skills workshop commands, then remove this plugins config entry)";
  }
...
function formatMissingOfficialExternalPluginWarning(
  ...
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: aether plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: aether plugins install ${installSpec}`;
}
  ...
  const pushMissingPluginIssue = (
    ...
    if (
      ...
      shouldSuppressMissingCodexPluginDiagnostics(
        config,
        env ?? process.env,
        isRecord(raw) ? (raw as AetherConfig) : undefined,
      )
    ) {
      return;
    }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Aether config schema.
