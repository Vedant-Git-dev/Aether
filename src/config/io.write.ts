...
import { err, ok } from "@aether/normalization-core/result";
...
export async function writeConfigFileFromContext(
  ...
  const resolveValidationCandidate = (candidate: unknown) => {
    ...
    const config = applyUnsetPathsForWrite(candidate as AetherConfig, unsetPaths);
    return containsConfigIncludeDirective(config)
      ? context.resolveRuntimePreflightSourceConfig(
          restoreEnvVarRefs(config, snapshot.parsed, envForRestore) as AetherConfig,
        )
      : config;
  };
  ...
  const validatedCandidate = validationCandidate as AetherConfig;
  ...
  let cfgToWrite = persistCandidate as AetherConfig;
  try {
    if (deps.fs.existsSync(configPath)) {
      ...
      if (parsed.ok) {
        ...
        cfgToWrite = restoreEnvVarRefs(cfgToWrite, parsed.parsed, envForRestore) as AetherConfig;
  ...
  const logConfigOverwrite = () => {
    if (
      ...
      (isVitestRuntimeEnv(deps.env) && !readTestLogFlag("AETHER_TEST_CONFIG_WRITE_LOG"))
    ) {
      return;
    }
    const testLog = readTestLogFlag("AETHER_TEST_CONFIG_WRITE_LOG");
    if (!isVerbose() && deps.env.AETHER_CONFIG_OVERWRITE_LOG !== "1" && !testLog) {
      return;
    }
    deps.logger.warn(
      formatConfigOverwriteLogMessage({
  ...
  const logConfigWriteAnomalies = () => {
    ...
    const showMissingMeta =
      isVerbose() || deps.env.AETHER_CONFIG_WRITE_ANOMALY_LOG === "1" || testLog;
  ...
  const preCommitRuntimePreflight =
    ...
    (async (sourceConfig: AetherConfig) => {
