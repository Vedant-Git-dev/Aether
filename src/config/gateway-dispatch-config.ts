...
function resolveGatewayDispatchConfig(value: unknown, env: NodeJS.ProcessEnv): AetherConfig {
  if (!isRecord(value)) {
    return {};
  }
  if (Object.hasOwn(value, "env")) {
    applyConfigEnvVars(value as AetherConfig, env);
  }
  ...
  return resolveConfigEnvVars(projected, env, { onMissing: () => undefined }) as AetherConfig;
}

// Main session keys are process-local; Gateway dispatch always sees the canonical main key.
function applyGatewayDispatchSessionDefaults(config: AetherConfig): AetherConfig {
