...
export type WideAreaDiscoveryConfig = {
  /** Optional unicast DNS-SD domain (e.g. "aether.internal"). */
  domain?: string;
};
...
export type TalkConfig = {
  ...
  /** BCP 47 locale id used for Talk speech recognition on device nodes and the system-voice fallback. */
  speechLocale?: string;
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/aether"). */
  basePath?: string;
  experimental?: {
   ...
   * Default off; prefer hosted /__aether__/canvas or /__aether__/a2ui content.
   */
 ...
 * Used when Aether runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayNodePairingConfig = {
   ...
   * host keys) and approves only when the remote `aether node identity`
