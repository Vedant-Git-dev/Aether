// Validates normalized Aether config and reports user-facing errors.
...
type ValidateConfigWithPluginsResult =
  | { ok: true; config: AetherConfig; warnings: ConfigValidationIssue[] }
...
function validateConfigObjectWithPluginsBase(
  ...
  const parsedConfig = inheritLegacyDefaultAgentId(raw as AetherConfig, base.config);
  ...
  const validateWebSearchProvider = (): void => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path: issuePath,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run aether vitals --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or aether vitals --fix repairs the config)`,
        });
      } else {
        issues.push(issue);
      }
    ...
    if (hasStaleEvidence) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run aether vitals --fix to remove stale config, or install the plugin)`,
      });
    } else {
      issues.push(issue);
    }
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run aether vitals --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
