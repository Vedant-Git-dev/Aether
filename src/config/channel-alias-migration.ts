...
function buildAliasRuleMessage(params: {
  ...
  return `${keyList} are legacy; use ${prefix}.streaming.{${nested.join(",")}}. Run "aether vitals --fix".`;
}
...
export function defineChannelAliasMigration<TMode extends string = StreamingAliasMode>(
  ...
  normalizeChannelConfig: (params: { cfg: AetherConfig; changes?: string[] }) => {
    config: AetherConfig;
    changes: string[];
  };
} {
  ...
  const normalizeChannelConfig = (params: { cfg: AetherConfig; changes?: string[] }) => {
  ...
  if (spec.dm?.root) {
    legacyConfigRules.push({
      path: ["channels", spec.channelId],
      message: `${pathPrefix}.dm.policy and ${pathPrefix}.dm.allowFrom are legacy; use ${pathPrefix}.dmPolicy and ${pathPrefix}.allowFrom. Run "aether vitals --fix".`,
  ...
  if (spec.dm?.accounts) {
    legacyConfigRules.push({
      path: ["channels", spec.channelId, "accounts"],
      message: `${pathPrefix}.accounts.<id>.dm.policy and dm.allowFrom are legacy; use ${pathPrefix}.accounts.<id>.dmPolicy and allowFrom. Run "aether vitals --fix".`,
