...
import { asOptionalObjectRecord, isRecord } from "@aether/normalization-core/record-coerce";
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: AetherConfig): boolean {
...
function isProviderConfigured(cfg: AetherConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: AetherConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: AetherConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: AetherConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: AetherConfig): boolean {
...
function hasConfiguredVoiceProviderSelection(cfg: AetherConfig): boolean {
  return Boolean(
...
function hasBrowserToolReference(cfg: AetherConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: AetherConfig): string[] {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function hasOwnPluginEntry(cfg: AetherConfig, pluginId: string): boolean {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  return entries !== undefined && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: AetherConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: AetherConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: AetherConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: AetherConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: AetherConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: AetherConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: AetherConfig): boolean {
  return (
...
function hasPluginEntries(cfg: AetherConfig): boolean {
  const entries = asOptionalObjectRecord(cfg.plugins?.entries);
  return entries !== undefined && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: AetherConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: AetherConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: AetherConfig, env: NodeJS.ProcessEnv): boolean {
