...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Aether env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Aether-managed keys:
 ...
 * literal references for keys Aether previously managed.
 */
...
/**
 * Read and parse `~/.aether/.env` (or `$AETHER_STATE_DIR/.env`), returning
