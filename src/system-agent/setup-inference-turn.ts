...
export async function runSetupInferenceTurn(params: {
  ...
  const workspaceDir = await (
    deps.createTempDir ?? (() => fs.mkdtemp(path.join(os.tmpdir(), "aether-setup-inference-")))
  )();
  ...
  try {
    ...
    if (route.runner === "cli") {
      ...
      result = await runEmbedded({
        ...
        ...(params.prompt === undefined && (!harness || harness === "aether")
    ...
    if (params.requireExecutionOwner && !successfulAuth) {
      return failed(
        "unknown",
        "Inference succeeded, but its runtime did not report an owner that Aether can safely reuse.",
      );
    }
    return {
      ok: true,
...
export async function verifySetupInference(
  ...
  if (!snapshot.exists) {
    return {
      ok: false,
      status: "unavailable",
      error: "No Aether config exists. Run `aether awaken` first.",
    };
  }
  if (!snapshot.valid) {
    return { ok: false, status: "format", error: invalidSetupConfigError(snapshot) };
  }
  const cfg: AetherConfig = snapshot.runtimeConfig ?? snapshot.config;
  ...
  if (!verifiedBinding) {
    return {
      ...
      status: "unknown",
      error:
        "The successful inference run did not report an exact execution binding. Retry setup before starting Aether.",
    };
  }
...
export async function verifySetupInferenceConfig(
  ...
  if (!configuredRoute) {
    return {
      ok: false,
      status: "unavailable",
      error: "No agent model is configured. Run `aether awaken` first.",
    };
  }
...
export async function completeSetupInference(
  ...
  const snapshot = await readSnapshot();
  if (!snapshot.exists) {
    return { ok: false, status: "unavailable", error: "No Aether config exists." };
  }
  if (!snapshot.valid) {
    return { ok: false, status: "format", error: invalidSetupConfigError(snapshot) };
  }
