...
export function formatChannelDocsUrl(docsPath: string): string {
  return `https://docs.aether.ai${docsPath.startsWith("/") ? docsPath : `/${docsPath}`}`;
}
 ...
 * `[aether] running/done` markers the e2e lanes assert on; each spec only
...
export async function applyPersistentOperation(params: {
  ...
  runtime.log(`[aether] running: ${auditOperation}`);
  const { readConfigFileSnapshot } = await loadConfigModule();
  ...
  try {
    ...
    runtime.error(
      `${outcome.summary}, but Aether could not record its audit entry: ${formatErrorMessage(error)}`,
    );
  }
  runtime.log(`[aether] done: ${auditOperation}`);
  return {
    applied: true,
...
export async function assertConfigWriteDoesNotBypassInferenceVerification(
  ...
  if (verdict === "plugin-entry") {
    ...
    throw new Error(
      `Direct config writes cannot change plugin "${pluginId}" because it may back Aether's own active inference route. Editing it is a human-only change, made with Aether stopped from a trusted shell on the machine running it.`,
    );
  }
  ...
  throw new Error(
    denialReason
      ...
      : "Direct config writes cannot change the default inference route or include alternate config. Use `set_default_model` (optionally with agentId) for an already configured route; changing provider or auth access is `aether awaken` on the machine running Aether.",
  );
}
...
async function verifyCurrentSetupInference(
  ...
  if (!before.exists || !before.valid) {
    throw new Error(
      "Aether setup requires a valid configured inference route. Run `aether awaken` on the machine running Aether, then retry.",
    );
  }
  ...
  if (!beforeRoute.route) {
    throw new Error(
      "Aether setup requires working inference first. Run `aether awaken` on the machine running Aether, then retry.",
    );
  }
  ...
  if (!verification.ok) {
    throw new Error(
      `Aether setup requires working inference first. The configured route failed a live check: ${verification.error} Run \`aether awaken\` on the machine running Aether, then retry.`,
    );
  }
...
export async function executeSetup(
  ...
  if (!defaultModel) {
    throw new Error(
      "Aether setup requires working inference first. Run `aether awaken` on the machine running Aether to configure and verify a default model, then start Aether again.",
    );
  }
  const requestedModel = operation.model?.trim();
  if (requestedModel && requestedModel !== defaultModel) {
    throw new Error(
      `Aether setup will preserve the verified default model ${defaultModel}. Staging, live-testing, and saving a different inference route is \`aether awaken\` on the machine running Aether.`,
    );
  }
  if (!opts.approved) {
    const message = [
  ...
  if (requestedModel && requestedModel !== verified.modelRef) {
    throw new Error(
      `The verified default model is now ${verified.modelRef}, not ${requestedModel}. Review the current route, or run \`aether awaken\` on the machine running Aether, before retrying setup.`,
    );
  }
  return await applyPersistentOperation({
    auditOperation: "aether.setup",
    operation,
    runtime,
    opts,
    run: async (ctx) => {
...
export async function executeSetDefaultModel(
  ...
  return await applyPersistentOperation({
    ...
    run: async (ctx) => {
      ...
      const projectRoute = (config: AetherConfig) => projectInferenceRoute(config, targetAgentId);
      ...
      const result = await mutateConfigFile({
        ...
        writeOptions: {
          ...
          preCommitRuntimePreflight: async (sourceConfig) => {
            ...
            // The live probe can outlive the original Aether authority.
