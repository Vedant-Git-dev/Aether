// Applies Aether's conversational setup: config, workspace files, gateway.
...
export type SystemAgentSetupApplyParams = {
  ...
  finalizeConfig?: (config: AetherConfig, sourceConfig: AetherConfig) => AetherConfig;
  /** Synchronous cross-store guard receives authored config under the final write lock. */
  assertCommitPreconditions?: (sourceConfig: AetherConfig) => void;
...
export function createQuickstartNotePrompter(runtime: RuntimeEnv): WizardPrompter {
  const unexpected = (kind: string) => {
    throw new Error(`aether ignite hit an interactive ${kind} prompt; quickstart must not ask`);
  };
  return {
    intro: async () => {},
    outro: async () => {},
...
function applySecurityAcknowledgement(config: AetherConfig): AetherConfig {
...
export async function applySystemAgentSetup(
  ...
  if (hasExpectedConfigHash && resolveConfigSnapshotHash(snapshot) !== expectedConfigHash) {
    throw new Error("Aether config changed while AI access was being tested. Try setup again.");
  }
  ...
  let guardModules =
    expectedAgentId || expectedAgentDir || expectedModelRef
      ...
      : undefined;
  const assertExpectedTarget = (config: AetherConfig): void => {
    if (!guardModules) {
      return;
    }
    assertSetupTarget({
  ...
  const assertVerifiedRoute = async (
    ...
    if (
      ...
      throw new Error(
        phase === "before"
          ? "The default-agent inference route changed before setup could start, so no workspace or Gateway settings were changed. Retry setup from the current Aether session."
          : "The default-agent inference route changed after the config write, so no further setup effects were applied. Retry setup from the current Aether session.",
      );
    }
    return currentRoute;
  };
  ...
  if (startedWithoutAuthoredRoster) {
    ...
    if (!created.createdAgent || !created.configHash) {
      throw new Error(
        "Aether did not create the approved first agent because the roster changed. Retry setup.",
      );
    }
    ...
    if ((resolveConfigSnapshotHash(snapshot) ?? null) !== created.configHash) {
      throw new Error("Aether config changed after first-agent creation. Retry setup.");
    }
    ...
    if (
      ...
      throw new Error("Aether first-agent ownership changed during setup. Retry setup.");
    }
  ...
  const buildSetupCandidate = async (
    currentBaseConfig: AetherConfig,
  ...
  const committed = await transformConfigWithPendingPluginInstalls({
    ...
    transform: async (currentConfig, context) => {
      ...
      if (
        ...
        throw new Error(
          "Aether config changed while AI access was being tested. Try setup again.",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "The setup candidate no longer preserves the exact verified inference route, so it was not saved. Retry setup from the current Aether session.",
        );
      }
  ...
  if (!settings) {
    throw new Error("Aether setup committed without resolved Gateway settings.");
  }
  ...
  if (verifiedRoute) {
    ...
    if (!expectedRuntime.ok) {
      ...
      throw new Error(
        `Aether could not validate the setup route after its config write${detail}. No further setup effects were applied. Retry setup from the current Aether session.`,
      );
    }
    ...
    if (!sameSetupConfiguredRoute(expectedPersistedRoute.route, verifiedRoute.route, false)) {
      throw new Error(
        "The materialized inference route no longer matches the exact verified route, so no further setup effects were applied. Retry setup from the current Aether session.",
      );
    }
  }
  ...
  // Setup approval includes consent for Aether's local model harnesses.
  ...
  await runCommittedFollowUp(
    async () => {
      ...
      await updateExecApprovals({
        update: (approvals) =>
          approvals.agents?.aether
            ? null
            : {
                ...approvals,
                agents: {
                  ...approvals.agents,
                  aether: { security: "full", ask: "off" },
                },
              },
      });
    },
    (error) =>
      lines.push(
        `Aether exec approval: ${formatErrorMessage(error)}; local model harnesses may ask again.`,
      ),
  );
    ...
    await runCommittedFollowUp(
      async () => {
        ...
        if (gateway.status === "failed") {
          ...
          lines.push(
            "Gateway: service installation skipped. Run `aether gateway run` to start it in the foreground.",
          );
        } else {
          lines.push(
