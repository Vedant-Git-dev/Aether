// Classifies whether a user's chat message approves a pending Aether proposal.
...
export type SystemAgentApprovalClassifier = (params: {
  ...
  proposal?: string;
  /** Exact execution owner that completed the live Aether inference gate. */
...
export async function classifySystemAgentApprovalIntent(
  ...
  try {
    ...
    if (!route || route.runner !== "embedded" || route.agentHarnessRuntimeOverride !== "aether") {
      return "other";
    }
