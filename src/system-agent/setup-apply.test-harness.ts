...
export type CommitTransform = (
  ...
  | { nextConfig: AetherConfig; result?: unknown }
  | Promise<{ nextConfig: AetherConfig; result?: unknown }>;

const mocks = vi.hoisted(() => ({
  state: {
    initialSnapshot: {} as ConfigSnapshot,
    commitConfig: {} as AetherConfig,
    commitSnapshot: {} as ConfigSnapshot,
    commitPreviousHash: "probe" as string | null,
    persistedConfig: undefined as AetherConfig | undefined,
  },
  events: [] as string[],
...
vi.mock("../commands/onboard-helpers.js", () => ({
  applyWizardMetadata: (config: AetherConfig) => ({
    ...config,
    wizard: {
      ...config.wizard,
...
vi.mock("../agents/agent-scope.js", async (importOriginal) => ({
  ...
  resolveAgentDir: (config: AetherConfig, agentId: string) =>
...
export function snapshot(
  hash: string | null,
  sourceConfig: AetherConfig,
  runtimeConfig: AetherConfig = sourceConfig,
): ConfigSnapshot {
  return {
    exists: hash !== null,
    valid: true,
      ...
      plugins: [
        {
          ...
          manifestPath: "/tmp/codex/aether.plugin.json",
          configSchema: {
            type: "object",
...
export function baseParams(
  ...
  return {
    workspace: "/tmp/aether-workspace",
...
export function mainAgentModelConfig(model = "openai/gpt-5.5"): AetherConfig {
  return { agents: { defaults: { model }, entries: { main: { default: true } } } };
}

export function setSetupCommitState(config: AetherConfig, initialSnapshot: ConfigSnapshot): void {
