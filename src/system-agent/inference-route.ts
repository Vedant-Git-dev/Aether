// Resolves the configured default agent route shared by Aether inference calls.
...
export type DefaultInferenceRouteProjection = {
  ...
  env: AetherConfig["env"];
  secrets: AetherConfig["secrets"];
  plugins: AetherConfig["plugins"];
  tools: AetherConfig["tools"];
};
...
export async function projectInferenceRoute(
  ...
  requestedAgentId?: string,
  deps: SystemAgentRouteProjectionDeps = {},
  sourceConfig: AetherConfig = config,
): Promise<DefaultInferenceRouteProjection> {
...
export function assertSetupTarget(params: {
  ...
  resolveAgentDir: (config: AetherConfig, agentId: string) => string;
  resolveDefaultAgentId: (config: AetherConfig) => string;
  resolveDefaultModelForAgent: (params: { cfg: AetherConfig; agentId: string }) => {
    provider: string;
    model: string;
  };
}): void {
