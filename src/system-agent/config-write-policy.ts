...
export const SYSTEM_AGENT_CONFIG_WRITE_DENYLIST: Readonly<Record<string, string>> = {
  $include: "alternate-config inclusion; edit aether.json in a trusted shell",
  auth: "provider auth; `aether awaken` on the machine running Aether",
  env: "environment/credential injection; edit aether.json in a trusted shell",
  models:
    "provider/catalog definitions feed routing; use `set_default_model` or `aether awaken`",
  secrets: "secret providers; edit aether.json in a trusted shell",
};
