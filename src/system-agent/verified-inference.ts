...
import type { AetherPackageBuild } from "../plugins/manifest.js";
...
type SystemAgentOwnerPluginRegistryRecord = {
  ...
  packageBuild?: AetherPackageBuild;
};
...
/** Exact child harness artifact every verified embedded Aether call must carry. */
export function resolveSystemAgentExpectedAgentHarnessRuntimeArtifact(
  ...
  if (
    ...
    binding.execution.agentHarnessRuntimeOverride === "aether"
  ) {
    return undefined;
  }
...
async function resolveCurrentRuntimeOwnerFingerprint(params: {
  ...
  if (params.kind === "plugin-harness") {
    if (params.route.agentHarnessRuntimeOverride === "aether") {
      return undefined;
    }
...
function resolveRouteHarnessOwnerPluginIds(
  ...
  if (
    ...
    route.agentHarnessRuntimeOverride === "aether"
  ) {
    return [];
  }
  ...
  if (params.authProfileId) {
    ...
    if (
      ...
      (params.route.runner === "embedded" &&
        params.route.agentHarnessRuntimeOverride !== "aether")
    ) {
...
export async function createSystemAgentVerifiedInferenceBinding(params: {
  ...
  const pluginHarnessId =
    execution.runner === "embedded" && successfulHarnessId !== "aether"
      ? successfulHarnessId
      : undefined;
  if (pluginHarnessId) {
    if (
