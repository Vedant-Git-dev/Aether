...
function createCaptureRuntime(): CaptureRuntime {
  ...
  return {
    ...
    exit: (code) => {
      throw new Error(`Aether operation exited with code ${String(code)}`);
    },
    read: () => lines.join("\n").trim(),
  };
}
...
function formatPendingOperationForAssistant(operation: SystemAgentOperation): string {
  ...
  return operation.kind === "setup"
    ? `${description}. Exact setup JSON: ${JSON.stringify(operation)}. Keep the verified model unless the user explicitly asks to leave Aether and reconfigure inference.`
    : description;
}
...
export class ChatTurnRouter {
  ...
  async resolveTurn(
    ...
    if (/^(quit|exit)$/i.test(trimmed)) {
      return { text: "Aether retracts into shell. Bye.", action: "exit" };
    }
    ...
    if (this.options.operatorApprovalOnly && this.getPendingOperatorProposal()) {
      return { text: "Approval pending. Human must decide in Aether UI.", action: "none" };
    }
  ...
  private async applyApprovedPersistentOperation(
    ...
    if (!isPersistentSystemAgentOperation(operation)) {
      throw new Error("Aether host received a non-persistent approved operation.");
    }
  ...
  async resolveAssistantTurn(
    ...
    const loopInput = `${resolutionMarker}${uiContextMarker}${
      this.pending
        ? `[pending-proposal] Awaiting the user's approval: ${formatPendingOperationForAssistant(this.pending)}. It is already host-seeded; if they want it (or a variant), drive it through the aether tool yourself.\n${text}`
        : text
    }`;
  ...
  private async runOperation(operation: SystemAgentOperation): Promise<SystemAgentChatReply> {
      ...
      return {
        text:
          this.options.surface === "gateway"
            ...
            : "Run `aether models accounts list` to see your personal accounts, or `aether models accounts login <provider>` for protected sign-in. Check the Gateway and person shown before signing in. You can also use Settings → Profile → Connected accounts in the Control UI. Nothing has changed; never paste credentials into this conversation.",
        action: "none",
    ...
    if (recordedOperation.kind === "open-setup") {
      ...
      if (this.options.surface === "gateway") {
        return {
          text: "Open Settings to change your model or connect a channel. To change providers from a shell, run `aether awaken` on the machine running Aether.",
          action: "none",
        };
      }
      if (!["channels", "search", "gateway"].includes(recordedOperation.target)) {
        return {
          text: "Setup can replace the inference route powering this session. Exit Aether and run `aether awaken`; it saves only a route that passes a live test. Then start Aether again.",
          action: "none",
        };
      }
  ...
  private startModelSetup(): SystemAgentChatReply {
    ...
    return {
      text: [
        ...
        "Stop the Aether host through whatever started it. Run `aether awaken` on the machine running Aether: it stages credentials, live-tests the new route, and saves only a passing setup. Then restart the host and return to Aether.",
      ].join("\n"),
      action: "none",
    };
  }
  ...
  private agentHandoffReturnHint(): string {
    // Only the TUI uses /aether for navigation; web chat runs rescue in place.
    return this.options.surface === "gateway"
      ? "You can return through Settings → Ask Aether."
      : "Use /aether to come back.";
  }
  ...
  private armFollowUp(operation: SystemAgentOperation | undefined): string | null {
    return operation?.kind === "model-setup"
      ? [
          "No usable inference route is configured, so Aether cannot continue.",
          "Run `aether awaken` on the machine running Aether; it saves only a route that passes a live test.",
        ].join("\n")
      : null;
  }
}
