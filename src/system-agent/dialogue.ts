// Aether dialogue parses direct commands and optionally asks the assistant planner.
...
/**
 * Dialogue helpers for turning user text into Aether operations.
 *
...
/** Resolve user input to an Aether operation, optionally using the assistant planner. */
...
function logAssistantPlan(
  ...
  runtime.log(`[aether] planner: ${modelLabel}`);
  if (plan.reply) {
    runtime.log(plan.reply);
  }
  runtime.log(`[aether] interpreted: ${plan.command}`);
}
