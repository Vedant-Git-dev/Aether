...
async function loadAuthoredSetupConfig(params: {
  ...
  authoredConfig?: import("../config/types.aether.js").AetherConfig;
...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "Aether onboarding requires working inference first. Run `aether awaken` on the machine running Aether to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm Aether — let's hatch your agent.",
    "",
    ...
    "",
    "Heads up: your agent gets real access to this machine — https://docs.aether.ai/security",
