// Aether rescue messages expose approved setup-helper commands over message channels.
import { createHash } from "node:crypto";
import {
  asDateTimestampMs,
  resolveExpiresAtMsFromDurationMs,
} from "@aether/normalization-core/number-coercion";
import { hasNonEmptyString as isNonEmptyString } from "@aether/normalization-core/string-coerce";
...
/**
 * Message-channel rescue command handling for Aether.
 *
 * Rescue mode accepts `/aether` commands from approved message contexts,
...
/** Input required to process one possible `/aether` rescue message. */
...
const SYSTEM_AGENT_COMMAND = "/aether";
...
function createCaptureRuntime(): { runtime: RuntimeEnv; read: () => string } {
  ...
  return {
    runtime: {
      ...
      error: push,
      exit: (code) => {
        throw new Error(`Aether operation exited with code ${code}`);
      },
    },
    read: () => lines.join("\n").trim(),
  };
...
/** Extract the command body after `/aether`, or null when the message is not for rescue. */
...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      "Aether rescue cannot open the local TUI from a message channel.",
      "Use local `aether` for agent handoff, or ask for status, doctor, config, gateway, agents, or models.",
    ].join(" ");
  }
  if (operation.kind === "channel-setup") {
    return [
      "Aether rescue cannot host the interactive channel setup from a message channel.",
      "Run `aether ignite` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "Aether rescue cannot host model-provider credential setup from a message channel.",
      "Run `aether awaken` locally; it live-tests the candidate route before saving it.",
    ].join(" ");
  }
  if (operation.kind === "doctor-fix") {
    return [
      "Aether rescue cannot run doctor repairs from a message channel because they can change the inference route powering this session.",
      "On the machine running Aether, with Aether stopped, run `aether vitals --fix`.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      "Aether rescue cannot install plugins from a message channel by default because plugin install downloads executable code.",
      "Use local `aether ignite` or `aether plugins install` instead.",
    ].join(" ");
  }
  return null;
}
...
export async function runSystemAgentRescueMessage(
  ...
  if (approvalIntent === "approve") {
    ...
    if (!operation) {
      return "No pending Aether rescue change is waiting for approval.";
    }
    ...
    return capture.read() || "Aether rescue change applied.";
  }

  if (approvalIntent === "decline") {
    const pending = parsePendingOperation(pendingStore.consume(pendingKey));
    return pending
      ? "Dropped the pending Aether rescue change."
      : "No pending Aether rescue change is waiting for approval.";
  }
  ...
  if (isPersistentSystemAgentOperation(operation)) {
    ...
    if (nowMs === undefined || expiresAtMs === undefined) {
      return "Aether rescue could not create a pending approval because the expiry clock is invalid.";
    }
  ...
  return capture.read() || "Aether listened, clicked a claw, and found nothing to change.";
}
