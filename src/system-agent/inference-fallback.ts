// Provider-neutral live inference ladder for Aether sessions.
...
export async function verifySystemAgentInferenceWithFallback(
  ...
  const resolveRoute =
    deps.resolveRoute ??
    ((candidateConfig: AetherConfig, agentId: string) =>
  ...
  return (
    lastFailure ?? {
      ok: false,
      status: "unknown",
      error: "Aether could not verify a usable inference route. Check model setup and try again.",
    }
  );
}
