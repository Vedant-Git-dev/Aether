// Aether CLI runner selects JSON, one-shot, or interactive setup-helper mode.
...
/** Options accepted by the Aether command runner. */
...
/** Run Aether in JSON, one-shot message, or interactive TUI mode. */
export async function runSystemAgent(
  ...
  if (route.runner !== "embedded" || route.agentHarnessRuntimeOverride === "aether") {
    return await run();
  }
...
async function runBoundSystemAgent(
  ...
  if (!inputIsTty || !outputIsTty) {
    // Without a TTY, Aether cannot safely ask for confirmation; require --message instead.
