// Aether chat engine: stable transport-agnostic facade over turn and wizard owners.
import type {
...
/**
 * One conversation with Aether, independent of transport. The facade owns
  ...
  async resolveOperatorApproval(
    ...
    const turn = this.turnQueue.then(async () => {
      ...
      if (reply && terminalStatus && !reply.applied) {
        reply.text = `Aether change ${terminalStatus}. No change. Retry the request if it is still needed.`;
      }
      if (reply && decision === "allow-once" && !reply.applied) {
        reply.text += " Check the current settings and Aether status before retrying.";
      }
      if (reply?.text) {
