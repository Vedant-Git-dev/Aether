// Aether operation grammar, approval descriptions, and public types.
...
export type SystemAgentOperationResult = {
  ...
  message?: string;
  nextInput?: string;
  /** Agent TUI exited via /aether: re-enter the shell even without a request. */
...
const SETUP_RE = new RegExp(
  String.raw`^(?:setup|set\s+me\s+up|set\s+up\s+aether|onboard(?:\s+me)?|bootstrap|first\s+run)(?:\s+workspace\s+(?<workspace>${ARG_WORD}))?(?:\s+model\s+(?<model>\S+))?$`,
  "i",
);
...
/**
 * Parse one user command into Aether's closed operation union. Anything
...
export function parseSystemAgentOperation(input: string): SystemAgentOperation {
  ...
  switch (lower) {
    ...
    case "quit":
    case "exit":
      return { kind: "none", message: "Aether retracts into shell. Bye." };
    default:
      break;
  }
...
export function describeSystemAgentPersistentOperation(operation: SystemAgentOperation): string {
  switch (operation.kind) {
    ...
    case "doctor-fix":
      return "run aether vitals --fix on the machine running Aether, with Aether stopped";
    case "plugin-install":
...
export const SYSTEM_AGENT_OPERATOR_NAVIGATION_HANDOFF =
  "Channel, model, and setup flows need a human operator in the Aether app; they cannot run from a delegated agent request. Open `aether dashboard` or run `aether ignite` on the Gateway host.";
...
function formatSetupPlanDescription(
  ...
  return `bootstrap Aether setup for workspace ${workspace}`;
}
