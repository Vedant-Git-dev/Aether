...
export async function loadProviderAuthMethod(params: {
  ...
  { config: AetherConfig; provider: ProviderPlugin; method: ProviderAuthMethod } | StageFailure
> {
...
}): Promise<{ profile: ProviderAuthResult["profiles"][number]; config: AetherConfig }> {
...
export async function stageProviderAuthCandidate(
  ...
  if (interactive && authChoice === "custom-api-key") {
    if (params.isRemoteProviderAuth ?? params.surface === "gateway") {
      return {
        error:
          "For a custom provider, run aether awaken --auth-choice custom-api-key on the Gateway host, then return here and refresh connections.",
      };
    }
    if (!params.prompter) {
