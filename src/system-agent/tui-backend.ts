// Aether TUI backend runs setup-helper dialogue inside the shared local TUI shell.
...
class SystemAgentTuiBackend implements TuiBackend {
  readonly connection = { url: "aether local" };
  ...
  stop(): void {
    // The enclosing TUI owns terminal shutdown; Aether has no transport to close.
  }
  ...
  async listAgents(): Promise<TuiAgentsList> {
    return {
      ...
      mainKey: "main",
      scope: "per-sender",
      agents: [{ id: SYSTEM_AGENT_ID, kind: "system", name: "Aether" }],
    };
  }
  ...
  async patchSession(opts: SessionsPatchParams): Promise<SessionsPatchResult> {
    if (opts.model !== undefined) {
      throw new Error(
        "Aether cannot change the model inside its active verified session. Exit and run `aether awaken`, then start Aether again.",
      );
    }
    return {
      ok: true,
  ...
  async createSession(_opts: TuiSessionCreateOptions) {
    ...
    return {
      ok: true as const,
      key: SYSTEM_AGENT_SESSION_KEY,
      entry: { sessionId: "aether", updatedAt: Date.now() },
    };
  }
  ...
  private emitFinal(runId: string, sessionKey: string, text: string): void {
    const assistant = message(
      "assistant",
      text || "Aether listened and found nothing to change.",
    );
  ...
  private async respond(runId: string, sessionKey: string, text: string): Promise<void> {
    ...
    try {
      ...
      if ((reply.action === "open-tui" || reply.action === "open-setup") && reply.handoff) {
        // The outer loop owns interactive handoffs after the Aether TUI exits.
...
async function runSetupHandoff(
  ...
  if (
    ...
    runtime.error(
      "Setup cannot replace the inference route powering Aether. Exit and run `aether awaken`, then start Aether again.",
    );
    return;
  }
  ...
  if (handoff.target === "gateway") {
    if (opts.runGatewaySetupHandoff) {
      ...
      runtime.log("Done — gateway settings saved. Run `aether gateway restart` to apply them.");
      return;
    }
...
export async function runSystemAgentTui(
  ...
  for (;;) {
    ...
    // replay it when Aether re-enters the chat shell.
    ...
    if (handoff.kind === "model-setup") {
      runtime.error(
        "Aether cannot replace its active inference route. Run `aether awaken` outside this session, then start Aether again.",
      );
      return;
    }
