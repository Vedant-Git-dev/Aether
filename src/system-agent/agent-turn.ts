// Aether agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * Aether is a real agent: same loop, session transcript, and tool pipeline
 * as regular agents — restricted to the single ring-zero `aether` tool.
...
const SYSTEM_AGENT_TOOL_NAME = "aether";
...
async function ensureSystemAgentDirs(): Promise<{ workspaceDir: string }> {
  const base = path.join(resolveStateDir(), "aether");
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. Aether also forces a
...
function resolveSystemAgentCliToolAvailability(
  backend: ResolvedCliBackend | null,
): { native: []; aether: string[] } | undefined {
  ...
  if (
    ...
    return { native: [], aether: [SYSTEM_AGENT_TOOL_NAME] };
  }
  const backendId = backend?.id ?? "unknown";
  throw new Error(`CLI backend ${backendId} cannot enforce Aether's exact tool availability`);
}

/**
 * CLI harnesses run the aether tool in a stdio MCP subprocess, so the
...
async function mirrorSystemAgentToolStateFromEvents(params: {
  ...
  return onAgentEventForRun(params.runId, (evt) => {
    ...
    // CLI harnesses report MCP tools with transport prefixes (mcp__aether__aether).
    if (name !== "aether" && !name.endsWith("__aether")) {
      return;
    }
    const args =
...
/**
 * Run one Aether turn through the embedded agent loop. Route, runner, and
...
async function runSystemAgentTurnWithDeps(
  ...
  const runId = `aether-turn-${randomUUID()}`;
  ...
  const shared = {
    ...
    runId,
    messageChannel: "aether",
    messageProvider: "aether",
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.systemAgentTurnTestApi")] = {
