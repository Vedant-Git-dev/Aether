...
async function stageCodexCandidate(ctx: StageContext): Promise<StagedCandidate | StageFailure> {
  ...
  return await withPluginLifecycleLease({ signal: ctx.params.signal }, async () => {
    ...
    if (install?.source === "npm" && install.installPath) {
      ...
      if (
        !(await markRetained({
          ...
          pluginId: "codex",
          reason: "aether-inference-activation-not-committed",
        }))
      ) {
    ...
    let authenticatedConfig: AetherConfig = {
      ...config,
      plugins: {
        ...config.plugins,
        entries: {
...
async function stageCandidate(ctx: StageContext): Promise<StagedCandidate | StageFailure> {
  ...
  switch (params.kind) {
    ...
    case "anthropic-api-key": {
      ...
      return {
        modelRef,
        ...(params.kind === "gemini-cli" ? {} : { agentRuntimeId: "aether" }),
        config: cfg,
      };
    }
    default:
...
async function verifyAndActivateCandidate(
  ...
  const buildCandidate = (base: AetherConfig) => {
    let patched = base;
    if (!isRecord(providerPatch) || Object.keys(providerPatch).length > 0) {
      ...
      patched = applyMergePatch(base, providerPatch) as AetherConfig;
    }
      ...
      ? await withPluginLifecycleLease({ signal: params.signal }, async () =>
          loadSetupInferencePluginGeneration({
            ...
            selection: {
              ...
              runtime: staged.agentRuntimeId ?? "aether",
  ...
  const project = (config: AetherConfig, sourceConfig: AetherConfig) =>
    projectInferenceRoute(config, requestedAgentId, routeDeps, sourceConfig);
  const resolveRoute = (config: AetherConfig, currentSnapshot = snapshot) =>
    resolveSystemAgentConfiguredRouteFromConfig(
      config,
      requestedAgentId,
      routeDeps,
  ...
  if (params.surface === "gateway" && params.recordSetupAudit !== false) {
    ...
    try {
      await appendSystemAgentAuditEntry({
        operation: "aether.setup",
        summary: "Verified and configured AI access through Aether setup",
      ...
      const warning = `Inference setup completed, but Aether could not record its audit entry: ${formatErrorMessage(error)}`;
