...
async function assertArtifactConfigPublicationSupported(): Promise<void> {
  ...
  if (
    ...
    throw new Error(
      `${GUARDED_CONFIG_INCLUDE_WRITE_ERROR} Install the reviewed archive with aether plugins install.`,
    );
  }
}
...
async function readVerifiedArtifact(filePath: string, sha256: string): Promise<Buffer> {
  if (!path.isAbsolute(filePath) || !/\.(?:tgz|tar\.gz)$/u.test(filePath)) {
    throw new Error(
      "Plugin artifact path must be an absolute .tgz or .tar.gz file from aether plugins pack.",
    );
  }
...
async function inspectArtifact(rootDir: string): Promise<ArtifactReview> {
  ...
  for (const field of ["dependencies", "optionalDependencies", "scripts"]) {
    ...
    if (value !== undefined && (!isRecord(value) || Object.keys(value).length > 0)) {
      throw new Error(
        `Plugin artifacts cannot contain ${field}. Use aether plugins pack to bundle the plugin first.`,
      );
    }
  }
  ...
  if (
    (peers !== undefined &&
      (!isRecord(peers) || Object.keys(peers).some((name) => name !== "aether"))) ||
    (await artifact.exists("node_modules"))
  ) {
    throw new Error(
      "Plugin artifacts must bundle dependencies and may only reference the host aether peer.",
    );
  }
  const extensions = resolvePackageExtensionEntries(packageJson);
  if (extensions.status !== "ok" || extensions.entries.length !== 1) {
    throw new Error("Plugin artifact must declare exactly one compiled aether.extensions entry.");
  }
...
async function withVerifiedArtifact<T>(
  ...
  return await withInstallWorkspace("aether-plugin-artifact-", async (workspace) => {
    ...
    const artifactRoot = await resolvePackedRootDir(extractDir, {
      rootMarkers: ["package.json", "aether.plugin.json"],
    });
...
export async function prepareSystemAgentPluginArtifact(
  ...
  if (await isPluginBackingDefaultInferenceRoute(review.pluginId)) {
    throw new Error(
      "This plugin backs Aether's active inference route. Stop Aether and install the artifact from a trusted shell.",
    );
  }
      ...
      await withArtifactImports(async (files, assertOwned) => {
        ...
        return await withVerifiedArtifact(bytes, async ({ archivePath, review }) => {
          ...
          const guard = async () => {
            ...
            if (await isPluginBackingDefaultInferenceRoute(review.pluginId)) {
              throw new Error(
                "Artifact activation stopped: this plugin now backs the active inference route. Stop Aether and install it from a trusted shell.",
              );
            }
