...
// Aether overview gathers config, agent, tool, docs, source, and gateway status.
import { listAgentEntries, resolveAgentEffectiveModelPrimary } from "../agents/agent-scope.js";
import {
  AETHER_DOCS_URL,
  AETHER_SOURCE_URL,
  resolveAetherReferencePaths,
...
type AetherReferencePaths = Awaited<ReturnType<typeof resolveAetherReferencePaths>>;
...
type SystemAgentOverviewDependencies = {
  ...
  resolveAetherReferencePaths?: typeof resolveAetherReferencePaths;
};
...
function buildAgentSummaries(cfg: AetherConfig, defaultAgentId: string): SystemAgentSummary[] {
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): AetherReferencePaths | undefined {
...
export async function loadSystemAgentOverview(
  ...
  const resolveReferences = deps.resolveAetherReferencePaths ?? resolveAetherReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: AETHER_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: AETHER_SOURCE_URL,
    },
  };
}
...
export function formatSystemAgentOverview(overview: SystemAgentOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Aether online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `AI: ${
      overview.defaultModel
        ...
        : "inference unavailable; run aether awaken before starting Aether"
    }`,
...
function recommendSystemAgentNextStep(overview: SystemAgentOverview): string {
  if (!overview.config.exists) {
    return 'run "aether awaken" to establish inference';
  }
...
function formatStartupAction(overview: SystemAgentOverview): string | undefined {
  ...
  if (!overview.defaultModel) {
    return "Inference is unavailable. Run `aether awaken` and complete a live model check.";
  }
  return undefined;
}

/**
 * Welcome shown right after inference activation. Aether owns the
...
export function formatSystemAgentStartupMessage(overview: SystemAgentOverview): string {
  ...
  return [
    "Hi, I'm Aether — caretaker of this gateway, config, channels, and agents.",
