...
export type ActivateSetupInferenceParams = {
  ...
  onCommitStarted?: (sourceConfig: AetherConfig) => void;
...
export function invalidSetupConfigError(snapshot: {
  ...
  return `Aether config ${snapshot.path} is invalid${detail}. Fix it before running setup.`;
}
...
export function validateSetupInferenceOwnerEvidence(params: {
  ...
  if (
    ...
    return {
      ...
      status: "unknown",
      error:
        "Inference succeeded, but its runtime did not report an owner that Aether can safely reuse. No default model was changed.",
    };
  }
  if (
    params.runner === "cli" &&
