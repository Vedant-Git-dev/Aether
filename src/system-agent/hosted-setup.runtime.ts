...
export function requireLocalGateway(config: AetherConfig): void {
  if (config.gateway?.mode === "local") {
    return;
  }
  throw new Error(
    "Hosted Gateway setup manages only a local Gateway. Use `aether awaken` for fresh setup or `aether tune` for the mode question, then retry after selecting local mode.",
  );
}
...
export async function runHostedSetup(params: {
  ...
  run: (context: { baseConfig: AetherConfig; runtime: RuntimeEnv }) => Promise<
    | {
  ...
  if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
    throw new Error(
      `${params.label} requires a valid saved config snapshot. On the machine running Aether, run \`aether vitals --fix\` and resolve any remaining validation errors; then retry.`,
    );
  }
...
export async function runHostedMemoryImport(
  ...
  if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
    throw new Error(
      "Memory import requires a valid saved config. Run `aether vitals --fix`, then retry.",
    );
  }
...
export async function renderMemoryImport(
  ...
  if (outcome.status === "workspace-missing") {
    return [
      ...
      "Finish onboarding first with `aether awaken`, then retry.",
    ].join("\n");
  }
