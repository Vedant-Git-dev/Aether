...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. Aether config reloads create a new
...
export function resolveEffectiveCompactionMode(cfg?: AetherConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Aether runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (aether#75799).
 *
...
/**
 * Disable Aether runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Aether or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (aether#75799).
 * Default-mode runs against ordinary providers keep Aether runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Aether runtime's
