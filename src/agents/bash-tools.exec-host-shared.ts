...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  const approvalSurfaceFix =
    params.trigger === "cron" && params.host === "gateway"
      ? "- keep the Control UI or a connected desktop app available and answer the next run's approval card; Allow Always mints a standing grant"
  ...
  return [
    ...
    approvalSurfaceFix,
    'Tip: run "aether vitals" and "aether approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
