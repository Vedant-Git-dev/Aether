...
export function buildCredentialSafetyPrompt(
  ...
  return [
    ...
    ...(typeof input !== "string" && input?.controlToolsAvailable === false
      ? [
          "Channel, provider, and credential setup: use terminal `aether channels add <channel>` or `aether tune`; prompts mask secrets. Never collect tokens, API keys, or passwords in chat.",
        ]
      : []),
  ].join("\n");
}
