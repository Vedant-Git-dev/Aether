...
export type AgentDeletionOperation = {
  ...
  finish: () => void;
  completeInTransaction: (database: AetherStateDatabase) => void;
  rollback: () => void;
};
    ...
    async (lease) => {
      ...
      try {
        return await run((entry) => {
          ...
          const journal = runAetherStateWriteTransaction((database) => {
          ...
          const completeInTransaction = (database: AetherStateDatabase) => {
            assertCurrent(database);
          ...
          return {
            ...
            runDatabaseCleanup: createAgentDeletionDatabaseCleanup({
              statePath,
              assertAdmission: () => assertNoAetherAgentDatabaseLeases(id, stateOptions),
              assertCurrent,
              assertJournal,
              withCommit: (commit) => {
                let committed = false;
            ...
            finish: () => runAetherStateWriteTransaction(completeInTransaction, stateOptions),
            rollback: () =>
              mutateJournal(() => {
