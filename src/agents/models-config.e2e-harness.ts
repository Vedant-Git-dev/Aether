...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousAetherAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousAetherAgentDir = process.env.AETHER_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousAetherAgentDir === undefined) {
      delete process.env.AETHER_AGENT_DIR;
    } else {
      process.env.AETHER_AGENT_DIR = previousAetherAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const CUSTOM_PROXY_MODELS_CONFIG: AetherConfig = {
  models: {
    providers: {
      "custom-proxy": {
