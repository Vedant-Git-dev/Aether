...
import { closeAetherAgentDatabaseByPath } from "../state/aether-agent-db.js";
...
export function wrappedPluginSystemContext(text: string) {
  return `---\n\nAether plugin-injected system context. This block is not workspace file content.\n\n${text}\n\n---`;
}
    ...
    mcpServers: {
      aether: {
        ...
        headers: {
          ...
          "x-aether-cli-capture-key": "${AETHER_MCP_CLI_CAPTURE_KEY}",
        },
      },
    },
  };
...
export function createCliRunnerPrepareFixture(prepareCliRun: PrepareCliRun) {
  ...
  const hadStateDir = Object.hasOwn(process.env, "AETHER_STATE_DIR");
  ...
  const createSession = () => {
    const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "aether-cli-prepare-")));
    tempDirs.add(dir);
    process.env.AETHER_STATE_DIR = dir;
    const sessionTarget = {
      agentId: "main",
      sessionId: "session-test",
      sessionKey: "agent:main:main",
      storePath: path.join(dir, "agents", "main", "agent", "aether-agent.sqlite"),
    };
  ...
  return {
    ...
    cleanup() {
      ...
      for (const databasePath of databasePaths) {
        closeAetherAgentDatabaseByPath(databasePath);
      }
      databasePaths.clear();
      for (const dir of tempDirs) {
        closeAetherStateDatabaseByPath(
          resolveAetherStateSqlitePath({ AETHER_STATE_DIR: dir }),
        );
