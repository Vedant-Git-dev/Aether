/**
 * Aether stdio transport wrapper for MCP server subprocesses.
 */
...
type AetherStdioServerParameters = {
  command: string;
  args?: string[];
...
export class AetherStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: AetherStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.starting || this.closing) {
      throw new Error(
        "AetherStdioClientTransport already started or closed; Client.connect() starts transports automatically.",
      );
    }
