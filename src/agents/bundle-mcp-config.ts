...
const AETHER_TRANSPORT_TO_CLI_BUNDLE_TYPE: Record<string, string> = {
...
/**
 * User config stores Aether MCP transport names, while CLI backends such as
 * Claude Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded Aether still consumes the raw
 * Aether `transport` shape directly.
 */
export function toCliBundleMcpServerConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  ...
  if (typeof rawTransport === "string") {
    const mapped = AETHER_TRANSPORT_TO_CLI_BUNDLE_TYPE[rawTransport];
    if (mapped) {
      next.type = mapped;
    }
  }
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // Aether config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...Object.fromEntries(
