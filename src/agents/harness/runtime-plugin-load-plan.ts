...
function restrictiveAllowlistOmitsPlugin(config: AetherConfig | undefined, pluginId: string) {
...
function resolveAgentRuntimeMetadataPluginIds(params: {
  ...
  const runtimeIds = dedupePluginIds(
    selections
      ...
      .filter(
        (runtime) => !isDefaultAgentRuntimeId(runtime) && runtime !== AETHER_AGENT_RUNTIME_ID,
      ),
  );
...
export function requiresAgentHarnessPluginSelection(
  ...
  if (isDefaultAgentRuntimeId(runtime) || runtime === AETHER_AGENT_RUNTIME_ID) {
    return false;
  }
...
export function resolveAgentRuntimePluginLoadPlan(params: {
  ...
  selections: readonly AgentHarnessPluginSelection[];
  metadataSnapshot: PluginMetadataSnapshot;
}): { config?: AetherConfig; pluginIds?: string[] } {
