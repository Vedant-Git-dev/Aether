...
export function resolveAgentHarnessAvailabilityDecision(
  ...
  if (policy.runtime === "auto" || policy.runtime === "aether") {
  ...
  if (!registered) {
    return implicit && params.mode !== "projection"
      ? { kind: "implicit-unavailable", policy: { ...policy, runtime: "aether" } }
  ...
  if (!support.supported && !policy.forcedByEnvironment) {
    if (implicit || support.fallbackRuntime === "aether") {
      return {
        kind: implicit ? "implicit-unsupported" : "declared-fallback",
        policy: { ...policy, runtime: "aether" },
        support,
      };
    }
  }
