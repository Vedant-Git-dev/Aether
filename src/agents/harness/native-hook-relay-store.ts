...
import { withAetherStateDatabaseReadOnly } from "../../state/aether-state-db-readonly.js";
...
type NativeHookRelayBridgeRow = AetherStateKyselyDatabase["native_hook_relay_bridges"];
...
export function readNativeHookRelayBridgeRecord(
  ...
  return withAetherStateDatabaseReadOnly(
    (database) =>
...
export function pruneNativeHookRelayBridgeRecords(params: {
  ...
  const database = openAetherStateDatabase({ path: params.stateDbPath });
  const db = getNodeSqliteKysely<NativeHookRelayBridgeDatabase>(database.db);
