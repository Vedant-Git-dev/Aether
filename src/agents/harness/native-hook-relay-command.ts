...
function resolveNativeHookRelayExecutable(): string {
  const envPath = process.env.AETHER_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      ...
      path.join(packageRoot, "aether.mjs"),
  ...
  throw new Error("Cannot resolve Aether CLI executable path for native hook relay");
}
