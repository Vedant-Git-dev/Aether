...
import { createAetherAgentHarness, isBuiltInAetherAgentHarness } from "./builtin-aether.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_aether"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so Aether handled the run.
    | "implicit_plugin_unavailable_aether"
    // Implicit Codex preference cannot reproduce the prepared transport, so Aether handled it.
    | "implicit_plugin_unsupported_aether"
    // The requested plugin declared Aether as a lossless fallback for this prepared request.
    | "plugin_declared_fallback_aether"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_aether"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so Aether handled the run.
    | "auto_aether";
...
export function selectAgentHarnessForPreparedModelProviders(
  ...
  return (
    decisions.find((decision) => decision.selectedHarnessId === "aether")?.harness ??
    createAetherAgentHarness()
  );
}

/** Returns whether a plugin harness constructs Aether tools inside its runtime. */
export function agentHarnessBuildsAetherTools(harnessId: string): boolean {
  return harnessId === "codex" || harnessId === "copilot";
}

/** Returns whether the selected harness exposes Aether's agent-tool surface. */
export function agentHarnessExposesAetherTools(harnessId: string): boolean {
  return harnessId === "aether" || agentHarnessBuildsAetherTools(harnessId);
}
...
function selectAgentHarnessDecision(
  ...
  // Aether's built-in harness is intentionally not part of the plugin candidate list. Explicit plugin
  // runtimes fail closed unless the selected plugin declares Aether as a lossless fallback.
  const aetherHarness = createAetherAgentHarness();
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (!support || support.supported || support.fallbackRuntime === "aether") {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_aether",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
...
export async function runAgentHarnessSettledTurnFinalization(
  ...
  if (internalParams.systemAgentTool && !isSystemAgentOnlyAllowlist(internalParams.toolsAllow)) {
    throw new Error('Aether host authority requires toolsAllow: ["aether"]');
  }
  const attemptParams = prepareHarnessFinalizationParams(
    ...
    isBuiltInAetherAgentHarness(harness),
  );
  ...
  try {
    result = await runAgentHarnessOperation(harness, params, () =>
      runWithAgentRingZeroTools(ringZeroTools, () => {
        ...
        const hostAetherAuthority =
          isHostScopedAgentToolActive("aether") &&
        ...
        const effectiveAttemptParams =
          hostAetherAuthority && preparedParams.pluginHarnessToolPolicyRestricted
...
async function runAgentHarnessOperation<T>(
  ...
  if (isBuiltInAetherAgentHarness(harness)) {
  ...
  try {
    ...
    log.warn(`${harness.label} failed; not falling back to embedded Aether backend`, {
...
function isSystemAgentOnlyAllowlist(toolsAllow: readonly string[] | undefined): boolean {
  return toolsAllow?.length === 1 && normalizeToolPolicyName(toolsAllow[0] ?? "") === "aether";
}
...
function withoutPluginHarnessPrivateState(
  ...
  const {
    ...
    __aetherSourceReplyDeliveryRuntime: _sourceReplyDeliveryRuntime,
    ...pluginParams
  } = params as EmbeddedRunAttemptInternalParams & {
    __aetherSourceReplyDeliveryRuntime?: unknown;
  };
  return pluginParams;
}
...
function applyPluginHarnessDenyAllToolPolicy(
  ...
  if (
    ...
    normalizeToolPolicyName(params.toolsAllow[0] ?? "") === "aether"
  ) {
    return params;
  }
...
export function resolvePluginHarnessToolPolicies(
  ...
  return {
    ...
    // Native tools bypass the collector's noninteractive Aether wrappers.
...
function buildSelectionDecision(params: {
  ...
  const builtIn = isBuiltInAetherAgentHarness(params.harness);
  return {
