/**
 * Built-in Aether harness registration.
 *
 * Harness selection uses this factory to expose the embedded Aether runtime
 * through the same AgentHarness contract as external harness plugins.
 */
import { AETHER_EMBEDDED_CONTEXT_ENGINE_HOST } from "../../context-engine/host-compat.js";
...
const builtInAetherHarnesses = new WeakSet<object>();
...
/** Creates the built-in harness backed by the embedded Aether agent runner. */
export function createAetherAgentHarness(): AgentHarnessV2 {
  const harness: AgentHarnessV2 = {
    id: "aether",
    label: "Aether embedded agent",
    contextEngineHostCapabilities: AETHER_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
  ...
  builtInAetherHarnesses.add(harness);
  return harness;
}

/** Distinguishes the internal runtime from an untrusted harness that copies its public id. */
export function isBuiltInAetherAgentHarness(harness: AgentHarness): boolean {
  return builtInAetherHarnesses.has(harness);
}
