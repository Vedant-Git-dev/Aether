...
type MiddlewareContentBlock = AetherAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is AetherAgentToolResult {
...
function coerceMiddlewareToolResult(
  value: unknown,
  options: MiddlewareToolResultCoerceOptions = {},
): AetherAgentToolResult | undefined {
  ...
  const content: AetherAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: AetherAgentToolResult): AetherAgentToolResult {
...
function buildMiddlewareFailureResult(): AetherAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
function reconcileDeliveredMessagingFailure(
  result: AetherAgentToolResult,
  fallback: AetherAgentToolResult | undefined,
): AetherAgentToolResult {
