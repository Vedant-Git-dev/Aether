import { hasHttpUrlPrefix } from "@aether/net-policy/url-protocol";
...
export function compileStructuredInputForm(params: {
  ...
  if (!schema || ownString(schema, "type") !== "object" || !properties) {
    return unsupported(
      `Aether cannot show this ${protocol} form because its schema is not an object with properties.`,
    );
  }
  if (!structuredInputEntries(schema, MAX_SCHEMA_KEYS)) {
    return unsupported(`Aether declined an over-limit ${protocol} form schema.`);
  }
  const propertyEntries = structuredInputEntries(properties, MAX_FORM_FIELDS);
  if (!propertyEntries) {
    return unsupported(
      `Aether supports at most ${MAX_FORM_FIELDS} fields in one ${protocol} form.`,
    );
  }
  if (propertyEntries.length === 0 && options.allowEmptyForm !== true) {
    return unsupported(`Aether cannot show an empty ${protocol} form.`);
  }
  ...
  if (!intro) {
    return unsupported(
      `Aether declined ${protocol} form display text that is invalid or over-limit.`,
    );
  }
  ...
  for (const [fieldId, rawSchema] of propertyEntries) {
    ...
    if (fieldMetadata.otherAnswer) {
      ...
      if (!target || otherFields.has(target)) {
        return unsupported(`Aether declined invalid ${protocol} Other-field metadata.`);
      }
  ...
  for (const target of otherFields.keys()) {
    if (!Object.hasOwn(properties, target)) {
      return unsupported(`Aether declined ${protocol} Other-field metadata without its target.`);
    }
  }
  ...
  if (fields.length === 0 && propertyEntries.length > 0) {
    return unsupported(`Aether cannot show a ${protocol} form containing only synthetic fields.`);
  }
...
export function compileStructuredInputUrl(params: {
  ...
  if (
    ...
    return unsupported(
      `Aether declined an invalid or over-limit ${params.protocolName} elicitation URL.`,
    );
  }
  let parsed: URL;
  try {
    ...
    return unsupported(`Aether declined an invalid ${params.protocolName} elicitation URL.`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    return unsupported(
      `Aether only presents http or https ${params.protocolName} elicitation URLs.`,
    );
  }
  if (parsed.username || parsed.password) {
    return unsupported(
      `Aether does not present ${params.protocolName} elicitation URLs containing credentials.`,
    );
  }
  return {
    kind: "ready",
...
function readRequired(
  ...
  if (!Array.isArray(value) || value.length > MAX_FORM_FIELDS) {
    return `Aether declined a ${protocol} form with an invalid required list.`;
  }
  const required = new Set<string>();
  for (const entry of value) {
    if (typeof entry !== "string" || !Object.hasOwn(properties, entry)) {
      return `Aether declined a ${protocol} form with an invalid required field.`;
    }
    required.add(entry);
  }
  return required;
