...
function describeMissingHarnessRegistration(
  ...
  if (failedOwner) {
    ...
    return `(reason=owner-plugin-degraded, ownerPluginId=${failedOwner.id}). Run "aether plugins inspect ${failedOwner.id} --runtime --json". Owner plugin "${failedOwner.id}" failed during ${phase}. Repair the reported plugin failure, restart the Gateway, then retry or select a model that does not require this runtime.`;
  }
  const loadedOwner = ownerPluginIds
    .map((pluginId) => pluginRegistry?.plugins.find((plugin) => plugin.id === pluginId))
    .find((plugin) => plugin?.status === "loaded");
  if (loadedOwner) {
    return `(reason=owner-plugin-degraded, ownerPluginId=${loadedOwner.id}). Run "aether plugins inspect ${loadedOwner.id} --runtime --json". Owner plugin "${loadedOwner.id}" loaded but did not register agent harness "${runtime}". Repair the reported plugin failure, restart the Gateway, then retry or select a model that does not require this runtime.`;
  }
  ...
  return `(${reason}${ownerField}=${ownerPluginIds.slice(0, 3).join(",")}). Run "aether vitals --fix". ${detail}. Repair the plugin or select a model that does not require this runtime, restart the Gateway, then retry.`;
}
...
export async function ensureSelectedAgentHarnessPlugin(params: {
  ...
  if (
    ...
    runtime === AETHER_AGENT_RUNTIME_ID ||
