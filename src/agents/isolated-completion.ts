...
async function runCliIsolatedCompletion(params: {
  request: RunIsolatedCompletionParams & { config: AetherConfig };
  provider: string;
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredAetherTmpDir(), prefix: "aether-isolated-completion-" },
    async ({ dir }) => {
...
async function resolveHarness(runtime: string): Promise<AgentHarness> {
  if (runtime === "aether") {
    const { createAetherAgentHarness } = await import("./harness/builtin-aether.js");
    return createAetherAgentHarness();
  }
