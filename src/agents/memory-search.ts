...
function getConfiguredMemoryEmbeddingProvider(providerId: string, cfg: AetherConfig) {
...
export function resolveMemorySearchIndexConfig(cfg: AetherConfig, agentId: string) {
...
function produceMemorySearchConfig(cfg: AetherConfig, agentId: string) {
  ...
  const store = {
    ...
    databasePath: resolveAetherAgentSqlitePath({ agentId, env: process.env }),
    fts,
    vector,
  };
  const chunking = {
