...
export type ModelCatalogDecisionParams = {
  ...
  observationConfig?: AetherConfig;
  preferredProfileId?: string;
  pinnedProfileId?: string;
  profileProvider?: string;
  runtimeOverride?: string;
...
export function resolveCatalogDecisionRuntime(params: {
  ...
  const select = () => {
    ...
    return {
      ...
      runtime:
        policy.runtime === "auto"
          ? (resolveAutoAgentHarnessId(context) ?? "aether")
          : policy.runtime,
    };
  };
    ...
    : (() => {
        ...
        return {
          ...
          runtime:
            ...
            (policy.runtime === "auto" ? "aether" : policy.runtime),
        };
      })();
  // Route projection must retain the native owner that supplied availability. Recomputing
  // implicit policy from its API-key route alone would relabel that owner as Aether.
  const runtime =
