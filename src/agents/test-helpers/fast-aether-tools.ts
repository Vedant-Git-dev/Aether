...
const createAetherToolsMock = vi.fn(
  (options?: { enableHeartbeatTool?: boolean; recordToolPrepStage?: (name: string) => void }) => {
    options?.recordToolPrepStage?.("aether-tools:test-helper");
    return coreTools
...
vi.mock("../aether-tools.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../aether-tools.js")>();
  return {
    createAetherTools: createAetherToolsMock,
