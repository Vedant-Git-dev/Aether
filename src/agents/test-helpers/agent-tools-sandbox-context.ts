...
export function createAgentToolsSandboxContext(
  ...
  return {
    ...
    runtimeId: params.containerName ?? "aether-sbx-test",
    runtimeLabel: params.containerName ?? "aether-sbx-test",
    containerName: params.containerName ?? "aether-sbx-test",
    containerWorkdir: params.containerWorkdir ?? "/workspace",
    fsBridge: params.fsBridge,
    docker: {
      image: "aether-sandbox:bookworm-slim",
      containerPrefix: "aether-sbx-",
      workdir: "/workspace",
      readOnlyRoot: true,
      tmpfs: [],
      network: "none",
