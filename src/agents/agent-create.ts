...
type CreateAgentResult =
  | (CreateAgentSuccess & { config: AetherConfig; configPath: string })
  | CreateError;
type AgentEntryConfig = NonNullable<NonNullable<AetherConfig["agents"]>["entries"]>[string];
...
type CreateAgentParams = {
  ...
  stagedConfig?: { config: AetherConfig; writeSnapshot: ReadConfigFileSnapshotForWriteResult };
  workspace?: string;
  model?: string;
  emoji?: unknown;
  avatar?: unknown;
...
async function evaluateMainCreationGate(
  ...
  if (blocked) {
    ...
    return createError(
      "legacy-session-migration-required",
      `Cannot create agent "main": ${details}. Run aether vitals --fix, then retry.`,
      agentId,
    );
  }
  ...
  if (resolveSharedAuthStoreOwnership().location !== "state-db") {
    return createError(
      "shared-auth-store-owned-by-main",
      'Cannot create agent "main" while agents/main/agent owns the shared auth store. Run aether vitals --fix to relocate shared auth, then retry.',
      agentId,
    );
  }
  return undefined;
