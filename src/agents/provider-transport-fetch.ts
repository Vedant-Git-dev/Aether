import { emitModelTransportDebug, formatModelTransportDebugUrl } from "@aether/ai/diagnostics";
/**
 ...
 */
import { parseRetryAfterHeadersSeconds as parseRetryAfterSeconds } from "@aether/ai/internal/retry-after";
import {
...
function resolveMaxSdkRetryWaitSeconds(): number | undefined {
  const raw = process.env.AETHER_SDK_RETRY_MAX_WAIT_SECONDS?.trim();
  if (!raw) {
...
function withModelProviderNetworkRemediation(
  ...
  return new SsrFBlockedError(
    `Configured model provider ${params.providerId} uses local-use NAT64 origin ` +
      `${baseOrigin}, which Aether blocks by default. Move the provider to a ` +
