 ...
 */
import { resolveIntegerOption } from "@aether/normalization-core/number-coercion";
...
import { isTranscriptOnlyAetherAssistantModel } from "../shared/transcript-only-aether-assistant.js";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000AETHER_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[Aether persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[Aether persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[Aether persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[Aether persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[Aether persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyAetherAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyAetherAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (
    ...
    const transcriptOnly =
      ...
      (nextRole === "assistant" &&
        toolCalls.length === 0 &&
        isTranscriptOnlyAetherAssistantMessage(nextMessage));
    if (!transcriptOnly) {
