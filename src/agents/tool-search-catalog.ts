...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "aether", sourceName: pluginId };
  }
  return { source: "aether", sourceName: "core" };
}
...
export function prepareToolSearchCatalogExecutionTool(
  ...
  const validateInput = options.validateInput && entry.source === "aether";
  if (!prepareInput && !validateInput) {
    return entry.tool;
  }
  // SAFETY: both gates above restrict wrapper execution to Aether-owned catalog tools.
...
function toCatalogEntry(
  ...
  return {
    ...
    ...(source === "aether" && (tool as AnyAgentTool).outputSchema
 ...
 * resolve to trusted Aether tools: an MCP lookalike must never become a
...
function getTelemetry(catalog: ToolSearchCatalogSession): ToolSearchCatalogTelemetry {
  const sources: Record<CatalogSource, number> = { aether: 0, mcp: 0, client: 0 };
...
export function compactToolSearchCatalogEntry(entry: ToolSearchCatalogEntry) {
  const output =
    entry.source === "aether" ? compactToolOutputHint(entry.outputSchema) : undefined;
  ...
  return {
    ...
    input: entry.source === "aether" ? compactToolInputHint(entry.parameters) : "unknown",
    ...(output ? { output } : {}),
  };
}
