...
import { isIncognitoAetherAgentSqlitePath } from "../state/aether-agent-db.js";
...
export function resolveInternalSessionEffectsTarget(params: {
  ...
  const incognito = isIncognitoAetherAgentSqlitePath(params.storePath, {
...
export async function prepareInternalSessionEffectsSession(params: {
  ...
  const entry = await upsertSessionEntryCore(scope, {
    ...
    ...(isIncognitoAetherAgentSqlitePath(params.storePath, { agentId: params.agentId })
