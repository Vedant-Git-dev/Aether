...
type SkillDiscoveryMode = "aether" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { aether?: ResourceManifest };
    return pkg.aether ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "aether");
  }
...
const AUTO_RESOURCE_COLLECTORS = {
  ...
  skills: (dir: string) => collectAutoSkillEntries(dir, "aether"),
