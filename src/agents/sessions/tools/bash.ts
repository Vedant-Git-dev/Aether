...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.bashToolTestApi")] = {
    resolveBashTimeoutMs,
  };
}

/**
 * Create bash operations using Aether runtime's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want Aether runtime's
...
export function createBashToolDefinition(
  ...
  return {
    ...
    async execute(
      ...
      const output = new OutputAccumulator({ tempFilePrefix: "aether-bash" });
