 ...
 *
 * The backend contract keeps the upstream session SDK shape while Aether
...
import { findEnvKeys, getEnvApiKey } from "@aether/ai/internal/runtime";
...
class AuthStorageLegacyPathMigrationRequiredError extends Error {
  ...
  constructor() {
    super(
      "Deprecated AuthStorage path contains unmigrated credentials; run aether vitals --fix for standard agent auth.json or migrate plugin storage to AuthStorage.forAgent(agentDir).",
    );
...
export class AuthStorage {
  ...
  static forAgent(agentDir: string = getAgentDir(), config?: AetherConfig): AuthStorage {
