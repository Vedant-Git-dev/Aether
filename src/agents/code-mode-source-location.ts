...
export const USER_SOURCE_FILE = "aether-code-mode:user.js";
const GENERATED_SOURCE_FILE = "aether-code-mode:generated.js";
export const SOURCE_LOCATION_KEY = "__aetherSourceLocation";
...
export function normalizeSourceStack(
  ...
  return stack.replace(
    /^[^\S\r\n]+at [^\r\n]*aether-code-mode:(?:user|controller)\.js:\d+:\d+\)?(?:\r?\n|$)/gmu,
    (frame) => {
      const match = /aether-code-mode:user\.js:(\d+):(\d+)(?=\)?(?:\r?\n)?$)/u.exec(frame);
      if (!match) {
        return "";
      }
...
export function buildUserSource(
  ...
  const prefix = `globalThis.__aetherResult = (async () => {\n${prelude}`;
