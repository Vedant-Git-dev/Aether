...
export { resolvePrimaryStringValue as normalizeModelSelection } from "@aether/normalization-core/string-coerce";
...
function listConfiguredModelMaps(cfg: AetherConfig, agentId?: string) {
  return [
...
export function listModelAliasCandidates(cfg: AetherConfig, agentId?: string) {
...
function hasConfiguredProviderModelRows(cfg: AetherConfig): boolean {
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: AetherConfig): boolean {
  const providers = cfg.models?.providers;
  if (!providers || typeof providers !== "object") {
    return false;
  }
