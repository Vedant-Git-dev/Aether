import { OPENAI_RESPONSES_APIS } from "@aether/ai/internal/openai-responses-payload-policy";
/**
...
function resolveTranscriptLoggingConfig(cfg?: AetherConfig) {
...
const GOOGLE_REASONING_APIS = new Set([
  ...
  "google-vertex",
  "google-gemini-cli",
  "aether-google-generative-ai-transport",
]);
const ANTHROPIC_REASONING_APIS = new Set([
  "anthropic-messages",
  "bedrock-converse-stream",
  "aether-anthropic-messages-transport",
]);
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "aether-openai-completions-transport",
]);
...
function isGitHubCopilotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.api === "openai-responses" || route?.api === "aether-openai-responses-transport") &&
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__aether_replay";
...
function redactTranscriptStructuredValue(
  ...
  if (location === "root" && source.role === "user" && next && next.content !== source.content) {
    const metadata = asOptionalRecord(next["__aether"]);
