...
const modelFallbackStops = resolveGlobalSingleton(
  Symbol.for("aether.modelFallbackStops"),
  () => new WeakSet<Error>(),
);
...
export function buildFailoverRemediationHint(err: unknown): string | undefined {
  ...
  if (provider === "google-gemini-cli") {
    return `Authenticate in Gemini CLI directly, or configure a supported Google API key with: ${formatCliCommand("aether tune")}`;
  }
...
export function buildProviderReauthCommand(
  ...
  return formatCliCommand(
    `aether models auth login --provider ${quotePosixShellArg(trimmed)} --force`,
    env,
  );
}
