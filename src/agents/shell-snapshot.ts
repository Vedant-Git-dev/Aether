...
const CAPTURE_MARKER = "__AETHER_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__AETHER_SHELL_SNAPSHOT_ENV__";
const EXEC_SHELL_SNAPSHOT_ENV = "AETHER_EXEC_SHELL_SNAPSHOT";
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // AETHER_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.AETHER_SHELL === "exec") {
    env.AETHER_SHELL = "exec";
  }
  return env;
}
...
function buildSnapshotFile(stdout: string): string | null {
  ...
  return [
    "# Aether exec shell snapshot. Generated; do not edit.",
