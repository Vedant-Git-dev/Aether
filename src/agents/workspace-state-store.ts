...
type WorkspaceStateDatabaseHandle = Pick<AetherStateDatabase, "db" | "path">;
  ...
  if (setupRow?.version != null && setupRow.version !== WORKSPACE_SETUP_STATE_VERSION) {
    throw new Error(
      formatDoctorStateRepairFailure(
        ...
        "Use a compatible Aether build that supports this workspace version; preserve the database unchanged.",
      ),
    );
  }
...
export async function readWorkspaceStateSnapshot(
  workspaceDir: string,
  options: AetherStateDatabaseOptions & WorkspaceStateOperationOptions = {},
): Promise<WorkspaceStateSnapshot> {
  if (options.readOnly) {
    const snapshot = withExistingAetherStateDatabaseReadOnly(
      (database) =>
  ...
  return runAetherStateWriteTransaction((writeDatabase) => {
...
export async function deleteWorkspaceState(
  ...
  if (!existsSync(resolveAetherStateSqlitePath())) {
    options.assertCurrent?.();
