...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const aetherTmpDir = path.resolve(resolvePreferredAetherTmpDir());
  if (!isPathInside(aetherTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: aetherTmpDir });
  return resolved;
}
