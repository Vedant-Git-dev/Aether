...
import { AETHER_AGENT_RUNTIME_ID, normalizeOptionalAgentRuntimeId } from "../agent-runtime-id.js";
import {
...
type CliCompactionDeps = {
  ...
  resolveContextEngine: (cfg: AetherConfig) => Promise<ContextEngine>;
...
function isNativeHarnessCompactionSession(
  ...
  if (!harnessId || normalizeOptionalAgentRuntimeId(harnessId) === AETHER_AGENT_RUNTIME_ID) {
    return false;
  }
...
export async function runCliTurnCompactionLifecycle(
  ...
  if (
    params.sessionEntry?.modelSelectionLocked === true &&
    lockedHarnessRuntime !== AETHER_AGENT_RUNTIME_ID &&
