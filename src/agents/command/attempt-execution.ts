 ...
 */
import type { AcpRuntimeEvent } from "@aether/acp-core/runtime/types";
...
export function runAgentAttempt(params: {
  ...
  const requestedAgentHarnessId = isRawModelRun ? "aether" : undefined;
  ...
  const agentHarnessPolicy = isRawModelRun
    ? ({ runtime: "aether", runtimeSource: "model" } as const)
    : sessionRuntimeOverride
  ...
  const embeddedAgentHarnessOverride =
    ...
    sessionRuntimeOverride ??
    (agentHarnessPolicy.runtime === "aether" && agentHarnessPolicy.runtimeSource !== "implicit"
      ? "aether"
      : undefined);
    ...
    return withLocalSessionPlacementTurnSettlement(
      ...
      async (assertSettlementCurrent) => {
        ...
        const resolveReusableCliSessionBinding = async () => {
          ...
          // raw-transcript reseed from prior Aether history. Returning undefined
