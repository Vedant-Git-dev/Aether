 ...
 */
type ModelsConfig = NonNullable<AetherConfig["models"]>;
/** Provider config entry from the canonical Aether models config. */
export type ProviderConfig = NonNullable<ModelsConfig["providers"]>[string];

/** Default secret reference sources applied when config omits an explicit source. */
export type SecretDefaults = NonNullable<NonNullable<AetherConfig["secrets"]>["defaults"]>;
...
export function normalizeHeaderValues(params: {
  ...
  source?: { config: AetherConfig; providerKey: string };
