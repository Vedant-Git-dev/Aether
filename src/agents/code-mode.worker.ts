...
// (e.g. "at aether-code-mode:user.js:2:37"). Lead with name+message so the
...
function trackPromiseRejection(
  ...
  const vm = promise.vm;
  vm.global
    .getProp("__aetherTrackRejection")
    .consume((track) =>
...
async function createVm(input: CodeModeWorkerPayload, bridge: BridgeState): Promise<VmRun> {
  ...
  try {
    ...
    const callbacks = [
      ["__aetherHostRequest", createHostRequestHandler({ vm, bridge, config: input.config })],
      ["__aetherHostCancelRequest", createHostCancelRequestHandler({ vm, bridge })],
    ] as const;
    ...
    if (input.kind === "exec") {
      for (const [name, value] of [
        ["__aetherCatalog", input.catalog],
        ["__aetherNamespaces", input.namespaces],
        ["__aetherApiFiles", input.apiFiles ?? []],
        ["__aetherSwarmEnabled", input.swarmEnabled === true],
        ["__aetherMaxPendingToolCalls", input.config.maxPendingToolCalls],
      ] as const) {
        vm.hostToHandle(value).consume((handle) => vm.global.setProp(name, handle));
      }
      vm.evalCode(CODE_MODE_CONTROLLER_SOURCE, "aether-code-mode:controller.js").dispose();
    }
    return {
      vm,
...
function takeOutput(vm: QuickJS): unknown[] {
  return vm.global.getProp("__aetherTakeOutput").consume((take) =>
...
function serializeCompletedCatalogHandles(vm: QuickJS, value: JSValueHandle): unknown {
  return vm.global
    .getProp("__aetherSerializeCatalogHandles")
    .consume((serialize) =>
  ...
  try {
    for (;;) {
      ...
      const admissionError = params.vm.global
        .getProp("__aetherAdmissionError")
        .consume((read) =>
          params.vm
      ...
      try {
        ...
        using rejection = params.vm.global
          .getProp("__aetherUnhandledRejection")
...
function settleRequests(vm: QuickJS, requests: SettledBridgeRequest[]): void {
  try {
    vm.global.getProp("__aetherSettleBridge").consume((settle) => {
