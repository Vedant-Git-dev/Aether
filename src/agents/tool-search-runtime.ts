...
function toolSearchEntryText(entry: ToolSearchCatalogEntry, parameterText?: string): string {
  ...
  const parameters =
    parameterText ?? (entry.source === "aether" ? readParameterText(entry.parameters) : "");
    ...
    entries.every((entry, index) => {
      ...
      return (
        ...
        snapshot.parameterText ===
          (entry.source === "aether" ? readParameterText(entry.parameters) : "")
      );
    })
  );
}
...
export class ToolSearchRuntime {
  ...
  private readonly executeEntry = async (
    ...
    const validateInput = this.options.validateInput && entry.source === "aether";
