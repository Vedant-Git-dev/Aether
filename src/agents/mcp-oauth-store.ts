...
import { ensureMcpOAuthPendingSchema } from "../state/aether-state-db-schema-additive.js";
...
export function readMcpOAuthStore(storeKey: string): McpOAuthStore {
  return readFromDatabase(openAetherStateDatabase().db, storeKey);
}
...
function runPendingWrite<T>(run: (database: DatabaseSync) => T): T {
  ensurePendingSchema(openAetherStateDatabase().db);
  return runAetherStateWriteTransaction(({ db }) => run(db));
}
...
export function readMcpOAuthPendingAuthorization(state: string): string | undefined {
  ...
  return withExistingAetherStateDatabaseReadOnly(({ db }) => {
