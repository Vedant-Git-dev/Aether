 ...
 *
 * See: https://github.com/aether/aether/issues/17328
 */
...
function buildMissingProviderModelRegistrationHint(params: {
  ...
  if (normalizeProviderId(params.provider) === "openai-codex") {
    return `"openai-codex" is a legacy provider ID. Run \`aether vitals --fix\` to migrate legacy model and provider config to the current OpenAI format. If the provider has no authenticated profile, run \`aether models status\` to check provider auth and re-authenticate if needed. See https://docs.aether.ai/concepts/model-providers.`;
  }
