 ...
 */
import type { LlmRuntime } from "@aether/ai";
import { notifyLlmRequestActivity, onLlmRequestActivity } from "@aether/ai/internal/runtime";
import { stripSystemPromptCacheBoundary } from "@aether/ai/internal/shared";
import { createBoundaryAwareStreamFnForModel } from "@aether/ai/transports";
import { hasNonEmptyString as hasResolvedRuntimeApiKey } from "@aether/normalization-core/string-coerce";
...
function isDefaultAetherStreamFnForModel(
...
function resolveAetherNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  currentStreamFn: StreamFn | undefined;
  llmRuntime: LlmRuntime;
}): StreamFn | undefined {
  ...
  if (
    !isDefaultAetherStreamFnForModel(params.model, params.currentStreamFn, params.llmRuntime) &&
...
export function resolveEmbeddedAgentStream(
  ...
  const nativeStreamFn = resolveAetherNativeCodexResponsesStreamFn({
    model: params.model,
  ...
  if (nativeStreamFn) {
    return {
      ...
      strategy: "aether-native-codex-responses",
    };
  }

  const isDefault = isDefaultAetherStreamFnForModel(
    params.model,
