...
import type { createAetherCodingTools } from "../agent-tools.js";
...
export const resolveAgentHarnessPolicyMock = vi.fn(() => ({ runtime: "aether" }));
function createSelectedAgentHarnessMock(params: {
  ...
  const id =
    params.agentHarnessId ?? params.agentHarnessRuntimeOverride ?? configured.runtime ?? "aether";
  return {
    id,
...
export const createAetherCodingToolsMock = vi.fn<typeof createAetherCodingTools>(() => []);
...
export function resetCompactSessionStateMocks(): void {
  ...
  resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "aether" });
...
export function resetCompactHooksHarnessMocks(workspaceDir: string): void {
  ...
  createAetherCodingToolsMock.mockReset();
  createAetherCodingToolsMock.mockReturnValue([]);
...
export async function loadCompactHooksHarness(options: { durableSession?: boolean } = {}): Promise<{
  ...
  vi.doMock("../models-config.js", () => ({
    ensureAetherModelsJson: vi.fn(async () => {}),
  }));
  ...
  vi.doMock("../docs-path.js", () => ({
    resolveAetherReferencePaths: vi.fn(async () => ({
      docsPath: undefined,
  ...
  vi.doMock("../agent-tools.js", () => ({
    createAetherCodingTools: createAetherCodingToolsMock,
  }));
