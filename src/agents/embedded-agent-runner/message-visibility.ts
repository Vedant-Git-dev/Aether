...
export function readTerminalSourceReplyDeliveryMirror(
  ...
  const marker = (message as { aetherDeliveryMirror?: unknown }).aetherDeliveryMirror;
...
export function isIntermediateAssistantTranscriptMessage(message: unknown): boolean {
  ...
  const asyncDelivery = record.aetherAsyncDelivery;
  ...
  const fallback = record.aetherStreamFallback;
