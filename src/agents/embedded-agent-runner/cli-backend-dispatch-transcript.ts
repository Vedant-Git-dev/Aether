 ...
 * no Aether transcript records, but one-shot callers (e.g. active-memory
...
export function createCliDispatchTranscriptRecorder(params: {
  ...
  const buildZeroUsageAssistantMessage = (
    ...
    return tainted ? ({ ...message, __aether: { turnTainted: true } } as AgentMessage) : message;
  };
  ...
  enqueue(() => ({
    ...
    ...(params.senderIsOwner !== undefined
      ? { __aether: { senderIsOwner: params.senderIsOwner } }
      : {}),
  }));
    ...
    noteToolEvent: (event) => {
      ...
      enqueue(() => ({
        ...
        ...(event.resultContentSource
          ? { __aether: { resultContentSource: event.resultContentSource } }
          : {}),
      }));
    },
