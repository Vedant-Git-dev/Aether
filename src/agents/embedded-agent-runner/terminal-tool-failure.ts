...
// phrasing is `tools.*` from the exec bridge and `aether.tools.*` from the
// gated tool-search surface. Match the exact formatter line on the first line.
const SAFE_MCP_CATALOG_MISS =
  /^(?:Error: )?Unknown tool id: MCP\.[A-Za-z0-9][A-Za-z0-9._-]*\. (?:Did you mean: [^\r\n]+\? )?Use (?:aether\.tools\.search to find a tool, aether\.tools\.describe to inspect it, then aether\.tools\.call|tools\.search to find a tool, tools\.describe to inspect it, then tools\.call) with the exact id or name\.$/;
