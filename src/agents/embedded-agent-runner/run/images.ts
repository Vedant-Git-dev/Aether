...
import { MAX_VIDEO_BYTES } from "@aether/media-core/constants";
import { normalizeMimeType } from "@aether/media-core/mime";
...
export function detectImageReferences(prompt: string): MediaFileRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isAetherCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
...
async function materializePromptMediaMessages(
  ...
  for (const [index, message] of messages.entries()) {
    ...
    const meta = Reflect.get(message, "__aether");
    ...
    if (Object.keys(nextMeta).length > 0) {
      Reflect.set(hydratedMessage, "__aether", nextMeta);
    } else {
      Reflect.deleteProperty(hydratedMessage, "__aether");
    }
    if (runtimeMedia) {
