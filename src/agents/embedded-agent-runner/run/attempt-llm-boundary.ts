...
import {
  ...
  AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE,
...
const runtimeContextDetailsSchema = z.object({
  source: z.literal("aether-runtime-context"),
...
function projectRuntimeContextMessages(messages: AgentMessage[]): AgentMessage[] {
  return messages.map((message) => {
    if (message.role === "custom" && message.customType === AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE) {
 ...
 * are NOT collapsed.
 *
 * @see https://github.com/aether/aether/issues/3658
 */
...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const aether = Reflect.get(message, "__aether");
    if (!aether || typeof aether !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (aether as { beforeAgentRunBlocked?: unknown })
    ...
    const nextAether = {
      ...(aether as Record<string, unknown>),
