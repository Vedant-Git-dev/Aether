...
export function isAetherCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".aether-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "aether-cli-images" && /^aether(?:-\d+)?$/.test(parent);
  });
}
...
function mediaFactToImageRef(fact: MediaFact, factIndex: number): MediaImageRef | undefined {
  ...
  if (!localRef || isAetherCliImageCachePath(localRef.resolved)) {
    return {
