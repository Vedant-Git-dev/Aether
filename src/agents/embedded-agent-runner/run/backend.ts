/**
 * Dispatches embedded attempts to native harness or Aether backend execution.
 */
import {
...
export async function runEmbeddedAttemptWithBackend(
  ...
  let requesterContinuationSettled =
    result.agentHarnessId === "aether" && result.requesterContinuationSettled === true;
  if (
    result.agentHarnessId !== "aether" &&
    params.sessionKey &&
