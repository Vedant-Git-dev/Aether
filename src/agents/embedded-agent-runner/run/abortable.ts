...
const AETHER_ABORTABLE_WRAPPER = Symbol.for("aether.abortable.wrapper");

export function isAetherAbortableWrapper(err: unknown): boolean {
  return err !== null && typeof err === "object" && AETHER_ABORTABLE_WRAPPER in err;
}

function tagAsAbortableWrapper(err: Error): Error {
  (err as Error & { [AETHER_ABORTABLE_WRAPPER]?: true })[AETHER_ABORTABLE_WRAPPER] = true;
  return err;
}
