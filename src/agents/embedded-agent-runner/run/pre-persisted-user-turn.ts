...
import { AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../../internal-runtime-context.js";
import {
  ...
  const readCurrentTurn = () => {
    ...
    const userId = sessionManager.resolveCurrentTurnEntryId((entry) => {
      if (entry.type === "custom_message") {
        return (
          entry.customType === "aether:turn-aborted" ||
          entry.customType === AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE
        );
      }
      ...
      return (
        ...
        Reflect.get(aborted, "__aether")?.runId === runId &&
        (errorCode !== undefined
