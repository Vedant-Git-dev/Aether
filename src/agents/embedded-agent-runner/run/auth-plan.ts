...
export async function prepareEmbeddedRunAuthPlan(params: {
  ...
  const initialPluginHarnessOwnsTransport = initialHarness.id !== "aether";
  const aetherNativeCodexResponsesNeedsAuthBootstrap =
  ...
  let externalCliAuthScope = initialPluginHarnessOwnsTransport
    ...
    : aetherNativeCodexResponsesNeedsAuthBootstrap
      ? {
