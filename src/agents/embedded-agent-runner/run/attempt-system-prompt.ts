...
const ATTEMPT_PROMPT_SECTION =
  /<!-- aether:attempt:(STABLE|DYNAMIC|PERMISSION) -->[\s\S]*?<!-- \/aether:attempt:\1 -->/g;

function renderAttemptPromptSection(section: "STABLE" | "DYNAMIC" | "PERMISSION", text: string) {
  return `<!-- aether:attempt:${section} -->\n${text}\n<!-- /aether:attempt:${section} -->`;
}
...
export function buildAttemptSystemPrompt(
  ...
  const systemPrompt = params.isRawModelRun
    ? ""
    : splitPrompt
      ? `${stablePrompt}${SYSTEM_PROMPT_CACHE_BOUNDARY}${dynamicPrompt}` // nosemgrep: security.opengrep.ghsa-2qj5-gwg2-xwc4.aether.prompt-unsanitized-literal-interpolation -- These are complete prompts from trusted builders/transforms; path literals are sanitized at their producers, not by flattening prompt newlines here.
      : stablePrompt;
