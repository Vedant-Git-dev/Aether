 ...
 * aether harness and call the provider API directly with that runtime's
 * credentials (`cli_runtime_passthrough_aether`). Anthropic routes direct
...
import { stripAetherMcpToolPrefix } from "../cli-runner/tool-policy.js";
...
async function runEmbeddedAgentViaCliBackend(
  ...
  const unsubscribe = onAgentEventForRun(params.runId, (evt) => {
    ...
    const toolName = normalizeToolPolicyName(stripAetherMcpToolPrefix(rawName));
