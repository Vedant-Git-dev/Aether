...
async function compactEmbeddedAgentSessionPrepared(
  ...
  // not incorrectly require an Aether model API credential.
...
async function compactResolvedContextEngine(
  ...
  const lockedNativeHarness = Boolean(lockedHarnessRuntime && lockedHarnessRuntime !== "aether");
  ...
  const attemptNativeHarnessCompaction =
    selectedNativeHarnessCompaction && preparedHarnessRuntime !== "aether";
  ...
  const contextEngineRuntimeSettings = buildContextEngineRuntimeSettings({
    contextEngineHost: AETHER_EMBEDDED_CONTEXT_ENGINE_HOST,
    provider: ceProvider,
