...
import { isAetherAbortableWrapper } from "./embedded-agent-runner/run/abortable.js";
import {
  FailoverError,
...
function isTerminalAbortFromError(err: unknown): boolean {
  ...
  return isAetherAbortableWrapper(err) && causeCandidates.some(isTerminalAbortCandidate);
}
...
function isCliAgentRuntime(runtime: string | undefined, cfg: AetherConfig | undefined): boolean {
...
export async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(runtime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale Aether provider
