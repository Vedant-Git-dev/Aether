...
import { isSubscribeTranscriptOnlyAetherAssistantMessage } from "./embedded-agent-subscribe.handlers.messages.stream.js";
  ...
  return {
    captureModelEvent: (evt: AgentSessionEvent): void => {
      ...
      if (
        ...
        isSubscribeTranscriptOnlyAetherAssistantMessage(message)
      ) {
        return;
      }
      switch (evt.type) {
