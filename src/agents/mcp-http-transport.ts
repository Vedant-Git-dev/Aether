...
abstract class AetherMcpHttpTransport implements Transport {
  onclose?: () => void;
...
export class AetherSSEClientTransport extends AetherMcpHttpTransport {
...
type AetherStreamableHttpOptions = StreamableHTTPClientTransportOptions & {
  fetch?: FetchLike;
  requestInit?: RequestInit;
};

/** Owns Streamable HTTP notification recovery and stateful cleanup around SDK 1.30.0. */
export class AetherStreamableHTTPClientTransport extends AetherMcpHttpTransport {
  ...
  constructor(url: URL, options: AetherStreamableHttpOptions = {}) {
    super();
    this.url = url;
