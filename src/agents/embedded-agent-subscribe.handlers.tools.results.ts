...
function isAetherExecutable(token: string | undefined): boolean {
  const executable = normalizeOptionalLowercaseString(token);
  return executable?.split(/[\\/]/).at(-1) === "aether";
}

function isAetherPackageSpec(token: string | undefined): boolean {
  const packageSpec = normalizeOptionalLowercaseString(token);
  return packageSpec?.startsWith("aether@") === true && packageSpec.length > "aether@".length;
}

function skipAetherPackageRunner(
  tokens: string[],
  startIndex: number,
...
function isAetherCronAddShellCommand(args: unknown): boolean {
  ...
  const packageRunner = skipAetherPackageRunner(tokens, commandIndex);
  ...
  return (
    (isAetherExecutable(tokens[commandIndex]) ||
      (packageRunner.acceptsPackageSpec && isAetherPackageSpec(tokens[commandIndex]))) &&
...
export function didShellCronAddSucceed(args: unknown, result: unknown): boolean {
  if (!isAetherCronAddShellCommand(args)) {
    return false;
  }
