...
function assertAuthProfileNotRetired(params: {
  ...
  throw new Error(
    `Auth profile "${params.profileId}" is retired. Run ${formatCliCommand("aether vitals --fix")}.`,
  );
}
...
export async function resolveApiKeyForProviderCore(input: {
  ...
  throw new ProviderAuthError(
    ...
    [
      ...
      `Configure an API key (${formatCliCommand(`aether models auth paste-api-key --provider ${provider}`)}; add --agent <id> for a non-default agent) or copy only portable static auth profiles from the main agentDir.`,
    ].join(" "),
  );
}
