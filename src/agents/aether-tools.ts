...
import { resolveAetherPluginToolsForOptions } from "./aether-plugin-tools.js";
import { filterToolsByClientCaps } from "./aether-tools.client-caps.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./aether-tools.media-factory-plan.js";
import { createMediaGenerationAsyncStartCallback } from "./aether-tools.media-yield.js";
import { applyNodesToolWorkspaceGuard } from "./aether-tools.nodes-workspace-guard.js";
import {
  collectPresentAetherTools,
  shouldIncludeAskUserToolForAetherTools,
  shouldIncludeProgressCardToolForAetherTools,
  shouldIncludeSecretsToolForAetherTools,
} from "./aether-tools.registration.js";
import { createRequesterYieldCallback } from "./aether-tools.requester-yield.js";
import { createAetherSwarmToolGroups } from "./aether-tools.swarm.js";
import { resolveTranscriptsTool } from "./aether-tools.transcripts.js";
import type { AetherToolsOptions } from "./aether-tools.types.js";
import { resolveWidgetPresentationForRun } from "./aether-tools.widget-presentation.js";
...
import { createAetherDelegateToolsForRun } from "./tools/aether-delegate-tool.js";
...
export { filterToolsByClientCaps } from "./aether-tools.client-caps.js";
export function createAetherTools(options?: AetherToolsOptions): AnyAgentTool[] {
  ...
  const swarmToolGroups = createAetherSwarmToolGroups({
    config: resolvedConfig,
  ...
  options?.recordToolPrepStage?.("aether-tools:session-workspace");
  ...
  const imageTool =
    ...
    resolveImageToolFactoryAvailable({
      ...
      : null;
  options?.recordToolPrepStage?.("aether-tools:image-tool");
  ...
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("aether-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("aether-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("aether-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("aether-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("aether-tools:nodes-tool");
  ...
  const progressCardTool = shouldIncludeProgressCardToolForAetherTools({
    ...options,
    agentId: sessionAgentId,
  })
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentAetherTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentAetherTools([transcriptsTool]),
    ...collectPresentAetherTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
          ...
          ...createAetherDelegateToolsForRun({ ...options, sessionAgentId }),
        ]),
    ...
    ...collectPresentAetherTools([progressCardTool]),
    ...
    ...collectPresentAetherTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("aether-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    allTools = [
      ...tools,
      ...resolveAetherPluginToolsForOptions({
        options: { ...options, activeProjectKeys },
        resolvedConfig,
        existingToolNames: new Set(tools.map((tool) => tool.name)),
      }),
    ];
    options?.recordToolPrepStage?.("aether-tools:plugin-tools");
  }

  allTools = finalizeAgentToolAvailability(filterToolsByClientCaps(allTools, options?.clientCaps));
  options?.recordToolPrepStage?.("aether-tools:client-capabilities");
  ...
  options?.recordToolPrepStage?.("aether-tools:tool-hooks");
  return allTools
    .map((tool) =>
