 ...
 */
export async function readLiveTestConfig(): Promise<AetherConfig> {
...
export function isLiveTestEnabled(
  ...
  return [...extraEnvVars, "LIVE", "AETHER_LIVE_TEST"].some((name) =>
...
export function isLiveProfileKeyModeEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AETHER_LIVE_REQUIRE_PROFILE_KEYS);
}
