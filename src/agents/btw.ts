...
export async function runBtwSideQuestion(
  ...
  return await withBtwPreparedRuntime(runtimeInput, async (preparedModelRuntime) => {
    ...
    let preparedAetherFallback:
      | Extract<BtwHarnessSideQuestionDispatch, { kind: "aether" }>
      | undefined;
    const runHarnessSideQuestion = async (
      ...
      // Aether would incorrectly require a host credential before handoff.
      ...
      if (!selectedHarness.runSideQuestion) {
        if (selectedHarness.id !== "aether" || !("auth" in resolvedAttempt)) {
          throw new Error(
    ...
    if (harness.runSideQuestion) {
      ...
      preparedAetherFallback = dispatch;
    }
    ...
    const initialAetherFallback = preparedAetherFallback;
    const runtimeSelectionForHarness =
      initialAetherFallback?.runtime ?? (await resolveRuntimeSelection());
    ...
    const finalizedAetherFallback = preparedAetherFallback;
    const effectiveRuntimeSelection =
      finalizedAetherFallback?.runtime ?? runtimeSelectionForHarness;
    const { authStorage, model, modelRegistry, authProfileStore, runtimeAuthPreparation } =
      effectiveRuntimeSelection;
    const resolvedAttempt =
      finalizedAetherFallback?.resolvedAttempt ??
      (await resolveBtwPreparedRuntimeAuth({
        preparation: runtimeAuthPreparation,
        model,
