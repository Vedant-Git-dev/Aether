...
const OAUTH_REFRESH_FENCE_PREFIX = "aether-oauth-refresh-fence:v1:";
const OAUTH_REFRESH_ACCESS_PATTERN =
  /^aether-oauth-refresh-fence:v1:([a-f0-9]{32}):(failed:)?access:([a-f0-9]{64})$/;
const OAUTH_REFRESH_TOKEN_PATTERN =
  /^aether-oauth-refresh-fence:v1:([a-f0-9]{32}):(failed:)?refresh:([a-f0-9]{64})$/;
...
function buildOAuthRefreshSecretDigest(params: {
  ...
  const generationInput = JSON.stringify([
    "aether.oauth-refresh-generation",
    1,
    params.profileId,
    params.provider,
    params.kind,
