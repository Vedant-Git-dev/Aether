...
>;
type SharedAuthProfileDatabase = Pick<AetherStateKyselyDatabase, "config_machine_state">;
export type AuthProfileDatabase = AetherAgentDatabase | AetherStateDatabase;
...
function prepareAuthProfileSharedOwner(env: NodeJS.ProcessEnv) {
  const preparedEnv = { ...env, AETHER_STATE_DIR: resolveStateDir(env) };
  return {
    env: preparedEnv,
...
export function deferAuthProfilePostCommitPublication(
  ...
  if ("agentId" in database) {
    return deferAetherAgentPostCommitPublication(database, publish);
  }
...
// The auth database lives in the agent dir and shares the aether-agent schema
...
export function resolveAuthProfileDatabasePath(agentDir: string): string {
  return agentDir
    ? path.join(resolveUserPath(agentDir), "aether-agent.sqlite")
    : resolveSharedAuthStorePath();
}
...
function acquireAuthProfileReadDatabase(
  ...
  try {
    ...
    if (readSqliteUserVersion(db) > AETHER_AGENT_SCHEMA_VERSION) {
      clearNodeSqliteKyselyCacheForDatabase(db);
      db.close();
      return { status: "unreadable" };
    }
...
export function runAuthProfileWriteTransaction<T>(
  ...
  const env = {
    ...
    ...(!options.env && options.stateDir
      ? { AETHER_STATE_DIR: options.stateDir, AETHER_AGENT_DIR: undefined }
      : {}),
  };
  ...
  if (databaseTarget.kind === "agent") {
    return runAetherAgentWriteTransaction((database) => {
  ...
  const database = openAetherStateDatabase({ env, path: databaseTarget.path });
  ...
  try {
    ...
    result = runAetherStateWriteTransaction((transaction) => operation(transaction, owner), {
      env,
      database,
    });
  } catch (error) {
