...
export function withAuthProfileStoreAgentDir<T>(
  ...
  const env = { ...process.env, AETHER_STATE_DIR: sharedStateDir };
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.authProfileStoreTestApi")] =
    testing;
}
...
function assertAuthProfilePersistenceOwner(
  ...
  if (
    stateDir &&
    path.resolve(resolveStateDir({ ...owner.env, AETHER_STATE_DIR: stateDir })) !==
...
export function createAuthProfileStoreRuntime(
  ...
  function createAuthProfileStoreReadScope(agentDir: string, config: AetherConfig | undefined) {
