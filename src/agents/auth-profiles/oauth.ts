...
type SecretDefaults = NonNullable<AetherConfig["secrets"]>["defaults"];
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.oauthTestApi")] = {
...
export async function resolveApiKeyForProfile(
  ...
  // resolve, refresh, or leave Aether as bearer tokens.
  if (isRetiredOAuthProfileId(profileId)) {
    return null;
  }
