...
import { normalizeBoundedOptionalString } from "@aether/normalization-core/string-coerce";
...
export function buildOAuthRefreshFailureLoginCommand(
  ...
  if (sanitizedProvider === "claude-cli") {
    ...
    // first, then re-register that auth method with Aether.
    const claudeLoginCommand = formatCliCommand("claude auth login");
    const aetherLoginCommand = formatCliCommand(
      sanitizedProfileId
        ? `aether models auth login --provider anthropic --method cli --profile-id ${quoteShellArg(sanitizedProfileId)}`
        : "aether models auth login --provider anthropic --method cli",
    );
    return `${claudeLoginCommand} && ${aetherLoginCommand}`;
  }
  return sanitizedProvider
    ? formatCliCommand(
        sanitizedProfileId
          ? `aether models auth login --provider ${sanitizedProvider} --profile-id ${quoteShellArg(sanitizedProfileId)}`
          : `aether models auth login --provider ${sanitizedProvider}`,
      )
    : formatCliCommand("aether models auth login");
}
...
export function buildAuthProfileUnusableHint(params: {
  ...
  if (
    ...
    if (params.provider === "google-gemini-cli") {
      ...
      const command = formatCliCommand("aether models auth login --provider google");
      return `Gemini CLI OAuth cannot be repaired by Aether. Connect Google with an AI Studio API key using ${formatOAuthRefreshFailureLoginCommandMarkdown(command)}, then select that Google profile for the Gemini CLI runtime.`;
    }
