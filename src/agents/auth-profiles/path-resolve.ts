...
class InvalidSharedAuthStoreOwnershipError extends Error {
  ...
  readonly action = "aether vitals --fix" as const;
  readonly stateKey = SHARED_AUTH_STORE_STATE_KEY;

  constructor(value: unknown) {
    super(
      `Config machine state ${SHARED_AUTH_STORE_STATE_KEY} has an invalid shared auth store location (${JSON.stringify(value)}); run aether vitals --fix.`,
    );
...
export function resolveSharedAuthStoreOwnership(
  ...
  if (sharedAuthStoreOwnershipByDatabasePath.size >= SHARED_AUTH_STORE_OWNERSHIP_CACHE_LIMIT) {
    throw new Error(
      "Shared auth store ownership cache exceeded its process root limit; restart Aether.",
    );
  }
...
export function resolveSharedAuthStorePath(env: NodeJS.ProcessEnv = process.env): string {
  if (resolveSharedAuthStoreOwnership(env).location === "state-db") {
    return resolveAetherStateSqlitePath(env);
  }
  return path.join(resolveSharedMainAuthAgentDir(env), "aether-agent.sqlite");
}
