/**
 * Builds the effective Aether agent tool surface.
 * Assembles core, shell, channel, Aether, plugin, and Tool Search tools, then
...
import { mergeGatewayAgentCliPath } from "../infra/aether-cli-shim.js";
...
import type { AetherCodingToolConstructionPlan } from "./core-tool-factory-descriptors.js";
import {
...
import { createAetherTools, filterToolsByClientCaps } from "./aether-tools.js";
import { filterRequesterYieldTools } from "./aether-tools.requester-yield.js";
...
type AetherCodingToolsOptions = {
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep Aether web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  ...
  toolConstructionPlan?: AetherCodingToolConstructionPlan;
  /** Ring-zero Aether tool; set only by the Aether agent runner. */
...
function createAetherCodingToolsInternal(options?: AetherCodingToolsOptions): AnyAgentTool[] {
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeAetherTools: includeCoreTools,
  ...
  const includeAetherTools = includeCoreTools && toolConstructionPlan.includeAetherTools;
  ...
  // Plugin-only plans bypass createAetherTools, so the capability gate must
  ...
  const pluginToolsOnly = filterToolsByClientCaps(
    includeAetherTools || !includePluginTools
      ? []
  ...
  const ringZeroTools = includeAetherTools ? getActiveAgentRingZeroTools() : [];
  ...
  options?.recordToolPrepStage?.("aether-tools");
...
export function createAetherCodingTools(options?: AetherCodingToolsOptions): AnyAgentTool[] {
  return createAetherCodingToolsInternal(options);
}
