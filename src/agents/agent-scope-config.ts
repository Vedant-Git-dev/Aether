...
type AgentEntry = NonNullable<NonNullable<AetherConfig["agents"]>["list"]>[number];
type AgentEntriesConfig = NonNullable<NonNullable<AetherConfig["agents"]>["entries"]>;
 ...
 */
export function withAgentRosterFactsBatch<T>(config: AetherConfig, callback: () => T): T {
...
function readAgentRosterFacts(cfg: AetherConfig): AgentRosterFacts | undefined {
...
export function listAgentEntriesWithSource(cfg: AetherConfig): ListedAgentEntry[] {
...
export function listAgentEntries(cfg: AetherConfig): AgentEntry[] {
...
export function listAgentIds(cfg: AetherConfig): string[] {
...
export function resolveConfiguredAgentId(cfg: AetherConfig, agentId: string): string {
  if (!listAgentIds(cfg).includes(agentId)) {
    ...
    throw new Error(
      `Unknown agent id "${agentId}". Run ${formatCliCommand("aether agents list")} to see configured agents.`,
    );
  }
  return agentId;
}

export function tryResolveSoleAgentId(cfg: AetherConfig): string | undefined {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
...
export function resolveSoleAgentId(cfg: AetherConfig, context?: AgentSelectionContext): string {
  ...
  if (agentIds.length === 0) {
    throw new Error("No agents configured. Run `aether awaken` or `aether agents add` first.");
  }
  throw new AgentSelectionRequiredError(agentIds, context);
}

function tryResolveRawLegacyDefaultAgentId(cfg: AetherConfig): string | undefined {
...
export function tryResolveLegacyCompatibilityAgentId(cfg: AetherConfig): string | undefined {
...
export function tryResolveDefaultAgentId(cfg: AetherConfig): string | undefined {
  return tryResolveRawLegacyDefaultAgentId(cfg) ?? tryResolveSoleAgentId(cfg);
}

export function resolveAgentEntry(cfg: AetherConfig, agentId: string): AgentEntry | undefined {
...
function tryResolveInheritedWorkspaceAgentId(cfg: AetherConfig): string | undefined {
  return tryResolveLegacyCompatibilityAgentId(cfg);
}
...
export function resolveAgentRunCwd(cfg: AetherConfig, agentId: string): string | undefined {
  const cwd =
 ...
 */
export function isImplicitAcpWorkspaceCandidate(cfg: AetherConfig, agentId: string): boolean {
