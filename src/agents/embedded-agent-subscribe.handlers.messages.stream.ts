...
export function isSubscribeTranscriptOnlyAetherAssistantMessage(
  ...
  return provider === "aether" && (model === "delivery-mirror" || model === "gateway-injected");
}
...
export function isOpenAiCompletionsAssistantMessage(message: AgentMessage | undefined): boolean {
  ...
  const api = normalizeOptionalString((message as { api?: unknown }).api) ?? "";
  return api === "openai-completions" || api === "aether-openai-completions-transport";
}
