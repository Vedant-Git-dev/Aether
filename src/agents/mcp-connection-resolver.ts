...
import { resolveAetherMcpTransportAlias } from "../config/mcp-config-normalize.js";
...
type McpServerConnectionResolverEntry = AetherPluginMcpServerConnectionResolver & {
  pluginId: string;
};
...
export function applyMcpConnectionOverride(
  ...
  const fromTransport =
    typeof base.transport === "string"
      ? resolveAetherMcpTransportAlias(base.transport)
      : undefined;
  const fromType = resolveAetherMcpTransportAlias(base.type);
