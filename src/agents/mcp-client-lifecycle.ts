...
import { AetherStreamableHTTPClientTransport } from "./mcp-http-transport.js";
...
export function isStatefulMcpHttpSessionExpired(
  ...
  return (
    ...
    session.transport instanceof AetherStreamableHTTPClientTransport &&
      ...
      await disposeMcpClient(
        {
          ...
          transportType:
            params.transport instanceof AetherStdioClientTransport
              ? "stdio"
              : params.transport instanceof AetherStreamableHTTPClientTransport
                ? "streamable-http"
                : "sse",
        },
...
export async function disposeMcpClient(
  ...
  try {
    ...
    const closeTransport =
      session.transportType === "stdio" && transport instanceof AetherStdioClientTransport
