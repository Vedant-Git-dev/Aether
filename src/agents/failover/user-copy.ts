...
const SELECTED_AUTH_PROFILE_UNAVAILABLE_USER_TEXT =
  "The selected auth profile is unavailable in this agent's Aether credential store. " +
  "Import or migrate that credential into the agent, select another configured profile, or run `aether tune`, then retry.";
...
export function formatDiskSpaceErrorCopy(raw: string): string | undefined {
  ...
  return /\benospc\b/i.test(raw) ||
    ...
    ? "Aether could not write local session data because the disk is full. Free some disk space and try again."
    : undefined;
}
...
export function renderMissingApiKeyReplyCopy(params?: {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `aether vitals --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `aether models auth login --provider openai` or run `aether tune`.';
  }
...
export function renderCliTimeoutReplyCopy(params: {
  ...
  if (params.replayPrevented) {
    workStatus += " Aether did not replay this turn automatically.";
  }
  return mode === "no-output"
    ...
    : `⚠️ CLI turn${routingSuffix}: timed out after ${seconds}s (overall turn limit). The gateway is unaffected.${workStatus} For long work, use a detached Aether sub-agent (no run timeout by default), or raise \`agents.defaults.timeoutSeconds\`.`;
}
...
const CONTROL_UI_LOG_HINT = "To view logs, run `aether logs --follow` in a terminal.";
