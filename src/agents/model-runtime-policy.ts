...
export function resolveAgentRuntimePolicyAgentId(
  params: AgentRuntimePolicyScope & { config?: AetherConfig },
): string | undefined {
...
export function resolveModelRuntimePolicy(
  ...
  if (process.env.AETHER_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.AETHER_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "aether" || forcedRuntime === "codex") {
