...
export const LEGACY_WORKSPACE_STATE_DIRNAME = ".aether";
const LEGACY_WORKSPACE_STATE_FILENAME = "workspace-state.json";
export const LEGACY_WORKSPACE_STATE_CURRENT_FILENAME = "aether-workspace-state.json";
...
export const LEGACY_WORKSPACE_ATTESTATION_HEADER = "aether-workspace-attestation:v1";
...
function workspaceMigrationError(
  ...
  return new StartupMaintenanceRequiredError(
    ...
    operation === "doctor"
      ...
      : `Legacy workspace setup state requires migration for ${blockedPaths.join(", ")}; run ${formatCliCommand("aether vitals --fix", env)}.`,
  );
}
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.workspaceLegacyStateTestApi")] =
...
export function prepareLegacyWorkspaceStateReset(
  ...
  const candidates = [
    ...
    ...sources.stateDirAttestationPaths.map((sourcePath) => ({
      ...
      sourcePath,
      // Hashed paths inside Aether-owned attestation directories are
    ...
    candidate,
    {
      ...
      // Sibling claims remain outside Aether-owned roots. Renaming a claimed
