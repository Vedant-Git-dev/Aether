...
function resolveContainerExecEnv(env: Record<string, string>): Record<string, string> {
  ...
  if (requestedPath) {
    containerEnv.AETHER_PREPEND_PATH = requestedPath;
  }
  return containerEnv;
}
...
function buildContainerExecArgs(params: {
  ...
  const pathExport = params.env.PATH
    ? 'export PATH="${AETHER_PREPEND_PATH}:$PATH"; unset AETHER_PREPEND_PATH; '
    : "";
...
function resolveConfiguredDockerRuntimeImage(params: {
  config: CreateSandboxBackendParams["cfg"] | import("../../config/config.js").AetherConfig;
  agentId?: string;
