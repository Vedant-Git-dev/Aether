...
function resolvePreprovisionedSshRuntimePaths(params: {
  ...
  return {
    ...
    remoteSkillsWorkspaceDir: path.posix.join(remoteWorkspaceDir, ".aether", "sandbox-skills"),
  };
}
...
function buildSshSandboxRuntimeId(scopeKey: string): string {
  ...
  if (/:workspace:[a-f0-9]{32}$/i.test(trimmed)) {
    return `aether-ssh-workspace-${hashTextSha256(trimmed).slice(0, 32)}`;
  }
  ...
  return `aether-ssh-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
