...
const CDP_SOURCE_RANGE_ENV_KEY = "AETHER_BROWSER_CDP_SOURCE_RANGE";
const CDP_AUTH_TOKEN_ENV_KEY = "AETHER_BROWSER_CDP_AUTH_TOKEN";
const SANDBOX_BROWSER_IMAGE_CONTRACT_LABEL = "org.aether.sandbox-browser.contract";
...
function buildSandboxCdpAuthHeader(token: string): string {
  return `Basic ${Buffer.from(`aether:${token}`).toString("base64")}`;
}
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_AETHER_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_AETHER_BROWSER_PROFILE_NAME,
    extraArgs: [],
    tabCleanup: {
      enabled: true,
      idleMinutes: 120,
...
async function ensureSandboxBrowserContainer(
  ...
  if (hasContainer) {
    ...
    currentHash = await readDockerContainerLabel(containerName, "aether.configHash");
      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `aether sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `aether sandbox recreate --browser --agent ${agentId}`;
          }
          return "aether sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
  ...
  if (!hasContainer) {
    ...
    const { argv: args, env } = buildSandboxCreateArgs({
      ...
      labels: {
        "aether.sandboxBrowser": "1",
        "aether.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
      },
    ...
    Object.assign(env, {
      AETHER_BROWSER_HEADLESS: params.cfg.browser.headless ? "1" : "0",
      AETHER_BROWSER_ENABLE_NOVNC: params.cfg.browser.noVncEnabled ? "1" : "0",
      AETHER_BROWSER_CDP_PORT: String(params.cfg.browser.cdpPort),
      [CDP_AUTH_TOKEN_ENV_KEY]: cdpAuthToken,
      AETHER_BROWSER_AUTO_START_TIMEOUT_MS: String(params.cfg.browser.autoStartTimeoutMs),
      AETHER_BROWSER_VNC_PORT: String(params.cfg.browser.vncPort),
      AETHER_BROWSER_NOVNC_PORT: String(params.cfg.browser.noVncPort),
      AETHER_BROWSER_NO_SANDBOX: "1",
    });
    if (cdpSourceRange) {
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_AETHER_BROWSER_PROFILE_NAME)
    : null;
