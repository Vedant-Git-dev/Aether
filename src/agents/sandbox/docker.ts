...
import { markAetherExecEnv } from "../../infra/aether-exec-env.js";
/**
...
export async function ensureContainerImage(engine: SandboxContainerEngine, image: string) {
  ...
  if (image === DEFAULT_SANDBOX_IMAGE) {
    if (engine.id === "docker") {
      throw new Error(
        `Sandbox image not found: ${image}. Build it with scripts/sandbox-setup.sh before enabling Docker sandboxing. The default image includes python3 for sandbox write/edit helpers; Aether will not substitute plain debian:bookworm-slim.`,
      );
    }
    throw new Error(
      `Sandbox image not found in ${engine.displayName}: ${image}. Build it with podman build -t ${image} -f scripts/docker/sandbox/Dockerfile . before enabling container sandboxing. The default image includes python3 for sandbox write/edit helpers; Aether will not substitute plain debian:bookworm-slim.`,
    );
  }
  if (engine.id === "docker") {
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--init");
  args.push("--label", "aether.sandbox=1");
  args.push("--label", `aether.sessionKey=${params.scopeKey}`);
  args.push("--label", `aether.createdAtMs=${createdAtMs}`);
  args.push("--label", `aether.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  args.push("--label", `aether.createArgsEpoch=${SANDBOX_DOCKER_CREATE_ARGS_EPOCH}`);
  if (params.configHash) {
    args.push("--label", `aether.configHash=${params.configHash}`);
  }
  ...
  const env = markAetherExecEnv(envSanitization.allowed);
...
async function readContainerConfigHash(
  ...
  return await readContainerLabel(engine, containerName, "aether.configHash");
}
