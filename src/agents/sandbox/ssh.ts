...
export async function prepareSshSandboxExec(params: {
  ...
  const remoteDir = `/tmp/aether-sandbox-exec-${randomUUID()}`;
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredAetherTmpDir() ?? os.tmpdir());
}
...
async function createSshSandboxSession(
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "aether-sandbox-ssh-"));
