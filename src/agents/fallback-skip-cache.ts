 ...
 * AETHER_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "AETHER_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    aetherFallbackSkipCache?: SkipBySession;
    aetherFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.aetherFallbackSkipCacheState) {
    ...
    const buckets = globalStore.aetherFallbackSkipCache ?? new Map();
    globalStore.aetherFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.aetherFallbackSkipCache = buckets;
  }
  return globalStore.aetherFallbackSkipCacheState;
}
