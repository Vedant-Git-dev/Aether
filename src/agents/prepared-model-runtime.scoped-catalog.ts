...
import { ensureAetherModelsJson, planAetherModelsJsonSource } from "./models-config.js";
...
export async function prepareAgentCatalogSource(
  ...
  const prepareSource = async () => {
    if (!persist) {
      const source = await planAetherModelsJsonSource(input.config, input.agentDir, {
        ...options,
    ...
    if (!input.readOnly) {
      await ensureAetherModelsJson(input.config, input.agentDir, {
        ...options,
        ...(catalogMode === "live" ? { onProviderCatalogOutcome: recordProviderOutcome } : {}),
      });
    }
