...
/** Opening delimiter for protected Aether runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_AETHER_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected Aether runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_AETHER_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[AETHER_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[AETHER_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const AETHER_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
/** Custom message type used for structured runtime-context messages. */
export const AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE = "aether.runtime-context";
...
const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["Aether runtime context (internal):", AETHER_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
const RUNTIME_CONTEXT_PROMPT_HEADERS: readonly string[] = [
  "Aether runtime context for the active user request in this turn. Do not reply to or describe this context. Use it to continue answering the active user request now. Do not wait for another message.",
  "Aether runtime context for the immediately preceding user message.",
  "Aether runtime event.",
];

const RUNTIME_CONTEXT_NOTICE_PATTERN = new RegExp(
  AETHER_RUNTIME_CONTEXT_NOTICE.split(/\s+/).map(escapeRegExp).join("\\s+"),
);
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    RUNTIME_CONTEXT_PROMPT_HEADERS.some((header) =>
      text.includes(`${header}\n${AETHER_RUNTIME_CONTEXT_NOTICE}`),
    )
  );
}

/** Identifies hidden runtime context independently of its queue or transcript owner. */
export function isAetherRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === AETHER_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isAetherRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isAetherRuntimeContextCustomMessage(message));
}
...
export function resolvePendingRuntimeContextReplay<T>(params: {
  ...
  const replayPersistedCarrier =
    ...
    isAetherRuntimeContextCustomMessage(params.messages[persistedUserIndex + 1]);
  return {
    persistedUserIndex,
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isAetherRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isAetherRuntimeContextCustomMessage(message)) {
      return true;
    }
...
export function relocateCurrentRuntimeContextCarrierToTail<T>(messages: T[]): T[] {
  const carrierIndex = messages.findIndex(isAetherRuntimeContextCustomMessage);
  const userIndex = messages.findIndex(
    (message, index) => index > carrierIndex && isUserMessage(message),
  );
  ...
  const prefix = messages
    .slice(0, boundary)
    .filter((message) => !isAetherRuntimeContextCustomMessage(message));
  const carriers = messages.filter(isAetherRuntimeContextCustomMessage);
  return [
    ...prefix,
    ...carriers,
    ...messages
