/** Tool Search catalog compaction for large Aether, MCP, and client tool inventories. */
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess over a large tool catalog. APIs: `aether.tools.search(query: string, options?)`, `aether.tools.describe(id: string)`, and `aether.tools.call(id: string, args?)`. Search takes a positional query string, which must be in English: matching is lexical against tool names and descriptions, which are written in English. Call returns `{ tool, result }`; JSON values normally live in `result.details`.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The aether.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call an exact Tool Search result id or name through Aether.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.toolSearchTestApi")] = testing;
}
