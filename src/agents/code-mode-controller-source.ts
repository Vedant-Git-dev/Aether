...
(() => {
  ...
  let admissionError;
  const maxPending = globalThis.__aetherMaxPendingToolCalls;
  delete globalThis.__aetherMaxPendingToolCalls;
  const catalogBindings = Array.isArray(globalThis.__aetherCatalog) ? globalThis.__aetherCatalog : [];
  const apiFiles = Array.isArray(globalThis.__aetherApiFiles) ? globalThis.__aetherApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__aetherNamespaces) ? globalThis.__aetherNamespaces : [];
  const hostRequest = globalThis.__aetherHostRequest;
  const hostCancelRequest = globalThis.__aetherHostCancelRequest;
  delete globalThis.__aetherHostRequest;
  delete globalThis.__aetherHostCancelRequest;
  delete globalThis.__aetherCatalog;
  delete globalThis.__aetherApiFiles;
  delete globalThis.__aetherNamespaces;
  ...
  if (globalThis.__aetherSwarmEnabled === true) {
  ...
  Object.defineProperties(globalThis, {
    ...
    __aetherSettleBridge: { value: settle },
    __aetherDrainQueuedRequests: { value: drainQueuedRequests },
    __aetherAdmissionError: { value: () => admissionError },
    __aetherSerializeCatalogHandles: { value: serializeOutputValue },
    __aetherTakeOutput: { value: () => output.splice(0) },
    __aetherTrackRejection: {
      value: (promise, reason, handled) => {
        if (handled) unhandledRejections.delete(promise);
        else unhandledRejections.set(promise, reason);
      },
    },
    __aetherUnhandledRejection: { value: () => unhandledRejections.keys().next().value },
  });
})();
`;
