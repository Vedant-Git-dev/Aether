...
const AETHER_LOCK_PATTERN = /^aether pid=(\d+)$/;
...
export async function lockState(record: ManagedWorktreeRecord): Promise<LockState> {
  ...
  const match = AETHER_LOCK_PATTERN.exec(entry.lockedReason);
  if (!match) {
    return { kind: "foreign", reason: entry.lockedReason };
  }
  const pid = Number(match[1]);
  // A cross-user (EPERM) Aether lock is treated as live so a run's checkout is
...
export async function lockWorktreeForProcess(record: ManagedWorktreeRecord): Promise<void> {
  ...
  // A lock naming a dead Aether pid is restart residue: the owner died (crash or
  ...
  // isProtectedFromAutoRemoval() (aether#114129); today's behavior instead loses
  // the lock every time.
  if (state.kind !== "dead") {
