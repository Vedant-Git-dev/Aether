...
import { withAetherStateLease } from "../../state/aether-state-lease.js";
...
const SNAPSHOT_REF_PREFIX = "refs/aether/snapshots";
      ...
      finish(outcome: "returned" | "threw") {
        try {
          ...
          const state = resolveGlobalSingleton(
            Symbol.for("aether.worktreeRemovalDiagnostics"),
            () => ({
...
type ServiceOptions = {
  ...
  now?: () => number;
  getConfig?: () => Pick<AetherConfig, "worktreeRoot">;
};
...
async function runSetupScript(
  ...
  const setupScript = path.join(repoRoot, ".aether", "worktree-setup.sh");
  ...
  const result = await runCommandWithTimeout([setupScript], {
    ...
    env: {
      AETHER_SOURCE_TREE_PATH: repoRoot,
      AETHER_WORKTREE_PATH: worktreePath,
    },
  });
  params.signal?.throwIfAborted();
  if (result.code !== 0) {
...
async function snapshotWorktree(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-worktree-index-"));
  ...
  try {
    ...
    const commit = await requireGit(
      ...
      [
        ...
        parent,
        "-m",
        `Aether worktree snapshot: ${reason}`,
      ],
      { env },
    );
    commitGuard?.();
  ...
  private async createForRepository(
    ...
    const setupStat =
      params.runSetupScript === false
        ? undefined
        : await fs
            .stat(path.join(repository.sourceRoot, ".aether", "worktree-setup.sh"))
            .catch(() => undefined);
    ...
    try {
      ...
      if (state.kind === "live" || state.kind === "foreign") {
        throw new WorktreeRemovalLockError(
          ...
          state.kind === "live"
            ? `worktree is locked by live Aether pid ${state.pid}`
