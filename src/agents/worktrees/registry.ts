...
type WorktreesTable = AetherStateKyselyDatabase["worktrees"];
type WorktreeRow = Selectable<WorktreesTable>;
type WorktreeRegistryDatabase = Pick<AetherStateKyselyDatabase, "worktrees">;
type WorktreeProvisionedDatabase = Pick<
  AetherStateKyselyDatabase,
  "worktree_provisioned_file_chunks"
>;
type WorktreeLeaseDatabase = Pick<AetherStateKyselyDatabase, "worktrees" | "state_leases">;

function dbFor(env: NodeJS.ProcessEnv): DatabaseSync {
  return openAetherStateDatabase({ env }).db;
}
...
function readRegistry<T>(
  ...
  return behavior.artifactPreservingReadOnly
    ? withExistingAetherStateDatabaseArtifactPreservingReadOnly(operation, { env })
    : withExistingAetherStateDatabaseReadOnly(operation, { env });
}
