 ...
 * streamable-HTTP SDK transports with Aether auth, redirect, and logging rules.
 */
...
import {
  AetherSSEClientTransport,
  AetherStreamableHTTPClientTransport,
...
import { AetherStdioClientTransport } from "./mcp-stdio-transport.js";
...
function attachStderrLogging(serverName: string, transport: AetherStdioClientTransport) {
...
export function resolveMcpTransport(
  ...
  if (resolved.kind === "stdio") {
    const transport = new AetherStdioClientTransport({
  ...
  if (resolved.transportType === "streamable-http") {
    return {
      transport: new AetherStreamableHTTPClientTransport(new URL(resolved.url), {
  ...
  return {
    transport: new AetherSSEClientTransport(new URL(resolved.url), {
