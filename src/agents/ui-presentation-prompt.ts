...
export function buildUiPresentationPrompt(params: {
  ...
  return [
    ...
    ...(clawHubMessageToolName
      ? [
          `\`${clawHubMessageToolName}\`: When the user asks to install an integration or whether you can perform an action, check ClawHub first with \`${clawHubMessageToolName}(action="send", clawhub={query:"capability"})\`, omitting channel and target. This presents official plugin or skill cards, including when it is already installed. Report availability and installation state from the result. An installed desktop app does not establish that its Aether plugin is installed. Treat an unqualified service install request as an Aether capability request unless the user explicitly asks for a desktop app.`,
        ]
      : []),
    ...(showWidgetToolName
      ? [
