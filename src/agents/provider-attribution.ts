...
const AETHER_ATTRIBUTION_PRODUCT = "Aether";
const AETHER_ATTRIBUTION_ORIGINATOR = "aether";
...
function formatAetherUserAgent(version: string): string {
  return `${AETHER_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function resolveProviderAttributionIdentity(
  ...
  return {
    product: AETHER_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in Aether runtime wrapper.",
    ...identity,
    headers: {
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": AETHER_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildGoogleAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      "x-goog-api-client": `${AETHER_ATTRIBUTION_ORIGINATOR}/${identity.version}`,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: AETHER_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatAetherUserAgent(identity.version),
    },
  };
}
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard aether User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: AETHER_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
...
export function resolveProviderRequestPolicy(
  ...
  if (isCanonicalOrLegacyOpenAIProvider(provider) && usesVerifiedOpenAIAttributionHost) {
    ...
    // A custom baseUrl is a proxy and must not inherit Aether attribution.
