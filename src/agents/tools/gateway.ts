...
function resolveLocalGatewayUrlKeys(cfg: AetherConfig): Set<string> {
...
function resolveConfiguredRemoteGatewayKey(cfg: AetherConfig): string | undefined {
...
export function resolveGatewayOptions(opts?: GatewayCallOptions) {
  ...
  const timeoutMs =
    typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs)
      ...
      : 30_000;
  const envGatewayUrl = trimToUndefined(process.env.AETHER_GATEWAY_URL);
  const target =
  ...
  try {
    ...
    if (isNodeApprovalReplay) {
      throw new Error(
        [
          ...
          "Fix the Aether state directory permissions and retry the approval.",
        ].join(" "),
        { cause: error },
      );
    }
    throw new Error(
      [
        ...
        "Fix the Aether state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
...
function staleGatewayAgentRuntimeIdentityError(cause: unknown): Error {
  return new Error(
    [
      "The running Gateway is from an older Aether build and rejected current agent runtime connection metadata.",
      "Restart the Gateway with `aether gateway restart`, then retry.",
    ].join(" "),
    { cause },
  );
}
