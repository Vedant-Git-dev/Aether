...
// Keep the schema local to avoid a circular import through acp-spawn/aether-tools.
...
function resolveAcpUnavailableMessage(opts?: { sandboxed?: boolean; config?: AetherConfig }) {
    ...
    execute: async (_toolCallId, args, signal) =>
      withToolEffectBoundary(async (onSpawnEffectsStart) => {
        ...
        if (deliveryPressure.blocked) {
          return jsonResult({
            status: "forbidden",
            error: `sessions_spawn is paused because ${deliveryPressure.suspended} completed tasks have blocked delivery. Run aether tasks list, then retry or dismiss blocked deliveries.`,
            ...roleContext,
          });
        }
