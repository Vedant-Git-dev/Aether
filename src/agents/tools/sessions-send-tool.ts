...
import { isRequesterParentOfBackgroundAcpSession } from "@aether/acp-core/session-interaction-mode";
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (normalizedLabelAgentId && !normalizedLabelAgentId.ok) {
        return jsonResult({
          runId: crypto.randomUUID(),
          status: "error",
          error: `Agent "${labelAgentIdInput}" not found. Run aether agents list to see configured agents.`,
        });
      }
