...
function hasExplicitDefaultPrimaryModel(cfg?: AetherConfig): boolean {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.imageToolTestApi")] = {
    ...testing,
...
function pickMaxBytes(cfg?: AetherConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const providerCfg: AetherConfig = effectiveCfg ?? {};
  const preparedProviders =
    params.preparedModelRuntime?.mediaCapabilityProviders?.mediaUnderstandingProviders;
