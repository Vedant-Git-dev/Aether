/**
 * aether built-in tool: ring-zero setup/repair actions for the Aether
 * agent. Never exposed to normal agents — construction is bound to a host-owned
 * per-run scope, and every action funnels through Aether's typed operation
...
export type SystemAgentToolOptions = {
  /** Verified inference owner, distinct from the internal Aether execution agent. */
  agentId?: string;
...
const SystemAgentToolSchema = Type.Object({
  ...
  sha256: Type.Optional(
    Type.String({
      ...
      description: "Exact SHA256 from aether plugins pack for plugin_activate_artifact",
    }),
  ),
  ...
  target: Type.Optional(
    stringEnum(["guided", "classic", "channels", "search", "gateway"], {
      description:
        "Setup target for open_setup. channels/search/gateway open masked terminal flows; guided/classic require exiting Aether and running aether awaken.",
    }),
  ),
...
function requireParam(params: Record<string, unknown>, name: string): string {
  ...
  if (!value?.trim()) {
    throw new ToolInputError(`aether: "${name}" is required for this action`);
  }
  return value.trim();
}
...
function readSetupTarget(
  ...
  throw new ToolInputError(`aether: unknown setup target "${target}"`);
}
...
function operationForAction(params: Record<string, unknown>): SystemAgentOperation {
  ...
  switch (action) {
    ...
    case "plugin_install": {
      ...
      if (validationError) {
        throw new ToolInputError(`aether: ${validationError}`);
      }
    ...
    case "plugin_activate_artifact": {
      ...
      if (
        ...
        throw new ToolInputError(
          "aether: plugin_activate_artifact requires an absolute packed .tgz path and its exact SHA256 from aether plugins pack",
        );
      }
    ...
    default:
      throw new ToolInputError(`aether: unknown action "${action}"`);
  }
}
...
export function createSystemAgentTool(options: SystemAgentToolOptions): AnyAgentTool {
  return {
    ...
    description: [
      ...
      "Personal model accounts: manage_model_accounts opens the human-owned account controls; no change is made by the handoff. Shared provider/auth setup: exit; run `aether awaken`. Never request credentials.",
                ...
                : directive.kind === "gateway-config-setup"
                  ...
                  : directive.kind === "memory-import"
                    ...
                    : directive.kind === "model-setup"
                      ? `${SYSTEM_AGENT_DIRECTIVE_PREFIX} the active inference route cannot be changed inside Aether. Tell the user to exit Aether and run \`aether awaken\`; do not ask for provider credentials here.`
                        ...
                        : directive.target === "channels"
                          ...
                          : directive.target === "search"
                            ...
                            : directive.target === "gateway"
                              ...
                              : `${SYSTEM_AGENT_DIRECTIVE_PREFIX} ${directive.target} setup cannot run inside Aether because it may change the active inference route. Tell the user to exit Aether and run \`aether awaken\`.`,
          {},
        );
      }
