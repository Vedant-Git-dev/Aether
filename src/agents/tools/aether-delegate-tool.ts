/** Regular-agent client for the Aether system agent. */
...
const AetherDelegateSchema = Type.Object({
  message: Type.String({ description: "What system must do." }),
  sessionId: Type.Optional(Type.String({ description: "Continue prior Aether talk." })),
});

const AetherDelegateOutputSchema = Type.Object(
  {
    reply: Type.String(),
...
export function createAetherDelegateToolsForRun(
  options: Pick<
    AetherToolsOptions,
    | "sandboxed"
    | "runSessionKey"
  ...
  const tool: AnyAgentTool = {
    ...
    parameters: AetherDelegateSchema,
    outputSchema: AetherDelegateOutputSchema,
    execute: async (_toolCallId, args, signal) => {
      ...
      const result = await withGatewayToolCallerIdentity(caller, () =>
        callInProcessGatewayTool<AetherDelegateResult>("aether.chat", {
          sessionId,
          message,
          delegation: {
            agentId: options.sessionAgentId,
