...
import { planAetherModelsJson, type PreparedModelsConfigContext } from "./models-config.plan.js";
...
type PreparedAetherModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};
...
type EnsureAetherModelsJsonOptions = {
...
type PlanAetherModelsJsonSourceOptions = EnsureAetherModelsJsonOptions & {
  authStore?: AuthProfileStore;
};

type PlannedAetherModelsJsonSource = Readonly<{
  agentDir: string;
...
function listPreparedPluginModelCatalogs(agentDir: string) {
  ...
  if (warnings.length > 0) {
    throw new Error(
      `Cannot safely prepare provider models until legacy catalog migration succeeds: ${warnings.join("; ")}. Run aether vitals --fix.`,
    );
  }
  return catalogs;
}
...
function resolveModelsConfigInput(config?: AetherConfig): {
  config: AetherConfig;
  discoveryAuthConfig: AetherConfig;
  sourceConfigForSecrets: AetherConfig;
} {
...
function prepareModelsConfigContext(
  ...
  options: EnsureAetherModelsJsonOptions = {},
...
async function prepareAetherModelsJsonSource(
  config?: AetherConfig,
  agentDirOverride?: string,
  options: EnsureAetherModelsJsonOptions = {},
): Promise<PreparedAetherModelsJsonSource> {
  ...
  const pending: Promise<ModelsJsonReadyState> = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planAetherModelsJson({
      context,
 ...
 */
export async function planAetherModelsJsonSource(
  config?: AetherConfig,
  agentDirOverride?: string,
  options: PlanAetherModelsJsonSourceOptions = {},
): Promise<PlannedAetherModelsJsonSource> {
  const context = prepareModelsConfigContext(config, agentDirOverride, options);
...
/** Ensures models.json and the agent SQLite catalog cache are current. */
export async function ensureAetherModelsJson(
  config?: AetherConfig,
  agentDirOverride?: string,
  options: EnsureAetherModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await prepareAetherModelsJsonSource(config, agentDirOverride, options);
