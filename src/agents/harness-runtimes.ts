 ...
 */
import { listModelRefsFromConfigValue } from "@aether/model-catalog-core/configured-model-refs";
...
// are selectable here; built-in Aether/default runtime ids are excluded.
...
function isSelectablePluginRuntime(runtime: string | undefined): runtime is string {
  return (
    ...
    normalizeOptionalAgentRuntimeId(runtime) !== AETHER_AGENT_RUNTIME_ID
  );
}
...
function pushConfiguredModelRuntimeIds(config: AetherConfig, runtimes: Set<string>): void {
