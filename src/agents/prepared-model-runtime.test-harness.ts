...
const preparedModelRuntimeMocks = vi.hoisted(() => ({
  ...
  ensureAetherModelsJson: vi.fn(async (...args: unknown[]) => ({
    agentDir: String(args[1]),
    wrote: false,
  })),
  loadAgentRuntimePluginRegistryHandle: vi.fn(),
  loadStaticCatalog: vi.fn<LoadStaticCatalog>(async () => []),
  planAetherModelsJsonSource: vi.fn(async (...args: unknown[]) => ({
    agentDir: String(args[1]),
...
vi.mock("./models-config.js", () => ({
  ensureAetherModelsJson: (...args: unknown[]) =>
    preparedModelRuntimeMocks.ensureAetherModelsJson(...args),
  planAetherModelsJsonSource: (...args: unknown[]) =>
    preparedModelRuntimeMocks.planAetherModelsJsonSource(...args),
}));
...
export function getPreparedModelRuntimeTestApi(): PreparedModelRuntimeTestApi {
  return (globalThis as Record<PropertyKey, unknown>)[
    Symbol.for("aether.preparedModelRuntimeTestApi")
  ] as PreparedModelRuntimeTestApi;
}

export async function resetPreparedModelRuntimeHarness(state: AetherTestState): Promise<void> {
  ...
  preparedModelRuntimeMocks.ensureAetherModelsJson
    .mockReset()
  ...
  preparedModelRuntimeMocks.planAetherModelsJsonSource
    .mockReset()
    .mockImplementation(async (_config, agentDir) => ({
      agentDir: String(agentDir),
      modelsJsonContents: null,
