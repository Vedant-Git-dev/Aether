...
} from "@aether/ai/internal/shared";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
} from "@aether/normalization-core/string-coerce";
import {
  normalizeStringEntries,
  normalizeStringEntriesLower,
  normalizeUniqueStringEntries,
} from "@aether/normalization-core/string-normalization";
...
import {
  buildAetherToolFallbackText,
  shouldRenderAetherToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- Hosted doc: `[embed ref="cv_123" title="Status" height="320" /]`; URL form: `[embed url="/__aether__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    "- Never local/file:// or arbitrary URL. URL must start `/__aether__/canvas/`; else use `ref`.",
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Provider messaging: never exec/curl; Aether routes.",
    messageToolAvailable
      ? [
          "",
          "### message tool",
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `Docs: ${docsPath}` : "Docs: https://docs.aether.ai",
    docsPath ? "Mirror: https://docs.aether.ai" : undefined,
    sourcePath ? `Source: ${sourcePath}` : "Source: https://github.com/aether/aether",
    docsPath
      ? `Aether behavior questions: docs first${params.readToolName ? ` via \`${params.readToolName}\`/local search` : " using available tools"}. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.`
      : "Aether behavior questions: docs mirror first when web exists. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    params.hasGateway
    ...
    "Diagnosis: run `aether status` when possible; ask only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to Aether main. */
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside Aether.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }

  const acpEnabled = params.acpEnabled === true;
  const promptSurface = params.promptSurface ?? "aether_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    aether: "Gateway restart/system setup/config",
    gateway:
    ...
    agents_list: acpSpawnRuntimeEnabled
      ? "List allowed Aether subagent ids; not ACP ids"
  ...
  const renderAetherToolWorkflowHints =
    shouldRenderAetherToolWorkflowHints({
  ...
  const hasAether = availableTools.has("aether");
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? `tools.fs.workspaceOnly ON: file-tool scratch/temp/meta stays in ${hasSeparateRuntimeCwd ? "working directory" : "workspace"}, preferably \`.aether/tmp/\`. If file tools need it later, never exec-write \`/tmp\`; use ${hasSeparateRuntimeCwd ? "working directory" : "workspace"} path.`
      : "";
  ...
  const safetySection = [
    ...
    buildCredentialSafetyPrompt({
      controlToolsAvailable: availableTools.has("aether") || availableTools.has("gateway"),
    }),
    "",
  ];
  // CLI backends own native file tools outside Aether's projected tool list.
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      "You are a personal assistant running inside Aether.",
      "",
      ...(includeToolGuidance
        ? [
            "## Tooling",
      ...
      ...(acpHarnessSpawnAllowed
        ...
        : []),
      ...(renderAetherToolWorkflowHints && subagentStatusTools.length > 0
        ? [
        ...
        : []),
      ...(renderAetherToolWorkflowHints && sessionLookupTools.length > 0
        ? [
      ...
      ...safetySection,
      "## Runtime Context",
      "Messages delimited by <<<BEGIN_AETHER_INTERNAL_CONTEXT>>> and <<<END_AETHER_INTERNAL_CONTEXT>>> contain runtime context for the user request they follow, not user-authored text.",
      ...
      hasAether
        ? "Gateway restart, config, channels, plugins, agents, models/providers: ask `aether`."
        : hasGateway
      ...
      [
        ...
        hasGateway
          ? "Update Aether: `gateway` action update.run, only on explicit user request; restart and completion notice are automatic."
          : `${hasAether ? "Updates" : "System controls unavailable. Updates and restarts"} need the Aether owner: tell the user to run \`aether update\` in a terminal or use the Control UI.`,
        `Never run ${hasGateway ? "aether update, npm install -g aether, or stop/restart" : "npm install -g aether or stop"} the gateway service via exec.`,
      ].join(" "),
      ...(hasExec
        ? [
            "For a user-requested update on another host, verify it is not this Gateway, then use exec/SSH with `aether update --yes`; normal exec approvals still apply.",
          ]
        : []),
      "",
      ...skillsSection,
      ...
      "User-editable; Aether loads below as Project Context.",
      "",
    ];
