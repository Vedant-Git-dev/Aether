...
export function formatUnknownToolIdError(
  ...
  const canReadSkills = entries.some(
    (entry) => entry.source === "aether" && entry.sourceName === "core" && entry.name === "read",
  );
  ...
  const recoveryText =
    options.recoverySurface === "code-mode"
      ? "Use aether.tools.search to find a tool, aether.tools.describe to inspect it, then aether.tools.call with the exact id or name."
