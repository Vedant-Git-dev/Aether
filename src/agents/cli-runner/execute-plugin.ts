import { stripSystemPromptCacheBoundary } from "@aether/ai/internal/shared";
import { clampPositiveTimerTimeoutMs } from "@aether/normalization-core/number-coercion";
...
function createPluginToolPermissionHandler(params: {
  ...
  return async (request) => {
    ...
    try {
      ...
      return denyTool("Aether denied native tool use: the admitted run is no longer active.");
    }

    const toolName = request.toolName.trim();
    if (!toolName) {
      return denyTool("Aether denied an unnamed native tool.");
    }
    if (run.cliToolAvailability && !run.cliToolAvailability.native.includes(toolName)) {
      return denyTool(`Aether denied native tool ${toolName}: it is unavailable to this run.`);
    }
    ...
    if (nativeFileTool) {
      ...
      if (typeof nativePath !== "string") {
        return denyTool("Aether denied native file tool use: invalid file path.");
      }
      if (Object.hasOwn(request.toolInput, "path") && request.toolInput.path !== nativePath) {
        return denyTool("Aether denied native file tool use: conflicting file paths.");
      }
      ...
      if (canonicalToolName === "edit") {
        ...
        if (typeof oldText !== "string" || typeof newText !== "string") {
          return denyTool("Aether denied native edit tool use: invalid replacement.");
        }
        if (
          edits !== undefined &&
          (!Array.isArray(edits) ||
          ...
          return denyTool("Aether denied native edit tool use: conflicting replacements.");
        }
    ...
    try {
      ...
      return denyTool("Aether denied native tool use: the admitted run closed during policy.");
    }
    if (hookResult.blocked) {
      return denyTool(hookResult.reason);
    }
    if (!isRecord(hookResult.params)) {
      return denyTool("Aether denied native tool use: before_tool_call returned invalid input.");
    }
    let toolInput = hookResult.params;
    // SDK permission replies must return the native schema, never policy-only aliases.
    if (nativeFileTool) {
      if (typeof toolInput.path !== "string") {
        return denyTool("Aether denied native file tool use: invalid rewritten file path.");
      }
      if (toolInput === policyInput) {
        ...
        if (canonicalToolName === "edit") {
          ...
          if (
            ...
            return denyTool("Aether denied an unrepresentable native edit rewrite.");
          }
    ...
    if (plan === "deny") {
      return denyTool(
        `Aether exec policy denied native tool use (security=${permission.security}, ask=${permission.ask}).`,
      );
    }
    ...
    try {
      ...
      return denyTool("Aether denied native tool use: the admitted run closed during approval.");
    }
    if (outcome.kind !== "allow") {
      return denyTool(
        outcome.message ??
          (outcome.reason === "user"
            ? `Aether user denied native tool use (${toolName}).`
            : `Aether approval was not granted for native tool use (${toolName}).`),
      );
    }
  ...
  return async (request) => {
    ...
    try {
      ...
      return cancelUserInput(
        "Aether cancelled operator input: the admitted run is no longer active.",
      );
    }
    ...
    if (
      ...
      return cancelUserInput(
        toolName
          ? `Aether cancelled operator input from ${toolName}: it is unavailable to this run.`
          : "Aether cancelled an unnamed operator input request.",
      );
    }
    if (request.questions.length === 0 || request.questions.length > 12) {
      return cancelUserInput("Aether cancelled an invalid operator input request.");
    }
    ...
    try {
      ...
      try {
        ...
        return cancelUserInput(
          "Aether cancelled operator input: the admitted run closed before the answer was committed.",
        );
      }
      return result.status === "answered"
        ...
        : cancelUserInput(
            result.message ??
              "Aether cancelled operator input; continue with your best judgment.",
          );
    } catch {
      return cancelUserInput(
        "Aether could not collect operator input; continue with your best judgment.",
      );
    } finally {
      params.onPendingInput(-1);
    }
