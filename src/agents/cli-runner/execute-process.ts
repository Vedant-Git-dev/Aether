  ...
  try {
    if (params.nodePlacement) {
      ...
      result = await executePluginOwnedProcess({
        ...
        forceNewSession:
          params.cliSessionIdToUse === undefined && context.aetherHistoryPrompt !== undefined,
  ...
  if (!terminalInterruption && (result.exitCode !== 0 || result.reason !== "exit")) {
    ...
    if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
      ...
      const deferNotice =
        ...
        Boolean(context.aetherHistoryPrompt) &&
      ...
      if (runParams.sessionKey && params.events.emitLiveEvents && !deferNotice) {
        const stallNotice = [
          ...
          "Check CLI permission settings and Aether approval prompts.",
        ].join(" ");
