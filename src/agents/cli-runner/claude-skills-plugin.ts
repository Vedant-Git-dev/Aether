/**
 * Materializes selected Aether skills as a temporary Claude CLI plugin.
 */
...
const AETHER_CLAUDE_PLUGIN_NAME = "aether-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, AETHER_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: AETHER_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Aether skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
