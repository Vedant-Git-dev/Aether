...
export const CLI_BACKEND_PRESERVE_ENV = "AETHER_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.AETHER_MCP_TOKEN ? "set" : "missing"}`,
    `capture=${childEnv.AETHER_MCP_CLI_CAPTURE_KEY ? "set" : "missing"}`,
  ].join(" ");
}
...
export function logCliInvocation(params: {
  ...
  if (params.env.AETHER_MCP_TOKEN) {
