...
import { MAX_IMAGE_BYTES } from "@aether/media-core/constants";
import { extensionForMime } from "@aether/media-core/mime";
import {
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredAetherTmpDir(), "aether-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".aether-cli-images");
  }
  return path.join(resolvePreferredAetherTmpDir(), "aether-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "aether-cli-system-prompt-",
  });
