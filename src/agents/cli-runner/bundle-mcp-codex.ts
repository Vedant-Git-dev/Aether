...
/**
 * Applies Codex-only agent scoping before Aether resolves credentials or opens transports.
...
function selectCodexProjectableMcpServers(
  ...
  return Object.fromEntries(
    Object.entries(staticServers).filter(([serverName, server]) => {
      ...
      // Remote app servers cannot receive Aether-managed bearer credentials.
...
/** Async runtime projection that resolves Aether-managed MCP bearer tokens. */
...
export async function buildCodexUserMcpServersThreadConfigPatchForRun(params: {
  ...
  const projectionConfig: AetherConfig = {
    ...run.config,
