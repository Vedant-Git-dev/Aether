...
export async function executePreparedCliRun(
  ...
  const context =
    !cliSessionIdToUse && inputContext.aetherHistoryPrompt && inputContext.cliHistoryWriter
      ? {
          ...inputContext,
          params: {
            ...inputContext.params,
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.aetherHistoryPrompt ?? params.prompt);
  let prompt =
    ...
    try {
      cliBackendLog.info(
        buildCliExecLogLine({
          ...
          hasHistoryPrompt: Boolean(context.aetherHistoryPrompt),
        }),
      );
        ...
        const resolvedExecutionArgs = context.backendResolved.resolveExecutionArgs?.({
          ...
          toolAvailability:
            params.cliToolAvailability && nodePlacement
              ? { native: params.cliToolAvailability.native, aether: [] }
