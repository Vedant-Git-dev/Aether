/**
 * Claude CLI argument helpers for Aether-managed bundle MCP config.
 */
...
import { withAetherMcpCaptureHeader } from "./bundle-mcp-runtime.js";
...
export function findClaudeMcpConfigPaths(args?: string[]): string[] {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mcp-config") {
      ...
      // as positional prompts after Aether injects its strict overlay.
...
/** Return Claude args with Aether's strict MCP config path injected. */
...
/** Writes the active per-attempt capture token into Aether's generated Claude MCP config. */
export async function writeClaudeMcpCaptureConfig(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      withAetherMcpCaptureHeader(
        raw,
        params.captureKey,
        "Claude MCP capture requires an aether server config",
      ),
      null,
      2,
    )}\n`,
