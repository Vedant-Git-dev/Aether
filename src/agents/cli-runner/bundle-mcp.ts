...
import {
  ...
  AETHER_TOOLS_MCP_SYSTEM_AGENT_PROPOSAL_ENV,
  AETHER_TOOLS_MCP_TOOLS_ENV,
} from "../../mcp/aether-tools-serve-config.js";
import {
  extractMcpServerMap,
...
function normalizeAetherLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<aether-loopback>${match[2]}`;
}
...
function canonicalizeSystemAgentTurnStateForResume(
  ...
  if (!isRecord(server.env) || server.env[AETHER_TOOLS_MCP_TOOLS_ENV] !== "aether") {
    return server;
  }
  ...
  return {
    ...
    env: {
      ...server.env,
      [AETHER_TOOLS_MCP_SYSTEM_AGENT_APPROVAL_ARMED_ENV]: "<aether-turn-state>",
      [AETHER_TOOLS_MCP_SYSTEM_AGENT_PROPOSAL_ENV]: "<aether-turn-state>",
    },
  };
}

function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  // The Aether loopback MCP port changes across runs. Replace it before
  ...
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      ...
      if (name !== "aether" || typeof canonicalServer.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...canonicalServer,
          url: normalizeAetherLoopbackUrl(canonicalServer.url),
        }),
      ];
    }),
...
const AETHER_MCP_ENV_TEMPLATE_PATTERN = /\$\{(AETHER_MCP_[A-Z0-9_]+)\}/g;
...
function resolveAetherMcpEnvTemplates(value: unknown, env?: Record<string, string>): unknown {
  if (!env) {
    return value;
  }
  if (typeof value === "string") {
    return value.replace(AETHER_MCP_ENV_TEMPLATE_PATTERN, (match, name: string) => {
      const replacement = env[name];
      return Object.hasOwn(env, name) && replacement !== undefined ? replacement : match;
    });
  }
  if (Array.isArray(value)) {
    return value.map((entry) => resolveAetherMcpEnvTemplates(entry, env));
  }
  if (!isRecord(value)) {
    return value;
  }
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [key, resolveAetherMcpEnvTemplates(entry, env)]),
  );
}
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const runtimeConfig = resolveAetherMcpEnvTemplates(
    params.mergedConfig,
    params.env,
  ) as BundleMcpConfig;
...
export async function prepareCliBundleMcpConfig(params: {
   ...
   * Ring-zero Aether runs use this so the CLI harness sees only the
   * aether MCP server instead of the normal aether tool surface.
   */
  ...
  if (params.nativeMcpPolicy && Object.keys(policyConfig.mcpServers).length > 0) {
    ...
    const runtimeConfig: AetherConfig = {
      ...params.config,
...
export async function prepareCliBundleMcpCaptureAttempt(params: {
  ...
  return {
    env: {
      ...params.env,
      AETHER_MCP_CLI_CAPTURE_KEY: params.captureKey,
    },
  };
}
