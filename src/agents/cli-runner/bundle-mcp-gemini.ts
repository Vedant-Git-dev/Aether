...
import { withAetherMcpCaptureHeader, writeTemporaryBundleMcpJson } from "./bundle-mcp-runtime.js";
...
async function writeGeminiSettings(
  ...
  const temporary = await writeTemporaryBundleMcpJson("aether-gemini-mcp-", settings);
  return {
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const temporary = await writeTemporaryBundleMcpJson(
    "aether-gemini-mcp-attempt-",
    withAetherMcpCaptureHeader(settings, params.captureKey),
  );
  return {
    env: {
      ...params.inheritedEnv,
