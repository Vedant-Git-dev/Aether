...
export function buildCliLiveSessionFingerprint(params: {
  ...
  const normalizeGrantToken = params.env.AETHER_MCP_TOKEN === managedGrant?.transportToken;
  ...
  return sha256Hex(
    JSON.stringify({
      ...
      env: Object.keys(params.env)
        .toSorted()
        .filter((key) => key !== "AETHER_MCP_CLI_CAPTURE_KEY")
        .map((key) => [
          key,
          key === "AETHER_MCP_TOKEN" && normalizeGrantToken
            ? "<managed-mcp-grant>"
            : params.env[key]
