...
export function withAetherMcpCaptureHeader(
  config: Record<string, unknown>,
  captureKey: string,
  missingServerError?: string,
): Record<string, unknown> {
  const mcpServers = isRecord(config.mcpServers) ? config.mcpServers : {};
  const aether = isRecord(mcpServers.aether) ? mcpServers.aether : undefined;
  if (!aether && missingServerError) {
  ...
  return applyMergePatch(config, {
    mcpServers: {
      aether: {
        headers: {
          "x-aether-cli-capture-key": captureKey,
        },
      },
    },
