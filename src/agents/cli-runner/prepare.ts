import { ensureSystemPromptCacheBoundary } from "@aether/ai/internal/shared";
/**
...
import { buildSystemAgentToolsMcpServerConfig } from "../../mcp/aether-tools-serve-config.js";
...
function unsupportedIsolatedCompletionError(backendId: string): Error & { code: "unsupported" } {
  const error = new Error(
    `CLI backend "${backendId}" does not support isolated completion; Aether did not start the run.`,
...
type RunCliAgentPrepareParams = RunCliAgentParams & {
  /** Ring-zero tool transport supplied only by the Aether orchestrator. */
...
const defaultPrepareDeps = {
  ...
  resolveMcpLoopbackPolicyTools,
  resolveMcpLoopbackScopedTools,
  resolveAetherReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveAetherReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveAetherReferencePaths(params),
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.cliRunnerPrepareTestApi")] = {
...
function buildCliAuthProfileResolutionError(params: {
  ...
  return new CliAuthProfilePreparationError({
    message: `CLI backend "${params.backendId}" ${reason}. Re-authenticate with: ${loginCommand}. Aether did not start the run.`,
    profileId: params.profileId,
...
async function prepareCliRunContextWithinReadFence(
  ...
  const rootedToolsAllow = params.rootedExecution
    ? params.cliToolAvailability?.aether
    : undefined;
  if (params.toolsAllow !== undefined) {
    ...
    if (params.toolsAllow.some((toolName) => normalizeToolPolicyName(toolName) === "*")) {
      ...
      const fallbackAetherTools = uniqueStrings(
        expandToolGroups(params.toolsAllow)
          .map((toolName) => normalizeToolPolicyName(toolName))
          .filter(Boolean),
      );
      if (
        fallbackAetherTools.includes("write") &&
        !fallbackAetherTools.includes("apply_patch")
      ) {
        fallbackAetherTools.push("apply_patch");
      }
      params = {
        ...params,
  ...
  if (params.disableTools === true && !isSideQuestion && canEnforceExactToolAvailability) {
    ...
    params = {
      ...params,
      toolsAllow: undefined,
      cliToolAvailability: { native: [], aether: [] },
    };
  }
  ...
  if (params.rootedExecution) {
    ...
    params = {
      ...
      cliToolAvailability: { native: [], aether: params.cliToolAvailability?.aether ?? [] },
    };
  ...
  if (nodeClaudePlacement && params.cliToolAvailability) {
    ...
    params = {
      ...
      cliToolAvailability: {
        ...
        aether: params.cliToolAvailability.aether.filter((name) => name === "skill_workshop"),
      },
    };
  }
  if (params.cliToolAvailability !== undefined && !canEnforceExactToolAvailability) {
    ...
    throw new Error(
      `CLI backend "${backendResolved.id}" cannot enforce this run's tool cap. Upgrade its plugin and retry; if current, ask its maintainer to add exact-cap support. Aether did not start the run.`,
    );
  }
  ...
  // preflight, refresh, or forward Aether's snapshot; the installed Claude
  ...
  let aetherHistoryMessages: unknown[] | undefined;
  const loadAetherHistoryMessages = async () => {
    aetherHistoryMessages ??= await loadCliSessionHistoryMessages(params);
    return aetherHistoryMessages;
  };
  ...
  const promptBuildHookResult = await (async () => {
    ...
    try {
      return await resolvePromptBuildHookResult({
        ...
        messages: await loadAetherHistoryMessages(),
  ...
  // Aether's system prompt. Only a declared native-tool backend can complete
  ...
  // Ring-zero Aether runs replace the bundle MCP surface entirely: no
  // loopback server, no plugin/user servers. A selectable backend also removes
  // its native tools, leaving only this aether stdio server.
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    try {
      ...
      throw new Error(
        `Bundled MCP is enabled, but the Aether MCP loopback server failed to start: ${String(error)}`,
        { cause: error },
      );
    }
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    throw new Error(
      "Bundled MCP is enabled, but the Aether MCP loopback server did not publish a runtime after startup.",
    );
  }
  ...
  const requestedLoopbackToolsAllow =
    ...
    (rootedExecution ? rootedToolsAllow : params.cliToolAvailability?.aether);
  ...
  if (
    ...
    throw new Error(
      `CLI backend "${backendResolved.id}" cannot enforce before_prompt_build tool restrictions. Use a backend with exact tool availability or remove the hook restriction. Aether did not start the run.`,
    );
  }
  if (
    (promptBuildRestrictsTools &&
    ...
    params = {
      ...
      cliToolAvailability: {
        native: [],
        aether: hookFilteredProjectedTools.map((tool) => tool.name),
      },
    };
  }
  if (params.cliToolAvailability && promptBuildToolsAllow !== undefined) {
    ...
    params = {
      ...
      cliToolAvailability: {
        ...
        aether: filterToolNames(params.cliToolAvailability.aether),
      },
    };
  }
  ...
  const toolBoundExtraSystemPromptHash = params.cliToolAvailability
    ? hashCliSessionText(
        JSON.stringify([
          ...
          params.cliToolAvailability.aether.toSorted(),
        ]),
      )
  ...
  try {
      ...
      rawLoopbackServerConfig && backendResolved.bundleMcpMode === "claude-config-file"
        ? {
            ...
            mcpServers: {
              ...
              aether: {
                ...rawLoopbackServerConfig.mcpServers.aether,
    ...
    const preparedBackend = await prepareCliBundleMcpConfig({
      ...
      env:
        mcpLoopbackRuntime && mcpClientGrant
          ? {
              AETHER_MCP_TOKEN: mcpClientGrant.token,
              AETHER_MCP_CLI_CAPTURE_KEY: "",
            }
          : undefined,
      warn: (message) => cliBackendLog.warn(message),
    ...
    const aetherReferences = skipsTurnPreparation
      ? { docsPath: null, sourcePath: null }
      : await prepareDeps.resolveAetherReferencePaths({
          workspaceDir,
    ...
    const builtSystemPrompt = isControlOperation
      ...
      : isSideQuestion
        ...
        : buildCliAgentSystemPrompt({
            ...
            docsPath: aetherReferences.docsPath ?? undefined,
            sourcePath: aetherReferences.sourcePath ?? undefined,
    ...
    // gateway-side Aether transcript, so a fresh remote CLI session still
    // receives prior conversation context via stdin.
    const shouldPrepareAetherHistoryPrompt =
      !skipsTurnPreparation && (!reusableCliSessionId || allowRawTranscriptReseed);
    let aetherHistoryPrompt = shouldPrepareAetherHistoryPrompt
      ? buildCliSessionHistoryPrompt({
    ...
    if (note) {
      ...
      if (executionTarget.kind === "plugin") {
        ...
        if (aetherHistoryPrompt) {
          aetherHistoryPrompt = append(aetherHistoryPrompt);
        }
      }
    }
    return {
      ...
      ...(aetherHistoryPrompt ? { aetherHistoryPrompt } : {}),
      ...(cliHistoryWriter ? { cliHistoryWriter } : {}),
      authEpoch,
      authBindingFingerprint,
      ...(skipLocalCredentialEpoch ? { authBindingSkipsLocalCredential: true } : {}),
