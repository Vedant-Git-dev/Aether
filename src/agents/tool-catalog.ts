...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  {
    id: "gateway",
    description: "Read Gateway config/schema; owner-only Aether self-update",
    sectionId: "automation",
    profiles: [],
    includeInAetherGroup: true,
  },
...
function buildCoreToolGroupMap() {
  ...
  const aetherTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInAetherGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:aether": aetherTools,
