...
import { AETHER_CLI_ENV_VAR } from "../infra/aether-exec-env.js";
import {
...
const CHANNEL_CONTEXT_ENV_KEY = "AETHER_CHANNEL_CONTEXT";
...
export function resolvePreparedExecEnvironment(params: {
  ...
  const storeEnvResult = params.storeEnv
    ...
    : undefined;
  const { [AETHER_CLI_ENV_VAR]: _storeMarker, ...acceptedStoreEnv } = storeEnvResult?.env ?? {};
  ...
  if (params.storeEnv && Object.hasOwn(params.storeEnv, AETHER_CLI_ENV_VAR)) {
    rejectedStoreKeys.add(AETHER_CLI_ENV_VAR);
  }
