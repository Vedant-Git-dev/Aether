...
export function isSharedAuthStoreOwner(params: {
  ...
  return (
    ...
    isSameAetherAgentDatabasePath(params.agentAuthDbPath, params.sharedAuthDbPath)
  );
}

export function formatSharedAuthStoreOwnerDeleteError(agentId: string): string {
  return `Agent "${agentId}" owns the legacy shared auth store and cannot be deleted. Run aether vitals --fix to migrate shared auth, then retry.`;
}

export function isInheritedAuthStoreOwner(cfg: AetherConfig, agentId: string): boolean {
