...
export function createMcpStdioClient(params: McpStdioClientParams): McpStdioClient {
  ...
  class StdioTransport extends AetherStdioClientTransport {
    override onerror: AetherStdioClientTransport["onerror"] = (error) =>
      fail(
    ...
    override onexit: AetherStdioClientTransport["onexit"] = ({ code, signal }) => {
      if (!stopped && !failure) {
