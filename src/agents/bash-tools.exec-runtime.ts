...
export const DEFAULT_MAX_OUTPUT = clampWithDefault(
  readEnvInt("AETHER_BASH_MAX_OUTPUT_CHARS", "PI_BASH_MAX_OUTPUT_CHARS"),
  200_000,
  1_000,
  200_000,
);
...
export const DEFAULT_PENDING_MAX_OUTPUT = clampWithDefault(
  readEnvInt("AETHER_BASH_PENDING_MAX_OUTPUT_CHARS"),
  30_000,
  1_000,
  200_000,
);
...
function emitExecProcessCompleted(params: {
  ...
  // Payload stays untrusted, but the ambient trace context is the Aether run
  ...
  switch (params.failureKind) {
    ...
    case "overall-timeout": {
      ...
      return params.processContinuationAvailable
        ? `${retryGuidance}\n\nIf it should keep running, start it with exec background=true or yieldMs so Aether can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`
        : retryGuidance;
    }
...
function wrapPosixCommandWithPathPrepend(
  ...
  env.AETHER_PREPEND_PATH = pathPrepend.join(path.delimiter);

  return `export PATH="\${AETHER_PREPEND_PATH}\${PATH:+:$PATH}"; unset AETHER_PREPEND_PATH; ${command}`;
}
