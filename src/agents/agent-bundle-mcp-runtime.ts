...
const BUNDLE_MCP_TEST_STATE_KEY = Symbol.for("aether.bundleMcpTestState");
  ...
  const loadCatalog = async (): Promise<McpToolCatalog> => {
    ...
    const inFlight = (async (): Promise<McpToolCatalog> => {
      ...
      try {
        ...
        for (const [tool, excludedFromAetherCatalog, deniedBySession] of [
          ...
          const entry: McpCatalogTool = {
            ...
            ...(excludedFromAetherCatalog ? { excludedFromAetherCatalog: true as const } : {}),
          ...
          if (!entry.excludedFromAetherCatalog) {
