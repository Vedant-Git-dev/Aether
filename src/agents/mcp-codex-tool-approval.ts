...
function isAetherLoopbackServer(name: string, server: McpServerConfig): boolean {
  return (
    name === "aether" &&
...
export function resolveProjectedMcpCodexToolApprovalMode(
  ...
  return (
    ...
    (isAetherLoopbackServer(serverName, server) ? "approve" : undefined)
  );
}
...
export function formatMcpCodexApprovalRemedy(serverName?: string): string {
  ...
  return `Run aether mcp configure ${server} --approval approve for a trusted server, or change the session permission mode.`;
}
