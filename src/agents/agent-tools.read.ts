...
import { detectMime } from "@aether/media-core/mime";
...
export function resolveAdaptiveReadMaxBytes(options?: AetherReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".aether/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.aether/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with Aether result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  ...
  return createAetherReadTool(base, {
...
/** Wrap the base read tool with Aether paging, MIME, and image handling. */
export function createAetherReadTool(
  base: AnyAgentTool,
  options?: AetherReadToolOptions,
): AnyAgentTool {
    ...
    execute: async (toolCallId, args, signal, onUpdate) => {
      ...
      const instructionTool =
        typeof instructionContent.get(instructionPath) === "string"
          ? (virtualRead ??= createAetherReadTool(
