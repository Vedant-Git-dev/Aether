...
/** Returns whether a server needs an Aether-managed bearer projected externally. */
...
function buildTokenEnvVarName(serverName: string): string {
  ...
  return `AETHER_MCP_AUTH_${hash.toUpperCase()}_TOKEN`;
}

function stripAetherOnlyOAuthConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  const next = { ...server };
  delete next.auth;
  delete next.oauth;
  return next;
...
export async function resolveMcpBearerBundleConfig(
  ...
  for (const [serverName, server] of Object.entries(params.config.mcpServers)) {
    ...
    nextServers ??= { ...params.config.mcpServers };
    nextServers[serverName] = stripAetherOnlyOAuthConfig({
      ...server,
      headers: {
        ...headers,
