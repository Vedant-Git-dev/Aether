...
export async function analyzeNodeApprovalRequirement(params: {
  ...
  if (
    ...
    params.request.warnings.push(
      `${obsoleteGeneratedApprovalCount} older generated exec ${obsoleteGeneratedApprovalCount === 1 ? "approval is" : "approvals are"} inactive on this node because they are not tied to a working directory. Run "aether vitals --fix" on the node, then rerun the workflow and choose "Always allow here".`,
    );
  }
