  ...
  try {
    ...
    const blockedReason = alreadyWedged
      ...
      : attempts >= MAX_RECOVERY_ATTEMPTS
        ? `subagent orphan recovery blocked after ${attempts} rapid accepted resume attempts; ` +
          `run "aether tasks maintenance --apply" or "aether vitals --fix" to reconcile it`
        : undefined;
    if (blockedReason) {
      if (!alreadyWedged) {
        try {
    ...
    const configChanged = recoveryMessages.some(
      (message) =>
        ...
        /aether\.json|aether gateway restart|config\.patch/i.test(message.text ?? ""),
    );
          ...
          dispatched = await admission.run(() =>
            params.gatewayRuntime!.dispatchAgent<{ runId: string; status: unknown }>({
              message:
                ...
                (configChanged
                  ? "\n\n[config changes from your previous run were already applied — do not re-modify aether.json or restart the gateway]"
                  : ""),
              sessionKey: childSessionKey,
