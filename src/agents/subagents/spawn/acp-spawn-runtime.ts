...
} from "@aether/acp-core/runtime/session-identifiers";
...
export function resolveAcpSpawnRuntimeOptions(params: {
  ...
  if (modelExplicit && modelSelection.profile) {
    return {
      ok: false,
      error:
        "ACP model overrides cannot select Aether auth profiles; configure credentials in the ACP runtime instead.",
    };
  }
