...
export async function prepareSubagentSessionContext(params: {
  ...
  try {
    ...
    if (forkedResult.status === "failed" || forkedResult.status === "missing-entry") {
      throw new Error(
        'context="fork" requested but Aether could not fork the requester transcript.',
      );
    }
    if (forkedResult.status === "skipped") {
      ...
      if (!forkFallbackNote) {
        throw new Error('context="fork" requested but Aether could not prepare forked context.');
      }
      return {
        status: "ok",
        mode: "isolated",
