  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an Aether config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for Aether config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: AetherConfig, agentId: string): boolean {
...
export function resolveConfiguredAcpSubagentTargetIds(cfg: AetherConfig): string[] {
