...
function resolveAttachmentLimits(config: AetherConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".aether", "attachments");
  const relDir = path.posix.join(".aether", "attachments", attachmentId);
