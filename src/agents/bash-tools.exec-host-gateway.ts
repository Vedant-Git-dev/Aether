...
function formatDiagnosticsExportSuccess(aggregated: string): string {
  ...
  try {
    ...
    if (typeof manifest.aetherVersion === "string") {
      lines.push(`Aether version: ${manifest.aetherVersion}`);
    }
...
export async function processGatewayAllowlist(
  ...
  if (hostSecurity === "allowlist" && !allowlistSatisfied && obsoleteGeneratedApprovalCount > 0) {
    params.warnings.push(
      `${obsoleteGeneratedApprovalCount} older generated exec ${obsoleteGeneratedApprovalCount === 1 ? "approval is" : "approvals are"} inactive because they are not tied to a working directory. Run "aether vitals --fix", then rerun the workflow and choose "Always allow here".`,
    );
  }
