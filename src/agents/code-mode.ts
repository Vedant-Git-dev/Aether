...
function createCodeModeExecDescription(
  ...
  const shellTool = bindings?.find((entry) => entry.id === "aether:core:exec");
  const shellGuidance = shellTool
    ? ` Use the shell tool \`${shellTool.callableName}\` for heavier computation.`
    : "";
  return (
    `Run JavaScript or TypeScript in Aether code mode. Guest work and inline tool waits share a ${timeoutMs} ms wall-clock budget per \`exec\`/\`wait\`; approvals pause it. Guest computation over this budget times out; pending tools may return \`waiting\` for \`wait\`.` +
    shellGuidance +
  ...
  const execTool = markCodeModeControlTool({
    ...
    parameters: Type.Object({
      ...
      restartSafe: Type.Optional(
        Type.Boolean({
          description:
            "Do not set on a new exec. Set true only when Aether explicitly requests replay after a gateway restart; never for write, edit, exec, or any mutation. True rejects unmarked or namespace surfaces.",
        }),
      ),
    }),
    execute: async (
