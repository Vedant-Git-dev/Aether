...
function loadLocalSkillTiers(
  ...
  const bundledSkills = bundledSkillsDir
    ? loadSkills({ dir: bundledSkillsDir, source: "aether-bundled" })
    : [];
  ...
  const custodianSkills = custodianSkillsDir
    ? loadSkills({ dir: custodianSkillsDir, source: "aether-custodian" })
    : [];
  const extraSkills = [
    ...extraDirs.flatMap((dir) =>
      loadSkills({ dir: resolveUserPath(dir), source: "aether-extra" }),
    ),
  ...
  const managedSkills = workspaceOnly
    ? []
    : loadSkills({ dir: managedSkillsDir, source: "aether-managed" });
...
export function loadBundledSkillEntryByName(
  ...
  const loaded = loadSingleSkillDirectory({
    ...
    onDiagnostic: (diagnostic) => warnInvalidSkill("aether-bundled", diagnostic),
  });
