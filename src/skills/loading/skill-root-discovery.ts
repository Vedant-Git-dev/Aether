...
export function resolveSkillDiscoveryLimits(config?: AetherConfig): ResolvedSkillDiscoveryLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "aether-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "aether-bundled") {
    return {
...
function shouldEnforceConfiguredSkillRootContainment(source: string): boolean {
  return source !== "aether-managed" && source !== "agents-skills-personal";
}
...
export function discoverSkillCandidates(params: {
  ...
  const rootIsContainer = params.source === "aether-workshop";
