...
export async function materializeSkillResources(
  ...
  assertCurrent();
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "aether-skill-resources-"));
  ...
  try {
    ...
    for (const [index, { skill, bundle }] of bundles.entries()) {
      ...
      resolvedSkills.push({
        ...
        baseDir,
        source: "aether-resources",
        sourceInfo: createSyntheticSourceInfo(filePath, { source: "aether-resources", baseDir }),
