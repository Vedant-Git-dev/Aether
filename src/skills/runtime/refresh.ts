...
function resolveWatchTargets(
  ...
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "aether-managed" });
  ...
  for (const resolved of extraDirs) {
    addSkillSourceWatchTargets(targets, resolved, "aether-extra", allowedSymlinkTargetRealPaths);
  }
  for (const dir of pluginSkillDirs) {
    addSkillSourceWatchTargets(targets, dir, "aether-plugin", allowedSymlinkTargetRealPaths);
  }
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "aether-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.skillsRefreshTestApi")] = {
