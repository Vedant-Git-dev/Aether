...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "aether");
  }
  return path.join(homedir(), ".aether");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "aether-bundled"
        ? { kind: "bundled", authority: "aether", mutable: false, network: false }
        : skillSource === "aether-managed" || skillSource === "aether-extra"
          ? { kind: "managed", authority: "aether", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["aether-bundled", "aether-managed", "aether-extra"]);
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.skillsInstallTestApi")] =
    testing;
}
