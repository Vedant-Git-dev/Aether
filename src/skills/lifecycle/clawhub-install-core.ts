...
async function installArchiveResolution(params: {
  ...
  archivePath: string;
  registry: string;
  authority: "official" | "aether" | "third-party";
  force?: boolean;
  logger?: Logger;
  config?: AetherConfig;
  onInstallPolicyWarning?: InstallSafetyOverrides["onInstallPolicyWarning"];
  ...
  return await withExtractedArchiveRoot({
    ...
    tempDirPrefix: "aether-skill-clawhub-",
    timeoutMs: 120_000,
...
async function installGitHubResolution(params: {
  ...
  return await withExtractedArchiveRoot({
    archivePath: params.archivePath,
    tempDirPrefix: "aether-skill-clawhub-github-",
    timeoutMs: 120_000,
...
function assertInstallResolutionAllowed(
  ...
  if (!resolution.ok) {
    if (resolution.reason === "ambiguous_slug") {
      ...
      throw new Error(
        `Skill "${resolution.slug}" is ambiguous on ClawHub. Install an owner-qualified skill, for example: aether skills install @owner/${resolution.slug}.${message}`,
      );
    }
