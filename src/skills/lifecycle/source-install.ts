...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".aether", "source-origin.json");
...
async function removeClawHubInstallMetadata(targetDir: string): Promise<void> {
  await Promise.all([
    ...
    fs.rm(path.join(targetDir, ".aetherhub"), { recursive: true, force: true }),
  ]);
}
...
async function installGitSkill(params: {
  ...
  return await withInstallWorkspace("aether-git-skill-", async (tmpDir) => {
