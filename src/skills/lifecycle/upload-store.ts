...
type SkillUploadStoreOptions = AetherStateDatabaseOptions & {
...
function createSkillUploadStore(options?: SkillUploadStoreOptions) {
  ...
  function lockRoot(): string {
    return openAetherStateDatabase(stateOptions).path;
  }
  ...
  return {
    ...
    async withCommittedUpload<T>(
      ...
      return await withLock(`${root}:upload:${uploadId}`, async () => {
        ...
        const currentTime = now();
        assertNotExpired(requireUploadMetadata(uploadId, stateOptions), currentTime, stateOptions);
        const row = runAetherStateWriteTransaction(({ db }) => {
          const kysely = getNodeSqliteKysely<SkillUploadDatabase>(db);
          const current = executeSqliteQueryTakeFirstSync(
            db,
        ...
        try {
          return await withTempWorkspace(
            {
              rootDir: tempRootDir ?? resolvePreferredAetherTmpDir(),
              prefix: "aether-skill-upload-",
            },
            async (tmp) => {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.skillUploadStoreTestApi")] = {
