...
// Aether process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/aether/aether/issues/78063.
...
type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the Aether service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  const lines: string[] = [
    "GitHub CLI auth was found at a different HOME than the one this Aether process uses.",
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the Aether service environment, then restart the gateway.`,
  );
  return lines;
}
