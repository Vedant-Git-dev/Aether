...
export async function planTrackedClawHubSkillState(params: {
  ...
  if (!link.valid || !link.skillFile || !link.fileTreeSha256) {
    return {
      ...
      code: "ambiguous",
      error: link.valid
        ? `Skill ${JSON.stringify(slug)} was installed before Aether recorded file fingerprints, so local changes cannot be detected.`
        : link.reason,
    };
  }
...
export async function applyClawHubSkillUninstall(
  ...
  const before = shouldDispatchChange
    ...
    : undefined;
  const stagedDir = `${plan.targetDir}.aether-skill-remove-${randomUUID()}`;
  let staged = false;
