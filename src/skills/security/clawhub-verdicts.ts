...
type AetherSkillSecurityVerdictItem = Omit<
...
function projectClawHubVerdictItem(
...
): AetherSkillSecurityVerdictItem {
  const projected: AetherSkillSecurityVerdictItem = {
  ...
  if (item.error) {
    const error: AetherSkillSecurityVerdictItem["error"] = {};
...
export async function fetchAetherSkillSecurityVerdicts(
  targets: ClawHubVerdictTarget[],
): Promise<AetherSkillSecurityVerdictItem[]> {
  ...
  const items: AetherSkillSecurityVerdictItem[] = [];
