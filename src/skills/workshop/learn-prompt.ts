...
export function buildLearnPrompt(request: string): string {
  ...
  return [
    "Improve the Aether skill collection from the learning request below.",
    "",
    ...
    "- Include optional `metadata.aether` fields such as `emoji` or `requires.bins` only when the gathered sources prove they are true and useful.",
