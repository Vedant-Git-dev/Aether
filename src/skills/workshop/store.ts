...
export class SkillProposalDraftMissingError extends Error {
  constructor(
    ...
    super(
      `Skill proposal draft is missing: ${proposalId}. Run aether vitals --fix for recovery.`,
      options,
    );
  }
}
  ...
  return runAetherStateWriteTransaction(
    ({ db }) => {
      ...
      if (
        ...
        throw new Error(
          "Skill proposal has unfinished apply recovery. Run aether vitals --fix and restore the files it identifies before retrying.",
        );
      }
