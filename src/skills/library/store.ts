...
import type { DB as StateDatabase } from "../../state/aether-state-db.generated.js";
import {
...
export function ensureSkillLibrarySchema(options: AetherStateDatabaseOptions): void {
  ...
  const end = AETHER_STATE_SCHEMA_SQL.indexOf("-- End profile-owned skill library.", start);
  if (start < 0 || end < start) {
    throw new Error("Canonical skill library schema missing.");
  }
  runAetherStateWriteTransaction(
    ({ db: transactionDb }) => {
      transactionDb.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end)); // sqlite-allow-raw -- canonical first-use additive DDL.
    },
    options,
