...
async function publishDirectory(
  ...
  return saveSkillLibrary(
    authority,
    {
      ...
      files: files.filter(
        (file) => file !== markdown && !/^\.(?:clawhub|aetherhub)\//u.test(file.path),
      ),
    },
    options,
    uploadId,
...
export async function importSkillLibrary(
  ...
  requireSkillLibraryProfile(openAetherStateDatabase(options).db, authority);
  return withTempWorkspace(
    { rootDir: resolvePreferredAetherTmpDir(), prefix: "aether-library-source-" },
    async ({ dir }) => {
...
export async function uploadSkillLibrary(
  ...
  const readOwned = () =>
    requireSkillLibraryUpload(openAetherStateDatabase(options).db, params.uploadId, authority);
  ...
  if (upload.published_skill_id) {
    return skillLibraryReceipt(
      requireSkillLibraryEntry(
        openAetherStateDatabase(options).db,
        upload.published_skill_id,
        authority,
      ),
      "unchanged",
