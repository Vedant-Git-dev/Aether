...
import {
  openExistingAetherStateDatabaseReadOnly,
...
export async function buildClawUpdatePlan(params: {
  ...
  targetAetherProfile?: ClawAetherProfile;
  targetSource: ClawSourceIdentity;
  config: AetherConfig;
  sourceMcpServers: Record<string, Record<string, unknown>>;
  stateOptions?: AetherStateDatabaseOptions & { packageDeps?: PackageRemovalDeps };
  ...
  const database =
    ...
    (await openExistingAetherStateDatabaseReadOnly(params.stateOptions));
  if (!database) {
  ...
  const readOnlyStateOptions: AetherStateDatabaseOptions & {
  ...
  try {
    ...
    const targetPlan = await buildClawAddPlan({
      ...
      aetherProfile: params.targetAetherProfile,
      source: params.targetSource,
      diagnostics: params.diagnostics,
      context: {
        agentId,
    ...
    const targetPackages = clawTargetPackages(params.targetManifest, params.targetAetherProfile);
    ...
    for (const [key, target] of targetPackages) {
      ...
      if (failedPackageMutationPreflight) {
        ...
        const extensionIndex =
          params.targetAetherProfile?.extensions?.findIndex(
            (extension) => clawPackageKey(extension) === key,
          ) ?? -1;
        const path =
          packageIndex >= 0
            ? `$.packages[${packageIndex}]`
            : `$.profiles.aether.extensions[${extensionIndex}]`;
