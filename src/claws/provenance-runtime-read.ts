...
import {
  ...
  resolveDatabasePath,
} from "../state/aether-state-db-maintenance.js";
import { withExistingAetherStateDatabaseReadOnly } from "../state/aether-state-db-readonly.js";
import {
  registerAetherStateDatabaseLifecycleListener,
...
registerAetherStateDatabaseLifecycleListener((event) => {
  ...
  if (event.kind === "opened") {
    ...
    snapshotsByPath.set(event.path, {
      kind: "state-error",
      error: new Error("Aether state database closed before consent provenance verification."),
      knownAgentIds: knownAgentIds(previous),
      ownershipUnknown: isOwnershipUnknown(previous),
    });
  }
...
function resolveSnapshotPath(options: AetherStateDatabaseOptions): string {
...
export function initializeCachedClawInstallSchemaVersions(
  ...
  try {
    const snapshot = withExistingAetherStateDatabaseReadOnly(({ db, path: pathname }) => {
      ...
      snapshotsByPath.set(
        ...
        previousAgentIds.size > 0 || (previous !== undefined && isOwnershipUnknown(previous))
          ? {
              kind: "state-error",
              error: new Error(
                "Aether state database disappeared after Claw ownership was observed.",
              ),
