...
import type { Aetheriagnostic, ClawReadResult } from "./types.js";

export const CLAW_PROJECT_RESULT_SCHEMA_VERSION = "aether.clawProject.v1" as const;
...
type ClawProjectValidationResult =
  ...
  | { ok: false; root: string; diagnostics: Aetheriagnostic[] };
...
function diagnostic(code: string, path: string, message: string): Aetheriagnostic {
...
export async function createClawProject(
  ...
  const clawMarkdown = [
    ...
    "---",
    `You are ${displayName(agentId)}, a purpose-built Aether agent.`,
    "",
  ].join("\n");
...
export async function validateClawProject(
  ...
  const record =
    packageValue && typeof packageValue === "object" && !Array.isArray(packageValue)
      ...
      : undefined;
  const aether =
    record?.aether && typeof record.aether === "object" && !Array.isArray(record.aether)
      ? (record.aether as Record<string, unknown>)
      : undefined;
  const scripts = record?.scripts;
  const diagnostics: Aetheriagnostic[] = [];
  if (aether?.claw !== "CLAW.md") {
    diagnostics.push(
      diagnostic(
        "project_manifest_must_be_claw_markdown",
        "package.json.aether.claw",
        'A Claw project must set aether.claw to "CLAW.md".',
      ),
    );
  }
  if (
  ...
  const excludedSource = [
    ...(claw.snapshot.aetherProfile
      ? [
          {
            path: claw.snapshot.aetherProfile.sourcePath,
            diagnosticPath: "$.metadata.aether.config",
          },
        ]
      : []),
  ...
  const selectedPathList = [
    ...
    ...(claw.snapshot.aetherProfile ? [claw.snapshot.aetherProfile.sourcePath] : []),
