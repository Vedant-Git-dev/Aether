// Shared types for grouped Aether Claw manifests and read-only add plans.
import type { ToolProfileId } from "../agents/tool-policy-shared.js";
import type { AgentConfig } from "../config/types.agents.js";

export const CLAW_SCHEMA_VERSION = 1 as const;
export const CLAW_ADD_PLAN_SCHEMA_VERSION = "aether.clawAddPlan.v1" as const;
export const CLAW_INSPECT_RESULT_SCHEMA_VERSION = "aether.clawInspect.v1" as const;
export const CLAW_OUTPUT_STABILITY = "experimental" as const;

type AetheriagnosticLevel = "error" | "warning";

export type Aetheriagnostic = {
  level: AetheriagnosticLevel;
  code: string;
...
type ClawExtensionFormat = "aether" | "claude" | "codex" | "cursor";

export type ClawAetherExtension = {
  id: string;
  kind: "plugin";
...
type ClawSourceSnapshot = {
  ...
  aetherProfile?: ClawProfileSourceSnapshot;
...
export type ClawReadResult =
  | {
      ...
      legacyAetherProfile?: ClawAetherProfile;
...
export type ClawExtensionPlan = ClawAetherExtension & {
  detectedFormat?: ClawExtensionFormat;
  integrity?: string;
  installId?: string;
