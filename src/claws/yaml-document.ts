...
import type { Aetheriagnostic } from "./types.js";

function diagnostic(code: string, message: string): Aetheriagnostic {
...
export function parseClawYaml(
...
): { ok: true; value: unknown } | { ok: false; diagnostics: Aetheriagnostic[] } {
  const [invalidCode, unsupportedCode, label]: [string, string, string] =
    kind === "frontmatter"
      ...
      : [
          "invalid_aether_profile",
          "unsupported_aether_profile_yaml_feature",
          "Aether profile YAML",
        ];
