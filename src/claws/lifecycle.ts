...
export async function buildClawAddPlan(params: {
  ...
  const workspace = canonicalWorkspacePath(
    context.workspace ?? resolve(homedir(), ".aether", `workspace-${finalId}`),
  );
  ...
  const aetherAgentSettings = params.aetherProfile?.agent ?? {};
  const persistedAetherAgentSettings = params.reconstructLegacyDynamicToolProfilePlan
    ? aetherAgentSettings
    : materializeClawToolProfile(aetherAgentSettings);
  const agentConfig: ClawAddPlan["agent"]["config"] = {
    ...params.manifest.agent,
    ...persistedAetherAgentSettings,
    id: finalId,
    workspace,
  };
  if (agentBlocked) {
  ...
  const agentCapabilityEffect = {
    ...(aetherAgentSettings.sandbox ? { sandbox: aetherAgentSettings.sandbox } : {}),
    ...(aetherAgentSettings.tools ? { tools: aetherAgentSettings.tools } : {}),
    ...(aetherAgentSettings.memory ? { memory: aetherAgentSettings.memory } : {}),
    ...(aetherAgentSettings.heartbeat ? { heartbeat: aetherAgentSettings.heartbeat } : {}),
  };
  ...
  const extensionPlan = await planClawExtensions({
    extensions: params.aetherProfile?.extensions ?? [],
    workspace,
