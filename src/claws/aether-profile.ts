// Safe loader for the conventional package-local Aether profile.
import { asOptionalRecord as record } from "@aether/normalization-core/record-coerce";
...
import { parseClawAetherProfile } from "./schema.js";
import {
  materializeClawToolProfile,
  resolveClawToolProfileSnapshot,
} from "./tool-profile-consent.js";
import type { Aetheriagnostic, ClawAetherProfile } from "./types.js";
import { parseClawYaml } from "./yaml-document.js";

const MAX_PROFILE_BYTES = 256 * 1024;
const CLAW_PROFILE_PATH = "profiles/aether.yml";
const LEGACY_PROFILE_POINTER_KEY = "aether.config";
const LEGACY_PROFILE_POINTER_PATH = "$.metadata.aether.config";
const CONVENTIONAL_PROFILE_PATH = "$.profiles.aether";

function diagnostic(code: string, message: string, path = "$"): Aetheriagnostic {
  return { level: "error", code, phase: "parse", path, message };
}

function warning(code: string, message: string, path: string): Aetheriagnostic {
...
function migrateLegacyDynamicToolProfile(value: unknown): {
  value: unknown;
  legacyProfile?: ClawAetherProfile;
} {
  ...
  const validationProbe = parseClawAetherProfile({
    ...profile,
    agent: {
      ...agent,
      tools: {
  ...
  const legacyProfile: ClawAetherProfile = {
...
/**
 * Resolves the Aether profile for a package.
 *
 * `profiles/aether.yml` is the conventional location. The retired
 * `metadata.aether.config` pointer is still read for compatibility with
 ...
 */
export async function readClawAetherProfile(params: {
  packageRoot: string;
  ...
  | {
      ok: true;
      profile?: ClawAetherProfile;
      legacyProfile?: ClawAetherProfile;
      raw?: Buffer;
      path?: string;
      diagnostics?: Aetheriagnostic[];
    }
...
> {
  ...
  const diagnostics: Aetheriagnostic[] = [];
  ...
  if (legacyPointer !== undefined) {
    ...
    if (conventionalExists && legacyPointer !== CLAW_PROFILE_PATH) {
      return {
        ...
        diagnostics: [
          diagnostic(
            "conflicting_aether_profile_pointer",
    ...
    diagnostics.push(
      warning(
        "deprecated_aether_profile_pointer",
  ...
  try {
    ...
    return {
      ...
      diagnostics: [
        diagnostic(
          unsafe
            ? "aether_profile_unsafe"
            : tooLarge
              ? "aether_profile_too_large"
              : "aether_profile_read_failed",
          unsafe
            ? "The Aether profile must be a regular, non-symlinked, non-hardlinked file."
            : tooLarge
              ? `The Aether profile exceeds ${MAX_PROFILE_BYTES} bytes.`
  ...
  const migration = params.allowLegacyDynamicToolProfile
    ...
    : { value: yaml.value };
  const parsed = parseClawAetherProfile(migration.value);
  if (!parsed.ok) {
    return {
      ok: false,
