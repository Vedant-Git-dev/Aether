...
const aetherExtensionSchema = z
  .object({
    id: agentId,
    kind: z.literal("plugin"),
    format: z.enum(["aether", "claude", "codex", "cursor"]),
...
const aetherProfileSchema = z
  .object({
    ...
    agent: z
          ...
          .object({
            profile: nonEmptyString
              .refine(
                ...
                "Tool profile must name a registered Aether built-in profile.",
              )
              .optional(),
            allow: z
              ...
              .superRefine((search, ctx) => {
                if (
                  ...
                  ctx.addIssue({
                    ...
                    message:
                      "The sessions source requires rememberAcrossConversations: true in the Aether profile.",
                  });
                }
              })
              .optional(),
      ...
      .strict(),
    extensions: z.array(aetherExtensionSchema).optional().default([]),
  })
  .strict()
...
function diagnosticsFromZodError(error: z.ZodError): Aetheriagnostic[] {
...
export function parseClawManifest(
  ...
  | { ok: true; manifest: ClawManifest; diagnostics: Aetheriagnostic[] }
...
export function parseClawAetherProfile(value: unknown):
  | {
      ok: true;
  ...
  | { ok: false; diagnostics: Aetheriagnostic[] } {
  const parsed = aetherProfileSchema.safeParse(value);
  if (!parsed.success) {
    return { ok: false, diagnostics: diagnosticsFromZodError(parsed.error) };
  }
  return {
    ok: true,
    profile: parsed.data as ClawAetherProfile,
    diagnostics: [],
  };
}
