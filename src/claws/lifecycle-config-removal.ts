...
} from "../state/aether-state-db-contract.js";
import {
...
type ClawAgentConfigRemovalResult = {
  ...
  configBeforeDelete: AetherConfig;
  nextConfig: AetherConfig;
};

export { digestClawAgentConfig } from "./agent-config-digest.js";

export function digestClawAgentRemovalSurface(config: AetherConfig, agentId: string): string {
...
export async function withClawAgentConfigRemoval<T>(
  ...
  const stateOptions = {
    ...
    path: openAetherStateDatabase(params.stateDatabase).path,
  };
  return await withAgentDeletion(
    ...
    async (begin) => {
      ...
      const matchesInstall = (database: AetherStateDatabase) =>
      ...
      const { existingJournal, deletion } = runAetherStateWriteTransaction((database) => {
      ...
      const assertCurrent = (database?: AetherStateDatabase) => {
        const check = (current: AetherStateDatabase) => {
        ...
        if (database) {
          ...
          runAetherStateWriteTransaction(check, stateOptions);
        }
      };
      try {
