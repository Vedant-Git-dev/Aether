...
import { coerceErrorMessage } from "@aether/normalization-core";
...
export const CLAW_ADD_RESULT_SCHEMA_VERSION = "aether.clawAddResult.v1" as const;

type ConfigCommit = (transform: (config: AetherConfig) => AetherConfig) => Promise<void>;
type ClawAddApplyOptions = AetherStateDatabaseOptions & {
...
export async function applyClawAddPlan(
  ...
  try {
    ...
    await commit((config) => {
      ...
      const configWithPreservedAgents: AetherConfig = {
        ...config,
        agents: {
          ...config.agents,
    ...
    if (options.resumePlan && installRecord.schemaVersion === "aether.clawInstallRecord.v1") {
      installRecord = persistRecord(plan, {
        ...options,
        status: "pending",
        expectedExistingRecord: options.resumeRecord,
