...
import { readClawAetherProfile } from "./aether-profile.js";
...
function fileDiagnostic(code: string, message: string, path = "$"): Aetheriagnostic {
...
function workspaceSourceDiagnostic(error: unknown, sourcePath: string): Aetheriagnostic {
...
async function buildDevelopmentSnapshot(params: {
  ...
  manifestRaw: Buffer;
  aetherProfile?: { path: string; raw: Buffer };
}): Promise<
  | {
      ok: true;
      integrity: string;
      ...
      aetherProfile?: { sourcePath: string; byteLength: number; digest: string };
      workspaceSources: ClawWorkspaceSourceSnapshot[];
      packageBootstrap?: ClawWorkspaceSourceSnapshot;
    }
  | { ok: false; diagnostics: Aetheriagnostic[] }
> {
  ...
  const aetherProfile = params.aetherProfile
    ? {
        sourcePath: params.aetherProfile.path.replaceAll("\\", "/"),
        ...snapshotFile(params.aetherProfile.raw),
      }
    : undefined;
  add("canonical-source", Buffer.from(params.source.manifestPath, "utf8"));
  add("manifest", params.manifestRaw);
  if (params.aetherProfile) {
    add(`profile:${params.aetherProfile.path.replaceAll("\\", "/")}`, params.aetherProfile.raw);
  }
...
function parsePackageJson(value: unknown): PackageJson | undefined {
  ...
  const aether = record.aether;
  if (!aether || typeof aether !== "object" || Array.isArray(aether)) {
    return undefined;
  }
  const claw = (aether as Record<string, unknown>).claw;
  if (
  ...
  return { name: record.name, version: record.version, aether: { claw } };
}
...
async function readJson(
  ...
  { ok: true; raw: Buffer; value: unknown } | { ok: false; diagnostics: Aetheriagnostic[] }
> {
  let raw: Buffer;
  try {
...
export function parseClawMarkdown(
  raw: Buffer,
  path: string,
): { ok: true; value: unknown; body: Buffer } | { ok: false; diagnostics: Aetheriagnostic[] } {
  const markdown =
...
async function readAetherocument(
  path: string,
  code: string,
...
async function resolvePackageSource(
  ...
  { ok: true; source: ResolvedClawSource } | { ok: false; diagnostics: Aetheriagnostic[] }
> {
    ...
    return {
      ...
      diagnostics: [
        fileDiagnostic(
          ...
          "package.json must declare non-empty name, version, and aether.claw fields.",
        ),
      ],
    };
  }
  if (isAbsolute(packageJson.aether.claw)) {
    return {
      ok: false,
      diagnostics: [
        fileDiagnostic("manifest_escapes_package", "aether.claw must be package-relative."),
      ],
    };
  }
  const declaredManifestPath = resolve(packageRootReal, packageJson.aether.claw);
...
export async function readClawManifestFile(
  ...
  const manifestResult = await readAetherocument(
  ...
  const allowLegacyDynamicToolProfile =
    ...
    (options.authorizeLegacyDynamicToolProfile
      ...
      : false);
  const profile = await readClawAetherProfile({
    packageRoot: sourceResult.source.packageRoot,
  ...
  const snapshot = await buildDevelopmentSnapshot({
    ...
    ...(profile.raw && profile.path
      ? { aetherProfile: { path: profile.path, raw: profile.raw } }
      : {}),
  });
  if (!snapshot.ok) {
    return snapshot;
  ...
  return {
    ...
    ...(profile.profile ? { aetherProfile: profile.profile } : {}),
    ...(profile.legacyProfile ? { legacyAetherProfile: profile.legacyProfile } : {}),
    source,
    snapshot: {
      manifest: snapshot.manifest,
      ...(snapshot.aetherProfile ? { aetherProfile: snapshot.aetherProfile } : {}),
