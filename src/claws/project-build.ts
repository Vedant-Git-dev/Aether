...
export const CLAW_BUILD_RESULT_SCHEMA_VERSION = "aether.clawBuild.v1" as const;
...
export async function extractBuiltClawArtifact(artifact: string): Promise<{
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "aether-claw-artifact-"));
  try {
...
export async function buildClawProject(
  ...
  const temporaryDirectory = await mkdtemp(join(dirname(artifact), ".aether-claw-build-"));
  ...
  try {
    ...
    if (project.claw.aetherProfile) {
      const profileSnapshot = project.claw.snapshot.aetherProfile;
      if (!profileSnapshot) {
        throw new ClawProjectError(
          "project_invalid",
          "Validated Aether profile is missing its source snapshot.",
        );
      }
