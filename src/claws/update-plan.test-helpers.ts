...
import type { ClawAetherProfile, ClawSourceIdentity, ResolvedClawPackage } from "./types.js";
...
export async function createUpdatePlanFixture(
  root: string,
  fixtureOptions: { config?: AetherConfig; aetherProfile?: ClawAetherProfile } = {},
) {
  ...
  const env = { AETHER_STATE_DIR: join(root, "state") };
  const addPlan = await buildClawAddPlan({
    manifest: parsed.manifest,
    aetherProfile: fixtureOptions.aetherProfile,
    source,
  ...
  let config: AetherConfig = fixtureOptions.config ?? {};
