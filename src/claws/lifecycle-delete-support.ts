...
import { unregisterAetherAgentDatabases } from "../state/aether-agent-db-registry.js";
import type { AetherStateDatabase } from "../state/aether-state-db-contract.js";
...
export function synthesizeOrphanInstall(params: {
  ...
  return {
    schemaVersion: "aether.clawInstallRecord.v1" as PersistedClawInstall["schemaVersion"],
    claw: {
      kind: "development",
...
export async function readClawRemoveCronInventory(
  agentId: string,
  options: AetherStateDatabaseOptions & { monitorGateway?: ClawMonitorCleanupGateway },
) {
...
export async function cleanupClawAgentFilesystem(params: {
  ...
  stateDatabase?: AetherStateDatabaseOptions;
  assertCurrent: () => void;
}): Promise<string[]> {
  const errors: string[] = [];
...
export async function removeClawWorkspaceFile(
  ...
  try {
    ...
    const stagedPath = `${record.path}.aether-claw-remove-${randomUUID()}`;
    assertCurrent();
...
export function releaseClawRemoveRows(
  ...
  assertCurrent: (database: AetherStateDatabase) => void,
  completeDeletion: (database: AetherStateDatabase) => void,
  options: AetherStateDatabaseOptions,
): boolean {
  ...
  try {
    runAetherStateWriteTransaction((database) => {
      ...
      if (complete) {
        ...
        unregisterAetherAgentDatabases({ agentId, env: options.env, database });
      }
      const { db } = database;
