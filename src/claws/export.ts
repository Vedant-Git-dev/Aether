...
import { parseClawManifest, parseClawAetherProfile } from "./schema.js";
...
import {
  ...
  type ClawManifest,
  type ClawMcpServer,
  type ClawAetherExtension,
  type ClawAetherProfile,
  type ClawPackagePreflight,
} from "./types.js";

export const CLAW_EXPORT_RESULT_SCHEMA_VERSION = "aether.clawExportResult.v1" as const;
const MAX_EXPORT_FILE_BYTES = 1024 * 1024;

type AgentConfig = NonNullable<NonNullable<AetherConfig["agents"]>["list"]>[number];
...
function portableAetherProfile(
  agent: AgentConfig,
  extensions: ClawAetherExtension[],
): ClawAetherProfile | undefined {
  ...
  let tools: NonNullable<ClawAetherProfile["agent"]["tools"]> = configuredTools;
...
export async function exportClawAgent(
  ...
  const aetherProfile = portableAetherProfile(agent, extensions);
  const aetherProfilePath = "profiles/aether.yml";
  const aetherProfileRaw = aetherProfile
    ? Buffer.from(stringifyYaml(aetherProfile))
    : undefined;
  ...
  if (aetherProfile) {
    const parsedProfile = parseClawAetherProfile(aetherProfile);
    if (!parsedProfile.ok) {
      throw new ClawExportError(
        "export_aether_profile_invalid",
  ...
  try {
    ...
    if (aetherProfileRaw) {
      await output.write(aetherProfilePath, aetherProfileRaw, {
        mkdir: true,
        overwrite: false,
      });
      filesWritten.push(aetherProfilePath);
    }
    const packageJson = {
      name: `aether-claw-${record.install.agentId}`,
      version: derivativePackageVersion(manifest, [
        ...contents,
        ...(clawMarkdownBody ? [{ path: "CLAW.md#body", content: clawMarkdownBody }] : []),
        ...(aetherProfileRaw ? [{ path: aetherProfilePath, content: aetherProfileRaw }] : []),
