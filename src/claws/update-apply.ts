...
export const CLAW_UPDATE_RESULT_SCHEMA_VERSION = "aether.clawUpdateResult.v1" as const;

type ConfigCommit = (transform: (config: AetherConfig) => AetherConfig) => Promise<void>;
...
export async function applyClawUpdatePlan(
  ...
  const fresh = await rebuildPlan({
    ...
    targetAetherProfile: params.targetAetherProfile,
