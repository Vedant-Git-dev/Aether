...
import { coerceErrorMessage, stableStringify } from "@aether/normalization-core";
...
type AetheroctorOptions = AetherStateDatabaseOptions & {
  cfg?: AetherConfig;
  ...
  if (record.install.status !== "complete") {
    findings.push(
      finding({
        ...
        fixHint: "Inspect `aether claws status` before retrying or removing this Claw.",
      }),
    );
  }
  ...
  for (const pkg of record.packages) {
    ...
    findings.push(
      finding({
        ...
        fixHint:
          "Inspect package state with `aether claws status` before updating or removing the Claw.",
      }),
    );
  }
...
function orphanedAgentIds(options: AetherStateDatabaseOptions): string[] {
...
export async function collectClawStateHealthFindings(
  options: AetheroctorOptions = {},
): Promise<readonly HealthFinding[]> {
  if (!isExperimentalClawsEnabled(options.env ?? process.env)) {
    return [];
  }
  let database: AetherStateDatabase | undefined;
  try {
    database = await openExistingAetherStateDatabaseReadOnly(options);
    if (!database) {
      return [];
    }
