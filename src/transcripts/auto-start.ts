...
export function createTranscriptsAutoStartService(
  ...
  getConfig: () => AetherConfig | undefined = () => ctx.config,
): {
  start: () => void;
  stop: () => Promise<void>;
} {
  ...
  const stopCapture = async (capture: OwnedCapture, store: TranscriptsStore) => {
    ...
    try {
      ...
      if (typeof details.summaryExportError === "string") {
        warnings.push(
          `summary saved; export failed intendedSummaryPath=${formatAutoStopDiagnostic(details.intendedSummaryPath)}: ${formatAutoStopDiagnostic(details.summaryExportError)}. Correct the export destination, then run aether transcripts path <session> or aether transcripts show <session>.`,
        );
      }
