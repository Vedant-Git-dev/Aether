...
import { resolveOptionalIntegerOption } from "@aether/normalization-core/number-coercion";
...
export class TranscriptsStore {
  constructor(
    ...
    private readonly databaseOptions: AetherStateDatabaseOptions = {},
  ) {}

  private database() {
    ensureMeetingTranscriptsSchema(this.databaseOptions);
    return openAetherStateDatabase(this.databaseOptions);
  }

  private transaction(
    operationLabel: string,
    operation: (database: AetherStateDatabase) => void,
  ): void {
    runAetherStateWriteTransaction(operation, this.databaseOptions, { operationLabel });
  }
  ...
  private hasSummary(database: AetherStateDatabase, row: MeetingTranscriptSessionRow): boolean {
    return Boolean(
  ...
  private async assertExportDestinationOwned(
    ...
    for (const entry of entries) {
      ...
      if (stat.isSymbolicLink() || !stat.isFile()) {
        throw new Error(
          `legacy transcript artifacts require migration before writing ${sessionDir}; run aether vitals --fix`,
        );
      }
