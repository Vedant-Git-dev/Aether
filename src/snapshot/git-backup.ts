...
import { createAetherSnapshotCopy } from "./aether-snapshot-copy.js";
...
export async function initializeGitBackupRepository(params: {
  ...
  if (
    ...
    throw new Error(
      `Git backup repository must be outside the Aether state directory: ${stateDir}`,
    );
  }
  try {
...
async function assertBackupOwnedScope(scopePath: string): Promise<void> {
  if (!(await isBackupOwnedScope(scopePath))) {
    throw new Error(
      `Refusing to replace non-backup-owned path ${scopePath}; the repository must be dedicated to Aether backups.`,
    );
  }
}
...
async function commitGitBackup(params: {
  ...
  const identityArgs =
    email.code === 0 && email.stdout.trim()
      ? []
      : ["-c", "user.name=Aether", "-c", "user.email=backup@aether.local"];
  await requireGit(
...
export async function createGitBackup(params: {
  ...
  const stagingRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aether-git-backup-"));
  ...
  try {
    for (const database of params.databases) {
      ...
      await createAetherSnapshotCopy({ database, targetPath: copyPath });
      manifests.push(
  ...
  if (changed) {
    ...
    commit = await commitGitBackup({
      repositoryPath,
      message: `aether backup ${now.toISOString()}`,
      scopes: commitScopes,
      env: params.gitEnv,
    });
  }
  ...
  if (params.push) {
    ...
    const nonBackupCommitCount = await requireGit(
      repositoryPath,
      ["rev-list", "HEAD", "--invert-grep", "--grep=^aether backup ", "--count"],
      { env: params.gitEnv },
    );
...
async function materializeGitBackupRef(params: {
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "aether-git-restore-"));
...
export async function verifyGitBackupRef(params: {
  ...
  const scratch = await fs.mkdtemp(path.join(os.tmpdir(), "aether-git-verify-"));
