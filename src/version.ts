...
const CORE_PACKAGE_NAME = "aether";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["AETHER_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["AETHER_VERSION"]];
  return (
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.AETHER_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Aether version.
// - Embedded/bundled builds: bundled-version env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  bundledVersion: process.env.AETHER_BUNDLED_VERSION,
});
