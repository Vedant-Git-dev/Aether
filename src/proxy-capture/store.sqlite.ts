...
function runSharedDebugProxyCaptureWrite<T>(owner: object, operation: () => T): T {
  ...
  return runAetherStateWriteTransaction(() => operation(), {
...
class DebugProxyCaptureStoreImpl {
  ...
  constructor(
    ...
    const database = openAetherStateDatabase({ env: optionsOrDbPath.env });
...
function resolveDebugProxyCaptureStoreKey(
  ...
  return typeof optionsOrDbPath === "string"
    ...
    : `shared:${openAetherStateDatabase({ env: optionsOrDbPath.env }).path}`;
}
