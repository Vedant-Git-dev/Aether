...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[aether proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[aether proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`aether proxy coverage\` for details.`,
  );
}
