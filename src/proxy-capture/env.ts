...
const AETHER_DEBUG_PROXY_ENABLED = "AETHER_DEBUG_PROXY_ENABLED";
const AETHER_DEBUG_PROXY_URL = "AETHER_DEBUG_PROXY_URL";
const AETHER_DEBUG_PROXY_CERT_DIR = "AETHER_DEBUG_PROXY_CERT_DIR";
const AETHER_DEBUG_PROXY_SESSION_ID = "AETHER_DEBUG_PROXY_SESSION_ID";
const AETHER_DEBUG_PROXY_REQUIRE = "AETHER_DEBUG_PROXY_REQUIRE";
...
export function resolveDebugProxySettings(
  ...
  const enabled = isTruthy(env[AETHER_DEBUG_PROXY_ENABLED]);
  const explicitSessionId = env[AETHER_DEBUG_PROXY_SESSION_ID]?.trim() || undefined;
  ...
  return {
    enabled,
    required: isTruthy(env[AETHER_DEBUG_PROXY_REQUIRE]),
    proxyUrl: env[AETHER_DEBUG_PROXY_URL]?.trim() || undefined,
    dbPath: resolveDebugProxyDbPath(env),
    blobDir: resolveDebugProxyBlobDir(env),
    certDir: env[AETHER_DEBUG_PROXY_CERT_DIR]?.trim() || resolveDebugProxyCertDir(env),
    sessionId,
    sourceProcess: "aether",
  };
}
...
export function resolveEnabledDebugProxySettings(
  ...
  if (!(resolved?.enabled ?? isTruthy(process.env[AETHER_DEBUG_PROXY_ENABLED]))) {
    return undefined;
  }
...
export function applyDebugProxyEnv(
  ...
  return {
    ...env,
    [AETHER_DEBUG_PROXY_ENABLED]: "1",
    [AETHER_DEBUG_PROXY_REQUIRE]: "1",
    [AETHER_DEBUG_PROXY_URL]: params.proxyUrl,
    [AETHER_DEBUG_PROXY_CERT_DIR]: params.certDir ?? resolveDebugProxyCertDir(env),
    [AETHER_DEBUG_PROXY_SESSION_ID]: params.sessionId,
