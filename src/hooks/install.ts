...
function resolveAetherHooks(
  ...
  if (!Array.isArray(hooks)) {
    return {
      ok: false,
      error: "package.json missing aether.hooks",
      code: HOOK_INSTALL_ERROR_CODE.MISSING_AETHER_HOOKS,
    };
  }
  ...
  if (list.length === 0) {
    return {
      ok: false,
      error: "package.json aether.hooks is empty",
      code: HOOK_INSTALL_ERROR_CODE.EMPTY_AETHER_HOOKS,
    };
  }
...
async function installHookPackageFromDir(
  ...
  const hookManifest = resolveAetherHooks(manifest);
  if (!hookManifest.ok) {
    return hookManifest;
  }
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `aether.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `aether.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "aether-hook-pack-",
    spec,
    timeoutMs,
