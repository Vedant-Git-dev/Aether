...
import type { AetherConfig, HookConfig } from "../config/config.js";
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "aether-plugin") {
    return { enabled: true };
  }
  ...
  if (entry.hook.source === "aether-workspace" && hookConfig?.enabled !== true) {
...
export function resolveHookEntries<T extends HookPolicyEntry>(
  ...
  for (const { entry } of ordered) {
    ...
    if (
      entry.hook.source === "aether-workspace"
        ? sameSource
        : !sameSource || entry.hook.source === "aether-managed"
    ) {
      merged.set(entry.hook.name, entry);
      continue;
    }
