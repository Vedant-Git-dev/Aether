...
const hookOwner = resolveGlobalSingleton<{ generation: HookGeneration }>(
  Symbol.for("aether.loadedInternalHookRegistrations"),
...
export async function prepareInternalHooks(
  ...
  for (const entry of discovery.entries) {
    ...
    try {
      ...
      if (entry.hook.source === "aether-workspace" || entry.hook.source === "aether-managed") {
        log.warn(
          `Loading ${entry.hook.source.slice("aether-".length)} hook code into the gateway process. Hooks are trusted local code.`,
        );
      }
      ...
      if (unknownEvents.length > 0) {
        log.warn(
          `Hook '${safeLogValue(entry.hook.name)}' subscribes to event${unknownEvents.length === 1 ? "" : "s"} ` +
            `${unknownEvents.map((event) => safeLogValue(event)).join(", ")} not emitted by Aether core — ` +
            `likely a typo; unless a plugin emits it, the hook never fires. ` +
            `Known events: https://docs.aether.ai/automation/hooks`,
        );
      }
