...
function resolveHookDiscoveryRoots(
  ...
  return [
    ...normalizeTrimmedStringList(opts?.config?.hooks?.internal?.load?.extraDirs).map((dir) => ({
      ...
      source: "aether-managed" as const,
      includeRoot: true,
    })),
    ...(bundledHooksDir ? [{ dir: bundledHooksDir, source: "aether-bundled" as const }] : []),
    ...resolvePluginHookDirs({ workspaceDir, config: opts?.config }).map(
      ({ dir, pluginId, rootDir }) => ({
        dir,
        pluginId,
        rootDir,
        source: "aether-plugin" as const,
      }),
    ),
    { dir: opts?.managedHooksDir ?? path.join(CONFIG_DIR, "hooks"), source: "aether-managed" },
    { dir: path.join(workspaceDir, "hooks"), source: "aether-workspace" },
  ];
}
