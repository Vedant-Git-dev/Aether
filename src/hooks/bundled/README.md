...
These internal hooks ship with Aether. They subscribe to colon-separated events
...
troubleshooting, use the canonical [Hooks guide](https://docs.aether.ai/automation/hooks).
For command flags and Gateway targeting, see the
[hooks CLI reference](https://docs.aether.ai/cli/hooks).
...
The default state directory is `~/.aether`. Agent workspaces can differ; see
[Agent workspace](https://docs.aether.ai/concepts/agent-workspace).
...
```bash
aether hooks info command-logger
```

```bash
aether hooks enable command-logger
```
...
`metadata.aether.events` declares subscriptions. Custom hooks can also use
`handler.js`, `index.ts`, or `index.js`; see the
[authoring guide](https://docs.aether.ai/automation/hooks#writing-hooks) for a
...
[typed plugin hooks and services](https://docs.aether.ai/plugins/hooks) for
