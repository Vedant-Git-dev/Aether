...
const HOST_LABEL = "aether.node-worker.host";
const GATEWAY_LABEL = "aether.node-worker.gateway";
const LAUNCH_LABEL = "aether.node-worker.launch";
...
export async function createNodeWorkerContainer(
  ...
  const launchHash = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  const containerName = `aether-node-worker-${namespace.slice(0, 12)}-${launchHash}`;
  const args = [
    "create",
    "--interactive",
    "--name",
  ...
  if (containerEnv.NODE_COMPILE_CACHE !== undefined) {
    containerEnv.NODE_COMPILE_CACHE = "/tmp/aether-node-worker-compile-cache";
  }
