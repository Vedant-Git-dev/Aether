...
export function snapshotNodeWorkerEnv(
  ...
  const hostCacheFenced =
    ...
    source.AETHER_SERVICE_KIND === NODE_SERVICE_KIND &&
    source.AETHER_LAUNCHD_LABEL === resolveNodeLaunchAgentLabel();
  ...
  if (!workerCacheDisabled) {
    ...
    snapshot.NODE_COMPILE_CACHE =
      requestedCache || path.join(resolvePreferredAetherTmpDir(), "node-worker-compile-cache");
  } else {
