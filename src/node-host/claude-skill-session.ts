...
export async function prepareNodeClaudeSkillSession(io: AetherPluginNodeHostCommandIo) {
  ...
  try {
    ...
    if (workshop) {
      directory = await fs.mkdtemp(path.join(os.tmpdir(), "aether-node-claude-skills-"));
      assertCurrent();
      ...
      server = http.createServer((req, res) => {
        void (async () => {
          ...
          const mcp = new Server(
            { name: "aether-node-skills", version: "1" },
            { capabilities: { tools: {} } },
          );
      ...
      argv.push("--mcp-config", configPath, "--allowedTools", "mcp__aether__skill_workshop");
    }
    return {
      argv,
