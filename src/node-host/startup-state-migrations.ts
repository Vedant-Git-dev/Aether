...
import { initializeNativeAetherStateDatabase } from "../state/aether-state-db.js";
...
export async function runStartupMigrations(params?: {
  ...
  const env = { ...(params?.env ?? process.env), AETHER_STATE_DIR: stateDir };
  ...
  initializeNativeAetherStateDatabase({ env });
