...
type TurnDatabase = Pick<AetherStateDatabase, "node_worker_turns">;
...
function ensureTurnSchema(database: DatabaseSync): void {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf("CREATE TABLE IF NOT EXISTS node_worker_turns (");
  const endMarker = "\n  WHERE state = 'running';";
  const end = AETHER_STATE_SCHEMA_SQL.indexOf(endMarker, start);
  if (start < 0 || end < start) {
    throw new Error("Aether node worker turn schema marker is missing.");
  }
  database.exec(AETHER_STATE_SCHEMA_SQL.slice(start, end + endMarker.length)); // sqlite-allow-raw -- Canonical feature-local additive DDL only.
}
