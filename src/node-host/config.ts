...
export type NodeHostGatewayConfig = {
  ...
  /** Gateway WebSocket context path (e.g. "/aether-gw"). */
  contextPath?: string;
...
type NodeHostConfigDatabase = Pick<AetherStateKyselyDatabase, "config_machine_state">;

function databaseOptions(env: NodeJS.ProcessEnv): AetherStateDatabaseOptions {
  return { env };
}
...
function assertNodeHostLegacyStateMigrated(env: NodeJS.ProcessEnv = process.env): void {
  ...
  throw new Error(
    `retired node-host state remains at ${sourcePath}; stop the node host and run \`aether vitals --fix\``,
  );
}
...
export async function configureNodeHost(params: {
  ...
  const config = runAetherStateWriteTransaction(({ db }) => {
