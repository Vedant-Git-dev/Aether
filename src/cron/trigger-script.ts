...
type PreparedTriggerRuntime = {
  ...
  context: HookContext & { config: AetherConfig; agentId: string; sessionKey: string };
...
type TriggerRuntimeCacheEntry = {
  ...
  configEpoch: AetherConfig;
  agentId: string;
  toolsAllowKey: string;
};

function resolveTriggerAgentId(config: AetherConfig, agentId?: string): string {
