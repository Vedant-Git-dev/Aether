...
import { isArtifactPreservingStateRead } from "../state/aether-state-db-readonly.js";
...
function loadCronStoreFromDatabase(
  ...
  if (readOnly) {
    ...
    const removed = runAetherStateWriteTransaction(
      ({ db }) => {
...
function repairLoadedCronRuntimeAuthority(params: {
  ...
  const repaired = runAetherStateWriteTransaction(
    ({ db }) => {
...
export async function loadCronJobsStoreWithConfigJobsReadOnly(
  ...
  const statePath = resolveAetherStateSqlitePath(env);
...
export async function saveCronJobsStoreChanges(
  ...
  const committed = runAetherStateWriteTransaction(
    ({ db }) => {
...
export async function saveCronJobsStoreWithMetadata(
  ...
  const storeKey = cronStoreKey(resolvedStorePath);
  assertCronStoreCanPersist(store);
  const committed = runAetherStateWriteTransaction((database) => {
