...
/** Runs a test callback with an isolated Aether home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "aether-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".aether", "sessions");
...
export function makeCfg(
  ...
  const base: AetherConfig = {
    agents: {
      entries: { main: { default: true } },
      defaults: {
        model: "anthropic/claude-opus-4-6",
        workspace: path.join(home, "aether"),
      },
    },
