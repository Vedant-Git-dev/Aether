...
type CronRunReceiptDatabase = Pick<AetherStateDatabase, "cron_run_receipts">;
...
function ensureCronRunReceiptSchema(database: DatabaseSync): void {
  const start = AETHER_STATE_SCHEMA_SQL.indexOf(CRON_RUN_RECEIPT_SCHEMA_START);
  const endMarker = AETHER_STATE_SCHEMA_SQL.indexOf(CRON_RUN_RECEIPT_SCHEMA_END, start);
  if (start < 0 || endMarker < start) {
    throw new Error("Aether cron run receipt schema marker is missing.");
  }
  // sqlite-allow-raw -- Canonical feature-local additive DDL only.
  database.exec(
    AETHER_STATE_SCHEMA_SQL.slice(start, endMarker + CRON_RUN_RECEIPT_SCHEMA_END.length),
  );
}
