...
const CRON_EXECUTION_ROOT_RUNTIME_ERROR =
  "collection review requires a runtime that enforces the Workshop root through Aether tools";
...
export function assertCronExecutionRootRuntime(
  ...
  if (executionRoot && runtime !== "aether" && !rootedCliExecution) {
...
function hasConfiguredAuthProfiles(cfg: AetherConfig): boolean {
  return (
