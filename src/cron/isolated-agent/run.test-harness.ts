...
function resetRunConfigMocks(): void {
  ...
  resolveEffectiveAgentRuntimeMock.mockReturnValue("aether");
...
export function clearFastTestEnv(): string | undefined {
  const previousFastTestEnv = process.env.AETHER_TEST_FAST;
  delete process.env.AETHER_TEST_FAST;
...
export function restoreFastTestEnv(previousFastTestEnv: string | undefined): void {
  ...
  process.env.AETHER_TEST_FAST = previousFastTestEnv;
}
