...
type AgentDefaults = NonNullable<NonNullable<AetherConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.aether/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  try {
...
export function buildStatusMessageParts(args: StatusArgs): StatusMessageParts {
  ...
  const versionLine = `🔮 Aether ${VERSION}${commit ? ` (${commit})` : ""}`;
