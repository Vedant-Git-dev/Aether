...
async function prepareSessionStatusDetails(cfg: AetherConfig, now: number) {
  const {
    classifySessionKey,
...
export async function getStatusSummary(
  ...
  const tasks: StatusSummary["tasks"] = {
    ...
    ...(taskInspection.state === "migration-required"
      ? {
          warning:
            "Task history is unavailable until Gateway startup or aether vitals --fix repairs the state database.",
        }
      : {}),
  };
