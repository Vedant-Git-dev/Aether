...
type PluginBlobTable = AetherStateKyselyDatabase["plugin_blob_entries"];
type PluginBlobDatabase = Pick<AetherStateKyselyDatabase, "plugin_blob_entries">;
...
function createError(params: {
  ...
  return new PluginBlobStoreError(params.message, {
    ...
    path: resolveAetherStateSqlitePath(params.env ?? process.env),
    cause: params.cause,
  });
}
...
function openDatabase(operation: PluginBlobStoreOperation, env?: NodeJS.ProcessEnv) {
  try {
    const database = openAetherStateDatabase(env ? { env } : {});
    return database;
  } catch (error) {
    throw wrapError(
      error,
...
function readDatabase<T>(
  ...
  try {
    return withExistingAetherStateDatabaseReadOnly(
      ({ db }) => {
        readStarted = true;
        try {
          return read(db);
          ...
          if (
            ...
            !hasAetherStateTablesBeyondStartupCheckpoint(db)
          ) {
            return undefined;
          }
          throw error;
