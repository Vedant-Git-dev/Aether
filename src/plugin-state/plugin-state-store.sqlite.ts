// Plugin state SQLite helpers persist plugin state in the Aether state database.
...
import {
  hasAetherStateTablesBeyondStartupCheckpoint,
  withExistingAetherStateDatabaseReadOnly,
} from "../state/aether-state-db-readonly.js";
...
type PluginStateEntriesTable = AetherStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<AetherStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveAetherStateSqlitePath(process.env),
): PluginStateStoreError {
  ...
  if (fallbackCode === "PLUGIN_STATE_OPEN_FAILED") {
    if (isSqliteSchemaVersionError(error)) {
      publicMessage +=
        "\nThe state database uses a newer schema. Run an Aether build that supports it.";
    } else if (error instanceof Error && isTerminalSqliteIntegrityError(error)) {
      publicMessage +=
        "\nDatabase integrity verification failed. Restore or repair the state database, then run aether vitals --fix.";
    }
  }
...
function openPluginStateDatabase(
  ...
  const pathname = resolveAetherStateSqlitePath(env);
  try {
    return openAetherStateDatabase(options);
  } catch (error) {
...
function withPluginStateDatabaseReadOnly<T>(
  ...
  const pathname = resolveAetherStateSqlitePath(options.env ?? process.env);
  let operationStarted = false;
  try {
    return withExistingAetherStateDatabaseReadOnly(({ db, path }) => {
      ...
      try {
        ...
        if (isMissingPluginStateTableError(error)) {
          ...
          if (!hasAetherStateTablesBeyondStartupCheckpoint(db)) {
            return undefined;
          }
        }
        throw error;
...
function envOptions(env?: NodeJS.ProcessEnv): AetherStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  if (!isAetherStateDatabaseOpen(resolveAetherStateSqlitePath(options.env ?? process.env))) {
    openPluginStateDatabase(operation, options);
  }
  return runAetherStateWriteTransaction(write, options);
}
...
export function pluginStateLookup(params: {
  ...
  const pathname = resolveAetherStateSqlitePath(params.env ?? process.env);
  try {
...
export function countPluginStateLiveEntries(pluginId: string, env?: NodeJS.ProcessEnv): number {
  const pathname = resolveAetherStateSqlitePath(env ?? process.env);
  try {
    return (
      withPluginStateDatabaseReadOnly(
        "entries",
...
function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveAetherStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const stateWasOpen = isAetherStateDatabaseOpen();
  ...
  try {
    ...
    openAetherStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.pluginStateSqliteTestApi")] = {
