...
import { ensureMessageToolRunOutcomeSchema } from "../state/aether-agent-message-tool-outcome-schema.js";
import { executeSqliteQuerySync, getNodeSqliteKysely } from "./kysely-sync.js";

const MESSAGE_TOOL_RUN_OUTCOME_MAX_ROWS = 10_000;

type MessageToolRunOutcomeTable = AetherAgentKyselyDatabase["message_tool_run_outcomes"];
type MessageToolRunOutcomeDatabase = Pick<AetherAgentKyselyDatabase, "message_tool_run_outcomes">;
...
export function recordMessageToolRunOutcome(params: {
  ...
  ensureMessageToolRunOutcomeSchema(openAetherAgentDatabase(databaseOptions).db);
  runAetherAgentWriteTransaction(
    ({ db }) => {
