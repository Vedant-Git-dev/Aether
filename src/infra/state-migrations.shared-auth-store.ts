...
import { safeParseJsonRecord } from "@aether/normalization-core/json-coercion";
...
function readSourceSnapshot(params: { env: NodeJS.ProcessEnv; sourcePath: string }): {
  ...
  try {
    const rows = runAetherAgentWriteTransaction(
    ...
    closeAetherAgentDatabaseByPath(params.sourcePath);
  ...
  return runAetherStateWriteTransaction(
    ({ db: database, path: targetDatabasePath }) => {
      ...
      if (conflicts.length > 0) {
        throw new Error(
          `shared auth rows conflict with the relocation target: ${conflicts.join("; ")}. Back up source ${JSON.stringify(params.sourcePath)} and target ${JSON.stringify(targetDatabasePath)} with Aether stopped. Preserve target-only profiles, copy missing source profiles into the target, and reconcile differing entries/metadata/state locally; then rerun aether vitals --fix. No auth rows were changed.`,
        );
      }
...
function cleanupSourceRows(params: { env: NodeJS.ProcessEnv; sourcePath: string }): boolean {
  ...
  try {
    const removed = runAetherAgentWriteTransaction(
      ({ db: database }) => {
...
export function detectSharedAuthStoreMigration(params: {
  ...
  const env = { ...(params.env ?? process.env), AETHER_STATE_DIR: params.stateDir };
  const sourcePath = path.join(resolveSharedMainAuthAgentDir(env), "aether-agent.sqlite");
