...
function createLocalOverrideMutationPath(relativePath: string, label: string): string {
  ...
  return path.posix.join(
    ...
    `.aether-override-${label}-${randomUUID()}.tmp`,
  );
}
...
export async function applyLocalPackageOverrides(params: {
  ...
  if (!params.reapply) {
    return {
      ...
      applied: 0,
      warnings: [
        "Local Aether changes were preserved in the recovery bundle and were not reapplied. Inspect the bundle and copy back trusted files manually, or run the update with --reapply-local-overrides when you want trusted edits replayed during that update.",
      ],
    };
  }
  ...
  if (changesToApply.length === 0) {
    return {
      ...
      warnings:
        conflicts.length > 0
          ? [
              "Local Aether changes were preserved but not reapplied because the update changed the same file(s).",
            ]
          : [],
    };
  }
  ...
  try {
    ...
    return {
      ...
      warnings: [
        "Local Aether changes were preserved but could not be reapplied.",
