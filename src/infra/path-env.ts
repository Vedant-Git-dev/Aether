// Builds PATH values for Aether child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current Aether process is on.
  try {
  ...
  // Bundled macOS app: `aether` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "aether");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AETHER_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin && cwd) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "aether"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `aether`
...
/**
 * Best-effort PATH bootstrap so skills that require the `aether` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAetherCliOnPath(opts: EnsureAetherPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AETHER_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.AETHER_PATH_BOOTSTRAPPED = "1";
