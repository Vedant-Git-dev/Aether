...
$targetPort = ${port}
function Test-AetherPortMatch($value) {
...
function Resolve-AetherProgramScope($rule) {
...
function Get-AetherManagedRules {
  try {
    ...
    foreach ($rule in $rules) {
      foreach ($portFilter in @($rule | Get-NetFirewallPortFilter)) {
        ...
        if (($protocol -eq 'Any' -or $protocol -eq 'TCP') -and (Test-AetherPortMatch $portFilter.LocalPort)) {
...
$managedMatchingRules = @(Get-AetherManagedRules)
...
foreach ($rule in $policy.Rules) {
  ...
  if (($protocol -ne 'TCP' -and $protocol -ne 'Any') -or -not (Test-AetherPortMatch $rule.LocalPorts)) { continue }
  [void]$matchingRules.Add([pscustomobject]@{
    ...
    Program = (Resolve-AetherProgramScope $rule)
...
export async function inspectWindowsGatewayFirewall(
  ...
  if (quickJson === null) {
    return {
      ...
      message: "Aether could not quickly inspect Windows Firewall LAN Gateway policy.",
      details: [
        "Run `aether gateway status --deep` again, or verify the advertised LAN URL from another device.",
      ],
    };
  }
  ...
  if (!quickPayload || typeof quickPayload !== "object" || Array.isArray(quickPayload)) {
    return {
      ...
      severity: "warning",
      code: "windows_firewall_inspection_failed",
      message: "Aether could not parse Windows Firewall LAN Gateway policy.",
      details: [
        "Run `aether gateway status --deep` again, or verify the advertised LAN URL from another device.",
      ],
    };
...
export function formatWindowsGatewayFirewallGuidance(params: {
  ...
  return [
    "Windows firewall: if another device cannot connect to the LAN URL, run `aether gateway status --deep` from this Windows host.",
  ];
}
