...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.aether";
...
export function resolveExecApprovalsDisplayPath(): string {
  ...
  const locator = path.join("state", "aether.sqlite#exec_approvals_config");
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.AETHER_STATE_DIR?.trim()
    ? "$AETHER_STATE_DIR/state/aether.sqlite#exec_approvals_config"
    : `${DEFAULT_EXEC_APPROVALS_STATE_DIR}/state/aether.sqlite#exec_approvals_config`;
}
