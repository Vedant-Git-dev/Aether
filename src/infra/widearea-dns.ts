...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AETHER_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aether-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "aether";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "aether");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "aether-gw");
  ...
  records.push(`_aether-gw._tcp IN PTR ${instanceLabel}._aether-gw._tcp`);
  records.push(`${instanceLabel}._aether-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aether-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aether-content-hash: ${contentHash}\n${contentBody}`;
}
