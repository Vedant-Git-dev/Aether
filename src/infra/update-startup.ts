...
import { extractErrorCode } from "@aether/normalization-core/error-coercion";
import {
  asDateTimestampMs,
  timestampMsToIsoString,
} from "@aether/normalization-core/number-coercion";
import { sleepWithAbort } from "@aether/retry";
import type {
  UpdateAvailable,
  UpdateScheduleState,
...
import { resolveAetherPackageRoot } from "./aether-root.js";
import {
...
type UpdateCheckLifecycle = {
  signal: AbortSignal;
  refreshes: WeakMap<AetherConfig, Promise<void>>;
...
export function refreshGatewayUpdateStatus(cfg: AetherConfig): Promise<void> {
...
async function runGatewayUpdateCheckOwned(
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.AETHER_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldNotify) {
      const updateNotice = `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("aether update")}`;
