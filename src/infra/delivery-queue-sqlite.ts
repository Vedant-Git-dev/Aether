...
function openStateDatabase(stateDir?: string) {
  return openAetherStateDatabase({
    env: stateDir ? { ...process.env, AETHER_STATE_DIR: stateDir } : process.env,
  });
}
...
export function reserveDeliveryQueueEntryAttempt(params: {
  ...
  return runAetherStateWriteTransaction(
    ...
    {
      env: params.stateDir ? { ...process.env, AETHER_STATE_DIR: params.stateDir } : process.env,
    },
    {
...
export function pruneExpiredDeliveryQueueTombstones(stateDir?: string): void {
  ...
  runSqliteImmediateTransactionSync(
    ...
    { databaseLabel: "aether-state", operationLabel: "expire delivery queue tombstones" },
  );
}
