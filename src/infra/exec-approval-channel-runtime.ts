...
type ApprovalReplayMethod = Extract<
  ...
  "exec.approval.list" | "plugin.approval.list" | "aether.approval.list"
>;
...
function resolveApprovalReplayMethods(
  ...
  if (eventKinds.has("system-agent")) {
    methods.push("aether.approval.list");
  }
  return methods;
}
...
>(
  ...
  const handleGatewayEvent = (evt: EventFrame): void => {
    ...
    if (evt.event === "aether.approval.requested" && eventKinds.has("system-agent")) {
      spawn(
        "error handling approval request",
    ...
    if (evt.event === "aether.approval.resolved" && eventKinds.has("system-agent")) {
