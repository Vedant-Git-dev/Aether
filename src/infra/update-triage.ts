...
const TRIAGE_OUTPUT_HINT =
  "See the Gateway host command output for saved diagnostics and the installation-specific aether triage command.";
...
async function runPreparedUpdateFailureTriage(
  ...
  delete env.AETHER_UPDATE_IN_PROGRESS;
  ...
  try {
    ...
    const command = formatInstallationTargetCommand(
      ["aether", "triage", ...(contextPath ? ["--update-result", contextPath] : [])],
      installationTarget,
      { env: targetEnv },
    );
