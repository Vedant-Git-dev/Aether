...
import { filterStringRecord, isRecord } from "@aether/normalization-core/record-coerce";
import { normalizeOptionalString as readOptionalString } from "@aether/normalization-core/string-coerce";
...
type ManagedNpmRootAetherMetadata = {
...
type ManagedNpmRootAetherHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedAetherMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootAetherMetadata | undefined {
  const metadata: ManagedNpmRootAetherMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.aether)) {
...
/** Read host Aether pnpm overrides for reuse inside a managed npm root. */
export async function readAetherManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.aether));
  ...
  const aetherMetadata = buildManagedAetherMetadata({
    current: manifest.aether,
    managedOverrideKeys,
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "aether" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.aether;
  ...
  if (isRecord(value.peerDependenciesMeta) && "aether" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.aether;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])aether(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.aether);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aether-managed-peer-plan-"));
  try {
    delete dependencies.aether;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_aether-pack-archives"),
      path.join(tempRoot, "_aether-pack-archives"),
    );
...
/** Remove stale managed-root aether peer installs while preserving active host links. */
export async function repairManagedNpmRootAetherPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  signal?: AbortSignal;
  ...
  const activeHostState = await readManagedNpmRootAetherHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "aether" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasAetherPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "aether"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootAetherPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall aether" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall aether" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootAetherPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootAetherHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootAetherHostState> {
  const packageRoot =
  ...
  const managedAetherPackageDir = path.join(params.npmRoot, "node_modules", "aether");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedAetherPackageDir),
    lstatIfExists(managedAetherPackageDir),
  ]);
...
async function managedNpmRootLockfileHasAetherPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "aether" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/aether" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "aether" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootAetherPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("aether" in dependencies) {
    const { aether: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("aether" in dependenciesValue) {
          delete dependenciesValue.aether;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/aether" in parsed.packages) {
        delete parsed.packages["node_modules/aether"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "aether" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.aether;
  ...
  const aetherPackageDir = path.join(params.npmRoot, "node_modules", "aether");
  if (!params.preservePackageDir && (await pathExists(aetherPackageDir))) {
    await fs.rm(aetherPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["aether", "aether.cmd", "aether.ps1"].map((binName) =>
