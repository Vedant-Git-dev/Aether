// Validates the current runtime before Aether startup.
import process from "node:process";
import { format } from "node:util";
import { expectDefined } from "@aether/normalization-core/expect";
import {
...
/** Returns whether a detected runtime meets Aether's minimum runtime contract. */
...
/** Returns whether the current process runtime satisfies Aether's engine contract. */
export function isCurrentRuntimeSupported(): boolean {
  return runtimeSatisfies(detectRuntime());
}

/** Checks a Node version label against Aether's supported Node version range. */
export function isSupportedNodeVersion(version: string | null): boolean {
  return isSupportedAetherNodeVersion(version);
}

/** Checks a Bun version label against Aether's minimum supported release. */
...
export async function assertSupportedRuntime(
  ...
  if (
    details.kind === "node" &&
    canRunAetherNodeDiagnostics(details.version, details.hasNodeSqlite) &&
    argv &&
  ...
  const requirement =
    details.kind === "bun"
      ? "aether requires Bun 1.4 or newer with WAL-reset-safe node:sqlite (SQLite 3.51.3+ or a patched 3.50.x/3.44.x release)."
      : (details.sqliteProbe && nodeRuntimeFailure(details.version, details.sqliteProbe)) ||
        "aether requires Node >=24.16.0 <25, or >=26.1.0.";
  const retryHint =
    details.kind === "bun"
      ? "Upgrade Bun or run Aether with a supported Node release."
      : "Upgrade Node and re-run aether.";
