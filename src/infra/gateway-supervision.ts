...
const GATEWAY_SUPERVISOR_MODE_ENV = "AETHER_SUPERVISOR_MODE";
...
export function formatExternalSupervisorActionRequired(action: string): string {
  return [
    `Aether gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
...
export function formatExternalSupervisorUpdateRequired(): string {
  return [
    `Aether self-update is disabled while gateway lifecycle is managed by an external supervisor (${GATEWAY_SUPERVISOR_MODE_ENV}=external).`,
...
export function assertGatewayServiceMutationAllowed(
  ...
  if (serviceIdentityConflict) {
    ...
    throw new Error(
      `service management skipped: named profiles cannot override ${serviceIdentityConflict.envKey} for ${platformName} service management. Unset ${serviceIdentityConflict.envKey} so Aether derives the native service identity from AETHER_PROFILE to ${action}, or keep this profile runtime-only without a native service.`,
    );
  }
  if (!isDefaultInstallIdentity(env)) {
    throw new Error(
      `${NON_DEFAULT_INSTALL_SERVICE_SKIP_REASON}. Rerun with HOME set to the OS account home, without AETHER_HOME, and with AETHER_STATE_DIR and AETHER_CONFIG_PATH either unset or pointing at the canonical paths for that account home and profile to ${action}.`,
    );
  }
}
