...
import { uniqueValues } from "@aether/normalization-core/string-normalization";
 ...
 * returning control to the caller (typically `triggerAetherRestart` →
...
function parsePidsFromLsofOutput(
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("aether")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an aether
...
function findGatewayPidsOnPortWithProtectedPidSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only aether gateway processes.
 ...
 * Returns only PIDs that belong to aether gateway processes (not the current process).
 */
...
function pollPortOnce(port: number): PollResult {
  ...
  try {
    ...
    // its PID field is present, valid, or attributable to an Aether process.
    return { free: false };
  } catch {
    return { free: null, permanent: false };
  }
