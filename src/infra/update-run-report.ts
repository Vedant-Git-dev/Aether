import { sliceUtf16Safe } from "@aether/normalization-core/utf16-slice";
...
export function renderUpdateRunNotice(
  ...
  if (kind === "ack") {
    return `⬆️ Updating Aether ${from ?? "the current version"} → ${to ?? "the latest release"}. The gateway stays available while the update is validated; you'll get a message here when it finishes.`;
  }
...
function recoveryHints(run: ReportInput, nextAction?: string): string[] {
  if (run.status === "running") {
    return ["Check progress with aether update status."];
  }
  ...
  if (!nextAction) {
    hints.push("Run aether triage to diagnose and repair the failed update.");
  }
  return hints;
}
...
export function renderUpdateRunReport(
  ...
  switch (run.status) {
    case "succeeded":
      headline = after
        ? `✅ Aether updated to ${after}${before ? ` (from ${before})` : ""}.`
        : "✅ Aether updated.";
      break;
    case "failed":
      headline =
        run.reason === LEGACY_UPDATE_RUN_EXPIRED_REASON
          ? `ℹ️ Aether update abandoned: ${reason}.`
          : `⚠️ Aether update failed: ${reason}.${running ? ` The gateway is running ${running}.` : ""}`;
      break;
    case "skipped":
      headline = `ℹ️ Aether update skipped: ${reason}.`;
      break;
    case "rolled-back":
      headline = `↩️ Aether update rolled back to ${after ?? running ?? before ?? "the previous version"}: ${reason}.`;
      break;
    case "running":
      headline = `⬆️ Aether update in progress: ${run.phase}.`;
      break;
  }
  ...
  const repairHint =
    run.status === "failed" && repairStopReason === "requester-revoked"
      ? nextAction
        ...
        : "Repair stopped because the chat requester is no longer a command owner. A current command owner must start a new update, or the operator can run aether triage locally."
