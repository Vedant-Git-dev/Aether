...
import { asSafeIntegerInRange } from "@aether/normalization-core/number-coercion";
...
export type DeviceIdentityStoreOptions = AetherStateDatabaseOptions & {
  identityKey?: string;
};

type DeviceIdentityDatabase = Pick<AetherStateKyselyDatabase, "device_identities">;
...
function invalidStoredIdentityError(
  ...
  return new DeviceIdentityStorageError(
    `SQLite contains an invalid persisted device identity "${identityKey}". Run "aether vitals --fix" before starting the gateway or connecting this client.`,
