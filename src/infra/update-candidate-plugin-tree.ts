...
const isHostLauncher = (file: string) =>
  ...
  ["aether", "aether.cmd", "aether.ps1"].includes(path.basename(file));
...
export async function copyUpdateCandidatePluginTrees(params: {
  ...
  const isOwnedHostEdge = (file: string) =>
    path.basename(file) === "aether" && moduleOwners.has(path.dirname(file));
  ...
  async function refreshHostEdges(): Promise<void> {
    ...
    for (const owner of moduleOwners) {
      const source = path.join(owner, "aether");
  ...
  for (;;) {
    ...
    for (const [file, { real }] of edges) {
      ...
      if (excludesInferredRoot(owner)) {
        throw new Error(
          `Cannot privately copy host-owned plugin link ${file} -> ${real}; use the aether package/SDK import or a separately owned plugin dependency.`,
        );
      }
      assertSource(owner);
      addRoot(owner);
