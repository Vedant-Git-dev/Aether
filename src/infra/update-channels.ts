// Resolves Aether update channels from config, tags, and versions.
import { normalizeOptionalLowercaseString } from "@aether/normalization-core/string-coerce";
import { parse as parseSemver } from "semver";
import { compareAetherReleaseVersions } from "./npm-registry-spec.js";
...
/**
 * Env var carrying the *effective* update channel into `aether update finalize`
 ...
 * channel split (`AETHER_UPDATE_POST_CORE_CHANNEL` vs `…_REQUESTED_CHANNEL`).
 */
export const UPDATE_EFFECTIVE_CHANNEL_ENV = "AETHER_UPDATE_EFFECTIVE_CHANNEL";
...
/** Maps an Aether update channel to the npm dist-tag used for package lookups. */
...
export function selectNpmChannelVersion<T extends { version: string | null }>(
  ...
  const comparison =
    compareAetherReleaseVersions(beta.version, latest.version) ??
    compareValidSemver(
