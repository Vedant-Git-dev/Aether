...
// Plugins resolve `aether/plugin-sdk/runtime` through their own staged
...
const HANDLERS_GLOBAL_KEY = Symbol.for("aether.unhandledRejection.handlers");
const EXCEPTION_HANDLERS_GLOBAL_KEY = Symbol.for("aether.uncaughtException.handlers");
...
function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[aether] Unhandled rejection handler failed:",
...
export function isUncaughtExceptionHandled(error: unknown): boolean {
  for (const handler of exceptionHandlers) {
    try {
      ...
      console.error(
        "[aether] Uncaught exception handler failed:",
        err instanceof Error ? (err.stack ?? err.message) : err,
      );
    }
  }
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[aether] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[aether] FATAL unhandled rejection:", formatUncaughtError(reason));
      exitWithTerminalRestore("fatal unhandled rejection", reason, "fatal_unhandled_rejection");
      return;
    }

    if (isConfigError(reason)) {
      console.error("[aether] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
      const exitCode =
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[aether] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[aether] Unhandled promise rejection:", formatUncaughtError(reason));
