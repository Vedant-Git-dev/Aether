import { isLoopbackIpAddress } from "@aether/net-policy/ip";
import { isHttpUrl, isWebSocketUrl } from "@aether/net-policy/url-protocol";
...
const PROXY_ACTIVE_KEYS = [
  "AETHER_PROXY_ACTIVE",
  "AETHER_PROXY_LOOPBACK_MODE",
  "AETHER_PROXY_CA_FILE",
] as const;
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    AETHER_PROXY_ACTIVE: process.env["AETHER_PROXY_ACTIVE"],
    AETHER_PROXY_LOOPBACK_MODE: process.env["AETHER_PROXY_LOOPBACK_MODE"],
    AETHER_PROXY_CA_FILE: process.env["AETHER_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["AETHER_PROXY_ACTIVE"] = "1";
  process.env["AETHER_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["AETHER_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["AETHER_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["AETHER_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or AETHER_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["AETHER_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["AETHER_PROXY_CA_FILE"],
  });
...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  if (
    ...
    (!config?.proxyUrl?.trim() && !process.env["AETHER_PROXY_URL"]?.trim())
  ) {
    return null;
  }
 ...
 * loopback CDP probe to a Chromium instance Aether spawned itself.
 *
