// Proxy validation resolves operator config and probes allowed and denied
// destinations through an explicit HTTP(S) forward proxy.
import { randomUUID } from "node:crypto";
import { createServer, type Server } from "node:http";
import { isHttpUrl } from "@aether/net-policy/url-protocol";
...
const DENIED_CANARY_HEADER = "x-aether-proxy-validation-canary";
...
type ProxyValidationCheckKind = "allowed" | "denied";
...
type ResolveProxyValidationConfigOptions = {
  config?: ProxyConfig;
  env?: NodeJS.ProcessEnv | Partial<Record<"AETHER_PROXY_URL", string | undefined>>;
...
function validateProxyUrl(value: string | undefined): string[] {
  if (!value) {
    return ["proxy validation requires proxy.proxyUrl, --proxy-url, or AETHER_PROXY_URL"];
  }
  if (!isHttpUrl(value)) {
...
function resolveProxyValidationConfig(
  ...
  const envUrl = normalizeProxyUrl(options.env?.AETHER_PROXY_URL);
  if (envUrl) {
    const proxyCaFile = resolveManagedProxyCaFileForUrl({
      proxyUrl: envUrl,
      config: options.config,
  ...
  return {
    enabled: false,
    source: "disabled",
    errors: ["proxy validation requires proxy.proxyUrl, AETHER_PROXY_URL, or --proxy-url"],
  };
}
...
/** Runs allowed and denied proxy validation probes. */
