...
export type NpmSpecResolution = {
  ...
  shasum?: string;
  resolvedAt?: string;
  packageAether?: Record<string, unknown>;
};
...
function normalizeNpmViewMetadata(value: unknown, spec: string): NpmSpecResolution | null {
  ...
  return {
    ...
    ...(isRecord(rec.aether) ? { packageAether: rec.aether } : {}),
  };
}
...
> {
  ...
  if (res.code !== 0) {
    ...
    if (/E404|is not in this registry/i.test(raw)) {
      return {
        ok: false,
        error: `Package not found on npm: ${params.spec}. See https://docs.aether.ai/tools/plugin for installable plugins.`,
      };
    }
...
export async function withInstallWorkspace<T>(
  ...
  const rootDir = options?.rootDir ?? resolvePreferredAetherTmpDir();
