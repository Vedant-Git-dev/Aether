...
const schemaStart = AETHER_STATE_SCHEMA_SQL.indexOf(
...
type GatewayRestartIntentDatabase = Pick<AetherStateKyselyDatabase, "gateway_restart_intent">;
...
export function writeGatewayRestartIntentSync(opts: {
  ...
  try {
    if (!existsSync(resolveAetherStateSqlitePath(env))) {
...
function readGatewayRestartIntentPayloadSync(
  ...
  try {
    const { db } = openAetherStateDatabase({ env });
    const stateDb = getNodeSqliteKysely<GatewayRestartIntentDatabase>(db);
