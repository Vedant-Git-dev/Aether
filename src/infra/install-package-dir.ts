...
import { isRecord as isObjectRecord } from "@aether/normalization-core/record-coerce";
...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".aether-install-hidden-npmrc-";
...
const PACKAGE_DIR_INSTALL_TRANSACTION = Symbol.for("aether.packageDirInstallTransaction");
const PACKAGE_DIR_INSTALL_TRANSACTION_REQUEST = Symbol.for(
  "aether.packageDirInstallTransactionRequest",
);
...
>(params: {
    ...
    if (installedIdentity) {
      ...
      if (!quarantine) {
        const directory = await fs.mkdtemp(
          path.join(installBaseRealPath, ".aether-install-rollback-"),
        );
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".aether-install-stage-"));
  ...
  if (params.mode === "update" && (await pathExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".aether-install-backups");
