...
function resolveHeartbeatPromptRaw(cfg: AetherConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveConfiguredHeartbeatPrompt(cfg: AetherConfig, heartbeat?: HeartbeatConfig) {
...
export function resolveHeartbeatTypingIntervalSeconds(cfg: AetherConfig) {
