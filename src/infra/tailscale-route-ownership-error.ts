...
import { asNullableObjectRecord as readRecord } from "@aether/normalization-core/record-coerce";
...
export class TailscaleRouteOwnershipConflictError extends Error {
  ...
  constructor(port = 443, serveStatus = "{}") {
    ...
    const route = host
      ...
      : `HTTPS port ${port}`;
    super(
      `Tailscale HTTPS port ${port} is already owned by a route whose ownership Aether cannot prove; it was not modified. ` +
        (session
          ...
          : "Inspect `tailscale serve status`. If the route belongs to the current Tailscale hostname and is stale from an older Aether release, remove its background root handler with " +
