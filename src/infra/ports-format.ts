...
/** Classifies a listener as Aether Gateway, SSH tunnel, known non-gateway, or unknown. */
export function classifyPortListener(listener: PortListener, _port: number): PortListenerKind {
  ...
  // where a socat forward may name Aether. Observed macOS output also uses `socat1`.
  if (command === "socat" || command === "socat1" || command === "socat.exe") {
    return "non_gateway";
  }
  const raw = `${commandLine} ${command}`;
  if (raw.includes("aether")) {
    return "gateway";
  }
...
export function buildPortHints(listeners: PortListener[], port: number): string[] {
  ...
  if (kinds.has("gateway") && !expectedGatewayListeners) {
    hints.push(
      `Gateway already running locally. Stop it (${formatCliCommand("aether gateway stop")}) or use a different port.`,
    );
  }
  if (kinds.has("ssh")) {
    hints.push(
