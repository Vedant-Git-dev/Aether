...
/** Supported package managers for Aether global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "aether";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
const AETHER_MAIN_PACKAGE_SPEC = "github:aether/aether#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_AETHER_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
const BUN_AETHER_TRUST_FLAG = "--trust";
...
/**
 * Extracts a pinned installed version from package specs like `aether@1.2.3`.
...
export async function collectInstalledGlobalPackageErrors(params: {
  ...
  if (params.expectedGitCheckout) {
    ...
    if (installedRoot !== params.expectedGitCheckout.root) {
      ...
      if (!(await pathExists(path.join(installedRoot, "aether.mjs")))) {
        errors.push(`missing ${path.join(installedRoot, "aether.mjs")}`);
      }
    }
  } else {
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "aether.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "Aether", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.AETHER_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.AETHER_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return AETHER_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where Aether creates the prefix itself.
 */
...
function resolveBunGlobalInstallSpec(spec: string): string {
  ...
  // tarball is added beside it and can form an aether dependency loop.
...
/**
 * Detects an installed global Aether package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global Aether install.
 * npm receives quiet/freshness-bypass flags; pnpm and Bun approve Aether's lifecycle.
 */
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [resolved.command, "add", "-g", PNPM_AETHER_BUILD_ALLOWLIST_FLAG, spec];
  }
  if (resolved.manager === "bun") {
    return [
      resolved.command,
