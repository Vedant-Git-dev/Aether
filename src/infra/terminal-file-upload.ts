...
const TERMINAL_UPLOAD_PREFIX = "aether-terminal-upload-";
...
const uploadLocks = createFileLockManager("aether.terminal-upload");
...
function resolveTerminalUploadRoot(options?: TerminalUploadRootOptions): string {
  return (options?.platform ?? process.platform) === "win32"
    ? path.join(options?.homeDir ?? homedir(), ".aether", "tmp")
...
async function withUploadLock<T>(
  ...
  const privateRoot = resolvePreferredAetherTmpDir({
    preferredDir: path.join(root, `.aether-terminal-staging-${uid}`),
    tmpdir: () => root,
  });
      ...
      if (hasErrnoCode(error, "file_lock_timeout") || hasErrnoCode(error, "file_lock_stale")) {
        ...
        const recoveryLocation =
          process.platform === "win32"
            ? `${path.join(".aether", "tmp", relativeLockDirectory)} under the home directory of the account running this terminal's Gateway or node host`
