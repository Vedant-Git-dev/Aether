...
type LegacyPluginStateImportDatabase = Pick<AetherStateKyselyDatabase, "plugin_state_entries">;
...
async function withPluginStateImportEnv(stateDir: string | undefined, run: () => Promise<void>) {
  ...
  const previous = process.env.AETHER_STATE_DIR;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.AETHER_STATE_DIR = previous;
    }
  }
}
