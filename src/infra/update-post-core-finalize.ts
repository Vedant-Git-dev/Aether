...
// binary's hidden `aether update finalize` entrypoint — the designed
...
> {
  ...
  return {
    ...
    authoredConfig: isRecord(snapshot.parsed)
      ? (snapshot.parsed as AetherConfig) // SAFETY: the valid snapshot supplies a parsed config object.
...
function buildFinalizeEnv(
  ...
  delete env.AETHER_SERVICE_MARKER;
  delete env.AETHER_SERVICE_KIND;
  delete env[GATEWAY_SERVICE_RUNTIME_PID_ENV];
  env[UPDATE_EFFECTIVE_CHANNEL_ENV] = effectiveChannel;
  if (serviceRepairPolicy) {
    env.AETHER_SERVICE_REPAIR_POLICY = serviceRepairPolicy;
  }
  return env;
}
...
// re-enters the full `aether update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "aether update finalize",
