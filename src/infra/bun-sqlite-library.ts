...
function createSelector(deps: SelectionDependencies) {
  ...
  return (options: SelectionOptions = {}): SqliteLibrarySelection => {
    ...
    const override =
      options.explicitPath ?? (deps.env.AETHER_SQLITE_LIBRARY?.trim() || undefined);
    if (!deps.isBun || deps.platform !== "darwin") {
      selection = override
        ? { source: "runtime", ignoredOverride: "AETHER_SQLITE_LIBRARY requires Bun on macOS" }
        : { source: "runtime" };
      return selection;
    }
...
function selectionError(path: string, error: unknown): Error {
  return new Error(
    `Cannot use SQLite library ${path}: ${error instanceof Error ? error.message : String(error)}. ` +
      "Fix or unset AETHER_SQLITE_LIBRARY; install a supported library with brew install sqlite.",
    { cause: error },
  );
}
...
export function ensureSqliteLibrarySelected(
  ...
  const select = dependencies
    ...
    : resolveGlobalSingleton(Symbol.for("aether.bunSqliteLibrarySelection"), () =>
        createSelector({
