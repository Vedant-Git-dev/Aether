...
import { inspectAetherRegisteredAgentDatabases } from "../state/aether-agent-db-registry.js";
import {
  detectAetherStateDatabaseSchemaMigrations,
  repairAetherStateDatabaseSchema,
  repairAetherStateDatabaseSchemaIfNeeded,
...
function describeStateSchemaMigration(migration: AetherStateDatabaseSchemaMigration): string {
  switch (migration.kind) {
    ...
    case "operator-approvals-system-agent":
      return "operator approvals → Aether system changes";
...
function tryResolveDoctorStateMigrationAgentId(cfg: AetherConfig): string | undefined {
  const agentId = tryResolveAmbientOwnerAgentId(cfg);
  return agentId && listAgentIds(cfg).includes(agentId) ? agentId : undefined;
}

function tryResolveDoctorSessionMigrationAgentId(cfg: AetherConfig): string | undefined {
  return (
...
function hasCustomAgentDirOverride(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.AETHER_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim());
}
...
async function detectManagedWorktreeStateMigration(params: {
  ...
  const stateEnv = { ...params.env, AETHER_STATE_DIR: params.stateDir };
  const databaseExists = migrationFileExists(resolveAetherStateSqlitePath(stateEnv));
  const legacyIds =
...
export async function detectLegacyStateMigrations(params: {
  ...
  const stateSchemaMigrations = detectAetherStateDatabaseSchemaMigrations(
    { env: { ...env, AETHER_STATE_DIR: stateDir } },
    { artifactPreservingReadOnly: params.artifactPreservingReadOnly },
  );
...
function createStateSchemaMigrationStep(params: {
  ...
  const database: LegacyStateMigrationEndpoint = {
    kind: "sqlite",
    path: resolveAetherStateSqlitePath(stateEnv),
  };
  return {
    id: "state-schema",
    phase: "shared",
    ...
    run: () =>
      params.mode === "doctor"
        ? repairAetherStateDatabaseSchema({ env: stateEnv })
        : repairAetherStateDatabaseSchemaIfNeeded({ env: stateEnv }),
  };
}
...
function createPluginInstallIndexStep(params: {
  ...
  return {
    ...
    target: [
      {
        kind: "sqlite",
        path: resolveAetherStateSqlitePath({
          ...params.env,
          AETHER_STATE_DIR: params.stateDir,
        }),
      },
    ],
...
function createConfigMachineStateStep(params: {
  ...
  return {
    ...
    target: [{ kind: "sqlite", path: resolveAetherStateSqlitePath(stateEnv) }],
    requiredness: "conditional",
    reversibility: "checkpoint-required",
...
type LegacyStateMigrationExecutionPlan = {
  ...
  legacySessionSurfaces: PreparedLegacySessionSurfaces;
  beforeWorkspaceStateMigration?: (config: AetherConfig) => Promise<void>;
};
...
function buildLegacyStateMigrationSteps(
  ...
  const stateDatabase: LegacyStateMigrationEndpoint = {
    kind: "sqlite",
    path: resolveAetherStateSqlitePath({ ...env, AETHER_STATE_DIR: stateDir }),
  };
  ...
  const managedWorktreePrelude: LegacyStateMigrationStep[] = [
    sharedStep("managed-worktrees", () => {
      const stateEnv = { ...env, AETHER_STATE_DIR: stateDir };
  ...
  if (repairSessionFiles) {
    ...
    finalSteps.push({
      ...finalStep("acp-session-metadata", () =>
        migrateLegacyAcpSessionMetadata({
          cfg: params.sessionConfig ?? params.config,
          env: isDoctor ? { ...env, AETHER_STATE_DIR: stateDir } : env,
          now,
          pluginSessionStoreAgentIds,
          legacySessionSurfaces: params.legacySessionSurfaces,
        }),
...
export async function planLegacyStateMigrationsReadOnly(params: {
  ...
  const rawOAuthDir = (params.env ?? process.env).AETHER_OAUTH_DIR?.trim();
  const callerOAuthDir = rawOAuthDir
    ? resolveOAuthDir({ ...callerEnv, AETHER_OAUTH_DIR: rawOAuthDir }, requestedSnapshot.stateDir)
    : undefined;
  ...
  if (callerOAuthDir && !oauthDirOutsideSnapshot) {
    env.AETHER_OAUTH_DIR = callerOAuthDir;
  }
    ...
    params.mode === "doctor"
      ? listMigrationEndpointsOutsideRoot(
          [
            {
              kind: "sqlite",
              path: path.join(resolveSharedMainAuthAgentDir(env), "aether-agent.sqlite"),
            },
          ],
          snapshot.stateDir,
        )
  ...
  try {
    registeredDatabases = inspectAetherRegisteredAgentDatabases({
      env,
...
async function executeLegacyStateMigrations(
  ...
  try {
    if (detectAetherStateDatabaseSchemaMigrations(stateSchemaOptions).length > 0) {
