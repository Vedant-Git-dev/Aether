...
type DiagnosticEventsTable = AetherStateKyselyDatabase["diagnostic_events"];
type AuditRecordDatabase = Pick<AetherStateKyselyDatabase, "diagnostic_events">;
...
export function createSqliteAuditRecordStore<T>(
  options: AetherStateDatabaseOptions & { scope: string; maxEntries: number },
) {
  ...
  return {
    ...
    size(): number {
      return countAuditRecords(openAetherStateDatabase(options).db, scope);
    },
