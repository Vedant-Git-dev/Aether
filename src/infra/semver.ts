...
export function isAetherCorrectionSemver(version: SemVer): boolean {
  return version.prerelease.length === 1 && typeof version.prerelease[0] === "number";
}

function toAetherComparableVersion(version: SemVer): string {
  if (isAetherCorrectionSemver(version)) {
...
/** Compares prereleases, stable releases, then Aether numeric corrections. */
export function compareAetherSemver(left: SemVer, right: SemVer): number {
  return compareBuild(toAetherComparableVersion(left), toAetherComparableVersion(right));
}

/** Converts legacy Aether `1.2.3.beta.N` tags into valid SemVer prereleases. */
