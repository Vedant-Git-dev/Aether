...
export class StateDatabaseCoordinatorContentionError extends SqliteCoordinatorError {
  constructor(readonly family: CoordinatorFamily) {
    super(`another Aether process owns ${family}`);
...
export class StateSchemaMutationConflictError extends SqliteCoordinatorError {
  constructor(databasePath: string, cause: unknown) {
    super(
      `Aether refused shared state schema mutation at ${databasePath} because another Gateway owns that state directory. Stop that Gateway or perform the update through its managed restart path, then retry.`,
      cause,
    );
...
export function resolveStateLifecycleRuntimeDirectory(): string {
  return process.platform === "win32"
    ? path.join(os.homedir(), "AppData", "Local", "Aether", "locks")
    : "/tmp";
}
...
function resolveLifecycleCoordinatorPath(
  ...
  const suffix =
    params.uid === undefined ? "aether-state-locks" : `aether-state-locks-${params.uid}`;
  return path.join(
