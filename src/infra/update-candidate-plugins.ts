...
function bundledPluginRedirects(
  ...
  const sourcePackageRoot = sourceDir && resolveAetherPackageRootSync({ cwd: sourceDir });
  ...
  const bundledEnv = { ...env, AETHER_DISABLE_BUNDLED_SOURCE_OVERLAYS: "1" };
  ...
  for (const directory of [sourceDir, resolveBundledSourceCheckoutExtensionsDir(sourceDir)]) {
    ...
    for (const source of listBundledPluginMetadata({
      ...
      const manifest = { name: source.packageName, aether: source.packageManifest };
...
export async function projectUpdateCandidatePlugins(params: {
  ...
  const shared = path.join(targetStateDir, "state", "aether.sqlite");
