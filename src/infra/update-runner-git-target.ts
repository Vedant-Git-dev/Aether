...
export async function withGitTargetInspectionRoot<T>(
  ...
  const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aether-git-admission-"));
...
type GitTargetSchemaMetadata =
  | { status: "ok"; schemaVersions?: AetherSchemaVersions }
...
export async function readGitTargetSchemaVersions(params: {
  ...
  try {
    const schemaVersions = parsePackageAetherSchemaVersions(JSON.parse(result.stdout) as unknown);
