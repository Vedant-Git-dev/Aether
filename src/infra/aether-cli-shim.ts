...
import {
  resolveCurrentAetherCliInvocation,
  type AetherCliInvocation,
} from "./aether-cli-invocation.js";

const AGENT_CLI_BIN_DIR = path.join("tmp", "agent-cli");
const GATEWAY_AGENT_CLI_STATE_KEY = Symbol.for("aether.gatewayAgentCliShim");
...
function renderPosixShim(invocation: AetherCliInvocation, profile: string | null): string {
...
function renderWindowsShim(invocation: AetherCliInvocation, profile: string | null): string {
  const args = [...invocation.args, ...(profile ? ["--profile", profile] : [])];
...
export async function prepareGatewayAgentCliShim(
  options: {
    ...
    invocation?: AetherCliInvocation;
  ...
  const invocation = options.invocation ?? resolveCurrentAetherCliInvocation([]);
  ...
  const executablePath = path.join(binDir, platform === "win32" ? "aether.cmd" : "aether");
  const content =
    platform === "win32"
