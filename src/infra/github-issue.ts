...
/** Prepares and submits bounded issue content to aether/aether. */
...
const GITHUB_REPOSITORY = "github.com/aether/aether";
const GITHUB_REPOSITORY_ISSUES_API = "repos/aether/aether/issues";
...
const GITHUB_MARKER_RE = /^aether-report:[a-f0-9]{64}$/u;
...
function buildPrefilledUrl(title: string, body: string): string {
  ...
  return `https://github.com/aether/aether/issues/new?${query.toString()}`;
}
...
export function prepareGithubIssue(input: { body: string; title: string }): PreparedGithubIssue {
  ...
  const marker = `aether-report:${createHash("sha256")
    .update(title)
    .update("\0")
    .update(boundedBody)
    .digest("hex")}`;
...
function createdIssueUrl(value: unknown): string | undefined {
  ...
  try {
    ...
    if (
      ...
      !url.search &&
      !url.hash &&
      /^\/aether\/aether\/issues\/\d+$/u.test(url.pathname)
    ) {
      return url.toString();
    }
  } catch {
