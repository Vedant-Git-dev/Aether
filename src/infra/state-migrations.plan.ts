...
export async function captureLegacyStateSnapshotIdentity(
  ...
  return new Promise((resolve) => {
    ...
    const child = execFile(
      process.execPath,
      [...resolveRuntimeWorkerArgv(worker), "--aether-state-snapshot"],
      { encoding: "utf8" },
      (error, stdout, stderr) => {
        if (error) {
...
export function createLegacyStateMigrationCallerEnv(params: {
  ...
  for (const key of ["AETHER_HOME", "AETHER_OAUTH_DIR", "STATE_DIRECTORY"]) {
    delete env[key];
  }
  env.HOME = path.resolve(params.snapshot.homeDir);
  env.USERPROFILE = env.HOME;
  env.AETHER_CONFIG_PATH = path.resolve(params.snapshot.configPath);
  return env;
}
...
export function createLegacyStateMigrationPlanEnv(params: {
  ...
  env.AETHER_STATE_DIR = path.resolve(params.snapshot.stateDir);
  return env;
}
