...
import { parseModelCatalogRef } from "@aether/model-catalog-core/model-catalog-refs";
...
const DEFAULT_TELEMETRY_ENDPOINT = "https://telemetry.aether.ai/api/latest-version";
...
function isAutomatedEnvironment(): boolean {
  if (process.env.AETHER_TELEMETRY_ENDPOINT?.trim()) {
    return false;
  }
  return isTruthyEnvValue(process.env.CI);
}

function isUpdateCheckDisabled(config: AetherConfig): boolean {
  return (
    config.update?.checkOnStart === false ||
    isTruthyEnvValue(process.env.AETHER_NO_AUTO_UPDATE) ||
...
function countRecentSessions(nowMs: number): number {
  try {
    return (
      withExistingAetherStateDatabaseReadOnly(({ db: database }) => {
        const db =
          getNodeSqliteKysely<Pick<AetherStateKyselyDatabase, "session_state_events">>(database);
...
function resolveTelemetryEndpoint(): string {
  return process.env.AETHER_TELEMETRY_ENDPOINT?.trim() || DEFAULT_TELEMETRY_ENDPOINT;
}

export function buildTelemetryUserAgent(surface: TelemetrySurface): string {
  return `aether/${VERSION} (${process.platform}; node/${process.versions.node}; ${process.arch}; ${surface})`;
}
...
export function resolveTelemetryStatus(config: AetherConfig): {
  enabled: boolean;
...
export async function checkTelemetryUpdate(
  ...
  const databasePath = path.resolve(resolveAetherStateSqlitePath());
  ...
  const stateDirectory = process.env.AETHER_STATE_DIR;
  if (
