...
} from "../state/aether-state-db-schema-additive.js";
import { tableExists, tableHasColumn } from "../state/aether-state-db-schema-helpers.js";
import type {
  DB as AetherStateKyselyDatabase,
  DevicePairingPaired,
  DevicePairingPending,
  DeviceBootstrapTokens,
} from "../state/aether-state-db.generated.js";
import {
...
function resolveDevicePairingStateDbOptions(baseDir?: string): AetherStateDatabaseOptions {
  return baseDir ? { env: { ...process.env, AETHER_STATE_DIR: baseDir } } : {};
}
...
function invalidateDevicePairingStoreCache(database: AetherStateDatabase): void {
  if (
...
function runDevicePairingStoreMutation<T>(
  ...
  mutate: (database: AetherStateDatabase) => DevicePairingStoreMutation<T>,
): T {
  const databaseOptions = resolveDevicePairingStateDbOptions(baseDir);
  const database = openAetherStateDatabase(databaseOptions);
  const result = runAetherStateWriteTransaction(mutate, { ...databaseOptions, database });
  if (result.mutated) {
...
export function readDevicePairingStoreStateFromDatabase(db: DatabaseSync): DevicePairingStoreState {
  const kysely = getNodeSqliteKysely<AetherStateKyselyDatabase>(db);
...
export function loadDevicePairingStoreState(baseDir?: string): DevicePairingStoreState {
  const database = openAetherStateDatabase(resolveDevicePairingStateDbOptions(baseDir));
  const { db } = database;
...
export function loadPairedDevicePairingStoreRecord(
  ...
  const { db } = openAetherStateDatabase(resolveDevicePairingStateDbOptions(baseDir));
