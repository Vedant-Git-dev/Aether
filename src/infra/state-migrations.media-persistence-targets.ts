...
import {
  createAetherAgentDatabasePathMatcher,
  isPersistentAetherAgentDatabasePath,
...
export function discoverAgentDatabaseMigrationTargets(params: {
  ...
  const configuredPathMatcher = createAetherAgentDatabasePathMatcher();
  ...
  for (const candidate of candidates) {
    ...
    if (!isPersistentAetherAgentDatabasePath(pathname, params.env)) {
      discard(
        candidate,
...
export function resolveAgentDatabaseMigrationTargets(params: {
  ...
  let registeredAgentDatabases: ReturnType<typeof listAetherRegisteredAgentDatabases> = [];
  let registryReadFailed = false;
  try {
    registeredAgentDatabases = listAetherRegisteredAgentDatabases({
      env: params.env,
  ...
  for (const removed of discovery.registryRemovals) {
    unregisterAetherAgentDatabase({ ...removed, env: params.env });
    if (removed.change) {
