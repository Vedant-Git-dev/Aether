// Resolves the Aether package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { getPluginCache } from "../plugins/plugin-cache.js";
import { aetherRootFs, aetherRootFsSync } from "./aether-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["aether"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await aetherRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(aetherRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another Aether checkout keeps its own package root.
  try {
    const resolved = aetherRootFsSync.realpathSync(normalized);
...
export async function resolveAetherPackageRoot(opts: PackageRootOptions): Promise<string | null> {
...
// Every distinct Aether package root among the runtime hints, in candidate order (symlinked
...
export function resolveAetherPackageRootsSync(opts: PackageRootOptions): string[] {
  const candidates = buildCandidates(opts);
  const cacheKey = createPackageRootCacheKey(candidates);
  const searches = getPluginCache().sdk.packageSearches;
...
export function resolveAetherPackageRootSync(opts: PackageRootOptions): string | null {
  const candidates = buildCandidates(opts);
  const cacheKey = createPackageRootCacheKey(candidates);
  const searches = getPluginCache().sdk.packageSearches;
  const cached = searches.get(cacheKey);
