...
function redactDiagnosticLines(value: string): string {
  ...
  const privatePathLine =
    /\$AETHER_STATE_DIR[\\/]|(?:^|[^\p{L}\p{N}._~-])(?:\/+|\\+|[A-Za-z]:[\\/]|~[\\/])/u;
  return value
    .split(/(\r\n|[\n\r\u2028\u2029])/u)
    .map((line) => {
      if (/^(?:\r\n|[\n\r\u2028\u2029])$/u.test(line)) {
...
export async function prepareUpdateFailureReport(
  ...
  const bodyWithoutMarker = [
    "# Aether update failure report",
    "",
    "This report was explicitly reviewed and confirmed in Aether.",
    "",
  ...
  const reconciliationMarker = `aether-update-report:${createHash("sha256")
