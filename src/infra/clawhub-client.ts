...
function normalizeBaseUrl(baseUrl?: string): string {
  const envValue =
    normalizeOptionalString(process.env.AETHER_CLAWHUB_URL) ||
...
function resolveClawHubConfigPathsIn(configHome: string): string[] {
  return ["clawhub", "aetherhub"].map((directory) =>
...
function resolveClawHubConfigPaths(): string[] {
  const explicit =
    ...
    normalizeOptionalString(process.env.AETHERHUB_CONFIG_PATH); // legacy misspelling from older clawhub CLI builds; keep for back-compat
  if (explicit) {
    return [explicit];
  }
...
export function isClawHubTelemetryDisabled(): boolean {
  const raw =
    ...
    normalizeOptionalString(process.env.AETHERHUB_DISABLE_TELEMETRY);
  if (!raw) {
    return false;
  }
