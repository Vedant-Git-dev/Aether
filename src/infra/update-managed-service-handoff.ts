...
const HANDOFF_READY_MARKER = "AETHER_UPDATE_HANDOFF_READY\n";
...
const { assertAetherStateWriteAllowed, createManagedHandoffLeaseStore, resolveImmutableSqliteFileUri, hasManagedUpdateRecoveryRecord, resolveUpdateRestartNoticeMeta, shouldPublishUpdateRestartNotice } =
...
function assertStateDatabaseWriteAllowed(database) {
  ...
  try {
    ...
    assertAetherStateWriteAllowed({ database: db, databasePath: params.stateDatabasePath });
  } finally {
    if (ownsDatabase) {
      db.close();
    }
...
async function admitTriageScope() {
  ...
  if (
    ...
    throw new Error(
      "automatic triage primary ownership changed before native admission; run aether triage manually",
    );
  }
...
async function enterTriageAfterUpdate(continuation) {
  if (
    ...
    appendLog("automatic triage continuation unavailable; run aether triage manually");
    return;
  }
  const primary = await inspectSystemdService(params.serviceRecovery.unit);
  if (
    ...
    appendLog(
      "automatic triage could not verify the installed service after update restoration; run aether triage manually",
    );
    return;
  }
  const scopeUnit = params.scopeUnit.replace(/^aether-update-/, "aether-triage-");
  const action = {
  ...
  try {
    ...
    appendLog("automatic triage destination admission failed: " + String(error) + "; run aether triage manually");
    return;
  }
  if (!retargeted) {
    appendLog("automatic triage lost its completed update owner; run aether triage manually");
    return;
  }
  ...
  Object.assign(params, {
    ...
    commandLabel: "aether triage (automatic)",
    scopeUnit,
...
async function restoreGatewayService(reason, decision = params.recovery, childStatus, previousGeneration = false) {
  ...
  const restart = () => runOwnedUpdateCommand("recovery", params.recoveryCommandArgv,
    params.recoveryTimeoutMs, params.cwd, previousGeneration
      ? { ...process.env, AETHER_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1" } : process.env);
...
async function runOwnedUpdateCommand(phase, commandArgv, timeoutMs, cwd = params.cwd, env = process.env) {
  ...
  try {
        ...
        : ["-e", ${JSON.stringify(HANDOFF_EXEC_RUNNER_SCRIPT)}, JSON.stringify(commandArgv)],
      {
        ...
        env:
          params.action === "triage"
            ...
            : phase === "update" ? { ...env, AETHER_UPDATE_RUN_HANDOFF: "1" } : env,
        detached: true,
    ...
    try {
      ...
      if (params.action === "triage") {
        admissionDeadline = setTimeout(() => {
          appendLog("installed candidate did not admit triage; run aether triage manually");
          stopTriageScope();
        }, 30000);
    ...
    if (params.action === "triage" && !triageAdmitted) {
      appendLog(
        "installed candidate cannot accept automatic triage; run aether triage manually",
      );
      process.exitCode = 1;
    }
...
export function buildManagedServiceHandoffUnavailableMessage(command: string): string {
  return [
    "Aether updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "schtasks") {
    const taskName =
      env.AETHER_WINDOWS_TASK_NAME?.trim() || resolveGatewayWindowsTaskName(env.AETHER_PROFILE);
...
async function spawnManagedServiceUpdateHandoff(
  ...
  const triageContextPath = path.join(
    ...
    "logs",
    "support",
    `aether-update-failure-${randomUUID()}.json`,
  );
  ...
  if (params.supervisor === "systemd") {
    ...
    scopeUnit = `aether-${params.action ? "triage" : "update"}-${suffix}.scope`;
    spawnArgs.unshift(
      "--user",
      "--scope",
      "--collect",
  ...
  const stateDatabasePath = resolveAetherStateSqlitePath(serviceEnv);
  ...
  const helperParams = {
    ...
    triageContextCommand: formatInstallationTargetCommand(
      ["aether", "triage", "--update-result", triageContextPath],
      installationTarget,
      { env: serviceEnv },
    ),
    ...
    triageHint:
      "Update triage runs after service recovery; see the managed update helper log for the outcome and the installation-specific aether triage command.",
    commandLabel,
...
export async function startManagedServiceUpdateHandoff(
  ...
  if (params.action && params.supervisor !== "systemd") {
    throw new Error(
      "Automatic managed triage requires a Linux user-systemd scope; run aether triage manually.",
    );
  }
  if (
...
export async function isCurrentManagedServiceUpdateHandoffProcess(params: {
  ...
  if (process.env.AETHER_UPDATE_RUN_HANDOFF !== "1" || !params.runId) {
    return false;
  }
