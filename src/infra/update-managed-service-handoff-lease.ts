...
export function resolveManagedUpdateLeaseDatabasePath(): string {
  return path.join(resolvePreferredAetherTmpDir(), "managed-update-handoffs.sqlite");
}
...
export function createManagedHandoffLeaseStore(
  ...
  function bootIdentity() {
    ...
    if (!parsed.success) {
      throw new Error("OS boot identity unavailable; run aether triage manually");
    }
    return parsed.data;
  }
  ...
  function handle(root: string, value: LeaseRow): ManagedHandoffLease {
    ...
    if (!payload || !text.safeParse(value.owner).success) {
      throw new Error(
        "existing managed handoff lease is incompatible; retain diagnostics and run aether triage manually",
      );
    }
    return {
      key: root,
  ...
  function hasUnsettledChildren(lease: ManagedHandoffLease, connection?: HandoffDatabase): boolean {
    ...
    const prefix = `${lease.key}/.aether-update-child-`;
  ...
  function admit(
    ...
    return withDatabase(true, (db) => {
      ...
      return transact(db, () => {
        ...
        const childMarker = root.indexOf("/.aether-update-child-");
        if (childMarker >= 0) {
  ...
  function release(lease: ManagedHandoffLease) {
    if (
      !current(lease) ||
      hasUnsettledChildren(lease) ||
      (lease.key.includes("/.aether-update-child-") &&
        lease.executor.pid !== lease.helper.pid &&
