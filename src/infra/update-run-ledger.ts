...
import { runExistingAetherStateWriteTransaction } from "../state/aether-state-db-existing-write.js";
import {
  withExistingAetherStateDatabaseArtifactPreservingReadOnly,
  withExistingAetherStateDatabaseArtifactPreservingReadOnlyAsync,
} from "../state/aether-state-db-readonly.js";
import { tableExists } from "../state/aether-state-db-schema-helpers.js";
import type { DB } from "../state/aether-state-db.generated.js";
import { runAetherStateWriteTransaction } from "../state/aether-state-db.js";
import { AETHER_STATE_SCHEMA_SQL } from "../state/aether-state-schema.js";
...
const schemaStart = AETHER_STATE_SCHEMA_SQL.indexOf("CREATE TABLE IF NOT EXISTS update_runs (");
const schemaEndMarker = "ON update_runs(status, created_at_ms DESC, run_id);";
const schemaEnd = AETHER_STATE_SCHEMA_SQL.indexOf(schemaEndMarker, schemaStart);
if (schemaStart < 0 || schemaEnd < 0) {
  throw new Error("Update run schema markers are missing");
}
const schema = AETHER_STATE_SCHEMA_SQL.slice(schemaStart, schemaEnd + schemaEndMarker.length);
...
function writeRun<T>(operation: (db: DatabaseSync) => T, options: AetherStateDatabaseOptions): T {
  if (options.database) {
  ...
  if (withExistingAetherStateDatabaseArtifactPreservingReadOnly(() => true, options)) {
    return runExistingAetherStateWriteTransaction(({ db }) => operation(db), options, {
      schemaSql: schema,
  ...
  const result = runAetherStateWriteTransaction(
    ({ db }) => {
...
export async function reconcileAbandonedUpdateRunsAsync(
  ...
  const candidates =
    (await withExistingAetherStateDatabaseArtifactPreservingReadOnlyAsync(
      ({ db }) => readUpdateRunReconciliationCandidates(db, input),
      options,
    )) ?? [];
  return reconcileCandidates(candidates, input, options).reconciled;
...
export async function getUpdateRunWithReconciliationAsync(
  ...
  const candidate = await withExistingAetherStateDatabaseArtifactPreservingReadOnlyAsync(
    ({ db }) => {
...
export function finishInterruptedUpdatePreview(
  ...
  runExistingAetherStateWriteTransaction(
    ({ db }) => {
      if (
...
export function getUpdateRun(
  ...
  return withExistingAetherStateDatabaseArtifactPreservingReadOnly(
