...
import { assertAetherStateDatabaseOwner } from "../state/aether-state-db-maintenance.js";
...
function resolveBackupAgentDatabaseOwner(
  ...
  if (path.basename(resolvedSourcePath) !== "aether-agent.sqlite") {
    return undefined;
  }
...
export async function createBackupSqliteSnapshotPlan(params: {
  ...
  const globalStateSqlitePath = path.resolve(
    resolveAetherStateSqlitePath({
      ...process.env,
      AETHER_STATE_DIR: params.inventory.stateDir,
    }),
  );
  ...
  for (const { archiveSourcePath, canonicalSource } of sources) {
    ...
    const sourcePath = path.join(params.tempDir, `aether-state-db-${snapshots.length}.sqlite`);
    try {
      const capture = () =>
        createVerifiedSqliteSnapshot({
          ...
          validate:
            canonicalSource?.role === "global"
              ? (database, pathname) => assertAetherStateDatabaseOwner(database, { pathname })
          ...
          transform:
            canonicalSource?.role === "global"
              ? (database) => {
                  ...
                  sanitizeAetherGlobalStateSnapshot(database);
