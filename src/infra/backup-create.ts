...
import { resolveDateTimestampMs } from "@aether/normalization-core/number-coercion";
import type {
  BackupAgentRoot,
...
function formatBackupOutputFailure(
  ...
  const retry = "run `aether backup create --output <archive>` again.";
  let detail: string;
...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Aether config file was found to back up."
        : "No local Aether state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(tempRoot, "aether-backup-"));
