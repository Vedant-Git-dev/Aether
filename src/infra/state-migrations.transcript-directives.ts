...
function hasActiveAgentDatabaseLease(agentId: string, env: NodeJS.ProcessEnv): boolean {
  try {
    assertNoAetherAgentDatabaseLeases(agentId, { env });
    return false;
  } catch (error) {
...
async function migrateAgentDatabase(
  ...
  await migrateAetherAgentDatabaseForMaintenance(params, maintenance);
  maintenance.assertOwned();
  const database = openNodeSqliteDatabase(params.pathname);
  try {
    database.exec(`PRAGMA busy_timeout = ${AETHER_SQLITE_BUSY_TIMEOUT_MS};`);
    assertAetherAgentDatabaseForMaintenance(database, params);
