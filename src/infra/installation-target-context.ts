...
const installationTargetContext = resolveGlobalSingleton(
  Symbol.for("aether.installationTargetContext"),
...
export function installationTargetEnv(target: InstallationTarget | undefined) {
  return target
    ? Object.freeze({
        AETHER_STATE_DIR: target.stateDir,
        AETHER_CONFIG_PATH: target.configPath,
        AETHER_WORKSPACE_DIR: target.defaultWorkspaceDir,
      })
    : undefined;
}
