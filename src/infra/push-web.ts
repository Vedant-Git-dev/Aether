...
import { expectDefined, normalizeOptionalString } from "@aether/normalization-core";
...
function resolveVapidSubjectFromEnv(): string {
  return (
    normalizeOptionalString(process.env.AETHER_VAPID_SUBJECT) ?? DEFAULT_WEB_PUSH_VAPID_SUBJECT
  );
}

function resolveVapidPublicKeyFromEnv(): string | undefined {
  return normalizeOptionalString(process.env.AETHER_VAPID_PUBLIC_KEY);
}

function resolveVapidPrivateKeyFromEnv(): string | undefined {
  return normalizeOptionalString(process.env.AETHER_VAPID_PRIVATE_KEY);
}
