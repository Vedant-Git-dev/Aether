...
import { AGENT_MEDIA_SCHEMA_VERSION } from "../state/aether-agent-db-contract.js";
import { assertAetherAgentDatabaseOwner } from "../state/aether-agent-db-maintenance.js";
import {
  registerAetherAgentDatabase,
  unregisterAetherAgentDatabase,
} from "../state/aether-agent-db-registry.js";
import { assertAetherAgentSchemaContains } from "../state/aether-agent-db-schema-helpers.js";
import {
  ensureAetherAgentDatabaseSchema,
  migrateAetherAgentDatabaseToMediaPrerequisiteSchema,
} from "../state/aether-agent-db-schema.js";
import type { DB as AetherAgentKyselyDatabase } from "../state/aether-agent-db.generated.js";
import {
  AETHER_AGENT_SCHEMA_VERSION,
  withAgentDatabaseMaintenanceLease,
  type AetherAgentDatabase,
} from "../state/aether-agent-db.js";
import { withLegacySessionParticipantsSchema } from "../state/aether-agent-participants-migration.js";
import { AETHER_AGENT_SCHEMA_SQL } from "../state/aether-agent-schema.js";
import { AETHER_SQLITE_BUSY_TIMEOUT_MS } from "../state/aether-state-db.js";
...
async function migrateAgentDatabase(params: {
  ...
  try {
    ...
    let metadata = assertAetherAgentDatabaseOwner(database, {
      agentId: params.agentId,
      pathname: params.pathname,
    });
    ...
    if (userVersion <= PREVIOUS_MEDIA_SCHEMA_VERSION) {
      migrateAetherAgentDatabaseToMediaPrerequisiteSchema(database, {
        agentId: params.agentId,
        path: params.pathname,
      });
      metadata = assertAetherAgentDatabaseOwner(database, {
        agentId: params.agentId,
        pathname: params.pathname,
      });
    ...
    if (userVersion >= AGENT_MEDIA_SCHEMA_VERSION) {
      ...
      ensureAetherAgentDatabaseSchema(database, {
        agentId: params.agentId,
        path: params.pathname,
      });
      userVersion = readSqliteUserVersion(database);
    }
    const schemaMode = userVersion < AETHER_AGENT_SCHEMA_VERSION ? "legacy" : "current";
    const schemaSql =
      schemaMode === "legacy"
    ...
    if (userVersion === PREVIOUS_MEDIA_SCHEMA_VERSION) {
      repairCanonicalSqliteIndexes(database, params.pathname, schemaSql, {
        validateAfterRepair: () =>
          assertAetherAgentSchemaContains(database, params.pathname, schemaSql, schemaMode),
      });
    }
    assertAetherAgentSchemaContains(database, params.pathname, schemaSql, schemaMode);
    ...
    ensureAetherAgentDatabaseSchema(database, { agentId: params.agentId, path: params.pathname });
    const rewrittenArchives = await migrateArchives();
    refreshAgentDatabasePlannerStatistics(database);
    return {
      ...rewritten,
      ...
      for (const entry of discovery.targets) {
        ...
        if (recovery) {
          ...
          if (recovery.recovered) {
            unregisterAetherAgentDatabase({ agentId: entry.agentId, env, path: pathname });
        ...
        try {
          ...
          if (entry.source !== "registry" || schemaAdvanced) {
            registerAetherAgentDatabase({ agentId: entry.agentId, env, path: pathname });
          }
          if (schemaAdvanced) {
            changes.push(
          ...
          if (result.rewrittenSessions > 0 || result.rewrittenTrajectoryRows > 0) {
            changes.push(
              `Migrated media persistence in ${pathname}: ${result.rewrittenSessions} transcript session(s), ${result.rewrittenTrajectoryRows} trajectory row(s), schema v${AETHER_AGENT_SCHEMA_VERSION}.`,
            );
          }
