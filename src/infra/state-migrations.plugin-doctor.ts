...
import { withAgentDatabaseMaintenanceLease } from "../state/aether-agent-db.js";
import { repairAetherStateDatabaseSchemaIfNeeded } from "../state/aether-state-db.js";
...
async function migratePluginDoctorStatePlans(
  ...
  try {
    lock = await acquireGatewayLock({
      allowInTests: true,
      env: { ...input.env, AETHER_STATE_DIR: input.stateDir },
...
export async function autoMigrateLegacyPluginDoctorState(params: {
  ...
  const stateSchema = repairAetherStateDatabaseSchemaIfNeeded({
