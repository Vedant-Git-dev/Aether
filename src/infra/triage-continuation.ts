...
export async function resolveTriageEntrypoint(root: string): Promise<[string, string, "triage"]> {
  ...
  if (!entry) {
    throw new Error(
      "installed CLI entry is unavailable; repair the installation and run aether triage manually",
    );
  }
...
export async function queueManagedUpdateTriage(
  ...
  if (process.env.AETHER_UPDATE_RUN_HANDOFF !== "1") {
    return false;
  }
  ...
  if (!root || !claim || !process.connected) {
    throw new Error("managed update triage lost its live owner; run aether triage manually");
  }
  const request = {
    type: "triage-request",
    version: 2,
...
export async function continueTriageInFreshProcess(params: {
  ...
  if (acquired.kind === "busy") {
    params.output(
      "Automatic triage already owned for this installation; wait for its cleanup or inspect the saved diagnostics and run aether triage manually.\n",
    );
    return;
  }
  ...
  try {
    ...
    if (forced || (!store.release(lease) && admitted)) {
      ...
      throw new Error(
        "automatic triage cleanup is uncertain; automatic admission remains blocked for this OS boot. Inspect saved diagnostics and run aether triage manually; do not delete the claim while work may remain. A verified different OS boot allows a fresh automatic attempt",
      );
    }
    params.signal.throwIfAborted();
    if (!admitted || exit.code !== 0 || exit.signal) {
      throw new Error(
        `automatic triage candidate ${admitted ? `failed (exit ${exit.code ?? "signal"})` : "is incompatible"}; run aether triage manually`,
      );
    }
  } finally {
    clearTimeout(timeout);
...
> {
  ...
  if (!process.send || !process.connected) {
    throw new Error(
      "automatic triage requires its original connected owner; run aether triage manually",
    );
  }
  const store = createManagedHandoffLeaseStore();
  ...
  try {
    ...
    assertCurrent();
    delete process.env.AETHER_UPDATE_RUN_HANDOFF;
