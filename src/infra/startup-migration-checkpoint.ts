...
import { withAetherStateStartupMigrationCheckpointDatabase } from "../state/aether-state-db.js";
import { resolveAetherStateSqlitePath } from "../state/aether-state-db.paths.js";
import { assertAetherStateWriteAllowed } from "../state/aether-state-ownership.js";
...
function withStartupMigrationCheckpointDatabase<T>(
  ...
  return withAetherStateStartupMigrationCheckpointDatabase(callback, { env });
}
...
function writeStartupMigrationCheckpointDatabase<T>(
  ...
  return withStartupMigrationCheckpointDatabase(env, (db) =>
    runSqliteImmediateTransactionSync(db, () => {
      assertAetherStateWriteAllowed({ database: db, databasePath, env });
      return callback(db);
    }),
  );
}
...
function assertStartupMigrationLeaseOwnedInTransaction(params: {
  ...
  if (!activeLease) {
    throw new Error(
      "Aether startup migration lease was lost before startup migrations completed; retry so migrations can run under a fresh lease.",
    );
  }
}
  ...
  writeStartupMigrationCheckpointDatabase(env, (db) => {
    ...
    if (existing && isStartupMigrationLeaseOwnerDefinitelyGone(existingOwner)) {
      ...
      throw new StartupMigrationLeaseConflictError(
        `Aether startup migrations are already running for this state directory; retry after the other Aether process finishes or after ${new Date(existing.expiresAt ?? expiresAt).toISOString()}.${ownerHint}`,
