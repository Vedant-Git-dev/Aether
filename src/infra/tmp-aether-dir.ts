// Creates temporary Aether directories for runtime scratch work.
import { getSealedRuntimeSecureTempRoot } from "./sealed-runtime-registry.js";

/** Preferred shared Aether temp root on POSIX systems when ownership and permissions are safe. */
export const DEFAULT_POSIX_TMP_ROOT = "/tmp/aether";
...
export type ResolvePreferredAetherTmpDirOptions = {
...
/** Resolves a safe Aether temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredAetherTmpDir(
  options: ResolvePreferredAetherTmpDirOptions = {},
): string {
  return loadResolveSecureTempRoot()({
    ...options,
    preferredDir: options.preferredDir ?? DEFAULT_POSIX_TMP_ROOT,
    fallbackPrefix: "aether",
    warningPrefix: "[aether]",
    unsafeFallbackLabel: "Aether temp dir",
