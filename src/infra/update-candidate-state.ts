...
import type { AetherSchemaVersions } from "../state/aether-schema-versions.js";
import { tableExists } from "../state/aether-state-db-schema-helpers.js";
import { readStateSchemaContentVersion } from "../state/aether-state-db-schema-version.js";
import type { DB } from "../state/aether-state-db.generated.js";
import { resolveAetherRegisteredAgentDatabasePath } from "../state/aether-state-db.paths.js";
...
type StateInput = { stateDir: string; config: AetherConfig; env?: NodeJS.ProcessEnv };
...
export function updateStateSchemaVersionsMatch(
  ...
  params: { sharedPath: string; candidateSchemaVersions?: AetherSchemaVersions },
): boolean {
...
function collectRegisteredPaths(db: DatabaseSync, shared: string, files: string[]) {
  ...
  return rows.map(({ path: stored }) => {
    const source = resolveAetherRegisteredAgentDatabasePath(shared, stored);
...
async function collectStateDatabasePaths(input: StateInput): Promise<string[]> {
  const shared = path.resolve(input.stateDir, "state", "aether.sqlite");
  ...
  for (const directory of [input.env?.AETHER_AGENT_DIR, input.env?.PI_CODING_AGENT_DIR]) {
    if (directory?.trim()) {
      files.add(path.join(resolveUserPath(directory, input.env), "aether-agent.sqlite"));
    }
  }
  ...
  for (const [id, agent] of [...configured, ...projected]) {
    directories.push(id);
    if (agent.agentDir) {
      files.add(path.join(resolveUserPath(agent.agentDir, input.env), "aether-agent.sqlite"));
    }
  }
  for (const id of new Set(["main", ...directories])) {
    files.add(path.resolve(input.stateDir, "agents", id, "agent", "aether-agent.sqlite"));
  }
  ...
  const result = await runCommandBuffered(
    ...
    ],
    {
      ...
      input: JSON.stringify({
        ...
        env: {
          HOME: sourceEnv.HOME,
          AETHER_HOME: sourceEnv.AETHER_HOME,
          USERPROFILE: sourceEnv.USERPROFILE,
          AETHER_AGENT_DIR: sourceEnv.AETHER_AGENT_DIR,
...
export async function snapshotUpdateCandidateState(
  ...
  const shared = path.join(sourceRoot, "state", "aether.sqlite");
              ...
              transform: (db: DatabaseSync) => {
                ...
                for (const { stored, source } of collectRegisteredPaths(db, shared, files)) {
                  ...
                  if (
                    ...
                    source === resolveAetherRegisteredAgentDatabasePath(shared, reboundStored)
                  ) {
