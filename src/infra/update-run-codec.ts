...
import type { UpdateRuns } from "../state/aether-state-db.generated.js";
...
export type UpdateRunLedgerOptions = AetherStateDatabaseOptions & {
...
export function encodeRun(input: UpdateRunRecord, options: UpdateRunLedgerOptions): UpdateRuns {
  ...
  const roots: [string | undefined, string][] = [
    ...
    [env.HOME, "~"],
    [env.USERPROFILE, "~"],
    [resolveStateDir(env), "$AETHER_STATE_DIR"],
    [env.AETHER_CONFIG_PATH, "[path]"],
