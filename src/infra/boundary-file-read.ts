...
function preserveAetherOverflowError(error: unknown, maxBytes: number): never {
...
/** Read a pinned descriptor without changing Aether's user-facing overflow error. */
export async function readFileDescriptorBounded(fd: number, maxBytes: number): Promise<Buffer> {
  try {
    ...
    return preserveAetherOverflowError(error, maxBytes);
  }
}
