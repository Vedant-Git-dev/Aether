...
type GatewayRestartSentinelDatabase = Pick<AetherStateKyselyDatabase, "gateway_restart_sentinel">;
...
function isValidTerminalReceipt(receipt: UpdateFailureReportReceipt): boolean {
  ...
  if (receipt.status === "created") {
    return (
      ...
      isCanonicalGithubUrl(receipt.url, /^\/aether\/aether\/issues\/\d+$/u, {
        allowSearch: false,
      })
    );
  }
  if (receipt.status === "fallback") {
    return (
      ...
      isCanonicalGithubUrl(receipt.fallbackUrl, /^\/aether\/aether\/issues\/new$/u, {
        allowSearch: true,
      })
    );
  }
...
function parseReceipt(sentinel: RestartSentinel | null): UpdateFailureReportReceipt | null {
  ...
  if (
    ...
    (value.status === "created" &&
      !isCanonicalGithubUrl(value.url, /^\/aether\/aether\/issues\/\d+$/u, {
        allowSearch: false,
      })) ||
    (value.status === "fallback" &&
      !isCanonicalGithubUrl(value.fallbackUrl, /^\/aether\/aether\/issues\/new$/u, {
        allowSearch: true,
      })) ||
