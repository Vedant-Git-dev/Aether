  ...
  if (siblingPackages.length > 0) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `Aether shares a pnpm ${owner.layoutVersion} global install group with ${siblingPackages.join(", ")}. Automatic update stopped before mutation; update the group manually to preserve its sibling packages.`,
      },
    };
  }
  ...
  if (!invokingPackageRoot || activePackageRoots.length !== 1 || ownerMatchCount !== 1) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `Expected exactly one active pnpm ${owner.layoutVersion} Aether install owned by the invoking project; found ${activePackageRoots.length} active installs and ${ownerMatchCount} owner matches. Automatic update stopped before mutation.`,
      },
    };
  }
  ...
  if (
    ...
    return {
      ...
      failedStep: {
        ...
        stderrTail: `The active pnpm command owns ${reportedGlobalRoot || "an unknown global root"}, not the invoking Aether install at ${expectedGlobalRoot ?? "an unknown root"}. Automatic update stopped before mutation.`,
      },
    };
  }
...
async function createStagedPackageInstall(
  ...
  // Active stages must stay outside cleanupGlobalRenameDirs' disposable ".aether-" namespace.
  const prefix = await fs.mkdtemp(path.join(targetLayout.globalRoot, ".aether.update-stage-"));
...
async function prepareNpmGitSourceInstallSpec(params: {
  ...
  const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-update-pack-"));
  ...
  try {
    ...
    if (finalInstallStep.exitCode === 0 && verificationPackageRoot) {
      ...
      const blockingVerificationErrors = verificationErrors.filter(
        (error) =>
          params.installSpec !== "aether@2026.8.1" ||
