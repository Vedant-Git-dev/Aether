...
const GATEWAY_LOCKS = createFileLockManager("aether.gateway-lock");
...
async function resolveGatewayOwnerStatus(
  ...
  if (
    ...
    if (role === "agent-embedded") {
      ...
      // local TUI, and CLI model probes), so validate the owning Aether process
      // instead of baking one command spelling into stale-lock recovery.
      return isAetherArgv(args) ? "alive" : "dead";
    }
    const command = role === "sqlite-maintenance" ? "doctor" : "skills";
    return isAetherCommandArgv(args, command) ? "alive" : "dead";
  }
...
export async function acquireGatewayLock(
  ...
  try {
    stateLifecycle = acquireGatewayLifecycleCoordinator({
      databasePath: path.join(paths.stateDir, "state", "aether.sqlite"),
  ...
  const shouldAcquireConfigLock = role !== "gateway" || env.AETHER_ALLOW_MULTI_GATEWAY !== "1";
...
async function acquireLockFile(
  ...
  const owner =
    lastPayload?.role === "agent-embedded"
      ? `another embedded Aether state writer is active${ownerPid}`
