import { normalizeNullableString as toOptionalTrimmedString } from "@aether/normalization-core/string-coerce";
import { readProviderJsonResponse } from "../agents/provider-http-errors.js";
import { runCommandWithTimeout } from "../process/exec.js";
import {
  parseAetherSchemaVersions,
  type AetherSchemaVersions,
} from "../state/aether-schema-versions.js";
...
type NpmPackageTargetStatus = {
  ...
  schemaVersions?: AetherSchemaVersions;
  error?: string;
};
...
function parseNpmPackageTargetMetadata(raw: string): {
  ...
  const aether = rec.aether && typeof rec.aether === "object" ? rec.aether : null;
  const schemaVersions =
    parseAetherSchemaVersions(rec["aether.schemaVersions"]) ??
    (aether
      ? parseAetherSchemaVersions((aether as Record<string, unknown>).schemaVersions)
      : undefined);
  return {
...
function packageTargetSpec(params: { target: string; spec?: string }): string {
  ...
  return spec || `aether@${params.target.trim() || "latest"}`;
}

const PUBLIC_NPM_REGISTRY_URL = "https://registry.npmjs.org/";
const PUBLIC_NPM_PACKAGE_NAME = "aether";
...
async function fetchNpmPackageTargetStatusFromRegistry(params: {
  ...
  try {
    ...
    const json = await readProviderJsonResponse<{
      ...
      aether?: { schemaVersions?: unknown };
    }>(res, "npm package target status");
    const schemaVersions = parseAetherSchemaVersions(json.aether?.schemaVersions);
    return {
      target: params.target,
