...
function snapshotFromExecApprovalsDatabase(
  db: ReturnType<typeof openAetherStateDatabase>["db"],
...
function readExecApprovalsSnapshotFromDatabase(
  ...
  assertNoPendingLegacyExecApprovals();
  return snapshotFromExecApprovalsDatabase(openAetherStateDatabase(options).db);
}

function readExecApprovalsSnapshotFromDatabaseReadOnly(): ExecApprovalsSnapshot {
  assertNoPendingLegacyExecApprovals();
  return (
    withExistingAetherStateDatabaseReadOnly(({ db }) => snapshotFromExecApprovalsDatabase(db)) ??
    snapshotFromExecApprovalsRow({
      path: resolveExecApprovalsDisplayPath(),
      row: undefined,
    })
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.execApprovalsStoreTestApi")] =
    testing;
}
