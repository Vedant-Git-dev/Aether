...
const AETHER_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "aether.diagnostics.v1";
...
const timelineWriter = resolveGlobalSingleton(
  Symbol.for("aether.diagnosticsTimelineWriter"),
  () => {
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.AETHER_DIAGNOSTICS)) &&
    typeof env.AETHER_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.AETHER_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  ...
  const normalized = {
    schemaVersion: AETHER_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.AETHER_DIAGNOSTICS_RUN_ID ? { runId: env.AETHER_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.AETHER_DIAGNOSTICS_ENV ? { envName: env.AETHER_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.AETHER_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
