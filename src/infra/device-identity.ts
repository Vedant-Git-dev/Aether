...
import { resolveAetherStateDirForDatabasePath } from "../state/aether-state-db.paths.js";
...
function resolveLegacyDeviceIdentityPath(options: DeviceIdentityStoreOptions = {}): string {
  ...
  return path.join(
    resolveAetherStateDirForDatabasePath(databasePath),
...
function withDeviceIdentityCoordinator<T>(
  ...
  const coordinator = acquireDeviceIdentityCoordinator({
    ...
    stateDir: resolveAetherStateDirForDatabasePath(resolved.databasePath),
  });
  let result: T;
  try {
