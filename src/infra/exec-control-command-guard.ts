...
function stripAetherPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "aether") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "aether"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "aether"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "aether") {
...
function parseAetherChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const aetherArgv = stripAetherPackageRunner(argv);
  return (
    normalizeCommandBaseName(aetherArgv[0]) === "aether" &&
    (aetherArgv[1] === "channels" || aetherArgv[1] === "channel") &&
    aetherArgv[2] === "login"
  );
}
...
function expandSqliteDatabaseToken(token: string, stateDir: string): string | null {
  ...
  const stateVariable = expanded.match(
    /^\$(?:AETHER_STATE_DIR|\{AETHER_STATE_DIR\})(?=$|[\\/])/u,
  );
  if (stateVariable) {
...
function targetsLiveStateSqliteDatabase(
  ...
  // External SQLite clients bypass Aether's runtime/version guard and can join the live WAL.
...
export async function detectUnsafeExecControlShellCommand(
  ...
  for (const candidate of controlCandidates) {
    ...
    if (parseAetherChannelsLoginShellCommand(candidate)) {
...
export async function rejectUnsafeExecControlShellCommand(command: string): Promise<void> {
  ...
  if (unsafeKind === "channel-login") {
    throw new Error(
      [
        "exec cannot run interactive Aether channel login commands.",
        "Run `aether channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
      ].join(" "),
    );
  }
}
...
export async function rejectUnsafeExecLiveStateSqliteShellCommand(
  ...
  throw new Error(
    [
      "external sqlite3 cannot open databases under the active Aether state directory.",
      "Use Aether commands for live state, or inspect a private backup copy outside `AETHER_STATE_DIR`.",
    ].join(" "),
  );
}
