...
function emitGatewayRestart(reasonOverride?: string, intent?: GatewayRestartIntent): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerAetherRestart() uses schtasks to restart the gateway.
      const result = triggerAetherRestart();
      if (!result.ok) {
...
export function triggerAetherRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.AETHER_SYSTEMD_UNIT,
      process.env.AETHER_PROFILE,
    );
  ...
  const label =
    process.env.AETHER_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AETHER_PROFILE);
