// Computes git, dependency, and registry update status for Aether installs.
...
function resolveExtendedStableRegistryTarget(params: {
  ...
  const packageSpecOverride = env.AETHER_UPDATE_PACKAGE_SPEC?.trim();
...
async function isLocklessAetherNpmInstall(params: {
  root: string;
  manager: PackageManager;
}): Promise<boolean> {
  if (
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const aetherReleaseCmp = compareAetherReleaseVersions(a, b);
    if (aetherReleaseCmp != null) {
      return aetherReleaseCmp;
    }
  }
...
export async function checkUpdateStatus(params: {
  ...
  const packageManager =
    !isGit &&
    (await isLocklessAetherNpmInstall({
      root,
