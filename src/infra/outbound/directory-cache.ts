...
import { resolveNonNegativeIntegerOption } from "@aether/normalization-core/number-coercion";
...
export class DirectoryCache<T> {
  private cachesByConfig = new WeakMap<AetherConfig, Map<string, CacheEntry<T>>>();
   ...
   */
  get(key: string, cfg: AetherConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: AetherConfig): void {
    const cache = this.cacheForConfig(cfg);
    const now = Date.now();
   ...
   */
  clearMatching(match: (key: string) => boolean, cfg: AetherConfig): void {
  ...
  private cacheForConfig(cfg: AetherConfig): Map<string, CacheEntry<T>> {
