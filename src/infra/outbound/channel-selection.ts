...
function resolveAvailableChannel(params: {
  ...
  // been eagerly loaded (e.g. `aether agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: AetherConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: aether doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run aether channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use aether channels list --all to see available channel ids.",
  ].join(" ");
}
...
export async function listConfiguredMessageChannels(cfg: AetherConfig): Promise<string[]> {
