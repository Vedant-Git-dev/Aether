...
export function installDeliveryQueueTmpDirHooks(): { readonly tmpDir: () => string } {
  ...
  beforeAll(() => {
    fixtureRoot = fs.mkdtempSync(path.join(resolvePreferredAetherTmpDir(), "aether-dq-suite-"));
  });
...
export function readQueuedEntry(tmpDir: string, id: string): Record<string, unknown> {
  const { db } = openAetherStateDatabase({ env: { ...process.env, AETHER_STATE_DIR: tmpDir } });
  const row = db
