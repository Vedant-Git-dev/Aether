...
import { resolveAetherPackageRoot, resolveAetherPackageRootSync } from "./aether-root.js";
...
async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAetherPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "aether" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/aether -> .../dist/index.js)
...
export async function ensureControlUiAssetsBuilt(
  ...
  if (
    ...
    return controlUiAssetsFailure(
      `${hint}. Reinstall Aether to restore bundled Control UI assets.`,
    );
  }
