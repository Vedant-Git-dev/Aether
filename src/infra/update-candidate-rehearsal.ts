...
export async function prepareUpdateCandidateRehearsal(params: {
  ...
  const tempDir = await fs.realpath(
    await fs.mkdtemp(path.join(os.tmpdir(), "aether-update-canary-")),
  );
  const sourceEnv = params.env ?? process.env;
  const configPath = path.join(tempDir, "aether.json");
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    AETHER_HOME: tempDir,
    AETHER_STATE_DIR: tempDir,
    AETHER_CONFIG_PATH: configPath,
    AETHER_WORKSPACE_DIR: workspaceDir,
    AETHER_AGENT_DIR: copiedAgentDir(sourceEnv.AETHER_AGENT_DIR),
    PI_CODING_AGENT_DIR: copiedAgentDir(sourceEnv.PI_CODING_AGENT_DIR),
    AETHER_BUNDLED_PLUGINS_DIR: undefined,
    AETHER_DISABLE_BUNDLED_PLUGINS: undefined,
    AETHER_SKIP_CHANNELS: "1",
    AETHER_SKIP_PROVIDERS: "1",
    AETHER_SKIP_CRON: "1",
    ...
    AETHER_NO_AUTO_UPDATE: "1",
    NODE_DISABLE_COMPILE_CACHE: "1",
    AETHER_GATEWAY_SERVICE_PID: undefined,
    AETHER_GATEWAY_PORT: undefined,
    AETHER_COMPATIBILITY_HOST_VERSION: undefined,
    AETHER_GATEWAY_TOKEN: undefined,
    AETHER_GATEWAY_PASSWORD: undefined,
    AETHER_PROFILE: undefined,
    AETHER_DIAGNOSTICS_TIMELINE_PATH: undefined,
    AETHER_TEST_MINIMAL_GATEWAY: undefined,
      ...
      ],
      {
        input: JSON.stringify({
          ...
          env: {
            ...
            AETHER_BUNDLED_PLUGINS_DIR: sourceEnv.AETHER_BUNDLED_PLUGINS_DIR,
            AETHER_DISABLE_BUNDLED_PLUGINS: sourceEnv.AETHER_DISABLE_BUNDLED_PLUGINS,
          },
        }),
        baseEnv: env,
        timeoutMs: remaining(),
