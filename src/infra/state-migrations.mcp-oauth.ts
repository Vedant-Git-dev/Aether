...
type McpOAuthMigrationDatabase = Pick<AetherStateKyselyDatabase, "mcp_oauth_stores">;
    ...
    try {
      ...
      const staleGuidance =
        (error as { code?: unknown }).code === "file_lock_stale"
          ? " Verify no older Aether process is running, remove the retired .lock sidecar, and rerun Doctor."
          : "";
      warnings.push(
