...
const abortedTransactionSymbol = Symbol.for("aether.sqliteAbortedTransaction");
...
const writeAdmissionServices = resolveGlobalSingleton(
  Symbol.for("aether.sqliteWriteAdmissionServices"),
...
function runSqliteTransactionSync<T>(
  ...
  if (db.isTransaction) {
    ...
    db.exec("SAVEPOINT aether_tx_nested");
    try {
      const result = operation();
      assertSyncTransactionResult(result);
      assertTransactionUsable(db);
      db.exec("RELEASE SAVEPOINT aether_tx_nested");
      return result;
    } catch (error) {
      ...
      try {
        db.exec("ROLLBACK TO SAVEPOINT aether_tx_nested");
        db.exec("RELEASE SAVEPOINT aether_tx_nested");
      } catch {
