...
import { triggerAetherRestart } from "./restart.js";
...
const PNPM_VERSIONED_AETHER_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2aether@[^\\/]+\2node_modules\2aether\2.+$/;

function rewritePnpmVersionedAetherEntryPath(entryPath: string): string {
  // pnpm can expose argv[1] as a versioned realpath that self-update removes.
  // Respawn through the stable Aether package wrapper instead.
  return entryPath.replace(
    PNPM_VERSIONED_AETHER_ENTRY_PATTERN,
    "$1$2node_modules$2aether$2aether.mjs",
  );
}
 ...
 * - AETHER_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthyEnvValue(process.env.AETHER_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerAetherRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthyEnvValue(process.env.AETHER_NO_RESPAWN)) {
    return { mode: "disabled", detail: "AETHER_NO_RESPAWN" };
  }
  try {
    const [entryArg, ...entryArgs] = process.argv.slice(1);
    const args = [
      ...process.execArgv,
      ...(entryArg ? [rewritePnpmVersionedAetherEntryPath(entryArg)] : []),
      ...entryArgs,
    ];
