...
const PACKAGE_LIFECYCLE_LOCK_RELATIVE_PATH = ".aether-lifecycle-lock";
  ...
  while (await isPackageLifecyclePending(paths)) {
    try {
      ...
      if (Date.now() >= waitDeadline) {
        throw new Error("timed out waiting for another Aether package lifecycle", {
          cause: error,
        });
      }
...
function runPackageLifecycleScript(
  ...
  if (result.status !== 0) {
    throw new Error(
      `Aether package ${script.name} failed${result.signal ? ` with ${result.signal}` : ` with exit code ${result.status ?? "unknown"}`}`,
    );
  }
}
...
export async function completePendingPackageLifecycle(params: {
  ...
  try {
    ...
    if (await isPackageLifecyclePending(paths)) {
      throw new Error("Aether package postinstall did not complete its lifecycle marker");
    }
    return true;
  } catch (error) {
