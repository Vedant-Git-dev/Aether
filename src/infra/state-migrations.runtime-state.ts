...
type LegacyVoiceWakeImportDatabase = Pick<AetherStateKyselyDatabase, "config_machine_state">;
type LegacyConfigHealthImportDatabase = Pick<AetherStateKyselyDatabase, "config_health_entries">;
...
const DEFAULT_VOICEWAKE_TRIGGERS = ["aether", "claude", "computer"];
...
export function migrateLegacyJsonState<Value>(params: {
  ...
  try {
    outcome = runAetherStateWriteTransaction(({ db }) => params.migrate(db, value), {
