...
import { ensureColumn } from "../state/aether-state-db-schema-helpers.js";
...
export const DEFAULT_WEB_PUSH_VAPID_SUBJECT = "https://aether.ai";
...
const ensureWebPushApprovalDeliveryStateSchema = createAetherStateSchemaEnsurer({
...
function webPushStateDatabaseOptions(stateDir?: string): AetherStateDatabaseOptions {
  return stateDir
...
export function findBoundWebPushSubscriptionByEndpoint(params: {
  ...
  const database = openAetherStateDatabase(webPushStateDatabaseOptions(params.stateDir));
...
export function listWebPushSubscriptions(stateDir?: string): WebPushSubscription[] {
  ...
  const database = openAetherStateDatabase(webPushStateDatabaseOptions(stateDir));
