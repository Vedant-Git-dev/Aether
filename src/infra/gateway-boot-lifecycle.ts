...
export function formatGatewayCrashLoopManualChannelStartHint(target?: {
  ...
  const command = formatCliCommand("aether gateway call channels.start");
...
type GatewayBootLifecycleDatabase = Pick<AetherStateKyselyDatabase, "gateway_boot_lifecycle">;
...
export function repairGatewayMaintenanceStartupFailures(
  ...
  if (!pathMayExistSync(resolveAetherStateSqlitePath(env))) {
    return 0;
  }
  try {
