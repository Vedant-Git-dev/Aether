...
type AgentCacheDatabase = Pick<AetherAgentKyselyDatabase, "cache_entries">;
...
function readCacheDatabase<T>(
  ...
  try {
    const result = withAetherAgentDatabaseReadOnly(operation, {
...
export function acquireSessionCostUsageRefreshLock(
  ...
  const acquired = runAetherAgentWriteTransaction(
    (database) => {
      const kysely = getNodeSqliteKysely<AgentCacheDatabase>(database.db);
      const currentRaw =
        executeSqliteQuerySync(
