...
export function createFleetService(options: FleetServiceOptions = {}) {
  ...
  return {
        ...
        operation: async (checkpoint) => {
          ...
          if (result.started) {
            try {
              ...
              throw new Error(
                `Fleet cell ${tenantId} was created but did not become healthy within 60s; inspect it with \`aether fleet status ${tenantId}\` or \`aether fleet logs ${tenantId}\`, or remove it with \`aether fleet rm ${tenantId} --force\`.`,
                { cause: error },
              );
            }
          }
    ...
    async logs(logOptions: FleetLogsOptions): Promise<void> {
      ...
      const gatewayCredential = inspection.environment.AETHER_GATEWAY_TOKEN;
