...
/**
 * Runtime bridge between MCP tools and the Aether Gateway channel APIs.
 *
...
export class AetherChannelBridge {
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`aether mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(
    ...
    try {
      ...
      process.stderr.write(`aether mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `aether mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
      return "failed";
    }
  ...
  private async dispatchGatewayEvent(event: EventFrame): Promise<void> {
    try {
      ...
      process.stderr.write(`aether mcp: gateway event ${event.event} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `aether mcp: gateway event ${event.event} error: ${String(error)}\n`,
        );
      }
    }
  }
