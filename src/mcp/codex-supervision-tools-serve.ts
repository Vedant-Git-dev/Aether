...
function withCodexSupervisionEnabled(config: AetherConfig): AetherConfig {
  const next = structuredClone(normalizePluginTargetConfig(config, "codex")) as AetherConfig &
  ...
  if (process.env.AETHER_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS === "1") {
    supervision.allowRawTranscripts = true;
  }
  if (process.env.AETHER_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS === "1") {
...
function requireCompleteCodexSupervisionToolSet(tools: readonly AnyAgentTool[]): void {
  ...
  throw new Error(
    `Codex supervision MCP could not load the official @aether/codex plugin tools (missing: ${missing.join(", ") || "none"}). Install or update @aether/codex, then enable Codex supervision.`,
  );
}
...
export function createCodexSupervisionToolsMcpServer(params: {
  ...
  return createToolsMcpServer({ name: "aether-codex-supervisor", ...params });
}
