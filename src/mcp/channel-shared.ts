...
/** Resolve the visible message id, including Aether metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__aether"] && typeof entry["__aether"] === "object"
      ? toText((entry["__aether"] as { id?: unknown }).id)
      : undefined)
  );
}
