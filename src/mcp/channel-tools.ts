...
import { z } from "zod";
import type { AetherChannelBridge } from "./channel-bridge.js";
import {
...
export function registerChannelMcpTools(server: McpServer, bridge: AetherChannelBridge): void {
  server.tool(
    "conversations_list",
    "List Aether channel-backed conversations available through session routes.",
    {
  ...
  server.tool(
    "conversation_get",
    "Get one Aether conversation by session key.",
  ...
  server.tool(
    "messages_read",
    "Read recent messages for one Aether conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "attachments_fetch",
    "List non-text attachments for a message in one Aether conversation.",
    {
      session_key: z.string().min(1),
  ...
  server.tool(
    "events_poll",
    "Poll queued Aether conversation events since a cursor.",
    {
  ...
  server.tool(
    "events_wait",
    "Wait for the next queued Aether conversation event.",
    {
      after_cursor: z.number().int().min(0).optional(),
      session_key: z.string().optional(),
  ...
  server.tool(
    "messages_send",
    "Send a message back through the same Aether conversation route.",
    {
      session_key: z.string().min(1),
      text: z.string().min(1),
    },
  ...
  server.tool(
    ...
    "List open Aether exec or plugin approval requests visible through the Gateway.",
    {},
    async () => {
  ...
  server.tool(
    "permissions_respond",
    "Allow or deny one pending Aether exec or plugin approval request.",
    {
