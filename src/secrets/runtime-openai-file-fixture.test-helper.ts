...
export function asConfig(value: unknown): AetherConfig {
...
export async function createOpenAIFileRuntimeFixture(home: string) {
  const configDir = path.join(home, ".aether");
...
export function createOpenAIFileRuntimeConfig(secretFile: string): AetherConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
