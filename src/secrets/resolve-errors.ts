...
export function describeSecretResolutionOperatorRecovery(value: unknown): string | undefined {
  ...
  return value.source === "exec"
    ? "Restore Windows path security verification, or use an existing provider command whose owner and ACLs Aether can verify"
    : "Restore Windows path security verification, or use an existing secret file whose owner and ACLs Aether can verify";
}
