...
function getSecretProviders(config: AetherConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: AetherConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: AetherConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "aether.json" | "auth-profile-store";
  path: string;
  agentId?: string;
}): string {
...
function resolveConfigureAgentId(config: AetherConfig, explicitAgentId?: string): string {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredAetherConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    if (sharedPlaintextCount > 0) {
      log.warn(
        `Shared auth-profile store has ${sharedPlaintextCount} plaintext credential(s). ` +
          ...
          "Run `aether secrets audit` to review them; a shared-store SecretRef migration path is tracked separately.",
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          ...
          candidate.configFile === "auth-profile-store" ? "auth profile store" : "aether.json",
