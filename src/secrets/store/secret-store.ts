...
import { ensureSecretStoreSchema } from "../../state/aether-state-db-schema-additive.js";
...
type SecretStoreDatabase = Pick<AetherStateKyselyDatabase, "secret_store_entries">;
type SecretStoreRow = Selectable<AetherStateKyselyDatabase["secret_store_entries"]>;
...
export function listSecretStoreEntries(params: {
  ...
  try {
    return (
      withExistingAetherStateDatabaseReadOnly(({ db: sqlite }) => {
...
export function readSecretStoreValue(params: {
  ...
  try {
    ...
    const { scopeKind, scopeId } = normalizeScope(params.scope);
    const row = withExistingAetherStateDatabaseReadOnly(({ db: sqlite }) => {
      const db = getNodeSqliteKysely<SecretStoreDatabase>(sqlite);
...
export function deleteSecretStoreEntry(params: {
  ...
  assertSecretStoreMutationName(params.name);
  const { scopeKind, scopeId } = normalizeScope(params.scope);
  const state = openAetherStateDatabase(params.database);
  const now = Date.now();
  try {
    runAetherStateWriteTransaction(
      ({ db: sqlite }) => {
