...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `aether.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built aether
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.AETHER_DEBUG_CHANNEL_CONTRACT_API === "1") {
