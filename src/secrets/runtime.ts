...
function hasConfiguredPluginEntries(config: AetherConfig): boolean {
...
function hasConfiguredChannelEntries(config: AetherConfig): boolean {
...
function hasConfiguredPluginIntegrationSecretProviders(config: AetherConfig): boolean {
...
function shouldLoadPluginMetadataForSecrets(config: AetherConfig): boolean {
  return (
...
export function activateSecretsRuntimeSnapshotWithSource(
  ...
  runtimeSourceConfig: AetherConfig,
): void {
  activateSecretsRuntimeSnapshotState({
...
export function activateSecretsRuntimeSnapshotIfCurrent(
  ...
  options?: { preserveActivationLineage?: boolean; runtimeSourceConfig?: AetherConfig },
): boolean {
...
export function restoreSecretsRuntimeSnapshotIfCurrent(
  ...
  options?: { runtimeSourceConfig?: AetherConfig },
): boolean {
...
async function prepareActiveSecretsRuntimeRefresh(
  sourceConfig: AetherConfig,
  includeAuthStoreRefs?: boolean,
  snapshotConfig: AetherConfig = sourceConfig,
...
function patchResolvedSecretRefLeaves(params: {
  ...
  source: unknown;
  resolved: unknown;
  defaults: NonNullable<AetherConfig["secrets"]>["defaults"];
...
function selectProviderAuthConfig(config: AetherConfig): AetherConfig {
  return {
...
export async function refreshActiveProviderAuthRuntimeSnapshot(): Promise<boolean> {
  for (;;) {
    ...
    if (modelsPatch.changed) {
      config.models = modelsPatch.value as AetherConfig["models"];
    }
