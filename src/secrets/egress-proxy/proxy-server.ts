...
const PROXY_AUTH_USERNAME = "aether";
const PROXY_AUTH_REALM = "Aether secret egress";
...
export async function startSecretEgressProxyServer(params: {
  ...
  const hostNotAllowedBody = (host: string): string =>
    `Host "${host}" is not in the secret egress proxy traffic allowlist. Add it to secrets.egressProxy.allowedHosts or bind a store secret to it with: aether secrets store set <NAME> --allow-host ${host}, then restart the Gateway.\n`;
  const authorize = (
    headers: IncomingHttpHeaders,
