...
public sealed class AetherPrivatePlanFile : IDisposable
{
    ...
    public static AetherPrivatePlanFile Open(
        string stagingPath,
        string finalPath,
        string securityDescriptor,
        out int errorCode)
    {
        ...
        try
        {
            ...
            return new AetherPrivatePlanFile(stagingPath, finalPath, handle);
        }
        finally
        {
...
export async function createPrivateWindowsPlanFile(
  ...
  const stagingPath = path.join(path.dirname(resolvedPath), `.aether-plan-${randomUUID()}.tmp`);
  const command = [
    ...
    "$openError = 0",
    "$native = [AetherPrivatePlanFile]::Open($stagingPath, $finalPath, $sddl, [ref]$openError)",
    "$errorCode = $openError",
