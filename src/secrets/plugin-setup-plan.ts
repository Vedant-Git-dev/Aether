...
import { isValidAgentId } from "@aether/normalization-core/agent-id";
...
export function parsePluginSecretTargetSpecifier(
  ...
  if (!value.startsWith("auth-profiles:")) {
    return {
      path: value.startsWith("aether:") ? value.slice("aether:".length) : value,
    };
  }
...
function createPluginConfigSecretTarget(params: {
  ...
  const resolved = resolveSecretPlanTargetByPathCore({
    configFile: params.agentId ? "auth-profile-store" : "aether.json",
    pathSegments,
