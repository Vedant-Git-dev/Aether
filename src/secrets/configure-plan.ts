/** Builds the interactive `aether secrets configure` target list and apply plan. */
...
/** Credential target shown by `aether secrets configure` before a SecretRef is selected. */
...
/** Builds configure candidates for Aether config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: AetherConfig;
  authoredAetherConfig?: AetherConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredAetherConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const aetherCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `aether.json` as const,
  ...
  return [...aetherCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "aether secrets configure",
