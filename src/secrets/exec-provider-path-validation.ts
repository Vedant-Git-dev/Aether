...
export async function assertSecureExecCommandPath(params: {
  ...
  if (process.platform === "win32" && perms.source === "unknown") {
    ...
    throw new FsSafeError(
      ...
      `${params.label} ACL verification unavailable on Windows for ${commandPath}. Move the command to a path whose ACLs Aether can verify; there is no provider-level bypass.`,
    );
  }
