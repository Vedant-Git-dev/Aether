...
export function hasSameSecretReloadContract(left: AetherConfig, right: AetherConfig): boolean {
...
export function hasSameSecretProviderDefinition(
  ...
  const dependency = (config: AetherConfig) => ({
...
function preserveResolvedSecretRefValues(
  ...
  restored: unknown,
  sourceConfig: AetherConfig,
  currentSourceConfig: AetherConfig,
): unknown {
...
function mergeRollbackAuthStoreCredentials(
  ...
  current: Record<string, AuthProfileStore>,
  restored: Record<string, AuthProfileStore>,
  configs: [AetherConfig, AetherConfig, AetherConfig],
  mutationLineage: typeof activeSnapshotLineageAuthMutations,
...
export function setSecretsRuntimeSourceSnapshotIfCurrent(params: {
  ...
  runtimeSourceConfig: AetherConfig;
  secretsSourceConfig: AetherConfig;
}): boolean {
...
function advanceSecretsRuntimeSourceSnapshot(sourceConfig: AetherConfig): void {
  if (activeSnapshot) {
