...
import { normalizeCsvOrLooseStringList } from "@aether/normalization-core/string-normalization";
...
/** Parses the JSON5 Aether manifest block embedded inside a string frontmatter field. */
export function resolveAetherManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
/** Extracts normalized runtime requirement lists from an Aether manifest block. */
export function resolveAetherManifestRequires(
  metadataObj: Record<string, unknown>,
): AetherManifestRequires | undefined {
  const requiresRaw =
...
export function resolveAetherManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an Aether manifest block. */
export function resolveAetherManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedAetherManifestInstallBase = {
...
export function parseAetherManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedAetherManifestInstallBase | undefined {
  ...
  const spec: ParsedAetherManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyAetherManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedAetherManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
