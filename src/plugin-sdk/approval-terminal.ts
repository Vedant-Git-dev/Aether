...
export function buildSystemAgentApprovalResolvedText(view: SystemAgentResolvedView): string {
  ...
  return outcome === "cancelled"
    ? "⚠️ Aether change was cancelled because its run ended. No change was made. Retry."
    : outcome === "deny"
      ? "❌ Aether change denied. No change was made."
      : outcome === "applied"
        ? `✅ Aether change approved and applied: ${view.operationSummary}`
        : outcome === "not-applied"
          ? "⚠️ Aether change approved, but it was not applied. Check the Gateway and retry."
          : `✅ Aether change approved. Applying: ${view.operationSummary}`;
}
