...
export function defineStrayPluginEntryConfigMigration(params: {
  ...
  normalizeConfig: (params: { cfg: AetherConfig }) => {
    config: AetherConfig;
    changes: string[];
  };
} {
  ...
  const readStrayEntryConfig = (cfg: AetherConfig): Record<string, unknown> | null => {
  ...
  return {
    legacyConfigRule: {
      ...
      message: `${entryConfigPath} is not read by the ${channelId} channel; run "aether vitals --fix" to move its keys to channels.${channelId}.`,
      match: (value) => {
