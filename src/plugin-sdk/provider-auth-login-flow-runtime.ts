...
export async function answerProviderLoginModelAccess(params: {
  ...
  runtime: RuntimeEnv;
  signal?: AbortSignal;
  assertCurrent: (config?: AetherConfig) => void;
  ...
  const assertCurrent = (config?: AetherConfig) => {
...
export async function prepareProviderChannelLogin(params: {
  ...
  if (
    ...
    return {
      status: "rejected",
      reply: {
        text: "Only a configured Aether owner/admin can start provider login from this channel.",
      },
    };
  }
  if (!params.isPrivateChat) {
    return {
      status: "reply",
      reply: {
        text: "Provider login requires a private chat or Control UI session. Open a private chat with Aether and send `/login` there.",
      },
    };
  }
