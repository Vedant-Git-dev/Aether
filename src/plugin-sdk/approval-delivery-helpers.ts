...
import type { AetherConfig } from "./config-runtime.js";
...
type ApproverRestrictedNativeApprovalFlatParams = {
  ...
  isNativeDeliveryEnabled: (params: { cfg: AetherConfig; accountId?: string | null }) => boolean;
...
type StandardNativeApprovalRoutingParams = {
  ...
  isTransportEnabled: (params: { cfg: AetherConfig; accountId?: string | null }) => boolean;
  /** Lists channel account ids for route and DM availability checks. */
  listAccountIds: (cfg: AetherConfig) => readonly string[];
  /** Resolves the channel's default account id. */
  resolveDefaultAccountId: (cfg: AetherConfig) => string;
...
function buildApproverRestrictedNativeApprovalCapability(
  ...
  return createChannelApprovalCapability({
    ...
    delivery: {
      hasConfiguredDmRoute: ({ cfg }: { cfg: AetherConfig }) =>
