 ...
 * model keeps refusing cross-session questions on those runtimes (aether#114797).
 */
...
export { isAetherRuntimeContextCustomMessage } from "../agents/internal-runtime-context.js";
...
export { VERSION as AETHER_VERSION } from "../version.js";
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@aether/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded Aether uses for progress logs.
 */
 ...
 * preserve the same Aether-owned fallback signals as the built-in Aether path
