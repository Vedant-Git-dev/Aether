...
const AETHER_PACKAGE_ROOT =
  resolveLoaderPackageRoot({
    modulePath: fileURLToPath(import.meta.url),
    moduleUrl: import.meta.url,
  }) ?? fileURLToPath(new URL("../..", import.meta.url));
const CURRENT_MODULE_PATH = fileURLToPath(import.meta.url);
const AETHER_SOURCE_EXTENSIONS_ROOT = path.resolve(AETHER_PACKAGE_ROOT, "extensions");
...
function resolveFacadeModuleLocationUncached(
  ...
  if (!areBundledPluginsDisabled(env)) {
    ...
    const bundledLocation = resolveBundledFacadeModuleLocation({
      ...
      packageRoot: AETHER_PACKAGE_ROOT,
      bundledPluginsDir,
    });
    if (bundledLocation) {
...
function resolveFacadeModuleLocation(
  ...
  const artifacts = getPluginCacheRoot(AETHER_PACKAGE_ROOT).artifacts;
...
function buildFacadeActivationCheckParams(
  ...
  return {
    ...params,
    location,
    sourceExtensionsRoot: AETHER_SOURCE_EXTENSIONS_ROOT,
  };
}
