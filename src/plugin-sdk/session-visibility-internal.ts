...
import { redactIdentifier } from "@aether/normalization-core/node-crypto";
...
export function lookupFailedDenialSuffix(kind: LookupFailureKind): string {
  if (kind === "transient") {
    return "spawned-session ownership lookup failed (transient); retry once, then ask the operator to inspect Aether logs.";
  }
  if (kind === "credentials") {
    return "spawned-session ownership lookup failed; ask the operator to check gateway configuration and credentials.";
  }
  return "spawned-session ownership lookup failed; ask the operator to inspect Aether logs.";
}
...
export function lookupFailedOperationMessage(
  ...
  const guidance =
    kind === "transient"
      ? "retry once, then ask the operator to inspect Aether logs"
      : kind === "credentials"
        ? "ask the operator to check gateway configuration and credentials"
        : "ask the operator to inspect Aether logs";
