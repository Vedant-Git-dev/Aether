...
export function restorePrivateQaCliEnv(originalPrivateQaCli: string | undefined): void {
  if (originalPrivateQaCli === undefined) {
    delete process.env.AETHER_ENABLE_PRIVATE_QA_CLI;
  } else {
    process.env.AETHER_ENABLE_PRIVATE_QA_CLI = originalPrivateQaCli;
  }
}
...
export function makePrivateQaSourceRoot(tempDirs: string[], prefix: string): string {
  ...
  process.env.AETHER_ENABLE_PRIVATE_QA_CLI = "1";
  return sourceRoot;
}
...
export async function expectPrivateQaLabRuntimeSurfaceLoad(params: {
  ...
  importRuntime: () => Promise<QaRuntimeModule>;
  loadBundledPluginPublicSurfaceModuleSync: SurfaceLoaderMock;
  resolveAetherPackageRootSync: SurfaceLoaderMock;
}) {
  const sourceRoot = makePrivateQaSourceRoot(params.tempDirs, "aether-qa-runtime-root-");
  params.resolveAetherPackageRootSync.mockReturnValue(sourceRoot);
  ...
  expect(params.loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
    ...
    env: expect.objectContaining({
      ...
      AETHER_BUNDLED_PLUGINS_DIR: path.join(sourceRoot, "extensions"),
    }),
  });
}
