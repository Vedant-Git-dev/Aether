...
import type { AetherPluginGatewayEvents } from "../plugins/gateway-events.js";
import {
  isPluginJsonValue,
...
export type FeatureInvocationContext = { api: AetherPluginApi } & (
  | { source: "session-action"; action: PluginSessionActionContext }
  | {
      source: "tool";
      tool: AetherPluginToolContext;
      toolCallId: string;
      signal?: AbortSignal;
...
export type DefineFeaturePluginOptions<C extends FeatureContract> = {
  ...
  setup: (api: AetherPluginApi, events: FeatureEventEmitter<C>) => FeatureHandlers<C>;
...
export function defineFeaturePlugin<C extends FeatureContract>(
  ...
  const entry = definePluginEntry({
    ...
    register(api) {
      ...
      let gatewayEvents: AetherPluginGatewayEvents | undefined;
