...
export async function closeQaRuntimeStores(tempRoot: string): Promise<void> {
  const [auth, agents, state, paths] = await Promise.all([
    ...
    import("../state/aether-state-db.paths.js"),
  ]);
  ...
  await agents.closeAetherAgentDatabasesAsync(tempRoot);
  state.closeAetherStateDatabaseByPath(
    paths.resolveAetherStateSqlitePath({ AETHER_STATE_DIR: path.join(tempRoot, "state") }),
  );
}
