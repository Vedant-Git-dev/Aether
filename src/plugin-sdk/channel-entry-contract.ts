...
export type AetherPluginApi = import("../plugins/types.js").AetherPluginApi;
export type AetherPluginCommandDefinition =
  import("../plugins/types.js").AetherPluginCommandDefinition;
...
type DefineBundledChannelSetupEntryOptions = {
  ...
  registerSetupRuntime?: (api: AetherPluginApi) => void;
...
export type BundledChannelSetupEntryFeatures = {
  /**
   * @deprecated Declare doctorContract.stateMigrations in aether.plugin.json instead.
   * Removal plan: remove the setup-entry adapter after the 2027.1 external-plugin migration window.
   */
...
const disableBundledEntrySourceFallbackEnv = "AETHER_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK";
...
function getSourceModuleLoader(
  modulePath: string,
  options: BundledEntryModuleLoadOptions,
  transformAetherDependencies = false,
) {
