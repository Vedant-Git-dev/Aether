...
import type { AuthProfileStore, AetherConfig } from "../provider-auth.js";
...
function installDiscoveryHooks(state: DiscoveryState, options: DiscoveryContractOptions) {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("aether/plugin-sdk/provider-auth", async (importOriginal) => {
      const actual = await importOriginal<typeof import("../provider-auth.js")>();
      return {
        ...actual,
        DEFAULT_COPILOT_API_BASE_URL: "https://api.individual.githubcopilot.com",
        MINIMAX_OAUTH_MARKER: "minimax-oauth",
        applyAuthProfileConfig: (config: AetherConfig) => config,
    ...
    vi.doMock("aether/plugin-sdk/provider-setup", async () => {
      const actual = await vi.importActual<typeof import("../provider-setup.js")>(
        "aether/plugin-sdk/provider-setup",
      );
      return {
        ...actual,
