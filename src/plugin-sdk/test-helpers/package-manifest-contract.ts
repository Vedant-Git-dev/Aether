    ...
    if (minHostVersionBaseline) {
      it("declares a parseable minHostVersion floor at or above the baseline", () => {
        ...
        const requirement = parseMinHostVersionRequirement(
          manifest.aether?.install?.minHostVersion ?? null,
        );

        expect(
          requirement,
          `${packagePath} should declare aether.install.minHostVersion`,
        ).not.toBeNull();
        if (!requirement) {
          return;
        }

        const comparison = compareAetherVersions(
        ...
        expect(
          comparison,
          `${packagePath} should require at least Aether ${minHostVersionBaseline}`,
