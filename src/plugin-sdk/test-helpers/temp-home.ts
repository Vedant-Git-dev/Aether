// Temp home test helpers create isolated Aether home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AETHER_HOME.
  deleteTestEnvValue("AETHER_HOME");
  setTestEnvValue("AETHER_STATE_DIR", path.join(base, ".aether"));
...
export async function withTempHomeCore<T>(
  ...
  const base = await allocateTempHomeBase(opts.prefix ?? "aether-test-home-");
  ...
  try {
    ...
    setTempHome(base);
    await fs.mkdir(path.join(base, ".aether", "agents", "main", "sessions"), { recursive: true });
    if (opts.env) {
    ...
    if (!opts.skipSessionCleanup) {
      await cleanupSessionStateForTest({ stateDir: path.join(base, ".aether") }).catch(
        () => undefined,
      );
    }
    snapshot.restore();
