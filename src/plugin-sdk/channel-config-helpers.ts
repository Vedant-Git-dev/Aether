...
type ChannelConfigAccessorParams<Config extends AetherConfig = AetherConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends AetherConfig = AetherConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends AetherConfig = AetherConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: AetherConfig; accountId?: string | null }) {
