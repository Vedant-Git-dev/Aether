...
function resolveFacadeModuleLocation(params: {
  ...
  const artifacts = getPluginCacheRoot(getAetherPackageRoot()).artifacts;
  ...
  const location = resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getAetherPackageRoot(),
    bundledPluginsDir,
  });
...
function resolveFacadeBoundaryOpenParams(boundaryRoot: string): {
  ...
  if (isPathAtOrInside(boundaryRoot, getAetherPackageRoot())) {
    return { boundaryLabel: "Aether package root", rejectHardlinks: false };
  }
