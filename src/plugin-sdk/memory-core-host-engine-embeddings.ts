...
export function createLocalEmbeddingProvider(..._args: unknown[]): Promise<never> {
  return Promise.reject(
    new Error(
      "The legacy in-process llama.cpp embedding runtime is retired. Run `aether update repair` to install the managed llama-server provider, then restart Aether.",
    ),
  );
}
