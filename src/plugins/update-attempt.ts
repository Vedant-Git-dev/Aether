...
export function formatNewerExactPinnedNpmDefaultLineMessage(params: {
  ...
  return (
    ...
    `Pass \`aether plugins update ${params.newer.packageName}@${params.newer.registryLine}\` to replace this version pin.`
  );
}
...
function formatNewerExactPinnedClawHubDefaultLineMessage(params: {
  ...
  return (
    ...
    `Pass \`aether plugins install clawhub:${params.newer.packageName}${selector} --force\` to replace this version pin.`
  );
}
