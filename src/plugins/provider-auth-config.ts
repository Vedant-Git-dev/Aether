...
export function createProviderAuthConfigPatch(base: AetherConfig, next: AetherConfig) {
...
export async function writeProviderAuthConfig(params: {
  ...
  finalizeConfig?: (next: AetherConfig, current: AetherConfig) => AetherConfig;
  beforeCommit?: () => void;
    ...
    const written = await transformConfigWithPendingPluginInstalls({
      ...
      transform: (current, { snapshot }) => {
        ...
        if (params.configPatch) {
          ...
          next = applyMergePatch(next, params.configPatch, patchOptions) as AetherConfig;
        }
