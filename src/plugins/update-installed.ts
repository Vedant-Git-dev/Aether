...
import { auditDeclaredAetherHostDependency } from "./plugin-peer-link.js";
import {
...
import {
  ...
  repairRegisteredAetherHostLink,
...
export async function updateNpmInstalledPlugins(params: {
  ...
  for (const pluginId of targets) {
    ...
    if (!params.dryRun && record.source === "npm" && currentVersion) {
      changed = (await repairRegisteredAetherHostLink({ pluginId, record, logger })) || changed;
    }
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !(await auditDeclaredAetherHostDependency({
