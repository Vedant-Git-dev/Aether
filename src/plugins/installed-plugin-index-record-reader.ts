...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  ...
  const extensions = aether.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "aether.plugin.json"));
...
function readManagedNpmInstallTimestampMs(params: {
  ...
  // Isolated flat/generation roots have an Aether-owned project manifest that
...
function emitManagedNpmRecoveryFallbackWarning(params: {
  ...
  process.emitWarning(
    `Managed npm recovery found ${params.candidates.length} installs for plugin "${params.pluginId}" without an authoritative active path; selected the most recently installed candidate. Run \`aether vitals --fix\` to persist and retire stale generations.`,
    {
      code: "AETHER_PLUGIN_INSTALL_RECOVERY_FALLBACK",
      type: "AetherPluginRecoveryWarning",
...
function requireLoadablePluginInstallRecordState(
  ...
  if (state.status === "invalid") {
    throw new Error(
      "Persisted plugin install records are invalid. Run aether vitals to inspect and repair plugin installation state.",
    );
  }
