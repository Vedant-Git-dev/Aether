/** Persists hosted official external plugin catalog snapshots in Aether state. */
...
function resolveStoreEnv(
  ...
  return {
    ...
    AETHER_STATE_DIR: options.stateDir,
  };
}
...
function resolveStateDatabasePath(
  ...
  return resolveAetherStateSqlitePath(resolveStoreEnv(options) ?? process.env);
}
...
/** Creates a snapshot store backed by the shared `state/aether.sqlite` database. */
export function createSqliteHostedOfficialExternalPluginCatalogSnapshotStore(
  ...
  return {
    async read(url) {
      ...
      const database = openAetherStateDatabase(resolveStateDatabaseOptions(options));
