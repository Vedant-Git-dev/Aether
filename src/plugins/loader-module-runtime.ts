...
import { installAetherPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
import type { AetherPluginApi, AetherPluginDefinition } from "./types.js";
...
function createGuardedPluginRegistrationApi(api: AetherPluginApi): {
  api: AetherPluginApi;
  close: () => void;
} {
  let closed = false;
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<AetherPluginDefinition["register"]>>[0],
...
export function createPluginModuleLoader(options: {
  ...
  const createLoaderForModule = (modulePath: string) => {
    if (captured.installNativeSdkResolver !== false && captured.tryNative !== false) {
      installAetherPluginSdkNativeResolver({
...
export function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AetherPluginDefinition;
  register?: AetherPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return { register: resolved as AetherPluginDefinition["register"] };
    }
    if (resolved && typeof resolved === "object") {
      const definition = resolved as AetherPluginDefinition;
