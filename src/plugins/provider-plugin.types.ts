...
export type ProviderPlugin = {
   ...
   *
   * Aether calls this only from async model resolution paths. Return the
   ...
   *
   * Runs after Aether performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Aether registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Aether wrappers.
   *
   ...
   *
   * Called after Aether resolves the raw configured credential but before the
   ...
   * hint than Aether's generic auth-store guidance.
   */
   ...
   * Aether's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Aether should surface a provider-specific hint for
   ...
   * merging. Aether deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Aether stores the
   ...
   * without replacing the full Aether-owned system prompt.
   */
   ...
   *
   * Runs after Aether's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Aether's default fallback policy.
   */
   ...
   * rewrites after Aether has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Aether handles
   ...
   *
   * Aether uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that generic auth must ignore and `aether vitals` should remove.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `aether vitals` should offer.
   *
   ...
   *
   * Aether calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Aether's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Keep process/network I/O here; Aether publishes the completed result for this generation.
   */
   ...
   * Aether falls back to that stored profile.
   */
