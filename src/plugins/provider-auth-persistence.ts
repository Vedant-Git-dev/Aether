...
function rollbackStoreWrites(
  ...
  if (errors.length > 0) {
    throw new AggregateError(
      errors,
      "Could not confirm rollback of protected provider credentials; run aether vitals --fix before retrying.",
    );
  }
}
  ...
  try {
    const profiles = params.profiles.map((profile): ProviderAuthProfile => {
      ...
      try {
        ...
        throw new Error(
          "Could not write the protected secret store. Check the Aether state-directory permissions and retry; the auth profile was not changed.",
          { cause: error },
        );
      }
...
function resolvePersistenceEnv(params: {
  ...
  return params.stateDir
    ? { ...(params.env ?? process.env), AETHER_STATE_DIR: params.stateDir }
