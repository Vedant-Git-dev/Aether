...
function collectConfiguredPluginEntryIds(config: AetherConfig): string[] {
...
function resolveRegister(mod: AetherPluginModule): {
...
function resolveSetupRegistration(
  ...
  let mod: AetherPluginModule;
  try {
    mod = getModuleLoader(setupSource, setupArtifact.rootDir)(setupSource) as AetherPluginModule;
  } catch (error) {
  ...
  const resolved = resolveRegister((mod as { default?: AetherPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Aether ignored the plugin setup runtime entry. Remove setup-api/aether.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
