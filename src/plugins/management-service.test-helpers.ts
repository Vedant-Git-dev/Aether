...
export function configSnapshot(config: Record<string, unknown> = {}) {
  return {
    ...
    writeOptions: {
      expectedConfigPath: "/tmp/aether.json",
...
export function metadataSnapshot(params: {
  ...
  const manifest: PluginManifestRecord = {
    ...
    manifestPath: `/tmp/${id}/aether.plugin.json`,
...
export const hostedDiffsEntry = {
  ...
  aether: {
    ...
    install: { clawhubSpec: "clawhub:@aether/diffs", defaultChoice: "clawhub" },
  },
};
...
export const hostedFeedDiffsEntry = {
  ...
  state: "available",
  featured: true,
  publisher: { id: "aether", trust: "official" },
  install: {
    candidates: [
      {
