...
const INSTALL_POLICY_REVIEW_GUIDANCE = [
  ...
  "To continue:",
  "  • Run the matching direct `aether plugins ...` or `aether skills ...` command interactively.",
...
function isPackageRootAetherPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "aether") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootAetherPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedAetherPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootAetherPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostAetherRootRealPath(): Promise<string | null> {
...
function isTrustedHostAetherPath(params: {
  resolvedTargetPath: string;
  trustedHostAetherRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostAetherRootRealPath !== null &&
    isPathInside(params.trustedHostAetherRootRealPath, params.resolvedTargetPath)
  );
}
...
async function inspectNodeModulesSymlinkTarget(params: {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      ...
      isTrustedHostAetherPath({
        resolvedTargetPath,
        trustedHostAetherRootRealPath: params.trustedHostAetherRootRealPath,
      })
    ) {
      return;
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "aether" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectInstalledPackageScanRoots(params: {
  ...
  const trustedHostAetherRootRealPath = await resolveTrustedHostAetherRootRealPath();
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "clawhub":
        return { kind: "clawhub", authority: "aether", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "aether-bundled":
        return { kind: "bundled", authority: "aether", mutable: false, network: false };
      case "aether-managed":
      case "aether-extra":
        return { kind: "managed", authority: "aether", mutable: false, network: false };
      default:
...
function shouldBypassAetherInstallFriction(params: {
  source?: InstallPolicySource;
  ...
  return (
    source.authority === "aether" && (source.kind === "bundled" || source.kind === "managed")
  );
}
...
export async function scanBundleInstallSourceRuntime(
  ...
  if (shouldBypassAetherInstallFriction({ source: params.source })) {
...
export async function scanPackageInstallSourceRuntime(
  ...
  if (
    shouldBypassAetherInstallFriction({
