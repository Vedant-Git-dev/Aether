...
export function createToolHookRegistrars(state: PluginRegistryState) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<AetherPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<AetherPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<AetherPluginApi["registerAgentToolResultMiddleware"]>[1],
  ...
  const registerTool = (
    ...
    const factory: AetherPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: AetherPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: AetherPluginHookOptions | undefined,
    config: AetherPluginApi["config"],
