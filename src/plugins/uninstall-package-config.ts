...
function removeMatchingLoadPaths(
  load: NonNullable<AetherConfig["plugins"]>["load"],
  ownedPaths: readonly string[],
): { load: NonNullable<AetherConfig["plugins"]>["load"] | undefined; changed: boolean } {
...
export function removePluginRuntimePolicyFromConfig(
...
): { config: AetherConfig; actions: PluginConfigUninstallActions } {
  ...
  return {
    config: {
      ...
      channels: channels as AetherConfig["channels"],
    },
    actions,
  };
}
