...
import { loadAetherPlugins } from "./loader.js";
import { pluginLoaderCacheState } from "./registry-lifecycle.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export { loadAetherPlugins };

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadAetherPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadAetherPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "aether-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.AETHER_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.AETHER_DISABLE_BUNDLED_PLUGINS;
...
export function writePluginMetadata(params: {
  ...
  fs.writeFileSync(
    path.join(params.dir, "aether.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function useNoBundledPlugins() {
  ...
  delete process.env.AETHER_BUNDLED_PLUGINS_DIR;
}
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".aether", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ AETHER_STATE_DIR: stateDir, ...params.env }, () =>
    loadAetherPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AETHER_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.AETHER_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.AETHER_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
