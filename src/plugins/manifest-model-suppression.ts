...
import { buildModelCatalogMergeKey } from "@aether/model-catalog-core/model-catalog-refs";
    ...
    return {
      ...
      errorMessage: buildManifestSuppressionError({
        ...
        reason: suppression.retirement
          ? `${suppression.reason ?? "This model has retired."} Run \`aether vitals --fix\` to ${suppression.retirement.replacedBy ? `replace it with ${suppression.retirement.replacedBy}` : "clear the retired override and use the default model"}.`
          : suppression.reason,
      }),
