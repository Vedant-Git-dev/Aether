...
export function formatPluginTrustRefusal(params: {
  ...
  const remedy =
    trust.reason === "provenance-missing"
      ? "Run aether vitals --fix to repair proven legacy official provenance; if it cannot be verified, reinstall from the official npm package or ClawHub listing."
      : trust.reason === "record-missing"
        ? "Compare this registryPath with aether plugins inspect <plugin-id> --json. If the CLI and Gateway state paths differ, align the service environment; otherwise reinstall from the official npm package or ClawHub listing."
