...
type ConfigScopedPromiseLoader<T> = {
  load(config?: AetherConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: AetherConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<AetherConfig, Promise<T>>();

  const createPromise = (config?: AetherConfig): Promise<T> => {
  ...
  const loader: ConfigScopedPromiseLoader<T> = {
    async load(config?: AetherConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<AetherConfig, Promise<T>>();
    },
  };
