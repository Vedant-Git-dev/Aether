...
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_aether-quarantined-npm-projects";
...
export async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  const packageDirs = await listNpmPackageDirs(npmRoot, {
    ...
    includeEntry: (entry, scoped) =>
      (scoped || (entry.name !== ".bin" && entry.name !== "aether")) &&
...
export function resolveRequiredPlatformPackageNames(
  ...
  if (!Array.isArray(raw)) {
    return {
      ok: false,
      error: "package.json aether.install.requiredPlatformPackages must be an array",
    };
  }
  ...
  for (const value of raw) {
    if (typeof value !== "string") {
      return {
        ok: false,
        error:
          "package.json aether.install.requiredPlatformPackages must contain only npm package names",
      };
    }
    ...
    if (specError || !parsed || parsed.selectorKind !== "none") {
      return {
        ok: false,
        error: `package.json aether.install.requiredPlatformPackages contains invalid package name: ${value}`,
      };
    }
