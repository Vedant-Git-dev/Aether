...
import type { AetherPluginApi, PluginRegistrationMode } from "./types.js";
...
export function createCapabilityRegistrars(state: PluginRegistryState) {
  ...
  const registerDetachedTaskRuntime = (
    record: PluginRecord,
    runtime: Parameters<AetherPluginApi["registerDetachedTaskRuntime"]>[0],
  ) => {
  ...
  const registerInteractiveHandler = (
    record: PluginRecord,
    registration: Parameters<AetherPluginApi["registerInteractiveHandler"]>[0],
  ) => {
  ...
  const registerContextEngine = (
    record: PluginRecord,
    id: Parameters<AetherPluginApi["registerContextEngine"]>[0],
    factory: Parameters<AetherPluginApi["registerContextEngine"]>[1],
  ...
  const registerCompactionProvider = (
    record: PluginRecord,
    provider: Parameters<AetherPluginApi["registerCompactionProvider"]>[0],
  ) => {
    const id = normalizeOptionalString(
      (provider as Partial<Parameters<AetherPluginApi["registerCompactionProvider"]>[0]> | null)
        ?.id,
    );
    if (!id) {
