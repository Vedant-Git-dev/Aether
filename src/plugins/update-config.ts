...
import { reconcileRegisteredAetherHostLinks } from "./plugin-peer-link.js";
...
export async function repairRegisteredAetherHostLink(params: {
  pluginId: string;
  record: PluginInstallRecord;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  const result = await reconcileRegisteredAetherHostLinks({
...
export async function repairAetherPeerLinksForNpmInstalls(params: {
  config: AetherConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  const result = await reconcileRegisteredAetherHostLinks({
    ...
    onPackageReadError: (error, packageDir) => {
      params.logger.warn?.(
        `Could not repair aether peer link at ${packageDir}: ${String(error)}`,
      );
    },
  });
  return result.repaired > 0;
