...
import {
  ...
  readAetherManagedNpmRootOverrides,
  repairManagedNpmRootAetherPeer,
...
import {
  ...
  formatUnresolvedAetherPeerLinkError,
...
import {
  auditDeclaredAetherHostDependency,
...
export async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (npmRoot: string): Promise<InstallPluginResult> => {
    ...
    if (params.packageName !== "aether") {
      const repairedAetherPeer = await repairManagedNpmRootAetherPeer({
        npmRoot,
        timeoutMs,
        signal: params.signal,
        logger,
      });
      if (repairedAetherPeer) {
        logger.info?.(`Repaired stale aether peer dependency in ${npmRoot}`);
      }
    }
    const managedOverrides = await readAetherManagedNpmRootOverrides();
    const quarantineForRecovery = async (
      ...
      try {
        ...
        return {
          ok: false,
          error: `${cause.error}, but Aether could not quarantine ${npmRoot} for rebuild: ${String(error)}`,
        };
      }
      logger.warn?.(
    ...
    if (incompletePlatformPackages.length > 0) {
      ...
      try {
        ...
        freshCacheDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-npm-cache-"));
    ...
    if (params.packageName !== "aether") {
      ...
      if (repairedAetherPeer) {
        logger.info?.(`Repaired stale aether peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkAetherPeerDependenciesInManagedNpmRoot({
        npmRoot,
        logger,
      });
    } catch (error) {
      return {
        ok: false,
        error: `Failed to repair aether peer links after npm install: ${String(error)}`,
      };
    }
    if (await auditDeclaredAetherHostDependency({ packageDir: installRoot })) {
      return {
        ok: false,
        error: formatUnresolvedAetherPeerLinkError(params.packageName),
      };
    }
