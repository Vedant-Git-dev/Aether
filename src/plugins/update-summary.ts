...
import {
  ...
  repairAetherPeerLinksForNpmInstalls,
...
export function recordPluginUpdateFailure(params: {
  ...
  if (params.disableOnFailure && !params.dryRun && !preserveInstalledPayload) {
    const message =
      `Disabled "${params.pluginId}" after plugin update failure; Aether will continue without it. ` +
      params.message;
...
export async function finalizePluginUpdateSummary(params: {
  ...
  if (params.ranNpmInstaller) {
    try {
      changed =
        (await repairAetherPeerLinksForNpmInstalls({
