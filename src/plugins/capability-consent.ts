...
function throwManagedPluginCapabilityConsentRequired(
  review: PluginCapabilityConsentReview,
  recovery = `Run "aether plugins enable ${review.pluginId} --accept-capabilities" to accept its capabilities.`,
): never {
...
async function resolvePluginArtifactCapabilityConsent(params: {
  ...
  if (
    ...
    return throwManagedPluginCapabilityConsentRequired(
      finalReview,
      `The plugin was not ${outcome}. Re-run the same "aether plugins install" or "aether plugins update" command with --accept-capabilities, keeping its source and other options. For Doctor or setup, complete the plugin command first, then retry Doctor or setup.`,
    );
  }
