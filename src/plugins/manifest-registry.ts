...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  ...
  packageChannel?: AetherPackageManifest["channel"];
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "aether",
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add aether.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
...
export function loadPluginManifestRegistryCore(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "aether") === "bundle";
    ...
    if (isManifestlessConfiguredFile && isCoreReservedPluginId(candidate.idHint)) {
      diagnostics.push({
        ...
        message: `plugin manifest id "${candidate.idHint}" is reserved by Aether core`,
      });
      continue;
    }
    const manifestRes:
    ...
    if (candidate.origin !== "bundled") {
          ...
          message:
            minHostVersionCheck.kind === "invalid"
              ...
              : minHostVersionCheck.kind === "unknown_host_version"
                ? `plugin requires Aether >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
                : `plugin requires Aether >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
        });
        continue;
      }
      ...
      if (
        ...
        diagnostics.push({
          ...
          pluginId: effectivePluginId,
          source: packageManifestSource,
          message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${currentHostVersion}; skipping load (check "aether --version", AETHER_COMPATIBILITY_HOST_VERSION, or run "aether vitals")`,
        });
        continue;
      }
    }
...
export function loadBundledPluginManifestRegistry(
  ...
  return loadPluginManifestRegistryCore({
    env,
    installRecords,
    discovery: discoverAetherPlugins({
      env,
      installRecords,
      rootScope: "bundled",
