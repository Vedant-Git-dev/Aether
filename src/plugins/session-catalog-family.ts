...
export type SessionCatalogNodeHostBindingsOptions = {
  ...
  executable: string;
  args: (threadId: string) => string[];
  listAvailable: (context: AetherPluginNodeHostCommandAvailabilityContext) => boolean;
  terminalAvailable: (context: AetherPluginNodeHostCommandAvailabilityContext) => boolean;
...
export function createSessionCatalogNodeHostBindings(
  ...
  commands: AetherPluginNodeHostCommand[];
  policies: AetherPluginNodeInvokePolicy[];
} {
  const terminal: AetherPluginNodeHostCommand = {
