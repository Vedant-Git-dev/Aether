...
import { listAetherPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
export function applyProviderDefaultModel(cfg: AetherConfig, modelRef: string): AetherConfig {
...
function buildOpenAICompatibleSelfHostedProviderConfig(params: {
  ...
  maxTokens?: number;
}): { config: AetherConfig; modelRef: string; profileId: string } {
...
function isProviderOwnedSyntheticAuthMarker(
  ...
  return listAetherPluginManifestMetadata().some(
    ({ origin, manifest }) =>
      origin === "bundled" &&
