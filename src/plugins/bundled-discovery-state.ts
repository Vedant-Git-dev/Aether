    ...
    options.path || options.database || !hasActivePluginInstallRoots()
      ...
      : {
          ...
          env: {
            ...
            AETHER_STATE_DIR: resolveActivePluginInstallRoots(options.env).stateDir,
          },
        };
...
function resolveBundledDiscoveryMemoKey(env: NodeJS.ProcessEnv): string {
  const scopedEnv = hasActivePluginInstallRoots()
    ? { ...env, AETHER_STATE_DIR: resolveActivePluginInstallRoots(env).stateDir }
    : env;
  return resolveAetherStateSqlitePath(scopedEnv);
}
