...
export async function installBundledPluginSource(params: {
  ...
  const configWarning = shouldEnable
    ? undefined
    : `Installed bundled plugin "${params.bundledSource.pluginId}" without enabling it because it requires configuration first. Configure it, then run \`aether plugins enable ${params.bundledSource.pluginId}\`.`;
