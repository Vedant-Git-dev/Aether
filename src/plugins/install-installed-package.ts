...
import { linkAetherPeerDependencies } from "./plugin-peer-link.js";
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid aether.plugin.json: ${ocManifestResult.error}`,
  ...
  const compatibilityError = validateAetherPackageInstallCompatibility({
  ...
  const extensionsResult = ensureAetherExtensions({
    manifest,
  });
...
export async function scanAndLinkInstalledPackage(params: {
  ...
  const peerLinkRepair = await linkAetherPeerDependencies({
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedAetherPeerLinkError(params.pluginId),
    };
  }
  return null;
}
