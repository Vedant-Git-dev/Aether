...
export function loadAetherPluginsCore(
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'aether plugins inspect <id> --runtime --json' for runtime diagnostics, 'aether plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
    if (context.shouldActivate) {
