...
import { resolveAetherDevSourceRoot } from "./dev-source-root.js";
...
function resolvePluginLoaderJitiNativeModules(): string[] {
  try {
    ...
    const nativeModules = Array.isArray(configured)
      ...
      : [];
    return [...new Set([...nativeModules, "aether"])];
  } catch {
    return ["aether"];
  }
}
...
function hasTrustedAetherRootIndicator(params: {
  packageRoot: string;
  ...
  const hasAetherBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("aether")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.aether === "string");
  return (facts.trustedRoot =
    hasCliEntryExport ||
    hasAetherBin ||
    pluginCacheExistsSync(path.join(params.packageRoot, "aether.mjs")));
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedAetherRootIndicator({ packageRoot, packageJson: pkg })) {
    return (facts.exportedSubpaths = null);
  }
...
function resolveTrustedAetherRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedAetherRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveAetherDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  return (
    (params.moduleUrl ? resolveAetherPackageRootSync({ moduleUrl: params.moduleUrl }) : null) ??
    resolveAetherPackageRootSync({ cwd }) ??
    resolveTrustedAetherRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
...
const PLUGIN_SDK_PACKAGE_NAMES = ["aether/plugin-sdk", "@aether/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@aether/codex",
  ...
  },
  {
    ...
    officialInstalledPackageName: "@aether/llama-cpp-provider",
    allowPrivateQaCli: false,
    subpaths: [CONFIGURED_LOCAL_ORIGIN_RUNTIME_PLUGIN_SDK_SUBPATH],
  },
];
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.AETHER_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function buildPluginLoaderJitiOptions(
  ...
  return {
    ...
    tryNative: true,
    // When jiti must transform a plugin entry, keep Aether's own package
