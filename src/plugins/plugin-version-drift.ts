...
import {
  ...
  resolveAetherReleaseCohortVersion,
  ...
  if (exactNpmPackageName) {
    if (
      ...
      entry.targetResolution.requestedTarget !==
        resolveAetherReleaseCohortVersion(entry.gatewayVersion)
    ) {
      return undefined;
    }
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `aether plugins update ${exactNpmTarget}`;
    }
    return undefined;
  }
  return `aether plugins update ${entry.pluginId}`;
}
...
async function resolveEntryTarget(
  ...
  const requestedTarget = resolveAetherReleaseCohortVersion(entry.gatewayVersion);
...
function isPluginEnabled(config: AetherConfig | undefined, pluginId: string): boolean {
...
/**
 * Compare active official external plugin installs against an Aether host
 ...
 * @param params.config The merged daemon-side AetherConfig (optional).
...
export function detectPluginVersionDrift(params: {
  ...
  const normalizedGateway = resolveAetherReleaseCohortVersion(gatewayVersion);
  ...
  for (const [pluginId, record] of Object.entries(installRecords)) {
    ...
    if (resolveAetherReleaseCohortVersion(installedVersion) === normalizedGateway) {
      continue;
    }
    drifts.push({
      pluginId,
