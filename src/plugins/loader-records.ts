...
export function createPluginRecord(params: {
  ...
  return {
    ...
    builtWithAetherVersion: params.builtWithAetherVersion,
...
function resolvePluginImportHint(
  ...
  try {
    ...
    const seam =
      /(?:The requested module ['"]aether\/|Package subpath ['"]\.\/)(plugin-sdk\/[\w./-]{1,160})['"] (?:does not provide an export named|is not defined by ["']exports["'] in .*\/aether\/package\.json)/.exec(
        text,
      )?.[1];
    if (seam) {
      const sdkCompatibility = {
        seam: `aether/${seam}`,
        coreVersion: VERSION,
        builtWithAetherVersion: record.builtWithAetherVersion,
        nestedSdk: Boolean(
          record.rootDir &&
          text.includes(
            `${record.rootDir.replaceAll("\\", "/")}/node_modules/aether/package.json`,
          ),
        ),
      };
      // Plugin ids need not be shell-safe; keep unsafe ids out of copy-paste commands.
      const repair = sdkCompatibility.nestedSdk
        ? "this plugin bundles an incompatible Aether SDK; update it or contact its author"
        : /^[a-z0-9_][a-z0-9_.-]*$/i.test(record.id)
          ? `run \`aether plugins update ${record.id}\``
          : "update this plugin or contact its author";
      return {
        hint: `Plugin ${record.id} cannot import ${sdkCompatibility.seam} (built with Aether ${record.builtWithAetherVersion ?? "unknown"}; running core ${VERSION}); ${repair}`,
        sdkCompatibility,
      };
    }
...
export function formatMissingPluginRegisterError(
  ...
  if (parseBooleanValue(env.AETHER_PLUGIN_LOAD_DEBUG) !== true) {
    return message;
  }
