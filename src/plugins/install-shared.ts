...
import { resolvePackageExtensionEntries, type AetherPackageManifest } from "./manifest.js";
...
export function formatUnresolvedAetherPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares an aether dependency, but Aether could not create a plugin-local node_modules/aether link. Run from a packaged Aether install or reinstall Aether, then retry.`;
}

const MISSING_EXTENSIONS_ERROR =
  'package.json missing aether.extensions; update the plugin package to include aether.extensions (for example ["./dist/index.js"]). See https://docs.aether.ai/help/troubleshooting#plugin-install-fails-with-missing-aether-extensions';
function validateAetherPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: AetherPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json aether.compat.pluginApi: ${pluginApiRangeCheck.error}`,
  ...
  if (pluginApiRange && !satisfiesPluginApiRange(params.currentHostVersion, pluginApiRange)) {
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires plugin API ${pluginApiRange}, but this Aether runtime exposes ${params.currentHostVersion}. Upgrade Aether or install a compatible plugin version and retry.`,
...
export function validateAetherPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json aether.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${params.pluginId}" requires Aether >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set AETHER_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${params.pluginId}" requires Aether >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Aether and retry.`,
      code: PLUGIN_INSTALL_ERROR_CODE.INCOMPATIBLE_HOST_VERSION,
    };
  }

  return validateAetherPackageCompatibility({
...
export function ensureAetherExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_AETHER_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json aether.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_AETHER_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_AETHER_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
export async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "aether security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
