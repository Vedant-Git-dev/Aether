...
function normalizePluginApiVersionForComparator(version: string, target: string): string {
  const normalizedCorrection = normalizeAetherNumericCorrectionForPluginApi(version);
  if (normalizedCorrection) {
    return normalizedCorrection;
  }
  return shouldPreservePluginApiPrereleaseFloor(target)
    ? version
    : normalizeAetherReleaseSuffixForPluginApi(version);
}
...
const AETHER_RELEASE_SUFFIX_PATTERN =
  /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)(?:-\d+|-(?:alpha|beta|rc)\.\d+)$/i;
const AETHER_NUMERIC_CORRECTION_PATTERN = /^[vV]?(\d{4}\.[1-9]\d?\.[1-9]\d*)-\d+$/;

function normalizeAetherNumericCorrectionForPluginApi(
  pluginApiVersion: string,
): string | undefined {
  return AETHER_NUMERIC_CORRECTION_PATTERN.exec(pluginApiVersion.trim())?.[1];
}

function normalizeAetherReleaseSuffixForPluginApi(pluginApiVersion: string): string {
  const match = AETHER_RELEASE_SUFFIX_PATTERN.exec(pluginApiVersion.trim());
  return match?.[1] ?? pluginApiVersion;
}

/** Result of reading package.json aether.compat.pluginApi metadata. */
...
export function resolvePackagePluginApiRange(
  ...
  if (!isRecord(compat)) {
    return { ok: false, error: "package.json aether.compat must be an object" };
  }
  ...
  if (typeof pluginApi !== "string") {
    return { ok: false, error: "package.json aether.compat.pluginApi must be a string" };
  }
  const range = pluginApi.trim();
  if (!range) {
    return { ok: false, error: "package.json aether.compat.pluginApi must not be empty" };
  }
