...
type CurrentPluginMetadataSnapshotOptions = {
  config?: AetherConfig;
  compatibleConfigs?: readonly AetherConfig[];
...
type PluginMetadataSnapshotCandidate = {
  ...
  hasConfigIdentity?: (config: AetherConfig) => boolean;
...
const SCOPED_PLUGIN_METADATA_SNAPSHOT_KEY = Symbol.for("aether.scopedPluginMetadataSnapshot");
...
function publishCurrentPluginMetadataSnapshot(
  ...
  const fingerprint = (config: AetherConfig | undefined, policyHash: string | undefined) =>
...
export function withPluginMetadataSnapshotScope<T>(
  ...
  const configIdentities = new WeakSet<AetherConfig>();
  if (options.config) {
    const policyHash = resolveInstalledPluginIndexPolicyHash(options.config, options.env);
    if (
