...
async function detectNativePackageInstallSource(
  ...
  return manifest && ensureAetherExtensions({ manifest }).ok ? manifest : undefined;
}
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains aether.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
