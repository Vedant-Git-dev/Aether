...
import type { AetherPluginCommandDefinition } from "./plugin-command.types.js";
import type {
  AetherPluginChannelRegistration,
  AetherPluginCliRegistrationOptions,
  AetherPluginCliRegistrar,
  AetherGatewayDiscoveryService,
  AetherPluginHostedMediaResolver,
  AetherPluginHttpRouteParams,
  AetherPluginNodeCliFeatureOptions,
...
import type {
  AetherPluginHookOptions,
  AetherPluginToolFactory,
  AetherPluginToolOptions,
} from "./tool-types.js";
...
type AetherPluginSessionStateApi = {
...
type AetherPluginSessionWorkflowApi = {
...
type AetherPluginSessionControlsApi = {
...
type AetherPluginSessionApi = {
  state: AetherPluginSessionStateApi;
  workflow: AetherPluginSessionWorkflowApi;
  controls: AetherPluginSessionControlsApi;
};

type AetherPluginAgentEventsApi = {
...
type AetherPluginAgentApi = {
  events: AetherPluginAgentEventsApi;
};

type AetherPluginRunContextApi = {
 ...
 * @experimental All plugin APIs are experimental. Pin and test Aether host versions.
 * @see https://docs.aether.ai/plugins/sdk-overview#api-stability
 */
export type AetherPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
   ...
   */
  session: AetherPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: AetherPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: AetherPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: AetherPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | AetherPluginToolFactory,
    opts?: AetherPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: AetherPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: AetherPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: AetherPluginHostedMediaResolver) => void;
  ...
  /** Bind a declared MCP server's transport to the trusted message requester. */ registerMcpServerConnectionResolver: (
    resolver: import("./types.mcp-connection.js").AetherPluginMcpServerConnectionResolver,
  ) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: AetherPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: AetherPluginCliRegistrar,
    opts?: AetherPluginCliRegistrationOptions,
  ) => void;
  /**
   * Register a plugin-owned node feature command group under `aether nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: AetherPluginCliRegistrar,
    opts?: AetherPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: AetherPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: AetherPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: AetherPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: AetherPluginSecurityAuditCollector) => void;
  registerService: (service: AetherPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: AetherGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `aether migrate` (migration capability). */
   ...
   */
  registerCommand: (command: AetherPluginCommandDefinition) => void;
