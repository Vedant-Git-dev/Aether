...
import type { AetherPluginCommandDefinition } from "./types.js";
...
type PluginCommandEntrySpec = {
  ...
  acceptsArgs: boolean;
  nativeName?: string;
  clientPresentation?: NonNullable<AetherPluginCommandDefinition["clientPresentation"]>;
};

function resolvePluginTextName(command: AetherPluginCommandDefinition): string {
...
function serializePluginCommandSpec(
  cmd: AetherPluginCommandDefinition,
  provider?: string,
): {
  name: string;
  description: string;
