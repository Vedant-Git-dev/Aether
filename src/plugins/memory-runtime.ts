...
function resolveMemoryRuntimePluginIds(config: AetherConfig): string[] {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: AetherConfig; agentId: string }) {
  const owner = ensureMemoryRuntime(params);
  return owner
...
export async function closeActiveMemorySearchManagersCore(cfg?: AetherConfig): Promise<void> {
  void cfg;
  await Promise.all(
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("aether.memoryRuntimeTestApi")] = {
