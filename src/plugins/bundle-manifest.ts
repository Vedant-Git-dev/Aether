/** Reads Agent/Codex/Claude/Cursor bundle manifests into Aether plugin manifest metadata. */
import path from "node:path";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalString,
} from "@aether/normalization-core/string-coerce";
import { normalizeUniqueSingleOrTrimmedStringList } from "@aether/normalization-core/string-normalization";
...
const AGENT_BUNDLE_EXTENSION_NAMESPACE = "ai.aether";
...
type BundlePluginManifest = {
  ...
  // Only include hook roots that Aether can execute via HOOK.md + handler files.
  hooks: string[];
...
function resolveAgentActivation(
  ...
  const aetherExtension = raw.extensions[AGENT_BUNDLE_EXTENSION_NAMESPACE];
  if (aetherExtension === undefined) {
    return undefined;
  }
  if (!isRecord(aetherExtension)) {
    log.warn(
      `ignoring Agent Plugins ${AGENT_BUNDLE_EXTENSION_NAMESPACE} extension in ${manifestPath}: expected an object`,
    );
    return undefined;
  }
  return normalizeManifestActivation(aetherExtension.activation);
}
...
export function detectBundleManifestFormat(
  ...
  // Client-specific bundle dirs and native Aether manifests take precedence;
