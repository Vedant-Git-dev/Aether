...
function requiresDerivedRegistryValidation(
  ...
  return (
    ...
    resolveAetherDevSourceRoot(env) !== null ||
...
export function loadPluginRegistrySnapshotWithMetadata(
  ...
  if (!persistedIndex) {
    ...
    diagnostics.push({
      ...
      message:
        "Persisted plugin registry policy does not match current config; using derived plugin index. Run `aether plugins registry --refresh` to update the persisted registry.",
    });
  ...
  if (persistedIndex && contentMatches) {
    ...
    diagnostics.push({
      ...
      message:
        "Persisted plugin registry no longer matches current plugin discovery or metadata; using derived plugin index. Run `aether plugins registry --refresh` to update the persisted registry.",
