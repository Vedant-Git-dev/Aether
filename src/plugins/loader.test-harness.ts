...
import { loadAetherPlugins, type PluginLoadOptions } from "./loader.js";
import {
...
export function updatePluginManifest(
  ...
  const manifestPath = path.join(plugin.dir, "aether.plugin.json");
...
export function setupBundledDreamingMemoryPlugins(params?: {
  ...
  process.env.AETHER_BUNDLED_PLUGINS_DIR = bundledDir;
...
export function writeBundledPlugin(params: {
  ...
  delete process.env.AETHER_DISABLE_BUNDLED_PLUGINS;
  process.env.AETHER_BUNDLED_PLUGINS_DIR = bundledDir;
  return { bundledDir, plugin };
}

export function makeAetherDevSourceRoot() {
  const root = makePluginLoaderTempDir();
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "aether" }), "utf-8");
...
export function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".aether", "extensions", params.id);
...
export function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ AETHER_STATE_DIR: stateDir }, () => run(stateDir));
}
...
export function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.AETHER_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadAetherPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          aether: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
export function setupBundledTelegramPlugin() {
  ...
  process.env.AETHER_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

export function expectTelegramLoaded(registry: ReturnType<typeof loadAetherPlugins>) {
...
export function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadAetherPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
export function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadAetherPlugins>[0], "cache" | "config">,
) {
  return loadAetherPlugins({
    cache: false,
    ...options,
  ...
  expect(
    params.warnings.some(
      (msg) =>
        ...
        msg.includes("Aether can't verify where this plugin came from"),
    ),
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "aether.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadAetherPlugins>,
  pluginId: string,
) {
...
export function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadAetherPlugins>;
  loadSecond: () => ReturnType<typeof loadAetherPlugins>;
...
export function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadAetherPlugins>;
  loadVariant: () => ReturnType<typeof loadAetherPlugins>;
}) {
  const first = params.loadFirst();
...
export function createSetupEntryChannelPluginFixture(params: {
  ...
  writeFixtureJson(pluginDir, "package.json", {
    ...
    aether: { extensions: ["./index.cjs"], setupEntry: "./setup-entry.cjs" },
  });
  writeFixtureJson(pluginDir, "aether.plugin.json", pluginManifest(params.id, [params.id]));
  fs.writeFileSync(
...
export function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const aetherHome = makePluginLoaderTempDir();
  const ignoredHome = makePluginLoaderTempDir();
  const stateDir = makePluginLoaderTempDir();
  const pluginDir = path.join(aetherHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...
    HOME: ignoredHome,
    AETHER_STATE_DIR: stateDir,
    AETHER_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
export function expectEscapingEntryRejected(params: {
  ...
  const registry = loadAetherPlugins({
    cache: false,
    config: {
      plugins: {
