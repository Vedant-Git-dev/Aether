...
import type { AetherPluginModule, PluginLogger } from "./types.js";
...
export function loadRuntimePluginCandidate(params: {
  ...
  let mod: AetherPluginModule | null = null;
  ...
  try {
    ...
    mod = withProfile(
      ...
      () => params.loadPluginModule(safeSource) as AetherPluginModule,
    );
  } catch (error) {
    recordPluginError({
...
function recordBundleDiagnostics(params: {
  ...
  for (const capability of unsupportedCapabilities) {
    params.registry.diagnostics.push({
      ...
      message: `bundle capability detected but not wired into Aether yet: ${capability}`,
    });
  }
  if (
