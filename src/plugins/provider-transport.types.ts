...
/**
 * Provider-owned stream wrapper hook after Aether applies its generic
 ...
 *
 * Return `true` or `false` to override Aether's built-in provider cache TTL
 ...
 *
 * Runs only after Aether exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Aether should append to the generic `Unknown model`
 * error.
 */
