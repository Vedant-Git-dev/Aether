...
const MIN_HOST_VERSION_FORMAT =
  'aether.install.minHostVersion must use a semver floor in the form ">=x.y.z[-prerelease][+build]"';
...
export function checkMinHostVersion(params: {
  ...
  const comparison = compareAetherVersions(currentVersion, requirement.minimumLabel);
  if (comparison === null) {
    return {
      ok: false,
