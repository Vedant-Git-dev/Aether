...
import { ok, err, type Result } from "@aether/normalization-core/result";
...
export type PreparedPluginUninstall = {
  ...
  planForConfig: (config: AetherConfig) => ReturnType<typeof planPluginUninstall>;
};
...
export async function preparePluginUninstall(
  ...
  const planForConfig = (source: AetherConfig) =>
    planPluginUninstall(
      recordPluginPackageUninstallPlan(
        {
