...
import type { AetherPluginCliRootCommandDescriptor, PluginLogger } from "./types.js";
...
export async function getPluginCliCommandDescriptors(
  ...
  const descriptorGroups: AetherPluginCliRootCommandDescriptor[][] = [];
  try {
    ...
    if (legacyExternalPluginIds.length > 0) {
      const { loadAetherPluginCliRegistry } = await import("./loader.js");
      const registry = await loadAetherPluginCliRegistry(
