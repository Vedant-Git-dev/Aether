...
/** Env var that points bundled-plugin lookup at an Aether source checkout. */
const AETHER_DEV_SOURCE_ROOT_ENV = "AETHER_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured Aether development source root. */
export function resolveAetherDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[AETHER_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  const valid =
    realRoot &&
    readPackageName(path.join(realRoot, "package.json")) === "aether" &&
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveAetherDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }
