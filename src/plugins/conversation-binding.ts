...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "aether-app-server:thread:",
  "aether-codex-app-server:thread:",
] as const;
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Aether instead. If this started after an update, run "aether vitals --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
