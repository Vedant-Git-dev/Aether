...
import { safeParseJson } from "@aether/normalization-core/json-coercion";
import {
...
type InstalledPluginIndexDatabase = Pick<AetherStateKyselyDatabase, "config_machine_state">;
...
function assertWritableInstalledPluginIndexStoreOptions(
  ...
  if (options.filePath?.endsWith(".json")) {
    throw new Error(
      "Explicit JSON installed plugin index paths are retired. Use the shared SQLite state DB or run aether vitals --fix to migrate legacy plugins/installs.json.",
    );
  }
}
...
function writePersistedInstalledPluginIndexToSqlite(
  ...
  return runAetherStateWriteTransaction(({ db, path: databasePath }) => {
...
export async function restorePersistedInstalledPluginIndexIfCurrent(
  ...
  const restored = runAetherStateWriteTransaction(({ db }) => {
    lease.assertOwnedInTransaction(db);
...
function resolveRefreshedPersistedInstalledPluginIndex(
  ...
  if (params.reason === "manual" && !params.installRecords) {
    ...
    if (foreignPluginIds.length > 0) {
      throw new Error(
        `Plugin registry refresh cannot verify npm install ownership outside the selected state directory: ${foreignPluginIds.join(", ")}. Reinstall copied plugins in this state directory, then run \`aether plugins registry --refresh\` again.`,
      );
    }
  }
