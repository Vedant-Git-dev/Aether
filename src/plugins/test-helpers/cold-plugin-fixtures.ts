...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/aether-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "aether.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): AetherConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    AETHER_HOME: path.join(homeDir, "home"),
    AETHER_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    AETHER_VERSION: "2026.4.25",
    VITEST: "true",
  };
}
