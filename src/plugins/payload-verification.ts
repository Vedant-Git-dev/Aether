...
import {
  auditAetherPeerDependencyLink,
  resolveAetherHostDependency,
...
async function validatePackagePayload(params: {
  ...
  const hostDependency = resolveAetherHostDependency(params.manifest);
  ...
  if (
    ...
    const peerIssue = await auditAetherPeerDependencyLink({
    ...
    if (peerIssue) {
      failures.push({
        pluginId: params.pluginId,
        installPath: params.installPath,
        reason: "missing-aether-peer-link",
    ...
    failures.push({
      ...
      detail: `Plugin extension entry validation failed: ${
        extensionResolution.status === "invalid"
          ...
          : "package.json aether.extensions is empty"
      }`,
    });
    return failures;
