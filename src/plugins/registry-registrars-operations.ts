...
import type {
  ...
  AetherPluginCliRootCommandDescriptor,
  AetherPluginCommandDefinition,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@aether/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@aether/codex");
}
...
export function createOperationRegistrars(state: PluginRegistryState) {
    ...
    const descriptors = (opts?.descriptors ?? [])
      .map((descriptor) => {
        ...
        const machineOutput = rootRegistration
          ? (descriptor as AetherPluginCliRootCommandDescriptor).machineOutput
          : undefined;
        if (!name || !description) {
          return null;
        }
        const normalized: AetherPluginCliRootCommandDescriptor = {
          name,
          description,
      ...
      .filter(
        (descriptor): descriptor is AetherPluginCliRootCommandDescriptor => descriptor !== null,
      );
  ...
  const registerReload = (record: PluginRecord, registration: AetherPluginReloadRegistration) => {
    const normalized: AetherPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: AetherPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: AetherPluginNodeInvokePolicy,
  ...
  const registerService = (record: PluginRecord, service: AetherPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: AetherPluginCommandDefinition) => {
