...
function isTrustedSourceLinkedOfficialPackage(pkg: NonNullable<ClawHubPackageDetail["package"]>) {
  ...
  return (
    ...
    (sourceRepo === "aether/aether" ||
      sourceRepo === "github.com/aether/aether" ||
      sourceRepo === "https://github.com/aether/aether")
  );
}
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    ...
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "aether skills install ${installRef}" instead.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Aether runtime exposes ${runtimeVersion}.`,
  ...
  if (minGatewayVersion) {
    ...
    if (minGatewayVersionCheck.kind === "unknown_host_version") {
      return buildClawHubInstallFailure(
        `Plugin "${pkg.name}" requires Aether >=${minGatewayVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set AETHER_VERSION and retry.`,
        CLAWHUB_INSTALL_ERROR_CODE.UNKNOWN_GATEWAY_VERSION,
      );
    }
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Aether >=${minGatewayVersionCheck.requirement.minimumLabel}, but this host is ${minGatewayVersionCheck.currentVersion}.`,
...
> {
  ...
  try {
    ...
    const clawhubAuthority = isDefaultClawHubBaseUrl(params.baseUrl) ? "aether" : "third-party";
    params.logger?.info?.(
