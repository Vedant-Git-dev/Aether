...
function formatPluginInspectCommand(pluginId: string): string {
  return `aether plugins inspect ${quoteCliArg(pluginId)}`;
}
...
export function warnWhenAllowlistIsOpen(params: {
  ...
  const remediation = truncated
    ? "Run 'aether plugins list --enabled --verbose' to enumerate every discovered plugin id, inspect trusted ids with 'aether plugins inspect <id>', and add the ones you trust to plugins.allow in aether.json."
    : `To trust them explicitly, set plugins.allow in aether.json (e.g. "plugins": { "allow": [${autoDiscoverable
        .map((entry) => JSON.stringify(entry.id))
        .join(
          ", ",
        )}] }). Run 'aether plugins list --enabled --verbose' or ${inspectCommands} to confirm plugin ids.`;
