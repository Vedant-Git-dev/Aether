...
import { normalizeOptionalString as normalizeString } from "@aether/normalization-core/string-coerce";
...
function getConfigPath(config: AetherConfig | undefined, path: string): unknown {
...
function setConfigPath(target: AetherConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: AetherConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: AetherConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: AetherConfig) =>
