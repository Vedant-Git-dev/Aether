...
import { sortUniqueStrings } from "@aether/normalization-core/string-normalization";
...
function pushInvalidPackageExtensionDiagnostic(params: {
  ...
  if (params.resolution.status === "empty") {
    params.diagnostics.push({
      level: "error",
      source: params.source,
      message: "package.json aether.extensions is empty",
      ...(params.pluginId ? { pluginId: params.pluginId } : {}),
    });
    return true;
  }
...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "aether vitals --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
...
function shouldSkipIncompatiblePackagePluginApi(params: {
  origin: PluginOrigin;
  packageManifest: AetherPackageManifest | undefined;
  pluginId: string;
  packageDir: string;
  env: NodeJS.ProcessEnv;
  diagnostics: PluginDiagnostic[];
  ...
  if (!packagePluginApiRangeCheck.ok) {
    params.diagnostics.push({
      ...
      message: `invalid package plugin API metadata: ${packagePluginApiRangeCheck.error}; skipping discovery (check package.json aether.compat.pluginApi)`,
  ...
  params.diagnostics.push({
    level: "warn",
    source: path.join(params.packageDir, "package.json"),
    message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${compatibilityHostVersion}; skipping discovery (check "aether --version", AETHER_COMPATIBILITY_HOST_VERSION, or run "aether vitals")`,
    pluginId: params.pluginId,
  });
  return true;
}
...
function discoveryPolicy(
  ...
  return {
    ...
    sourceOverlaysDisabled: env.AETHER_DISABLE_BUNDLED_SOURCE_OVERLAYS ?? "",
    home: env.AETHER_HOME ?? "",
    userHome: env.HOME ?? "",
...
export function discoverAetherPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
    ...
    tracePluginLifecyclePhase(
      ...
      () => {
        ...
        if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
          // Keep workspace auto-discovery constrained to the Aether extensions root.
