// Shared policy and messaging for installs outside Aether's trusted plugin sources.
...
export function resolveAetherTrustedNpmPackageInstall(
  npmSpec: string,
...
export function isAetherTrustedPluginInstallSpec(
  spec: string,
  bundledSources: ReadonlyMap<string, BundledPluginSource> = getProcessBundledPluginSources(),
): boolean {
  ...
  if (explicitNpm) {
    return resolveAetherTrustedNpmPackageInstall(npmSpec, bundledSources) !== null;
  }
  ...
  return Boolean(
    bundled ??
    resolveAetherTrustedNpmPackageInstall(npmSpec, bundledSources) ??
