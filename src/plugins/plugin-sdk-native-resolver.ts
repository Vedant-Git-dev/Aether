/** Installs native Node resolution aliases so plugins can import the Aether SDK in dev and tests. */
...
type InstallAetherPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const INTERNAL_CORE_PACKAGE_ALIASES = [
  {
    packageName: "@aether/markdown-core",
...
function resolveLoaderModulePath(options: InstallAetherPluginSdkNativeResolverOptions): string {
...
function resolveLoaderPackageRootFromModulePath(modulePath: string): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (pluginCacheExistsSync(packageJsonPath)) {
        ...
        try {
          ...
          facts.nativePackage = isRecord(parsed)
            ? {
                ...
                hasAetherBin: isRecord(parsed.bin) && typeof parsed.bin.aether === "string",
              }
            : null;
        } catch {
      ...
      if (facts.nativePackage?.name === "aether" || facts.nativePackage?.hasAetherBin) {
        roots.set(normalizedModulePath, cursor);
        return cursor;
      }
    }
...
function resolveAllowedParentRoots(
  options: InstallAetherPluginSdkNativeResolverOptions,
): string[] {
...
export function installAetherPluginSdkNativeResolver(
  options: InstallAetherPluginSdkNativeResolverOptions = {},
): void {
...
export function installAetherInternalCorePackageNativeResolver(
  options: Pick<InstallAetherPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  registerInternalCorePackageNativeAliases(options);
  installResolver();
