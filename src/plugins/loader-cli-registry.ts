...
import type { AetherPluginModule } from "./types.js";

export async function loadAetherPluginCliRegistry(
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: AetherPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
        () => loadPluginModule(safeSource) as AetherPluginModule,
      );
    } catch (error) {
      recordPluginError({
        logger,
