...
import { discoverAetherPlugins } from "./discovery.js";
...
function addInstalledPluginToAllowlist(cfg: AetherConfig, pluginId: string): AetherConfig {
...
function removeInstalledPluginFromDenylist(cfg: AetherConfig, pluginId: string): AetherConfig {
...
function logShadowedNpmInstallWarning(params: {
  ...
  params.warn(
    [
      ...
      "Run `aether plugins doctor` for repair options.",
    ].join("\n"),
    `Installed plugin "${params.pluginId}" is shadowed by a configured plugin source. Run \`aether plugins doctor\`.`,
  );
}
...
export function prepareConfigForDisabledInstall(cfg: AetherConfig, id: string): AetherConfig {
...
export async function persistPluginInstall(params: {
  ...
  try {
    return await withPluginCache(createPluginCache(), async () => {
      ...
      const installedDiscovery = discoverAetherPlugins({ installRecords: nextInstallRecords });
      ...
      if (manifests.length === 0) {
        throw new Error(
          `Plugin package "${params.pluginId}" has no authoritative runtime child list. Refresh the plugin registry, then reinstall the package or run aether vitals before retrying.`,
        );
      }
      ...
      if (replacedInstallRemoval) {
        ...
        for (const warning of removalResult.warnings) {
          warn(
            warning,
            "A previous plugin installation could not be fully cleaned up. Run `aether plugins doctor`.",
          );
        }
      ...
      const configWarning =
        params.enable !== false && configurationRequiredPluginIds.length > 0
          ? configurationRequiredPluginIds.length === 1
            ? `Installed plugin "${configurationRequiredPluginIds[0]}" without enabling it because it requires configuration first. Configure it, then run \`aether plugins enable ${configurationRequiredPluginIds[0]}\`.`
            : `Installed plugin entries ${configurationRequiredPluginIds.join(", ")} without enabling them because they require configuration first. Configure each entry, then run \`aether plugins enable <plugin-id>\`.`
          : undefined;
      const warningMessage = [params.warningMessage, configWarning].filter(Boolean).join("\n");
      if (warningMessage) {
        warn(
          warningMessage,
          configWarning ?? "Plugin installation reported a warning. Run `aether plugins doctor`.",
        );
      }
      runtime.log(
