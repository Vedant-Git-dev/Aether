...
import { installAetherInternalCorePackageNativeResolver } from "./plugin-sdk-native-resolver.js";
import {
...
type ResolvePluginModuleLoaderCacheEntryParams = {
  ...
  transformAetherDependencies?: boolean;
};
...
function resolvePluginModuleLoaderCacheEntry(
  ...
  const transformAetherDependencies = params.transformAetherDependencies ?? tryNative;
  const cacheKey = `${moduleConfigCacheKey}\0transform-aether=${transformAetherDependencies ? "1" : "0"}`;
  ...
  return () => {
    ...
    const jitiLoader = (params.createLoader ?? loadCreateJitiLoaderFactory())(
      params.loaderFilename,
      {
        ...
        virtualModules: params.transformAetherDependencies
          ? undefined
        ...
        nativeModules: params.transformAetherDependencies
          ? jitiOptions.nativeModules.filter((moduleName) => moduleName !== "aether")
...
function createPluginModuleLoader(params: {
  ...
  // fallback must transform both the entry and Aether SDK dependencies.
...
export function getCachedPluginModuleLoader(
  ...
  installAetherInternalCorePackageNativeResolver({ moduleUrl: params.importerUrl });
  retainModuleLifecycle(cache);
  const loader = createPluginModuleLoader({
    ...
    transformAetherDependencies: cacheEntry.transformAetherDependencies,
