...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "aether-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "AETHER_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.AETHER_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return readPluginCacheDirectory(pluginsDir).some((entry) => {
      ...
      return (
        ...
        pluginCacheExistsSync(path.join(pluginDir, "aether.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveAetherPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveAetherPackageRootSync({ moduleUrl: import.meta.url });
...
export function resolveSourceCheckoutDependencyDiagnostic(
  ...
  for (const packageRoot of resolvePackageRootsForBundledPlugins()) {
    ...
    return {
      source: packageRoot,
      message:
        "Aether source checkout detected without pnpm workspace dependencies; run `pnpm install` from the repo root so bundled plugins can load package-local dependencies.",
    };
  }
  return null;
}
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveAetherPackageRootSync({ moduleUrl: import.meta.url });
  if (
    !modulePackageRoot ||
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.AETHER_BUNDLED_PLUGINS_DIR?.trim();
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = disabled ? undefined : env.AETHER_BUNDLED_PLUGINS_DIR?.trim();
