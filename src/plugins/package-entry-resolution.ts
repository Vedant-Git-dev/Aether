...
function runtimeExtensionsLengthMismatchMessage(params: {
  ...
  return (
    `package.json aether.runtimeExtensions length (${params.runtimeExtensionsLength}) ` +
    `must match aether.extensions length (${params.extensionsLength})`
  );
}
...
function resolvePackageRuntimeExtensionEntries(params: {
  ...
  const runtimeExtensionsResult = readPackageManifestStringList({
    fieldName: "aether.runtimeExtensions",
...
export async function validatePackageExtensionEntriesForInstall(params: {
  ...
  if (runtimeSetupEntry && !setupEntry) {
    return {
      ok: false,
      error: "package.json aether.runtimeSetupEntry requires aether.setupEntry",
    };
  }
  if (setupEntry) {
