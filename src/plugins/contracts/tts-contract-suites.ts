...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "aether/plugin-sdk/speech-core";
...
type TtsRuntimeModule = typeof import("aether/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("aether/plugin-sdk/speech-core");
...
function asLegacyTtsConfig(value: unknown): AetherConfig {
  return value as AetherConfig;
}

function asLegacyAetherConfig(value: Record<string, unknown>): AetherConfig {
...
function createResolvedSummarizationConfig(cfg: AetherConfig): ResolvedTtsConfig {
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyAetherConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: AetherConfig = asLegacyAetherConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      tts: {
        auto: "inbound",
        provider: "test-speech",
    ...
    async function expectAutoTtsOutcome(params: {
      ...
      await withEnvAsync({ AETHER_TTS_PREFS: `/tmp/tts-test-${Date.now()}.json` }, async () => {
