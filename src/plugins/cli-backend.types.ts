...
export type CliBackendConfig = {
  ...
  modelArg?: string;
  /** Model aliases mapping (Aether model id → CLI model id). */
...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Aether consumes it. */
...
export type CliBackendPrepareExecutionContext = {
  ...
  /** Effective Aether context budget selected for this run. */
  contextTokenBudget?: number;
  /** Effective Aether thinking level selected for this run. */
...
/** Exact backend-native plus canonical Aether tool surface for one CLI run. */
export type CliBackendToolAvailability = {
  native: readonly string[];
  /** Canonical Aether tool names served through the host-isolated transport. */
  aether: readonly string[];
};
...
type CliBackendPluginBase = {
  ...
  /**
   * Whether Aether should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Aether's bundle
   ...
   *
   * Use this when the backend should consume a canonical Aether auth profile
   ...
   * `profile-only` treats the selected Aether auth profile as the sole auth
   ...
   * hooks that do not consume Aether auth profiles.
   */
   ...
   * as static config, such as mapping Aether thinking levels to a backend's
   * native effort flag.
   */
   ...
   *
   * Tool events report execution already performed by the backend. Aether
  ...
  /**
   * Whether this CLI backend can expose native tools outside Aether's tool
