...
export { isRecord } from "@aether/normalization-core/record-coerce";
...
export { sliceUtf16Safe, truncateUtf16Safe } from "@aether/normalization-core/utf16-slice";

/** Resolves the Aether config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const configPath = env.AETHER_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  return path.join(resolveRequiredHomeDir(env, homedir), ".aether");
}

/** Resolves the effective Aether home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.AETHER_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$AETHER_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$AETHER_HOME`. */
