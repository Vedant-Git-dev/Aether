...
import { ensureAetherAgentBoardSchemaInTransaction } from "../state/aether-agent-board-schema.js";
...
>;
type BoardDatabaseHandle = Pick<AetherAgentDatabase, "db" | "path">;
...
// not "no such table".
function boardTablesPresent(database: Pick<AetherAgentDatabase, "db">): boolean {
...
function ensureBoardSchema(database: AetherAgentDatabase): void {
  ...
  runSqliteImmediateTransactionSync(
    database.db,
    () => ensureAetherAgentBoardSchemaInTransaction(database.db),
    {
...
export class SqliteBoardStore implements BoardStore {
  ...
  private prepareWrite(target: BoardSessionTarget): {
    ...
    const database = openAetherAgentDatabase({
      agentId: resolved.agentId,
      ...(resolved.path ? { path: resolved.path } : {}),
      env: this.options.env,
    });
