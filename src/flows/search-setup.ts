...
type SearchConfig = NonNullable<NonNullable<NonNullable<AetherConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.aether.ai/tools/web";
...
function defaultModelUsesCodexRuntime(config: AetherConfig): boolean {
...
function rawKeyValue(config: AetherConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: AetherConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: AetherConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: AetherConfig, result: AetherConfig): AetherConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: AetherConfig = {
    ...result,
    tools: {
      ...result.tools,
...
type SearchSetupResult =
  | { outcome: "completed"; config: AetherConfig }
  | {
...
function completedSearchSetup(config: AetherConfig): SearchSetupResult {
  return { outcome: "completed", config };
}

async function finalizeSearchProviderSetup(params: {
  originalConfig: AetherConfig;
...
export async function runSearchSetupFlow(
  ...
  const finalizeSelection = (nextConfig: AetherConfig) =>
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Aether will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.aether.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Aether will store a reference instead of the API key.",
        `Env var: ${ref.id}${envAvailable ? " (detected)" : ""}.`,
        ...(envAvailable ? [] : [`Set ${ref.id} in the Gateway environment.`]),
        "Docs: https://docs.aether.ai/tools/web",
      ].join("\n"),
      "Web search",
    );
