...
function shouldSkipLegacyUpdateDoctorConfigWrite(env: NodeJS.ProcessEnv): boolean {
  return (
    isExplicitOptOutEnvValue(env.AETHER_UPDATE_IN_PROGRESS) &&
    ...
    try {
      ...
      if (isConfigIncludeOwnershipError(error)) {
        ...
        note(
          [
            ...
            `${unpersistedLine} Repair ${error.ownedConfigPath} in ${includedFile} by hand, then rerun "aether vitals --fix" for the remaining changes.`,
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      if (isConfigValidationFailedError(error)) {
        ...
        note(
          [
            ...
            ...issueLines,
            `${unpersistedLine} Fix the value(s) above in ${shortenHomePath(ctx.configPath)} by hand, then rerun "aether vitals --fix".`,
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      note(
        [
          ...
          'Resolve the reported Gateway or cron-store condition, then rerun "aether vitals --fix".',
        ].join("\n"),
        "Doctor warnings",
      );
...
export async function collectWriteConfigHealthFindings(
  ...
  if (resolveIsConfigReadOnly(process.env)) {
    findings.push({
      ...
      severity: "warning",
      message: isNixMode
        ? "Doctor config writes are disabled because Aether is running in Nix mode."
