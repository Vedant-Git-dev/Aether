...
type PromptDefaultModelResult = { model?: string; config?: AetherConfig };
...
function resolveConfiguredModelRaw(cfg: AetherConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AetherConfig): string[] {
...
function resolveModelPickerConfig(cfg: AetherConfig, agentId?: string): AetherConfig {
  if (!agentId) {
    return cfg;
  }
...
function createModelRouteRuntimeResolver(params: {
  ...
  const cache = new Map<string, "codex" | "aether" | undefined>();
  return (route) => {
    const baseUrlKey =
    ...
    const runtime =
      policy.runtime === "codex" ? "codex" : policy.runtime === "aether" ? "aether" : undefined;
