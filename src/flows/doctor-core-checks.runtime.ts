...
export function detectUnavailableSkills(cfg: AetherConfig, workspaceDir: string) {
    ...
    if (
      ...
      return [
        warning(
          ...
          "Rerun `aether vitals --lint --only core/doctor/gateway-health --allow-exec` to permit configured secret execution.",
        ),
      ];
    }
    ...
    if (!probeDetails) {
      return [
        warning(
          ...
          "Fix Gateway connection configuration, then rerun `aether vitals --lint --only core/doctor/gateway-health`.",
        ),
      ];
    }
        ...
        : {
            ...
            fixHint:
              mode === "remote"
                ...
                : "Inspect the service with `aether gateway status --deep`, or run `aether vitals` for guided checks.",
          };
...
export async function collectGatewayDaemonFindings(
  ...
  if (state.loadState.status === "unknown") {
    findings.push({
      ...
      fixHint: "Run `aether gateway status --deep`, restore service-manager access, and retry.",
    });
    return findings;
  }
  if (!state.installed) {
    findings.push({
      ...
      path: "gateway.mode",
      target: service.label,
      fixHint: "Run `aether gateway install` to install the service.",
    });
    return findings;
  }
          ...
          ? {
              fixHint: [
                ...(runtime.status === "unsupported"
                  ...
                  : []),
                "Repair the Node runtime, then run `aether gateway install`.",
              ].join("\n"),
            }
          : {}),
      });
  ...
  if (state.loadState.status === "not-loaded") {
    findings.push({
      ...
      path: state.command?.sourcePath,
      target: service.label,
      fixHint: "Start the installed service with `aether gateway start`.",
    });
  }
  ...
  if (state.loadState.status === "loaded" && !state.running) {
    findings.push({
      ...
      target: service.label,
      fixHint:
        "Run `aether gateway status --deep` to inspect the service before choosing a recovery action.",
    });
  }
...
async function collectAgentRuntimeToolSchemaFindings(params: {
  ...
  try {
    const { createAetherCodingTools } = await import("../agents/agent-tools.js");
    tools = createAetherCodingTools({
      agentId: params.agentId,
      workspaceDir: params.workspaceDir,
      config: params.cfg,
      modelProvider: params.modelRef.provider,
...
function isAcpRuntimeAgent(cfg: AetherConfig, agentId: string): boolean {
