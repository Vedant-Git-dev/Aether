...
async function assertDoctorDatabaseSchemasCompatible(scope?: "state") {
  const [databasePreflight, agentDatabase, stateDatabase] = await Promise.all([
    ...
    import("../state/aether-agent-db-contract.js"),
  ...
  const databaseSchemas = await databasePreflight.preflightAetherDatabaseSchemas({
    env: process.env,
    scope,
  ...
  if (databaseSchemas.incompatible.length > 0) {
    throw new databasePreflight.AetherDatabaseSchemaPreflightError(databaseSchemas.incompatible, {
      operation: "doctor",
    });
  }
...
async function runDoctorHealthFlowWithResult(
  ...
  const { resolveAetherPackageRoot } = await import("../infra/aether-root.js");
  const root = await resolveAetherPackageRoot({
  ...
  try {
    ...
    if (options.repair === true || options.yes === true) {
      ...
      const { assertAetherDatabasesReady } =
