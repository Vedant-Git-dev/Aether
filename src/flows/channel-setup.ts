    ...
    try {
      if (!snapshot.exists || !snapshot.valid) {
        const reason = snapshot.exists
          ...
          : "file not found";
        throw new Error(
          `Saved config is unavailable: ${reason}. Run aether vitals --fix, then retry setup.`,
        );
      }
...
export async function setupChannels(
    ...
    if (disabledHint) {
      await prompter.note(
        t("wizard.channels.disabledDuringSetup", {
          channel,
          hint: disabledHint,
          command: formatCliCommand("aether channels add"),
        }),
    ...
    if (!result.enabled) {
      await prompter.note(
        t("wizard.channels.pluginEnableFailed", {
          ...
          command: formatCliCommand("aether plugins list"),
        }),
        t("wizard.channels.setupTitle"),
      );
      return false;
      ...
      if (adapter) {
        await prompter.note(
          t("wizard.channels.pluginMissingRecoverable", {
            channel,
            listCommand: formatCliCommand("aether plugins list"),
            enableCommand: formatCliCommand("aether plugins enable " + channel),
          }),
          t("wizard.channels.setupTitle"),
        );
  ...
  const applySetupResult = async (channel: ChannelChoice, result: ChannelSetupResult) => {
    ...
    try {
      ...
      await prompter.note(
        `Status unavailable (${detail}).\nRetry: ${formatCliCommand(`aether channels status --channel ${channel}`)}`,
        t("wizard.channels.statusTitle"),
      );
    }
  };
    ...
    if (!adapter) {
      await prompter.note(
        t("wizard.channels.noInteractiveSetup", {
          channel,
          command: formatCliCommand(`aether channels add --channel ${channel} --help`),
        }),
        t("wizard.channels.setupTitle"),
      );
      return;
