...
import {
  ...
  type LegacyAetherBrowserProfileResidue,
...
import type { ConfigValidationIssue, AetherConfig } from "../config/types.aether.js";
...
const BROWSER_AETHER_PROFILE_RESIDUE_CHECK_ID = "core/doctor/browser-aether-profile-residue";
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "aether.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `aether tune` and set Gateway mode (local/remote), or `aether config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `aether config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `aether config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};
    ...
    if (
      ...
      fixHints.push(
        ctx.mode === "doctor"
          ...
          : "Run `aether vitals --fix` to process eligible Workshop relocations and legacy collection backups.",
      );
    }
...
function resolveDoctorMode(cfg: AetherConfig): "local" | "remote" {
...
function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `aether vitals --allow-exec` to verify exec SecretRefs during doctor, or `aether secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
  ...
  return [
    {
      ...
      path: "gateway.auth.token",
      fixHint:
        "Run `aether vitals --fix --generate-gateway-token` to generate a token, then restart the Gateway.",
    },
  ];
}
  ...
  async detect(ctx) {
    ...
    return [
      ...detected.preview.map((line): HealthFinding => ({
        ...
        fixHint: "Run `aether vitals --fix` to migrate legacy state.",
      })),
      ...detected.warnings.map((warning): HealthFinding => ({
        ...
        message: warning,
        path: detected.stateDir,
        fixHint: "Resolve the warning, then rerun `aether vitals --fix`.",
      })),
    ];
  },
};
      ...
      return inspectConfiguredModelReferences({
        ...
        const migrationFinding = migrationTarget
          ? {
              ...
              fixHint: `Run \`aether vitals --fix\` to migrate this model reference to "${migrationTarget}".`,
            }
          : undefined;
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.repairBlocked
          ? [
              ...
              "or set the affected OpenAI models to an Aether runtime policy.",
            ].join(" ")
          : [
              "Run `aether vitals --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an Aether runtime policy.",
            ].join(" "),
      }),
    );
...
const telegramGeneralTopicConversationsCheck: HealthCheck = {
  ...
  async detect(ctx) {
    ...
    return repairs.map((repair) => ({
      ...
      fixHint: "Run `aether vitals --fix` to merge the stale topic-qualified identity.",
    }));
  },
  async repair(ctx) {
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `aether vitals --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyAetherBrowserProfileResidue): HealthFinding {
  return {
    checkId: BROWSER_AETHER_PROFILE_RESIDUE_CHECK_ID,
    severity: "warning",
    message: `Legacy managed browser profile residue was found at ${residue.legacyProfileDir}.`,
    path: residue.legacyProfileDir,
    ocPath: "oc://state/browser/aether",
    fixHint:
      "Run `aether vitals --fix` to archive the stale aether profile safely instead of deleting it in place.",
  };
}

function formatWouldArchiveBrowserResidue(residue: LegacyAetherBrowserProfileResidue): string {
  return [
    "Would archive legacy aether managed browser profile residue.",
    `- legacy profile: ${residue.legacyProfileDir}`,
    `- canonical profile: ${residue.canonicalUserDataDir}`,
  ].join("\n");
}

const browserAetherProfileResidueCheck: HealthCheck = {
  id: BROWSER_AETHER_PROFILE_RESIDUE_CHECK_ID,
  kind: "core",
  description:
    "Legacy aether managed browser profile residue has been archived after the Aether rename.",
  source: "doctor",
  async detect(ctx, scope) {
    const residue = await detectLegacyAetherBrowserProfileResidue(ctx.cfg, browserResidueDeps(ctx));
    if (!residue) {
      return [];
    }
  ...
  async repair(ctx) {
    ...
    if (!residue) {
      return {
        status: "skipped",
        reason: "legacy aether browser profile residue no longer exists",
        changes: [],
      };
    }
    const effect = {
    ...
    const result = await maybeArchiveLegacyAetherBrowserProfileResidue(
      ctx.cfg,
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active aether.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
...
export function createCoreHealthChecks(
  ...
  const checks: readonly DoctorHealthCheck[] = [
    ...
    browserAetherProfileResidueCheck,
