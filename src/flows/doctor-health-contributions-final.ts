...
export function resolveFinalDoctorHealthContributions(params: {
  ...
  return [
      ...
      healthChecks: {
        ...
        async detect(ctx) {
          ...
          return collectBundledChannelPackageStateLoadFailures().map((failure) => ({
            ...
            fixHint: `Rebuild or reinstall plugin ${failure.pluginId}, then rerun \`aether vitals\`.`,
          }));
        },
      },
    }),
    ...
    createDoctorHealthContribution({
      id: "doctor:browser",
      label: "Browser",
      healthCheckIds: ["core/doctor/browser", "core/doctor/browser-aether-profile-residue"],
