...
import { asOptionalObjectRecord as readRecord } from "@aether/normalization-core/record-coerce";
...
function registerCodexHealthChecks(
  ...
  if (shouldRegisterCodexManagedHealth(params.cfg)) {
    ...
    if (!owner) {
      // Implicit preferences may use Aether while plugin provisioning is deferred.
    ...
    if (!owner) {
      throw new MissingPublicSurfaceError(
        "The configured Codex plugin was not found. Install it with aether plugins install @aether/codex.",
      );
    }
    if (owner.origin !== "bundled" && owner.trustedOfficialInstall !== true) {
      throw new MissingPublicSurfaceError(
        "The selected Codex plugin is not a bundled or verified official installation. Run aether plugins inspect codex --runtime --json to inspect its source; install the official plugin with aether plugins install @aether/codex.",
      );
    }
    ...
    if (owner.doctorHealthChecks === true) {
      ...
      try {
        ...
        throw new MissingPublicSurfaceError(
          "The selected Codex plugin declares Doctor health checks but its health API could not be loaded. Run aether plugins inspect codex --runtime --json for details, or aether triage for repair help.",
          { cause },
        );
      }
      if (typeof api.registerCodexManagedAppServerDoctorChecks !== "function") {
        throw new MissingPublicSurfaceError(
          "The selected Codex plugin's Doctor health checks are incomplete. Run aether plugins inspect codex --runtime --json for details, or aether triage for repair help.",
          {
            cause: new TypeError(
...
function shouldRegisterCodexManagedHealth(cfg: AetherConfig): boolean {
...
function isMemoryCoreActive(cfg: AetherConfig): boolean {
...
function shouldRegisterPluginHealth(cfg: AetherConfig, pluginId: string): boolean {
...
function shouldRegisterPolicyHealth(params: { cfg: AetherConfig; cwd?: string }): boolean {
