...
export function resolveChannelSetupWorkspaceDir(cfg: AetherConfig): string {
  ...
  await prompter.note(
    [
      ...
      t("wizard.channelsPrimer.approveWith", {
        command: formatCliCommand("aether pairing approve <channel> <code>"),
      }),
      t("wizard.channelsPrimer.openDm"),
      t("wizard.channelsPrimer.multiUserDm", {
        command: formatCliCommand('aether config set session.dmScope "per-channel-peer"'),
      }),
