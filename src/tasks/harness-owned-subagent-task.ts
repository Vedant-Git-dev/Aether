...
// Harness-mirrored subagents (Codex/Copilot native threads) have no Aether child session, so
// they cannot be recovered or cancelled through one. Both halves of the test are load-bearing:
// Aether-owned rows always carry childSessionKey, and only the agent-harness task runtime stamps
