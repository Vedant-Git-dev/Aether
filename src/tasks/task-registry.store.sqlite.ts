// Persists task registry records and events through the Aether SQLite state database.
...
import { tableExists, tableHasColumns } from "../state/aether-state-db-schema-helpers.js";
...
type TaskRunsTable = AetherStateKyselyDatabase["task_runs"];
type TaskDeliveryStateTable = AetherStateKyselyDatabase["task_delivery_state"];
...
// SQLite-backed task store mirrors task records and delivery state into aether-state.db.
...
function openTaskRegistryDatabase(): TaskRegistryDatabase {
  const database = openAetherStateDatabase();
...
function withWriteTransaction(write: (database: AetherStateDatabase) => void) {
  // Open once before BEGIN; the callback receives that exact shared-state owner.
  openTaskRegistryDatabase();
  runAetherStateWriteTransaction((database) => write(database));
}
...
export function loadTaskRegistryStateFromSqliteReadOnlyResult(): TaskRegistryReadOnlyLoadResult {
  return (
    withExistingAetherStateDatabaseReadOnly(({ db, path }) => {
