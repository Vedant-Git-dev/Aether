// Persists managed task-flow records through the Aether SQLite state database.
...
type FlowRunsTable = AetherStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<AetherStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into aether-state.db.
...
export function loadTaskFlowRegistryStateFromSqliteReadOnly(): TaskFlowRegistryStoreSnapshot {
  return (
    withExistingAetherStateDatabaseReadOnly(({ db }) => readTaskFlowRegistrySnapshot(db)) ?? {
      flows: new Map(),
    }
  );
}
