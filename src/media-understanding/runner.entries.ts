...
import { ok, type Result } from "@aether/normalization-core/result";
import {
...
function resolveAudioProviderPrompt(params: {
  ...
  // match the audio language; omit Aether's English default for autodetection
...
function formatMissingProviderHint(providerId: string): string {
  ...
  return ` Install the official external plugin with: ${formatCliCommand(catalogHint.installCommand)}, then run ${formatCliCommand("aether plugins registry --refresh")} and stop and start the gateway service, or run ${formatCliCommand(catalogHint.doctorFixCommand)} to repair automatically.`;
}
...
export async function runCliEntry(params: {
  ...
  const outputDir = await fs.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-media-cli-"),
  );
  try {
