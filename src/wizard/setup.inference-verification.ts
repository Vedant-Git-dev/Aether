...
export async function completeSetupModelAuth(params: {
...
}): Promise<{ config: AetherConfig; verified: boolean; persisted: boolean }> {
  ...
  if (
    ...
    if (!verification.verified && verification.attempted && stagedCandidate) {
      ...
      config = applyMergePatch(config, inversePatch) as AetherConfig;
...
export async function offerLiveModelVerification(params: {
  ...
  const requiresCandidateVerification = (config: AetherConfig) => {
  ...
  const verify = async (candidate: SetupModelAuthCandidate) => {
    ...
    try {
      ...
      let config = migratePersistedImplicitMainRoster(candidate.config).config as AetherConfig;
      ...
      if (candidate.authProfiles.length > 0) {
        ...
        config = migratePersistedImplicitMainRoster(candidate.config).config as AetherConfig;
      }
  ...
  while (true) {
    ...
    if (result.ok) {
      ...
      const revalidateCredential = async (config: AetherConfig) => {
