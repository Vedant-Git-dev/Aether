...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 Aether 配置",
      ...
      tokenStoreProvisioned:
        "已生成 Gateway 令牌并以 {name} 存入 Aether 密钥存储。配置中只保留引用；可用 `aether secrets store list` 查看。",
    ...
    migration: {
      ...
      appliedTitle: "迁移已应用",
      cancelled: "迁移已取消",
      complete: "迁移完成。下一步运行 `aether vitals`。",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 Aether 工作区？",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 AETHER_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextTokenHint: "将密钥直接存储在 Aether 配置中",
    ...
    guided: {
      laneQuestion: "你想如何开始？",
      laneSecurityLine:
        "Aether 运行的 AI 智能体拥有这台机器的实际访问权限。安全指南：https://docs.aether.ai/gateway/security",
      ...
      quickstartBackground: "如需以后让网关在后台持续运行，请运行：aether gateway install",
      quickstartReopen: "运行 aether 打开 TUI，或运行 aether dashboard 重新打开网页仪表板。",
      quickstartBrowserUnavailable: "浏览器交接未完成。网关仍在运行；准备好后请打开仪表板。",
      quickstartGatewayPending:
        "网关尚不可达。请保持此终端打开，并运行 aether gateway status 检查启动状态。",
      ...
      custodianIntro: "你好 — 我是 Aether，负责维护这套系统。我们开始设置吧。",
      findMeLater: "以后随时可以找到我 — 在终端运行 `aether`，或在仪表盘中打开设置。",
      ...
      setupDone: "一切就绪。",
      appliedTitle: "推理已就绪",
      complete: "Aether 已准备就绪。",
      completeWithoutAi: "Aether 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "Aether 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
        "未进行任何更改。请运行 {fixCommand} 修复，再运行 {inspectCommand} 检查，然后重试 onboarding。",
      invalidConfigUnknown: "- 无法解析配置。",
      manualChoice: "你想如何连接 AI？",
      nextSteps:
        "工作区：{workspace}\n添加频道：`aether channels add`\n更喜欢聊天？运行 `aether ignite`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`aether dashboard`\n稍后聊天：`aether`",
      nextStepsWithoutAi:
        "工作区：{workspace}\n稍后添加 AI：重新运行 `aether awaken`\n连接 AI 后添加频道：`aether channels add`\n打开 dashboard：`aether dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要交互式 TTY。自动化请使用 `aether awaken --non-interactive --accept-risk ...`。",
      welcomeTitle: "设置选项",
    ...
    security: {
      ...
      attribution:
        "Aether 是由 Aether 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
      ...
      confirm: "我理解 Aether 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 Aether。",
      learnMore: "了解更多请访问",
      notMultitenant: "默认情况下，Aether 并非旨在安全地隔离多个用户。",
      personalAgent: "默认情况下，Aether 是供一位操作员使用的个人 agent。",
    ...
    telemetry: {
      ...
      decline: "不用，谢谢",
      description:
        "在每日更新检查中分享你使用的功能（频道、提供商、插件数量）。绝不包含消息或标识符。查看实际发送的内容：`aether telemetry show`。随时更改：`aether telemetry on|off`。",
      title: "帮助 Aether 变得更好？",
    },
    skills: {
      configure: "现在配置技能？（推荐）",
      containerBrewHidden:
      ...
      docsLine: "文档：https://docs.aether.ai/skills",
      enterEnv: "输入 {env}",
      ...
      manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `aether vitals` 检查技能和要求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → Aether → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "Aether" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 Aether 将你加入允许列表。",
      ...
      separatePhoneLabel: "Aether 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 Aether 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；Aether 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "Aether"',
    ...
    twitch: {
      ...
      envPrompt: "检测到 Twitch 环境变量 AETHER_TWITCH_ACCESS_TOKEN。使用环境 token？",
      helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
      helpEnvVars: "支持环境变量：AETHER_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 Aether，并启用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已配置",
      configureLaterHint: "稍后可用 aether tune --section web 配置",
      docsLine: "文档：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文档：https://docs.aether.ai/web/control-ui",
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 Aether。",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 Aether。",
      ...
      gatewayTokenStored:
        "存储位置：$AETHER_CONFIG_PATH（默认 ~/.aether/aether.json）中的 gateway.auth.token，或 AETHER_GATEWAY_TOKEN。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 Aether。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 Aether。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。Aether 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.aether.ai/security",
      ...
      webDocs: "文档：https://docs.aether.ai/tools/web",
      ...
      workspaceDocs: "文档：https://docs.aether.ai/concepts/agent-workspace",
