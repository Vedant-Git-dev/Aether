...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 Aether 設定",
      ...
      tokenStoreProvisioned:
        "已產生 Gateway 權杖並以 {name} 存入 Aether 祕密儲存。設定中只保留參照；可用 `aether secrets store list` 檢視。",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `aether vitals`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 Aether 工作區？",
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 AETHER_ALLOW_INSECURE_PRIVATE_WS=1。",
      ...
      plaintextTokenHint: "將密鑰直接儲存在 Aether 設定中",
    ...
    guided: {
      laneQuestion: "你想如何開始？",
      laneSecurityLine:
        "Aether 執行的 AI 代理程式擁有這台機器的實際存取權限。安全指南：https://docs.aether.ai/gateway/security",
      ...
      quickstartBackground: "如需日後讓閘道在背景持續執行，請執行：aether gateway install",
      quickstartReopen: "執行 aether 開啟 TUI，或執行 aether dashboard 重新開啟網頁儀表板。",
      quickstartBrowserUnavailable: "瀏覽器交接未完成。閘道仍在執行；準備好後請開啟儀表板。",
      quickstartGatewayPending:
        "閘道尚無法連線。請保持此終端機開啟，並執行 aether gateway status 檢查啟動狀態。",
      ...
      custodianIntro: "你好 — 我是 Aether，負責維護這套系統。我們開始設定吧。",
      findMeLater: "之後隨時找得到我 — 在終端機執行 `aether`，或在儀表板中開啟設定。",
      ...
      setupDone: "一切就緒。",
      appliedTitle: "推理已就緒",
      complete: "Aether 已準備就緒。",
      completeWithoutAi: "Aether 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `aether awaken --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "Aether 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`aether channels add`\n偏好聊天？執行 `aether ignite`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`aether dashboard`\n稍後聊天：`aether`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `aether awaken`\n連接 AI 後新增頻道：`aether channels add`\n開啟 dashboard：`aether dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `aether awaken --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    ...
    security: {
      ...
      attribution:
        "Aether 是由 Aether 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 Aether 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 Aether。",
      learnMore: "了解更多請參閱",
      notMultitenant: "預設情況下，Aether 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，Aether 是供一位操作者使用的個人 agent。",
    ...
    telemetry: {
      ...
      decline: "不用，謝謝",
      description:
        "在每日更新檢查中分享你使用的功能（頻道、提供者、插件數量）。絕不包含訊息或識別碼。查看實際傳送的內容：`aether telemetry show`。隨時變更：`aether telemetry on|off`。",
      title: "幫助 Aether 變得更好？",
    },
    skills: {
      configure: "現在設定技能？（建議）",
      containerBrewHidden:
      ...
      docsLine: "文件：https://docs.aether.ai/skills",
      enterEnv: "輸入 {env}",
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `aether vitals` 檢查技能和需求。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → Aether → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "Aether" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 Aether 將你加入允許清單。",
      ...
      separatePhoneLabel: "Aether 專用獨立手機",
    ...
    irc: {
      ...
      nextStatusCommand: "命令：aether channels status --probe",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 Aether 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；Aether 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "Aether"',
    ...
    twitch: {
      ...
      envPrompt: "偵測到 Twitch 環境變數 AETHER_TWITCH_ACCESS_TOKEN。使用環境 token？",
      helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
      helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
      helpEnvVars: "支援環境變數：AETHER_TWITCH_ACCESS_TOKEN",
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 Aether，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 aether tune --section web 設定",
      docsLine: "文件：{url}",
    ...
    finalize: {
      ...
      controlUiDocs: "文件：https://docs.aether.ai/web/control-ui",
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Aether。",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Aether。",
      ...
      gatewayTokenStored:
        "儲存位置：$AETHER_CONFIG_PATH（預設 ~/.aether/aether.json）中的 gateway.auth.token，或 AETHER_GATEWAY_TOKEN。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Aether。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Aether。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。Aether 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.aether.ai/security",
      ...
      webDocs: "文件：https://docs.aether.ai/tools/web",
      ...
      webUiUrl: "Web UI：{url}",
      whatNow: '下一步：https://aether.ai/showcase（"What People Are Building"）。',
      whatNowTitle: "下一步",
      workspaceBackupTitle: "工作區備份",
      workspaceDocs: "文件：https://docs.aether.ai/concepts/agent-workspace",
