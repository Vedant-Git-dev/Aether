    ...
    await params.prompter.note(
      [
        ...
        t("wizard.finalize.startGatewayNow", {
          command: formatCliCommand("aether gateway run"),
        }),
      ].join("\n"),
      "Gateway",
    );
...
function buildGatewayRecoveryProjection(params: {
  ...
  if (gateway.status === "ready") {
    ...
    const detail = t("wizard.finalize.managedGatewayUnreachable", {
      service,
      statusCommand: formatCliCommand("aether gateway status --deep"),
      recoveryCommand: formatCliCommand("aether gateway restart"),
    });
  ...
  if (gateway.status === "failed") {
    ...
    const detail = t("wizard.finalize.managedGatewaySetupFailed", {
      ...
      error: gateway.error,
      statusCommand: formatCliCommand("aether gateway status --deep"),
      recoveryCommand: formatCliCommand("aether gateway install --force"),
    });
    return {
      detail,
  ...
  return {
    detail: [
      ...
      t("wizard.finalize.rerunInstallDaemon", {
        command: formatCliCommand("aether awaken --install-daemon"),
      }),
      t("wizard.finalize.skipHealthNextTime", {
        command: formatCliCommand("aether awaken --skip-health"),
      }),
    ].join("\n"),
    summary,
  };
...
export async function finalizeSetupWizard(
  ...
  try {
    if (!opts.skipHealth) {
      ...
      if (gatewayProbe.ok) {
        try {
          const healthConfig: AetherConfig =
          ...
          await prompter.note(
            [
              t("common.docs"),
              "https://docs.aether.ai/gateway/health",
              "https://docs.aether.ai/gateway/troubleshooting",
            ].join("\n"),
    ...
    if (!opts.skipUi && controlUiEnabled && gatewayProbe.ok) {
      ...
      try {
        const target = await resolveControlUiHandoffTarget({
          ...
          env: { ...process.env, AETHER_GATEWAY_PORT: String(settings.port) },
        });
        ...
        await prompter.note(
          [
            ...
            t("wizard.finalize.noModelAuthNext", {
              command: formatCliCommand("aether tune --section model"),
            }),
          ].join("\n"),
      ...
      if (gatewayProbe.ok) {
        const tokenNotes = [
          ...
          t("wizard.finalize.gatewayTokenView", {
            command: formatCliCommand("aether gateway auth-token --show"),
          }),
          t("wizard.finalize.gatewayTokenGenerate", {
            command: formatCliCommand("aether vitals --generate-gateway-token"),
          }),
          t("wizard.finalize.dashboardOpenAnytime", {
            command: formatCliCommand("aether dashboard --no-open"),
          }),
        ].filter(Boolean);
      ...
      if (!entry) {
        await prompter.note(
          [
            ...
            `  ${formatCliCommand("aether tune --section web")}`,
            "",
        ...
        await prompter.note(
          [
            ...
            t("wizard.finalize.webSearchGetKey", {
              url: entry?.signupUrl ?? "https://docs.aether.ai/tools/web",
            }),
            t("wizard.finalize.webDocs"),
          ].join("\n"),
          t("wizard.finalize.webSearchTitle"),
        ...
        await prompter.note(
          [
            ...
            t("wizard.finalize.webSearchReenable", {
              command: formatCliCommand("aether tune --section web"),
            }),
            "",
            t("wizard.finalize.webDocs"),
          ].join("\n"),
      ...
      !gatewayProbe.ok || gateway.status === "failed"
        ...
        : gatewayHealthCheckFailed
          ? t("wizard.finalize.outroHealthCheckFailed", {
              command: formatCliCommand("aether health"),
            })
          : dashboardReady
            ...
            : controlUiEnabled
              ? [
                  ...
                  t("wizard.finalize.dashboardWhenReady", {
                    command: formatCliCommand("aether dashboard"),
                  }),
                ].join(" ")
              : t("wizard.guided.complete"),
    );
