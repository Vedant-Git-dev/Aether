...
import {
  disposeAetherAgentDatabaseByPath,
...
type SetupMigrationStage = {
  ...
  getFinalConfig: () => AetherConfig;
  getStagedConfig: () => AetherConfig;
  replaceStagedConfig: (config: AetherConfig) => void;
  projectPlanToStage: (plan: MigrationPlan) => MigrationPlan;
  projectResultToFinal: (result: MigrationApplyResult) => MigrationApplyResult;
  promote: (params: {
    expectedConfig: AetherConfig;
    continuation: Omit<
      SetupMigrationPromotionContinuation,
      "stagedReportDir" | "stagedRoots" | "workspaceDir"
    >;
    readConfigFile: () => Promise<AetherConfig>;
    commitConfigFile: (
      config: AetherConfig,
      expectedConfig: AetherConfig,
    ) => Promise<AetherConfig>;
  }) => Promise<{ config: AetherConfig; resume: SetupMigrationPromotionResume }>;
...
async function makePrivateStageNear(target: string, label: string): Promise<string> {
  ...
  const staged = await fs.mkdtemp(path.join(ancestor, `.aether-${label}-`));
...
function createInMemoryConfigRuntime(params: {
  finalConfig: AetherConfig;
  stagedConfig: AetherConfig;
  projectToFinal: (config: AetherConfig) => AetherConfig;
}): {
  runtime: MigrationConfigRuntime;
  getFinalConfig: () => AetherConfig;
  getStagedConfig: () => AetherConfig;
  replaceConfigs: (next: { finalConfig: AetherConfig; stagedConfig: AetherConfig }) => void;
} {
...
export async function createSetupMigrationStage(params: {
  ...
  const finalEnv = { ...process.env, AETHER_STATE_DIR: params.stateDir };
  ...
  const stageEnv = { ...process.env, AETHER_STATE_DIR: stagedStateDir };
  const stagedConfig: AetherConfig = {
  ...
  const projectConfigToFinal = (config: AetherConfig) =>
    projectValue(config, toFinal) as AetherConfig;
  const configs = createInMemoryConfigRuntime({
    finalConfig: params.targetConfig,
    stagedConfig,
    projectToFinal: projectConfigToFinal,
  });
  openAetherAgentDatabase({ agentId, env: stageEnv });
  ...
  const disposeDatabases = () => {
    ...
    const stagedAgentDatabasePath = path.join(stagedAgentDir, "aether-agent.sqlite");
    disposeAetherAgentDatabaseByPath(stagedAgentDatabasePath, { env: stageEnv });
    closeAetherStateDatabaseByPath(resolveAetherStateSqlitePath(stageEnv));
      ...
      try {
        for (const component of journal.components) {
          ...
          if (component.name === "agent") {
            registerAetherAgentDatabase({
              agentId,
              path: path.join(finalAgentDir, "aether-agent.sqlite"),
              env: finalEnv,
            });
        ...
        try {
          ...
          if (current && hashSetupMigrationConfig(current) === journal.configHashTarget) {
            ...
            await writePromotionJournal(journalPath, journal);
            throw new Error(
              `Migration config commit is indeterminate. Review ${journalPath} and run aether vitals before retrying.`,
              { cause: error },
            );
          }
        }
        ...
        await writePromotionJournal(journalPath, journal);
        throw new Error(
          `Migration promotion could not be rolled back. Review ${journalPath} and run aether vitals before retrying.`,
          { cause: error },
        );
      }
    },
