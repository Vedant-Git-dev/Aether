...
import type { GatewayBindMode, GatewayTailscaleMode, AetherConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    const ambientToken = normalizeGatewayTokenInput(process.env.AETHER_GATEWAY_TOKEN);
    if (tokenMode === "ref") {
      if (quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "AETHER_GATEWAY_TOKEN",
          copy: {
            sourceMessage: t("wizard.gateway.authTokenStoredMessage"),
            envVarPlaceholder: "AETHER_GATEWAY_TOKEN",
          },
        });
    ...
    if (!password) {
      ...
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "AETHER_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: t("wizard.gateway.authPasswordStoredMessage"),
            envVarPlaceholder: "AETHER_GATEWAY_PASSWORD",
          },
        });
