  ...
  return await transformConfigWithPendingPluginInstalls({
    ...
    transform: (current) => ({
      nextConfig: opts.mergeBase
        ? (applyMergePatch(current, createMergePatch(opts.mergeBase, config)) as AetherConfig)
        : config,
    }),
  });
}
...
export async function readValidSetupConfigFile(): Promise<AetherConfig> {
