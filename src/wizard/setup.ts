...
async function runSetupWizardOnce(
  ...
  let baseConfig: AetherConfig = snapshot.valid
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("aether vitals")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("aether vitals")}`,
        `Inspect: ${formatCliCommand("aether plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("aether tune"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: aether awaken --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  while (opts.importFrom || flow === "import" || flow.startsWith("import:")) {
    ...
    try {
      migrationOutcome = await runSetupMigrationImport({
        ...
        async commitConfigFile(cfg, expectedConfig) {
          ...
          if (!latest.valid) {
            throw new Error("Migration target config became invalid. Run `aether vitals`.");
          }
    ...
    if (!migratedSnapshot.valid) {
      throw new Error("Migration produced an invalid Aether config. Run `aether vitals`.");
    }
  ...
  let localGatewayToken = process.env.AETHER_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.AETHER_GATEWAY_PASSWORD;
  try {
  ...
  const remoteSeedConfig: AetherConfig =
