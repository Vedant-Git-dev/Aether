...
function hasMeaningfulConfig(config: AetherConfig): boolean {
...
function buildSetupMigrationSnapshotConfig(config: AetherConfig): Record<string, unknown> {
...
/** Serializes onboarding writes that share one Aether state target. */
export async function withSetupMigrationTargetLock<T>(
  ...
  if (activeStateDir) {
    if (activeStateDir !== resolvedStateDir) {
      throw new Error("nested onboarding target lock cannot switch the Aether state directory");
    }
    return await fn();
  }
  ...
  try {
    ...
    throw new SetupTargetLockedError(holderPid, process.env.AETHER_PROFILE?.trim(), error);
  }
}
...
export function assertFreshSetupMigrationTarget(freshness: {
  ...
  throw new SetupMigrationFreshnessError(
    [
      "Migration import during onboarding requires a fresh Aether setup.",
