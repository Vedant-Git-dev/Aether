 ...
 * `aether migrate` already answers an unknown provider this way; onboarding has to match, because
...
function assertListedMigrationProvider(
  ...
  const listCommand = formatCliCommand("aether migrate list");
  throw new Error(
...
async function withSetupMigrationProvider<T>(
  params: { providerId: string; baseConfig: AetherConfig },
  run: (resolved: { provider: MigrationProviderPlugin; baseConfig: AetherConfig }) => Promise<T>,
): Promise<T> {
