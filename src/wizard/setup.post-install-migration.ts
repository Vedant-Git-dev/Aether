...
function logMigrationHint(runtime: RuntimeEnv, candidate: ResolvedProviderCandidate): void {
  const command = formatCliCommand(`aether migrate ${candidate.provider.id} --dry-run`);
 ...
 * prompt) is owned by `aether migrate <provider>`; this helper only owns
 * the gate prompt.
 */
    ...
    const logFailure = (error: unknown) => {
      params.runtime.log(
        `${candidate.provider.label} migration failed: ${formatErrorMessage(error)}. ` +
          `Re-run with ${formatCliCommand(`aether migrate ${candidate.provider.id} --dry-run`)} to inspect.`,
      );
    };
