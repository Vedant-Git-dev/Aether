/**
 * Knip configuration for Aether root and bundled plugin dependency hygiene.
 */
...
const repositoryScriptEntries = [
  ...
  // aether-performance.yml invokes the paired benchmark CLI by path.
  ...
  "scripts/aether-release-clawhub-plan.ts!",
  "scripts/aether-release-clawhub-runtime-state.ts!",
  // Protected preparation/button workflows invoke this coordinator by path.
  "scripts/aether-release-ready.mjs!",
  ...
  "scripts/secrets/aether-bws-resolver.mjs!",
...
const rootEntries = [
  ...
  "scripts/aether-cross-os-release-checks.ts!",
  ...
  // Jiti virtualizes aether/plugin-sdk/agent-sessions through this cycle-safe barrel.
...
const rootToolingAndWorkspaceDependencies = [
  ...
  "dompurify",
  // Root typecheck/test projects compile @aether/net-policy source directly.
  ...
  workspaces: {
    ".": {
      ignoreDependencies: [
        ...
        // Docker packaging stages @aether/ai without nested dependencies after
      ...
      ignoreBinaries: ["mint", "ngrok", "open", "aether", "sleep", "xcrun"],
