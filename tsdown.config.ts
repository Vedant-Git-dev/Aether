...
const RUN_NODE_SKIP_DTS_BUILD = process.env.AETHER_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(
  ...
  if (process.env.AETHER_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.AETHER_BUILD_PRIVATE_QA === "1";
...
function shouldAlwaysBundleDependency(id: string): boolean {
  return (
    id === "aether/plugin-sdk/ssrf-runtime-internal" ||
    id === "@aether/normalization-core" ||
    id.startsWith("@aether/normalization-core/") ||
    id === "@aether/retry" ||
    id === "@aether/media-core" ||
    id.startsWith("@aether/media-core/") ||
    [
      "@aether/acp-core",
      "@aether/session-url-contract",
      "@aether/workboard-contract",
    ].includes(id) ||
    id.startsWith("@aether/acp-core/") ||
    id === "zod" ||
    id.startsWith("zod/")
  );
}
...
function buildCoreDistEntries(): Record<string, string> {
  return {
    ...
    "mcp/aether-tools-serve": "src/mcp/aether-tools-serve.ts",
  };
}
...
function shouldExternalizeAgentCoreDependency(id: string): boolean {
  return (
    id === "@aether/ai" ||
    id.startsWith("@aether/ai/") ||
    id === "@aether/llm-core" ||
    id.startsWith("@aether/llm-core/") ||
    id === "ignore" ||
    id === "aether" ||
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return ["ws", "@aether/gateway-protocol", "ipaddr.js"].some(
