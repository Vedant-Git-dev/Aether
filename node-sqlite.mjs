import { isSupportedAetherNodeVersion, parseNodeReleaseVersion } from "./node-version.mjs";
...
const capabilityCacheKey = Symbol.for("aether.sqliteCapabilities");
...
export function nodeRuntimeFailure(version, probe) {
  ...
  if (!release || release.major < 24) {
    return `${label}: aether requires Node >=24.16.0 <25, or >=26.1.0.`;
  }
  return null;
}

export function nodeRuntimeNote(version, probe) {
  return !nodeRuntimeFailure(version, probe) && !isSupportedAetherNodeVersion(version)
