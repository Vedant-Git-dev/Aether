...
export function formatUnsupportedNodeVersionMessage(version) {
  return [
    `Node ${version ?? "unknown"} is unsupported; Aether requires ${SUPPORTED_NODE_VERSIONS}.`,
    "npm can finish installing Aether without running its preinstall check; a successful install does not mean Node is supported.",
    "Re-run the installer: curl -fsSL https://aether.ai/install.sh | bash",
    "On Windows: iwr -useb https://aether.ai/install.ps1 | iex",
    "Or with nvm: nvm install 26 && nvm use 26 && nvm alias default 26",
    "Then rerun aether update. See https://docs.aether.ai/install/node",
  ].join("\n");
}
...
export function classifyUnsupportedNodeCommand(argv) {
  ...
  // Aether database writable and never starts a Gateway or a repair agent.
...
export function canRunAetherNodeDiagnostics(value, hasNodeSqlite) {
  return (
    hasNodeSqlite &&
...
/** Checks Aether's supported release lines. Older Node lines with lossy SQLite TEXT reads are unsupported. */
export function isSupportedAetherNodeVersion(value) {
...
// Worker bootstrap runs before Aether is transferred. Carry the canonical
