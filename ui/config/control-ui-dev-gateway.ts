...
export function createControlUiDevGateway(
  ...
  try {
    ...
    throw new Error(
      "AETHER_UI_DEV_GATEWAY_URL must be an absolute HTTP(S) or WS(S) Gateway URL.",
    );
  }
  if (
    ...
    throw new Error(
      "AETHER_UI_DEV_GATEWAY_URL must not contain credentials, a query, or a fragment.",
    );
  }
  ...
  const proxyPath = `/__aether_dev_gateway__/${encodeURIComponent(gatewayUrl)}`;
