...
class CronPage extends AetherLightDomElement {
...
export const cronPageComponent = {
  header: true,
  render: (search: unknown) => html`<aether-cron-page
    .routeSearch=${typeof search === "string" ? search : ""}
  ></aether-cron-page>`,
};

// Module re-evaluation can retain the shared registry (for example, in Vitest).
if (!customElements.get("aether-cron-page")) {
  customElements.define("aether-cron-page", CronPage);
}
