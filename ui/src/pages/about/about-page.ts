...
// Mirrors the about-aether-wave animation duration in about.css so the wave
// class comes off right as the claw settles and the next poke can replay it.
const AETHER_WAVE_MS = 1400;

class AboutPage extends AetherLightDomElement {
  @consume({ context: applicationContext, subscribe: true })
  private context!: ApplicationContext;

  @state() private copyState: AboutCommitCopyState = "idle";
  @state() private aetherWaving = false;
  ...
  private pokeAether() {
    if (this.aetherWaving) {
      return;
    }
    this.aetherWaving = true;
    this.waveResetTimer = globalThis.setTimeout(() => {
      this.waveResetTimer = null;
      this.aetherWaving = false;
    }, AETHER_WAVE_MS);
  }
  ...
  override render() {
    ...
    const body = renderAbout({
      ...
      aetherWaving: this.aetherWaving,
      onPokeAether: () => this.pokeAether(),
    });
    return html`
...
if (!customElements.get("aether-about-page")) {
  customElements.define("aether-about-page", AboutPage);
}
