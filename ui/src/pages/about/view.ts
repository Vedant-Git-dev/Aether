...
const ABOUT_LINKS: ReadonlyArray<{ href: string; icon: TemplateResult; label: () => string }> = [
  { href: "https://aether.ai", icon: icons.globe, label: () => t("aboutPage.linkWebsite") },
  { href: "https://docs.aether.ai", icon: icons.book, label: () => t("aboutPage.linkDocs") },
  {
  ...
  {
    href: "https://docs.aether.ai/releases",
...
// The same canonical crimson Aether as the chat welcome hero, rendered big.
...
function renderHero(props: AboutProps) {
  ...
  return html`
    <section class="about-hero">
      <button
        type="button"
        class="about-hero__aether ${props.aetherWaving ? "about-hero__aether--wave" : ""}"
        style=${lobsterLookStyle(look)}
        aria-label=${t("aboutPage.waveHello")}
        @click=${props.onPokeAether}
      >
