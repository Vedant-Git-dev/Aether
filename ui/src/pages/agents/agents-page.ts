...
const AGENTS_DOCS_URL = "https://docs.aether.ai/concepts/multi-agent";
  ...
  implements Omit<AgentsState, "agentsLoading" | "agentsError">
{
  ...
  override render() {
    ...
    const access = {
      canCreateAgent: this.canCall("aether.chat", "operator.admin"),
      ...
      ${renderSettingsWorkspace(
        this.identityAvatarLoader.withActiveRoutes(() =>
          renderAgents({
            ...
            onCreateAgent: () => {
              if (this.canCall("aether.chat", "operator.admin")) {
...
if (!customElements.get("aether-agents-page")) {
  customElements.define("aether-agents-page", AgentsPage);
}
