...
export abstract class ChatPaneHeader extends ChatPaneDiscussion {
  ...
  private renderPanelLayoutActions(
    ...
    ${
      split || layout.expanded
        ? html`<aether-tooltip .content=${focusLabel}>
            <button
    ...
    ${
      split && side && swapLabel
        ? html`<aether-tooltip .content=${swapLabel}>
            <button
  ...
  protected renderPaneHeader(
    ...
    const sidePanelAction = html`<aether-tooltip
    ...
    const browserPanelAction = sessionWorkspace.onToggleBrowser
      ? html`<aether-tooltip .content=${t("browser.toggle")}>
          <button
    ...
    const header = renderChatPaneHeader({
      ...
      sessionMenuAction:
        row && this.state
          ? html`<aether-chat-header-session-menu
              .session=${{
                label:
            ...
            ></aether-chat-header-session-menu>`
          : nothing,
