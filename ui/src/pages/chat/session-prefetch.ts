...
const SESSION_PREFETCH_LOCK_NAME = "aether-chat-prefetch";
...
class SessionPrefetchController implements ReactiveController {
  ...
  hostConnected(): void {
    ...
    this.paneRoot = this.host.closest("aether-app-shell") ?? this.host;
  ...
  private readonly handleNavigationIntent = (event: Event) => {
    ...
    if (row && row.closest("aether-app-shell") !== this.paneRoot.closest("aether-app-shell")) {
      return;
    }
  ...
  private readonly sync = () => {
    ...
    const panes = [...this.paneRoot.querySelectorAll<ChatPaneElement>("aether-chat-pane")].filter(
      (pane) => pane.closest("aether-app-shell") === this.paneRoot.closest("aether-app-shell"),
    );
