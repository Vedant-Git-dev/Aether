import { asNullableRecord as recordOrNull } from "@aether/normalization-core/record-coerce";
import { normalizeOptionalString as stringValue } from "@aether/normalization-core/string-coerce";
...
function clearTypingActorForUserMessage(
  ...
  const identity = readTranscriptSenderIdentity(
    recordOrNull(message?.["__aether"])?.senderIdentity,
  );
