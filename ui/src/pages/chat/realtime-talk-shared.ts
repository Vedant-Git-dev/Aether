...
function getTerminalAgentWaitError(result: AgentWaitResult | undefined): Error | undefined {
  ...
  if (result.status === "error") {
    return new Error(message || "Aether tool call failed");
  }
  ...
  if (hasTerminalTimeoutMetadata) {
    return new Error(message || "Aether tool call timed out");
  }
  return undefined;
}
...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    if (params.signal?.aborted) {
      reject(new DOMException("Aether tool call aborted", "AbortError"));
      return;
    }
    const timer = window.setTimeout(() => {
      settleReject(new Error("Aether tool call timed out"));
    }, params.timeoutMs);
    let settled = false;
    let emptyFinalWaitStarted = false;
    let emptyFinalFallbackTimer: number | undefined;
    const onAbort = () => {
      settleReject(new DOMException("Aether tool call aborted", "AbortError"));
    };
      ...
      void params.client
        ...
        .then((result) => {
          ...
          emptyFinalFallbackTimer = window.setTimeout(() => {
            settleResolve("Aether finished with no text.");
    ...
    unsubscribe = params.client.addEventListener((evt: GatewayEventFrame) => {
      ...
      if (payload.state === "final") {
        ...
        settleReject(
          new DOMException(payload.errorMessage ?? "Aether tool call aborted", "AbortError"),
        );
      } else if (payload.state === "error") {
        settleReject(new Error(payload.errorMessage ?? "Aether tool call failed"));
      }
    });
    function cleanup() {
