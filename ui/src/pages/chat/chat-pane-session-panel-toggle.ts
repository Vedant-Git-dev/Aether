...
type PanelTagName = "aether-browser-panel" | "aether-desktop-panel" | "aether-terminal-panel";
...
export class ChatPaneSessionPanelToggleController {
  ...
  handle(slot: SessionPanelToggleSlot, tagName: PanelTagName, event: Event): boolean {
    ...
    if (slot === "terminal") {
      ...
      const embeddedTerminal = owner.renderRoot.querySelector("aether-terminal-panel[embedded]");
      const terminalConstructor = customElements.get("aether-terminal-panel");
    ...
    this.options.updateSidebarLayout(openSlot(owner.state.sidebarLayout, slot));
    void Promise.all([
      customElements.whenDefined("aether-chat-sidebar-region"),
      ...
      const region = owner.renderRoot.querySelector<
        HTMLElementTagNameMap["aether-chat-sidebar-region"]
      >("aether-chat-sidebar-region");
  ...
  flush(): void {
    ...
    for (const [slot, tagName] of [
      ["terminal", "aether-terminal-panel"],
      ["browser", "aether-browser-panel"],
      ["desktop", "aether-desktop-panel"],
    ] as const) {
