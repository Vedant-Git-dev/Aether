...
export class ChatPage extends AetherLightDomElement implements SessionSplitHost {
  ...
  private readonly handleDragOver = (event: DragEvent) => {
    ...
    const pane = target?.closest<ChatPaneElement>("aether-chat-pane");
  ...
  override render() {
    ...
    return this.mcpAppUnmountGate.render(JSON.stringify([...nextPaneKeys]), renderValue, () =>
      [...this.querySelectorAll<ChatPaneElement>("aether-chat-pane")].filter(
...
if (!customElements.get("aether-chat-page")) {
  customElements.define("aether-chat-page", ChatPage);
}
