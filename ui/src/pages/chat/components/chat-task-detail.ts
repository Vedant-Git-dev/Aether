...
export function renderTaskDetailPanel(params: {
  ...
  // transcript history without creating an Aether session.
        ...
        ${
          task
            ? html`<div class="chat-task-detail__meta">
                ...
                >
                ${active && startedMs > 0 ? html`<span aria-hidden="true">·</span><aether-elapsed-time .startMs=${startedMs}></aether-elapsed-time>` : duration ? html`<span aria-hidden="true">·</span><span>${duration}</span>` : nothing}
