...
import { asNullableObjectRecord as readCostRecord } from "@aether/normalization-core/record-coerce";
import { html, nothing } from "lit";
import { isTranscriptOnlyAetherAssistantMessage } from "../../../../../src/shared/transcript-only-aether-assistant.js";
...
function latestProviderCostStats(messages: unknown[] | undefined): ProviderCostStats | null {
  for (let index = (messages?.length ?? 0) - 1; index >= 0; index -= 1) {
    ...
    if (message?.role !== "assistant" || isTranscriptOnlyAetherAssistantMessage(message)) {
      continue;
    }
