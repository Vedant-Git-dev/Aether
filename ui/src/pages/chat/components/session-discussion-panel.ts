...
function resolveDiscussionEmbedUrl(value: string | undefined): string | null {
  ...
  if (
    url.searchParams.get("aetherHostTheme") !== "1" ||
...
class SessionDiscussionPanel extends AetherLightDomElement {
  @property() sessionKey = "";
...
if (!customElements.get("aether-session-discussion")) {
  customElements.define("aether-session-discussion", SessionDiscussionPanel);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-session-discussion": SessionDiscussionPanel;
  }
}
