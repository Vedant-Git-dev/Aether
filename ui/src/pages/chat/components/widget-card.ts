...
function isManagedCanvasDocumentPreview(preview: CanvasToolPreview): boolean {
  ...
  try {
    ...
    const prefix = "/__aether__/canvas/documents/";
...
const WIDGET_SIZE_MESSAGE_TYPE = "aether:widget-size";
const WIDGET_PROMPT_OFFER_MESSAGE_TYPE = "aether:widget-prompt-offer";
const WIDGET_PROMPT_MESSAGE_TYPE = "aether:widget-prompt";
const WIDGET_PROMPT_HOST_READY_MESSAGE_TYPE = "aether:widget-prompt-host-ready";
const WIDGET_CHAT_HOST_MESSAGE_TYPE = "aether:widget-chat-host";
...
function renderMcpAppView(params: {
  ...
  void ensureCustomElementDefined("mcp-app-view", loadMcpAppView).catch((error: unknown) => {
    console.error("[aether] failed to load MCP App view", error);
  });
  ...
  switch (kind) {
    case "canvas-html": {
      ...
      if (preview.sandbox !== "strict" && isManagedCanvasDocumentPreview(preview)) {
        void ensureCustomElementDefined("aether-canvas-widget-view", loadCanvasWidgetView).catch(
          (error: unknown) => console.error("[aether] failed to load widget view", error),
        );
        return keyed(
          `${preview.viewId}\0${getCanvasWidgetFrameConnectionGeneration()}`,
          html`
            <aether-canvas-widget-view
...
function handleWidgetExportAction(
  ...
  const documentHtml = frame.closest("aether-canvas-widget-view")?.documentHtml;
