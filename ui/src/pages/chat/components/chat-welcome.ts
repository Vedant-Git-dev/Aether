...
function renderWelcomeAether() {
  return html`
    <div class="agent-chat__welcome-aether" aria-hidden="true">
      <aether-mascot mood="idle" .size=${112}></aether-mascot>
    </div>
  `;
}
...
export function renderWelcomeState(props: ChatWelcomeProps) {
  ...
  const mascotFor = (event: DragEvent): WelcomeMascot | null => {
    ...
    return target instanceof HTMLElement
      ? target.querySelector<WelcomeMascot>(".agent-chat__welcome-aether aether-mascot")
      : null;
  };
