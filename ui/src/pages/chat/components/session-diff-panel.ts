...
const PREFERENCES_KEY = "aether.control.sessionDiff.v1";
...
class SessionDiffPanel extends AetherLightDomElement {
  ...
  private renderSummary(result: SessionsDiffResult): TemplateResult {
    ...
    return html`
      <div class="session-diff__summary">
        ...
        <aether-tooltip .content=${t("chat.sessionDiff.viewOptions")}>
          <button
        ...
        <aether-tooltip .content=${t("chat.sessionDiff.refresh")}>
          <button
...
if (!customElements.get("aether-session-diff")) {
  customElements.define("aether-session-diff", SessionDiffPanel);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-session-diff": SessionDiffPanel;
  }
}
