import type { ControlUiFocusBuildTarget } from "@aether/session-url-contract";
...
export function sidebarPanelDefinitions(
  ...
  const companion = params
    ? html`<aether-chat-session-rail
      ...
      ></aether-chat-session-rail>`
    : null;
  const desktop =
    state && params?.desktopAvailable
      ? html`<aether-desktop-panel
          embedded
          data-chat-autotype-exempt
          .client=${state.connected ? state.client : null}
  ...
  const discussion = params?.discussion
    ? html`<aether-session-discussion
  ...
  return [
    ...
    definePanel(
      ...
      companion,
      params
        ? html`<aether-tooltip .content=${t("chat.rail.clear")}>
            <button
    ...
    definePanel(
      ...
      params?.tasks ?? null,
      params
        ? html`<aether-tooltip .content=${t("chat.backgroundTasks.refresh")}>
            <button
