...
import { readSessionMessageIdentity } from "@aether/gateway-client/browser";
...
function readProjections(item: MessageItem, index: number): Projection[] {
  ...
  return blocks.map((block) => {
    ...
    const id = resolveToolBlockId(raw, message)!;
    const call = isToolCallContentType(raw.type);
    const live = message["__aetherToolStreamLive"] === true;
    ...
    return {
      ...
      rank: call
        ...
        : !live
          ? 3
          : message["__aetherToolStreamResultReceived"] === true
            ? 2
            : 1,
      block: {
        ...raw,
...
function coalesceTurn(items: ChatItem[]): ChatItem[] {
  ...
  for (const projection of projections) {
    ...
    if (projection.source.message["__aetherToolStreamLive"] === true) {
  ...
  for (const invocation of invocations.values()) {
    ...
    const metadata = asRecord(message["__aether"]);
    ...
    const transcript =
      ...
      asRecord(result?.source.message["__aether"])?.id;
    const content = [
    ...
    const bundleKey = JSON.stringify([
      ...
      completed,
      transcript,
      invocation.live?.["__aetherToolStreamDiffStat"],
      invocation.live?.["__aetherToolStreamReceivedAt"],
    ...
    const item: ProjectedItem = {
      ...
      message: {
        ...
        ...(invocation.live
          ? {
              __aetherToolStreamLive: true,
              __aetherToolStreamResultReceived: completed,
              __aetherToolStreamDiffStat: completed
                ? undefined
                : invocation.live["__aetherToolStreamDiffStat"],
              __aetherToolStreamReceivedAt: invocation.live["__aetherToolStreamReceivedAt"],
            }
          : {}),
      },
    };
