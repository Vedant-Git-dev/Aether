...
export class ModelSetupPage extends AetherLightDomElement {
  ...
  private readonly wizard = new ModelSetupWizardRunner({
    ...
    onStart: (method, intent) => {
      if (method === "aether.setup.prepare.start") {
        return undefined;
      }
  ...
  override updated(changed: PropertyValues) {
    ...
    if (this.wizardState.phase !== "idle") {
      this.querySelector("aether-modal-dialog")?.setReturnFocusTarget(this.wizardReturnFocus);
    }
  ...
  private canUseSetup(client: GatewayBrowserClient | null): client is GatewayBrowserClient {
    ...
    return Boolean(
      ...
      isGatewayMethodAdvertised(snapshot, "aether.setup.detect") === true,
    );
  }
  ...
  private canVerify(client: GatewayBrowserClient | null): client is GatewayBrowserClient {
    ...
    return (
      ...
      isGatewayMethodAdvertised(snapshot, "aether.setup.verify") === true
    );
  }
  ...
  private async handleWizardDone({
    ...
    const prepareOption =
      startMethod === "aether.setup.prepare.start" ? this.pendingPrepareOption : null;
    ...
    if (startMethod !== "aether.setup.prepare.start") {
      if (isCurrent?.() === false) {
        this.wizard.close();
        return;
      }
      if (!modelActivation) {
        this.wizard.fail(
          t(
            startMethod === "aether.setup.activate.start"
  ...
  override render() {
    ...
    return renderModelSetup({
      ...
      canPrepare:
        canAdmin &&
        !gatewayTooOld &&
        isGatewayMethodAdvertised(snapshot, "aether.setup.prepare.start") === true,
      ...
      onStartPrepare: (option: ModelSetupPrepareOption) => {
        ...
        void this.runWizardMutation(() =>
          this.wizard.start(option.id, "aether.setup.prepare.start"),
        );
      },
...
if (!customElements.get("aether-model-setup-page")) {
  customElements.define("aether-model-setup-page", ModelSetupPage);
}
