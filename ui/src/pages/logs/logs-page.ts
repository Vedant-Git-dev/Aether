...
class LogsPage extends AetherLightDomElement {
  ...
  override render() {
    const body = renderLogs({
      ...
      onExport: (lines, label) => {
        ...
        downloadTextFile(`aether-logs-${label}-${stamp}.log`, `${lines.join("\n")}\n`);
      },
...
if (!customElements.get("aether-logs-page")) {
  customElements.define("aether-logs-page", LogsPage);
}
