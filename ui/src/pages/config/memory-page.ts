...
const DREAMING_DOCS_URL = "https://docs.aether.ai/concepts/dreaming";
...
class MemorySettingsPage extends AetherLightDomElement {
  ...
  override render() {
    ...
    return renderMemory({
      ...
      memories: html`
        ...
        ></aether-memory-memories>
      `,
      dreams: html` <aether-memory-dreaming .agentId=${agentId}></aether-memory-dreaming> `,
      editor:
...
if (!customElements.get("aether-memory-settings")) {
  customElements.define("aether-memory-settings", MemorySettingsPage);
}

export function renderMemoryPage(props: MemoryPageProps) {
  return html`
    <aether-memory-settings
