...
import type { SystemChangeEntry, SystemChangesListResult } from "@aether/gateway-protocol";
import { html, nothing } from "lit";
import { property, state } from "lit/decorators.js";
import type { GatewayBrowserClient } from "../../api/gateway.ts";
import { applicationContext, type ApplicationContext } from "../../app/context.ts";
import "../../components/aether-mascot.ts";
...
export class CustodianPage extends AetherLightDomElement {
  ...
  private synchronizeHistoryClient(): void {
    ...
    const available =
      client !== null && isGatewayMethodAdvertised(snapshot, "aether.changes.list") === true;
  ...
  private async loadHistory(reset: boolean): Promise<void> {
    ...
    const isCurrent = () =>
      ...
      this.historyAvailable;
    try {
      const result = await client.request<SystemChangesListResult>("aether.changes.list", {
      ...
      >
        ...
        >
          ...
          <div class="custodian__header-actions">
            ${
              this.onboarding
                ? html`<aether-sidebar-attention></aether-sidebar-attention>`
                : nothing
            }
            ${
              this.historyAvailable
...
if (!customElements.get("aether-custodian-page")) {
  customElements.define("aether-custodian-page", CustodianPage);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-custodian-page": CustodianPage;
  }
}
