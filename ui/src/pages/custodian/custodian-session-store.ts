import type { SystemAgentChatParams, SystemAgentChatResult } from "@aether/gateway-protocol";
...
export class CustodianSessionStore {
  ...
  private synchronizeClient(): void {
    ...
    const chatSupported =
      client !== null && canCallGatewayMethod(snapshot, "aether.chat", "operator.admin");
    const chatUnsupported = isGatewayMethodAdvertised(snapshot, "aether.chat") === false;
  ...
  private async refreshTranscriptHistory(
    ...
    if (
      !context ||
      isGatewayMethodAdvertised(context.gateway.snapshot, "aether.chat.history") !== true
    ) {
      return false;
    }
  ...
  private async requestReply(
    ...
    const canRequest = () =>
      ...
      canCallGatewayMethod(context.gateway.snapshot, "aether.chat", "operator.admin");
    if (!canRequest()) {
      return "rejected";
    }
    this.requestAbort?.abort();
    ...
    try {
      ...
      const pending = client.request<SystemAgentChatResult>("aether.chat", params, {
