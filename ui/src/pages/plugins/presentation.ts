...
// unscoping would let third-party catalog ids claim bundled Aether art.
const AETHER_PLUGIN_ART_ID = /^@aether\/(.+?)(?:-(?:plugin|provider))?$/u;

export function pluginArtPath(id: string): string | null {
  const scopedSlug = AETHER_PLUGIN_ART_ID.exec(id)?.[1];
...
export function resolvePluginCatalogIconUrl(
  ...
  const artIdentity = plugin.packageName
    ? AETHER_PLUGIN_ART_ID.test(plugin.packageName)
      ? plugin.packageName
      : undefined
    : plugin.pluginId;
