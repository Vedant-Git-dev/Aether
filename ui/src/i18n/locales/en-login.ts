...
const enLogin = {
  login: {
    heading: "Connect to Aether",
    lede: "Enter the Gateway URL and secret, or open the one-time link that aether dashboard prints on the Gateway host.",
    gatewayUrl: "Gateway URL",
    secret: "Gateway secret",
    setupCodeHint:
      "This is a device setup code for the Aether mobile app, not the Gateway secret. Paste it in the app's Gateway settings instead; the Gateway secret comes from aether gateway auth-token --show on the Gateway host.",
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "Paste the token from aether gateway auth-token --show into Gateway secret.",
        stepPassword: "Type the configured Gateway password into Gateway secret.",
        stepGenerate:
          "If no token is configured, run aether doctor --generate-gateway-token on the gateway host.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Run aether dashboard --no-open for a fresh URL, or aether gateway auth-token --show to recover the token.",
      ...
      pairing: {
        ...
        stepDashboard:
          "Prefer a link? Run aether dashboard on the Gateway host and open the one-time URL it prints in this browser.",
        stepLatest:
      ...
      protocol: {
        ...
        stepDashboard:
          "Reopen the served dashboard with aether dashboard so the UI and Gateway come from the same install.",
        stepDevUi:
          "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
        stepRestart:
          "Restart the Gateway after updating Aether so it serves the current protocol.",
      },
      network: {
        title: "Gateway unreachable",
        summary:
          "The browser could not reach {host}. Check the address and transport before retrying credentials.",
        stepGateway: "Confirm the Gateway is running with aether status or aether gateway run.",
        stepUrl:
          "Check the Gateway URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
        stepDashboard:
          "Reopen the dashboard with aether dashboard --no-open to recopy the current URL and auth details.",
      },
    },
  },
