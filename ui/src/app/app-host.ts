...
import { AetherApp } from "./app-root.ts";
...
const APP_SIDEBAR_TAG = "aether-app-sidebar";
  ...
  implements ShellChromeHost, ShellGatewayHost, ShellNavigationHost, ShellViewHost
{
  ...
  readonly onboardingMemoryImportElement = {
    tagName: "aether-onboarding-memory-import",
  ...
  @query("aether-command-palette") commandPalette: CommandPaletteElement | undefined;
  @query("aether-exec-approval")
  ...
  private syncDocumentTitle() {
    ...
    let primaryContext = routeId === "custodian" ? t("nav.askAether") : titleForRoute(routeId);
  ...
  override updated(changed: PropertyValues<this>) {
    ...
    if (
      !customElements.get("aether-sidebar-update-card") &&
      this.querySelector("aether-sidebar-update-card")
    ) {
      this.loadSidebarUpdateCard();
    }
    const chatPage = this.querySelector<ChatPage>("aether-chat-page");
    if (chatPage) {
...
if (!customElements.get("aether-app")) {
  customElements.define("aether-app", AetherApp);
}
if (!customElements.get("aether-app-shell")) {
  customElements.define("aether-app-shell", AetherShell);
}
