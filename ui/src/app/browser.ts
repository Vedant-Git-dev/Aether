...
function readControlUiResourceBasePath(): string | null {
  const windowValue =
    typeof window === "undefined"
      ? undefined
      : (window as WindowWithControlUiBasePath)["__AETHER_CONTROL_UI_BASE_PATH__"];
  const value =
...
function nativeEmbedHistoryDepth(): number {
  ...
  if (state && typeof state === "object" && "aetherNativeEmbedDepth" in state) {
    const depth = state.aetherNativeEmbedDepth;
  ...
  return {
    ...
    push: (location) =>
      window.history.pushState(
        embedded ? { aetherNativeEmbedDepth: nativeEmbedHistoryDepth() + 1 } : {},
        "",
        writeLocation(location),
      ),
    replace: (location) =>
      window.history.replaceState(
        embedded ? { aetherNativeEmbedDepth: nativeEmbedHistoryDepth() } : {},
        "",
        writeLocation(location),
      ),
    listen: (listener) => {
