...
type NativeLinkPoster = (message: WebKitHostMessages["aetherLink"]) => void;

const NATIVE_UPDATE_DECLINED_EVENT = "aether:native-update-declined";
export const NATIVE_UPDATE_AVAILABILITY_CHANGED_EVENT =
  "aether:native-update-availability-changed";
const NATIVE_UPDATE_POSTED_EVENT = "aether:native-update-posted";
...
function getNativeLinkPoster(): NativeLinkPoster | undefined {
  ...
  const handler = webKitHostWindow()?.webkit?.messageHandlers?.aetherLink;
  return handler?.postMessage.bind(handler);
}

function getNativeUpdateHandler() {
  return webKitHostWindow()?.webkit?.messageHandlers?.aetherUpdate;
}
...
function menuContainer(event: Event): HTMLElement {
  ...
  const modalHost = path.find(
    (target) => target instanceof HTMLElement && target.localName === "aether-modal-dialog",
  );
...
export function startNativeLinkRouting(options: NativeLinkRoutingOptions = {}): NativeLinkRouting {
  ...
  const showMenu = async (
    ...
    const nextMenu = document.createElement("aether-native-link-menu") as NativeLinkMenu;
    nextMenu.x = x;
    nextMenu.y = y;
  ...
  const handleContextMenu = (event: MouseEvent) => {
    ...
    void showMenu(
      ...
      if (!disposed) {
        console.error("[aether] native link menu failed to load; right-click to retry", error);
      }
    });
  };
