...
import { buildControlUiFocusPath } from "@aether/session-url-contract";
...
declare global {
  interface Window {
    __AETHER_NATIVE_CONTROL_AUTH__?: NativeControlAuth;
  }
}
...
export function resolveApplicationStartupSettings(
  ...
  const nativeAuth =
    typeof window === "undefined" ? undefined : window["__AETHER_NATIVE_CONTROL_AUTH__"];
  if (nativeAuth) {
    try {
      delete window["__AETHER_NATIVE_CONTROL_AUTH__"];
    } catch {
      window["__AETHER_NATIVE_CONTROL_AUTH__"] = undefined;
    }
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      queryTokenUsed = true;
      console.warn(
        "[aether] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
