 ...
 * Handler: window.webkit.messageHandlers.aetherBrowser, Promise reply {ok:true,...}
 ...
 * Push: __AETHER_NATIVE_BROWSER__ plus aether:native-browser-state detail,
...
type NativeBrowserWindow = Window & {
  __AETHER_NATIVE_BROWSER__?: unknown;
  webkit?: {
    messageHandlers?: {
      aetherBrowser?: { postMessage(message: NativeBrowserMessage): Promise<unknown> };
    };
  };
};
const STATE_EVENT = "aether:native-browser-state";

function nativeWindow(): NativeBrowserWindow | undefined {
  return typeof window === "undefined" ? undefined : window;
}
function handler() {
  return nativeWindow()?.webkit?.messageHandlers?.aetherBrowser;
}
...
export function readNativeBrowserState(): NativeBrowserState | null {
  ...
  const value = nativeWindow()?.["__AETHER_NATIVE_BROWSER__"];
