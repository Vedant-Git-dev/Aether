...
} from "@aether/gateway-client/browser";
  ...
  const connect = (overrides: ApplicationGatewayConnectOptions = {}) => {
    ...
    if (configuredUiDevGateway() && !isConfiguredUiDevGateway(requestedGatewayUrl)) {
      ...
      setSnapshot({
        ...
        phase: "offline",
        lastError:
          "This development server targets a different Gateway. Restart ui:dev with AETHER_UI_DEV_GATEWAY_URL set to that Gateway, or reconnect to the configured Gateway.",
      });
      return;
    }
    ...
    const nextClient = createClient({
      ...
      clientName: options.clientOptions?.clientName ?? "aether-control-ui",
