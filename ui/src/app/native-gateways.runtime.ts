...
type NativeGatewaysWindow = Window & {
  __AETHER_NATIVE_GATEWAYS__?: unknown;
};

const NATIVE_GATEWAYS_CHANGED_EVENT = "aether:native-gateways-changed";
...
function createNativeGatewaysCapability(): NativeGatewaysCapability | null {
  ...
  const handler = webKitHostWindow()?.webkit?.messageHandlers?.aetherGateways;
  ...
  const postWithId = (type: "select" | "open-window" | "set-primary", id: string) =>
    post({ type, id });
  let snapshot = snapshotFrom(nativeWindow["__AETHER_NATIVE_GATEWAYS__"]);
