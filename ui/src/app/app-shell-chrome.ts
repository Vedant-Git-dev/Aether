...
export class ShellChromeOwner {
  ...
  connect(): void {
    ...
    for (const [type, listener] of [
      ...
      ["aether:native-toggle-sidebar", this.handleNativeToggleSidebar],
      ["aether:native-open-search", this.handleNativeOpenSearch],
      ["aether:native-toggle-search", this.handleNativeToggleSearch],
      ["aether:native-new-session", this.handleNativeNewSession],
      ["aether:native-navigate", this.handleNativeNavigate],
    ...
    Object.assign(window, { __AETHER_NATIVE_COMMANDS_READY__: true });
    window.dispatchEvent(new Event("aether:native-commands-state"));
  }
  ...
  disconnect(): void {
    ...
    if (listenerOwner && nativeCommandsOwner === listenerOwner) {
      ...
      Object.assign(window, { __AETHER_NATIVE_COMMANDS_READY__: false });
      window.dispatchEvent(new Event("aether:native-commands-state"));
    }
  }
  ...
  private readonly handleDocumentKeydownBubble = (event: KeyboardEvent): void => {
    ...
    if (host.navDrawerOpen && isMobileNavLayout() && !document.aetherModalLayers?.size) {
      event.preventDefault();
      host.closeNavDrawer({ restoreFocus: true });
    } else if (
  ...
  readonly handleDocumentKeydown = (event: KeyboardEvent): void => {
    const host = this.host;
    if (document.aetherModalLayers?.size) {
      return;
    }
  ...
  shouldIgnoreSettingsEscape(event: KeyboardEvent): boolean {
    ...
    if (
      ...
      document.aetherModalLayers?.size
    ) {
      return true;
    }
    const target = event.target;
