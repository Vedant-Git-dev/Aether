import { gatewayCredentialScope, gatewayOriginScope } from "@aether/gateway-client/browser";
import {
...
export function bootstrapApplication(): ApplicationRuntime {
  ...
  const nativeLinkRouting = startNativeLinkRouting({
    ...
    shouldOpenInControlUiBrowser: () =>
      ...
      document.querySelector("aether-app-shell")?.isConnected === true,
  });
  ...
  const navigateWithMode = (
    ...
    void navigationPromise.catch((error: unknown) => {
      console.error("[aether] route navigation failed", error);
    });
  ...
  return {
    ...
    start: () => {
      ...
      const nativeWindow = window as Window & {
        webkit?: {
          messageHandlers?: {
            aetherDeviceSettings?: { postMessage?: unknown };
            aetherNotifications?: { postMessage?: unknown };
          };
        };
      };
      if (
        typeof nativeWindow.webkit?.messageHandlers?.aetherNotifications?.postMessage ===
        "function"
      ) {
        steps.unshift(async () => {
      ...
      if (
        typeof nativeWindow.webkit?.messageHandlers?.aetherDeviceSettings?.postMessage ===
        "function"
      ) {
        steps.unshift(async () => {
      ...
      if (firstRunDefaultLanding) {
        steps.push(() => {
          ...
          startupLifecycle.trackDisposer(
            ...
            (error) => {
              console.error("[aether] initial session location failed", error);
            },
          );
        });
      }
