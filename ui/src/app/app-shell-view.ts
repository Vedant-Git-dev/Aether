...
export function renderApplicationShell(host: ShellViewHost) {
  ...
  const custodianPanelAvailable =
    ...
    canCallGatewayMethod(gatewaySnapshot, "aether.chat", "operator.admin");
    ...
    >
      ...
      ${
        nativeWebChrome && !onboarding
          ? html`
              <aether-macos-titlebar-controls
                ?inert=${navDrawerOpen}
      ...
      ${
        memoryImportActive && isOptionalElementDefined(host.onboardingMemoryImportElement)
          ? html`<aether-onboarding-memory-import
              .active=${true}
              .context=${context}
            ></aether-onboarding-memory-import>`
          : nothing
      }
      <aether-toast-host></aether-toast-host>
    </div>
  `;
