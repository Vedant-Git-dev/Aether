...
const RELOAD_GUARD_STORAGE_KEY = "aether.controlUi.staleChunkReloadBuildId";
...
export function reloadControlUiDocument(url = new URL(window.location.href)): void {
  ...
  url.searchParams.set("aether_mount_recovery", String(Date.now()));
...
export function installMissingStylesheetRecovery(
  ...
  const isCssApplied =
    deps.isCssApplied ??
    (() =>
      getComputedStyle(document.documentElement).getPropertyValue("--aether-css-ok").trim() ===
      "1");
