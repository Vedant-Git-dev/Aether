...
type NativeNotificationsWindow = Window & {
  __AETHER_NATIVE_NOTIFICATIONS__?: unknown;
};

// Wire contract with the Mac app's dashboard bridge (DashboardWindowController+Notifications.swift).
const NATIVE_NOTIFICATIONS_STATUS_EVENT = "aether:native-notifications-status";
...
function getNativeNotificationsPoster() {
  const handler = webKitHostWindow()?.webkit?.messageHandlers?.aetherNotifications;
...
export function createNativeNotificationsCapability(): NativeNotificationsCapability | null {
  ...
  let snapshot = snapshotFrom(nativeWindow["__AETHER_NATIVE_NOTIFICATIONS__"]) ?? {
