...
function measureRoutedRender<T>(routeId: string, render: () => T): T {
  ...
  if (durationMs >= 16) {
    console.debug("[aether] routed render", { routeId, durationMs });
  }
  return result;
}
...
class AetherRoutePresentation extends AetherLightDomElement {
...
> extends AetherLightDomElement {
  ...
  private readonly retainedPresentation = createRef<AetherRoutePresentation>();
      ...
      ${this.retainedUnmountGate.render(
        ...
        () =>
          retained
            ? keyed(
                retainedKey,
                html`<aether-route-presentation
...
if (!customElements.get("aether-route-presentation")) {
  customElements.define("aether-route-presentation", AetherRoutePresentation);
}

if (!customElements.get("aether-router-outlet")) {
  customElements.define("aether-router-outlet", AetherRouterOutlet);
}
