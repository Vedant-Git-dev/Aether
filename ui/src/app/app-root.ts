...
import { buildControlUiFocusPath, type ControlUiFocusTarget } from "@aether/session-url-contract";
...
export class AetherApp extends AetherLightDomElement {
  ...
  override connectedCallback() {
    ...
    void this.runtime
      ...
      .catch((error: unknown) => {
        console.error("[aether] application start failed", error);
      });
  }
  ...
  protected override firstUpdated(): void {
    if (this.runtime) {
      globalThis.dispatchEvent(new Event("aether-control-ui-rendered"));
    }
  }
  ...
  private renderApprovalDocument(runtime: ApplicationRuntime) {
    ...
    return html`<aether-approval-page .approvalId=${approvalId ?? ""}></aether-approval-page>`;
  }
  ...
  private renderQuestionDocument(runtime: ApplicationRuntime) {
    ...
    return html`<aether-question-page .questionId=${questionId ?? ""}></aether-question-page>`;
  }
  ...
  override render() {
    ...
    const gatewayUrlConfirmation = this.pendingGatewayUrl
      ? html`
          <aether-gateway-url-confirmation
            .props=${{
        ...
        `
      : nothing;
    return html`<aether-tooltip-provider>
      ${this.renderDocument(context, runtime)} ${gatewayUrlConfirmation}
    </aether-tooltip-provider>`;
  }
