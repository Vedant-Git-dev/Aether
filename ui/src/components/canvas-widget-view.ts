...
import type { CanvasDocumentViewResult } from "@aether/gateway-protocol";
...
export class AetherCanvasWidgetView extends AetherLightDomContentsElement {
  ...
  private postHostState(): void {
    ...
    frame.contentWindow?.postMessage({ type: "aether:widget-chat-host" }, this.sandboxOrigin);
  }
  ...
  private readonly handleMessage = (event: MessageEvent): void => {
    ...
    if (data?.type === "aether:widget-runtime-error") {
    ...
    if (data?.type === "aether:widget-bridge-ready") {
      this.postHostState();
    }
    if (data?.type !== "aether:widget-prompt-offer") {
      if (data?.type === "aether:widget-bridge-port-offer") {
        event.ports[0]?.close();
      }
      return;
    }
    ...
    port.addEventListener("message", (message: MessageEvent) => {
      if (
        this.isCurrent(binding) &&
        this.promptPort === port &&
        message.data?.type === "aether:widget-prompt"
      ) {
    ...
    port.start();
    port.postMessage({ type: "aether:widget-prompt-host-ready" });
  };
...
if (!customElements.get("aether-canvas-widget-view")) {
  customElements.define("aether-canvas-widget-view", AetherCanvasWidgetView);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-canvas-widget-view": AetherCanvasWidgetView;
  }
}
