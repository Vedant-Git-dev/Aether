...
class AetherMascot extends LitElement {
  static override styles = css`
    :host {
      display: inline-block;
      width: var(--aether-mascot-size, 120px);
      height: var(--aether-mascot-size, 120px);
      overflow: visible;
  ...
  protected override updated(changed: PropertyValues<this>): void {
    if (changed.has("size")) {
      this.style.setProperty("--aether-mascot-size", `${this.resolvedSize}px`);
    }
    if (changed.has("mood")) {
      this.animator.setMood(this.resolvedMood, currentSeconds());
    }
...
if (!customElements.get("aether-mascot")) {
  customElements.define("aether-mascot", AetherMascot);
}
