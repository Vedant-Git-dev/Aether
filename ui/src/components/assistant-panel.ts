...
export class AetherAssistantPanel extends AetherLightDomElement {
  ...
  private primaryChatPane(): ChatPaneElement | undefined {
    const root = this.closest("aether-app-shell") ?? this.parentElement;
    return [
      ...(root?.querySelectorAll<ChatPaneElement>(
        "aether-chat-pane.chat-pane-cache__pane--active",
      ) ?? []),
    ].find(
      (pane) =>
  ...
  private get targetStorageKey(): string {
    return `aether.assistant.panel.target.v1:${this.targetScope}`;
  }
  ...
  /** Ask Aether hydrates lazily; only refresh when it actually becomes visible. */
            ...
            ${(["home", "custodian"] as const).map((destination) =>
              (destination === "home" ? this.homeAvailable : this.custodianAvailable)
                ? html`<button
                  ...
                  >
                    ${t(destination === "home" ? "assistantPanel.home" : "nav.askAether")}
                  </button>`
                : nothing,
            )}
          </div>
...
if (!customElements.get("aether-assistant-panel")) {
  customElements.define("aether-assistant-panel", AetherAssistantPanel);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-assistant-panel": AetherAssistantPanel;
  }
}
