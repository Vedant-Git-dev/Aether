...
function buildFeedback(params: {
  ...
  const docsHref = params.docsHref ?? "https://docs.aether.ai/web/dashboard";
...
export function resolveLoginFailureFeedback(
  ...
  if (lastErrorCode === ConnectErrorDetailCodes.AUTHENTICATED_PROFILE_UNAVAILABLE) {
    return buildFeedback({
      ...
      docsHref: "https://docs.aether.ai/concepts/user-model#gateway-profile-and-github-credit",
    });
  }
  ...
  if (lastErrorCode === ConnectErrorDetailCodes.AUTH_VERIFIED_USER_REQUIRED) {
    return buildFeedback({
      ...
      docsHref: "https://docs.aether.ai/gateway/operator-scopes",
    });
  }
  ...
  if (lastErrorCode === ConnectErrorDetailCodes.CONTROL_UI_BUILD_MISMATCH) {
    return buildFeedback({
      ...
      stepKeys: [],
      docsHref: "https://docs.aether.ai/web/control-ui",
    });
  }
  ...
  if (pairing) {
    return buildFeedback({
      ...
      tone: "pending",
      rawError,
      docsHref: "https://docs.aether.ai/web/control-ui#device-pairing-first-connection",
      titleKey:
      ...
      primaryCommand: pairing.requestId
        ? `aether devices approve ${pairing.requestId}`
        : "aether devices approve --latest",
      stepKeys: [
        ...(pairing.requestId ? [] : ["login.failure.pairing.stepLatest"]),
        { key: "login.failure.pairing.stepDashboard", commands: ["aether dashboard"] },
  ...
  if (shouldShowInsecureContextHint(false, rawError, lastErrorCode)) {
    return buildFeedback({
      ...
      rawError,
      docsHref: "https://docs.aether.ai/web/control-ui#insecure-http",
  ...
  if (
    ...
    return buildFeedback({
      ...
      rawError,
      docsHref:
        "https://docs.aether.ai/web/control-ui/development#debugging%2Ftesting%3A-dev-server-%2B-remote-gateway",
  ...
  if (lower.includes("protocol mismatch")) {
    return buildFeedback({
      ...
      refreshAction: { label: t("login.failure.protocol.refresh") },
      stepKeys: [
        { key: "login.failure.protocol.stepDashboard", commands: ["aether dashboard"] },
  ...
  if (authHintKind === "trusted-proxy") {
    return buildFeedback({
      ...
      docsHref: "https://docs.aether.ai/gateway/trusted-proxy-auth",
    });
  }
        ...
        : [
            {
              ...
              commands: ["aether gateway auth-token --show"],
            },
            {
              key: "login.failure.authRequired.stepGenerate",
              commands: ["aether doctor --generate-gateway-token"],
            },
        ...
        : [
            {
              ...
              commands: ["aether dashboard --no-open", "aether gateway auth-token --show"],
            },
    ...
    stepKeys: [
      {
        ...
        commands: ["aether status", "aether gateway run"],
      },
      "login.failure.network.stepUrl",
      {
        key: "login.failure.network.stepDashboard",
        commands: ["aether dashboard --no-open"],
      },
    ],
    stepParams: { host },
  });
