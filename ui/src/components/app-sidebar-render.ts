...
function readSidebarNativeGateway(): SidebarNativeGateway | null {
  ...
  const snapshot = (
    window as Window & { __AETHER_NATIVE_GATEWAYS__?: SidebarNativeGatewaysSnapshot }
  )["__AETHER_NATIVE_GATEWAYS__"];
...
export function renderAppSidebarHomeRow(host: AppSidebarRenderHost) {
  ...
  return html`
    ...
    >
      ${
        attentionLabel
          ? html`<aether-tooltip .content=${attentionLabel}>${homeGlyph}</aether-tooltip>`
          : homeGlyph
      }
...
export function renderAppSidebarFooterBar(host: AppSidebarRenderHost) {
  ...
  return html`
    <div class="sidebar-footer-bar sidebar-footer-bar--one-action">
      ...
      >
        <aether-viewer-avatar .user=${avatarUser} variant="footer"></aether-viewer-avatar>
...
export function renderAppSidebarZoneEntry(
  ...
  const content =
    entry.type === "route"
      ...
      : entry.type === "plugin"
        ? html`<aether-plugin-contributions
            .kind=${"navigation"}
            .navigationKey=${entry.key}
          ></aether-plugin-contributions>`
...
function renderAppSidebarAttention(host: AppSidebarRenderHost) {
  return html`<aether-sidebar-attention
