...
export function installTitleTooltips(ownerDocument: Document) {
  let tooltip: HTMLElementTagNameMap["aether-tooltip"] | null = null;
  let active: {
  ...
  const discover = (event: Event) => {
    ...
    const explicit = elements.some((element) => element.localName === "aether-tooltip");
    ...
    if (!explicit && tooltipContent) {
      tooltip ??= ownerDocument.createElement("aether-tooltip");
      const mount =
        elements.find((element) => element.localName === "aether-modal-dialog") ??
        ownerDocument.body;
