import type { ProgressCard, ProgressCardGetParams } from "@aether/gateway-protocol";
...
export class SessionProgressHovercardProvider extends ReactiveElement {
  ...
  private activate(
    ...
    const agentId =
      ...
      target.closest<AppSidebarSessionNavigationElement>("aether-app-sidebar")?.expandedAgentId();
    if (!agentId) {
      return;
    }
  ...
  private showCurrent(): void {
    ...
    if (!card) {
      ...
      card = createPortaledHovercard(
        `aether-session-progress-hovercard-${nextHovercardId}`,
