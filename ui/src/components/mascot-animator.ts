  ...
  private basePose(mood: MascotMood, time: number): MascotPose {
    ...
    switch (mood) {
      ...
      case "working": {
        ...
        if (phase < 0.05) {
          pose.rightAetheregrees = -6;
        } else if (phase < 0.6) {
          pose.rightAetheregrees = -6 - 28 * easeInOut((phase - 0.05) / 0.55);
        } else if (phase < 0.72) {
          const strike = clamp((phase - 0.6) / 0.12);
          pose.rightAetheregrees = -34 + 46 * strike * strike;
        } else {
          pose.rightAetheregrees = 12 - 18 * easeInOut((phase - 0.72) / 0.28);
        }
        pose.leftAetheregrees = 4 + 2 * Math.sin(TAU * phase);
      ...
      case "celebrating": {
        ...
        pose.leftAetheregrees = 20 + 8 * clawWave;
        pose.rightAetheregrees = -20 + 8 * clawWave;
        pose.mouthCurve = 0.9;
        pose.mouthOpen = 0.35;
        pose.happyEyes = 0.7;
        pose.glowScale = 1.1;
  ...
  private applyGesture(gesture: Gesture, pose: MascotPose, progress: number): void {
    ...
    switch (gesture) {
      case "wave": {
        ...
        pose.rightAetheregrees += raised * (-28 + 9 * Math.sin(p * 6 * Math.PI));
      ...
      case "celebrate": {
        ...
        pose.leftAetheregrees += 38 * envelope;
        pose.rightAetheregrees += -38 * envelope;
      ...
      case "clawSnap":
        pose.leftAetheregrees += -8 * bell(clamp(p / 0.7));
        pose.rightAetheregrees += -8 * bell(clamp((p - 0.25) / 0.7));
        break;
      case "donHardHat": {
        ...
        pose.leftAetheregrees += -8 * ready;
        pose.rightAetheregrees += 8 * ready;
        break;
      }
      case "wipeBrow": {
        const envelope = plateau(p, 0.2, 0.8);
        pose.leftAetheregrees *= 1 - envelope;
        pose.rightAetheregrees *= 1 - envelope;
        pose.leftAetheregrees += 38 * envelope * (0.9 + 0.1 * Math.sin(p * 5 * Math.PI));
