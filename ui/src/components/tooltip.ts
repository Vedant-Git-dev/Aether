// Control UI adapter for Web Awesome tooltips. Aether keeps its terse
...
function createTooltipId() {
  return `aether-tooltip-${++nextTooltipId}`;
}

class TooltipProvider extends AetherLitElement {
...
class Tooltip extends AetherLitElement {
  ...
  static override styles = css`
    ...
    wa-tooltip {
      --max-width: var(--aether-tooltip-max-width, min(260px, calc(100vw - 16px)));
      --wa-tooltip-arrow-size: var(--aether-tooltip-arrow-size, 0px);
      --wa-tooltip-background-color: var(
        --aether-tooltip-background-color,
        color-mix(in srgb, var(--bg-elevated) 97%, var(--text) 3%)
      );
      --wa-tooltip-border-color: var(
        --aether-tooltip-border-color,
      ...
      --wa-tooltip-border-radius: var(--aether-tooltip-border-radius, var(--radius-md));
      --show-duration: var(--aether-tooltip-popup-show-duration, var(--wa-transition-fast));
      --hide-duration: var(--aether-tooltip-popup-hide-duration, var(--wa-transition-fast));
      font-family: var(--font-body);
    }

    wa-tooltip::part(body) {
      padding: var(--aether-tooltip-padding, 5px 7px);
      box-shadow: var(--aether-tooltip-shadow, var(--overlay-shadow, var(--shadow-md)));
      font-size: 11px;
      font-weight: 500;
      line-height: 1.25;
    ...
    :host(.sidebar-hover-tooltip) wa-tooltip[open]::part(base__popup) {
      animation: var(--aether-tooltip-open-animation);
    }
    ...
    @keyframes aether-tooltip-hover-card-in {
      from {
        opacity: 0;
  ...
  private attachTrigger() {
    ...
    while (owner) {
      const provider = owner.closest<TooltipProvider>("aether-tooltip-provider");
      if (provider) {
...
export function focusWithoutTooltip(target: HTMLElement | null | undefined) {
  const tooltip = target?.closest<Tooltip>("aether-tooltip");
  if (tooltip && target) {
...
if (!customElements.get("aether-tooltip-provider")) {
  customElements.define("aether-tooltip-provider", TooltipProvider);
}

if (!customElements.get("aether-tooltip")) {
  customElements.define("aether-tooltip", Tooltip);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-tooltip-provider": TooltipProvider;
    "aether-tooltip": Tooltip;
  }
}
