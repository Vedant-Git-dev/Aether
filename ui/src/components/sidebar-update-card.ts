...
class SidebarUpdateCard extends AetherLightDomContentsElement {
          ...
          actionable
            ? html`<div class="sidebar-update-card__actions">
                ${
                  this.canUpdate
                    ? updateAction
                    : html`<aether-tooltip open-on-click .content=${t("updates.adminRequired")}>
                        ${updateAction}
                      </aether-tooltip>`
                }
                ${
...
if (!customElements.get("aether-sidebar-update-card")) {
  customElements.define("aether-sidebar-update-card", SidebarUpdateCard);
}
