...
class McpServersCard extends AetherLightDomElement {
  @consume({ context: applicationContext, subscribe: true })
  private context?: ApplicationContext;

  @property() pluginsHref = "";

  @property() docsUrl = "https://docs.aether.ai/tools/mcp";
  ...
  private renderRow(server: McpServerSummary): TemplateResult {
    const command = `aether mcp ${server.auth === "oauth" ? "login" : "probe"} ${quoteShellArg(
      server.name,
    )}`;
    const meta = [
      server.transport,
...
if (!customElements.get("aether-mcp-servers-card")) {
  customElements.define("aether-mcp-servers-card", McpServersCard);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-mcp-servers-card": McpServersCard;
  }
}
