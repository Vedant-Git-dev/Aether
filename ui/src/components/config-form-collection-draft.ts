...
export function openCollectionDraft(event: Event, draftId: string): void {
  ...
  const draft = Array.from(
    block?.getElementsByTagName("aether-config-form-collection-draft") ?? [],
  ).find((child) => child.parentElement === block && child.id === draftId);
  draft?.openDraft?.();
}

export class ConfigFormCollectionDraft extends AetherLightDomElement {
...
if (!customElements.get("aether-config-form-collection-draft")) {
  customElements.define("aether-config-form-collection-draft", ConfigFormCollectionDraft);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-config-form-collection-draft": ConfigFormCollectionDraft;
  }
}
