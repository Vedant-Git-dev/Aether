...
class ViewerAvatar extends AetherLightDomContentsElement {
...
class ViewerFacepile extends AetherLightDomContentsElement {
  ...
  override render() {
    ...
    >
      ${visible.map(
        (user) => html`<aether-tooltip .content=${presenceViewerLabel(user)}>
          <span class="viewer-facepile__tooltip-anchor">
            ${renderStandalonePersonLink(
              html`<aether-viewer-avatar
                .user=${user}
                .identity=${user.identity}
                .markAsViewer=${!this.staticParticipants}
                variant="session"
              ></aether-viewer-avatar>`,
      ...
      ${
        overflowCount > 0
          ? html`<aether-tooltip .content=${overflowLabel}>
...
if (globalThis.customElements) {
  if (!customElements.get("aether-viewer-avatar")) {
    customElements.define("aether-viewer-avatar", ViewerAvatar);
  }
  if (!customElements.get("aether-viewer-facepile")) {
    customElements.define("aether-viewer-facepile", ViewerFacepile);
  }
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-viewer-avatar": ViewerAvatar;
    "aether-viewer-facepile": ViewerFacepile;
  }
}
