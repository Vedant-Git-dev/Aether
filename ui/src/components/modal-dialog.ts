...
const modalLayers = (document.aetherModalLayers ??= new Set<HTMLElement>());
...
export class AetherModalDialog extends AetherLitElement {
  ...
  static override styles = css`
    :host {
      ...
      --aether-modal-height-limit: var(--aether-modal-max-height, calc(100dvh - 48px));
      display: contents;
    }

    wa-dialog {
      --width: min(var(--aether-modal-width, 540px), calc(100vw - 48px));
      --spacing: 0;
      --backdrop-filter: var(--aether-modal-backdrop-filter, blur(4px));
    }

    wa-dialog::part(dialog) {
      max-width: var(--aether-modal-max-width, calc(100vw - 48px));
      max-height: var(--aether-modal-height-limit);
      padding: 0;
      border: 0;
    ...
    :host(.fullscreen) {
      --aether-modal-height-limit: calc(100dvh - 20px);
    }
    ...
    :host(.palette) wa-dialog {
      --aether-modal-backdrop-filter: none;
    ...
    :host(.drawer) wa-dialog {
      --width: min(var(--aether-modal-width, 100vw), 100vw);
      --show-duration: 200ms;
      --hide-duration: 0ms;
    }
    ...
    :host(.drawer) wa-dialog[open]::part(dialog) {
      animation: aether-drawer-in 200ms cubic-bezier(0.32, 0.72, 0, 1);
    }
    ...
    @media (max-width: 640px) {
      :host {
        --aether-modal-height-limit: 90dvh;
      }

      wa-dialog {
        --width: min(var(--aether-modal-width, 540px), calc(100vw - 24px));
      }

      wa-dialog::part(dialog) {
        max-width: var(--aether-modal-max-width, calc(100vw - 24px));
      }
    }
...
if (!customElements.get("aether-modal-dialog")) {
  customElements.define("aether-modal-dialog", AetherModalDialog);
}

declare global {
  interface Document {
    aetherModalLayers?: Set<HTMLElement>;
  }

  interface HTMLElementTagNameMap {
    "aether-modal-dialog": AetherModalDialog;
  }
}
