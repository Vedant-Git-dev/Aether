...
class SidebarAttention extends AetherLightDomElement {
  ...
  private async open(item: SidebarAttentionItem) {
    ...
    if (canCallGatewayMethod(snapshot, "aether.chat", "operator.admin")) {
...
if (!customElements.get("aether-sidebar-attention")) {
  customElements.define("aether-sidebar-attention", SidebarAttention);
}
