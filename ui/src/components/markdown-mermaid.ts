...
class AetherMermaid extends AetherLitElement {
...
if (!customElements.get("aether-mermaid")) {
  customElements.define("aether-mermaid", AetherMermaid);
}
...
export function mountMermaidBlocks(root: Element): boolean {
  ...
  for (const block of root.querySelectorAll(".markdown-mermaid")) {
    ...
    const diagram = document.createElement("aether-mermaid");
