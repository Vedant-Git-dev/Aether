...
class AetherBoardView extends AetherLightDomElement {
  ...
  private readonly handlePointerMove = (event: PointerEvent): void => {
    ...
    if (gesture.mode === "move") {
      ...
      const candidateTabId =
        tabTarget?.closest("aether-board-view") === this
      ...
      const targetName = pointerElement?.closest<
        HTMLElementTagNameMap["aether-board-widget-cell"]
      >("aether-board-widget-cell")?.widget?.name;
  ...
  private focusWidget(widget: BoardWidget, direction: BoardGridDirection): void {
    ...
    void this.updateComplete.then(() => {
      const cell = [...this.querySelectorAll("aether-board-widget-cell")].find(
...
if (!customElements.get("aether-board-view")) {
  customElements.define("aether-board-view", AetherBoardView);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-board-view": AetherBoardView;
  }
}
