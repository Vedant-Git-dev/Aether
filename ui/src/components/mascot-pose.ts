...
export type MascotPose = {
  ...
  antennaDroop: number;
  leftAetheregrees: number;
  rightAetheregrees: number;
...
export function clampMascotPose(pose: MascotPose): MascotPose {
  ...
  pose.leftAetheregrees = clamp(pose.leftAetheregrees, -45, 45);
  pose.rightAetheregrees = clamp(pose.rightAetheregrees, -45, 45);
...
export function staticMascotPose(mood: MascotMood): MascotPose {
  ...
  switch (mood) {
    ...
    case "celebrating":
      ...
      pose.mouthOpen = 0.4;
      pose.happyEyes = 0.8;
      pose.leftAetheregrees = 30;
      pose.rightAetheregrees = -30;
      break;
    case "sad":
