...
import type { HelloOk } from "@aether/gateway-protocol";
import { normalizeAgentId } from "@aether/normalization-core/agent-id";
import { buildControlUiSessionPath } from "@aether/session-url-contract";
...
export async function navigateToControlUiSession(page: Page, sessionKey: string): Promise<void> {
  const expectedPathname = await page.evaluate((sessionPath) => {
    const app = document.querySelector("aether-app") as HTMLElement & {
      runtime?: {
        context: {
          basePath: string;
    ...
    if (!app.runtime) {
      throw new Error("Aether application runtime is unavailable");
    }
  ...
  await page.waitForFunction(
    (targetSessionKey) =>
      [...document.querySelectorAll<HTMLElement>("aether-chat-pane")].some(
        (pane) =>
...
export function controlUiBundledSettingsStorageKey(baseUrl: string): string {
  return `aether.control.settings.v1:${controlUiBundledGatewayUrl(baseUrl)}`;
}
...
function installControlUiMockSameOriginGateway() {
  ...
  (
    window as Window & {
      ["__AETHER_NATIVE_CONTROL_AUTH__"]?: { gatewayUrl: string };
    }
  )["__AETHER_NATIVE_CONTROL_AUTH__"] = {
...
export async function waitForConfirmModal(page: Page): Promise<Locator> {
  await page.waitForFunction(() => {
    const modal = [...document.querySelectorAll("aether-modal-dialog")].at(-1);
  ...
  return page.locator("aether-modal-dialog").last();
}
...
export async function waitForControlUiSettingsTakeover(
  ...
  const appSidebar = page.locator("aether-app-sidebar");
...
async function installControlUiE2eUnhandledRejectionRing(page: Page): Promise<void> {
  ...
  await page.addInitScript(() => {
    const windowWithDiagnostics = window as Window & {
      __AETHER_CONTROL_UI_E2E_UNHANDLED_REJECTIONS__?: Array<{
        at: string;
        reason: unknown;
      }>;
    };
    const events: Array<{ at: string; reason: unknown }> = [];
    windowWithDiagnostics["__AETHER_CONTROL_UI_E2E_UNHANDLED_REJECTIONS__"] = events;
...
export async function reconnectMockGateway(
  ...
  await page.waitForFunction(
    (previousSocketCount) =>
      ((window as MockGatewayWindow).aetherControlUiE2eGateway?.socketCount() ?? 0) >
      previousSocketCount,
    socketCount,
    { timeout: controlUiE2eWaitTimeoutMs },
  );
...
export async function startControlUiE2eServer(
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      "globalThis.AETHER_CONTROL_UI_BUILD_INFO": JSON.stringify(resolvedBuildInfo),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function createBundledControlUiE2eConfig(
  ...
  return {
    ...
    define: {
      ...config.define,
      "globalThis.AETHER_CONTROL_UI_BUILD_INFO": JSON.stringify(
...
export async function buildProductionControlUiE2e(outDir: string, buildId: string): Promise<void> {
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    NODE_ENV: "production",
    AETHER_CONTROL_UI_BUILD_ID: buildId,
  };
...
function normalizeScenario(
  ...
  return {
    ...
    assistantName: scenario.assistantName?.trim() || "Aether",
    basePath,
...
export function createControlUiMockBootstrapConfig(scenario: ControlUiMockGatewayScenario = {}) {
  ...
  return {
    ...
    pluginFrameGrants: (normalizedScenario.pluginAssetsRequireAuth
      ...
      pluginId,
      path: `/__aether__/plugins/control-ui/${encodeURIComponent(pluginId)}/`,
      match: "prefix",
    })),
...
type MockGatewayWindow = Window & {
  __AETHER_CONTROL_UI_BASE_PATH__?: string;
  aetherControlUiE2eGateway?: ControlUiMockGateway;
};
...
function installControlUiMockGateway(
  ...
  if (scenario.communityInviteDismissed) {
    try {
      ...
      window.localStorage.setItem(
        "aether:control-ui:community-invite",
  ...
  const serverBuildIdStateKey = "aether.control-ui-e2e.serverBuildId";
  ...
  (window as MockGatewayWindow)["__AETHER_CONTROL_UI_BASE_PATH__"] = scenario.basePath;
  const protocolVersion = input.protocolVersion;
  const methodResponseOverridesStorageKey = "aether.control-ui-e2e.method-responses.v1";
  const canonicalSessionsStorageKey = "aether.control-ui-e2e.canonical-sessions.v1";
  ...
  const offlineStateKey = "aether.control-ui-e2e.gatewayOffline";
  ...
  const groupsStateKey = "aether.control-ui-e2e.sessionGroups";
  let groupsState: {
    names: string[];
  ...
  const configStateKey = "aether.control-ui-e2e.configState";
  ...
  type CommittedChatInput = {
    sessionId: string;
    runId: string;
    message: Record<string, unknown> & { __aether: { id: string; seq: number } };
  };
  const chatInputsStorageKey = "aether.control-ui-e2e.chatInputs";
  ...
  function sourceIdempotencyKey(message: unknown): unknown {
    ...
    const metadata = isRecord(message["__aether"]) ? message["__aether"] : undefined;
    return metadata?.idempotencyKey ?? message.idempotencyKey;
  }

  function messageSequence(message: unknown): number {
    const metadata =
      isRecord(message) && isRecord(message["__aether"]) ? message["__aether"] : null;
  ...
  function chatHistoryMessages(key: string): unknown[] {
    ...
    for (const source of committedChatInputs.filter((entry) => entry.sessionId === row.sessionId)) {
      ...
      const next = messages.findIndex(
        (message) => messageSequence(message) > source.message["__aether"].seq,
      );
    ...
    const sequence =
      Math.max(
        ...
        ...committedChatInputs
          ...
          .map((source) => source.message["__aether"].seq),
      ) + 1;
    ...
    if (media.length) {
      ...
      window.queueMicrotask(() => {
        emitGatewayEvent(MockWebSocket.latest, "session.message", {
          ...
          messageId: source.message["__aether"].id,
          messageSeq: source.message["__aether"].seq,
      ...
      return {
        ...
        inputReceipts: committedChatInputs
          ...
          .map((source) => ({
            runId: source.runId,
            state: "consumed",
            consumedByEventId: source.message["__aether"].id,
          })),
      };
    }
  ...
  function emitGatewayEvent(
    ...
    const approval = /^(exec|plugin|aether)\.approval\.(requested|resolved)$/u.exec(event);
  ...
  function buildResponse(method: string, params: unknown): unknown {
    ...
    switch (method) {
      ...
      case "skills.proposals.list":
        return {
          schema: "aether.skill-workshop.proposals-manifest.v1",
        ...
        const session: MockTerminalSession = {
          ...
          agentId:
            isRecord(params) && typeof params.agentId === "string"
              ...
              : scenario.defaultAgentId,
          shell: "/bin/zsh",
          cwd: scenario.workspace || "/workspace/aether",
          confined: false,
          attached: true,
          owner: "conn",
          createdAtMs: Date.now(),
  ...
  (window as MockGatewayWindow).aetherControlUiE2eGateway = exposed;
  ...
  const emitGatewayEvent = async (event: string, payload?: unknown) => {
    await page.evaluate(
      ({ eventName, eventPayload }) => {
        const gateway = (window as MockGatewayWindow).aetherControlUiE2eGateway;
        if (!gateway) {
...
async function captureControlUiE2eFailureDiagnosticsUnsafe(
  ...
  const configuredDir = process.env.AETHER_UI_E2E_DIAGNOSTIC_DIR?.trim();
  ...
  try {
    browserState = await page.evaluate(() => {
      ...
      const windowState = window as Window & {
        __AETHER_CONTROL_UI_E2E_UNHANDLED_REJECTIONS__?: unknown[];
        aetherControlUiE2eGateway?: MockGateway;
      };
      const app = document.querySelector("aether-app") as
        | (HTMLElement & { runtime?: Runtime })
        | null;
      const shell = document.querySelector("aether-app-shell") as
        | (HTMLElement & { runtime?: Runtime })
        | null;
      ...
      return {
        ...
        mockGateway: {
          installed: Boolean(windowState.aetherControlUiE2eGateway),
          requests: copy(windowState.aetherControlUiE2eGateway?.requests ?? []),
          socketStates: copy(windowState.aetherControlUiE2eGateway?.socketStates?.() ?? []),
          socketUrls: copy(windowState.aetherControlUiE2eGateway?.socketUrls?.() ?? []),
        },
        unhandledRejections: copy(
          windowState["__AETHER_CONTROL_UI_E2E_UNHANDLED_REJECTIONS__"] ?? [],
        ),
      };
    });
  } catch (evaluateError) {
