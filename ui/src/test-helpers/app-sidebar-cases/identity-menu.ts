...
describe("AppSidebar footer identity menu", () => {
  ...
  it("keeps a dismissed update as a discreet account-menu chip", async () => {
    ...
    sidebar.querySelector<HTMLButtonElement>(".sidebar-identity-card")?.click();
    await sidebar.updateComplete;
    const buildChip = sidebar.querySelector<HTMLElement>("aether-sidebar-build-chip");
  ...
  it("owns account utilities, restores focus, and routes Profile", async () => {
    ...
    expect(menu?.querySelector("aether-sidebar-build-chip")).not.toBeNull();
    expect(
      (menu?.querySelector("aether-sidebar-build-chip") as { variant?: string } | null)?.variant,
    ).toBe("identity");
    expect(menu?.querySelector("aether-theme-mode-toggle")).not.toBeNull();
    ...
    expect(
      ...
      "https://docs.aether.ai",
      "https://docs.aether.ai/help",
      "https://discord.gg/aether",
      "https://docs.aether.ai/releases",
    ]);
