...
describe("AppSidebar agent chip", () => {
  ...
  it("opens the agent-scoped menu with its inline roster", async () => {
    ...
    expect(menu?.querySelector("aether-sidebar-build-chip")).toBeNull();
    expect(menu?.querySelector("aether-theme-mode-toggle")).toBeNull();
    expect(
  ...
  it("opens the agent menu on right-click without toggling an open menu", async () => {
    ...
    const card = sidebar.querySelector<HTMLElement>("aether-sidebar-agent-card");
