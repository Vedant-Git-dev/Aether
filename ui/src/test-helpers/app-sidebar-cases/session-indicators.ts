...
describe("AppSidebar session indicators", () => {
  ...
  it("prioritizes session icons, then channel avatars, then owner chips", async () => {
    ...
    for (const row of result.sessions) {
      ...
      if (row.key !== ownerKey) {
        row.channelAvatarUrl = `/__aether__/channel-avatar/${encodeURIComponent(row.key)}`;
      }
    }
    ...
    expect(icon?.querySelector("aether-channel-avatar")).toBeNull();
    ...
    expect(owner?.querySelector("aether-channel-avatar")).toBeNull();
    expect(owner?.querySelector(".session-owner-chip")).not.toBeNull();
    expect(fetchMock).toHaveBeenCalledWith(
      `/__aether__/channel-avatar/${encodeURIComponent(avatarKey)}`,
      {
  ...
  it("refetches a mounted channel avatar when its route revision changes", async () => {
    ...
    const missingUrl = `/__aether__/channel-avatar/${encodeURIComponent(avatarKey)}?v=old`;
    const restoredUrl = `/__aether__/channel-avatar/${encodeURIComponent(avatarKey)}?v=new`;
  ...
  it("keeps the owner chip when avatar auth is not ready", async () => {
    ...
    row.channelAvatarUrl = `/__aether__/channel-avatar/${encodeURIComponent(avatarKey)}`;
    row.createdActor = { type: "human", id: "profile-ada", label: "Ada" };
    row.owner = { actor: row.createdActor };
    result.owners = [
      { type: "human", id: "profile-ada", label: "Ada" },
    ...
    expect(pendingRow?.querySelector("aether-channel-avatar")).not.toBeNull();
