...
describe("AppSidebar viewer presence", () => {
  ...
  it("shows only other online identities with active-first ordering and idle dimming", async () => {
    ...
    expect(localStorage.getItem("aether:sidebar:sessions:collapsed-sections")).toBe(
  ...
  it.each(["disconnect", "switch", "route", "collapse", "shell", "remove", "teardown"] as const)(
    ...
    async (reason) => {
      ...
      await vi.dynamicImportSettled();
      await vi.waitFor(() =>
        expect(document.querySelector("aether-elapsed-time")?.textContent).toBeTruthy(),
      );
      const elapsed = document.querySelector<LitElement>("aether-elapsed-time")!;
  ...
  it("restores the collapsed online section", async () => {
    ...
    const facepile = sidebar.querySelector<HTMLElement>(".sidebar-online aether-viewer-facepile");
  ...
  it("renders the self user's avatar route in the footer identity chip", async () => {
    ...
    await vi.waitFor(() => {
      const avatar = sidebar.querySelector<HTMLImageElement>(
        ".sidebar-identity-card aether-viewer-avatar img",
      );
  ...
  it("groups identified viewers for session rows and keeps the footer identity-only", async () => {
    ...
    const sessionFacepile = sidebar.querySelector<HTMLElement>(
      '[data-session-key="agent:main:work"] aether-viewer-facepile',
    );
    ...
    expect(
      [...(sessionFacepile?.querySelectorAll("aether-tooltip") ?? [])].map(
    ...
    const avatar = identityCard?.querySelector<HTMLImageElement>("aether-viewer-avatar img");
    expect(avatar?.getAttribute("src")).toBe("/api/users/00-self/avatar?v=1");
    const footer = sidebar.querySelector(".sidebar-footer-bar");
    expect(footer?.querySelector("aether-viewer-facepile")).toBeNull();
    expect(footer?.querySelector("aether-sidebar-build-chip")).toBeNull();
