...
type SidebarNativeGatewayTestWindow = Window & {
  __AETHER_NATIVE_WEB_CHROME__?: boolean;
  __AETHER_NATIVE_GATEWAYS__?: SidebarNativeGatewayTestSnapshot;
};
...
function setNativeGatewayTestState(snapshot: SidebarNativeGatewayTestSnapshot): void {
  ...
  nativeWindow["__AETHER_NATIVE_WEB_CHROME__"] = true;
  nativeWindow["__AETHER_NATIVE_GATEWAYS__"] = snapshot;
}

afterEach(() => {
  const nativeWindow = window as SidebarNativeGatewayTestWindow;
  Reflect.deleteProperty(nativeWindow, "__AETHER_NATIVE_WEB_CHROME__");
  Reflect.deleteProperty(nativeWindow, "__AETHER_NATIVE_GATEWAYS__");
...
describe("AppSidebar gateway footer subtitle", () => {
  ...
  it("stays hidden outside native chrome", async () => {
    const nativeWindow = window as SidebarNativeGatewayTestWindow;
    nativeWindow["__AETHER_NATIVE_GATEWAYS__"] = twoGateways;
    const gateway = createGateway({} as GatewayBrowserClient);
    const { sidebar } = await mountSidebar(gateway, createSessions("main", ["agent:main:main"]));
  ...
  it("shows the visible offline retry pill instead of a hidden reconnecting subtitle", async () => {
    ...
    expect(
      (status?.closest("aether-tooltip") as (HTMLElement & { content?: string }) | null)?.content,
  ...
  it("updates when the native gateway snapshot changes", async () => {
    ...
    window.dispatchEvent(new CustomEvent("aether:native-gateways-changed"));
    await sidebar.updateComplete;
