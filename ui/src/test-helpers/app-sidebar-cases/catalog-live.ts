...
describe("AppSidebar session catalog pagination", () => {
  ...
  it.each([
    ...
    expect(
      (pullRequestBadge?.closest("aether-tooltip") as (HTMLElement & { content?: string }) | null)
        ?.content,
  ...
  it("shows a catalog-owned Aether session only in its catalog section", async () => {
    const gateway = createGateway({} as GatewayBrowserClient);
    ...
    await sidebar.updateComplete;
    const linkedMenu = sidebar.querySelector<TestSessionMenu>("aether-session-menu");
