...
describe("AppSidebar session ownership", () => {
  it("renders durable actor avatars identically regardless of live presence", async () => {
    ...
    await waitForFast(() => {
      expect(
        sidebar.querySelector('[data-session-key="agent:main:ada"] aether-viewer-avatar img'),
      ).not.toBeNull();
      expect(
        sidebar.querySelector('[data-session-key="agent:main:bob"] aether-viewer-avatar img'),
      ).not.toBeNull();
    });
    const bobAvatarBefore = sidebar
      .querySelector('[data-session-key="agent:main:bob"] aether-viewer-avatar img')
      ?.getAttribute("src");
    expect(
      sidebar
  ...
  it("keeps an owner filter through transient or narrowed owner facets", async () => {
    ...
    expect(sidebar.querySelectorAll("aether-session-owner-chip")).toHaveLength(1);
    const menu = await openOwnerMenu(sidebar);
  ...
  it("shows the authenticated user first in the owner filter", async () => {
    ...
    expect(ownerRows[0]?.querySelector("aether-session-owner-chip")).not.toBeNull();
  });
  ...
  it("renders no ownership chrome when the listed sessions have fewer than two owners", async () => {
    ...
    expect(sidebar.querySelector("aether-session-owner-chip")).toBeNull();
  });
  ...
  it("groups sessions by owner based on the live session-owner roster", async () => {
    ...
    expect(
      ownerSections()[0]?.querySelector("aether-viewer-avatar")?.getAttribute("aria-hidden"),
    ).toBe("true");
    expect(
      ownerSections()[0]
  ...
  it("shows archive attribution only in collaborative archived-session lists", async () => {
    ...
    expect(
      sidebar.querySelector('aether-session-owner-chip span[title="Archived by Bob"]'),
    ).not.toBeNull();
    ...
    const archivedFacepile = sidebar.querySelector(
      '[data-session-key="agent:main:archived"] aether-viewer-facepile',
    ) as HTMLElementTagNameMap["aether-viewer-facepile"] | null;
