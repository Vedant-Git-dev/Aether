...
describe("AppSidebar session ownership filtering", () => {
  it("uses the complete owner facet instead of raw loaded-row principals", async () => {
    ...
    expect(
      sidebar.querySelector(
        '[data-session-key="agent:main:opaque-profile"] aether-session-owner-chip',
      ),
    ).not.toBeNull();
    expect(
      sidebar.querySelector(
        '[data-session-key="agent:main:discord-channel"] aether-session-owner-chip',
      ),
    ).toBeNull();
    expect(
      sidebar.querySelector(
        '[data-session-key="agent:main:session-principal"] aether-session-owner-chip',
      ),
    ).toBeNull();
  ...
  it("filters by effective owner and hides custom groups without matching sessions", async () => {
    ...
    expect(sidebar.querySelectorAll("aether-session-owner-chip")).toHaveLength(2);
  ...
  it("badges the owner avatar and announces unread once", async () => {
    ...
    expect(row?.querySelector(".session-glyph aether-session-owner-chip")).not.toBeNull();
  ...
  it("keeps owner avatars off child rows", async () => {
    ...
    expect(
      sidebar.querySelector(`[data-session-key="${parentKey}"] aether-session-owner-chip`),
    ).not.toBeNull();
    expect(
      sidebar.querySelector(`[data-session-key="${childKey}"] aether-session-owner-chip`),
    ).toBeNull();
    expect(
