...
function contentControlsDocument(): string {
  ...
  <input aria-label="Release note" placeholder="Release note"></main><script>
    const report=()=>parent.postMessage({type:"aether:widget-size",height:document.body.scrollHeight},"*");
...
class BoardFixture extends AetherLightDomElement {
...
if (!customElements.get("aether-board-fixture")) {
  customElements.define("aether-board-fixture", BoardFixture);
}

document.querySelector("#app")?.append(document.createElement("aether-board-fixture"));
