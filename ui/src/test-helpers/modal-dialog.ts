...
import type { AetherModalDialog } from "../components/modal-dialog.ts";
...
export function createModalDialogTestFixture(
  ...
  const captureModals = () => {
    for (const modal of document.body.querySelectorAll("aether-modal-dialog")) {
      modals.add(modal);
    }
  };
...
export function waitForConfirmDialogActions(): Promise<HTMLElement> {
  return waitForDialog(() => {
    const actions = document.body.querySelector<HTMLElement>(
      "aether-modal-dialog .exec-approval-actions",
    );
    if (!actions) {
...
export async function waitForRenderedModalDialog(container: HTMLElement) {
  await waitForDialog(() => {
    if (!container.querySelector("aether-modal-dialog")) {
      throw new Error("Expected aether-modal-dialog");
    }
  });
...
export async function waitForInputDialog(): Promise<HTMLInputElement> {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const input = document.body.querySelector("aether-modal-dialog input");
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<AetherModalDialog>("aether-modal-dialog");
