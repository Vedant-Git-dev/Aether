...
export async function dispatchOpenAiTalkEvent(page: Page, event: unknown) {
  await page.evaluate((payload) => {
    const channel = (
      window as Window & { aetherVideoTalkE2e?: { peer: { channel: EventTarget } } }
    ).aetherVideoTalkE2e?.peer.channel;
    if (!channel) {
...
function installTalkMediaBrowserFixture(profile: TalkMediaFixtureProfile) {
  ...
  const trackWindow = window as Window & { aetherVideoTalkTracks?: MediaStreamTrack[] };
  const recordTracks = (stream: MediaStream) => {
    trackWindow.aetherVideoTalkTracks = [
      ...(trackWindow.aetherVideoTalkTracks ?? []),
      ...stream.getTracks(),
    ];
  };
  ...
  Object.defineProperty(window, "aetherTalkE2eState", {
    configurable: true,
    value: state,
  });
}
  ...
  await page.addInitScript(() => {
    ...
    class FakePeerConnection extends EventTarget {
      ...
      constructor() {
        ...
        ).aetherVideoTalkE2e = { dataChannelCreated: false, peer: this };
      }
      ...
      createDataChannel() {
        const harness = (
          window as Window & {
            aetherVideoTalkE2e?: { dataChannelCreated: boolean };
          }
        ).aetherVideoTalkE2e;
        if (harness) {
...
export async function installMicrophoneLossWebRtcFixture(page: Page) {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, "aetherMicrophoneLossE2e", { value: proof });
...
async function installWebRtcSdpResponseFixture(page: Page, fixture: WebRtcSdpResponseFixture) {
  await page.addInitScript(() => {
    const proofWindow = window as Window & { aetherWebRtcSdpE2e?: WebRtcSdpE2eProof };
    ...
    proofWindow.aetherWebRtcSdpE2e = {
      bodyCancelCount: 0,
    ...
    window.fetch = async (input, init) => {
      ...
      const proof = proofWindow.aetherWebRtcSdpE2e;
