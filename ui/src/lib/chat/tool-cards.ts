...
function resolveTranscriptMessageId(message: Record<string, unknown>): string | undefined {
  ...
  const aetherMeta = message["__aether"];
  const transcriptMeta = asNullableRecord(aetherMeta);
...
function extractToolCards(message: unknown): ToolCard[] {
  ...
  const isLiveToolStream = m["__aetherToolStreamLive"] === true;
  const liveDiff = readRecord(m["__aetherToolStreamDiffStat"]);
  const liveDiffStat =
  ...
  for (let index = 0; index < content.length; index++) {
    ...
    if (isToolCallContentBlock(item)) {
      ...
      cards.push({
        ...
        ...(isLiveToolStream
          ? { live: true, completed: m["__aetherToolStreamResultReceived"] === true }
          : {}),
