...
import { mediaKindFromMime } from "@aether/media-core/constants";
import {
  asFiniteNumber,
...
export function resolveMessageSenderLabel(
  ...
  return formatSenderLabel(
    sender === undefined ? resolveMessageSender(asOptionalRecord(m?.["__aether"])) : sender,
  );
}
...
export function normalizeMessage(message: unknown): NormalizedMessage {
  ...
  const delivery = isAssistantMessage ? readMessageDelivery(m.aetherDelivery) : undefined;
  ...
  const aetherMeta = asOptionalRecord(m["__aether"]);
  const structuredReplyToId = readStringField(aetherMeta, "replyToId")?.trim() ?? "";
  if (structuredReplyToId) {
    replyTarget = { kind: "id", id: structuredReplyToId };
  }
  const replyPreviewRecord = asOptionalRecord(aetherMeta?.replyToPreview);
  ...
  const metaSender = resolveMessageSender(aetherMeta);
