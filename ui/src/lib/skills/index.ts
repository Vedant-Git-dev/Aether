import type { SkillsDetailResult, SkillsSecurityVerdictsResult } from "@aether/gateway-protocol";
...
export async function loadClawHubSecurityVerdicts(state: SkillsState, report: SkillStatusReport) {
  ...
  try {
    const response = await client.request<{
      schema: "aether.skills.security-verdicts.v1";
...
export async function saveSkillApiKey(
  ...
  await runSkillConfigUpdate(
    ...
    skillKey,
    { apiKey },
    `API key saved — stored in aether.json (skills.entries.${skillKey})`,
    canDispatch,
  );
}
