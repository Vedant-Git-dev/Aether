...
function activeModalToastLayer() {
  return [...(document.aetherModalLayers ?? [])].findLast((candidate) => candidate.isConnected);
}
...
class AetherToastHost extends AetherLightDomContentsElement {
  ...
  override disconnectedCallback() {
    ...
    if (this.parentElement?.localName === "aether-modal-dialog" && target) {
      target.append(this);
    } else {
...
export function showToast(options: ToastOptions): boolean {
  ...
  const host = document.querySelector<AetherToastHost>("aether-toast-host");
  if (!host) {
    queuedToast = options;
    return false;
  }
...
if (typeof customElements !== "undefined" && !customElements.get("aether-toast-host")) {
  customElements.define("aether-toast-host", AetherToastHost);
}

declare global {
  interface HTMLElementTagNameMap {
    "aether-toast-host": AetherToastHost;
  }
}
