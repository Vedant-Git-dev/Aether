...
export class BoardWidgetSandboxHost {
  ...
  handleMessage(event: MessageEvent): void {
    ...
    if (event.data?.type === "aether:widget-prompt-offer") {
      event.ports[0]?.close();
      return;
    }
    if (event.data?.type === "aether:widget-bridge-port-offer") {
    ...
    if (event.data?.type === "aether:widget-bridge-ready") {
      this.postHostInit();
    }
  ...
  private handleBridgeMessage(data: unknown): void {
    ...
    if (
      data &&
      typeof data === "object" &&
      Reflect.get(data, "type") === "aether:widget-host-init-ack" &&
  ...
  private postResponse(id: string, ok: boolean, result?: unknown, error?: string): void {
    this.bridgePort?.postMessage({
      type: "aether:widget-bridge-response",
      id,
      ok,
