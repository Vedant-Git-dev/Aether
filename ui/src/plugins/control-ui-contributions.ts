...
const PLUGIN_MANAGER_DIALOG = {
  tagName: "aether-plugin-manager-dialog",
  get label() {
...
export function renderControlUiPluginRecovery(plugins: ControlUiPluginCapability, routeId: string) {
  ...
  return routeId !== "plugins" || plugins.selectedReplacement("workspace")
    ? html`<aether-plugin-manager floating></aether-plugin-manager>`
    : nothing;
}

class ControlUiPluginManager extends AetherLightDomContentsElement {
  ...
  override render() {
    ...
    ${
      isOptionalElementDefined(PLUGIN_MANAGER_DIALOG)
        ? html`<aether-plugin-manager-dialog
            .runtime=${this.context?.plugins}
            .open=${showDialog}
            @modal-cancel=${() => {
              this.open = false;
            }}
          ></aether-plugin-manager-dialog>`
        : nothing
    }`;
  }
}

if (!customElements.get("aether-plugin-manager")) {
  customElements.define("aether-plugin-manager", ControlUiPluginManager);
}
