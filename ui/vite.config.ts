...
function normalizeBuildTimestamp(value: string | undefined): string | null {
  ...
  if (!timestamp) {
    throw new Error("AETHER_BUILD_TIMESTAMP must be a valid UTC ISO-8601 timestamp ending in Z");
  }
  return timestamp;
}
...
export function resolveControlUiBuildInfo(
  ...
  const commitAt = commit
    ...
    : null;
  const builtAt = normalizeBuildTimestamp(env.AETHER_BUILD_TIMESTAMP);
  ...
  const releaseFlag = env.AETHER_CONTROL_UI_RELEASE_BUILD?.trim();
  if (releaseFlag && releaseFlag !== "1") {
    throw new Error("AETHER_CONTROL_UI_RELEASE_BUILD must be 1 when set");
  }
  const release = releaseFlag === "1";
  const metadata = { version, commit, builtAt, release };
  const explicitBuildId = env.AETHER_CONTROL_UI_BUILD_ID?.trim();
  return {
    ...metadata,
    commitAt,
    branch,
...
function controlUiBuildOutputPlugin(buildId: string, buildOutDir: string): Plugin {
  ...
  return {
    ...
    writeBundle() {
      ...
      const placeholder = '"__AETHER_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(
  ...
  const envBase = process.env.AETHER_CONTROL_UI_BASE_PATH?.trim();
  ...
  const devGateway =
    options.command === "serve"
      ? createControlUiDevGateway(process.env.AETHER_UI_DEV_GATEWAY_URL)
      : undefined;
  const buildOutDir = options.outDir ?? outDir;
  return {
    base,
    define: {
      "globalThis.AETHER_CONTROL_UI_BUILD_INFO": JSON.stringify(buildInfo),
      "globalThis.AETHER_UI_DEV_GATEWAY": devGateway
