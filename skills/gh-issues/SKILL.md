...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${AETHER_CONFIG_PATH:-${AETHER_STATE_DIR:-$HOME/.aether}/aether.json}"
...
```text
${AETHER_STATE_DIR:-$HOME/.aether}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with aether message send if route provided.
```
