...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.aether`, `$AETHER_STATE_DIR`, or active Aether state dirs.
...
- Worker must send completion/failure via `aether message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/aether`; use an isolated checkout.
...
6. Launch the worker in the isolated worktree, never the primary checkout. For Aether, the primary checkout under `~/Projects/aether` remains forbidden.
...
aether message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use aether system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t aether-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
...
OpenAI auth overrides. Never export the worker home into the Aether Gateway
environment.
