---
name: node-connect
description: "Diagnose Aether Control UI browser and native Android, iOS, or macOS node connection failures across route, auth, pairing, QR/setup-code, and reconnect states."
---
...
Record the target environment/profile, Aether binary, config/state root, Gateway URL/port, and service before changing anything.

- Use the named deployment wrapper/profile for every config, log, service, device, and node command.
- Never drift to a bare `aether`, proof environment, or similarly named deployment.
...
- **Control UI browser:** the user says browser, dashboard, Control UI, or webchat; logs show `client=aether-control-ui` or `mode=webchat`.
...
A phone can be either client. Do not use `aether qr` or `aether nodes status` for a phone browser; those belong to native mobile/node pairing.
...
```bash
aether gateway status --deep
aether logs --follow --json
aether devices list
aether config get gateway.mode
aether config get gateway.bind
aether config get gateway.remote.url
aether config get gateway.auth.mode
aether config get gateway.auth.allowTailscale
aether config get gateway.tailscale.mode
```
...
- `gateway.tailscale.mode=off` means Aether is not managing Serve/Funnel. It does not prove that Tailscale or an externally managed Serve route is absent.
- When Tailscale is involved, inspect live state rather than inferring it from Aether config:
...
- Prefer `aether dashboard` on the Gateway host for a one-time signed handoff. Use `--no-open` only when the operator can retrieve that host's clipboard, and keep the host browser/clipboard outside agent tooling. Never capture `dashboard --json`: it can expose the handoff and shared credentials. Never relay, rewrite, or send a loopback handoff URL to a remote phone.
...
```bash
aether qr --json | jq '{gatewayUrl, gatewayUrls, auth, access, accessDowngraded, urlSource}'
```
...
Verify `gatewayUrl` and `urlSource`. The setup code is password-equivalent: have the operator copy it from **Control UI → Devices → Pair device**, or run `aether qr --setup-code-only` in a terminal outside agent tooling and paste it directly into the official app. Never relay it through agent/chat/tool output. Generate a fresh code after a URL/auth fix or expiry.

If the app reports `pairing required`:

```bash
aether devices list
aether devices approve --latest   # preview only; exits without approval
aether devices approve <requestId>
aether nodes status
```
...
- the browser completes initial requests, or the native node appears in `aether nodes status`;
