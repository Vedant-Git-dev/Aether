...
Prefer `aether dashboard` on the Gateway host. It creates a short-lived local
...
When the browser reaches the Gateway host directly, Aether can derive the
...
```bash
aether gateway --tailscale serve
```

Serve keeps the Gateway on loopback and publishes the Control UI and WebSocket
over HTTPS. With `gateway.auth.allowTailscale: true`, Aether can accept a
