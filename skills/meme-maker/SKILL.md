...
metadata: { "aether": { "emoji": "🖼️", "requires": { "bins": ["node"] } } }
---
...
- If PNG output fails because the browser is unavailable, use `--out meme.svg` (or another `.svg` path). Do not `npm install` packages into the skill directory or the Aether tree to enable PNG.
