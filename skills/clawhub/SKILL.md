...
Use `aether skills` to discover and manage skills for the current Aether
...
```bash
aether skills search "postgres backups"
```
...
```bash
aether skills verify my-skill
aether skills install my-skill
aether skills install my-skill --version 1.2.3
```
...
aether skills list
aether skills check
aether skills update my-skill
aether skills update --all
```
...
Use the original agent workspace for agent-specific skills or the Aether
state directory for skills installed with `--global`:

```bash
clawhub --workdir /path/to/agent-workspace uninstall @owner/my-skill
clawhub --workdir ~/.aether uninstall @owner/my-skill
```

If `AETHER_STATE_DIR` is set, use its value instead of `~/.aether`:

```bash
clawhub --workdir "$AETHER_STATE_DIR" uninstall @owner/my-skill
```
...
- `aether skills install` installs into the active workspace by default.
