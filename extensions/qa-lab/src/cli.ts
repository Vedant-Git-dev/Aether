...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. aether,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. aether,codex", "aether,codex")
    .option(
      "--provider-mode <mode>",
  ...
  credentials
    ...
    .option("--site-url <url>", "Override AETHER_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override AETHER_QA_CONVEX_ENDPOINT_PREFIX")
  ...
  qa.command("docker-scaffold")
    ...
    .option("--image <name>", "Prebaked image name", "aether:qa-local-prebaked")
  ...
  qa.command("docker-build-image")
    ...
    .option("--repo-root <path>", "Repository root to target when running from a neutral cwd")
    .option("--image <name>", "Image tag", "aether:qa-local-prebaked")
