...
import {
  startAetherCrablineAdapter,
  type AetherCrablineChannelDriverSelection,
  type AetherCrablineInbound,
  type StartedAetherCrablineAdapter,
  type StartedAetherCrablineCorrelatedAdapter,
...
function normalizeCrablineSignalGatewayConfig(config: AetherConfig): AetherConfig {
...
async function postCrablineInbound(params: {
  adapter: StartedAetherCrablineAdapter;
  providerInbound: AetherCrablineInbound;
}) {
...
class QaCrablineTransport extends QaStateBackedTransportAdapter {
  readonly #adapter: StartedAetherCrablineCorrelatedAdapter;
  readonly #selection: AetherCrablineChannelDriverSelection;
  ...
  constructor(params: {
    adapter: StartedAetherCrablineCorrelatedAdapter;
    transportPolicy?: QaTransportPolicy;
    selection: AetherCrablineChannelDriverSelection;
  ...
  createGatewayConfig = (params: { baseUrl: string }): QaTransportGatewayConfig => {
    const rawConfig = this.#adapter.createGatewayConfig(params) as AetherConfig;
    const config =
  ...
  createReportNotes = (_params: QaTransportReportParams) => [
    `Runs Aether's ${this.#selection.channel} channel plugin against a Crabline local provider server.`,
...
export async function createQaCrablineTransportAdapter(params: {
  ...
  const adapter = await startAetherCrablineAdapter({
