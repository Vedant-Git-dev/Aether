...
export function resolveQaGatewayChildCommand(repoRoot: string): QaGatewayChildCommand {
  ...
  throw new Error(
    "Aether CLI entry not found: expected scripts/run-node.mjs or dist/index.(m)js",
  );
}
...
async function readQaGatewayCliCommand(
  ...
  if (failure || exitCode !== 0) {
    ...
    const reason = failure?.message ?? `Aether CLI exited ${exitCode}`;
