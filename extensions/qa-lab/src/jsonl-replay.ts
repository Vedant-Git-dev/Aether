...
type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { aether: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { aether: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      aether: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.aether.push(parity.cells.aether);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# Aether JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
