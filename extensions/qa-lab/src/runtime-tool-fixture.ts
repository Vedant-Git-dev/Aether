...
import { loadTranscriptEventsSync } from "aether/plugin-sdk/session-store-runtime";
import {
  asBoolean,
  isRecord,
  ...
  if (Array.isArray(rawContent)) {
    for (const block of rawContent) {
      ...
      calls.push({
        ...
        tool,
        // Aether mirrors provider arguments separately; a placeholder input
...
function formatCodexNativeWorkspaceDetails(params: {
  ...
  return [
    `codex-native-workspace ${params.toolName}: Aether dynamic exposure is intentionally omitted because Codex owns this workspace operation natively`,
    params.reason ? `reason: ${params.reason}` : undefined,
    `available Aether dynamic tools: ${[...params.tools].toSorted().join(", ")}`,
    params.happyRequest
...
export async function runRuntimeToolFixture(
  ...
  const forcedCodexNativeWorkspace =
    env.gateway.runtimeEnv.AETHER_QA_FORCE_RUNTIME === "codex" &&
