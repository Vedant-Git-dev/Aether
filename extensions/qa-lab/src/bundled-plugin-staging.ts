...
function findQaBundledPluginDirsByManifestId(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of readdirSync(sourceRoot, { withFileTypes: true }).toSorted((left, right) =>
      ...
      const manifestId = readQaBundledManifestId(path.join(candidate, "aether.plugin.json"));
...
function resolveQaBundledPluginManifestPath(params: {
  ...
  return manifestDir ? path.join(manifestDir, "aether.plugin.json") : null;
}
...
export async function resolveQaOwnerPluginIdsForProviderIds(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of await fs.readdir(sourceRoot, { withFileTypes: true })) {
      ...
      const manifestPath = path.join(sourceRoot, entry.name, "aether.plugin.json");
...
export async function resolveQaRuntimeHostVersion(params: {
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    const candidate = parseStableSemverFloor(packageJson.aether?.install?.minHostVersion);
...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedAetherPackageDir = path.join(stagedRoot, "node_modules", "aether");
  await fs.mkdir(stagedAetherPackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedAetherPackageDir, "package.json"),
  );
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    if (manifestPath) {
      await fs.copyFile(manifestPath, path.join(targetDir, "aether.plugin.json"));
    }
  }
  await symlinkQaStagedDirEntry({
    sourcePath: path.join(stagedRoot, "dist"),
    targetPath: path.join(stagedAetherPackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,
