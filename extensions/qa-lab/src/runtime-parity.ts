...
export type RuntimeId = "aether" | "codex";

type RuntimeParityStatus = "pass" | "fail" | "skip";

const CANONICAL_RUNTIME_IDS = ["aether", "codex"] as const satisfies readonly RuntimeId[];
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[Aether heartbeat poll]";
...
const AETHER_FALLBACK_SELECTION_RE =
  /\bagent harness selected\b.*\brequested=codex\b.*\bselected=aether\b.*\breason=plugin_declared_fallback_aether\b/iu;
...
function hasForcedCodexAetherResponsesRecord(records: readonly RuntimeParityTranscriptRecord[]) {
  return records.some((record) => {
    ...
    const aetherMetadata = record.message["__aether"];
    const metadata = isMessageRecord(aetherMetadata) ? aetherMetadata : undefined;
    return readNonEmptyString(metadata?.mirrorOrigin) !== "codex-app-server";
  });
}

function classifyRuntimeParityCells(params: {
  aether: RuntimeParityCell;
  codex: RuntimeParityCell;
  aetherStatus: RuntimeParityStatus;
  codexStatus: RuntimeParityStatus;
  aetherDetails?: string;
  codexDetails?: string;
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.aether.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.aether.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.aether.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.aether.toolCalls) ||
  ...
  const aetherKnownHarnessGap = isKnownHarnessGapSkip({
    status: params.aetherStatus,
    details: params.aetherDetails,
  });
  ...
  if (
    aetherKnownHarnessGap !== codexKnownHarnessGap &&
    (aetherKnownHarnessGap ? params.codexStatus : params.aetherStatus) === "pass" &&
    isRuntimeParityCellPassable(params.aether) &&
    isRuntimeParityCellPassable(params.codex)
  ) {
    const skippedRuntime = aetherKnownHarnessGap ? "aether" : "codex";
    return {
      drift: "structural",
  ...
  if (
    ...
    return {
      drift: "failure-mode",
      driftDetails:
        params.aetherStatus === params.codexStatus
          ? params.aetherStatus === "skip"
            ? "both canonical runtime-pair cells skipped"
            : params.aetherStatus === "fail"
              ? "both canonical runtime-pair cells failed"
              : "at least one runtime failed"
          : `runtime-pair cell status differs (${params.aetherStatus} vs ${params.codexStatus})`,
    };
  }

  const toolCallShapeDetails = parity.compareToolCallShape(
    params.aether.toolCalls,
  ...
  const aetherTranscriptLines = params.aether.transcriptBytes.trim().length
    ? params.aether.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    aetherTranscriptLines !== codexTranscriptLines ||
    (!params.aether.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.aether.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${aetherTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    normalizeTextForParity(params.aether.finalText) ===
...
function runtimeParitySessionEnv(stateDir: string): NodeJS.ProcessEnv {
  return { ...process.env, AETHER_STATE_DIR: stateDir };
}
...
export async function captureRuntimeParityCell(
  ...
  const forcedCodexRuntimeLeak =
    ...
    (AETHER_FALLBACK_SELECTION_RE.test(gatewayLogs ?? "") ||
      hasForcedCodexAetherResponsesRecord(transcriptRecords));
...
export async function runRuntimeParityScenario(params: {
  ...
  const [aether, codex] = firstRuntime === "aether" ? [first, second] : [second, first];
  const drift = classifyRuntimeParityCells({
    aether: aether.cell,
    codex: codex.cell,
    aetherStatus: aether.status,
    codexStatus: codex.status,
    aetherDetails: aether.details,
  ...
  return {
    ...
    cells: {
      aether: {
        ...
        ...(aether.details ? { details: aether.details } : {}),
      },
      codex: {
        ...codex.cell,
        status: codex.status,
