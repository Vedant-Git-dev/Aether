...
const DOCKER_CANDIDATE_ENV_KEY =
  /^(?:AETHER_DOCKER_E2E_SELECTED_SHA|AETHER_CURRENT_PACKAGE_(?:TGZ|VERSION|SHA256)|AETHER_PREPUBLISH_PLUGIN_REGISTRY_(?:DIR|CANDIDATE_VERSION|MANIFEST_SHA256))$/u;
...
const dockerPackageSchema = z.strictObject({
  path: z.string(),
  name: z.literal("aether"),
  version: z.string(),
  sha256: z.string(),
});
const dockerCandidateManifestSchema = z.strictObject({
  schema: z.literal("aether.qa-docker-candidate/v1"),
...
export async function prepareDockerE2eEnvironment(params: {
  ...
  for (const key of Object.keys(env)) {
    if (
      key.startsWith("AETHER_DOCKER_ALL_") ||
  ...
  const result = await (params.runCommand ?? runQaScenarioCommandLifecycle)({
    ...
    env: {
      ...env,
      AETHER_DOCKER_ALL_LANES: laneNames.join(","),
      AETHER_DOCKER_ALL_LOG_DIR: prepDir,
      AETHER_DOCKER_E2E_REPO_ROOT: params.repoRoot,
    },
  });
  ...
  env.AETHER_DOCKER_E2E_REPO_ROOT = params.repoRoot;
  ...
  return Object.freeze(
    Object.assign(env, {
      AETHER_DOCKER_E2E_SELECTED_SHA: manifest.sourceSha,
      AETHER_CURRENT_PACKAGE_TGZ: packageCandidate.path,
      AETHER_CURRENT_PACKAGE_VERSION: packageCandidate.version,
      AETHER_CURRENT_PACKAGE_SHA256: packageCandidate.sha256,
      ...(registry
        ? {
            AETHER_PREPUBLISH_PLUGIN_REGISTRY_DIR: registry.dir,
            AETHER_PREPUBLISH_PLUGIN_REGISTRY_CANDIDATE_VERSION: registry.candidateVersion,
            AETHER_PREPUBLISH_PLUGIN_REGISTRY_MANIFEST_SHA256: registry.manifestSha256,
          }
        : {}),
    }),
  );
...
export async function runDockerE2eBatch(params: {
  ...
  try {
    commandResult = await params.runCommand({
      ...
      env: {
        ...params.env,
        AETHER_DOCKER_ALL_BUILD: "1",
        AETHER_DOCKER_ALL_FAIL_FAST: "0",
        AETHER_DOCKER_ALL_LANES: laneNames.join(","),
        AETHER_DOCKER_ALL_LANE_TIMEOUT_MS: String(params.commandTimeoutMs),
        AETHER_DOCKER_ALL_LOG_DIR: dockerOutputDir,
        AETHER_DOCKER_ALL_PROFILE: "all",
        AETHER_DOCKER_ALL_TIMINGS_FILE: path.join(dockerOutputDir, "lane-timings.json"),
      },
