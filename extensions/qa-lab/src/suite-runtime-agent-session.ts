...
import { buildSessionEntry } from "aether/plugin-sdk/memory-core-host-engine-sessions";
import {
  listSessionEntries,
...
import { appendSessionTranscriptMessageByIdentity } from "aether/plugin-sdk/session-transcript-runtime";
import {
  isRecord,
...
const SESSION_RESET_RECALL_CUTOFF = Symbol.for("aether.memory.sessionResetRecallCutoff");
...
function summarizeSessionTranscriptEvents(
  ...
  for (const event of events) {
    ...
    const aetherMeta = isRecord(message["__aether"]) ? message["__aether"] : undefined;
    const mirrorIdentity = readNonEmptyString(aetherMeta?.mirrorIdentity);
...
function qaSessionRuntimeEnv(tempRoot: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
    AETHER_STATE_DIR: path.join(tempRoot, "state"),
  };
}
