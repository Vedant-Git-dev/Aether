...
export type QaRuntimeTiming = QaRuntimeSpeedComparison & {
  aether: QaRuntimeWallClockMetrics;
...
export function compareRuntimeWallClockMs(
  aetherWallClockMs: number | null,
  codexWallClockMs: number | null,
): QaRuntimeSpeedComparison {
  if (aetherWallClockMs === null || codexWallClockMs === null) {
    return { fasterRuntime: null, speedupPercent: null };
  }
  if (aetherWallClockMs === codexWallClockMs) {
    return { fasterRuntime: "tie", speedupPercent: 0 };
  }
  const fasterRuntime = aetherWallClockMs < codexWallClockMs ? "aether" : "codex";
  const fasterWallClockMs = Math.min(aetherWallClockMs, codexWallClockMs);
  const slowerWallClockMs = Math.max(aetherWallClockMs, codexWallClockMs);
  return {
    fasterRuntime,
    speedupPercent:
      fasterWallClockMs === 0
...
export function summarizeRuntimeParityTiming(
  scenarios: readonly {
    aetherWallClockMs: number | null;
    codexWallClockMs: number | null;
    aetherBootstrapWallClockMs?: number | null;
    codexBootstrapWallClockMs?: number | null;
  }[],
): QaRuntimeTiming {
  const aether = summarizeRuntimeWallClock(
    scenarios.flatMap(({ aetherWallClockMs }) =>
      aetherWallClockMs === null ? [] : [aetherWallClockMs],
    ),
  );
  ...
  const pairedTimingCaptures = scenarios.flatMap(({ aetherWallClockMs, codexWallClockMs }) =>
    aetherWallClockMs === null || codexWallClockMs === null
      ? []
      : [{ aetherWallClockMs, codexWallClockMs }],
  );
  const aetherBootstrapValues = scenarios.flatMap(({ aetherBootstrapWallClockMs }) =>
    aetherBootstrapWallClockMs == null ? [] : [aetherBootstrapWallClockMs],
  );
  ...
  return {
    aether,
    codex,
    ...(aetherBootstrapValues.length > 0 || codexBootstrapValues.length > 0
      ? {
          bootstrap: {
            aether: summarizeRuntimeWallClock(aetherBootstrapValues),
    ...
    ...compareRuntimeWallClockMs(
      pairedTimingCaptures.length > 0
        ? pairedTimingCaptures.reduce((total, capture) => total + capture.aetherWallClockMs, 0)
        : null,
      pairedTimingCaptures.length > 0
