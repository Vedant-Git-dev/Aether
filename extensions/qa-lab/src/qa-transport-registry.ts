...
async function createBuiltInQaTransport(
  ...
  if (context.driver === "crabline") {
    const { resolveAetherCrablineChannelDriverSelection } = await import("@openclaw/crabline");
    const selection = resolveAetherCrablineChannelDriverSelection({ channel: context.channelId });
