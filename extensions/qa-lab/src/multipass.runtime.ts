...
import { coerceErrorMessage, toStringifiedError } from "aether/plugin-sdk/error-runtime";
import { isPathInside } from "aether/plugin-sdk/file-access-runtime";
import { runExec } from "aether/plugin-sdk/process-runtime";
import { sleep } from "aether/plugin-sdk/runtime-env";
import { appendRegularFile } from "aether/plugin-sdk/security-runtime";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/aether-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/aether";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/aether-codex-home";
...
type QaMultipassPlan = {
  ...
  channelDriverSelection?: AetherCrablineChannelDriverSelection;
...
function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `aether-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "AETHER_CONFIG_PATH" &&
          key !== "AETHER_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `AETHER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `AETHER_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm aether qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), `${plan.vmName}-qa-suite-`),
  );
