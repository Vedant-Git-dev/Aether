...
import { resolveModelRefFromString } from "aether/plugin-sdk/agent-runtime";
import { formatErrorMessage as formatQaErrorMessage } from "aether/plugin-sdk/error-runtime";
import { formatMemoryDreamingDay } from "aether/plugin-sdk/memory-core-host-status";
import { resolveSessionTranscriptsDirForAgent } from "aether/plugin-sdk/memory-host-core";
import { createPluginStateSyncKeyedStore } from "aether/plugin-sdk/plugin-state-store-runtime";
import { buildAgentSessionKey } from "aether/plugin-sdk/routing";
...
function setActiveMemorySessionDisabled(
  ...
  const store = createPluginStateSyncKeyedStore<{
    ...
    env: {
      ...process.env,
      AETHER_STATE_DIR: path.join(env.gateway.tempRoot, "state"),
    },
  });
...
export async function runQaSuiteScenarioSteps(
  ...
  for (const step of steps) {
    try {
      if (process.env.AETHER_QA_DEBUG === "1") {
