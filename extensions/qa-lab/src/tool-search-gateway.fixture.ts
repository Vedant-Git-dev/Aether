...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "AETHER_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv("AETHER_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS", 5_000, env),
  };
}
...
export async function runToolSearchGatewayLane(params: {
  ...
  const gatewayToken = params.env.gateway.runtimeEnv.AETHER_GATEWAY_TOKEN;
  ...
  const response = await fetchJson(
    `${params.env.gateway.baseUrl}/v1/responses`,
    {
      ...
      headers: {
        ...
        "x-aether-scopes": "operator.write",
        "x-aether-agent": "qa",
        "x-aether-session-key": sessionKey,
      },
      body: JSON.stringify({
        model: "aether/qa",
        input: [
