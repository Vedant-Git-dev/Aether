...
import { disposeRegisteredAgentHarnesses } from "aether/plugin-sdk/agent-harness";
...
export async function runQaFlowSuiteStandard(
  ...
  try {
    ...
    const activeGateway = await gateway.start({
      repoRoot,
      command: params?.sutAetherCommand,
        ...
        gatewayConfigPatches.length > 0 || params?.mutateConfig
          ? (cfg) => {
              const patchedConfig = gatewayConfigPatches.length
                ? (applyQaSuiteGatewayConfigPatches(cfg, gatewayConfigPatches) as AetherConfig)
                : cfg;
    ...
    const activeEnv: QaSuiteEnvironment = {
      ...
      runtimeId: params?.forcedRuntime ?? "aether",
      outputDir,
    ...
    const activeEnv = env;
    const keepTemp = process.env.AETHER_QA_KEEP_TEMP === "1" || false;
