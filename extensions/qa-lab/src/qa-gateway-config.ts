...
import { AETHER_VERSION } from "aether/plugin-sdk/agent-harness-runtime";
...
export function buildQaGatewayConfig(params: {
  ...
  const pluginEntries = Object.fromEntries(
    selectedPluginIds.map((pluginId) => [
      ...
      pluginId === "acpx"
        ? {
            enabled: true,
            config: { pluginToolsMcpBridge: true, aetherToolsMcpBridge: true },
          }
  ...
  const resolveModelEntry = (modelRef: string) => {
    // Codex owns its app-server transport. Aether provider params would make
  ...
  return {
    meta: {
      lastTouchedVersion: AETHER_VERSION,
    },
    // Keep daily rollover and pruning inside the owned QA workspace.
    logging: {
      file: `${params.workspaceDir}/logs/aether-YYYY-MM-DD.log`,
    },
    memory: {
      search: {
        ...mockMemorySearch,
