...
type QaToolCoverageRow = {
  ...
  scenarios: string[];
  sourcePaths: string[];
  aether: QaToolCoverageStatus;
  codex: QaToolCoverageStatus;
  drift: QaToolCoverageDrift;
  aetherToolCalls: number;
  codexToolCalls: number;
  aetherSuccessfulToolCalls: number;
...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "aether" ? result.cells.aether : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    aether: result ? cellStatus(result.cells.aether) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    aetherToolCalls: countRuntimeToolCalls(result, "aether", runtimeToolName),
    codexToolCalls: countRuntimeToolCalls(result, "codex", runtimeToolName),
    aetherSuccessfulToolCalls: countSuccessfulRuntimeToolCalls(
      result,
      "aether",
      runtimeToolName,
    ),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.aether !== "pass" || row.codex !== "pass") {
    return `${row.tool} status aether=${row.aether} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.aetherSuccessfulToolCalls === 0) {
    return `${row.tool} missing successful aether tool call/result ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "aether-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "aether-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# Aether Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- Aether dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | Aether | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];
