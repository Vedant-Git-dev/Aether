...
import { normalizeSortedUniqueTrimmedStringList } from "aether/plugin-sdk/string-coerce-runtime";
import { z } from "zod";
...
export const QA_EVIDENCE_SUMMARY_KIND = "aether.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["AETHER_E2E_PROFILE", params.env?.AETHER_E2E_PROFILE],
    ["AETHER_QA_PROFILE", params.env?.AETHER_QA_PROFILE],
  ] as const;
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.AETHER_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.AETHER_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.AETHER_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
...
function resolveQaEvidenceBuildContext(params: QaEvidenceBuildBase, defaultRunner?: string) {
  return {
    ...
    executionBase: {
      runner: params.env?.AETHER_QA_RUNNER?.trim() || (params.runner ?? defaultRunner) || "host",
