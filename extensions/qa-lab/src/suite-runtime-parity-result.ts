...
function runtimeParityScenarioResultStatus(result: RuntimeParityResult) {
  const cellStatuses = new Set([
    runtimeParityScenarioStepStatus(result.cells.aether),
...
export function buildRuntimeParityScenarioResult(params: {
  ...
  const aetherCell = params.result.cells.aether;
  ...
  return {
    ...
    steps: [
      {
        name: aetherCell.runtime,
        status: runtimeParityScenarioStepStatus(aetherCell),
        details: formatRuntimeParityScenarioCellDetails(aetherCell),
      },
      {
