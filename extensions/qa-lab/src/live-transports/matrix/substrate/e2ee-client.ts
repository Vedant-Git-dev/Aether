...
import type {
  ...
  MessageEventContent,
} from "@aether/matrix/test-api.js";
import type {
...
type MatrixQaE2eeRuntime = typeof import("@aether/matrix/test-api.js");
...
function resolveMatrixQaPluginStateNamespaceKey(options: OpenKeyedStoreOptions): string {
  return `${options.env?.AETHER_STATE_DIR ?? ""}\0${options.namespace}`;
}
...
export async function loadMatrixQaE2eeRuntime(): Promise<MatrixQaE2eeRuntime> {
  const { loadQaRunnerBundledPluginTestApi } =
    await import("aether/plugin-sdk/qa-runner-runtime");
