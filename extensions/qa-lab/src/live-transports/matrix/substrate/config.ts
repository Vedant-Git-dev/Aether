...
type MatrixQaAudioConfigOverrides = NonNullable<
  NonNullable<NonNullable<AetherConfig["tools"]>["media"]>["audio"]
>;
type MatrixQaMediaModelsOverrides = NonNullable<
  NonNullable<NonNullable<AetherConfig["tools"]>["media"]>["models"]
>;
...
export function buildMatrixQaConfig(
  ...
  config.approvals = approvals as AetherConfig["approvals"];
  config.agents = {
    ...currentCfg.agents,
    defaults: agentDefaults as NonNullable<AetherConfig["agents"]>["defaults"],
  };
  config.tools = tools as AetherConfig["tools"];
  config.plugins = {
    ...currentCfg.plugins,
    allow: pluginAllow,
    entries: {
  ...
  config.messages = {
    ...currentCfg.messages,
    groupChat: groupChat as NonNullable<AetherConfig["messages"]>["groupChat"],
  };
  config.channels = {
    ...currentCfg.channels,
    matrix: {
