...
import { toStringifiedError } from "aether/plugin-sdk/error-runtime";
import { buildQaTarget } from "aether/plugin-sdk/qa-channel-protocol";
import type { QaRunnerCliRegistration } from "aether/plugin-sdk/qa-runner-runtime";
...
export async function createMatrixQaTransportAdapter(
  ...
  try {
    provisioning = await provisionMatrixQaRoom({
      ...
      roomName: `Aether Matrix QA ${suffix}`,
  ...
  return {
    ...
    async sendInbound(input) {
      ...
      const hasPortableMention = input.text.includes("@aether");
      const body = input.text.replaceAll("@aether", provisioning.sut.userId);
    ...
    createGatewayConfig: () =>
      buildMatrixQaConfig({} as AetherConfig, {
    ...
    createRuntimeEnvPatch: () => ({
      AETHER_QA_MATRIX_DRIVER_USER_ID: provisioning.driver.userId,
      AETHER_QA_MATRIX_OBSERVER_USER_ID: provisioning.observer.userId,
      AETHER_QA_MATRIX_SUT_ACCOUNT_ID: accountId,
      AETHER_QA_MATRIX_MAIN_ROOM_ID:
        provisioning.topology.rooms.find((room) => room.key === "main")?.roomId ??
        provisioning.roomId,
      AETHER_QA_MATRIX_SECONDARY_ROOM_ID:
