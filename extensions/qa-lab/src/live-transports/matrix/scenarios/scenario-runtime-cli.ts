...
import { spawn as startAetherCliProcess } from "node:child_process";
import { randomUUID } from "node:crypto";
import { chmod, mkdir, mkdtemp, rm, stat, writeFile } from "node:fs/promises";
import path from "node:path";
import { setTimeout as sleep } from "node:timers/promises";
import { formatErrorMessage } from "aether/plugin-sdk/error-runtime";
import { redactSensitiveText } from "aether/plugin-sdk/logging-core";
...
import {
  ...
  resolveMatrixQaAetherCliEntryPath,
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `aether ${redactMatrixQaCliArgs(args).join(" ")}`;
}
...
export function startMatrixQaAetherCli(params: {
  ...
  const distEntryPath = resolveMatrixQaAetherCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startAetherCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaAetherCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaAetherCli(params).wait();
}
...
export async function createMatrixQaAetherCliRuntime(params: {
  accountId: string;
  accessToken: string;
  ...
  const rootDir = await mkdtemp(
    path.join(resolvePreferredAetherTmpDir(), "aether-matrix-cli-qa-"),
  );
