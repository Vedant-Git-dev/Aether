...
import {
  createMatrixQaAetherCliRuntime,
...
export type MatrixQaCliRuntime = Awaited<ReturnType<typeof createMatrixQaAetherCliRuntime>>;
...
export function requireMatrixQaGatewayConfigPath(context: MatrixQaScenarioContext) {
  const configPath = requireMatrixQaCliRuntimeEnv(context).AETHER_CONFIG_PATH?.trim();
  if (!configPath) {
...
export async function createMatrixQaRecoveryCliRuntime(params: {
  ...
  return await createMatrixQaAetherCliRuntime({
