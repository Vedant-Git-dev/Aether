...
export async function runMatrixQaE2eeCliSelfVerificationScenario(
  ...
  const account = await registerMatrixQaCliE2eeAccount({
    context,
    deviceName: "Aether Matrix QA CLI Self Verification Owner",
  ...
  try {
    ...
    const cliDevice = await loginClient.loginWithPassword({
      deviceName: "Aether Matrix QA CLI Self Verification Device",
    ...
    try {
      ...
      try {
        ...
        const cliSas = parseMatrixQaCliSasText(
          sasOutput.text,
          "interactive aether matrix verify self",
        );
        ...
        return {
          ...
          details: [
            "Matrix CLI self-verification established full Matrix identity trust through interactive aether matrix verify self",
