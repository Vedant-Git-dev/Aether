...
import type { WhatsAppQaDriverSession } from "@aether/whatsapp/api.js";
...
export async function createWhatsAppQaTransportAdapter(
  ...
  try {
    authRoot = await fs.mkdtemp(
      path.join(resolvePreferredAetherTmpDir(), "aether-whatsapp-qa-adapter-"),
    );
  ...
  return {
    ...
    createGatewayConfig: () =>
      buildWhatsAppQaConfig({} as AetherConfig, {
