...
import { normalizeE164 } from "aether/plugin-sdk/account-resolution";
...
const WHATSAPP_QA_ENV_KEYS = [
  "AETHER_QA_WHATSAPP_DRIVER_PHONE_E164",
  "AETHER_QA_WHATSAPP_SUT_PHONE_E164",
  "AETHER_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
  "AETHER_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64",
] as const;
...
export function resolveWhatsAppQaRuntimeEnv(
  ...
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "AETHER_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "AETHER_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "AETHER_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "AETHER_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.AETHER_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "AETHER_QA_WHATSAPP",
  );
}
...
type WhatsAppQaAgentConfig = NonNullable<NonNullable<AetherConfig["agents"]>["list"]>[number];
...
): Pick<AetherConfig, "agents" | "broadcast"> {
  if (!params.broadcast) {
    return {};
  }
