...
import { readProviderTextResponse } from "aether/plugin-sdk/provider-http";
import { z } from "zod";
import {
...
const CHUNKED_PAYLOAD_MAX_BYTES_ENV = "AETHER_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
const CHUNKED_PAYLOAD_MAX_CHUNKS_ENV = "AETHER_QA_CREDENTIAL_PAYLOAD_MAX_CHUNKS";
...
const CHUNKED_PAYLOAD_MARKER = "__aetherQaCredentialPayloadChunksV1";
...
function normalizeEndpointPrefix(value: string | undefined): string {
  return normalizeQaCredentialEndpointPrefix({
    ...
    invalidAbsoluteMessage:
      "AETHER_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    invalidSegmentsMessage:
      "AETHER_QA_CONVEX_ENDPOINT_PREFIX must not contain backslashes or .. path segments.",
  });
}
...
function resolveConvexAuthToken(env: NodeJS.ProcessEnv, role: QaCredentialRole): string {
  const roleToken =
    role === "ci"
      ? env.AETHER_QA_CONVEX_SECRET_CI?.trim()
      : env.AETHER_QA_CONVEX_SECRET_MAINTAINER?.trim();
  const token = roleToken;
  if (token) {
    return token;
  }
  if (role === "ci") {
    throw new Error("Missing AETHER_QA_CONVEX_SECRET_CI for CI credential access.");
  }
  throw new Error("Missing AETHER_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.");
}
...
function resolveConvexCredentialBrokerConfig(params: {
  ...
  const siteUrl = params.env.AETHER_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing AETHER_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const baseUrl = normalizeConvexSiteUrl(siteUrl, params.env);
  const endpointPrefix = normalizeEndpointPrefix(params.env.AETHER_QA_CONVEX_ENDPOINT_PREFIX);
  const ownerId =
    params.ownerId?.trim() ||
    params.env.AETHER_QA_CREDENTIAL_OWNER_ID?.trim() ||
  ...
  return {
    ...
    leaseTtlMs: parsePositiveIntegerEnv(
      params.env,
      "AETHER_QA_CREDENTIAL_LEASE_TTL_MS",
      DEFAULT_LEASE_TTL_MS,
    ),
    heartbeatIntervalMs: parsePositiveIntegerEnv(
      params.env,
      "AETHER_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS",
      DEFAULT_HEARTBEAT_INTERVAL_MS,
    ),
    acquireTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "AETHER_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS",
      DEFAULT_ACQUIRE_TIMEOUT_MS,
    ),
    httpTimeoutMs: parsePositiveIntegerEnv(
      params.env,
      "AETHER_QA_CREDENTIAL_HTTP_TIMEOUT_MS",
...
export async function acquireQaCredentialLease<TPayload>(
  ...
  const source = normalizeQaCredentialSource(opts.source ?? env.AETHER_QA_CREDENTIAL_SOURCE);
  if (source === "env") {
    return {
      source: "env",
      kind: opts.kind,
  ...
  const role = normalizeQaCredentialRole(opts.role ?? env.AETHER_QA_CREDENTIAL_ROLE, env);
