...
import type { LiveTransportQaCommandOptions } from "aether/plugin-sdk/qa-runtime";
...
const TELEGRAM_QA_SUT_AETHER_COMMAND_ENV = "AETHER_QA_TELEGRAM_SUT_AETHER_COMMAND";
const TELEGRAM_QA_SUT_UID_ENV = "AETHER_QA_TELEGRAM_SUT_UID";
const TELEGRAM_QA_SUT_GID_ENV = "AETHER_QA_TELEGRAM_SUT_GID";
const TELEGRAM_QA_SUT_BOUNDARY_DIR_ENV = "AETHER_QA_TELEGRAM_SUT_PROCESS_BOUNDARY_DIR";
const TELEGRAM_QA_SUT_CLEANUP_TIMEOUT_ENV = "AETHER_QA_TELEGRAM_SUT_CLEANUP_TIMEOUT_MS";
const TELEGRAM_QA_SUT_RUNTIME_EXECUTABLE_ENV = "AETHER_QA_TELEGRAM_SUT_RUNTIME_EXECUTABLE";
const TELEGRAM_QA_SUT_PRELOAD_PATH_ENV = "AETHER_QA_TELEGRAM_SUT_PRELOAD_PATH";
const TELEGRAM_QA_SUT_FORWARDED_ENV_KEYS_ENV = "AETHER_QA_TELEGRAM_SUT_FORWARDED_ENV_KEYS";
...
async function resolveTelegramQaSutAetherCommand(
  repoRoot: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<QaGatewayChildCommand | undefined> {
  const configuredCommand = env[TELEGRAM_QA_SUT_AETHER_COMMAND_ENV];
  if (configuredCommand === undefined) {
    return undefined;
  }
  const command = await resolveRegularPath({
    env,
    key: TELEGRAM_QA_SUT_AETHER_COMMAND_ENV,
    executable: true,
  });
...
type TelegramQaSuiteOptions = LiveTransportQaCommandOptions & {
  ...
  mutateConfig?: (cfg: AetherConfig) => AetherConfig;
};
...
export async function runQaTelegramSuite(opts: TelegramQaSuiteOptions) {
  ...
  const result = await runQaFlowSuiteFromRuntime({
    ...
    scenarioIds,
    sutAetherCommand: opts.sutAetherCommand,
...
export async function runQaTelegramCommand(opts: LiveTransportQaCommandOptions) {
  ...
  const sutAetherCommand = await resolveTelegramQaSutAetherCommand(runOptions.repoRoot);
