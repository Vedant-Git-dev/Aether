      ...
      discordScenarioContext: {
        configureScenario: async (implementation: DiscordQaScenarioImplementation) => {
          ...
          if (run.kind === "transcripts-voice-authorization" && !params.runtimeEnv.voiceChannelId) {
            throw new Error(
              "Discord transcript authorization requires an explicit QA voiceChannelId in the leased credential or AETHER_QA_DISCORD_VOICE_CHANNEL_ID. Reserve a dedicated empty QA voice channel before running; automatic room discovery is not allowed and the harness does not verify room occupancy.",
            );
          }
          const voiceChannel =
          ...
          const applyConfig = async (transcriptVoiceAuthorized?: boolean) => {
            ...
            const cfg = discordQaScenarioSupport.testing.buildDiscordQaConfig(
              snapshot.config as AetherConfig,
              {
                guildId: params.runtimeEnv.guildId,
