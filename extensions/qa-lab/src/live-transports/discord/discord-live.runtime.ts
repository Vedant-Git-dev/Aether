...
import { writeExternalFileWithinRoot } from "aether/plugin-sdk/security-runtime";
import { uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
import { escapeHtml } from "aether/plugin-sdk/text-utility-runtime";
...
const DISCORD_QA_CAPTURE_UI_METADATA_ENV = "AETHER_QA_DISCORD_CAPTURE_UI_METADATA";
const DISCORD_QA_KEEP_THREADS_ENV = "AETHER_QA_DISCORD_KEEP_THREADS";
const DISCORD_QA_ENV_KEYS = [
  "AETHER_QA_DISCORD_GUILD_ID",
  "AETHER_QA_DISCORD_CHANNEL_ID",
  "AETHER_QA_DISCORD_DRIVER_BOT_TOKEN",
  "AETHER_QA_DISCORD_SUT_BOT_TOKEN",
  "AETHER_QA_DISCORD_SUT_APPLICATION_ID",
] as const;
...
function resolveDiscordQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): DiscordQaRuntimeEnv {
  const voiceChannelId = env.AETHER_QA_DISCORD_VOICE_CHANNEL_ID?.trim();
  const runtimeEnv = {
    guildId: resolveEnvValue(env, "AETHER_QA_DISCORD_GUILD_ID"),
    channelId: resolveEnvValue(env, "AETHER_QA_DISCORD_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "AETHER_QA_DISCORD_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "AETHER_QA_DISCORD_SUT_BOT_TOKEN"),
    sutApplicationId: resolveEnvValue(env, "AETHER_QA_DISCORD_SUT_APPLICATION_ID"),
    ...(voiceChannelId ? { voiceChannelId } : {}),
  };
  validateDiscordQaRuntimeEnv(runtimeEnv, "AETHER_QA_DISCORD");
  return runtimeEnv;
}
...
async function resolveDiscordQaVoiceChannel(params: {
  ...
  if (!first) {
    throw new Error(
      "Discord voice auto-join scenario could not find a visible voice/stage channel for the SUT bot. Add voiceChannelId to the Convex discord credential payload or set AETHER_QA_DISCORD_VOICE_CHANNEL_ID.",
    );
  }
  return first;
}
...
<body>
  <main>
    ...
    <section class="message">
      <div class="author">Aether Discord SUT</div>
