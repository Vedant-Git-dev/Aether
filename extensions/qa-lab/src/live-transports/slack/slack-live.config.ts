...
import { asNonArrayRecord, uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
import {
...
export function resolveSlackQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): SlackQaRuntimeEnv {
  const runtimeEnv = {
    channelId: resolveEnvValue(env, "AETHER_QA_SLACK_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "AETHER_QA_SLACK_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "AETHER_QA_SLACK_SUT_BOT_TOKEN"),
    sutAppToken: resolveEnvValue(env, "AETHER_QA_SLACK_SUT_APP_TOKEN"),
  };
  return validateSlackQaRuntimeEnv(runtimeEnv, "AETHER_QA_SLACK");
}
