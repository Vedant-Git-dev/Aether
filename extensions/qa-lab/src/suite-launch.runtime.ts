...
import { runPluginCommandWithTimeout } from "aether/plugin-sdk/run-command";
...
async function resolveQaFlowChannelGroups(
  ...
  if (runParams.expandScenarioChannels) {
    ...
    return [...groups].map(([channel, groupedScenarios]) => ({
      ...
      channelDriverSelection: channel
        ? resolveAetherCrablineChannelDriverSelection({ channel })
        : undefined,
      scenarios: groupedScenarios,
    }));
  }
    ...
    return [
      {
        ...
        channelDriverSelection:
          ...
          resolveAetherCrablineChannelDriverSelection({ channel: singleChannel }),
        scenarios: [...scenarios],
      },
    ];
  }
  ...
  return channels.map((channel) => ({
    channel,
    channelId: undefined,
    channelDriverSelection: resolveAetherCrablineChannelDriverSelection({ channel }),
    scenarios: scenarios.filter(
      (scenario) =>
        (normalizeQaSuiteScenarioChannel(scenario) ?? AETHER_CRABLINE_DEFAULT_CHANNEL) ===
        channel,
    ),
  }));
}
    ...
    ...(params.env
      ...
      : params.kind !== "script"
        ? {
            ...
            env: { AETHER_E2E_USE_PREBUILT_DIST: "1" },
          }
        : {}),
...
function renderUnifiedQaSuiteReport(params: {
  ...
  return renderQaMarkdownReport({
    title: "Aether QA Scenario Suite",
