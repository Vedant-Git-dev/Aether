...
type TokenEfficiencyRow = {
  ...
  aether: TokenEfficiencyRuntimeUsage;
...
type TokenEfficiencyReport = {
  ...
  aggregate: {
    aether: TokenEfficiencyAggregateRuntimeUsage;
...
function deltaPercent(aetherTotalTokens: number, codexTotalTokens: number): number {
  if (aetherTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - aetherTotalTokens) / aetherTotalTokens) * 100;
}
...
function formatProcessedDelta(params: {
  deltaPercent: number;
  aether: Pick<TokenEfficiencyRuntimeUsage, "processedTokenEvidence">;
  codex: Pick<TokenEfficiencyRuntimeUsage, "processedTokenEvidence">;
}): string {
  return params.aether.processedTokenEvidence === "unavailable" ||
...
function toolNamesForCells(aether: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...aether.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const aether = runtimeUsage(params.result.cells.aether);
  const codex = runtimeUsage(params.result.cells.codex);
  const comparable =
    aether.processedTokenEvidence !== "unavailable" &&
    codex.processedTokenEvidence !== "unavailable";
  const delta = comparable ? deltaPercent(aether.processedTokens, codex.processedTokens) : 0;
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.aether, params.result.cells.codex),
  };
}
...
function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const aether = buildAggregateRuntime(rows, "aether");
  ...
  return {
    aether,
    codex,
    deltaPercent: comparable ? deltaPercent(aether.processedTokens, codex.processedTokens) : 0,
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.aether.totalTokens <= 0) {
    failures.push(`${row.scenarioId} aether live usage totalTokens=${row.aether.totalTokens}`);
  }
  if (row.codex.totalTokens <= 0) {
    failures.push(`${row.scenarioId} codex live usage totalTokens=${row.codex.totalTokens}`);
  }
  for (const runtime of ["aether", "codex"] as const) {
...
export function buildTokenEfficiencyReport(
  ...
  const failures = rows.flatMap((row, index) => {
    ...
    const rowFailures =
      liveUsage && result
        ? [
            ...liveUsageShapeFailures(row.scenarioId, "aether", result.cells.aether.usage),
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-Aether live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# Aether Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    `| aether | ${formatProcessedCount(report.aggregate.aether)} | ${report.aggregate.aether.totalTokens} | ${formatOptionalCount(report.aggregate.aether.cacheReadTokens)} | ${formatOptionalCount(report.aggregate.aether.cacheWriteTokens)} | ${formatOptionalCount(report.aggregate.aether.cacheMissCount)} | ${formatOptionalCount(report.aggregate.aether.p50PerScenario)} | ${formatOptionalCount(report.aggregate.aether.p90PerScenario)} |`,
    ...
    `| delta | ${formatProcessedDelta({ deltaPercent: report.aggregate.deltaPercent, aether: report.aggregate.aether, codex: report.aggregate.codex })} |  |  |  |  |  |  |`,
    "",
  );
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | Aether processed/in/out/cached/written/total/tools | Codex processed/in/out/cached/written/total/tools | Processed-token delta | Classification | Flagged | Aether cache misses | Codex cache misses | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${formatProcessedCount(row.aether)}/${row.aether.inputTokens}/${row.aether.outputTokens}/${formatOptionalCount(row.aether.cacheReadTokens)}/${formatOptionalCount(row.aether.cacheWriteTokens)}/${row.aether.totalTokens}/${row.aether.toolCallCount} | ${formatProcessedCount(row.codex)}/${row.codex.inputTokens}/${row.codex.outputTokens}/${formatOptionalCount(row.codex.cacheReadTokens)}/${formatOptionalCount(row.codex.cacheWriteTokens)}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatProcessedDelta({ deltaPercent: row.deltaPercent, aether: row.aether, codex: row.codex })} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${formatCacheMisses(row.aether.cacheMisses, row.aether.unmeasuredPostWarmTurns)} | ${formatCacheMisses(row.codex.cacheMisses, row.codex.unmeasuredPostWarmTurns)} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
