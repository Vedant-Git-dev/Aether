...
import { parseBooleanValue } from "aether/plugin-sdk/string-coerce-runtime";
...
export function createQaSuiteReportNotes(params: {
  ...
  createCrablineChannelReportNotes?: QaCrablineRuntime["createAetherCrablineChannelReportNotes"];
}) {
  return [
    ...
    ...(params.createCrablineChannelReportNotes?.(
      params.channelDriverSelection as AetherCrablineChannelDriverSelection | null | undefined,
    ) ?? []),
  ];
}
...
export function buildQaIsolatedScenarioWorkerParams(params: {
  ...
  channelDriverSelection?: AetherCrablineChannelDriverSelection | null;
  primaryModel: string;
  alternateModel: string;
  fastMode: boolean;
  ...
  return {
    ...
    sutAetherCommand: params.input?.sutAetherCommand,
...
export function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseBooleanValue(env.AETHER_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
