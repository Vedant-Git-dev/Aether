...
export const CODEX_PLUGIN_LIFECYCLE_MESSAGES = Object.freeze({
  missingPlugin:
    'Codex plugin is required for Codex runtime. Run "aether doctor --fix" to install @aether/codex, then retry.',
});
...
export async function installCodexPluginFixture(agentDir: string): Promise<void> {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ name: "@aether/codex" }, null, 2)}\n`,
    "utf8",
  );
  await fs.writeFile(
    path.join(targetDir, "aether.plugin.json"),
