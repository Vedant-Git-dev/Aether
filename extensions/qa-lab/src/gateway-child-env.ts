...
const QA_GATEWAY_CHILD_BLOCKED_ENV_VARS = Object.freeze([
  ...
  "BASHOPTS",
  "ENV",
  "AETHER_QA_CONVEX_SECRET_CI",
  "AETHER_QA_CONVEX_SECRET_MAINTAINER",
  "AETHER_QA_SUT_FORBIDDEN_SENTINEL",
  "AETHER_QA_TELEGRAM_GROUP_ID",
  "AETHER_QA_TELEGRAM_DRIVER_BOT_TOKEN",
  "AETHER_QA_TELEGRAM_SUT_BOT_TOKEN",
  "SHELLOPTS",
]);
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    AETHER_HOME: params.homeDir,
    AETHER_CONFIG_PATH: params.configPath,
    AETHER_STATE_DIR: params.stateDir,
    AETHER_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    AETHER_SKIP_STARTUP_MODEL_PREWARM: "1",
    AETHER_NO_RESPAWN: "1",
    AETHER_TEST_FAST: "1",
    AETHER_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    AETHER_QA_PARENT_PID: String(process.pid),
    AETHER_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { AETHER_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    AETHER_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    AETHER_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { AETHER_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { AETHER_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
  ...
  delete normalizedEnv.AETHER_SKIP_CHANNELS;
  delete normalizedEnv.AETHER_SKIP_PROVIDERS;
  Object.assign(normalizedEnv, params.runtimeEnvPatch);
  if (params.developmentSourceRoot === null) {
    delete normalizedEnv.AETHER_DEV_SOURCE_ROOT;
  } else {
    normalizedEnv.AETHER_DEV_SOURCE_ROOT = params.developmentSourceRoot;
  }
  ...
  normalizedEnv.AETHER_BUILD_PRIVATE_QA = "1";
  normalizedEnv.AETHER_ENABLE_PRIVATE_QA_CLI = "1";
...
export function buildQaForcedRuntimeEnvPatch(params: {
  ...
  const patch: NodeJS.ProcessEnv = {
    AETHER_BUILD_PRIVATE_QA: "1",
    AETHER_QA_FORCE_RUNTIME: params.forcedRuntime,
  };
  if (params.forcedRuntime !== "codex") {
    return patch;
  }
  if (params.providerMode !== "mock-openai") {
    patch.AETHER_CODEX_APP_SERVER_ARGS = buildQaCodexAppServerArgs({
