...
import { closeQaRuntimeStores } from "aether/plugin-sdk/qa-runtime";
    ...
    await runQaPackagedBootstrap(
      ...
      () =>
        runQaGatewayCliCommand({
          ...
          env: { ...params.env, AETHER_CONFIG_PATH: params.configPath },
...
export async function prepareQaGatewayChild(
  ...
  const tempParentDir = params.command?.tempParentDir ?? resolvePreferredAetherTmpDir();
  const tempRoot = await fs.mkdtemp(path.join(tempParentDir, "aether-qa-suite-"));
  ...
  const configPath = path.join(tempRoot, "aether.json");
  const packagedAuthConfigPath = path.join(stateDir, "qa-auth-bootstrap", "aether.json");
          ...
          env = buildQaRuntimeEnv({
            ...
            runtimeEnvPatch: {
              ...
              ...buildQaForcedRuntimeEnvPatch({
                ...
                nativeAppServerArgs:
                  params.runtimeEnvPatch?.AETHER_CODEX_APP_SERVER_ARGS ??
                  process.env.AETHER_CODEX_APP_SERVER_ARGS,
              }),
            },
