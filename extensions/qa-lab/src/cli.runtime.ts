...
import {
  ...
  AETHER_CRABLINE_DEFAULT_CHANNEL,
  resolveAetherCrablineChannelDriverSelection,
...
import { parseBooleanValue, uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
import {
...
const QA_CREDENTIAL_PAYLOAD_MAX_BYTES_ENV = "AETHER_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimeNames.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "aether,codex".');
  }
  const [left, right] = runtimeNames.map((part) => normalizeQaRuntimeId(part.trim().toLowerCase()));
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "aether" and "codex".');
  }
  if (left === right) {
...
async function runQaParityPreflight(params: {
  ...
  alternateModel?: string;
  allowFailures?: boolean;
  sutAetherCommand?: QaGatewayChildCommand;
}) {
  ...
  const result = await runQaSuiteWithInfraRetry(() =>
    runQaFlowSuiteFromRuntime({
      ...
      concurrency: 1,
      ...(params.sutAetherCommand ? { sutAetherCommand: params.sutAetherCommand } : {}),
    }),
  );
...
export async function resolveQaHarnessRepoRoot(moduleUrl = import.meta.url): Promise<string> {
  ...
  throw new Error(
    `Unable to resolve QA harness repository root from ${modulePath}: no ancestor package.json named "aether" within ${QA_HARNESS_ROOT_MAX_PARENT_HOPS} parent directories.`,
  );
}
...
export async function runQaProfileCommand(opts: QaProfileCommandOptions) {
  ...
  const executionSelection = resolveQaRunProfileExecutionSelection({
    ...
    defaultChannel:
      profileReport.channelDriver === "crabline" ? AETHER_CRABLINE_DEFAULT_CHANNEL : undefined,
    supportsChannel:
...
async function withTemporaryQaProfileEnv<T>(profile: string, run: () => Promise<T>): Promise<T> {
  const previousProfile = process.env.AETHER_QA_PROFILE;
  process.env.AETHER_QA_PROFILE = profile;
  try {
    return await run();
  } finally {
    if (previousProfile === undefined) {
      delete process.env.AETHER_QA_PROFILE;
    } else {
      process.env.AETHER_QA_PROFILE = previousProfile;
    }
  }
}
...
export async function runQaSuiteCommand(opts: QaSuiteCommandOptions) {
  ...
  const runtimePairLaneSelection = resolveQaRuntimePairLaneScenarioIds({
    ...
    channelDriver,
    claudeCliAuthMode,
    defaultChannel: channelDriver === "crabline" ? AETHER_CRABLINE_DEFAULT_CHANNEL : undefined,
  ...
  const channelDriverChannels =
    channelDriver === "crabline"
      ? resolveQaSuiteScenarioChannels({
          defaultChannel: AETHER_CRABLINE_DEFAULT_CHANNEL,
  ...
  if (opts.preflight === true) {
    await runQaParityPreflight({
      ...
      alternateModel,
      allowFailures,
      ...(sutAetherCommand ? { sutAetherCommand } : {}),
    });
    return undefined;
  }
  const thinkingDefault = parseQaThinkingLevel("--thinking", opts.thinking);
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["aether", "codex"];
  if (runtimePair[0] !== "aether" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "aether,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.AETHER_QA_CONTROL_UI_PROXY_TOKEN,
