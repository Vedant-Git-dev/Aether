...
import { runExec } from "aether/plugin-sdk/process-runtime";
...
const QA_LAB_UI_OVERLAY_DIR = "/opt/aether-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        AETHER_EXTENSIONS: "${QA_DOCKER_PLUGIN_SELECTION}"\n`;
}
...
${imageBlock}    pull_policy: never
    healthcheck:
      ...
      start_period: 3s
    environment:
      AETHER_ENABLE_PRIVATE_QA_CLI: "1"
      AETHER_PROFILE: ""
    command:
      - node
      - dist/index.js
...
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    environment:
      AETHER_ENABLE_PRIVATE_QA_CLI: "1"
      AETHER_CONFIG_PATH: /opt/aether-scaffold/aether.json
      AETHER_STATE_DIR: /tmp/aether/state
      AETHER_SKIP_GMAIL_WATCHER: "1"
      AETHER_SKIP_BROWSER_CONTROL_SERVER: "1"
      AETHER_SKIP_CANVAS_HOST: "1"
      AETHER_PROFILE: ""
    command:
      - sh
      - -lc
      - AETHER_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/aether-scaffold/aether.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://aether-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
...
${imageBlock}    pull_policy: never
    ...
    environment:
      AETHER_CONFIG_PATH: /tmp/aether/aether.json
      AETHER_STATE_DIR: /tmp/aether/state
      AETHER_NO_RESPAWN: "1"
      AETHER_SKIP_GMAIL_WATCHER: "1"
      AETHER_SKIP_BROWSER_CONTROL_SERVER: "1"
    ...
    volumes:
      - ./state:/opt/aether-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/aether-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/aether/workspace /tmp/aether/state && cp /opt/aether-scaffold/aether.json /tmp/aether/aether.json && cp -R /opt/aether-scaffold/seed-workspace/. /tmp/aether/workspace/ && rm -rf /tmp/aether/workspace/repo && ln -s /opt/aether-repo /tmp/aether/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
AETHER_GATEWAY_TOKEN=${params.gatewayToken}
...
1. Build the prebaked image once:
   - \`docker build -t aether:qa-local-prebaked --build-arg AETHER_EXTENSIONS="${QA_DOCKER_PLUGIN_SELECTION}" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "aether:qa-local-prebaked";
  ...
  const config = buildQaGatewayConfig({
    ...
    gatewayToken,
    providerBaseUrl,
    workspaceDir: "/tmp/aether/workspace",
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "aether.json"),
  ];
