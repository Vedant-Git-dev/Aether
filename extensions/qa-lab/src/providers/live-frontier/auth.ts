...
import { normalizeStringEntries, uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
import { writeQaAuthProfiles } from "../shared/auth-store.js";

export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "AETHER_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "AETHER_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "AETHER_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["AETHER_LIVE_ANTHROPIC_KEY"],
  gemini: ["AETHER_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "AETHER_LIVE_CODEX_API_KEY",
    "AETHER_LIVE_OPENAI_KEY",
    "OPENAI_API_KEY",
  ],
});
...
function qaLiveOpenAiUsesCodexByDefault(cfg: AetherConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: AetherConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: AetherConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.AETHER_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "aether") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or AETHER_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Aether OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
