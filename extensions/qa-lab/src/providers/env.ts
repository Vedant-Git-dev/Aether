...
const QA_LIVE_ENV_ALIASES = Object.freeze([
  {
    liveVar: "AETHER_LIVE_OPENAI_KEY",
    providerVar: "OPENAI_API_KEY",
  },
  {
    liveVar: "AETHER_LIVE_ANTHROPIC_KEY",
    providerVar: "ANTHROPIC_API_KEY",
  },
  {
    liveVar: "AETHER_LIVE_GEMINI_KEY",
    providerVar: "GEMINI_API_KEY",
  },
]);

export const QA_LIVE_PROVIDER_CONFIG_PATH_ENV = "AETHER_QA_LIVE_PROVIDER_CONFIG_PATH";
const QA_LIVE_CLI_BACKEND_PRESERVE_ENV = "AETHER_LIVE_CLI_BACKEND_PRESERVE_ENV";
const QA_LIVE_CLI_BACKEND_AUTH_MODE_ENV = "AETHER_LIVE_CLI_BACKEND_AUTH_MODE";
...
export const QA_PROVIDER_SECRET_ENV_VARS = Object.freeze([
  ...
  "OPENAI_API_KEY",
  "OPENAI_API_KEYS",
  "AETHER_LIVE_ANTHROPIC_KEY",
  "AETHER_LIVE_ANTHROPIC_KEYS",
  "AETHER_LIVE_GEMINI_KEY",
...
export function resolveQaLiveCliAuthEnv(
  ...
  const hasAnthropicKey = Boolean(
    baseEnv.ANTHROPIC_API_KEY?.trim() || baseEnv.AETHER_LIVE_ANTHROPIC_KEY?.trim(),
  );
  if (opts?.forwardHostHomeForClaudeCli && authMode === "api-key" && !hasAnthropicKey) {
    throw new Error(
      "Claude CLI API-key QA mode requires ANTHROPIC_API_KEY or AETHER_LIVE_ANTHROPIC_KEY",
    );
  }
...
export function resolveQaLiveProviderConfigPath(env: NodeJS.ProcessEnv = process.env) {
  const explicit =
    env[QA_LIVE_PROVIDER_CONFIG_PATH_ENV]?.trim() || env.AETHER_CONFIG_PATH?.trim();
  return explicit
    ? { path: resolveUserPath(explicit, env), explicit: true }
    : { path: path.join(os.homedir(), ".aether", "aether.json"), explicit: false };
}
