...
import { redactToolPayloadText } from "aether/plugin-sdk/logging-core";
...
import type { SessionCatalogTranscriptItem } from "aether/plugin-sdk/session-catalog";
import {
  listActiveSessionCatalogs,
  type ActiveSessionCatalog,
} from "aether/plugin-sdk/session-catalog-runtime";
import {
  fetchWithSsrFGuard,
...
export function createBeamMirrorRunner(params: {
  ...
  const scan = async (): Promise<void> => {
    try {
      ...
      let agentId: string;
      try {
        agentId = resolveSessionAgentIdsStrict({ config: config as AetherConfig }).defaultAgentId;
      } catch (error) {
