...
import { boundedJsonUtf8Bytes } from "aether/plugin-sdk/text-utility-runtime";
import {
...
function createAgentCard(params: A2aHttpHandlerParams, request: IncomingMessage) {
  ...
  return {
    name: instanceName,
    description: "Aether agent gateway using the Agent2Agent protocol.",
    supportedInterfaces: [
      {
    ...
    skills: agentIds.map((agentId) => ({
      id: agentId,
      name: agentId,
      description: `Aether agent ${agentId}.`,
      tags: ["aether"],
    })),
  };
}
