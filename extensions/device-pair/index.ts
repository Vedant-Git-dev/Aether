// Device Pair plugin entrypoint registers its Aether integration.
import { rm } from "node:fs/promises";
import { isIP } from "node:net";
import os from "node:os";
import path from "node:path";
import { createLazyRuntimeModule } from "aether/plugin-sdk/lazy-runtime";
import { definePluginEntry, type AetherPluginApi } from "aether/plugin-sdk/plugin-entry";
import {
...
function describeSecurePairingFix(source?: string): string {
  const sourceNote = source ? ` Resolved source: ${source}.` : "";
  return (
    "Tailscale and public pairing require a secure gateway URL (wss://) or Tailscale Serve/Funnel." +
    sourceNote +
    ...
    "ws:// setup codes are only valid for localhost/loopback, private LAN addresses, or .local hosts."
  );
}
...
function isPairingCleartextAllowedHost(host: string): boolean {
  const normalized = normalizeHostForIpCheck(host);
  return (
    isLoopbackHost(normalized) || isPrivateLanCleartextHost(normalized)
  );
}

function validatePairingUrl(url: string, source?: string): string | null {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
    return "Resolved pairing URL is invalid.";
  }
  const protocol =
  ...
  if (protocol !== "ws:" || isPairingCleartextAllowedHost(parsed.hostname)) {
    return null;
  }
  return describeSecurePairingFix(source);
}
...
function resolveAuthLabel(cfg: AetherPluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.AETHER_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.AETHER_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: AetherPluginApi): Promise<ResolveUrlResult> {
...
async function resolvePairingGatewayUrl(api: AetherPluginApi): Promise<ResolveUrlResult> {
  const result = await resolveGatewayUrl(api);
  if (!result.url) {
    return result;
  }
  const pairingUrlError = validatePairingUrl(result.url, result.source);
  if (pairingUrlError) {
    return { error: pairingUrlError };
  }
  const urls = result.urls?.filter(
    (url) => !validatePairingUrl(url, "tailscale serve status"),
  );
  return {
    ...result,
...
function buildAccessLines(payload: SetupPayload, markdown = false): string[] {
  ...
  return [`${prefix}Access: ${payload.access}`];
}
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Aether devices",
  register(api: AetherPluginApi) {
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        const urlResult = await resolvePairingGatewayUrl(api);
        if (!urlResult.url) {
        ...
        if (action === "qr") {
          ...
          if (target && qrChannelSender) {
            ...
            try {
              const { resolvePreferredAetherTmpDir, writeQrPngTempFile } =
                await loadDevicePairApiModule();
              qrFilePath = (
                await writeQrPngTempFile(setupCode, {
                  tmpRoot: resolvePreferredAetherTmpDir(),
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                sender: qrChannelSender,
                target,
                caption: ["Scan this QR code with the Aether iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the Aether iOS app:",
                "",
