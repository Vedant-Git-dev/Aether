import { parseAllowFromEntries } from "aether/plugin-sdk/allow-from";
...
function resolveIMessageCliPath(params: { cfg: AetherConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run Aether on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `aether channels status --probe`.",
    "Ensure Aether has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: AetherConfig; accountId?: string }) =>
