...
import { runCommandWithTimeout } from "aether/plugin-sdk/process-runtime";
...
import { detectBinary } from "aether/plugin-sdk/setup";
import {
  filterStringEntries,
...
function resolveIMessageNonMacHostError(
  ...
  return "iMessage via the default imsg CLI must run on macOS. Run Aether on the signed-in Messages Mac, or set channels.imessage.cliPath to an SSH wrapper that runs imsg on that Mac.";
}
...
// subcommand (added by aether/imsg#114, which lets a single bridge call
...
export async function probeIMessagePrivateApi(
  ...
  try {
    ...
    // aether/imsg#114. We do this even when the bridge is unavailable
    ...
    // Caption suppression is required for approval polls because Aether
