...
import { normalizeScpRemoteHost } from "aether/plugin-sdk/host-runtime";
import {
  type CommandOptions,
...
import { sanitizeTempFileName } from "aether/plugin-sdk/temp-path";
...
export async function withIMessageRemoteFile<T>(params: {
  ...
  const remoteDir = `/tmp/aether-imessage-${token}`;
