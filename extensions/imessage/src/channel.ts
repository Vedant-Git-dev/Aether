...
import { buildDmGroupAccountAllowlistAdapter } from "aether/plugin-sdk/allowlist-config-edit";
import type { ChannelApprovalKind } from "aether/plugin-sdk/approval-handler-runtime";
import { formatTrimmedAllowFromEntries } from "aether/plugin-sdk/channel-config-helpers";
import { createChatChannelPlugin, type ChannelPlugin } from "aether/plugin-sdk/channel-core";
import {
...
import type { ChannelOutboundAdapter } from "aether/plugin-sdk/channel-send-result";
...
import { questionGatewayRuntime } from "aether/plugin-sdk/question-gateway-runtime";
import { chunkMarkdownText } from "aether/plugin-sdk/reply-runtime";
import { buildOutboundBaseSessionKey, type RoutePeer } from "aether/plugin-sdk/routing";
import {
...
export const imessagePlugin: ChannelPlugin<ResolvedIMessageAccount, IMessageProbe> =
  createChatChannelPlugin<ResolvedIMessageAccount, IMessageProbe>({
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          "- iMessage current conversation: omit target, to, chatId, chatGuid, and chatIdentifier. Aether resolves the trusted current chat server-side; never copy a redacted display value such as `***` into message actions.",
        ],
      },
