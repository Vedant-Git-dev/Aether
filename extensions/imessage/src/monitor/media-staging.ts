...
import type { ChannelInboundMediaInput } from "aether/plugin-sdk/channel-inbound";
import { isInboundPathAllowed, kindFromMime } from "aether/plugin-sdk/media-runtime";
import { saveMediaBuffer } from "aether/plugin-sdk/media-store";
import { openLocalFileSafely } from "aether/plugin-sdk/security-runtime";
    ...
    if (isHeicAttachment(params.attachmentPath, params.mimeType)) {
      try {
        ...
        const converted = await withTempWorkspace(
          { rootDir: resolvePreferredAetherTmpDir(), prefix: "aether-imessage-heic-" },
          async (workspace) => {
