...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      ...
      'Run "aether doctor --fix" to remove disabled catchup blocks.',
  ...
  const aliases = streamingAliasMigration.normalizeChannelConfig({
    cfg:
      nextImessage === imessage
        ? cfg
        : ({ ...cfg, channels: { ...channels, imessage: nextImessage } } as AetherConfig), // SAFETY: only retired catchup keys are removed.
    changes,
  });
  if (changes.length === 0) {
    return { config: cfg, changes: [] };
