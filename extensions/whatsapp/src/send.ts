...
import type { ChannelOutboundContext } from "aether/plugin-sdk/channel-contract";
import {
...
import { generateSecureUuid } from "aether/plugin-sdk/core";
...
import { loadOutboundMediaFromUrl } from "aether/plugin-sdk/outbound-media";
import { requireRuntimeConfig } from "aether/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "aether/plugin-sdk/poll-runtime";
import { resolveChunkMode, resolveTextChunkLimit } from "aether/plugin-sdk/reply-chunking";
import { createSubsystemLogger, getChildLogger } from "aether/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: AetherConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    const cause = new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`aether channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: AetherConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
