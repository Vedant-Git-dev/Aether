...
import { createPluginRuntimeMock } from "aether/plugin-sdk/plugin-test-runtime";
import { resetInboundDedupe } from "aether/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "aether/plugin-sdk/runtime-env";
import { mockPinnedHostnameResolution } from "aether/plugin-sdk/test-env";
...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("aether:webAutoReplySessionSockets");
...
vi.mock("aether/plugin-sdk/agent-runtime", () => ({
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aether-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-session-"));
