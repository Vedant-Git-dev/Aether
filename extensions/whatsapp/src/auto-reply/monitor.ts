...
import { shouldDebounceTextInbound } from "aether/plugin-sdk/channel-inbound";
import { registerChannelRuntimeContext } from "aether/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "aether/plugin-sdk/cli-runtime";
import { drainPendingDeliveries } from "aether/plugin-sdk/delivery-queue-runtime";
...
import {
  ...
  warn,
  type RuntimeEnv,
} from "aether/plugin-sdk/runtime-env";
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("aether channels login --channel whatsapp")}\` to relink.`,
              );
            } else if (setupDecision.healthState === "conflict") {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then restart the channel. To force a fresh QR, run \`${formatCliCommand("aether channels logout --channel whatsapp")}\` before \`${formatCliCommand("aether channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("aether channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("aether channels login --channel whatsapp")}\` to relink.`,
          );
