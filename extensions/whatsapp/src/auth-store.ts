...
import { DEFAULT_ACCOUNT_ID } from "aether/plugin-sdk/routing";
import {
  info,
  success,
  getChildLogger,
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("aether channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
