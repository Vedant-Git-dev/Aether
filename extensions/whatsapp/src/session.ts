...
import { formatCliCommand, VERSION } from "aether/plugin-sdk/cli-runtime";
import { toErrorObject } from "aether/plugin-sdk/error-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "aether/plugin-sdk/fetch-runtime";
import { danger, success, getChildLogger, toPinoLikeLogger } from "aether/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "aether/plugin-sdk/text-utility-runtime";
import {
  readCredsJsonRaw,
...
const AETHER_WHATSAPP_WEB_SOCKET_URL_ENV = "AETHER_WHATSAPP_WEB_SOCKET_URL";
...
function resolveEnvWaWebSocketUrl(): string | undefined {
  const value = resolveWaWebSocketUrl(process.env[AETHER_WHATSAPP_WEB_SOCKET_URL_ENV]);
  if (!value) {
    return undefined;
  }
  let url: URL;
  try {
    ...
    throw new Error(`${AETHER_WHATSAPP_WEB_SOCKET_URL_ENV} must be a valid URL.`);
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(`${AETHER_WHATSAPP_WEB_SOCKET_URL_ENV} must use ws:// or wss://.`);
  }
  return url.toString();
}
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("aether channels login")}`,
              ),
            );
          }
        }
