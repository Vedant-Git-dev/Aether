...
type WhatsAppConfig = NonNullable<NonNullable<AetherConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: AetherConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: AetherConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: AetherConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: AetherConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("aether channels login"),
      }),
      "WhatsApp",
    );
  }
