...
import { logInfo } from "aether/plugin-sdk/logging-core";
import { getRuntimeConfig } from "aether/plugin-sdk/runtime-config-snapshot";
import { danger, success, defaultRuntime, type RuntimeEnv } from "aether/plugin-sdk/runtime-env";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("aether channels login")} and scan the QR again.`,
        ),
      );
