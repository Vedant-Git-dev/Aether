...
import { createChannelIngressQueueForTests } from "aether/plugin-sdk/channel-ingress-test-runtime";
import { coerceErrorMessage } from "aether/plugin-sdk/error-runtime";
import { resetLogger, setLoggerOverride } from "aether/plugin-sdk/runtime-env";
...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/aether-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/aether-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("aether/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("aether/plugin-sdk/channel-activity-runtime")
  >("aether/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
export async function waitForPairingPromptSent(sock: MockSock, jid: string, senderE164: string) {
  ...
  expect(text).toContain("Aether: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks() {
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "aether-auth-"));
  });
