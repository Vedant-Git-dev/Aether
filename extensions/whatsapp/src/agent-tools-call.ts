...
import { createActionGate, stringEnum } from "aether/plugin-sdk/channel-actions";
import type {
  AnyAgentTool,
  AetherPluginApi,
  AetherPluginToolContext,
} from "aether/plugin-sdk/core";
import { mulawToPcm } from "aether/plugin-sdk/realtime-voice";
import { detectBinary } from "aether/plugin-sdk/setup-tools";
import { resolveOAuthDir } from "aether/plugin-sdk/state-paths";
...
async function resolveRequesterE164(params: {
  accountId: string;
  cfg: NonNullable<AetherPluginToolContext["config"]>;
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      if (params.action === "status") {
        return jsonResult({
          ...
          note: "MeowCaller uses a separate WhatsApp linked-device session; it cannot reuse Aether's Baileys credentials.",
        });
      }
      ...
      if (linkedSelf === target) {
        throw new Error(
          "WhatsApp cannot call the linked account itself; use a dedicated Aether WhatsApp number",
        );
      }
      ...
      try {
        ...
        const tempDir = await fs.mkdtemp(
          path.join(resolvePreferredAetherTmpDir(), "aether-whatsapp-call-"),
        );
...
export function registerWhatsAppCallTool(api: AetherPluginApi): void {
