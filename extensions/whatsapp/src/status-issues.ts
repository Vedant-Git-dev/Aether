    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (healthState === "logged-out") {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("aether channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (healthState === "stale") {
        ...
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("aether doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
        return;
      }
      ...
      if (
        ...
        issues.push({
          ...
          fix: `Watch: ${formatCliCommand("aether logs --follow")} and run ${formatCliCommand("aether channels status --probe")} if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.`,
        });
        return;
      }
