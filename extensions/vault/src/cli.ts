...
import type { AetherConfig } from "aether/plugin-sdk/plugin-entry";
import { createPluginSecretRefSetupCli } from "aether/plugin-sdk/secret-ref-runtime";
...
const vaultSecretRefSetupCli: PluginSecretRefSetupCli = createPluginSecretRefSetupCli({
  ...
  defaultPlanPath: () =>
    path.join(resolvePreferredAetherTmpDir(), `aether-vault-secrets-${process.pid}.json`),
});
...
async function runStatus(config: AetherConfig, options: StatusOptions): Promise<void> {
  const { providerAlias, provider } = vaultSecretRefSetupCli.inspectProvider(
    config,
    options.providerAlias,
  );
  const authMethod = normalizeOptionalString(process.env.AETHER_VAULT_AUTH_METHOD) ?? "token";
  const result = {
    providerAlias,
    provider,
    ...
    authMethod,
    authMount:
      normalizeOptionalString(process.env.AETHER_VAULT_AUTH_MOUNT) ??
      (authMethod === "kubernetes" ? "kubernetes" : "jwt"),
    authRole: normalizeOptionalString(process.env.AETHER_VAULT_AUTH_ROLE),
    hasJwtFile: Boolean(normalizeOptionalString(process.env.AETHER_VAULT_JWT_FILE)),
    hasVaultTokenFile: Boolean(normalizeOptionalString(process.env.VAULT_TOKEN_FILE)),
    kvMount: normalizeOptionalString(process.env.AETHER_VAULT_KV_MOUNT) ?? "secret",
    kvVersion: normalizeOptionalString(process.env.AETHER_VAULT_KV_VERSION) ?? "2",
  ...
  writeLine(`AETHER_VAULT_JWT_FILE: ${result.hasJwtFile ? "set" : "not set"}`);
