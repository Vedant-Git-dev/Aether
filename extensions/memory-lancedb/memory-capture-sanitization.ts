import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "aether/plugin-sdk/chat-channel-ids";
...
/**
 * Provenance marker appended to every Aether-injected inbound context header
...
const CONTEXT_HEADER_RE = /^Context:[ \t]*⟦aether:ctx⟧[ \t]*$/m;

/**
 * Matches JSON blobs that look like Aether transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains Aether-injected envelope or
...
/**
 * Strips Aether-injected envelope metadata from a user message so that only
