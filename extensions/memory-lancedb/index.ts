...
import { enqueueKeyedTask } from "aether/plugin-sdk/keyed-async-queue";
import { createLazyRuntimeModule } from "aether/plugin-sdk/lazy-runtime";
import { readFiniteNumberParam, readPositiveIntegerParam } from "aether/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "aether/plugin-sdk/plugin-config-runtime";
import { isIncognitoSessionKey, normalizeAgentId } from "aether/plugin-sdk/routing";
...
import { textResult } from "aether/plugin-sdk/tool-results";
import { Type } from "typebox";
import { definePluginEntry, type AetherPluginApi } from "./api.js";
...
const loadMemoryHostCoreModule = createLazyRuntimeModule(
  () => import("aether/plugin-sdk/memory-host-core"),
);
...
export default definePluginEntry({
  ...
  register(api: AetherPluginApi) {
    ...
    const resolveRuntimeConfig = (): AetherConfig =>
      (api.runtime.config?.current?.() ?? api.config) as AetherConfig;
    ...
    const assertRetainedToolEnabled = (
      agentId: string,
      getRuntimeConfig: (() => AetherConfig | undefined) | undefined,
    ): void => {
      if (!getRuntimeConfig) {
        return;
      }
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as AetherConfig
          : undefined,
        "memory-lancedb",
