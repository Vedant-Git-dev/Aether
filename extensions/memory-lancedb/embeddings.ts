...
import { formatErrorMessage, toErrorObject } from "aether/plugin-sdk/error-runtime";
...
import type { MemoryEmbeddingProvider } from "aether/plugin-sdk/memory-core-host-engine-embeddings";
import { resolveTimerTimeoutMs } from "aether/plugin-sdk/number-runtime";
import { normalizeAgentId } from "aether/plugin-sdk/routing";
import { ensureGlobalUndiciEnvProxyDispatcher } from "aether/plugin-sdk/runtime-env";
import { asOptionalRecord } from "aether/plugin-sdk/string-coerce-runtime";
import { textResult, type AgentToolResult } from "aether/plugin-sdk/tool-results";
import type { AetherPluginApi } from "./api.js";
...
const loadMemoryEmbeddingProviderModule = createLazyRuntimeModule(
  () => import("aether/plugin-sdk/memory-core-host-engine-embeddings"),
);
...
const PROVIDER_ADAPTER_LIFECYCLE = resolveGlobalSingleton<ProviderAdapterLifecycleState>(
  Symbol.for("aether.memoryLanceDbEmbeddingProviderLifecycle.v1"),
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  constructor(private api: AetherPluginApi) {}

  private getProvider(agentId: string, embedding: EmbeddingConfig): AgentEmbeddingProvider {
    const config = (this.api.runtime.config?.current?.() ?? this.api.config) as AetherConfig;
...
export function createEmbeddings(api: AetherPluginApi): Embeddings {
