...
function buildLoadFailureMessage(error: unknown): string {
  return [
    ...
    "Install or repair the memory-lancedb plugin package dependencies, then restart Aether.",
    String(error),
  ].join(" ");
}
...
if (process.env.VITEST === "true") {
  Reflect.set(globalThis, Symbol.for("aether.memoryLanceDbRuntimeTestApi"), {
