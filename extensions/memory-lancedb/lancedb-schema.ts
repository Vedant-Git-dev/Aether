...
export function legacyMemorySchemaError(): Error {
  return new Error(
    'memory-lancedb: the existing memory table predates per-agent isolation. Run "aether doctor --fix" to assign legacy rows to the default agent, then restart Aether.',
  );
}
