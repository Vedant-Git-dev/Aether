...
export function buildConfigItems(params: {
  ...
  if (params.hasMemoryFiles || memoryProvider) {
    addConfigPatch({
      ...
      message: "Select the default Aether memory plugin for imported file memory.",
    });
  }
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes used Honcho memory. Aether keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createMigrationManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known Aether mapping.`,
        recommendation: "Install or configure an equivalent Aether memory plugin manually.",
      }),
    );
  }
