    ...
    if (transport && !HERMES_TRANSPORTS[transport]) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an equivalent Aether provider plugin or API adapter manually.",
        }),
      );
    ...
    if (normalizeOptionalString(inlineKey) && !readEnvReference(inlineKey)) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" contains an inline API key that was not copied into Aether config.`,
          recommendation: "Move the key to an environment variable or Aether secret provider.",
        }),
      );
    }
    ...
    if (isRecord(raw.extra_body) && Object.keys(raw.extra_body).length > 0) {
      items.push(
        createMigrationManualItem({
          ...
          message: `Hermes provider "${id}" adds request body fields that Aether cannot import generically.`,
          recommendation:
    ...
    if (apiKeyEnv && !env[apiKeyEnv]?.trim()) {
      items.push(
        createMigrationManualItem({
          ...
          recommendation:
            "Configure an Aether auth profile for this provider or expose the variable to the Aether runtime.",
        }),
      );
    }
  }
