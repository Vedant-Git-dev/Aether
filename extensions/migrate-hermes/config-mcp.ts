...
import { createMigrationManualItem } from "aether/plugin-sdk/migration";
import type { MigrationItem } from "aether/plugin-sdk/plugin-entry";
import {
  asBoolean,
  isRecord,
...
function hasUnsupportedToolPattern(pattern: string): boolean {
  // Hermes uses fnmatch; Aether supports only exact names and `*`.
...
function mapHermesToolFilter(value: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  if (include !== undefined) {
    ...
    // Hermes' explicit empty include disables native tools; Aether's empty
...
export function mcpManualItems(params: {
  ...
  if (
    ...
    add(
      ...
      "Define the missing values in Aether's MCP server environment or headers manually.",
    );
  }
  ...
  if (Array.isArray(cert) && cert.length === 3) {
    add(
      ...
      `Hermes MCP server "${name}" uses a password-protected client key, which Aether cannot represent in MCP config.`,
    ...
    add(
      ...
      "Configure separate Aether clientCert and clientKey file paths manually.",
    );
  }
  if (typeof (raw.sslVerify ?? raw.ssl_verify) === "string") {
    add(
      "tls-ca",
      `Hermes MCP server "${name}" uses a CA bundle path for TLS verification, which Aether MCP config cannot represent.`,
  ...
  if (transport && !["http", "streamable-http", "sse", "stdio"].includes(transport)) {
    add(
      ...
      "Configure an equivalent Aether MCP transport manually.",
    );
  }
  ...
  if (auth && auth !== "oauth") {
    add(
      ...
      "Configure an equivalent Aether MCP authentication mode manually.",
    );
  }
  ...
  if (auth === "oauth" || oauth) {
    add(
      "oauth-login",
      `Hermes MCP server "${name}" requires OAuth login in Aether.`,
      `Run "aether mcp login ${name}" after migration.`,
    );
  }
  if (
    oauth &&
    ...
    add(
      "oauth-client",
      `Hermes MCP server "${name}" uses pre-registered OAuth client settings that were not copied into Aether config.`,
      `Run "aether mcp login ${name}" and configure supported OAuth metadata manually.`,
    );
  }
  ...
  if (activePatterns?.some(hasUnsupportedToolPattern)) {
    add(
      ...
      include === undefined
        ...
        : `Hermes MCP server "${name}" has tool include patterns that were omitted because Aether supports only exact names and "*".`,
  ...
  for (const [feature, configured] of unsupported) {
    if (configured) {
      add(
        feature,
        `Hermes MCP server "${name}" uses ${feature} behavior that Aether MCP config does not expose.`,
