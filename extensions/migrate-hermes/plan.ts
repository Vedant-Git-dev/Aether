...
export async function buildHermesPlan(ctx: MigrationProviderContext): Promise<MigrationPlan> {
  ...
  if (
    ...
    authItems.unshift(
      createMigrationManualItem({
        ...
        message: "Hermes Qwen Portal OAuth and Qwen CLI credentials cannot be reused by Aether.",
        recommendation:
          "Authenticate qwen with an API key after migration: aether onboard --auth-choice qwen-api-key.",
      }),
    );
  }
  ...
  const warnings = [
    ...
    ...(items.some(
      ...
      ? [
          "Hermes and Aether must not keep using the same imported OpenAI OAuth refresh grant after migration; reauthenticate one side before running both.",
        ]
      : []),
    ...
    ...(source.archivePaths.length > 0
      ? [
          "Some Hermes files are archive-only. They will be copied into the migration report for manual review, not loaded into Aether.",
        ]
      : []),
  ...
  return {
    ...
    items,
    warnings,
    nextSteps: ["Run aether doctor after applying the migration."],
