...
export function teamsMeetingStatusPreludeSource(params: MeetingStatusPreludeParams): string {
  ...
  if (canMutateSession && (identityVerifiedBeforeCall || identityPreservedInCall)) {
    ...
    delete window.__aetherTeamsMeeting;
  }
  ...
  const ensureVirtualAudioInput = async () => {
    ...
    try {
      ...
      const preparedInput = window.__aetherTeamsMeeting;
      ...
      if (selected && window.__aetherTeamsMeeting?.identity === expectedIdentity) {
        window.__aetherTeamsMeeting.audioInputDeviceId = input.deviceId;
      }
      return selected;
    } catch (error) {
  ...
  if (identityVerified && !inCall && allowMicrophone && microphone) {
    ...
    if (!audioInputRouted) {
      ...
      controlManualAction = manualActionFor("teams-audio-choice-required", "Select the Aether virtual audio device as the Teams microphone and verify it is selected before enabling talk-back.");
  ...
  if (committedOwnerConflict && !canMutateSession) {
    ...
    manualAction = manualActionFor("teams-login-required", tenantLoginRequired ? "This Teams tenant requires sign-in or email verification. Complete it in the Aether browser profile, then retry." : "Sign in to Microsoft Teams in the Aether browser profile, then retry the meeting join.");
  } else if (!inCall && lobbyWaiting) {
    manualAction = manualActionFor("teams-admission-required", "Admit the Aether guest from the Microsoft Teams lobby, then retry speech.");
  } else if (!inCall && permissionRequired) {
    manualAction = manualActionFor("teams-permission-required", allowMicrophone ? "Allow microphone permission for Teams in the Aether browser profile, then retry." : "Dismiss the Teams device-permission prompt or continue without devices, then retry.");
  ...
  if (canMutateSession && identityVerified && autoJoin && !inCall && join && !join.disabled && !manualAction) {
    ...
    platform: {
      displayName: "Teams",
      globals: {
        audioOutputs: "__aetherTeamsAudioOutputs",
        captionArchive: "__aetherTeamsCaptionArchive",
        captions: "__aetherTeamsCaptions",
        meeting: "__aetherTeamsMeeting",
      },
