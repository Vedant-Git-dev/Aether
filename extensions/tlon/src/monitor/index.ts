...
import type { GetReplyOptions, ReplyPayload } from "aether/plugin-sdk/reply-runtime";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.current() as AetherConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in Aether config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your Aether config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://docs.aether.ai/concepts/session#dm-isolation`;
