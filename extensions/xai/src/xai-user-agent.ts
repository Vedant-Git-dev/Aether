// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatAetherUserAgent`.

import { AETHER_VERSION as PACKAGE_VERSION } from "aether/plugin-sdk/agent-harness-runtime";
import { normalizeOptionalString } from "aether/plugin-sdk/string-coerce-runtime";

const ORIGINATOR = "aether";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = normalizeOptionalString(process.env.AETHER_VERSION);
  if (envVersion) {
    return envVersion;
  }
...
// an empty record so the aether identity is not forwarded to the proxy.
