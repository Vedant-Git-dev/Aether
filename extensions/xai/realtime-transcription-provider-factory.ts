...
} from "aether/plugin-sdk/realtime-transcription-session";
...
async function resolveXaiRealtimeApiKey(
  ...
  throw new Error(
    "xAI credentials missing for realtime STT. Sign in with `aether onboard --auth-choice xai-oauth`, or run `aether onboard --auth-choice xai-api-key`, or set XAI_API_KEY.",
  );
}
