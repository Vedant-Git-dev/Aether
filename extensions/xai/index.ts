...
// Xai plugin entrypoint registers its Aether integration.
...
import { defaultToolStreamExtraParams } from "aether/plugin-sdk/provider-stream-shared";
import { jsonResult } from "aether/plugin-sdk/provider-web-search";
import {
...
function createLazyCodeExecutionTool(ctx: AetherPluginToolContext) {
...
function createLazyXSearchTool(ctx: AetherPluginToolContext) {
  const effectiveConfig = ctx.runtimeConfig ?? ctx.config;
    ...
    catalog: {
      ...
      run: async (ctx) => {
        ...
        const { resolveApiKeyForProvider } =
          await import("aether/plugin-sdk/provider-auth-runtime");
