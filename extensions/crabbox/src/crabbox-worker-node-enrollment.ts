import type { WorkerProvider } from "aether/plugin-sdk/plugin-entry";
...
const setPhase = (next) => {
  ...
  phase = next;
  console.error("CRABBOX_PHASE:aether-bootstrap-" + next.toLowerCase().replaceAll(" ", "-"));
};
...
(async () => {
  ...
  const stateDir = path.join(home, ".aether", "cloud-workers", leaseId);
  const runtimeRoot = path.join(home, ".aether-worker", "node-runtimes");
  const runtimeDir = path.join(runtimeRoot, bootstrap.sha256);
  const cli = path.join(runtimeDir, "node_modules", "aether", "aether.mjs");
  ...
  const nodeEnv = { ...process.env, ...(mode ? { AETHER_STATE_DIR: stateDir } : {}) };
  ...
  if (mode && fs.existsSync(pidFile)) {
    ...
    if (alive) {
      ...
      if (process.platform === "linux") {
        ...
        // Aether changes process.title; the immutable install cwd survives that argv rewrite.
        const title = args[0];
        const nodeInvocation = args[1] === cli || ["aether", "aether-connect", "aether-node"].includes(title);
        verified = nodeInvocation && fs.realpathSync(path.join("/proc", pidText, "cwd")) === runtimeDir && env.includes("AETHER_STATE_DIR=" + stateDir);
        ...
        try {
          ...
          const nodeInvocation = command !== null && (["aether", "aether-connect", "aether-node"].includes(command) || command === invocation || command.startsWith(invocation + " "));
  ...
  const verifyRuntime = (root) => {
    ...
    const packageRoot = path.join(root, "node_modules", "aether");
    const manifest = JSON.parse(fs.readFileSync(path.join(packageRoot, "package.json"), "utf8"));
    if (manifest.name !== "aether" || manifest.version !== bootstrap.aetherVersion) throw new Error("Cloud worker bootstrap package identity does not match the Gateway");
    const probe = spawnSync(process.execPath, [path.join(packageRoot, "aether.mjs"), "--version"], { env: nodeEnv, encoding: "utf8", timeout: 60000 });
    const version = probe.stdout?.trim();
    const expected = "Aether " + bootstrap.aetherVersion;
  ...
  const workerArchivePath = (root) => {
    ...
    return path.join(root, "node_modules", "aether", ...parts);
  };
  ...
  try {
    const archive = path.join(stage, "aether.tgz");
  ...
  try {
    ...
    if (process.platform === "win32") {
      ...
      const launchScript = "$env:AETHER_STATE_DIR = " + quote(stateDir) + "; & " + [process.execPath, ...nodeArgs].map(quote).join(" ") + " *>> " + quote(logPath) + "; exit $LASTEXITCODE";
