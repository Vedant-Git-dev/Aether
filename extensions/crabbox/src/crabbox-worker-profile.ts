...
import { normalizeOptionalString as nonEmptyString } from "aether/plugin-sdk/string-coerce-runtime";
...
const CRABBOX_LEASE_ID_DOMAIN = "aether:crabbox-worker-lease-id:v1\0";
...
export function resolveAetherRoot(pluginRoot: string | undefined): string {
  if (!pluginRoot) {
    return process.cwd();
  }
...
export function operationSlug(operationId: string): string {
  return `aether-${createHash("sha256").update(operationId).digest("hex").slice(0, 32)}`;
}
