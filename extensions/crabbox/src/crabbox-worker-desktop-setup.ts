import type { WorkerDesktopEndpoint } from "aether/plugin-sdk/plugin-entry";

const CRABBOX_WORKER_BROWSER_PATH = "/usr/local/bin/aether-worker-browser";
const CRABBOX_WORKER_TERMINAL_PATH = "/usr/local/bin/aether-worker-terminal";
...
function browserLauncher(leaseId: string): string[] {
  return [
    "#!/bin/bash",
    "set -euo pipefail",
    '[ "$#" -eq 0 ] || { echo "aether-worker-browser does not accept arguments" >&2; exit 64; }',
    ...xfceDesktopEnvironment(),
    ...
    `export CRABBOX_BROWSER_PROFILE="$worker_home/.cache/aether/worker-browser/${leaseId}"`,
    'mkdir -p "$CRABBOX_BROWSER_PROFILE"',
    'chmod 700 "$CRABBOX_BROWSER_PROFILE"',
    'exec 9>"$CRABBOX_BROWSER_PROFILE/.aether-launch.lock"',
    "flock -x 9",
...
function terminalLauncher(): string[] {
  return [
    "#!/bin/bash",
    "set -euo pipefail",
    '[ "$#" -eq 0 ] || { echo "aether-worker-terminal does not accept arguments" >&2; exit 64; }',
    ...xfceDesktopEnvironment(),
...
export function createCrabboxWorkerDesktopSetup(leaseId: string, wallpaperBase64: string): string {
  return [
    ...
    'as_root install -d -o "$worker_user" -g "$worker_group" -m 0755 "$worker_home/.cache" "$worker_home/.cache/aether" "$worker_home/.cache/aether/worker-browser"',
    `as_root install -d -o "$worker_user" -g "$worker_group" -m 0700 "$worker_home/.cache/aether/worker-browser/${leaseId}"`,
    ...
    'wallpaper_path="$worker_home/.local/share/backgrounds/aether-worker.png"',
    ...
    'nohup xfdesktop >"$worker_home/.cache/aether/xfdesktop.log" 2>&1 </dev/null &',
