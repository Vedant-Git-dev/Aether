...
export function createCrabboxWarmImageManager(dependencies: {
  ...
  const retireImage = async (
    ...
    try {
      ...
      if (matches(openStore().lookup(key))) {
        warnOnce(
          `checkpoint retirement (${operation.checkpointId} deletion obligation retained; retry during periodic maintenance or next warm-image-enabled worker teardown; inspect with aether crabbox warm-images)`,
          error,
        );
      }
      return;
  ...
  const makeRoom = async (context: LeaseContext) => {
    ...
    if (openStore().entries().length >= WARM_IMAGE_MAX_ENTRIES) {
      throw new Error(
        "Crabbox warm-image profile capacity is full; stop outstanding workers or resolve cleanup with aether crabbox warm-images before retrying.",
      );
    }
  };
