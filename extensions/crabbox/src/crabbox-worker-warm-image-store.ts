...
import { createPluginStateSyncKeyedStore } from "aether/plugin-sdk/plugin-state-store-runtime";
...
export function assertCrabboxWarmImageMigrationReady(): void {
  if (listCrabboxLegacyWarmLeases().length > 0) {
    throw new Error(
      "Crabbox has legacy worker allocations whose original image choices are unknown; run aether doctor --fix and follow its provider-cleanup recovery instructions before provisioning workers.",
    );
  }
}
...
function requireCanonicalProfile(record: WarmProfileRecord | undefined) {
  if (record && record.version !== 3) {
    throw new Error(
      "Crabbox warm-image state requires migration; run aether doctor --fix before provisioning workers.",
    );
  }
  ...
  if (
    ...
    throw new Error("Crabbox warm-image preparation state is invalid; run aether doctor --fix.");
  }
  return record;
}
...
export function openCrabboxWarmImageStore(env?: NodeJS.ProcessEnv) {
  ...
  const lookupLease = (id: string) => {
    ...
    if (entries.length > 1) {
      throw new Error(
        `Crabbox lease ${id} has conflicting warm-image owners; run aether doctor --fix.`,
      );
    }
...
export const CRABBOX_WARM_IMAGE_WAIT_HINT =
  "The capture may still be preparing its source or waiting for provider readiness. Inspect aether crabbox warm-images --json and allow the owning capture to settle.";

export function crabboxWarmImageRecoveryHint(selector: string): string {
  return `Stop the owning Gateway and capture processes, confirm any worker being recovered is stopped, and resolve any untracked checkpoint in the Crabbox catalog before running: aether crabbox warm-images --recover ${selector} --acknowledge-provider-cleanup. Then restart the Gateway; the next eligible worker can capture again.`;
}
...
export function recoverCrabboxWarmImageCapture(
  ...
  if (selector.startsWith("legacy-lease-")) {
    ...
    if (
      ...
      throw new Error(
        "Legacy allocation selector is absent or changed; rerun aether crabbox warm-images --json. No state was changed.",
      );
    }
    return;
  }
  ...
  if (!entry || !clearCrabboxWarmImageCapture(entry.key, selector)) {
    throw new Error(
      "Capture selector is absent or changed; rerun aether crabbox warm-images --json. No state was changed.",
    );
  }
}
