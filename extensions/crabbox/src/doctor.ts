  ...
  return {
    ...
    async detect(ctx) {
      ...
      for (const [profileId, profile] of profiles) {
        ...
        const installed = findCrabboxBinary({ explicit: managedPath, aetherRoot });
        ...
        findings.push({
          ...
          severity: "warning",
          source: "crabbox",
          message: `Cloud worker profile "${profileId}" ${reason}. Aether will install its managed Crabbox before use.`,
          ...
          fixHint: `Run \`aether doctor --fix\` to install the managed Crabbox now, or provision Crabbox ${managedBinary.CRABBOX_MIN_VERSION} or newer using \`cloudWorkers.profiles.${profileId}.settings.binary\`. The existing executable and profile configuration are preserved.`,
        });
      }
      return findings;
    },
...
export function registerCrabboxWorkerProviderDoctorChecks(
  ...
  if (!host.getHealthCheck(CRABBOX_CLOUD_WORKER_PROFILE_CHECK_ID)) {
    host.registerHealthCheck(createCrabboxCloudWorkerProfileCheck(host.aetherRoot));
  }
        ...
        for (const image of listCrabboxWarmImages(ctx.env)) {
          ...
          if (image.retirement) {
            findings.push({
              ...
              fixHint:
                "Cleanup retries during the next warm-image capture or worker teardown. Inspect `aether crabbox warm-images --json` and resolve provider deletion errors if it remains pending.",
            });
          }
        }
        return findings;
