...
// under AETHER_STATE_DIR here; deleting it is the cross-session data boundary.
...
// in .aether-worker are machine-level caches and intentionally survive.
export const SCRUB_WORKER_STATE = `set -eu
worker_root="$HOME/.aether/cloud-workers"
node <<'CRABBOX_SCRUB_NODE_SCRIPT'
const fs = require("node:fs");
const path = require("node:path");
const os = require("node:os");
const root = path.join(os.homedir(), ".aether", "cloud-workers");
const runtimeRoot = path.join(os.homedir(), ".aether-worker", "node-runtimes") + path.sep;
...
(async () => {
  ...
  for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    const owned = () => {
      ...
      if (!runtime.startsWith(runtimeRoot) || cwd !== runtime || Number(fields[2]) !== pid || !env.includes("AETHER_STATE_DIR=" + stateDir)) throw new Error("Cannot scrub a worker whose live node ownership does not match");
      return true;
    };
    if (!owned()) continue;
...
rm -rf "$worker_root"
rm -rf "$HOME/.aether-worker/workspaces"
