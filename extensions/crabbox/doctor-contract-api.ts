      ...
      return pending || leases.length
        ? {
            preview: [
              ...
              ...leases.map(
                (lease) =>
                  `- Legacy Crabbox lease ${lease.leaseId} requires provider cleanup acknowledgement: aether crabbox warm-images --recover ${lease.selector} --acknowledge-provider-cleanup`,
              ),
            ],
          }
        : null;
    ...
    async migrateLegacyState({ context, env }) {
      ...
      if (leases.length) {
        ...
        for (const lease of leases) {
          warnings.push(
            `Resolve lease ${lease.leaseId} through its original Gateway or provider, stop the original Gateway/capture processes, and confirm provider cleanup before running: aether crabbox warm-images --recover ${lease.selector} --acknowledge-provider-cleanup. Then rerun aether doctor --fix. The row was not changed.`,
          );
        }
      }
