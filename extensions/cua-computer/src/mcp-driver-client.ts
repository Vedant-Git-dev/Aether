...
import { asOptionalRecord as record } from "aether/plugin-sdk/string-coerce-runtime";
import {
  ClickButton,
  EscalationReason,
  ScrollDirection,
...
function createClient(binaryPath: string, socketPath: string, env: NodeJS.ProcessEnv) {
  ...
  return import("aether/plugin-sdk/agent-harness-runtime")
    ...
    .then(({ createMcpStdioClient }) =>
      createMcpStdioClient({
        ...
        clientInfo: { name: "aether-cua-computer", version: "1" },
...
class McpCuaDriverSession implements CuaDriverSession {
  ...
  private readonly publicSession = `aether-${randomUUID()}`;
