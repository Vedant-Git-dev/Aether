...
import type { registerPluginHttpRoute } from "aether/plugin-sdk/webhook-ingress";
...
vi.mock("aether/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("aether/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("aether/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("aether/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./outbound-media.js", async () => {
  ...
  return {
    ...
    prepareSynologyHostedMedia: vi.fn(async () => ({
      url: "https://gateway.example.com/webhook/synology?__aether_synology_media_token_test=value",
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/aether/synology-chat-sessions.json"),
