/**
 * Synology Chat Channel Plugin for Aether.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "aether/plugin-sdk/account-id";
import type { AetherConfig } from "aether/plugin-sdk/account-resolution";
import {
...
import { createConditionalWarningCollector } from "aether/plugin-sdk/channel-policy";
import { createEmptyChannelDirectoryAdapter } from "aether/plugin-sdk/directory-runtime";
import {
  channelBlockedPatch,
  channelReadyPatch,
  channelStoppedPatch,
} from "aether/plugin-sdk/gateway-runtime";
import { parseStrictNonNegativeInteger } from "aether/plugin-sdk/number-runtime";
import type { OutboundMediaLoadOptions } from "aether/plugin-sdk/outbound-media";
import {
...
} from "aether/plugin-sdk/text-chunking";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "aether pairing approve synology-chat <code>",
...
> & {
  ...
  security: {
    resolveDmPolicy: (params: { cfg: AetherConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to Aether",
        order: 90,
      },
      capabilities: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "  Aether freezes the bytes and gives the NAS a short-lived download capability (max 32 MB).",
          "",
          "**Limitations**:",
