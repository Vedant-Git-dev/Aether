...
import { sendHttpRequestRejection } from "aether/plugin-sdk/webhook-request-guards";
...
function extractTokenFromHeaders(req: IncomingMessage): string | undefined {
  const explicit =
    ...
    headerValue(req.headers["x-aether-token"]);
  if (explicit) {
    return explicit;
  }
...
const SYNOLOGY_WEBHOOK_ACCEPTED_HEADER = "x-aether-delivery-accepted";
