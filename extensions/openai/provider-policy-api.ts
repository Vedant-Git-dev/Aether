...
import type { ProviderNormalizeResolvedModelContext } from "aether/plugin-sdk/plugin-entry";
import {
...
} from "aether/plugin-sdk/provider-model-types";
...
export function resolveFastModeSupport(ctx: ProviderFastModePolicyContext): boolean | undefined {
  if (!ctx.api || !ctx.baseUrl || ctx.runtimeId !== "aether") {
    return undefined;
  }
  return (
...
const OPENAI_AGENT_RUNTIME_ID = "aether";
const CODEX_AGENT_RUNTIME_ID = "codex";
const AETHER_RUNTIME_COMPATIBLE_IDS = [OPENAI_AGENT_RUNTIME_ID] as const;
...
function resolveSingleObservedModelRoute(
  ...
  // and let the Aether runtime own the request.
  const platformApi =
...
export function resolveModelRoutes(
  ...
  return {
    ...
    defaultRuntimeId: resolutions.some(
      (resolution) => resolution.kind === "routes" && resolution.defaultRuntimeId === "aether",
    )
      ? OPENAI_AGENT_RUNTIME_ID
