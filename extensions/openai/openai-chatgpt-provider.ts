 ...
 * entirely when they were written by older Aether versions. When that row wins
...
async function runOpenAICodexOAuth(ctx: OpenAICodexOAuthContext) {
  const [{ loginOpenAICodexOAuth }, { buildOauthProviderAuthResult }] = await Promise.all([
    ...
    import("aether/plugin-sdk/provider-auth-result"),
  ]);
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.aether.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `aether models auth login --provider openai` or `aether configure`.";
}
