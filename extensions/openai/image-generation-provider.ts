...
import { bufferToBlobPart, canonicalizeBase64 } from "aether/plugin-sdk/blob-runtime";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import type {
  ImageGenerationOutputFormat,
  ImageGenerationProvider,
  ImageGenerationResult,
} from "aether/plugin-sdk/image-generation";
import type { resolveClosestSize } from "aether/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "aether/plugin-sdk/media-mime";
import type { AetherPluginApi } from "aether/plugin-sdk/plugin-entry";
import type { AuthProfileStore } from "aether/plugin-sdk/provider-auth";
import type { resolveApiKeyForProvider } from "aether/plugin-sdk/provider-auth-runtime";
import { hasConfiguredSecretInput } from "aether/plugin-sdk/secret-input";
import { isPrivateNetworkOptInEnabled } from "aether/plugin-sdk/ssrf-policy";
import { filterStringRecord } from "aether/plugin-sdk/string-coerce-runtime";
...
function resolveConfiguredOpenAIImageBaseUrl(cfg: AetherConfig | undefined, model: string) {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.AETHER_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}

type OpenAIImageModelAuth = Pick<
  AetherPluginApi["runtime"]["modelAuth"],
...
function hasDirectOpenAIImageApiKeyAuth(
  params: { cfg?: AetherConfig; agentDir?: string },
  modelAuth: OpenAIImageModelAuth,
): boolean {
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: AetherConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: AetherConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: AetherConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: AetherConfig | undefined): AetherConfig | undefined {
...
async function resolveOptionalApiKeyForProvider(
  ...
  const { resolveApiKeyForProvider } = await import("aether/plugin-sdk/provider-auth-runtime");
  try {
...
async function logCodexImageAuthSelected(params: {
  ...
  const { createSubsystemLogger } = await import("aether/plugin-sdk/logging-core");
...
async function generateOpenAICodexImage(params: {
  ...
  const [
    ...
    import("aether/plugin-sdk/provider-http"),
    import("aether/plugin-sdk/image-generation"),
    import("aether/plugin-sdk/media-generation-runtime"),
  ]);
