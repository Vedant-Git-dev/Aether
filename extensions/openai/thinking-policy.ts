...
import { normalizeLowercaseStringOrEmpty as normalizeModelId } from "aether/plugin-sdk/string-coerce-runtime";
import {
...
function buildOpenAIThinkingProfile(params: {
  ...
  if (modelId === OPENAI_GPT_6_ASTRA_MODEL_ID) {
    ...
    const supportsUltra =
      ["aether", "codex", "auto"].includes(agentRuntime) && efforts.includes("max");
    return {
  ...
  // Aether owns its logical Ultra orchestration. Native Codex capabilities
