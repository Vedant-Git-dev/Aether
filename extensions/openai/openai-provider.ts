...
import type { LiveModelCatalogFetchGuard } from "aether/plugin-sdk/provider-catalog-live-runtime";
import type { ProviderCatalogOutcome } from "aether/plugin-sdk/provider-catalog-shared";
...
import {
  ...
  normalizeProviderId,
} from "aether/plugin-sdk/provider-model-metadata";
import type {
...
type OpenAILiveModelReaders = Pick<
  typeof import("aether/plugin-sdk/provider-catalog-live-runtime"),
...
async function buildOpenAILiveProviderConfig(
  ...
  const [
    ...
    import("aether/plugin-sdk/provider-catalog-live-runtime"),
    import("aether/plugin-sdk/provider-auth"),
  ]);
...
async function buildOpenAICodexLiveProviderConfig(params: {
  ...
  const catalogRuntime = await import("aether/plugin-sdk/provider-catalog-live-runtime");
...
function buildOpenAIUnknownModelHint(modelId: string): string | undefined {
  ...
  return "gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `aether models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.";
}
    ...
    catalog: {
      ...
      run: async (ctx) => {
        ...
        const [
          ...
          import("aether/plugin-sdk/provider-auth-runtime"),
          import("aether/plugin-sdk/provider-auth"),
        ]);
