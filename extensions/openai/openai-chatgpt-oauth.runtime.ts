...
import type { ProviderAuthContext } from "aether/plugin-sdk/plugin-entry";
import type { OAuthCredentials } from "aether/plugin-sdk/provider-oauth-runtime";
...
const openAICodexOAuthOriginator = "aether";
  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Aether process and then retry `aether models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
...
export async function loginOpenAICodexOAuth(params: {
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this Aether process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
  ...
  try {
    ...
    await prompter.note("Trouble with OAuth? See https://docs.aether.ai/start/faq", "OAuth help");
