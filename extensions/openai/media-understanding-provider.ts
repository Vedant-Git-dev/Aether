...
export const openaiMediaUnderstandingProvider: MediaUnderstandingProvider = {
  ...
  async describeImage(req) {
    const { describeImageWithModel } = await import("aether/plugin-sdk/media-understanding");
    return describeImageWithModel(req);
  },
  async describeImages(req) {
    const { describeImagesWithModel } = await import("aether/plugin-sdk/media-understanding");
