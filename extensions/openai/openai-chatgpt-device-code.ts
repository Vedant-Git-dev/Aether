...
import { resolveOpenAICodexAccessTokenExpiry } from "aether/plugin-sdk/provider-auth";
import { readResponseTextLimited } from "aether/plugin-sdk/provider-http";
import { classifyTransientNetworkErrorCode } from "aether/plugin-sdk/retry-runtime";
...
function resolveOpenAICodexDeviceCodeHeaders(contentType: string): Record<string, string> {
  const version = process.env.AETHER_VERSION?.trim();
  return {
    "Content-Type": contentType,
    originator: "aether",
    ...(version ? { version } : {}),
    "User-Agent": version ? `aether/${version}` : "aether",
  };
}
