...
- Extension production code should import from `aether/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `aether.plugin.json` and the package
  `aether` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `aether/plugin-sdk/*`.
