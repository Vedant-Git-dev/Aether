// Github Copilot plugin entrypoint registers its Aether integration.
...
function resolveCopilotConfiguredPrimary(cfg: AetherConfig): string {
...
function applyCopilotDefaultModel(cfg: AetherConfig, modelRef: string): AetherConfig {
...
function buildGithubCopilotDomainConfigPatch(domain: string): Partial<AetherConfig> {
  ...
  } as unknown as Partial<AetherConfig>;
}
...
function clearGithubCopilotDomainConfigPatch(): Partial<AetherConfig> {
  return {
    models: {
      providers: {
  ...
  register(api) {
    ...
    async function runGitHubCopilotDeviceAuth(
      ...
      const notes = [
        ...
        ...(persistInline
          ? [
              "Plaintext secret input mode was selected, so the GitHub Copilot token will remain inline in the auth profile and aether secrets audit --check will report it.",
            ]
          : []),
      ];
      return {
