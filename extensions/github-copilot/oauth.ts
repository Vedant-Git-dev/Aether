import { MAX_DATE_TIMESTAMP_MS } from "aether/plugin-sdk/number-runtime";
import type { ProviderAuthDoctorHintContext } from "aether/plugin-sdk/plugin-entry";
import type { OAuthCredential } from "aether/plugin-sdk/provider-auth";
import type {
  OAuthCredentials,
  OAuthLoginCallbacks,
} from "aether/plugin-sdk/provider-oauth-runtime";
import {
...
const LEGACY_OAUTH_KEY_PREFIX = "aether-github-copilot-oauth:v1:";
...
export function buildGithubCopilotAuthDoctorHint(
  ...
  return "This GitHub Copilot OAuth profile has an unsupported enterprise domain and can no longer refresh. Remove the legacy profile before re-authenticating with a supported host (github.com or a *.ghe.com tenant): aether models auth login --provider github-copilot --force.";
}
