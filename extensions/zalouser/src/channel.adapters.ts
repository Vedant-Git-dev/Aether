...
import { createScopedDmSecurityResolver } from "aether/plugin-sdk/channel-config-helpers";
import type {
  ChannelGroupContext,
...
import type { GroupToolPolicyConfig, AetherConfig } from "aether/plugin-sdk/config-contracts";
import { createStaticReplyToModeResolver } from "aether/plugin-sdk/conversation-runtime";
...
function resolveZalouserOutboundChunkMode(cfg: AetherConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: AetherConfig, accountId?: string) {
...
export const zalouserSecurityAdapter = {
  ...
  dmRouting: {
    resolveDmScope: ({ cfg }: { cfg: AetherConfig }) => resolveZalouserDmSessionScope(cfg),
  },
