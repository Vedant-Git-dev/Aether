...
import type { PluginLogger } from "aether/plugin-sdk/plugin-entry";
...
/** Input data used to publish Aether gateway Bonjour records. */
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.AETHER_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "Aether"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "Aether";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Aether\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.AETHER_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "aether";
    ...
    const hostnameLabel = hostnameWithoutLocal.slice(0, labelEnd).trim() || "aether";
    const hostname = truncateToDnsLabel(hostnameLabel, "aether");
    const instanceName =
