...
function resolveDefaultToolsAllow(cfg: AetherConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: AetherConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: AetherPluginApi, agentId: string): string {
  return path.join(
...
function readActiveMemoryConfig(api: AetherPluginApi): AetherConfig {
  try {
    return (api.runtime.config?.current?.() as AetherConfig | undefined) ?? api.config;
  } catch {
    return api.config;
  }
}
