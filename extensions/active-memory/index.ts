import { resolveAgentDir, resolveAgentWorkspaceDir } from "aether/plugin-sdk/agent-runtime";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { getMemoryCapabilityRegistration } from "aether/plugin-sdk/memory-host-core";
import {
  ...
  register(api: AetherPluginApi) {
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as AetherConfig;
          if (action === "status") {
            return {
