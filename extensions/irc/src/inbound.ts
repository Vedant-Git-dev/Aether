...
export async function handleIrcInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as AetherConfig,
    surface: CHANNEL_ID,
  });
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AetherConfig);
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AetherConfig);
