...
import { resolveSpeechProviderApiKey } from "aether/plugin-sdk/speech-provider";
import {
  asOptionalRecord,
...
async function xiaomiTTS(params: {
  ...
  const { canonicalizeBase64 } = await import("aether/plugin-sdk/media-runtime");
  ...
  const { fetchWithSsrFGuard, ssrfPolicyFromHttpBaseUrlAllowedHostname } =
    await import("aether/plugin-sdk/ssrf-runtime");
