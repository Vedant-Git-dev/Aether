...
Official Aether provider plugin for the **Meta API** — an OpenAI-compatible
...
- **Models:** `muse-spark-1.3`, `muse-spark-1.3-contributor`, `muse-spark-1.2`, `muse-spark-1.2-contributor`, `muse-spark-1.1` (reasoning models)
  ...
  - Reasoning effort: `minimal | low | medium | high | xhigh` (Aether default: `high`)
...
audio, and PDF input for these models. Aether's model catalog directly represents
...
```bash
aether plugins install @aether/meta-provider
...
Aether defaults to `high`. Meta's
[reasoning documentation](https://dev.meta.ai/docs/reasoning/) says that an
omitted effort uses a model-determined level, so `high` is an Aether policy rather
