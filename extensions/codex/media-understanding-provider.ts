 ...
 */
import { validateJsonSchemaValue } from "aether/plugin-sdk/json-schema-runtime";
import type {
...
} from "aether/plugin-sdk/media-understanding";
...
async function describeCodexImages(
  ...
  const { text } = await runBoundedCodexAppServerTurn({
    ...
    developerInstructions:
      "You are Aether's bounded image-understanding worker. Describe only the provided image content. Do not call tools, edit files, or ask follow-up questions.",
    input: [
...
async function extractCodexStructured(
  ...
  const { text } = await runBoundedCodexAppServerTurn({
    ...
    developerInstructions:
      "You are Aether's bounded structured-extraction worker. Return only the requested extraction. Do not call tools, edit files, ask follow-up questions, or include secrets.",
