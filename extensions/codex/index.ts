...
export default definePluginEntry({
  ...
  register(api) {
    ...
    const resolveCurrentConfig = () =>
      api.runtime.config?.current ? (api.runtime.config.current() as AetherConfig) : undefined;
    const resolvePluginConfig = (resolveConfig: () => AetherConfig | undefined) => {
    ...
    const sessionCatalogControlFactory = createCodexSessionCatalogControl({
      ...
      config: api.config as AetherConfig,
      getPluginConfig: resolveCurrentPluginConfig,
      getRuntimeConfig: resolveCurrentConfig,
    });
    ...
    if (sessionCatalogEnabled) {
      ...
      for (const command of createCodexSessionCatalogNodeHostCommands(
        ...
        {
          getPluginConfig: resolveCurrentPluginConfig,
          getRuntimeConfig: () => resolveCurrentConfig() ?? (api.config as AetherConfig),
          resolveRuntimeOptions: resolveCodexSupervisionAppServerRuntimeOptions,
        },
        bindingStore,
      )) {
      ...
      createCodexCommand({
        ...
        deps: {
          ...
          codexPluginsManagementIo: {
            readConfig: () => {
              const current = (api.runtime.config?.current?.() ?? {}) as AetherConfig;
            ...
            mutate: async (update) => {
              const { mutateConfigFile } = await import("aether/plugin-sdk/config-mutation");
