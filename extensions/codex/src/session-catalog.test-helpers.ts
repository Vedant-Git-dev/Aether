...
import { resolveAgentDir, resolveDefaultAgentDir } from "aether/plugin-sdk/agent-runtime";
...
} from "aether/plugin-sdk/json-schema-runtime";
...
import type { SessionCatalogProvider as RegisteredSessionCatalogProvider } from "aether/plugin-sdk/session-catalog";
import { resolveStorePath } from "aether/plugin-sdk/session-store-runtime";
import {
  closeAetherAgentDatabasesForTest,
  closeAetherStateDatabaseForTest,
} from "aether/plugin-sdk/sqlite-runtime-testing";
import { withEnvAsync } from "aether/plugin-sdk/test-env";
    ...
    "homesForAgent" in params.control
      ...
      : (() => {
          const resolver = createCodexCatalogHomeResolver({
            config: params.getRuntimeConfig() ?? (params.api.config as AetherConfig),
...
export const config = {} as AetherConfig;

export function compatibilityOwnerConfig(owner = "alpha"): AetherConfig {
  return {
    agents: {
...
export async function normalizeCodexManifestConfig(
  ...
  const manifest = JSON.parse(
    await fs.readFile(new URL("../aether.plugin.json", import.meta.url), "utf8"),
...
export function adoptedEntry(params: {
  ...
  return {
    sessionId: params.sessionId ?? "aether-session-existing",
    updatedAt: 1,
...
export function createGatewayApi(runtime: PluginRuntime, apiConfig: AetherConfig = {}) {
