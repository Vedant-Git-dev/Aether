...
import {
  ...
  isPathInside,
} from "aether/plugin-sdk/file-access-runtime";
import {
  createMigrationItem,
...
} from "aether/plugin-sdk/migration";
import type {
  MigrationItem,
  MigrationPlan,
  MigrationProviderContext,
} from "aether/plugin-sdk/plugin-entry";
import { extractErrorCode } from "aether/plugin-sdk/security-runtime";
import { asBoolean, isRecord } from "aether/plugin-sdk/string-coerce-runtime";
  ...
  for (const memory of params.memoryFiles) {
    ...
    if (!targetNotRegular) {
      ...
      if (isPathInside(source, destination) || isPathInside(destination, source)) {
        throw new Error(
          "Codex memory source and Aether import destination must be separate paths.",
        );
      }
    }
      ...
      createMigrationItem({
        ...
        reason: targetNotRegular
          ...
          : targetConflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: "Copy consolidated Codex memory into the Aether memory index.",
        details: {
      ...
      return createMigrationItem({
        ...
        reason: collision
          ...
          : conflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this Aether agent workspace.`,
    ...
    if (
      ...
      items.push(
        createMigrationItem({
          ...
          message: `Install Codex plugin "${plugin.pluginName}" in the Aether-managed Codex app-server runtime.`,
          details: {
            configKey,
            marketplaceName: CODEX_PLUGINS_MARKETPLACE_NAME,
            pluginName: plugin.pluginName,
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with aether plugins install <path> --force.",
      }),
    );
  }
  return items;
...
function buildPluginConfigItem(
  ...
  return createMigrationItem({
    ...
    applyPhase: "after-promotion",
    message:
      "Enable Aether's Codex plugin integration and record migrated source-installed curated plugins.",
    details: {
...
export async function buildCodexMigrationPlan(
  ...
  return {
    ...
    nextSteps: memoryOnly
      ? []
      : [
          "Run aether doctor after applying the migration.",
          "Review skipped or auth-required Codex plugin/config/hook items before exposing them in Aether sessions.",
        ],
    metadata: {
