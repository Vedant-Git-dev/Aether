...
import { loadAuthProfileStoreWithoutExternalProfiles } from "aether/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
...
import type { MigrationItem, MigrationProviderContext } from "aether/plugin-sdk/plugin-entry";
import {
...
function replaceConfigDraft(draft: AetherConfig, next: AetherConfig): void {
  for (const key of Object.keys(draft) as Array<keyof AetherConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<AetherConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config =
      (resolveMigrationConfigRuntime(ctx)?.current?.() as AetherConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: AetherConfig): AetherConfig {
...
function applyOAuthConfigToConfig(
  ...
  let next = mergeMigrationConfigValue(cfg, credential.result.configPatch) as AetherConfig;
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: AetherConfig) => AetherConfig,
...
export async function buildCodexAuthItems(params: {
  ...
  if (credentials.length === 0) {
    ...
    return [
      createMigrationItem({
        ...
        message:
          "No supported Codex credential could be imported. Continue with sign-in to connect Aether.",
        details: {
          provider: OPENAI_PROVIDER_ID,
          credentialKind,
  ...
  return credentials.map((credential) => {
    ...
    return createMigrationItem({
      ...
      target: `${params.targets.agentDir}/aether-agent.sqlite#auth_profile_store:${profileId}`,
