...
import type { PluginCommandResult } from "aether/plugin-sdk/plugin-entry";
import {
...
export function describeCodexHostedAppsSupport(support: CodexHostedAppsSupport): string {
  switch (support) {
    ...
    case "unsupported":
      return "Hosted app support is unknown: this Codex version cannot report the required feature state. Update to Aether's supported Codex version.";
    default:
...
export async function readCodexPluginReadiness(params: {
  ...
  const result: CodexPluginReadiness = {
    ...
    aetherEnabled: current.enabled === true && entry.enabled !== false,
...
export function formatCodexPluginReadiness(
  ...
  const lines = [
    ...
    `Aether app access: ${readiness.aetherEnabled ? "enabled" : "disabled"} (shared Codex plugin configuration; takes effect on your next message).`,
  ...
  } else if (!readiness.aetherEnabled) {
    lines.push(
  ...
  if (!readiness.detail) {
    ...
    if (visible.length > 0) {
      ...
      blocks.push({
        type: "text",
        text: "Flags reflect Codex's runtime snapshot; status does not refresh hosted tools. After connecting, /codex plugins refresh refreshes hosted inventory for the current Codex account/runtime, across all apps. Use Check status separately to inspect this plugin without refreshing. Aether app-access changes take effect on your next message; use /new or /reset after connecting.",
      });
      blocks.push({
        type: "buttons",
        buttons: [
...
function describeUnavailableEvidence(reason: "unsupported" | "request_failed"): string {
  return reason === "unsupported"
    ? "This Codex app-server does not support the required status method. Update to Aether's supported Codex version."
