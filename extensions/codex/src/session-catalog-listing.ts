...
import type { AetherPluginNodeHostCommand } from "aether/plugin-sdk/plugin-entry";
...
import { publishSessionCatalogHost } from "aether/plugin-sdk/session-catalog-paging";
...
async function listGatewayHost(params: {
  ...
  try {
    ...
    const { sessionCatalogAdoptedSourceKey } = await import("aether/plugin-sdk/session-catalog");
    params.signal?.throwIfAborted();
...
export function createCodexSessionCatalogNodeHostCommands(
  ...
  const commands: AetherPluginNodeHostCommand[] = [
    {
  ...
  return process.env.AETHER_NODE_EXEC_HOST?.trim().toLowerCase() === "app"
