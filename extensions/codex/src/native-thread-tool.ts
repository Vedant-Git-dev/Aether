 ...
 */
import type { AnyAgentTool, PluginRuntime } from "aether/plugin-sdk/core";
...
const ForkParamsSchema = Type.Object(
  {
    ...
    attach: Type.Optional(
      Type.Boolean({
        default: true,
        description: "Attach the fork to this Aether session for its next turn.",
      }),
    ),
  },
  { additionalProperties: false },
    ...
    async execute(_toolCallId, rawParams) {
      ...
      const run = async (archiveAdmission?: {
        ...
        const { isModelSelectionLocked, ModelSelectionLockedError } =
          await import("aether/plugin-sdk/model-session-runtime");
        ...
        if (archiveAdmission) {
          ...
          if (await options.bindingStore.hasOtherThreadOwner(threadId, identity)) {
            throw new Error(
              "cannot archive a native Codex thread owned by another Aether session",
            );
          }
        ...
        if (attach && !session) {
          throw new Error("cannot attach a Codex fork without an active Aether session");
        }
