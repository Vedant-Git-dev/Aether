...
function createCodexSessionCatalogControlFromRequests(params: {
  ...
  return {
    ...
    async listPage(pageParams) {
      ...
      const { sanitizeTerminalText } = await import("aether/plugin-sdk/text-chunking");
      ...
      for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
        ...
        const response = await requests.listThreads(
          {
            ...
            // created outside Aether enters the first catalog page immediately.
            sortKey: "updated_at",
            sortDirection: "desc",
            ...(cwd ? { cwd } : {}),
            ...(cursor ? { cursor } : {}),
        ...
        for (const thread of response.data) {
          if (await isAetherManagedCodexThread(thread, params.localSessionsRoot)) {
