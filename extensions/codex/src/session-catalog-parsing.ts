import { asFiniteNumber, isRecord } from "aether/plugin-sdk/string-coerce-runtime";
import type { sanitizeTerminalText } from "aether/plugin-sdk/text-chunking";
...
function parseCatalogSession(
  ...
  const sessionKey = options.allowSessionKey
    ? parseOptionalCatalogString(value.sessionKey, "Aether session key", MAX_SESSION_KEY_LENGTH)
    : undefined;
