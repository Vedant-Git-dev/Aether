import { resolveDefaultModelForAgent } from "aether/plugin-sdk/agent-runtime";
import { listAgentIds, resolveAgentDir } from "aether/plugin-sdk/agent-scope-runtime";
import { resolveEffectiveAgentRuntime } from "aether/plugin-sdk/command-auth-native";
import type { HealthCheck, HealthFinding } from "aether/plugin-sdk/health";
import { runUtf8CommandWithTimeout } from "aether/plugin-sdk/process-runtime";
import {
...
function createCodexManagedAppServerHealthCheck(params: {
  ...
  return {
    ...
    async detect(ctx) {
      ...
      const isFinalization = ctx.mode === "fix" && env.AETHER_UPDATE_POST_CORE === "1";
        ...
        try {
          ...
          return [
            managedCodexFinding({
              ...
              fixHint:
                "Reinstall the staged Aether package with its @openai/codex platform dependency, then rerun the candidate check.",
            }),
          ];
        }
      ...
      if (!nativeCommand) {
        return [
          managedCodexFinding({
            ...
            fixHint:
              "Reinstall the staged Aether package with the matching @openai/codex platform package, then rerun the candidate check.",
          }),
        ];
      }
      ...
      try {
        ...
        return [
          managedCodexFinding({
            ...
            fixHint:
              versionFailureHint ??
              "Repair or reinstall the staged Aether package, then rerun the candidate check before cutover.",
          }),
        ];
      }
      ...
      if (detectedVersion !== CODEX_APP_SERVER_VERSION) {
        return [
          managedCodexFinding({
            ...
            path: nativeCommand,
            requirement: `the exact Aether-pinned Codex version ${CODEX_APP_SERVER_VERSION}`,
            fixHint:
              versionFailureHint ??
              "Reinstall the staged Aether package so its managed @openai/codex dependency matches the pinned version, then rerun the candidate check.",
          }),
        ];
      }
      return [];
