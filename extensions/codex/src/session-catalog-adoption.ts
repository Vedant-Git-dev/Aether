...
export async function listAdoptedSessionEntries(params: {
  ...
  for (const { agentId, entry, sessionKey } of listSessionCatalogEntries({
    ...
    if (adopted.has(sourceKey)) {
      throw new Error(
        `multiple Aether sessions adopt Codex thread ${sourceThreadId} from the same home`,
      );
    }
...
async function ensurePendingAdoptionBinding(params: {
  ...
  if (!ownsGeneration) {
    throw new Error(`failed to claim the Aether session generation for ${params.sourceThreadId}`);
  }
  ...
  if (existing) {
    ...
    throw new Error(`Aether session is already bound to Codex thread ${existing.threadId}`);
  }
  const binding = {
...
async function continueLocalCodexSessionInner(
  ...
  if (existing) {
    ...
    const changedError = () =>
      new CatalogParamsError("Codex Aether session changed before it could be opened. Retry.");
...
/** Creates one locked Aether branch whose first harness run forks the Codex source. */
