...
import { readVisibleSessionTranscriptMessageEntries } from "aether/plugin-sdk/session-transcript-runtime";
...
const NATIVE_CONVERSATION_INTERACTIVE_APPROVALS_UNAVAILABLE =
  "Aether native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.";
...
export async function resolveConversationAppServerRuntime(params: {
  ...
  const configuredRuntime = resolveCodexAppServerRuntimeOptions({
    ...
    aetherSandboxActive: Boolean(sandboxForPolicy?.enabled),
  });
...
export const CODEX_CONVERSATION_THREAD_DEVELOPER_INSTRUCTIONS =
  "This Codex thread is bound to an Aether conversation. Answer normally; Aether will deliver your final response back to the conversation.";
