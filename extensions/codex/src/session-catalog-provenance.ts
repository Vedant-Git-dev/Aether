...
import { root as openSafeFilesystemRoot } from "aether/plugin-sdk/file-access-runtime";
import {
  isJsonObject,
  type CodexThread,
  type JsonObject,
...
/**
 * Codex 0.147 reports Aether app-server rollouts as `vscode`, so the rollout's
 * immutable session metadata is the authoritative historical provenance.
 */
export async function isAetherManagedCodexThread(
  thread: CodexThread,
  ...
  const managed = metadata === undefined ? undefined : metadata?.originator === "aether";
