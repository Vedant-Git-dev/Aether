...
import type { PluginCommandContext } from "aether/plugin-sdk/plugin-entry";
...
const CODEX_DIAGNOSTICS_SOURCE = "aether-diagnostics";
...
function formatCodexDiagnosticsTargetBlock(
  ...
  if (target.sessionKey) {
    lines.push(`Aether session key: ${formatCodexCopyableValueForDisplay(target.sessionKey)}`);
  }
  if (target.sessionId) {
    lines.push(`Aether session id: ${formatCodexCopyableValueForDisplay(target.sessionId)}`);
  }
...
function formatCodexDiagnosticsTargetLine(target: CodexDiagnosticsTarget): string {
  ...
  if (sessionLabel) {
    parts.push(`Aether session ${formatCodexDisplayText(sessionLabel)}`);
  }
