...
import { decodeNodePtyResumeParams, decodeNodePtyStartParams } from "aether/plugin-sdk/node-host";
import type {
  AetherPluginApi,
  AetherPluginNodeHostCommand,
} from "aether/plugin-sdk/plugin-entry";
import type { SessionCatalogTerminalPlan } from "aether/plugin-sdk/session-catalog";
...
export function createCodexTerminalStartNodeHostCommand(): AetherPluginNodeHostCommand {
  return {
...
function resolveCodexCatalogTerminalHome(
  ...
  if (!runtimeConfig) {
    throw new Error("Aether runtime config is unavailable");
  }
  const agentDir =
