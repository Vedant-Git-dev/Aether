...
import { resolveSessionAgentIdsStrict } from "aether/plugin-sdk/agent-scope-runtime";
import {
  loadCodexBundleMcpApprovalConfig,
  resolveCodexMcpToolOverridesForAgent,
} from "aether/plugin-sdk/codex-mcp-projection";
import { loadExecApprovals } from "aether/plugin-sdk/exec-approvals-runtime";
import type { PluginRuntime } from "aether/plugin-sdk/plugin-runtime";
import type { ReplyPayload } from "aether/plugin-sdk/reply-payload";
import { readStringField as readString } from "aether/plugin-sdk/string-coerce-runtime";
...
import {
  ...
  resolveAetherExecPolicyForCodexAppServer,
...
import {
  resolveCodexExternalSandboxPolicyForAetherSandbox,
...
export async function runCodexAppServerSideQuestion(
  ...
  const execPolicy = resolveAetherExecPolicyForCodexAppServer({
  ...
  const ensureSandboxEnvironment = async (targetClient: CodexAppServerClient) => {
    ...
    if (!environment) {
      throw new Error(
        "Codex app-server did not register an Aether sandbox exec-server environment.",
      );
    }
      ...
      if (request.method === "mcpServer/elicitation/request") {
        ...
        return approvalResult.kind === "handled"
          ...
          : createCodexElicitationResponse("decline", null, {
              message: "Aether Codex side questions do not support interactive MCP input.",
            });
      }
          ...
          "turn/start",
          {
            ...
            ...(sandboxEnvironment
              ? {
                  ...
                  sandboxPolicy: resolveCodexExternalSandboxPolicyForAetherSandbox(
                    params.sandbox ?? undefined,
                  ),
...
async function createCodexSideToolBridge(input: {
  ...
  if (supportsModelTools(runtimeModel)) {
    const createAetherCodingTools = (await import("aether/plugin-sdk/agent-harness"))
      .createAetherCodingTools;
    ...
    const questionPrompt = publishSideToolResult
      ...
      : undefined;
    const allTools = createAetherCodingTools({
