...
import { loadNodeExecAvailability } from "aether/plugin-sdk/node-selection-runtime";
...
export async function createNodeExecAliasDynamicTool(
  ...
  const execute: AetherDynamicTool["execute"] = async (toolCallId, args, signal, onUpdate) => {
  ...
  return {
    ...
    description: pinnedNode
      ? "Run a shell command to completion on the Aether configured remote node for this session. This tool always uses Aether host=node internally and follows the existing node exec approval and allowlist policy. Remote-node background follow-up is unavailable. Use Codex's native shell for local app-server work when it is available."
      : "Run a shell command to completion on an Aether remote node. The sole connected node that can execute commands is selected automatically; select by name or id when several can. This tool always uses Aether host=node internally and follows the existing node exec approval and allowlist policy. Remote-node background follow-up is unavailable. Use Codex's native shell for local app-server work when it is available.",
    execute,
  };
}
...
export function createGatewayExecProjection(
  ...
  return createProjection(execTool, {
    ...
    description:
      "Run a shell command through Aether on the Gateway host for Aether-managed Gateway environment access, including Secret Store agent-readable environment values and protected egress sentinels. Native Codex shell remains preferred for ordinary local work. This tool always uses Aether host=gateway internally and follows Gateway exec approval and allowlist policy.",
...
export function createGatewayProcessProjection(
  createProjection: CodexScheduledToolProjectionFactory,
  processTool: AetherDynamicTool,
): AetherDynamicTool {
  return createProjection(processTool, {
    kind: "process",
    name: CODEX_GATEWAY_PROCESS_DYNAMIC_TOOL_NAME,
    description:
      "Manage background shell sessions in the existing per-session Aether process scope: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use for gateway_exec follow-up; use native Codex shell session handling for ordinary local work.",
  });
}
