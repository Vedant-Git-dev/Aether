import { onInternalDiagnosticEvent } from "aether/plugin-sdk/diagnostic-runtime";
...
export function createCodexAttemptServerRequestController(
  ...
  const handleServerRequest = async (
    ...
    try {
      ...
      const replayedExecution = aetherDynamicToolExecutions.get(call);
      ...
      markCurrentTurnRequestProgress();
      pendingAetherDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        const { execution } = aetherDynamicToolExecutions.claim(call, async () => {
        ...
        pendingAetherDynamicToolCompletionIds.delete(call.callId);
