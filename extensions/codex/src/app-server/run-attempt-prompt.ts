...
import {
  ...
  prependCodexAetherPromptContext,
...
export async function prepareCodexAttemptPrompt(context: CodexAttemptContext) {
  ...
  > = async (message, maxChars) => {
    ...
    if (!prepare) {
      const media = asOptionalRecord(Reflect.get(message, "__aether"))?.media;
      if (
        ...
        throw new CodexContextAttachmentError(
          "Saved attachments require a newer Aether host. Update the Gateway and Codex plugin together, then retry.",
        );
      }
      return { images: [] };
    }
  ...
  const decorateCodexTurnPromptText = (
    ...
    const turnPromptText = prependCodexAetherPromptContext(
      promptBuildResult.prompt,
      buildAetherPromptContext(includeWorkspaceReferences),
      {
