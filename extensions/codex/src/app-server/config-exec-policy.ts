import { AgentHarnessPreflightError } from "aether/plugin-sdk/agent-harness-registration";
import type {
...
export function resolveApprovalPolicy(value: unknown): CodexAppServerApprovalPolicy | undefined {
  if (value === "untrusted") {
    throw new Error(
      'Codex app-server approval policy "untrusted" is retired; run "aether doctor --fix" and use "on-request".',
    );
  }
  if (value === "on-failure") {
    return "on-request";
...
export function resolveEffectiveAetherExecModeForCodexAppServer(params: {
  execMode?: AetherExecMode;
  execPolicy?: AetherExecPolicyForCodexAppServer;
}): AetherExecMode | undefined {
...
export function resolveCodexPolicyModeForAetherExecMode(
  mode: AetherExecMode | undefined,
...
export function assertCodexAppServerAllowedForAetherExecMode(
  mode: AetherExecMode | undefined,
): void {
  if (mode === "deny" || mode === "allowlist") {
    throw new AgentHarnessPreflightError(
      `Codex app-server local execution is unavailable because effective tools.exec.mode=${mode}. ` +
        "Execution-host approvals are authoritative. For gateway turns, inspect them with `aether approvals get --gateway` and update that same target with `aether approvals set --gateway --stdin`; for local `agent exec`, omit `--gateway`. Intentionally align that host policy before retrying.",
      { scope: "harness" },
    );
  }
}
