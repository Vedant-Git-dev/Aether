/**
 * Blocks direct Codex app-server requests that would bypass Aether sandbox or
 * node-exec routing guarantees.
 */
import { tryResolveDefaultAgentId } from "aether/plugin-sdk/agent-scope-runtime";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { parseAgentSessionKey } from "aether/plugin-sdk/routing";
import { resolveSandboxRuntimeStatus, type SandboxContext } from "aether/plugin-sdk/sandbox";
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-aether-environment"],
...
/** Returns a block message when a direct app-server method would bypass Aether execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-aether-environment" &&
    hasAetherSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasAetherSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("aether-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because Aether sandboxing is active for this session.`,
    "This mode cannot route execution through the Aether sandbox backend.",
