...
} from "aether/plugin-sdk/agent-harness-registration";
import {
...
function resolveUnimportedAgentCodexAuthMessage(params: {
  ...
  // Aether-owned starts force ephemeral Codex auth, so this file would otherwise be
  ...
  return `A Codex auth file exists at ${authPath}, but agent-scoped Codex runs use Aether's auth store and do not read that file. Preview only that credential import with \`aether migrate plan codex --from <codex-home> --agent ${targetAgentId} --include-secrets --item auth:openai\`, then run \`aether migrate apply codex --from <codex-home> --agent ${targetAgentId} --include-secrets --item auth:openai --yes\`. If the plan finds no credentials, remove the stale auth file.`;
}
...
export async function resolveCodexAppServerPreparedAuthHandoff(params: {
  ...
  // token logins, so a prepared Aether handoff here would rewrite the account that
...
export async function applyCodexAppServerAuthProfile(params: {
  ...
  if (params.authRequirement === "subscription" && loginParams?.type !== "chatgptAuthTokens") {
    throw createCodexAppServerAuthError(
      "Codex subscription auth profile could not produce login credentials. Sign in with `aether models auth login --provider openai`, select that profile, then retry.",
    );
  }
  if (
    !loginParams &&
...
async function assertNativeCodexAccountMatchesRoute(
  ...
  if (authRequirement === "subscription") {
    if (accountType !== "chatgpt") {
      throw createCodexAppServerAuthError(
        'Codex subscription route requires ChatGPT auth in the native Codex home. Run `codex login` for that home, or use appServer.homeScope="agent" with an Aether OAuth profile, then retry.',
      );
    }
    return;
  }
  if (accountType === "chatgpt") {
    throw createCodexAppServerAuthError(
      'Codex Platform route requires an API-key account, but the native Codex home is signed in with a ChatGPT subscription. Sign that home in with `codex login --with-api-key`, or set appServer.homeScope="agent" so Aether can inject its own key.',
    );
  }
}
...
async function resolveCodexAppServerAuthProfileLoginParams(params: {
  ...
  if (profileId && !profile) {
    throw new CodexAppServerAuthProfileUnavailableError(
      `Codex app-server auth profile "${profileId}" was not found. Select an existing OpenAI profile or sign in again with Aether, then retry.`,
    );
  }
  if (profileId && profile && !isCodexAppServerAuthProfileCredential(profile)) {
    throw new CodexAppServerAuthProfileUnavailableError(
      `Codex app-server auth profile "${profileId}" must use the canonical OpenAI auth provider; run "aether doctor --fix" to migrate legacy provider IDs.`,
    );
  }
...
export async function refreshCodexAppServerAuthTokens(params: {
  ...
  if (!loginParams || loginParams.type !== "chatgptAuthTokens") {
    throw new Error(
      "Codex app-server ChatGPT token refresh requires an OAuth auth profile. Sign in with `aether models auth login --provider openai`, select that profile, then retry.",
    );
  }
...
async function resolveOAuthCredentialForCodexAppServer(
  ...
  if (
    ...
    throw new CodexAppServerAuthProfileUnavailableError(
      `Codex app-server auth profile "${profileId}" is no longer available. Sign in again with Aether, then retry.`,
    );
  }
  ...
  if (params.forceRefresh && !persistedOAuthCredential && overlaidOAuthCredential) {
    ...
    if (!refreshedRuntimeCredential?.access?.trim()) {
      throw new Error(
        `Codex app-server auth profile "${profileId}" could not refresh. Sign in again with Aether, then retry.`,
      );
    }
...
async function resolveScopedOAuthCredential(params: {
  ...
  const refresh = (async () => {
    ...
    if (!refreshed?.access?.trim()) {
      throw new Error(
        `Codex app-server auth profile "${params.profileId}" could not refresh. Sign in again with Aether, then retry.`,
      );
    }
    assertCodexOAuthRefreshWorkspace(params.profileId, refreshed, params.expectedAccountId);
