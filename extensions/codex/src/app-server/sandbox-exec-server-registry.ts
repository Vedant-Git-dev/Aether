import type { AetherLeasedExecServer } from "./sandbox-exec-server/types.js";

export const sandboxExecServerRegistry = {
  servers: new Map<string, Promise<AetherLeasedExecServer>>(),
  async close(server: AetherLeasedExecServer): Promise<void> {
    if (server.closed) {
      return;
    }
    server.closed = true;
