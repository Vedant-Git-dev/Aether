...
// Aether opts into these under-development features deliberately, so silence
...
const CODEX_CODE_MODE_THREAD_CONFIG: JsonObject = {
  ...
  // Native code mode replaces Aether's own exec/read/write/edit tools with the
...
const CODEX_NATIVE_UPDATE_PLAN_DISABLED_THREAD_CONFIG: JsonObject = {
  // Aether owns the durable progress card; Codex's native checklist would create a second owner.
...
export function buildThreadStartParams(
  ...
  return {
    ...
    ...((options.hostSystemAgentActive ?? isHostScopedAgentToolActive("aether")) &&
...
function resolveDirectOnlyToolNamespaces(
  ...
  return (dynamicTools ?? [])
    .filter(
      (tool) =>
        tool.type === "namespace" && tool.name === CODEX_AETHER_DIRECT_DYNAMIC_TOOL_NAMESPACE,
    )
    .map((tool) => tool.name);
}
...
export function buildCodexRingZeroThreadConfigPatch(
  ...
  hostSystemAgentActive = isHostScopedAgentToolActive("aether"),
