...
import { buildHostnameAllowlistPolicyFromSuffixAllowlist } from "aether/plugin-sdk/ssrf-policy";
...
function hasManagedSearchProvider(config: AetherConfig | undefined): boolean {
  return normalizeOptionalString(config?.tools?.web?.search?.provider) !== undefined;
}

function hasNativeDomainRestrictions(config: AetherConfig | undefined): boolean {
  return (
