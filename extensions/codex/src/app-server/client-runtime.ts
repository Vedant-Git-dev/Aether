...
import { embeddedAgentLog, formatErrorMessage } from "aether/plugin-sdk/agent-harness-runtime";
  ...
  client.addRequestHandler(async (request) => {
    ...
    try {
      const tokens = await withTimeout(
        ...
        "Codex app-server ChatGPT token refresh timed out before its external-auth deadline. Retry the request; if it persists, sign in again with Aether.",
      );
