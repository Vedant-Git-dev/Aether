...
> &
  Partial<Pick<CodexAsyncAssistantMessage, "aetherAsyncDelivery">>;
...
export function applyCodexTranscriptTaint(
  ...
  return message.role === "assistant" && state.tainted
    ? ({ ...message, __aether: { ...metadata, turnTainted: true } } as AgentMessage) // SAFETY: Only provider metadata changes.
    : message;
}
