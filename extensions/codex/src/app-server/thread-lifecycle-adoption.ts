...
async function preparePendingCodexThreadResume(
  ...
  const fail = (reason: string) =>
    new Error(
      `Cannot configure resumed Codex thread ${binding.threadId}: ${reason}. ` +
        "The thread is preserved; continue it in native Codex or use /new for the current Aether tools.",
    );
  ...
  if (
    ...
    throw fail("configuration adoption requires an Aether-owned local Codex home");
  }
  ...
  try {
    ...
    if (
      ...
      throw fail("its immutable native tool catalog does not match the current Aether tools");
    }
    assertCurrent();
    return {
...
function observeCodexThreadConfiguration(
  ...
  return {
    ...
    assertConfigured: () => {
      ...
      // reservation against native-internal reloads outside Aether's thread queue.
      if (!unloaded) {
        throw new Error(
