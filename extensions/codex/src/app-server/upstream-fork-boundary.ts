  ...
  for (const [turnIndex, turn] of params.turns.entries()) {
    ...
    for (const item of turn.items) {
      ...
      if (nativeText === undefined) {
        return failure(
          "drift-mismatch",
          "A message before the fork point contains images or attachments that cannot be verified across Aether and Codex. Fork from a text-only span instead.",
        );
      }
...
export async function resolveCodexUpstreamForkBoundary(params: {
  ...
  try {
    ...
    const localPrefix = visibleUserEntries.slice(0, targetIndex + 1).filter((entry) => {
      ...
      const meta = "__aether" in entry.message ? entry.message["__aether"] : undefined;
