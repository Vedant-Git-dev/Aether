...
} from "aether/plugin-sdk/agent-scope-runtime";
/**
 * Resolves whether Codex app-server native execution can own shell/file work,
 * or whether Aether must keep exec/process on a configured node host.
 */
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { normalizeAgentId, parseAgentSessionKey } from "aether/plugin-sdk/routing";
import { resolveSandboxRuntimeStatus } from "aether/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "aether/plugin-sdk/session-store-runtime";
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "Aether exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected Aether node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because Aether exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected Aether node.",
    "Use a normal Codex harness turn so Aether exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
