...
import { createAetherCodingTools } from "aether/plugin-sdk/agent-harness";
import {
...
import { clearRuntimeAuthProfileStoreSnapshots } from "aether/plugin-sdk/agent-runtime";
import { resetDiagnosticEventsForTest } from "aether/plugin-sdk/diagnostic-runtime";
import type { ExecApprovalsFile } from "aether/plugin-sdk/exec-approvals-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "aether/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "aether/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "aether/plugin-sdk/plugin-runtime";
import { createAgentHarnessHostCapabilitiesForTest } from "aether/plugin-sdk/plugin-test-runtime";
import {
  deleteSessionEntry,
  resolveStorePath,
  upsertSessionEntry,
...
import { resolvePreferredAetherTmpDir } from "aether/plugin-sdk/temp-path";
...
function createHarnessHostCapabilities(
  ...
  return Object.freeze({
    ...
    createToolSurface: (options) => createAetherCodingTools(options),
...
vi.mock("aether/plugin-sdk/exec-approvals-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("aether/plugin-sdk/exec-approvals-runtime")>();
  return {
    ...actual,
...
export function threadStartResult(threadId = "thread-1", options: { cwd?: string } = {}) {
  const cwd = options.cwd ?? tempDir ?? "/tmp/aether-codex-test";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("AETHER_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredAetherTmpDir(), "aether-codex-run-"));
  ...
  afterEach(async () => {
    ...
    clearRuntimeAuthProfileStoreSnapshots();
    dynamicToolBuildState.aetherCodingToolsFactory = undefined;
