...
const registeredCodexPlugin = resolveGlobalSingleton<{ root?: string }>(
  Symbol.for("aether.codexManagedPluginRoot"),
  () => ({}),
);
...
/** Records the process-stable plugin root prepared by Aether's plugin loader. */
...
async function findManagedCodexAppServerCommandPaths(params: {
  ...
  throw new Error(
    [
      ...
      "Reinstall or update Aether, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or AETHER_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
