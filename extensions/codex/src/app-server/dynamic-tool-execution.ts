...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Aether dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Aether dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "Aether dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "Aether dynamic tool call aborted.";
    const terminalReason = resolveCodexToolAbortTerminalReason(params.signal);
    params.onFallbackSelected?.();
  ...
  try {
    ...
    const message = formatToolExecutionErrorMessage(error, "Aether dynamic tool call failed.");
    notifyFailedToolResult(message, terminalReason);
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Aether dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "Aether dynamic tool call failed.";
}

/** Strips Aether-only metadata before sending a dynamic tool response to Codex. */
...
type TerminalDynamicToolReleaseState = {
  ...
  pendingAetherDynamicToolCompletionIdsCount: number;
};
...
export function shouldReleaseTurnAfterTerminalDynamicTool(
  ...
  return (
    ...
    state.pendingAetherDynamicToolCompletionIdsCount === 0
  );
}
...
export function resolveDynamicToolCallTimeoutMs(params: {
  ...
  if (
    ...
    try {
      ...
      // Aether delegation uses that default for its ten-minute approval plus
      // staging/application; it has no model-authored timeout override.
      const timeoutSeconds = params.call.tool === "aether" ? undefined : args?.timeoutSeconds;
      return (
