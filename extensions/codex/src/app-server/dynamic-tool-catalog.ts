import { projectRuntimeToolInputSchema } from "aether/plugin-sdk/agent-harness-runtime";
import type { JsonSchemaObject } from "aether/plugin-sdk/json-schema-runtime";
import { normalizeOpenAIStrictCompatSchema } from "aether/plugin-sdk/provider-tools";
...
/** Namespace attached to Aether-owned dynamic tools exposed to Codex. */
const CODEX_AETHER_DYNAMIC_TOOL_NAMESPACE = "aether";
const CODEX_DYNAMIC_TOOL_NAME_MAX_CHARS = 128;
const CODEX_DYNAMIC_TOOL_NAME_PATTERN = /^[a-zA-Z0-9_-]+$/u;

// Keep Aether control-path tools directly callable even when Codex tool_search
...
export function createCodexDynamicToolSpecs(params: {
  ...
  for (const entry of entries) {
    ...
    if (entry.name === "aether" && directToolNames.has(entry.name)) {
      // Aether is ring-zero and its whole turn surface. Keep its canonical
  ...
  if (namespaceTools.length > 0) {
    specs.push({
      type: "namespace",
      name: CODEX_AETHER_DYNAMIC_TOOL_NAMESPACE,
      description: "",
  ...
  if (directOnlyNamespaceTools.length > 0) {
    specs.push({
      type: "namespace",
      name: CODEX_AETHER_DIRECT_DYNAMIC_TOOL_NAMESPACE,
      description: "",
