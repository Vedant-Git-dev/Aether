...
import { normalizeTrimmedStringList } from "aether/plugin-sdk/string-coerce-runtime";
...
import {
  assertCodexAppServerAllowedForAetherExecMode,
  resolveApprovalPolicy,
  resolveApprovalsReviewer,
  resolveCodexPolicyModeForAetherExecMode,
  resolveEffectiveAetherExecModeForCodexAppServer,
  resolveSandbox,
...
export function createCodexAppServerConfig({
  ...
  function resolveCodexAppServerRuntimeOptions(
    params: {
      ...
      execMode?: AetherExecMode;
      execPolicy?: AetherExecPolicyForCodexAppServer;
    ...
    const envCommand = readNonEmptyString(env.AETHER_CODEX_APP_SERVER_BIN);
    ...
    const args = resolveArgs(config.args, env.AETHER_CODEX_APP_SERVER_ARGS);
    ...
    const execMode = resolveEffectiveAetherExecModeForCodexAppServer({
    ...
    if (!params.sessionPermissionMode) {
      assertCodexAppServerAllowedForAetherExecMode(execMode);
    }
    const explicitPolicyMode =
      resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.AETHER_CODEX_APP_SERVER_MODE);
    const configuredSandbox =
      resolveSandbox(config.sandbox) ?? resolveSandbox(env.AETHER_CODEX_APP_SERVER_SANDBOX);
    const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
    const normalizedPolicyMode = resolveCodexPolicyModeForAetherExecMode(execMode);
    ...
    const execModeRequiringPromptingApprovals:
      | Extract<AetherExecMode, "auto" | "ask">
      | undefined =
          ...
          sandbox: preserveExplicitAutoSandbox
            ...
            : forceDangerFullAccessSandbox
              ? selectForcedDangerFullAccessSandbox({
                  configuredSandbox,
                  defaultPolicy,
                  aetherSandboxActive: Boolean(params.aetherSandboxActive),
                })
              : selectForcedPromptingSandbox({
                  configuredSandbox,
    ...
    const envApprovalPolicy = resolveApprovalPolicy(env.AETHER_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const healthCheckIntervalMinutes = normalizeComputerUseHealthCheckIntervalMinutes(
    params.overrides?.healthCheckIntervalMinutes ??
      ...
      readNumberEnv(env.AETHER_CODEX_COMPUTER_USE_HEALTH_CHECK_INTERVAL_MINUTES),
  );
  const healthCheckEnabled =
    params.overrides?.healthCheckEnabled ??
    config.healthCheckEnabled ??
    readBooleanEnv(env.AETHER_CODEX_COMPUTER_USE_HEALTH_CHECK_ENABLED) ??
    false;
  const pluginCacheMode =
    ...
    normalizeComputerUsePluginCacheMode(env.AETHER_CODEX_COMPUTER_USE_PLUGIN_CACHE_MODE) ??
    "independent";
  const strictReadiness =
    params.overrides?.strictReadiness ??
    config.strictReadiness ??
    readBooleanEnv(env.AETHER_CODEX_COMPUTER_USE_STRICT_READINESS) ??
    false;
  const autoRepair =
    params.overrides?.autoRepair ??
    config.autoRepair ??
    readBooleanEnv(env.AETHER_CODEX_COMPUTER_USE_AUTO_REPAIR) ??
    false;
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.AETHER_CODEX_COMPUTER_USE) ??
    Boolean(
      autoInstall ||
      marketplaceSource ||
      marketplacePath ||
