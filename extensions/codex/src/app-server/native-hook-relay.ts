/**
 * Bridges Codex native hook callbacks into Aether's native hook relay so
 * app-server tool events can still run Aether policy and diagnostics.
 */
...
import { emitTrustedDiagnosticEvent } from "aether/plugin-sdk/diagnostic-runtime";
import { createDeferred } from "aether/plugin-sdk/extension-shared";
import { registerRetainedNativeHookRelayForBundledRuntime } from "aether/plugin-sdk/native-hook-relay-runtime";
import type { NativeHookRelayCommandPlan } from "aether/plugin-sdk/native-hook-relay-runtime";
import {
  addTimerTimeoutGraceMs,
  finiteSecondsToTimerSafeMilliseconds,
} from "aether/plugin-sdk/number-runtime";
import type { PluginHookToolContext } from "aether/plugin-sdk/types";
...
/** Codex hook events that can be registered through Aether's native relay. */
...
/** Enterprise managed-only policy silently drops the session-layer hooks that enforce Aether. */
  ...
  if (!attestation) {
    attestation = client
      ...
      .then((response) => {
        ...
        if (managedOnly === true) {
          throw new Error(
            "Codex managed-only hooks disable the Aether native hook relay; refusing unenforced execution",
          );
        }
      });
...
/** Registers an Aether native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Aether plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("aether:codex:native-hook-relay:v1");
  hash.update("\0");
        ...
        hooks: [
          {
            ...
            timeout,
            async: false,
            statusMessage: "Aether native hook relay",
          },
        ],
      },
    ] satisfies JsonValue;
...
function buildCodexNativeToolMatcher(toolNames: readonly string[] | undefined): string | undefined {
  ...
  for (const toolName of toolNames) {
    ...
    if (!canonicalToolName || canonicalToolName === "*") {
      throw new TypeError("Codex native hook matcher requires canonical Aether tool ids");
    }
