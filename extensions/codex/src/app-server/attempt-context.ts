...
import { resolveAgentWorkspaceDir } from "aether/plugin-sdk/agent-runtime";
import { resolveBootstrapFilesForPreparation } from "aether/plugin-sdk/codex-mcp-projection";
import {
  buildMemorySystemPromptAddition,
  prepareMemorySystemPromptAddition,
} from "aether/plugin-sdk/core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "aether/plugin-sdk/message-tool-delivery-hints";
import type {
  SessionTranscriptTargetParams,
  TranscriptTurnAdmission,
} from "aether/plugin-sdk/session-transcript-runtime";
import { readNonBlankString as readNonEmptyString } from "aether/plugin-sdk/string-coerce-runtime";
...
export async function prepareCodexWorkspaceDeveloperInstructions(params: {
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    ...
    header: "## Aether Agent Workspace Instructions",
    preamble: "Aether loaded this bounded snapshot from the configured agent workspace.",
  });
}
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const injectAetherContext = shouldInjectCodexAetherPromptContext(params.params);
    const restrictedProjectDocNeedsAetherCarrier =
    ...
    const threadDeveloperInstructionFiles =
      ...
      (inheritsAgentWorkspace || restrictedProjectDocNeedsAetherCarrier)
        ? selectCodexWorkspaceAgentProjectInstructionFiles(contextFiles, params.resolvedWorkspace)
        : [];
    const turnScopedDeveloperInstructionFiles = injectAetherContext
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexAetherPromptContext(params.params)
...
/**
 * Builds Aether-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexAetherPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
  watchedSessionsContext?: string;
}): string | undefined {
  if (!shouldInjectCodexAetherPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## Aether Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ...
  return [
    "Aether runtime context for this turn:",
    "Treat this Aether-provided context as supporting project/user reference for the current request.",
    "",
    ...sections,
  ].join("\n");
}
 ...
 * keep refusing cross-session questions (aether#114797).
 */
export function buildCodexWatchedSessionsContext(params: {
  ...
  if (!shouldInjectCodexAetherPromptContext(params.attempt)) {
    return undefined;
  }
...
function shouldInjectCodexAetherPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded Aether skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  ...
  return params.skillsPrompt?.trim()
    ? ["## Aether Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends Aether context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexAetherPromptContext(
  prompt: string,
...
function renderCodexWorkspaceBootstrapPromptContext(
  ...
  const lines = [
    "Aether loaded these user-editable workspace files for the current turn. Codex loads project-local AGENTS.md natively. When execution uses another folder, Aether supplies the agent workspace AGENTS.md as thread-level developer instructions. SOUL.md, IDENTITY.md, and USER.md are prepared separately from user input and are not repeated here.",
    "",
    "# Project Context",
    "",
...
function renderCodexWorkspaceCollaborationDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## Aether Agent Soul",
    preamble:
      "Aether loaded these workspace instruction files from the active agent workspace. They are the canonical definitions of who you are, how you think and work, and the human you work alongside. Internalize and follow them accordingly.",
...
function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## Aether Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. Aether does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
