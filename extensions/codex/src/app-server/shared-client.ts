...
import type { AgentHarnessRuntimeArtifactBinding } from "aether/plugin-sdk/agent-harness-runtime";
...
const SHARED_CODEX_APP_SERVER_CLIENT_DISPOSER = codexBuildSymbol(
  "aether.codexAppServerClientDisposer",
);
...
async function resolveCodexAppServerClientStartContext(
  ...
  if (preparedAuth?.kind === "profile" && !preparedAuth.store.profiles[preparedAuth.profileId]) {
    throw new Error(
      `Prepared Codex auth profile "${preparedAuth.profileId}" was not found. Select an existing OpenAI profile or sign in again with Aether, then retry.`,
    );
  }
...
export function captureCodexAppServerClientLifetime(
  ...
  if (
    ...
    throw new AgentHarnessPreflightError(
      "Codex ordinary configuration refresh requires an Aether-managed local stdio process, not an external socket or app-server proxy. No turn was sent; reconnect through managed local stdio before continuing.",
    );
  }
