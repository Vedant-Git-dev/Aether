import { agentHarnessStructuredInput as structuredInput } from "aether/plugin-sdk/agent-harness-runtime";
...
export function compileCodexOrdinaryElicitation(params: {
  ...
  if (requestTurnId !== null && typeof requestTurnId !== "string") {
    return {
      ...
      input: {
        kind: "unsupported",
        message: "Aether declined an MCP elicitation with invalid turn correlation.",
      },
    };
  }
  ...
  if (mode !== "form" && mode !== "openai/form") {
    return {
      ...
      input: {
        kind: "unsupported",
        message: `Aether does not support MCP elicitation mode ${JSON.stringify(mode ?? "unknown")}.`,
      },
    };
  }
  return {
