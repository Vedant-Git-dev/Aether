...
import { embeddedAgentLog, AETHER_VERSION } from "aether/plugin-sdk/agent-harness-runtime";
...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to Aether. */
  ...
  addNotificationHandler(handler: CodexServerNotificationHandler): () => void {
    ...
    // Aether can reserve the first thread or install its shared turn router.
...
function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "Aether did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (request.method === "mcpServer/elicitation/request") {
    return createCodexElicitationResponse("decline", null, {
      message: "Aether has no interactive handler for this elicitation.",
    });
  }
  return {};
}
...
function timeoutServerRequestResponse(timeoutMs: number): JsonValue {
  return {
    contentItems: [
      {
        type: "inputText",
        text: `Aether dynamic tool call timed out after ${timeoutMs}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "Aether could not determine the running Codex version";
    super(
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): string {
  ...
  if (detected.compare(CODEX_APP_SERVER_VERSION) > 0) {
    embeddedAgentLog.warn(
      "codex app-server is newer than Aether's managed runtime; continuing with normal startup validation",
      {
        detectedVersion,
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // Aether, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods Aether can answer. */
