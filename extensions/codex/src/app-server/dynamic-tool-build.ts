 ...
 * Aether-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
import { resolveAgentDir } from "aether/plugin-sdk/agent-runtime";
import {
  resolveCodexScheduledToolProjectionFactory,
  runWithCronCreatorAuthorityCapabilityResolver,
} from "aether/plugin-sdk/codex-mcp-projection";
import { isToolAllowed } from "aether/plugin-sdk/sandbox";
import {
  createStageTimingTracker,
  formatStageTimings,
  type StageTimingSummary,
} from "aether/plugin-sdk/time-runtime";
import {
...
type AetherCodingToolsOptions = NonNullable<
  Parameters<(typeof import("aether/plugin-sdk/agent-harness"))["createAetherCodingTools"]>[0]
>;

/** Factory seam for constructing Aether runtime tools without eagerly loading agent-harness. */
type AetherCodingToolsFactory =
  (typeof import("aether/plugin-sdk/agent-harness"))["createAetherCodingTools"];
type AetherDynamicTool = ReturnType<AetherCodingToolsFactory>[number];
type AetherSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export function resolveCodexNodePlacementToolConstructionPlan(
  sandbox: AetherSandboxContext | undefined,
  nativeToolSurfaceEnabled: boolean | undefined,
): AetherCodingToolsOptions["toolConstructionPlan"] {
  if (
...
function preserveRingZeroSystemAgentTool<T extends { name: string; catalogMode?: string }>(
  ...
  const aether = allTools.find(
    (tool) => tool.name === "aether" && tool.catalogMode === "direct-only",
  );
  if (!aether) {
    return filteredTools;
  }
  return [aether, ...filteredTools.filter((tool) => tool.name !== "aether")];
}
...
type DynamicToolBuildParams = {
  ...
  cronCreatorToolAllowlistRef?: AetherCodingToolsOptions["cronCreatorToolAllowlistRef"];
  cronCreatorToolAllowlistCaptureRef?: AetherCodingToolsOptions["cronCreatorToolAllowlistCaptureRef"];
  resolveCronCreatorToolAuthority?: Parameters<
    typeof runWithCronCreatorAuthorityCapabilityResolver
  >[0]["resolve"];
  cronCreatorAuthorityUnavailableReason?: AetherCodingToolsOptions["cronCreatorAuthorityUnavailableReason"];
  ...
  claimYieldCompletion?: AetherCodingToolsOptions["claimYieldCompletion"];
  ...
  registerRunCleanup?: AetherCodingToolsOptions["registerRunCleanup"];
};
...
export async function buildDynamicTools(
  ...
  const injectedAetherCodingToolsFactory = dynamicToolBuildState.aetherCodingToolsFactory;
  ...
  const options: AetherCodingToolsOptions = {
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as AetherCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  const allTools = input.resolveCronCreatorToolAuthority
    ? runWithCronCreatorAuthorityCapabilityResolver({
        ...
        run: buildAetherCodingTools,
      })
    : buildAetherCodingTools();
  toolBuildStages.mark("create-aether-coding-tools");
  ...
  const normallyProfiledTools =
    input.nativeToolSurfaceEnabled === false
      ? filterCodexDynamicToolsForDisabledNativeSurface(readableAllTools, input.pluginConfig, {
          preserveShell: shouldKeepAetherShellDynamicTools(input, nativeExecutionPolicy),
        })
      : filterCodexDynamicTools(readableAllTools, input.pluginConfig);
  const hostSystemAgentActive =
    input.isHostScopedToolActive?.("aether") ?? isHostScopedAgentToolActive("aether");
  ...
  if (
    ...
    const webSearchPolicy = (
      await import("aether/plugin-sdk/agent-harness")
...
/** Keeps the Aether Gateway execution path available beside Codex native shell. */
function addGatewayShellDynamicToolsIfAvailable(
  filteredTools: AetherDynamicTool[],
  allTools: AetherDynamicTool[],
  input: DynamicToolBuildParams,
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow Aether allowlists must fail closed rather than
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  ...
  // active Aether sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside Aether sandboxing. */
...
/** Converts Aether sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForAetherSandbox(
  sandbox: AetherSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForAetherSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function usesDockerNetworkConfig(sandbox: AetherSandboxContext | undefined): boolean {
  const backendId = sandbox?.backendId.trim().toLowerCase();
  return backendId === "docker" || backendId === "podman";
}

function codexNetworkAccessForAetherSandbox(
  sandbox: AetherSandboxContext | undefined,
): boolean {
...
function addSandboxShellDynamicToolsIfAvailable(
  ...
  const sandboxExecTool: AetherDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
      "Run a shell command through Aether's configured sandbox backend for this session. Use when Aether sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no Aether sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: AetherDynamicTool = {
    ...processTool,
    name: "sandbox_process",
    description:
      "Manage background shell sessions through Aether's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox follow-up; use Codex's native shell session handling only when no Aether sandbox is active and native Code Mode is available.",
  };
...
function shouldKeepAetherShellDynamicTools(
  input: DynamicToolBuildParams,
  nodePolicy: CodexNativeExecutionPolicy,
): boolean {
  return (
    ...
    // advertise a shell. Preserve Aether's policy-filtered direct shell.
    input.nativeToolSurfaceEnabled === false &&
    input.sandbox?.enabled !== true &&
