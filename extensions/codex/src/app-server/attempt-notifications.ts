...
/** Detects completion for an Aether dynamic tool result still awaited by Codex. */
export function isPendingAetherDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingAetherDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingAetherDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
