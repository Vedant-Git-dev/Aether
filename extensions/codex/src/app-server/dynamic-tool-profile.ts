...
/** Tool names owned by Codex app-server and normally excluded from Aether dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  AETHER_BUILD_PRIVATE_QA?: string;
  AETHER_QA_FORCE_RUNTIME?: string;
};

/** Normalizes Aether/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped Aether run's exact tool contract. */
export function isSystemAgentOnlyCodexDynamicToolAllowlist(
  toolsAllow: readonly string[] | undefined,
): boolean {
  return (
    toolsAllow?.length === 1 && normalizeCodexDynamicToolName(toolsAllow[0] ?? "") === "aether"
  );
}
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    env.AETHER_BUILD_PRIVATE_QA === "1" &&
    env.AETHER_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters Aether tools that Codex owns natively or config explicitly excludes. */
export function filterCodexDynamicTools<T extends { name: string }>(
  ...
  return filterCodexDynamicToolsWithOptions(tools, config, env, {
    preserveAetherReplacements: false,
    preserveAetherShell: false,
  });
}

/** Keeps Aether coding tools that replace a disabled Codex native surface. */
export function filterCodexDynamicToolsForDisabledNativeSurface<T extends { name: string }>(
  ...
  return filterCodexDynamicToolsWithOptions(tools, config, env, {
    preserveAetherReplacements: true,
    preserveAetherShell: options.preserveShell,
  });
}
...
function filterCodexDynamicToolsWithOptions<T extends { name: string }>(
  ...
  options: { preserveAetherReplacements: boolean; preserveAetherShell: boolean },
): T[] {
  const excludes = new Set<string>();
  if (!options.preserveAetherReplacements) {
  ...
  if (isForcedPrivateQaCodexRuntime(env)) {
    ...
    for (const name of CODEX_APP_SERVER_OWNED_DYNAMIC_TOOL_EXCLUDES) {
      if (
        options.preserveAetherReplacements &&
        CODEX_APP_SERVER_OWNED_REPLACEABLE_TOOL_EXCLUDES.has(name)
      ) {
        continue;
      }
      if (options.preserveAetherShell && CODEX_APP_SERVER_OWNED_SHELL_TOOL_EXCLUDES.has(name)) {
        continue;
      }
      excludes.add(name);
    }
