...
export function createCodexAttemptLifecycleController(
  ...
  const { state, activeTurnItemIds, pendingAetherDynamicToolCompletionIds } = turnRuntime;
  const releaseTurnAfterTerminalDynamicTool = (value: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingAetherDynamicToolCompletionIdsCount: pendingAetherDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const scheduleTerminalDynamicToolReleaseCheck = () => {
    ...
    const immediate = setImmediate(() => {
      ...
      if (
        ...
        pendingAetherDynamicToolCompletionIds.size === 0
      ) {
  ...
  const emitFastModeAutoProgress = async (payload: {
    ...
    try {
      await params.onToolResult?.({
        text: summary,
        channelData: { aetherProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
      });
    } catch (error) {
