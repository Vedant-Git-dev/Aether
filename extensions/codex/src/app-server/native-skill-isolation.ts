...
import { resolveRequiredHomeDir, resolveStateDir } from "aether/plugin-sdk/state-paths";
...
async function usesDefaultStateDir(): Promise<boolean> {
  if (!process.env.AETHER_STATE_DIR?.trim()) {
    return true;
  }
  ...
  const [stateDir, defaultStateDir] = await Promise.all([
    ...
    canonicalizeExistingPath(path.join(home, ".aether")),
  ]);
...
/** Resolves the native user-scope skills that an isolated Aether thread must disable. */
