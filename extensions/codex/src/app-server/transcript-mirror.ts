...
import { withCodexSessionTranscriptMirrorWriteLock } from "aether/plugin-sdk/codex-session-transcript-runtime";
import {
...
/** Imports a bounded, user-visible Codex history tail into a new Aether transcript. */
      ...
      for (const { dedupeIdentity, idempotencyKey, message, sourceFingerprint } of candidates) {
        ...
        const preparedUserMessage =
          transcriptMessage.role === "user"
            ? {
                ...transcriptMessage,
                __aether: { ...Reflect.get(transcriptMessage, "__aether") },
              }
            : undefined;
        if (preparedUserMessage?.["__aether"].humanMentions !== undefined) {
          ...
          preparedUserMessage["__aether"].humanMentions = structuredClone(
            preparedUserMessage["__aether"].humanMentions,
          );
        }
        ...
        if (message.role === "assistant" && message.aetherAsyncDelivery) {
          ...
          messageToAppend = Object.assign(messageToAppend, {
            aetherAsyncDelivery: { itemId: message.aetherAsyncDelivery.itemId },
          });
        }
