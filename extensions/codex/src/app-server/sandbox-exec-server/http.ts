 ...
 * access through the active Aether sandbox backend.
 */
import { embeddedAgentLog } from "aether/plugin-sdk/agent-harness-runtime";
import { SsrFBlockedError, isBlockedHostnameOrIp } from "aether/plugin-sdk/ssrf-runtime";
...
tmp=$(mktemp "$TMPDIR/aether-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/aether-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
