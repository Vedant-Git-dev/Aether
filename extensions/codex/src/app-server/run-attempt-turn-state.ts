...
export function createCodexAttemptTurnState(resources: CodexAttemptResources) {
  ...
  const pendingAetherDynamicToolCompletionIds = new Set<string>();
  ...
  const aetherDynamicToolExecutions = createCodexDynamicToolExecutionRegistry();
  ...
  return {
    ...
    pendingAetherDynamicToolCompletionIds,
    aetherDynamicToolExecutions,
    activeTurnItemIds,
    turnIdRef,
    userInputBridgeRef,
    steeringQueueRef,
