/**
 * Bridges Codex app-server approval requests into Aether policy hooks and
 * plugin approval UX.
 */
import {
...
import { coerceErrorMessage } from "aether/plugin-sdk/error-runtime";
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    ...
    const policyOutcome = await runAetherToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
...
/** Converts an Aether approval outcome into the app-server method response. */
function buildApprovalResponse(
  ...
  return {
    decision: "decline",
    reason: "Aether codex app-server bridge does not grant native approvals yet.",
  };
}
...
async function runAetherToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  ...
  const policyRequest = buildAetherToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
  ...
  if ("params" in outcome && toolPolicyParamsWereRewritten(policyRequest.params, outcome.params)) {
    return {
      outcome: "denied",
      reason:
        "Aether tool policy rewrote Codex app-server approval params; refusing original request.",
    };
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `Aether native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        coerceErrorMessage(error),
      )}`,
...
function readNativeRelayPreToolUseDecision(response: NativeHookRelayProcessResponse | undefined):
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "Aether native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "Aether native hook policy denied Codex app-server approval.",
  ...
  return {
    blocked: true,
    reason: output
      ? "Aether native hook relay returned a non-deny Codex app-server approval decision."
      : "Aether native hook relay returned an unreadable Codex app-server approval result.",
    failureDisposition: "failed",
  };
}
...
function buildAetherToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
