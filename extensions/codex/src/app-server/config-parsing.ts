import { buildSecretInputSchema } from "aether/plugin-sdk/secret-input";
import { asNullableRecord } from "aether/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "aether/plugin-sdk/windows-spawn";
import { z } from "zod";
import {
...
export const DEFAULT_CODEX_APP_SERVER_NETWORK_PROXY_PROFILE_PREFIX = "aether-network";
...
export function readCodexPluginConfig(value: unknown): ParsedCodexPluginConfig {
  ...
  if (appServer?.approvalPolicy === "untrusted") {
    throw new Error(
      'plugins.entries.codex.config.appServer.approvalPolicy="untrusted" is retired; run "aether doctor --fix" to migrate it to "on-request".',
    );
  }
...
export function assertCodexAppServerCommandHasNoInlineArgs(params: {
  ...
  const sourceLabel =
    params.source === "env"
      ? "AETHER_CODEX_APP_SERVER_BIN"
      : "plugins.entries.codex.config.appServer.command";
  const argsLabel =
    params.source === "env"
      ? "AETHER_CODEX_APP_SERVER_ARGS"
