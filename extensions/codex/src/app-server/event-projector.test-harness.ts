...
import type { EmbeddedRunAttemptParamsV2 as EmbeddedRunAttemptParams } from "aether/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
  formatToolAggregate,
...
import { openFileBackedSessionManagerForTest } from "aether/plugin-sdk/agent-runtime-test-contracts";
import { SessionManager } from "aether/plugin-sdk/agent-sessions";
import {
...
import { createMockPluginRegistry } from "aether/plugin-sdk/plugin-test-runtime";
...
export async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aether-codex-projector-"));
