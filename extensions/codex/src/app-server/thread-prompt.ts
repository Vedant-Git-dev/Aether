...
import { listRegisteredPluginAgentPromptGuidance } from "aether/plugin-sdk/plugin-runtime";
import {
...
import {
  CODEX_AETHER_DIRECT_DYNAMIC_TOOL_NAMESPACE,
...
export function buildDeveloperInstructions(
  ...
  for (const spec of options.dynamicTools ?? []) {
    const isDirectNamespace =
      ...
      spec.name.trim() === CODEX_AETHER_DIRECT_DYNAMIC_TOOL_NAMESPACE;
    ...
    for (const tool of spec.type === "namespace" ? spec.tools : [spec]) {
      ...
      hasControlTools ||= name === "aether" || name === "gateway";
    }
  }
  ...
  const deferredToolDiscoveryGuidance =
    deferredToolNames.size > 0 || nativeDelegationAvailable
      ...
      : undefined;
  const sections = [
    "You are a personal agent running inside Aether. Aether has dynamic tools for Aether-owned messaging, cron, sessions, media, gateway, and nodes.",
    deferredToolNames.size > 0
      ? `Deferred searchable Aether dynamic tools available: ${[...deferredToolNames]
    ...
    nativeDelegationAvailable
      ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred.${hasSessionsSpawn ? " Use Aether `sessions_spawn` only for Aether or ACP delegation, never as a substitute for `spawn_agent` on internal legwork." : ""}`
      : undefined,
    hasSessionsYield && nativeDelegationAvailable
      ? "When a native child's result belongs in a later turn, end the current turn with `aether_direct.sessions_yield`; the completion arrives as the next model-visible input. Use native `wait_agent` only for an intentional same-turn wait when the immediate next step is blocked on the child. Never loop-poll for native child completion."
      : undefined,
    delegationGuidanceAvailable
      ? buildDelegationGuidanceSection({
