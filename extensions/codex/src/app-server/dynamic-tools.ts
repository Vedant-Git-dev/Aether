/**
 * Bridges Aether runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
} from "aether/plugin-sdk/agent-harness-tool-runtime";
...
import type { ImageContent, TextContent } from "aether/plugin-sdk/llm";
import {
  asNonArrayRecord,
  asOptionalRecord,
  isRecord,
...
const INTERNAL_TOOL_EXECUTION_VALIDATION = Symbol.for("aether.internalToolExecutionValidation");
...
/**
 * Creates dynamic tool specs and a call handler that executes Aether tools,
    ...
    handleToolCall: async (call, options) => {
      ...
      if (!toolEntry) {
        ...
        const message = registeredToolNames.has(call.tool)
          ? `Aether tool is not available for this turn: ${call.tool}`
          : `Unknown Aether tool: ${call.tool}`;
        finalizeToolTerminalPresentation({
      ...
      try {
        ...
        const errorMessage = formatToolExecutionErrorMessage(
          error,
          "Aether dynamic tool call failed.",
        );
        executionPrevented =
          executionPrevented ||
          consumePreExecutionBlockedToolCall(call.callId, toolResultHookContext.runId);
...
function convertToolContents(
  ...
  const noticeText = `...(Aether truncated dynamic tool result: original ${totalTextChars} chars, weighted budget ${maxChars}; rerun with narrower args.)`;
  const notice = `\n${noticeText}`;
