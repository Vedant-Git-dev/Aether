/**
 * Hosts the local Aether sandbox exec-server that Codex app-server native
...
import { embeddedAgentLog } from "aether/plugin-sdk/agent-harness-runtime";
import type { PluginRuntime } from "aether/plugin-sdk/plugin-runtime";
import type { SandboxContext } from "aether/plugin-sdk/sandbox";
...
import type {
  ...
  AetherExecServer,
  AetherLeasedExecServer,
  AetherNodeExecServer,
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  if (!canExposeLocalExecServerToAppServer(params.appServerStartOptions)) {
    throw new Error(
      "Aether Codex exec-server uses a local loopback URL and cannot be registered with a remote Codex app-server.",
    );
  }
  const { server: execServer, nodeLease } = await acquireAetherExecServer({
  ...
  const environmentId = nodeLease ? `aether-node-${nodeLease.id}` : execServer.environmentId;
  try {
    ...
    await releaseAetherExecServer(execServer);
    throw error;
  }
  const environment = {
    environmentId,
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    ...
    await releaseAetherExecServer(server);
  }
}
...
async function acquireAetherExecServer(params: {
  sandbox: SandboxContext;
  runtime?: PluginRuntime;
  signal?: AbortSignal;
  onExecutionDisconnect?: (error: Error) => void;
}): Promise<{ server: AetherLeasedExecServer; nodeLease?: CodexNodeExecServerLease }> {
  ...
  while (true) {
    ...
    const promise = existing ?? startAndRememberAetherExecServer(sandbox);
...
function startAndRememberAetherExecServer(
  sandbox: SandboxContext,
): Promise<AetherLeasedExecServer> {
  const created = startAetherExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startAetherExecServer(sandbox: SandboxContext): Promise<AetherLeasedExecServer> {
  ...
  if (placementNodeId) {
    ...
    if (!backend) {
      throw new Error("Aether sandbox backend is unavailable.");
    }
    if (!fsBridge) {
  ...
  if (!address || typeof address === "string") {
    throw new Error("Aether Codex exec-server did not bind to a TCP port.");
  }
  const environmentId = buildEnvironmentId(sandbox);
  const authPath = `/aether-${randomUUID()}`;
  ...
  const execServer: AetherLeasedExecServer =
    connection.kind === "node"
...
async function releaseAetherExecServer(execServer: AetherLeasedExecServer): Promise<void> {
...
function buildEnvironmentId(sandbox: SandboxContext): string {
  ...
  return `aether-sandbox-${hash}`;
}

function isAuthorizedExecServerRequest(
  execServer: AetherLeasedExecServer,
...
function handleNodeConnection(
  execServer: AetherNodeExecServer,
  socket: WebSocket,
  request: IncomingMessage,
): void {
...
function handleConnection(execServer: AetherExecServer, socket: WebSocket): void {
