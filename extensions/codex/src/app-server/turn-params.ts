...
function buildCodexCurrentSenderContextValue(params: EmbeddedRunAttemptParams): string | undefined {
  const metadata = asOptionalRecord(
    asOptionalRecord(params.userTurnTranscriptRecorder?.message as unknown)?.["__aether"],
  );
  const recorded = [
...
export function buildTurnStartParams(
  ...
  if (currentSenderContext) {
    additionalContext = {
      ...additionalContext,
      aether_current_sender: { kind: "untrusted", value: currentSenderContext },
    };
  }
  if (params.permissionChange?.notice) {
    ...
    additionalContext = {
      ...additionalContext,
      aether_permission_change: { kind: "application", value: params.permissionChange.notice },
    };
  }
  return {
    ...
    // an Aether-owned priority override left on this exact warm session.
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. Aether adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an Aether cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
