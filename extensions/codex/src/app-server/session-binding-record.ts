...
import type { EmbeddedRunAttemptParamsV2 } from "aether/plugin-sdk/agent-harness-runtime";
...
/** Resolves the same agent scope Aether uses for transcript/session ownership. */
...
const threadBindingSchema = z
  .object({
    ...
    // marker; public Aether session metadata must never authorize user-home access.
    ...
    // Freeze Aether-carried AGENTS.md at thread creation; bootstrap refreshes
    ...
    // this marker across resumes so Aether never substitutes a default or fallback.
    preserveNativeModel: z.literal(true).optional().catch(undefined),
    // Continue creates the Aether Chat before native execution. This closed
