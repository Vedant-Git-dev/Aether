...
import { acknowledgeInternalToolResult } from "aether/plugin-sdk/agent-harness-tool-runtime";
import { isTerminalCodexTurnNotificationForTurn } from "./attempt-notification-state.js";
import {
  isCodexTurnAbortMarkerNotification,
  isPendingAetherDynamicToolCompletionNotification,
    ...
    try {
      if (isCurrentTurn) {
        ...
        if (
          ...
          if (itemId) {
            pendingAetherDynamicToolCompletionIds.delete(itemId);
          }
        }
        if (notification.method === "item/completed") {
          ...
          if (item?.type === "dynamicToolCall" && typeof item.id === "string") {
            const response = await aetherDynamicToolExecutions.get({
              threadId: resourceState.thread.threadId,
              turnId,
              callId: item.id,
            });
