...
import type { AuthProfileStore } from "aether/plugin-sdk/agent-runtime";
...
const CODEX_APP_SERVER_ARGS_ENV_KEY = "AETHER_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  return {
    ...start,
    // A fresh private home has no native account; bridge Aether auth even
...
function createCodexBoundedApprovalHandler(taskLabel: string) {
  return (request: { method: string }): JsonValue | undefined => {
    if (
      ...
      return {
        decision: "decline",
        reason: `Aether Codex ${taskLabel} does not grant tool or file approvals.`,
      };
    }
    ...
    if (request.method.includes("requestApproval")) {
      return {
        decision: "decline",
        reason: `Aether Codex ${taskLabel} does not grant native approvals.`,
      };
    }
    if (request.method === "mcpServer/elicitation/request") {
      return createCodexElicitationResponse("decline", null, {
        message: `Aether Codex ${taskLabel} does not support interactive input.`,
      });
    }
    return undefined;
  };
