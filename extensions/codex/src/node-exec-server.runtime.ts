...
import type { AetherPluginNodeHostCommandIo } from "aether/plugin-sdk/node-host";
import { killProcessTree } from "aether/plugin-sdk/process-runtime";
import { sanitizeEnvVars } from "aether/plugin-sdk/sandbox";
...
export async function runCodexNodeExecServer(params: {
  ...
  try {
    ...
    return await withTempWorkspace(
      { rootDir: resolvePreferredAetherTmpDir(), prefix: "codex-node-exec-server-" },
      async ({ dir }) => {
