...
async function continueCodexSession(
  ...
  writeLine(`Aether session: ${singleLineTerminalText(result.sessionKey)}`);
}
...
export function registerCodexSessionCli(program: Command): void {
  ...
  addGatewayClientOptions(
    codex
      ...
      .description("Continue a Gateway-local Codex thread as an Aether branch")
  ...
  addGatewayClientOptions(
    codex
      ...
      .option(
        ...
        "Confirm no other Codex client or Aether runner is using this thread",
        false,
      )
      .option("--json", "Print the structured response", false),
