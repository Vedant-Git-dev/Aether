...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("aether.codex.conversationControl");
...
function requirePreparedThreadBinding(binding: CodexAppServerThreadBinding | undefined) {
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this Aether session yet.");
  }
  return binding;
}
