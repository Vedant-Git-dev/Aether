...
export function listNodeAdoptedSessionEntries(params: {
  ...
  for (const { agentId, entry, sessionKey } of listSessionCatalogEntries({
    ...
    if (adopted.has(sourceKey)) {
      throw new Error(
        `multiple Aether sessions adopt Codex thread ${marker.sourceThreadId} on ${marker.sourceHostId}`,
      );
    }
...
export async function finalizeNodeAdoptedSession(params: {
  ...
  const changedError = () =>
    new CatalogParamsError("Codex Aether session changed before it could be bound. Retry.");
