...
export async function handleNativeGoal(
  ...
  if (!binding?.threadId) {
    return "No Codex thread is attached to this Aether session yet.";
  }
...
export async function startThreadAction(
  ...
  if (kind === "compact") {
    ...
    if (
      ...
      return "Codex compaction is unavailable because the current Aether session is not using the Codex runtime.";
    }
