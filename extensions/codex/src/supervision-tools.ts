import { resolveDefaultAgentDir } from "aether/plugin-sdk/agent-harness-registration";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import type { AnyAgentTool } from "aether/plugin-sdk/core";
/**
...
import { readStringParam } from "aether/plugin-sdk/param-readers";
...
const LEGACY_CODEX_SUPERVISOR_ENDPOINTS_ENV = "AETHER_CODEX_SUPERVISOR_ENDPOINTS";
/** Legacy standalone-MCP transcript gate. Agent tools use canonical config. */
const LEGACY_CODEX_SUPERVISOR_RAW_TRANSCRIPTS_ENV =
  "AETHER_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS";
/** Legacy standalone-MCP write gate. Agent tools use canonical config. */
const LEGACY_CODEX_SUPERVISOR_WRITE_CONTROLS_ENV = "AETHER_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS";
...
type CodexSupervisionToolsOptions = {
  ...
  getRuntimeConfig?: () => AetherConfig | undefined;
...
function resolveEndpoints(
  ...
  env: NodeJS.ProcessEnv,
  runtimeConfig: AetherConfig | undefined,
...
function idleContinuationError(threadId: string): Error {
  return new Error(
    `Codex thread ${threadId} is idle. Continue it from Codex Sessions so Aether can install the Codex harness approval and tool handlers before resume.`,
  );
}
...
export function createCodexSupervisionTools(options: CodexSupervisionToolsOptions): AnyAgentTool[] {
  ...
  return [
    ...
    },
    {
      ...
      description: "List Codex sessions visible to the Aether supervisor.",
