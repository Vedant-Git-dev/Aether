...
import { sessionCatalogAdoptedSourceKey } from "aether/plugin-sdk/session-catalog";
import {
  ...
  for (const adopted of adoptedEntries) {
    if (adopted.entry.initializationPending === true) {
      throw new CatalogParamsError(
        "Codex session cannot be archived while its Aether branch is initializing",
      );
    }
    ...
    if (
      ...
      throw new CatalogParamsError(
        "Codex session cannot be archived until its Aether branch starts",
      );
    }
  }
}
      ...
      return await params.bindingStore.withThreadArchiveFence(async () => {
        const run = async (control: CodexSessionCatalogControl) => {
          ...
          if (await params.bindingStore.hasOtherThreadOwner(params.threadId)) {
            throw new CatalogParamsError(
              "Codex session cannot be archived while it is attached to an Aether session",
            );
          }
