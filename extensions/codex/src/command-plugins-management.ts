...
} from "aether/plugin-sdk/interactive-runtime";
...
import type { PluginCommandContext, PluginCommandResult } from "aether/plugin-sdk/plugin-entry";
...
// Plugin lifecycle changes (enable/disable) write to aether.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(configKey)}: ${wantEnabled ? "enabled" : "disabled"} in aether.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
async function installCodexPlugin(
  ...
  if (!alreadyInstalled && !plugin.marketplacePath && plugin.remotePluginId) {
    if (plugin.mustShowInstallationInterstitial === true) {
      return {
        text: `${formatCodexDisplayText(requestedId)} requires a Codex installation confirmation that Aether cannot display. Install it in Codex first, then rerun this command to authorize it here.`,
      };
    }
  ...
  try {
    ...
    return {
      text: `${formatCodexDisplayText(requestedId)} was installed in Codex but could not be authorized in Aether and will not be exposed: ${formatCodexDisplayText(errorMessage(error))}`,
    };
  }
  ...
  if (appsNeedingAuth.length > 0) {
    ...
    const presentation: MessagePresentation = {
      ...
      blocks: [
        {
          type: "text",
          text: `${formatCodexDisplayText(requestedId)} bundle was installed in Codex. Aether app access is configured. ${authRequirement} connector authentication in ChatGPT. Installation does not confirm app connections or current-conversation readiness.`,
        },
  ...
  return {
    text: `${formatCodexDisplayText(requestedId)} ${status}. Aether app access is configured.${refreshWarning} ${POLICY_REFRESH_HINT}`,
  };
}
...
function formatPluginList(
  ...
  return [
    "Codex sub-plugins in Aether config (~/.aether/aether.json):",
    "",
    ...rows.map(
      (r) =>
