...
import { runCommandBuffered } from "aether/plugin-sdk/process-runtime";
...
export function createCodexCliSessionNodeHostCommands(): AetherPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): AetherPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredAetherTmpDir(), "aether-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
