...
import { asNullableRecord } from "aether/plugin-sdk/string-coerce-runtime";
...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.codex.config.codexDynamicToolsProfile is retired; Codex app-server always keeps Codex-native workspace tools native. Run "aether doctor --fix".',
    match: hasRetiredDynamicToolsProfile,
  },
  {
    path: ["plugins", "entries", "codex", "config", "codexPlugins"],
    message:
      'plugins.entries.codex.config.codexPlugins.allow_destructive_actions="on-request" was renamed to "auto". Run "aether doctor --fix".',
    match: hasLegacyPluginDestructivePolicy,
  },
  {
    path: ["plugins", "entries", "codex", "config", "appServer"],
    message:
      'plugins.entries.codex.config.appServer.approvalPolicy values "on-failure" and "untrusted" are retired; use "on-request". Run "aether doctor --fix".',
    match: hasRetiredApprovalPolicy,
  },
  {
    path: ["plugins", "entries", "codex", "config", "appServer"],
    message:
      'Codex app-server turn idle timeouts are retired; native Codex owns provider liveness and turn completion. The existing agents.defaults.timeoutSeconds run limit remains unchanged. Run "aether doctor --fix" to remove the old settings.',
 ...
 */
export function normalizeCompatibilityConfig({ cfg }: { cfg: AetherConfig }): {
  config: AetherConfig;
  changes: string[];
} {
  ...
  const nextConfig = structuredClone(cfg) as AetherConfig & {
