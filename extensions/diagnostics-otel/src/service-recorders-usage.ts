...
export function createUsageRecorders(runtime: DiagnosticsRecorderRuntime) {
  ...
  const recordModelUsage = (
    ...
    const attrs = {
      "aether.channel": evt.channel ?? "unknown",
      "aether.agent": normalizeDiagnosticValue(evt.agentId),
    ...
    if (usage.input) {
      tokensCounter.add(usage.input, { ...attrs, "aether.token": "input" });
    ...
    if (usage.output) {
      tokensCounter.add(usage.output, { ...attrs, "aether.token": "output" });
    ...
    if (usage.cacheRead) {
      tokensCounter.add(usage.cacheRead, { ...attrs, "aether.token": "cache_read" });
    }
    if (usage.cacheWrite) {
      tokensCounter.add(usage.cacheWrite, { ...attrs, "aether.token": "cache_write" });
    }
    if (usage.promptTokens) {
      tokensCounter.add(usage.promptTokens, { ...attrs, "aether.token": "prompt" });
    }
    if (usage.total) {
      tokensCounter.add(usage.total, { ...attrs, "aether.token": "total" });
    }
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "aether.tokens.input": usage.input ?? 0,
      "aether.tokens.output": usage.output ?? 0,
      "aether.tokens.cache_read": usage.cacheRead ?? 0,
      "aether.tokens.cache_write": usage.cacheWrite ?? 0,
      "aether.tokens.total": usage.total ?? 0,
    };
    if (metadata.trusted && metadata.internal && hostPluginId) {
      spanAttrs["aether.plugin"] = normalizeDiagnosticValue(hostPluginId);
    }
    ...
    const span = spanWithDuration("aether.model.usage", spanAttrs, evt.durationMs, {
  ...
  const recordWebhookReceived = (
    ...
    const attrs = {
      "aether.channel": evt.channel ?? "unknown",
      "aether.webhook": evt.updateType ?? "unknown",
    };
  ...
  const recordWebhookProcessed = (
    ...
    const attrs = {
      "aether.channel": normalizeDiagnosticValue(evt.channel),
      "aether.webhook": normalizeDiagnosticValue(evt.updateType),
    };
    ...
    const span = spanWithDuration("aether.webhook.processed", spanAttrs, evt.durationMs);
    span.end();
  };
  ...
  const recordWebhookError = (evt: Extract<DiagnosticEventPayload, { type: "webhook.error" }>) => {
    ...
    const spanAttrs: Record<string, string | number> = {
      ...attrs,
      "aether.error": redactedError,
    };
    const span = tracer.startSpan("aether.webhook.error", {
  ...
  const recordMessageQueued = (
    ...
    const attrs = {
      "aether.channel": normalizeDiagnosticValue(evt.channel),
      "aether.source": normalizeDiagnosticValue(evt.source),
    };
  ...
  const recordMessageDispatchStarted = (
    ...
    trackInternalOrTrustedSpan(
      evt,
      metadata,
      spanWithDuration("aether.message.processed", attrs, undefined, {
  ...
  const recordMessageProcessed = (
    ...
    const attrs = {
      "aether.channel": normalizeDiagnosticValue(evt.channel),
      "aether.outcome": evt.outcome ?? "unknown",
    };
    ...
    const spanAttrs: Record<string, string | number> = { ...attrs };
    if (evt.reason) {
      spanAttrs["aether.reason"] = normalizeDiagnosticValue(evt.reason, "unknown");
    }
    const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
    const span =
      trackedSpan ??
      spanWithDuration("aether.message.processed", spanAttrs, evt.durationMs, {
        parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
        endTimeMs: evt.ts,
      });
  ...
  const messageDeliveryAttrs = (evt: MessageDeliveryDiagnosticEvent): Record<string, string> => ({
    "aether.channel": normalizeDiagnosticValue(evt.channel),
    "aether.delivery.kind": normalizeDiagnosticValue(evt.deliveryKind, "other"),
  });
  ...
  const recordMessageDeliveryCompleted = (
    ...
    const span = spanWithDuration(
      "aether.message.delivery",
      {
        ...attrs,
        "aether.delivery.result_count": evt.resultCount,
      },
      evt.durationMs,
  ...
  const recordMessageDeliveryError = (
    ...
    const span = spanWithDuration("aether.message.delivery", attrs, evt.durationMs, {
  ...
  const recordRunStarted = (
    ...
    const span = trackTrustedSpan(
      evt,
      metadata,
      spanWithDuration("aether.run", spanAttrs, undefined, {
        parentContext: activeTrustedParentContext(evt, metadata),
        startTimeMs: evt.ts,
      }),
    );
