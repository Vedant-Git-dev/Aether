...
export function createDiagnosticsLogExporter(params: {
  ...
  if (logsEnabled) {
    ...
    if (logsToOtlp) {
      ...
      otelLogger = logProvider.getLogger("aether");
    }
    ...
    const buildDiagnosticLogRecord = (
      ...
      assignOtelLogAttribute(attributes, "aether.log.level", logLevelName);
      if (evt.loggerName) {
        assignOtelLogAttribute(attributes, "aether.logger", evt.loggerName);
      }
      if (evt.loggerParents?.length) {
        assignOtelLogAttribute(attributes, "aether.logger.parents", evt.loggerParents.join("."));
      }
