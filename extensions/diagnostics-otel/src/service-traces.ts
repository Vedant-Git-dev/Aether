...
export function createDiagnosticsTraceRuntime(tracer: Tracer) {
  ...
  // whose trace context came from Aether-owned scope (metadata.trustedTraceContext).
  ...
  const addRunAttrs = (
    ...
    if (evt.provider) {
      spanAttrs["aether.provider"] = evt.provider;
    }
    if (evt.model) {
      spanAttrs["aether.model"] = evt.model;
    }
    if (evt.channel) {
      spanAttrs["aether.channel"] = evt.channel;
    }
    if (evt.trigger) {
      spanAttrs["aether.trigger"] = evt.trigger;
    }
  };
  ...
  const paramsSummaryAttrs = (
    ...
    return {
      "aether.tool.params.kind": summary.kind,
      ...("length" in summary ? { "aether.tool.params.length": summary.length } : {}),
    };
  };
  return {
    tracer,
