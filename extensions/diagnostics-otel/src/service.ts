...
import { registerUnhandledRejectionHandler } from "aether/plugin-sdk/runtime-env";
import type { DiagnosticTraceContext, AetherPluginService } from "../api.js";
import {
...
export function createDiagnosticsOtelService(): AetherPluginService {
    ...
    async start(ctx) {
      ...
      const meter = !metricsActive
        ...
        : active.meterProvider
          ? active.meterProvider.getMeter("aether")
          : metrics.getMeter("aether");
      const tracer = active.traceProvider
        ? active.traceProvider.getTracer("aether")
        : trace.getTracer("aether");
