...
import type { AetherPluginServiceContext } from "../api.js";
import type { ExporterHealthUpdate } from "./service-exporter-health.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/aether-diagnostics-otel-test";
...
type OtelConfig = NonNullable<
  NonNullable<AetherPluginServiceContext["config"]["diagnostics"]>["otel"]
>;
...
type StartOtelServiceOptions = OtelContextFlags & {
  endpoint?: string;
  configure?: (ctx: AetherPluginServiceContext) => void;
};
type InternalDiagnosticListener = Parameters<
  NonNullable<AetherPluginServiceContext["internalDiagnostics"]>["onEvent"]
>[0];
...
const exporterHealthReports = new WeakMap<AetherPluginServiceContext, ReportedExporterHealth[]>();
...
export function createOtelContext(
  ...
  const ctx: AetherPluginServiceContext = {
    config: {
      diagnostics: {
        enabled: true,
        otel: {
