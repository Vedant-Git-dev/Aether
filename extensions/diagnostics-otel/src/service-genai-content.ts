...
// Shared text-part reading for gen_ai message normalization: Aether emits
...
function redactInternalReasoningFromMessage(value: unknown): unknown {
  ...
  // instead of using Aether's normalized thinking blocks. Those fields are
...
export function assignOtelToolIdentityAttributes(
  ...
  // low-cardinality, and unlike the dropped aether.toolCallId passthrough keys
...
export function assignOtelModelContentAttributes(
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "aether.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "aether.content.output_messages",
      redactedContent?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "aether.content.system_prompt", content?.systemPrompt);
  }
}
...
export function assignOtelToolContentAttributes(
  ...
  // aether.content.* names; normalize once so both copies stay byte-identical.
  if (policy.toolInputs) {
    ...
    if (toolInput) {
      ...
      attributes["aether.content.tool_input"] = toolInput;
    }
  }
  if (policy.toolOutputs) {
    ...
    if (toolOutput) {
      ...
      attributes["aether.content.tool_output"] = toolOutput;
    }
  }
}
