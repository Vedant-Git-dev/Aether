...
export function assignOtelLogEventAttributes(
  ...
  assignOtelEventAttributes(attributes, eventAttributes, "aether.");
}
...
export function assignOtelSecurityAttributes(
  ...
  assignOtelLogAttribute(attributes, "aether.security.event_id", evt.eventId);
  assignOtelLogAttribute(attributes, "aether.security.category", evt.category);
  assignOtelLogAttribute(
    attributes,
    "aether.security.action",
    normalizeDiagnosticValue(evt.action),
  );
  assignOtelLogAttribute(attributes, "aether.security.outcome", evt.outcome);
  assignOtelLogAttribute(attributes, "aether.security.severity", evt.severity);
  if (evt.reason) {
    assignOtelLogAttribute(
      attributes,
      "aether.security.reason",
      normalizeDiagnosticValue(evt.reason),
    );
  }
  if (evt.actor) {
    assignOtelLogAttribute(attributes, "aether.security.actor.kind", evt.actor.kind);
    if (evt.actor.idHash) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.actor.id_hash",
    ...
    if (evt.actor.deviceIdHash) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.actor.device_id_hash",
    ...
    if (evt.actor.scopes?.length) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.actor.scopes",
        evt.actor.scopes.map((scope) => normalizeDiagnosticValue(scope)).join(","),
      );
    }
  }
  if (evt.target) {
    assignOtelLogAttribute(attributes, "aether.security.target.kind", evt.target.kind);
    if (evt.target.idHash) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.target.id_hash",
  ...
  if (evt.policy) {
    if (evt.policy.id) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.policy.id",
        normalizeDiagnosticValue(evt.policy.id),
      );
    }
    if (evt.policy.decision) {
      assignOtelLogAttribute(attributes, "aether.security.policy.decision", evt.policy.decision);
    }
    if (evt.policy.reason) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.policy.reason",
  ...
  if (evt.control) {
    if (evt.control.id) {
      assignOtelLogAttribute(
        attributes,
        "aether.security.control.id",
        normalizeDiagnosticValue(evt.control.id),
      );
    }
    if (evt.control.family) {
      assignOtelLogAttribute(attributes, "aether.security.control.family", evt.control.family);
    }
  }
