...
import { asFiniteNumber, asFiniteNumberInRange } from "aether/plugin-sdk/string-coerce-runtime";
...
export function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "aether.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "aether.model_call.response_bytes", evt.responseStreamBytes);
...
export function assignModelCallPromptStatsAttrs(
  ...
  for (const [key, value] of [
    ["aether.model_call.prompt.input_messages_count", stats.inputMessagesCount],
    ["aether.model_call.prompt.input_messages_chars", stats.inputMessagesChars],
    ["aether.model_call.prompt.system_prompt_chars", stats.systemPromptChars],
    ["aether.model_call.prompt.tool_definitions_count", stats.toolDefinitionsCount],
    ["aether.model_call.prompt.tool_definitions_chars", stats.toolDefinitionsChars],
    ["aether.model_call.prompt.total_chars", stats.totalChars],
  ] as const) {
...
export function assignModelCallUsageAttrs(
  ...
  for (const [key, value] of [
    ["aether.model_call.usage.input_tokens", usage.input],
    ["aether.model_call.usage.output_tokens", usage.output],
    ["aether.model_call.usage.cache_read_input_tokens", usage.cacheRead],
    ["aether.model_call.usage.cache_creation_input_tokens", usage.cacheWrite],
    ["aether.model_call.usage.reasoning_output_tokens", usage.reasoningTokens],
    ["aether.model_call.usage.prompt_tokens", promptTokens],
    ["aether.model_call.usage.total_tokens", usage.total],
...
export function assignGenAiModelCallAttrs(
  ...
  attrs["aether.model_call.observation_unit"] = modelCallObservationUnit(evt);
}
...
export function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("aether.provider.request", {
    "aether.upstreamRequestIdHash": boundedHash,
  });
}
