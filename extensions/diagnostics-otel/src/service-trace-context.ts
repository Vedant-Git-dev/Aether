...
export function addTraceAttributes(
  ...
  attributes["aether.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["aether.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["aether.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["aether.traceFlags"] = normalized.traceFlags;
  }
}
