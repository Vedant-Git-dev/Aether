...
- Start from `aether wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `aether wiki obsidian status` before shelling out, then prefer the dedicated helpers like `aether wiki obsidian search`, `aether wiki obsidian open`, `aether wiki obsidian command`, and `aether wiki obsidian daily`.
