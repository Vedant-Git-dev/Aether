...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Aether**.
...
In agent scope, `vault.path` is a parent directory. Aether appends the
normalized agent id:
...
`~/.aether/wiki/support` and `~/.aether/wiki/marketing`. With no explicit
path, the parent defaults to `<state-dir>/wiki`; the default `main` agent
therefore uses `<state-dir>/wiki/main`. The state directory is `~/.aether` by
default and follows `AETHER_STATE_DIR` when configured. In global scope,
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also persists a machine-readable snapshot in Aether plugin state so agent/runtime consumers do not have to scrape markdown pages.
...
Unmanaged raw Markdown can live under `sources/` without Aether page frontmatter. Add `<!-- aether:wiki:raw-source -->` near the top of the page body to opt it out of wiki page metadata and freshness lint; generated or source-sync tracked imports still require their structured metadata.
...
aether wiki doctor
aether wiki init
aether wiki ingest ./notes/alpha.md
aether wiki compile
aether wiki lint
aether wiki search "alpha"
aether wiki get entity.alpha --from 1 --lines 80

aether wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

aether wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

aether wiki bridge import
aether wiki unsafe-local import

aether wiki obsidian status
aether wiki obsidian search "alpha"
aether wiki obsidian open syntheses/alpha-summary.md
aether wiki obsidian command workspace:quick-switcher
aether wiki obsidian daily

# Agent-scoped vault
aether wiki status --agent support
aether wiki search "refund policy" --agent support
```
