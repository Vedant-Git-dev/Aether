...
const LEGACY_MEMORY_WIKI_COMPILED_CACHE_PATHS = [
  ".aether-wiki/cache/agent-digest.json",
  ".aether-wiki/cache/claims.jsonl",
] as const;
...
function readConfiguredPluginConfig(config: AetherConfig): MemoryWikiPluginConfig | undefined {
      ...
      for (const vaultRoot of resolveConfiguredVaultRoots({
        ...
        for (const relativePath of LEGACY_MEMORY_WIKI_COMPILED_CACHE_PATHS) {
          ...
          try {
            ...
            if (!isMissingPathError(error)) {
              warnings.push(
                `Skipped rebuildable Memory Wiki compiled cache cleanup. Run aether doctor --fix to retry. ${filePath}: ${String(error)}`,
              );
            }
          }
        }
