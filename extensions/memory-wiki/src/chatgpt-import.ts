...
const HUMAN_START_MARKER = "<!-- aether:human:start -->";
const HUMAN_END_MARKER = "<!-- aether:human:end -->";
  ...
  if (!params.dryRun && importRunRecord) {
    if (importRunRecord.createdPaths.length > 0 || importRunRecord.updatedPaths.length > 0) {
      ...
      const compile = await compileMemoryWikiVault(params.config).catch((error: unknown) => {
        ...
        throw new Error(
          `Memory Wiki ChatGPT import run ${importRunRecord.runId} changed source pages, but vault compilation failed: ${message}. After fixing the compile error, run \`aether wiki chatgpt rollback ${importRunRecord.runId}\` to restore the imported pages.`,
          { cause: error },
        );
      });
