...
import type { PluginBlobStore } from "aether/plugin-sdk/plugin-state-runtime";
...
const DASHBOARD_UNAVAILABLE_MESSAGES: Record<MemoryWikiDashboardPendingState["state"], string> = {
  ...
  "compile-required":
    'Memory Wiki dashboards need a compiled snapshot. Run "aether wiki compile", then reload.',
  failed: 'Memory Wiki dashboard rebuild failed. Run "aether wiki compile", then reload.',
};
