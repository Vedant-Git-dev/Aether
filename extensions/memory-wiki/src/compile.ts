...
import { retryTransientMemoryRead } from "aether/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "aether/plugin-sdk/memory-host-markdown";
import { root as fsRoot } from "aether/plugin-sdk/security-runtime";
import {
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- aether:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- aether:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
async function compileMemoryWikiVaultUnlocked(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- aether:wiki:index:start -->",
      endMarker: "<!-- aether:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- aether:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- aether:wiki:${group.dir}:index:end -->`,
