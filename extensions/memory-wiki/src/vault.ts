...
import { FsSafeError, pathExists, root as fsRoot } from "aether/plugin-sdk/security-runtime";
import {
...
const WIKI_VAULT_SCAFFOLD = ["AGENTS.md", "WIKI.md", "index.md", ".aether-wiki/log.jsonl"];
...
This vault is maintained by the Aether memory-wiki plugin.
...
- Compiled query and prompt snapshots live in Aether plugin state, not vault files.
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".aether-wiki/log.jsonl", "", createdFiles);
