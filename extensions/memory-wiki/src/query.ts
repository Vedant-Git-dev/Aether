...
import { filterMemorySearchHitsBySessionVisibility } from "@aether/memory-core/api.js";
import { resolveSessionAgentIdStrict } from "aether/plugin-sdk/agent-scope-runtime";
import { runTasksWithConcurrency } from "aether/plugin-sdk/concurrency-runtime";
import type { MemorySearchResult } from "aether/plugin-sdk/memory-core-host-runtime-files";
import { resolveDefaultAgentId } from "aether/plugin-sdk/memory-host-core";
import { getActiveMemorySearchManager } from "aether/plugin-sdk/memory-host-search";
import type { AetherPluginToolContext } from "aether/plugin-sdk/plugin-entry";
import { FsSafeError, root as fsRoot } from "aether/plugin-sdk/security-runtime";
import {
...
const RELATED_BLOCK_PATTERN =
  /<!-- aether:wiki:related:start -->[\s\S]*?<!-- aether:wiki:related:end -->/g;
const MARKDOWN_FRONTMATTER_PATTERN = /^\s*---\r?\n[\s\S]*?\r?\n---\r?\n?/;
const STRUCTURAL_MARKER_LINE_PATTERN = /^\s*<!--\s*aether:(?:wiki|human):[^>]*-->\s*$/;
...
type ConversationRecallContext = NonNullable<AetherPluginToolContext["conversationRecall"]>;
