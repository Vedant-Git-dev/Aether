...
import { readPositiveIntegerParam } from "aether/plugin-sdk/param-readers";
import type { AetherConfig, AetherPluginApi } from "../api.js";
...
type GatewayMethodContext = Parameters<
  Parameters<AetherPluginApi["registerGatewayMethod"]>[1]
>[0];
...
export function registerMemoryWikiGatewayMethods(params: {
  ...
  resolveConfig?: (agentId?: string, appConfig?: AetherConfig) => ResolvedMemoryWikiConfig;
  ...
  const getAppConfig = () => {
    ...
    if (typeof api.runtime.config?.current === "function") {
      return api.runtime.config.current() as AetherConfig;
    }
