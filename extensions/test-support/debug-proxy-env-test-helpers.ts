...
const DEBUG_PROXY_ENV_KEYS = [
  "AETHER_DEBUG_PROXY_ENABLED",
  "AETHER_DEBUG_PROXY_SESSION_ID",
  "AETHER_STATE_DIR",
] as const;
...
export function installDebugProxyTestResetHooks() {
  ...
  afterEach(async () => {
    const { closeDebugProxyCaptureStore } = await import("aether/plugin-sdk/proxy-capture");
    const { closeAetherStateDatabaseForTest } =
      await import("aether/plugin-sdk/sqlite-runtime-testing");
