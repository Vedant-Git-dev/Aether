...
import { buildOpenAICompatibleLiveProviderCatalog } from "aether/plugin-sdk/provider-catalog-live-runtime";
...
import { fetchMinimaxUsage } from "aether/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: AetherConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: AetherConfig): string | undefined {
...
export function registerMinimaxProviders(api: AetherPluginApi) {
