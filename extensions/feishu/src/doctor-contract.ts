...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  ...
  feishuStrayEntryConfigMigration.legacyConfigRule,
  {
    ...
    message:
      'channels.feishu[.accounts.<id>].webhookPath must be a canonical HTTP request path; run "aether doctor --fix".',
    match: (value) => {
  ...
  },
  {
    path: ["channels", "feishu"],
    message:
      'channels.feishu[.accounts.<id>].tools.base is legacy; use tools.bitable. Run "aether doctor --fix".',
    match: (value) => {
      const entry = asObjectRecord(value);
      return (
