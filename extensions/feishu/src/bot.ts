...
import { normalizeOptionalString, uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
...
export async function handleFeishuMessage(params: {
  ...
  // SAFETY: config.current() returns the canonical host-validated AetherConfig.
  const cfg = getFeishuRuntime().config.current() as AetherConfig;
  ...
  try {
    ...
    if (isDirect) {
      const currentCfg = getFeishuRuntime().config.current() as AetherConfig;
