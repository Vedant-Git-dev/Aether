...
import { isPathStrictlyInside } from "aether/plugin-sdk/file-access-runtime";
...
function collectConfiguredAgentIds(cfg: AetherConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: AetherConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "aether doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: AetherConfig): boolean {
