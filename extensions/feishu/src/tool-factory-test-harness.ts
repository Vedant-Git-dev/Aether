...
export function createToolFactoryHarness(cfg: AetherPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<AetherPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
