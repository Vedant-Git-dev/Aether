...
import { resolveChannelConfigWrites } from "aether/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "aether/plugin-sdk/routing";
import type { AetherConfig, PluginRuntime } from "../runtime-api.js";
import { resolveFeishuAccount } from "./accounts.js";
import type { DynamicAgentCreationConfig } from "./types.js";

type MaybeCreateDynamicAgentResult = {
  created: boolean;
  updatedCfg: AetherConfig;
  agentId?: string;
};
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: AetherConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: AetherConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as AetherConfig;
  ...
  let skippedCfg: AetherConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.aether/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.aether/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as AetherConfig,
