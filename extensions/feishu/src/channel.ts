import { describeAccountSnapshot } from "aether/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID } from "aether/plugin-sdk/account-resolution";
import { resolveAgentConfig } from "aether/plugin-sdk/agent-scope-runtime";
import { formatAllowFromLowercase } from "aether/plugin-sdk/allow-from";
import { createActionGate, ToolAuthorizationError } from "aether/plugin-sdk/channel-actions";
import {
  adaptScopedAccountAccessor,
  createHybridChannelConfigAdapter,
} from "aether/plugin-sdk/channel-config-helpers";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionContext,
  ChannelMessageToolDiscovery,
} from "aether/plugin-sdk/channel-contract";
import { createChatChannelPlugin } from "aether/plugin-sdk/channel-core";
import {
...
import { createPairingPrefixStripper } from "aether/plugin-sdk/channel-pairing";
import {
  createAllowlistProviderGroupPolicyWarningCollector,
  createConditionalWarningCollector,
} from "aether/plugin-sdk/channel-policy";
import { PAIRING_APPROVED_MESSAGE } from "aether/plugin-sdk/channel-status";
import { getSessionBindingService } from "aether/plugin-sdk/conversation-runtime";
import {
  createChannelDirectoryAdapter,
  createRuntimeDirectoryLiveAdapter,
} from "aether/plugin-sdk/directory-runtime";
import { PlatformMessageNotDispatchedError } from "aether/plugin-sdk/error-runtime";
import { resolveLegacyInteractiveTextFallback } from "aether/plugin-sdk/interactive-runtime";
...
import { patchTopLevelChannelConfigSection } from "aether/plugin-sdk/setup";
import {
...
import { sanitizeAssistantVisibleText } from "aether/plugin-sdk/text-chunking";
import type {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      config: {
        ...
        deleteAccount: ({ cfg, accountId }) => {
          ...
          if (isDefault) {
            ...
            const next = { ...cfg } as AetherConfig;
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("aether/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("aether/plugin-sdk/config-mutation");
