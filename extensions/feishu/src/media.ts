...
import type { MessageReceipt } from "aether/plugin-sdk/channel-outbound";
import { PlatformMessageNotDispatchedError } from "aether/plugin-sdk/error-runtime";
import { detectMime, mediaKindFromMime } from "aether/plugin-sdk/media-mime";
import {
...
import { saveMediaBuffer, type SavedMedia } from "aether/plugin-sdk/media-store";
import type { ReplyPayloadTtsSupplement } from "aether/plugin-sdk/reply-payload";
import { readRegularFile, writeExternalFileWithinRoot } from "aether/plugin-sdk/security-runtime";
...
function createConfiguredFeishuMediaClient(params: { cfg: AetherConfig; accountId?: string }): {
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "aether-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredAetherTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
...
async function probeMediaDurationMs(params: {
  ...
  try {
    return await withTempWorkspace(
      { rootDir: resolvePreferredAetherTmpDir(), prefix: "feishu-media-probe-" },
      async (workspace) => {
        const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
        const inferredExt =
          ext && ext.length <= 12
