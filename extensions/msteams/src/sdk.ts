...
import { readSecretFile } from "aether/plugin-sdk/secret-file";
...
import { buildAetherUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after Aether sender policy passes.
...
async function createMSTeamsApp(
  ...
  // backend can identify Aether traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: privateQaRuntime?.client ??
      options?.httpClient ?? {
        headers: { "User-Agent": buildAetherUserAgentFragment() },
