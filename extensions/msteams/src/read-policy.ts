import { ToolAuthorizationError } from "aether/plugin-sdk/channel-actions";
...
function resolveMSTeamsReadGroupPolicy(cfg: AetherConfig) {
  const teams = cfg.channels?.msteams;
...
function hasMutableChannelConfig(cfg: AetherConfig): boolean {
...
function bothUnknownScopesAllowed(cfg: AetherConfig): boolean {
  const teams = cfg.channels?.msteams;
...
export async function assertMSTeamsTeamEnumerationAllowed(params: {
  ...
  const directRoute = resolveMSTeamsRouteConfig({
    ...
    conversationId: "__aether_all_channels__",
    allowNameMatching: isDangerousNameMatchingEnabled(teams),
  });
