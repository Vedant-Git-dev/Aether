...
function renderMSTeamsQaText(text: string) {
  const mentionText = "<at>aether</at>";
  const renderedText = text.replace(/@aether\b/giu, mentionText);
  return {
    text: renderedText,
    ...(renderedText === text
      ? {}
          ...
          entities: [
            {
              type: "mention",
              text: mentionText,
              mentioned: { id: APP_ID, name: "Aether QA" },
            },
          ],
        }),
  };
...
export async function createMSTeamsQaTransportAdapter(
  ...
  try {
    ...
    await fs.writeFile(
      bootstrapPath,
      [
        'const key = Symbol.for("aether.msteams.privateQaRuntime");',
  ...
  return {
    ...
    async sendInbound(input) {
      ...
      const activity = {
        ...
        recipient: { id: APP_ID, name: "Aether QA" },
        conversation: {
    ...
    createGatewayConfig: () =>
      ...
      }) as Pick<AetherConfig, "channels" | "messages">,
