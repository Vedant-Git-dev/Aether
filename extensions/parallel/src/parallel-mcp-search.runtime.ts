...
import { readPluginPackageVersion } from "aether/plugin-sdk/extension-shared";
import {
  readProviderTextResponse,
  readResponseTextLimited,
} from "aether/plugin-sdk/provider-http";
import { withTrustedWebSearchEndpoint } from "aether/plugin-sdk/provider-web-search";
...
// without this, undici sends a generic `node` UA and Aether usage is only
// visible via the JSON-RPC `clientInfo` payload.
const USER_AGENT = `aether-parallel/${PLUGIN_VERSION} (${process.platform})`;
  ...
  const init = await postMcp({
    ...
    body: {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "aether-parallel", version: PLUGIN_VERSION },
      },
    },
  });
  if (!init.ok) {
