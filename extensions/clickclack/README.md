# ClickClack Aether channel

Official Aether channel plugin for ClickClack.

## Install

```sh
aether plugins install @aether/clickclack
```
...
```sh
aether channels add clickclack --code 'https://clickclack.example.com/#XXXX-XXXX-XXXX'
```

Split-origin and path-mounted ClickClack deployments generate an exact
`/api/bot-setup-codes/claim#CODE` endpoint. Aether validates the versioned
claim response and saves its canonical API base.

For manual token setup:

```sh
aether channels add clickclack \
...
Run `aether onboard` for guided setup. The workspace value can be a
...
authentication gateway such as Cloudflare Access while the local Aether
...
extension publishes Aether's native commands for composer autocomplete,
...
ClickClack can create one managed channel for each Aether session:
  ...
  channels: {
    clickclack: {
      ...
      discussions: {
        enabled: true,
        workspace: "default",
        controlUrlBase: "https://team.aether.ai",
        section: "Sessions",
      },
    },
  },
...
Aether Control UI, preserving base paths. Main sessions use `/chat/<agent>`.
...
two Aether gateways using the same ClickClack workspace do not adopt each
...
See `docs/channels/clickclack.md` in the Aether repository, or the published docs at `https://docs.aether.ai/channels/clickclack`.
