...
import { waitUntilAbort } from "aether/plugin-sdk/channel-outbound";
import {
  channelBlockedPatch,
  channelReadyPatch,
  channelStoppedPatch,
} from "aether/plugin-sdk/gateway-runtime";
import { registerPluginHttpRoute } from "aether/plugin-sdk/webhook-ingress";
...
export function collectSmsStartupWarnings(account: ResolvedSmsAccount): string[] {
  ...
  if (!account.publicWebhookUrl && !account.dangerouslyDisableSignatureValidation) {
    ...
    warnings.push(
      "- SMS: publicWebhookUrl must be a properly encoded absolute HTTP(S) URL with a valid hostname, no embedded credentials, and remain within Aether's 4,000-character callback safety limit; Aether will omit the per-message delivery callback until fixed.",
    );
  }
