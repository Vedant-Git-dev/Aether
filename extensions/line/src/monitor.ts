...
import { hasFinalInboundReplyDispatch } from "aether/plugin-sdk/channel-inbound";
import { resolveChannelStreamingBlockEnabled } from "aether/plugin-sdk/channel-outbound";
...
import { channelReadyPatch, channelStoppedPatch } from "aether/plugin-sdk/gateway-runtime";
...
interface MonitorLineProviderOptions {
  ...
  config: AetherConfig;
  runtime: RuntimeEnv;
  buildContext?: typeof import("aether/plugin-sdk/channel-inbound").buildChannelInboundEventContext;
        ...
        try {
          ...
          if (body.events && body.events.length > 0) {
            ...
            if ((await match.target.bot.handleWebhook(body)) === "durable") {
              res.setHeader("x-aether-delivery-accepted", "durable");
            }
          }
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json");
