...
Read-only diff viewer plugin for **Aether** agents.

## Install

```bash
aether plugins install @aether/diffs
```
...
Set plugin-wide defaults in `~/.aether/aether.json`:
...
- Plugin id: `diffs`
- Package: `@aether/diffs`
- Minimum Aether host: `2026.4.30`
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/aether",
        },
      },
    },
  },
...
Before:
Aether supports plugins.

After:
Aether supports plugins and hosted diff views.
```
...
- Rendered PNG/PDF files remain temporary materializations in `$TMPDIR/aether-diffs` because delivery APIs require a file path. No JSON metadata sidecars are written or imported.
