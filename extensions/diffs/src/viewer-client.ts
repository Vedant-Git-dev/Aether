...
declare const __AETHER_DIFFS_LANGUAGE_PACK__: boolean | undefined;
...
function readInjectedLanguagePackFlag(): boolean | undefined {
  return typeof __AETHER_DIFFS_LANGUAGE_PACK__ === "boolean"
    ? __AETHER_DIFFS_LANGUAGE_PACK__
    : undefined;
}
...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-aether-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-aether-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.aetherDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.aetherDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

const disableAutoStartKey = Symbol.for("aether.diffs.disableAutoStart");
