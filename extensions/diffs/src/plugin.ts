...
export function registerDiffsPlugin(api: AetherPluginApi): void {
  ...
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredAetherTmpDir(), "aether-diffs"),
  ...
  const resolveCurrentAccessConfig = () => {
    const currentConfig = (api.runtime.config?.current?.() ?? api.config) as AetherConfig;
      ...
      return createDiffsTool({
        getConfig: () =>
          (ctx.getRuntimeConfig?.() ??
            ctx.runtimeConfig ??
            ctx.config ??
            api.runtime.config.current()) as AetherConfig, // SAFETY: The tool only reads this immutable runtime snapshot.
        store,
...
function resolveDiffsLanguagePackAvailability(api: AetherPluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as AetherConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "aether.plugin.json")) &&
