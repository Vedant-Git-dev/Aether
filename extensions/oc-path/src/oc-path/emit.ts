 ...
 * `__AETHER_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__AETHER_REDACTED__")) {
      guardSentinel("__AETHER_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
