/**
 * `aether path` — shell access to the OcPath substrate verbs.
 *
...
export function registerPathCli(program: Command): void {
  const path = program
    ...
    .addHelpText("after", "\nDocs: https://docs.aether.ai/cli/path\n");
  ...
  // Bare `aether path` prints help and exits 0 (matches the core
  // applyParentDefaultHelpAction contract — see aether#73077).
  path.action(() => {
    path.outputHelp();
    process.exitCode = 0;
  });
