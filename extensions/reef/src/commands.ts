...
export async function handleReefCommand({
  ...
  if ((changesFriendship || decidesReview) && senderIsOwner !== true) {
    return {
      text: "Only an owner in commands.ownerAllowFrom can change Reef friends or decide reviews. Ask a configured owner; friendship changes can also use aether reef locally.",
    };
  }
