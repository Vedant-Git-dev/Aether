...
import type { AetherConfig } from "aether/plugin-sdk/core";
...
export const reefSetupWizard = {
  channel: "reef",
  getStatus: async ({ cfg }: { cfg: AetherConfig }) => {
  ...
  configure: async ({ cfg }: { cfg: AetherConfig }) => ({ cfg }),
  configureInteractive: async ({
    ...
    if (identity && (identity.handle !== handle || identity.relayUrl !== relayUrl)) {
      throw new Error(
        `This Aether state already holds the Reef identity @${identity.handle} on ${identity.relayUrl}. Re-register the same handle and relay.`,
      );
    }
            ...
            options: [
              {
                ...
                label: "Existing Aether OAuth profile",
    ...
    const nextConfig = { ...cfg, channels: { ...cfg.channels, reef } } as AetherConfig;
    return {
      cfg:
        authMode === "oauth"
...
async function confirmReefOAuthAgentRuntime(params: {
  ...
  const { resolveSessionAgentIdStrict } = await import("aether/plugin-sdk/agent-scope-runtime");
  ...
  const resolveRuntimeId = (config: AetherConfig) =>
