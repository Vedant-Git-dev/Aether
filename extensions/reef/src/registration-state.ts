...
function reefIdentityConflict(binding: ReefIdentityBinding): Error {
  return new Error(
    `This Aether state already holds the Reef identity @${binding.handle} on ${binding.relayUrl}. Re-register the same handle and relay.`,
  );
}
...
export function assertReefIdentityBinding(
  ...
  if (!existing) {
    throw new Error(
      "Reef identity binding is missing; run aether doctor --fix or register this claw",
    );
  }
