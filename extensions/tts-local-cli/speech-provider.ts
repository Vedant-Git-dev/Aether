...
} from "aether/plugin-sdk/speech-core";
import { truncateUtf16Safe } from "aether/plugin-sdk/speech-provider";
import { asOptionalRecord, filterStringRecord } from "aether/plugin-sdk/string-coerce-runtime";
...
async function readAudioFile(filePath: string): Promise<Buffer> {
  const { readRegularFileSync } = await import("aether/plugin-sdk/security-runtime");
...
async function runCli(params: {
  ...
  const { runCommandBuffered } = await import("aether/plugin-sdk/process-runtime");
...
async function runFfmpegToBuffer(params: {
  ...
  const { runFfmpeg } = await import("aether/plugin-sdk/media-runtime");
  const { writeExternalFileWithinRoot } = await import("aether/plugin-sdk/security-runtime");
...
export function buildCliSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    async synthesize(req: SpeechSynthesisRequest) {
      const { resolvePreferredAetherTmpDir, withTempWorkspace } =
        await import("aether/plugin-sdk/temp-path");
      const { createSubsystemLogger } = await import("aether/plugin-sdk/runtime-env");
