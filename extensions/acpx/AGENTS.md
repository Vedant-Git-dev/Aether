...
The ACPX extension is a thin Aether wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `aether/acpx`, not in this extension.
...
Use this flow when Aether needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `aether/acpx` repo first.
2. In Aether, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Aether and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Aether ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Aether-specific glue, move it to `aether/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
