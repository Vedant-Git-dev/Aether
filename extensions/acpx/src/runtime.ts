/**
 * Aether ACPX runtime adapter. It wraps the upstream acpx runtime with
 * Aether session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { parseStrictPositiveInteger } from "aether/plugin-sdk/number-runtime";
import { redactSensitiveText } from "aether/plugin-sdk/security-runtime";
import { normalizeStringEntries } from "aether/plugin-sdk/string-coerce-runtime";
import { sliceUtf16Safe } from "aether/plugin-sdk/text-utility-runtime";
import { AcpRuntimeError, type AcpRuntime, type AcpRuntimeErrorCode } from "../runtime-api.js";
import { CODEX_ACP_PACKAGE, AETHER_CODEX_CONFIG_ARG } from "./codex-adapter.js";
...
import {
  cleanupAetherOwnedAcpxPendingLease,
  cleanupAetherOwnedAcpxProcessTree,
  isAetherLeaseAwareAcpxProcessCommand,
...
type AetherAcpxRuntimeOptions = AcpRuntimeOptions & {
  aetherLegacyBareSessionKeys?: ReadonlySet<string>;
  aetherWrapperRoot?: string;
  aetherGatewayInstanceId?: string;
  aetherProcessLeaseStore?: AcpxProcessLeaseStore;
  pluginToolsMcpBridgeEnabled?: boolean;
  aetherToolsMcpBridgeEnabled?: boolean;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  aetherProcessCleanup?: AcpxProcessCleanupDeps;
};
type AetherRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type AetherRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
type AetherRuntimeHandle = Awaited<ReturnType<AcpRuntime["ensureSession"]>>;
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "aether-plugin-tools";
const ACPX_AETHER_TOOLS_MCP_SERVER_NAME = "aether-tools";
const AETHER_TOOLS_MCP_AGENT_SESSION_KEY_ENV = "AETHER_TOOLS_MCP_AGENT_SESSION_KEY";
type ResetAwareSessionStore = AcpSessionStore & {
  markFresh: (sessionKey: string) => void;
};

type AetherLeaseSessionMetadata = {
  aetherLeaseId: string;
  aetherGatewayInstanceId: string;
};

function withAetherLeaseSessionMetadata<T extends object>(
  record: T,
  lease: AcpxProcessLeaseIdentity,
): T & AetherLeaseSessionMetadata {
  return {
    ...record,
    aetherLeaseId: lease.leaseId,
    aetherGatewayInstanceId: lease.gatewayInstanceId,
  };
}
...
function readAetherLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { aetherLeaseId } = record as { aetherLeaseId?: unknown };
  return typeof aetherLeaseId === "string" ? aetherLeaseId.trim() || undefined : undefined;
}

function readAetherGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { aetherGatewayInstanceId } = record as { aetherGatewayInstanceId?: unknown };
  return typeof aetherGatewayInstanceId === "string"
    ? aetherGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withAetherLeaseSessionMetadata(record, lease);
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        if (ownsExisting) {
          ...
          recordToSave = withAetherLeaseSessionMetadata(
            {
              ...lifecycleRecord,
...
const AETHER_BRIDGE_EXECUTABLE = "aether";
const AETHER_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_AETHER_PREFIX = "openai/";
// Documented Aether provider prefixes the Claude Agent SDK does not understand.
...
const CLAUDE_ACP_AETHER_PREFIX = /^(?:anthropic|amazon-bedrock)\//i;
...
function readAgentFromHandle(handle: AetherRuntimeHandle): string | undefined {
...
function isAetherBridgeCommand(command: AcpxAgentCommand | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command));
  if (basename(parts[0] ?? "") === AETHER_BRIDGE_EXECUTABLE) {
    return parts[1] === AETHER_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^aether(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === AETHER_BRIDGE_SUBCOMMAND;
}
...
function classifyCodexAcpModelRequest(
  ...
  if (value.toLowerCase().startsWith(CODEX_ACP_AETHER_PREFIX)) {
    value = value.slice(CODEX_ACP_AETHER_PREFIX.length);
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  const prefix = raw.match(CLAUDE_ACP_AETHER_PREFIX);
  if (!prefix) {
    return raw;
  }
...
function withAcpxSessionOptions(input: AetherRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
async function ensureDelegateSessionWithModelFallback(
  ...
  input: AetherRuntimeEnsureInput,
): Promise<AetherRuntimeHandle> {
  try {
...
function appendCodexAcpConfigOverrides(
  ...
  return [...splitCommandParts(command), AETHER_CODEX_CONFIG_ARG, JSON.stringify(config)];
}
...
function withManagedToolsMcpSessionEnv(params: {
  ...
  if (
    (!params.pluginToolsEnabled && !params.aetherToolsEnabled) ||
    !sessionKey ||
  ...
  const nextServers = params.mcpServers.map((server): AcpxMcpServer => {
    ...
    const isManagedAetherTools =
      params.aetherToolsEnabled && server.name === ACPX_AETHER_TOOLS_MCP_SERVER_NAME;
    if ((!isManagedPluginTools && !isManagedAetherTools) || !("command" in server)) {
      return server;
    }
    changed = true;
    const env = [
      ...server.env.filter((entry) => entry.name !== AETHER_TOOLS_MCP_AGENT_SESSION_KEY_ENV),
      {
        name: AETHER_TOOLS_MCP_AGENT_SESSION_KEY_ENV,
        value: sessionKey,
      },
    ];
    return {
      ...server,
      env,
      args: params.agentId ? [...server.args, "--aether-agent-id", params.agentId] : server.args,
    };
  });
  return changed ? nextServers : params.mcpServers;
}

/** Aether-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements CompleteAcpRuntime {
  ...
  private readonly aetherToolsMcpBridgeEnabled: boolean;
  ...
  constructor(options: AetherAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    this.legacyBareSessionKeys = new Set(options.aetherLegacyBareSessionKeys);
    const { aetherProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = aetherProcessCleanup;
    this.wrapperRoot = options.aetherWrapperRoot;
    this.gatewayInstanceId = options.aetherGatewayInstanceId;
    this.processLeaseStore = options.aetherProcessLeaseStore;
    this.pluginToolsMcpBridgeEnabled = options.pluginToolsMcpBridgeEnabled === true;
    this.aetherToolsMcpBridgeEnabled = options.aetherToolsMcpBridgeEnabled === true;
    this.managedToolsMcpBridgeEnabled =
      this.pluginToolsMcpBridgeEnabled || this.aetherToolsMcpBridgeEnabled;
    ...
    const useBridgeSafeProbe =
      this.managedToolsMcpBridgeEnabled || isAetherBridgeCommand(probeCommand);
  ...
  private resolveDelegateForSession(params: {
    ...
    if (isAetherBridgeCommand(params.command)) {
    ...
    const delegate = new BaseAcpxRuntime(
      {
        ...
        mcpServers: withManagedToolsMcpSessionEnv({
          ...
          aetherToolsEnabled: this.aetherToolsMcpBridgeEnabled,
  ...
  private async finalizeCompletedProbeLease(identity: AcpxLaunchLeaseContext): Promise<void> {
    ...
    await this.releaseProcessLeaseOperation(identity, async () => {
      ...
      await cleanupAetherOwnedAcpxPendingLease({
  ...
  private async readCodexTurnFailureStderr(params: {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readAetherLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readAetherLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupAetherOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readAetherGatewayInstanceIdFromRecord(record);
    await cleanupAetherOwnedAcpxProcessTree({
      rootPid,
  ...
  private async ensureSessionUnlocked(
    ...
    const appliedModel: AetherRuntimeHandle["appliedModel"] =
  ...
  startTurn(input: AetherRuntimeTurnInput): CompleteAcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([snapshotPromise, turnLeasePromise]).then(([snapshot]) => {
      ...
      return withTurnDiagnostics(command, async () => ({
        ...
        turn: delegate.startTurn({
          ...
          // Aether owns deadlines; acpx timeouts can report partial output as completed.
          timeoutMs: 0,
        }),
      }));
    });
