...
import { readJsonFileWithFallback } from "aether/plugin-sdk/json-store";
import { isRecord as isConfigRecord } from "aether/plugin-sdk/string-coerce-runtime";
import {
  parse as parseToml,
...
import { AETHER_ACPX_LEASE_ID_ARG, AETHER_GATEWAY_INSTANCE_ID_ARG } from "./process-lease.js";

const CLAUDE_ACP_PACKAGE = "@agentclientprotocol/claude-agent-acp";
const CLAUDE_ACP_BIN = "claude-agent-acp";
const RUN_CONFIGURED_COMMAND_SENTINEL = "--aether-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @aether/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Aether's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const aetherWrapperArgs = new Set([
  ${JSON.stringify(AETHER_ACPX_LEASE_ID_ARG)},
  ${JSON.stringify(AETHER_GATEWAY_INSTANCE_ID_ARG)},
  ${(params.aetherWrapperArgs ?? []).map((arg) => JSON.stringify(arg)).join(",\n  ")}
]);

function readAetherWrapperArg(args, name) {
...
function readAetherWrapperArgs(args, name) {
  const values = [];
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    readAetherWrapperArg(args, ${JSON.stringify(AETHER_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripAetherWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (aetherWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripAetherWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[aether] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[aether] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildCodexAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    ...
    aetherWrapperArgs: [AETHER_CODEX_CONFIG_ARG],
...
const aetherCodexConfigs = readAetherWrapperArgs(
  rawConfiguredArgs,
  ${JSON.stringify(AETHER_CODEX_CONFIG_ARG)},
);
if (aetherCodexConfigs.length > 0) {
  ...
  if (typeof env.CODEX_CONFIG === "string" && env.CODEX_CONFIG.trim()) {
    try {
      ...
      console.error("[aether] CODEX_CONFIG must be a valid JSON object");
      process.exit(1);
    }
  }
  for (const aetherCodexConfig of aetherCodexConfigs) {
    try {
      const parsedAetherCodexConfig = JSON.parse(aetherCodexConfig);
      if (
        !parsedAetherCodexConfig ||
        typeof parsedAetherCodexConfig !== "object" ||
        Array.isArray(parsedAetherCodexConfig)
      ) {
        throw new Error("invalid Aether Codex config");
      }
      existingCodexConfig = mergeCodexConfig(existingCodexConfig, parsedAetherCodexConfig);
    } catch {
      console.error("[aether] invalid generated Codex ACP startup config");
      process.exit(1);
    }
  }
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Aether; fallback must not float to an unpatched newer release.
    ...
    return {
      args: [
        ...(migration.hadOverrides
          ? [AETHER_CODEX_CONFIG_ARG, JSON.stringify(migration.config)]
          : []),
