...
import { formatPluginConfigIssue } from "aether/plugin-sdk/extension-shared";
...
function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "aether.plugin.json")) &&
...
function resolveAcpxPluginRootFromAetherLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Aether layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromAetherLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
export function resolveAetherRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolveManagedToolsMcpServerConfig(
  entryPoint: "plugin-tools-serve" | "aether-tools-serve",
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const aetherRoot = resolveAetherRoot(pluginRoot);
  const distEntry = path.join(aetherRoot, "dist", "mcp", `${entryPoint}.js`);
  ...
  const sourceEntry = path.join(aetherRoot, "src", "mcp", `${entryPoint}.ts`);
  return {
    command: process.execPath,
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.aetherToolsMcpBridge && resolved[ACPX_AETHER_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_AETHER_TOOLS_MCP_SERVER_NAME} is reserved when aetherToolsMcpBridge=true`,
    );
  }
  ...
  if (params.aetherToolsMcpBridge) {
    resolved[ACPX_AETHER_TOOLS_MCP_SERVER_NAME] = resolveManagedToolsMcpServerConfig(
      "aether-tools-serve",
      params.moduleUrl,
    );
  }
...
/** Convert Aether MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const aetherToolsMcpBridge = normalized.aetherToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    aetherToolsMcpBridge,
