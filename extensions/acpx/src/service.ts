...
import { finiteSecondsToTimerSafeMilliseconds } from "aether/plugin-sdk/number-runtime";
import type {
  OpenKeyedStoreOptions,
  PluginStateKeyedStore,
} from "aether/plugin-sdk/plugin-state-runtime";
...
import {
  ...
  reapStaleAetherOwnedAcpxOrphans,
...
const ENABLE_STARTUP_PROBE_ENV = "AETHER_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "AETHER_SKIP_ACPX_RUNTIME_PROBE";
    ...
    runtimePromise ??= loadRuntimeModule().then(async (module) => {
      ...
      for (const name of names) {
        ...
        if (
          ...
          !recordId.startsWith(".aether-owner-") &&
      ...
      runtime = new module.AcpxRuntime({
        ...
        aetherLegacyBareSessionKeys: legacyBareSessionKeys,
        aetherGatewayInstanceId: params.gatewayInstanceId,
        aetherProcessLeaseStore: params.processLeaseStore,
        aetherWrapperRoot: params.wrapperRoot,
        ...
        aetherToolsMcpBridgeEnabled: params.pluginConfig.aetherToolsMcpBridge,
...
function resolveAllowedAgentsProbeAgent(ctx: AetherPluginServiceContext): string | undefined {
...
async function measureAcpxStartup<T>(
  ctx: AetherPluginServiceContext,
  name: string,
  run: () => T | Promise<T>,
): Promise<T> {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      const result = await cleanupAetherOwnedAcpxPendingLease({
  ...
  for (const wrapperRoot of legacyWrapperRoots) {
    const legacyResult = await reapStaleAetherOwnedAcpxOrphans({
      wrapperRoot,
      deps: params.deps,
    });
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: AetherPluginServiceContext): Promise<void> {
      if (process.env.AETHER_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (AETHER_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (startupReap.terminatedPids.length > 0) {
        ctx.logger.info(
          `reaped ${startupReap.terminatedPids.length} stale Aether-owned ACPX process${startupReap.terminatedPids.length === 1 ? "" : "es"}`,
        );
      }
    ...
    async stop(_ctx: AetherPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
      if (runtime) {
