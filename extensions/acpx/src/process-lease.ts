/**
 * Persistent lease store for ACPX wrapper processes. Leases let Aether attach
...
export const AETHER_ACPX_LEASE_ID_ARG = "--aether-acpx-lease-id";
/** CLI argument carrying the owning gateway instance id. */
export const AETHER_GATEWAY_INSTANCE_ID_ARG = "--aether-gateway-instance-id";
/** Synthetic session identity for generated-wrapper health probes. */
export const ACPX_PROBE_LEASE_SESSION_KEY = "aether:acpx:probe";
...
/** Read Aether lease identity from a generated wrapper command. */
export function readAcpxProcessLeaseIdentity(
  ...
  const parts =
    typeof command === "string"
      ? Array.from(
          command.matchAll(
            /(?:^|\s)(--aether-(?:acpx-lease-id|gateway-instance-id))\s+(?:"([^"]*)"|'([^']*)'|(\S+))(?=\s|$)/g,
          ),
        ).flatMap((match) => [match[1]!, match[2] ?? match[3] ?? match[4]!])
      : (command ?? []);
  const leaseIndex = parts.lastIndexOf(AETHER_ACPX_LEASE_ID_ARG);
  const gatewayIndex = parts.lastIndexOf(AETHER_GATEWAY_INSTANCE_ID_ARG);
...
export function withAcpxLeaseArgs(params: {
  ...
  return [
    ...
    AETHER_ACPX_LEASE_ID_ARG,
    params.leaseId,
    AETHER_GATEWAY_INSTANCE_ID_ARG,
