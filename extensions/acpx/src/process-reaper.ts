 ...
 * Aether-owned wrapper trees after validating paths, packages, and lease ids.
 */
import { createRequire } from "node:module";
import path from "node:path";
import { isPidAlive, runExec } from "aether/plugin-sdk/process-runtime";
import { escapeRegExp } from "aether/plugin-sdk/text-utility-runtime";
import { CODEX_ACP_PACKAGE, LEGACY_CODEX_ACP_PACKAGE } from "./codex-adapter.js";
import type { AcpxAgentCommand } from "./command-line.js";
import { resolveAcpxPluginRoot, resolveAetherRoot } from "./config.js";
...
const AETHER_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
const OWNED_ACP_PACKAGE_NAMES = [
  ...
  // recognition until their Aether-owned wrapper/process tree is gone.
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const aetherRoot = resolveAetherRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(aetherRoot, "node_modules", packageName),
...
/** Check whether a command references an Aether-generated ACPX wrapper path. */
export function isAetherLeaseAwareAcpxProcessCommand(params: {
...
/** Check whether a command is owned by Aether ACPX runtime packages or wrappers. */
function isAetherOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  ...
  if (!normalized.includes(AETHER_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated Aether-owned ACPX wrapper process tree. */
export async function cleanupAetherOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an Aether-owned wrapper, fail closed to
  ...
  if (
    ...
    !isAetherOwnedAcpxProcessCommand({
      command: rootCommand,
      wrapperRoot: params.wrapperRoot,
    }) ||
    ...
    return {
      ...
      terminatedPids: [],
      skippedReason: "not-aether-owned",
    };
  }
...
export async function cleanupAetherOwnedAcpxPendingLease(params: {
  leaseId: string;
...
/** Reap orphaned Aether-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleAetherOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
