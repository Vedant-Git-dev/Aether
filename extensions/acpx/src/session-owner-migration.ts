...
function sessionDirectory(input: MigrationInput): string {
  if (!input.serviceWorkspaceDir) {
    throw new Error(
      "ACP ownership repair requires the Gateway service workspace; upgrade Aether Doctor.",
    );
  }
  return path.join(
...
async function legacyRecords(input: MigrationInput): Promise<{ directory: string; ids: string[] }> {
  ...
  const ids = names
    ...
    .filter((id) => !id.startsWith("agent:") && !id.startsWith(".aether-owner-"));
  if (ids.length === 0) {
...
async function migrateRecord(
  ...
  const temporaryId = `.aether-owner-${randomUUID()}`;
...
export const acpxSessionOwnerMigration: PluginDoctorStateMigration = {
  ...
  async detectLegacyState(input) {
    ...
    return ids.length
      ? {
          preview: [
            `ACP backend has ${ids.length} unqualified record(s). Stop the Gateway and run aether doctor --fix; ambiguous histories remain intact.`,
          ],
        }
      : null;
  },
