...
/** Logical Aether keys stay intact; only bare backend resource names need a namespace. */
export function resolveAcpxSessionResource(target: {
  ...
  if (!sessionKey || (encodedOwner && agentId !== encodedOwner) || (!encodedOwner && !agentId)) {
    throw new AcpRuntimeError(
      ...
      "ACP session owner is missing or disagrees with its logical key. Pass the Aether agentId that owns this session.",
