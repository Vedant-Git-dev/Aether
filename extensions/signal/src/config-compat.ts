import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "aether/plugin-sdk/account-resolution";
...
const PENDING_LEGACY_TRANSPORT_WARNING =
  "- channels.signal: legacy auto transport is ambiguous while its endpoint is unavailable; bring the endpoint online and rerun aether doctor --fix, or replace the retired fields with an explicit account-owned transport in aether.json.";
const PENDING_LEGACY_INVALID_URL_WARNING =
  "- channels.signal: legacy httpUrl is invalid; keep the current config, correct httpUrl, then run aether doctor --fix.";
const PENDING_LEGACY_INVALID_HOST_WARNING =
  "- channels.signal: legacy httpHost is invalid; keep the current config, correct httpHost, then run aether doctor --fix.";
const PENDING_LEGACY_INVALID_PORT_WARNING =
  "- channels.signal: legacy httpPort must be an integer between 1 and 65535; correct httpPort, then run aether doctor --fix.";
const PENDING_LEGACY_CONTAINER_ACCOUNT_WARNING =
  "- channels.signal: legacy container transport requires an account number; add channels.signal.account (or the relevant channels.signal.accounts.*.account) and rerun aether doctor --fix.";
...
function hasContainerTransportWithoutEffectiveAccount(cfg: AetherConfig): boolean {
...
function prepareLegacySignalTransportMigration(cfg: AetherConfig):
