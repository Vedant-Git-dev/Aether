...
} from "aether/plugin-sdk/channel-ingress-test-runtime";
import type { PluginRuntime } from "aether/plugin-sdk/core";
import type { MockFn } from "aether/plugin-sdk/plugin-test-runtime";
import { closeAetherAgentDatabasesForTest } from "aether/plugin-sdk/sqlite-runtime-testing";
...
vi.mock("aether/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("aether/plugin-sdk/runtime-config-snapshot")
  >("aether/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/session-store-runtime")>(
    "aether/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/channel-inbound", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/channel-inbound")>(
    "aether/plugin-sdk/channel-inbound",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/conversation-runtime")>(
    "aether/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/security-runtime")>(
    "aether/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/system-event-runtime")>(
    "aether/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("aether/plugin-sdk/reply-runtime"),
      import("aether/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    const createdStateDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "aether-signal-tool-result-state-"),
    );
