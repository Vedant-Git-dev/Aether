...
import { normalizeAccountId, resolveAccountEntry } from "aether/plugin-sdk/account-resolution";
...
export const signalCompletionNote = {
  ...
  lines: [
    ...
    `Then run: ${formatCliCommand("aether gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
...
function restorePromotedSignalDefaultAccount(cfg: AetherConfig): AetherConfig {
