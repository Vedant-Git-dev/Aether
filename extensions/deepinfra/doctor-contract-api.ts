...
// canonical key; `aether doctor --fix` repairs shipped `nativeBaseUrl` and
...
const FIX_HINT = `Run "aether doctor --fix" (api.deepinfra.com endpoints migrate automatically; custom hosts must set ${PROVIDER_PATH}.baseUrl to an OpenAI-compatible videos endpoint manually).`;
