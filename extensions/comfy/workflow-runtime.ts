...
import { bufferToBlobPart } from "aether/plugin-sdk/blob-runtime";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { resolveGeneratedMediaMaxBytes } from "aether/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "aether/plugin-sdk/media-mime";
import { resolvePositiveTimerTimeoutMs } from "aether/plugin-sdk/number-runtime";
import {
...
} from "aether/plugin-sdk/secret-input-runtime";
...
function getComfyConfig(cfg?: AetherConfig): ComfyProviderConfig {
...
async function resolveComfyHeadersConfig(value: unknown, cfg: AetherConfig): Promise<Headers> {
...
function hasUnavailableComfyHeaderSecret(value: unknown, cfg?: AetherConfig): boolean {
  if (!isRecord(value)) {
    return false;
  }
