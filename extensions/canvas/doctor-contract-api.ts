...
export const legacyConfigRules = [
  {
    path: ["canvasHost"],
    message:
      'canvasHost is retired; only plugins.entries.canvas.config.host.enabled remains. Run "aether doctor --fix".',
  },
  ...(["root", "port", "liveReload"] as const).map((key) => ({
    path: [...RETIRED_CANVAS_HOST_CONFIG_PATH, key],
    message: `${[...RETIRED_CANVAS_HOST_CONFIG_PATH, key].join(".")} is retired. Run "aether doctor --fix".`,
  })),
];
...
export const stateMigrations: PluginDoctorStateMigration[] = [
  {
    ...
    async migrateLegacyState(params) {
      ...
      const { pathExists } = await import("aether/plugin-sdk/text-utility-runtime");
      ...
      for (const documentId of documentIds) {
        ...
        try {
          ...
          warnings.push(
            `Skipped Canvas document ${documentId}; core target may already exist: ${String(error)}. Keep plugins.entries.canvas.config.host.root, resolve the copy or target conflict, then rerun "aether doctor --fix".`,
          );
        } finally {
          if (tempParent) {
