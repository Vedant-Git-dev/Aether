...
const A2UI_V08_BUNDLE_PATH = "/__aether__/a2ui/a2ui.bundle.js";
const A2UI_V09_BUNDLE_PATH = "/__aether__/a2ui/a2ui-v0.9.bundle.js";
...
function bootScript(messages: unknown[], promptGranted: boolean): string {
  const boot = JSON.stringify({ messages, actionTier: promptGranted ? "prompt" : "state" })
    ...
    .replaceAll("\u2028", "\\u2028")
    .replaceAll("\u2029", "\\u2029");
  return `<script>globalThis.aetherA2UIBoot=${boot};</script>`;
}
...
function resourceScript(path: string, url: string): string {
  ...
  return `<script>(()=>{const match=location.pathname.match(/^\\/__aether__\\/cap\\/[^/]+/u);const script=document.createElement("script");script.src=(match?.[0]??"")+${serializedPath};document.head.appendChild(script);})();</script>`;
}

/** Canvas-owned A2UI source validation and document composition for boards. */
export const canvasA2UIBoardWidgetKind: Parameters<
  AetherPluginApi["registerBoardWidgetContentKind"]
>[0] = {
  kind: "a2ui",
  label: "A2UI",
  resources: {
  ...
  composeDocument({ source, resourceUrls, promptGranted }) {
    ...
    return `${bootScript(parsed.messages, promptGranted)}<style>html,body{height:100%;overflow:hidden;background:transparent}aether-a2ui-host{display:block;height:100%}</style><aether-a2ui-host></aether-a2ui-host>${resourceScript(bundlePath, bundleUrl)}`;
  },
};
