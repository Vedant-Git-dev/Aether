...
function readLegacyCanvasRoot(config: AetherConfig): unknown {
...
export function listLegacyCanvasDocumentIds(documentsDir: string): string[] {
  try {
    ...
    throw new Error(
      `Cannot read Canvas documents at ${documentsDir}: ${String(error)}. Keep plugins.entries.canvas.config.host.root, fix access, then rerun "aether doctor --fix".`,
      { cause: error },
    );
  }
}
...
export function migrateCanvasHostConfig(config: AetherConfig): {
  config: AetherConfig;
  changes: string[];
} | null {
  ...
  const next = structuredClone(config) as AetherConfig & { canvasHost?: unknown };
