...
const routeBoardAction = async (action) => {
  const api = globalThis.aether;
  if (!api?.state?.emit) {
    return false;
  }
  if (globalThis.aetherA2UIBoot?.actionTier === "prompt" && api.prompt?.send) {
...
class AetherA2UIV09Host extends LitElement {
...
if (!customElements.get("aether-a2ui-host")) {
  customElements.define("aether-a2ui-host", AetherA2UIV09Host);
}
