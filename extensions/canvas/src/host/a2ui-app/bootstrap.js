...
class AetherA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding: var(--aether-a2ui-inset-top, 0px) var(--aether-a2ui-inset-right, 0px)
        var(--aether-a2ui-inset-bottom, 0px) var(--aether-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--aether-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--aether-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--aether-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--aether-a2ui-empty-top, var(--aether-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.aetherA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["aether:a2ui-action-status"]) {
      globalThis.addEventListener(eventName, this.#statusListener);
    }
    this.#syncSurfaces();
    const bootMessages = globalThis.aetherA2UIBoot?.messages;
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis["__aetherLastA2UIAction"] = userAction;

    const boardApi = globalThis.aether;
    if (boardApi?.state?.emit) {
      const request =
        globalThis.aetherA2UIBoot?.actionTier === "prompt" && boardApi.prompt?.send
...
if (!customElements.get("aether-a2ui-host")) {
  customElements.define("aether-a2ui-host", AetherA2UIHost);
}
