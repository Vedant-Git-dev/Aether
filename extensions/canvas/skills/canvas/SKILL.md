...
Do not invent local file paths or arbitrary external URLs. Hosted documents use the capability-scoped `/__aether__/canvas/documents/...` route returned by `show_widget`. A2UI content renders on session boards and loads renderer assets from `/__aether__/a2ui`.
