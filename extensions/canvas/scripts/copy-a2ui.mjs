...
function getA2uiPaths(env = process.env) {
  const srcDir = env.AETHER_A2UI_SRC_DIR ?? path.join(pluginDir, "src", "host", "a2ui");
  const outDir = env.AETHER_A2UI_OUT_DIR ?? path.join(rootDir, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

function shouldSkipMissingA2uiAssets(env = process.env) {
  return env.AETHER_A2UI_SKIP_MISSING === "1" || Boolean(env.AETHER_SPARSE_PROFILE);
}
...
export async function copyA2uiAssets({ srcDir, outDir }) {
  ...
  try {
    ...
    if (skipMissing) {
      console.warn(
        `${message} Skipping copy because AETHER_A2UI_SKIP_MISSING=1 or AETHER_SPARSE_PROFILE is set.`,
      );
      return;
    }
