...
const hashFile =
  process.env.AETHER_A2UI_BUNDLE_HASH_FILE ??
  path.join(pluginDir, "src", "host", "a2ui", ".bundle.hash");
const outputFile =
  process.env.AETHER_A2UI_BUNDLE_OUT ??
  path.join(pluginDir, "src", "host", "a2ui", "a2ui.bundle.js");
const outputV09File = process.env.AETHER_A2UI_BUNDLE_OUT
  ? `${process.env.AETHER_A2UI_BUNDLE_OUT}.v0.9.js`
...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.AETHER_SPARSE_PROFILE || process.env.AETHER_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because AETHER_A2UI_SKIP_MISSING=1 or AETHER_SPARSE_PROFILE is set.",
      );
      return;
    }
