...
import { resolveAccountWithDefaultFallback } from "aether/plugin-sdk/account-core";
import type { AetherConfig, TelegramAccountConfig } from "aether/plugin-sdk/config-contracts";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "aether/plugin-sdk/routing";
import { tryReadSecretFileSync } from "aether/plugin-sdk/secret-file-runtime";
import {
  coerceSecretRef,
...
function inspectTokenValue(params: { cfg: AetherConfig; value: unknown }): {
  token: string;
...
function hasConfiguredTelegramAccounts(cfg: AetherConfig): boolean {
