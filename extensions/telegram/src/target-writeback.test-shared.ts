...
vi.mock("aether/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/config-mutation")>(
    "aether/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/cron-store-runtime")>(
    "aether/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      readConfigFileSnapshotForWrite.mockResolvedValue({
        ...
        writeOptions: { expectedConfigPath: "/tmp/aether.json" },
      });
      loadCronStore.mockResolvedValue({
        version: 1,
        jobs: [
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/aether.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
