...
function scanTelegramMalformedGroupsConfig(cfg: AetherConfig): TelegramMalformedGroupsHit[] {
...
function scanTelegramInvalidAllowFromEntries(cfg: AetherConfig): TelegramAllowFromInvalidHit[] {
...
function scanTelegramBotEndpointApiRoots(cfg: AetherConfig): TelegramApiRootBotEndpointHit[] {
...
function formatTelegramAccountConfigPath(cfg: AetherConfig, accountId: string): string {
...
function maybeRepairTelegramApiRoots(cfg: AetherConfig): {
  config: AetherConfig;
  changes: string[];
} {
...
async function repairTelegramConfig(params: { cfg: AetherConfig }): Promise<{
  config: AetherConfig;
  changes: string[];
}> {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: AetherConfig): Promise<{
  config: AetherConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("aether/plugin-sdk/runtime");
...
export const telegramDoctor: ChannelDoctorAdapter = {
  ...
  collectPreviewWarnings: ({ cfg, doctorFixCommand, env }) => [
    ...
    ...listEnabledTelegramAccounts(cfg)
      ...
      .map(
        ({ accountId }) =>
          `- Telegram account "${accountId}" resolves webhookPath to /healthz, which is reserved for webhook listener health checks. Change webhookPath and the public webhook URL or proxy route before restarting Aether.`,
      ),
