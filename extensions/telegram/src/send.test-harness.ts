...
const { loadConfig, resolveStorePath } = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(
    (storePath?: string) => storePath ?? "/tmp/aether-telegram-send-tests.json",
  ),
}));
...
vi.mock("aether/plugin-sdk/plugin-config-runtime", async () => {
  const actual = await vi.importActual<typeof import("aether/plugin-sdk/plugin-config-runtime")>(
    "aether/plugin-sdk/plugin-config-runtime",
  );
  return {
    ...actual,
...
export function installTelegramSendTestHooks() {
  beforeEach(() => {
    ...
    resolveStorePath.mockReturnValue("/tmp/aether-telegram-send-tests.json");
