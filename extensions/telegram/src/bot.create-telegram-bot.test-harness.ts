...
import type { GetReplyOptions, MsgContext } from "aether/plugin-sdk/reply-runtime";
...
type GetRuntimeConfigFn =
  typeof import("aether/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type GetSessionEntryFn = typeof import("aether/plugin-sdk/session-store-runtime").getSessionEntry;
type ResolveStorePathFn =
  typeof import("aether/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("aether/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
type SessionEntry = import("aether/plugin-sdk/session-store-runtime").SessionEntry;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("aether/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}aether-telegram-${
      process.pid
...
vi.mock("aether/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
vi.mock("aether/plugin-sdk/channel-inbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aether/plugin-sdk/channel-inbound")>();
  return {
    ...actual,
...
function resolveDefaultModelForAgentForTest(params: { cfg: AetherConfig }): {
  provider: string;
  model: string;
} {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: AetherConfig = {
  agents: {
    defaults: {
      userTimezone: "UTC",
    },
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: AetherConfig) => {
