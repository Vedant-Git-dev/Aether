...
import { formatErrorMessage, formatUncaughtError } from "aether/plugin-sdk/error-runtime";
import { normalizeGroupActivation } from "aether/plugin-sdk/group-activation";
import {
...
export function createTelegramBotCore(
  ...
  const resolveGroupPolicy = (chatId: string | number, turnCfg: AetherConfig) =>
    resolveChannelGroupPolicy({
      cfg: turnCfg,
      channel: "telegram",
      accountId: account.accountId,
  ...
  const resolveGroupRequireMention = (chatId: string | number, turnCfg: AetherConfig) =>
    resolveScopeRequireMention({
