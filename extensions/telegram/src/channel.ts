...
import { clearAccountFieldsFromConfigSection } from "aether/plugin-sdk/channel-config-helpers";
import {
...
import { channelBlockedPatch } from "aether/plugin-sdk/gateway-runtime";
import { createLazyRuntimeModule } from "aether/plugin-sdk/lazy-runtime";
import { resolveAgentRoute, type RoutePeer } from "aether/plugin-sdk/routing";
import {
...
function isTelegramRichMessagesEnabled(cfg: AetherConfig, accountId?: string | null): boolean {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<AetherConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside Aether state; translate it back
      ...
      inboundFormattingHints: ({ cfg, accountId }) => {
        ...
        if (richMessages) {
          return {
            ...
            rules: [
              "Telegram rich ON (Bot API 10.3 blocks; Aether maps markdown + these HTML islands to typed blocks).",
