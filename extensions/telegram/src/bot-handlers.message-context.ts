...
import { formatMediaPlaceholderText } from "aether/plugin-sdk/channel-inbound";
import { resolveStoredModelOverride } from "aether/plugin-sdk/command-auth-native";
import type { AetherConfig, TelegramAccountConfig } from "aether/plugin-sdk/config-contracts";
...
import { asFiniteNumber } from "aether/plugin-sdk/string-coerce-runtime";
import { stripInlineDirectiveTagsForDelivery } from "aether/plugin-sdk/text-chunking";
...
function legacyAssistantTextKey(node: TelegramCachedMessageNode, botUserId?: number) {
  ...
  const timestamp = (
    node.sourceMessage as Message & { aether_prompt_context_timestamp_ms?: unknown }
  ).aether_prompt_context_timestamp_ms;
  const legacySelf =
