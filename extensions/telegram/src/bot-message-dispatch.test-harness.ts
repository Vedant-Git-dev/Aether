...
} from "aether/plugin-sdk/plugin-state-test-runtime";
...
const getAgentScopedMediaLocalRootsHoisted = vi.hoisted(() =>
  vi.fn((_cfg: unknown, agentId: string) => [`/tmp/.aether/workspace-${agentId}`]),
);
...
vi.mock("aether/plugin-sdk/channel-outbound", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aether/plugin-sdk/channel-outbound")>();
  return {
    ...actual,
...
vi.mock("aether/plugin-sdk/channel-inbound", async (importOriginal) => {
  ...
  return {
    ...
    runChannelInboundEvent: async (params: RunParams) => {
      ...
      const delivery =
        resolved.delivery as unknown as import("aether/plugin-sdk/channel-inbound").ChannelInboundTurnPlan<"provider_message_sending">["delivery"];
...
vi.mock("aether/plugin-sdk/session-transcript-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("aether/plugin-sdk/session-transcript-runtime")>();
  return {
    ...actual,
...
export function createContext(overrides?: Partial<TelegramMessageContext>): TelegramMessageContext {
  ...
  base.turn = {
    storePath: "/tmp/aether/telegram-sessions.json",
