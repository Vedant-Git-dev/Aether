...
import { finalizeInboundContext, resetInboundDedupe } from "aether/plugin-sdk/reply-runtime";
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("aether/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("aether/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
const originalStateDir = process.env.AETHER_STATE_DIR;
let mediaHarnessStoreRoot: string | undefined;

function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "aether-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as AetherConfig) as TelegramBotDeps["getRuntimeConfig"];
...
beforeEach(() => {
  ...
  process.env.AETHER_STATE_DIR = ensureMediaHarnessStoreRoot();
