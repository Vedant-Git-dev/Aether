...
import { parseExecApprovalCommandText } from "aether/plugin-sdk/approval-reply-runtime";
import { buildCommandsMessagePaginated } from "aether/plugin-sdk/command-status";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { applySessionModelSelection } from "aether/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "aether/plugin-sdk/models-provider-runtime";
...
async function handleTelegramModelCallback(params: {
  ...
  try {
    ...
    const scopeText = isDefaultSelection
      ...
      : `Session-only model selection. ${runtimeText} The agent default in aether.json is unchanged. This chat keeps the model selection across /new and /reset; use /model default -s to clear the session model selection.`;
