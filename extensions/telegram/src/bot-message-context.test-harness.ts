...
export const baseTelegramMessageContextConfig = {
  agents: { defaults: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/aether" } },
...
type BuildTelegramMessageContextForTestParams = {
  ...
  groupHistories?: Map<string, import("aether/plugin-sdk/reply-history").HistoryEntry[]>;
...
function resolveSessionStorePathForTest(testName: string | undefined): string {
  const hash = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  return `/tmp/aether/session-store-${hash}.json`;
}
