...
import type { TelegramNetworkConfig } from "aether/plugin-sdk/config-contracts";
import {
...
import { resolveRequestUrl } from "aether/plugin-sdk/request-url";
...
// fix for the leak observed in aether#68128 is the transport lifecycle that
...
function resolveAetherProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.AETHER_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveAetherProxyUrlForTelegram() : undefined;
