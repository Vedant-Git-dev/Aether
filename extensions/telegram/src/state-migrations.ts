...
import { isRecord, uniqueStrings } from "aether/plugin-sdk/string-coerce-runtime";
...
function resolveTelegramLegacyStateOwnerAgentId(cfg: AetherConfig): string {
...
function listTelegramLegacyMessageCacheEntries(persistedPath: string) {
  ...
  for (const value of readLegacyMessageCacheValues(raw)) {
    ...
    const { aether_prompt_context_projection: _projection, ...canonicalSourceMessage } =
      sourceMessage as PersistedTelegramMessageCacheValue["sourceMessage"] & {
        aether_prompt_context_projection?: unknown;
      };
