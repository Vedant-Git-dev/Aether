...
import type { ApprovalResolveResult } from "aether/plugin-sdk/approval-gateway-runtime";
import type {
  ExpiredApprovalView,
...
export function buildTelegramCanonicalApprovalTerminalText(params: {
  ...
  if (approval.presentation?.kind === "system-agent" && params.result.applied) {
    if (approval.status === "allowed") {
      return `✅ Aether change approved. Applying: ${truncateDetail(approval.presentation.description)}`;
    }
    if (approval.status === "cancelled") {
      return "⚠️ Aether change was cancelled because its run ended. No change was made. Retry.";
    }
    if (approval.status === "denied") {
      return "❌ Aether change denied. No change was made.";
    }
    if (approval.status === "expired") {
      return "⏱️ Aether change expired. No change was made.";
    }
  }
