...
- Polling and webhook both: enqueue then pump
  ...
  loops.
- Stall timeout: `AETHER_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS` →
...
- Do not reintroduce `sendMessageDraft` for answer streaming. Telegram drafts
  ...
  requires a separate `sendMessage`. Aether uses `sendMessage` plus
...
- Reply context comes from Aether-observed messages. Bot API updates expose
