...
} from "aether/plugin-sdk/cli-backend";
import { isRecord, normalizeOptionalString } from "aether/plugin-sdk/string-coerce-runtime";
...
function throwUnsupportedGeminiCredential(credential: GeminiAuthProfileCredential): never {
  ...
  if (credential.provider === VERCEL_AI_GATEWAY_PROVIDER_ID) {
    throw new Error(
      "Gemini CLI execution cannot use a vercel-ai-gateway auth profile. Use the Aether vercel-ai-gateway provider instead.",
    );
  }
...
function requireGeminiOAuthCredential(
  ...
  if (
    ...
    throw new CliBackendAuthProfilePreparationError(
      `Gemini CLI OAuth profile is incomplete and cannot be repaired by Aether. ${GEMINI_CLI_SUPPORTED_AUTH_GUIDANCE}`,
    );
  }
...
async function buildGeminiCliSystemSettings(
  ...
  if (selectedType) {
    ...
    if (enforcedType && enforcedType !== selectedType) {
      throw new Error(
        `Gemini CLI system settings enforce ${enforcedType} auth, but the selected Aether profile requires ${selectedType}.`,
      );
    }
...
function applyGeminiCliToolAvailability(
  ...
  const exposesAetherTools = availability.aether.length > 0;
  let restrictedMcpServers: Record<string, unknown> = {};
  if (exposesAetherTools) {
    const aetherMcpServer = mcpServers.aether;
    if (!isRecord(aetherMcpServer)) {
      throw new Error("Gemini CLI exact tool availability requires the Aether MCP server.");
    }
    restrictedMcpServers = {
      aether: {
        ...aetherMcpServer,
        includeTools: [...availability.aether],
      },
    };
  }
  ...
  const allowedMcpServers = exposesAetherTools ? ["aether"] : [crypto.randomUUID()];
  const experimental = isRecord(base.experimental) ? { ...base.experimental } : {};
  ...
  return {
    ...
    tools: {
      ...nonAuthorityToolSettings,
      core: exposesAetherTools ? ["mcp_aether_*"] : [],
      discoveryCommand: "",
      callCommand: "",
    },
    mcp: {
...
async function createGeminiCliPrivateTempDir(prefix: string): Promise<string> {
  const directory = await fs.mkdtemp(path.join(resolvePreferredAetherTmpDir(), prefix));
  try {
...
async function prepareGeminiCliProfileHome(
  ...
  const systemSettingsDir = await createGeminiCliPrivateTempDir("aether-gemini-cli-");
...
async function clearGeminiCliCachedCredentials(geminiDir: string): Promise<void> {
  ...
  // from the selected Aether profile each run so stale CLI auth cannot win.
...
async function prepareGeminiCliRestrictedSystemSettings(
  ...
  const systemSettingsDir = await createGeminiCliPrivateTempDir("aether-gemini-cli-policy-");
  const systemSettingsPath = path.join(systemSettingsDir, "settings.json");
  const isolatedSystemPrompt = ctx.isolatedCompletionSystemPrompt;
  const isolatedSystemPromptPath = isolated ? path.join(systemSettingsDir, "system.md") : undefined;
