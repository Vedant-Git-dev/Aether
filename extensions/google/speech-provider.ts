...
import type { sanitizeConfiguredModelProviderRequest } from "aether/plugin-sdk/provider-http";
import type { AetherConfig } from "aether/plugin-sdk/provider-onboard";
import { normalizeResolvedSecretInputString } from "aether/plugin-sdk/secret-input";
import type {
...
import { retryAsync } from "aether/plugin-sdk/speech-provider";
import {
  asOptionalRecord,
...
function resolveGoogleTtsModelProviderApiKey(cfg?: AetherConfig): string | undefined {
...
function isAetherGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
async function synthesizeGoogleTtsPcmOnce(params: {
  ...
  const { assertOkOrThrowProviderError, postJsonRequest, readProviderJsonResponse } =
    await import("aether/plugin-sdk/provider-http");
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isAetherGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
    ...
    synthesize: async (req) => {
      ...
      if (req.target === "voice-note") {
        const { transcodeAudioBufferToOpus } = await import("aether/plugin-sdk/media-runtime");
        return {
