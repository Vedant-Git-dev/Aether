...
export function zoomMeetingStatusPreludeSource(params: MeetingStatusPreludeParams): string {
  ...
  if (canMutateSession && (identityVerifiedBeforeCall || identityPreservedInCall)) {
    ...
    delete window.__aetherZoomMeeting;
  }
  ...
  const ensureVirtualAudioInput = async () => {
    const preparedInput = window.__aetherZoomMeeting;
  ...
  if (committedOwnerConflict && !canMutateSession) {
    ...
    manualAction = manualActionFor("zoom-login-required", tenantLoginRequired ? "This Zoom tenant requires sign-in or email verification. Complete it in the Aether browser profile, then retry." : "Sign in to Zoom in the Aether browser profile, then retry the meeting join.");
  } else if (!inCall && lobbyWaiting) {
    manualAction = manualActionFor("zoom-admission-required", "Admit the Aether guest from the Zoom lobby, then retry speech.");
  } else if (!inCall && permissionRequired) {
    manualAction = manualActionFor("zoom-permission-required", allowMicrophone ? "Allow microphone permission for Zoom in the Aether browser profile, then retry." : "Dismiss the Zoom device-permission prompt or continue without devices, then retry.");
  ...
  if (canMutateSession && identityVerified && autoJoin && !inCall && join && !join.disabled && !manualAction) {
    ...
    platform: {
      ...
      globals: {
        ...
        captionArchive: "__aetherZoomCaptionArchive",
