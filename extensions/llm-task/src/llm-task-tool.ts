...
import { buildModelAliasIndex, resolveModelRefFromString } from "aether/plugin-sdk/agent-runtime";
import {
...
export const llmTaskToolDefinition = {
  ...
  label: "LLM Task",
  description:
    "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via aether.invoke.",
...
export function createLlmTaskTool(api: AetherPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams, signal?: AbortSignal) {
      ...
      let thinkLevel: ReturnType<AetherPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
