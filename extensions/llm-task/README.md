...
`aether.invoke --each`) without adding new Aether code per workflow.
...
default. Run `aether doctor --fix` once for entries created by older releases;
...
This extension depends on Aether internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Aether extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.aether/extensions` as a standalone plugin directory.
