...
> **Aether adaptation:** This copy keeps Slack's authoring guidance while using capabilities available in the current agent session. Native Block Kit JSON is developer output; ordinary Slack replies use Aether's portable `presentation` field.
...
Building the native payload is this skill's job. Return it for the developer's Slack app integration. Do not pass native Slack blocks to Aether's `presentation` field; translate the result into the portable presentation schema when the user instead wants the current Aether agent to post it.
...
- **Scope:** this skill owns building and validating native Block Kit payloads for a developer. Aether conversation delivery uses portable `presentation` instead.
