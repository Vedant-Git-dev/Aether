...
metadata: { "aether": { "emoji": "🧱", "requires": { "config": ["channels.slack"] } } }
...
Follow the adapted guide's live-documentation and `blocks.validate` workflow for native JSON. Do not pass native Slack blocks to Aether's `presentation` field; the plugin owns that conversion.
