...
import { defaultRuntime, logVerbose, warn, type RuntimeEnv } from "aether/plugin-sdk/runtime-env";
  ...
  try {
    ...
    if (
      ...
      (params.runtime ?? defaultRuntime).log?.(
        warn(
          "Slack's Stop button is unavailable until the app subscribes to agent_session_stopped. See https://docs.aether.ai/channels/slack#additional-manifest-settings",
        ),
      );
    }
