...
function readSlackAetherBlockIndex(blockId: string, prefix: string): number | undefined {
...
export function resolveSlackBlockOffsets(blocks?: readonly SlackBlock[]): SlackBlockRenderOptions {
  ...
  for (const block of blocks ?? []) {
    ...
    buttonIndexOffset = Math.max(
      buttonIndexOffset,
      readSlackAetherBlockIndex(blockId, "aether_reply_buttons_") ?? 0,
    );
    selectIndexOffset = Math.max(
      selectIndexOffset,
      readSlackAetherBlockIndex(blockId, "aether_reply_select_") ?? 0,
    );
  }
  return {
    buttonIndexOffset,
...
function buildSlackPresentationButtonBlock(
  ...
  return elements.length > 0
    ? {
        type: "actions",
        block_id: `aether_reply_buttons_${buttonIndex}`,
        elements,
      }
    : undefined;
}
...
function buildSlackPresentationSelectBlock(
  ...
  return options.length > 0 && optionKinds.size === 1
    ? {
        type: "actions",
        block_id: `aether_reply_select_${selectIndex}`,
        elements: [
          {
            type: "static_select",
            action_id:
