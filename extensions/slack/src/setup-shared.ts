...
import { patchChannelConfigForAccount } from "aether/plugin-sdk/setup-runtime";
...
import type { AetherConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "Aether") {
  const safeName = botName.trim() || "Aether";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Aether`,
    },
    features: {
      bot_user: {
      ...
      agent_view: {
        agent_description: `${safeName} connects Slack Agent View conversations to Aether agents.`,
        suggested_prompts: [
          {
      ...
      slash_commands: [
        {
          command: "/aether",
          description: "Send a message to Aether",
          should_escape: false,
        },
      ],
    },
