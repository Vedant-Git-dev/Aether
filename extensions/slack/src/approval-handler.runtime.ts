...
} from "aether/plugin-sdk/approval-handler-runtime";
import { buildChannelApprovalNativeTargetKey } from "aether/plugin-sdk/approval-native-runtime";
import { buildApprovalPresentationFromActionDescriptors } from "aether/plugin-sdk/approval-reply-runtime";
import { formatChannelApprovalResolvedLabel } from "aether/plugin-sdk/approval-runtime";
import type { AetherConfig } from "aether/plugin-sdk/config-contracts";
import { logError } from "aether/plugin-sdk/logging-core";
...
type SlackExecApprovalConfig = NonNullable<
  NonNullable<NonNullable<AetherConfig["channels"]>["slack"]>["execApprovals"]
>;
...
function buildSlackApprovalPayload(input: SlackApprovalRenderInput): SlackPendingDelivery {
  ...
  const approvalName = isPlugin ? "Plugin" : isSystemAgent ? "Aether change" : "Exec";
  let heading: string;
  ...
  if (phase === "pending") {
    ...
    description =
      view.approvalKind === "plugin"
        ...
        : isSystemAgent
          ? "An Aether change needs your approval."
