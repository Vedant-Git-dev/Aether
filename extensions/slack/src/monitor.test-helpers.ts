...
const SLACK_INGRESS_LIFECYCLE_CONTEXT_KEY = "aetherIngressLifecycle";
...
export function resetSlackTestState(config: Record<string, unknown> = defaultSlackTestConfig()) {
  ...
  const stateDir = fs.realpathSync(
    fs.mkdtempSync(path.join(resolvePreferredAetherTmpDir(), "aether-slack-monitor-state-")),
  );
  lastSlackTestStateDir = stateDir;
  process.env.AETHER_STATE_DIR = stateDir;
  setSlackRuntime({
    state: {
...
vi.mock("./monitor/config.runtime.js", async () => {
  ...
  return {
    ...
    resolveStorePath: vi.fn(() => "/tmp/aether-sessions.json"),
