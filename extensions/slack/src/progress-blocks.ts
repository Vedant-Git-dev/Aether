...
const SLACK_ATTENTION_TASK_PREFIX = "aether-attention-";
...
function buildSessionSources(url: string): NonNullable<TaskUpdateChunk["sources"]> {
  ...
  return [{ type: "url_source", url, text: "Open in Aether" }] as unknown as NonNullable<
...
export function buildSlackProgressStreamChunks(params: {
  ...
  if (
    ...
    tasks.push({ id: "aether_attention", title: "Failed", status: "error" });
  }
