...
} from "aether/plugin-sdk/channel-feedback";
import {
...
import { resolveChannelImplicitMentions } from "aether/plugin-sdk/channel-ingress-runtime";
import { resolveChannelMessageSourceReplyDeliveryMode } from "aether/plugin-sdk/channel-outbound";
import { hasControlCommand } from "aether/plugin-sdk/command-detection";
import { isAbortRequestText } from "aether/plugin-sdk/command-primitives-runtime";
import { shouldHandleTextCommands } from "aether/plugin-sdk/command-surface";
import { ensureConfiguredBindingRouteReady } from "aether/plugin-sdk/conversation-runtime";
import { formatErrorMessage } from "aether/plugin-sdk/error-runtime";
import { mimeTypeFromFilePath } from "aether/plugin-sdk/media-mime";
import { createChannelHistoryWindow } from "aether/plugin-sdk/reply-history";
import type { FinalizedMsgContext } from "aether/plugin-sdk/reply-runtime";
import { resolveInboundLastRouteSessionKey } from "aether/plugin-sdk/routing";
import { logVerbose, shouldLogVerbose } from "aether/plugin-sdk/runtime-env";
...
import { enqueueRoutedSystemEvent } from "aether/plugin-sdk/system-event-runtime";
...
const SLACK_CHANNEL_ACCESS_DOCS_URL =
  "https://docs.aether.ai/channels/slack#access-control-and-routing";
    ...
    if (
      ...
      try {
        await (params.eventScope?.client ?? ctx.app.client).chat.postEphemeral({
          ...
          user: message.user,
          text: `${subject} can’t reply here because this channel isn’t in its Aether channel allowlist. Ask the Aether owner to allow this channel. <${SLACK_CHANNEL_ACCESS_DOCS_URL}|Learn how to configure Slack channel access.>`,
        });
...
export async function prepareSlackMessage(params: {
  ...
  const resolveMessageIngress = async (
    contextBinding?: import("aether/plugin-sdk/channel-ingress-runtime").ChannelIngressContextBinding,
    threadId?: string,
  ) =>
