...
import { resolveAgentIdFromSessionKey } from "aether/plugin-sdk/routing";
...
const AETHER_MODAL_CALLBACK_PREFIX = "aether:";
...
function resolveSlackModalPluginInteractiveData(params: {
  ...
  if (!params.callbackId.startsWith(AETHER_MODAL_CALLBACK_PREFIX)) {
    return undefined;
  }
  const callbackData = params.callbackId.slice(AETHER_MODAL_CALLBACK_PREFIX.length).trim();
...
function shouldHandleSlackModalLifecycleBody(body: unknown): boolean {
  ...
  if (callbackId.startsWith(AETHER_MODAL_CALLBACK_PREFIX)) {
    return true;
  }
