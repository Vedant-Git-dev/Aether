...
function buildSlackHomeView(slashCommandName?: string): HomeView {
  const startSessionText = slashCommandName
    ? `Send a DM, mention Aether in a channel, or use \`/${slashCommandName}\` to start a session.`
    : "Send a DM or mention Aether in a channel to start a session.";
  return {
    type: "home",
    callback_id: "aether:home",
    blocks: [
      {
        type: "header",
        text: {
