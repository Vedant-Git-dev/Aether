...
import { toErrorObject } from "aether/plugin-sdk/error-runtime";
import { channelBlockedPatch, channelReadyPatch } from "aether/plugin-sdk/gateway-runtime";
import {
...
const AETHER_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const AETHER_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const AETHER_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__aetherNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, AETHER_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, AETHER_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : AETHER_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        const reconnectTimer = setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, AETHER_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
...
function shouldSkipAetherSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout: AETHER_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping Aether's fatal unhandled-rejection path.
  ...
  app.use(async (args) => {
    ...
    if (shouldSkipAetherSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
