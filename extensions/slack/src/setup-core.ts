...
} from "aether/plugin-sdk/channel-setup";
import { normalizeSecretInputString } from "aether/plugin-sdk/secret-input";
...
import {
  ...
  type AetherConfig,
} from "aether/plugin-sdk/setup-runtime";
import { formatDocsLink } from "aether/plugin-sdk/setup-tools";
...
function enableSlackAccount(cfg: AetherConfig, accountId: string): AetherConfig {
  ...
  return {
    ...
    prepare: async ({ cfg, accountId, prompter }) => {
      ...
      const identity = await prompter.select<"bot" | "user">({
        message: "How should Aether appear in Slack?",
        options: [
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: AetherConfig; accountId: string }) =>
        inspectSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: AetherConfig; accountId: string }) =>
        Object.entries(inspectSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: AetherConfig; accountId: string }) =>
    ...
    disable: (cfg: AetherConfig) => setSetupChannelEnabled(cfg, channel, false),
