// Google Meet plugin entrypoint registers its Aether integration.
import type { GatewayRequestHandlerOptions } from "aether/plugin-sdk/gateway-runtime";
...
export default definePluginEntry({
  ...
  register(api: AetherPluginApi) {
    ...
    registerGatewayMethod("googlemeet.testListen", async ({ params, client, respond }) => {
      const trustedParams = keepTrustedToolAgentId(asParamRecord(params), client);
      const runtime = await ensureRuntime();
      const { readPositiveIntegerParam } = await import("aether/plugin-sdk/param-readers");
      respond(
        true,
        ...
        async execute(_toolCallId, params) {
          ...
          try {
            const { normalizeAgentId, parseAgentSessionKey } =
              await import("aether/plugin-sdk/routing");
