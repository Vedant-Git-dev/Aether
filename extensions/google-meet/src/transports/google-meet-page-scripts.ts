...
export function meetStatusScript(params: {
  ...
  const audioDeviceFamily = (value) => {
    ...
    if (/\\bAether Meeting Audio\\b/i.test(label)) return 'aether-meeting-audio';
  ...
  const routeMeetAudioInput = async () => {
    ...
    try {
      ...
      audioInputDeviceLabel = input.label || 'Aether meeting audio';
  ...
  const routeMeetAudioOutput = async () => {
    ...
    try {
      ...
      audioOutputDeviceLabel = output.label || "Aether meeting audio";
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__aetherMeetCaptions) return undefined;
    // A reused tab starts a fresh logical transcript for each Aether session.
    // Status refreshes omit the id, so they preserve the active page-owned buffer.
    if (!w.__aetherMeetCaptions || (captionSessionId && w.__aetherMeetCaptions.sessionId !== captionSessionId)) {
      if (w.__aetherMeetCaptions?.settleTimer !== undefined) {
        clearTimeout(w.__aetherMeetCaptions.settleTimer);
      }
      w.__aetherMeetCaptions?.observer?.disconnect?.();
      w.__aetherMeetCaptions = {
    ...
    if (rows.length === 0) {
      ...
      if (captionState.visible.length > 0 && captionState.settleTimer === undefined) {
        ...
        pendingState.settleTimer = setTimeout(() => {
          if (window.__aetherMeetCaptions !== pendingState) return;
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    manualAction = manualActionFor("google-login-required", "Sign in to Google in the Aether browser profile, then retry the Meet join.");
    ...
    manualAction = manualActionFor("meet-admission-required", "Admit the Aether browser participant in Google Meet, then retry speech.");
  } else if (permissionNeeded) {
    manualAction = manualActionFor("meet-permission-required", allowMicrophone ? "Allow microphone/camera/speaker permissions for Meet in the Aether browser profile, then retry." : "Join without microphone/camera permissions in the Aether browser profile, then retry.");
    ...
    manualAction = manualActionFor(
      ...
      "Select BlackHole 2ch or Aether Meeting Audio as both the Meet microphone and speaker, then retry."
    );
    ...
    manualAction = manualActionFor("meet-audio-choice-required", allowMicrophone ? "Meet is showing the microphone choice. Click Use microphone in the Aether browser profile, then retry." : "Meet is showing the microphone choice. Choose the no-microphone option in the Aether browser profile, then retry.");
  }
...
export function meetTranscriptScript(
  ...
  const state = window.__aetherMeetCaptions;
