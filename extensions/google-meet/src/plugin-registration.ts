...
const loadGoogleMeetGatewayRuntimeModule = createLazyRuntimeModule(
  () => import("aether/plugin-sdk/gateway-runtime"),
);
...
) => Promise<AetherPluginNodeInvokePolicy>;
...
export function createLazyGoogleMeetNodeInvokePolicy(
  ...
  let policyPromise: Promise<AetherPluginNodeInvokePolicy> | undefined;
  return {
    commands: [GOOGLE_MEET_NODE_COMMAND],
    dangerous: true,
    async handle(ctx) {
      let policy: AetherPluginNodeInvokePolicy;
      try {
