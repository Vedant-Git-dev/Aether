...
} from "aether/plugin-sdk/meeting-runtime";
import { isBlockedHostnameOrIp } from "aether/plugin-sdk/ssrf-runtime";
import { asRecord, normalizeOptionalString } from "aether/plugin-sdk/string-coerce-runtime";
...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Aether browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Aether browser profile; configure browser.defaultProfile to choose another profile",
  });
