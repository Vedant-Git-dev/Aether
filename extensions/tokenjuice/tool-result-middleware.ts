...
} from "aether/plugin-sdk/agent-harness";
import { isRecord } from "aether/plugin-sdk/string-coerce-runtime";
import { createTokenjuiceAetherEmbeddedExtension } from "./runtime-api.js";

type TokenjuiceToolResultHandler = (
  event: {
    toolName: string;
    input: Record<string, unknown>;
    content: AetherAgentToolResult["content"];
    details: unknown;
    isError?: boolean;
  },
  ctx: { cwd: string },
) => Promise<Partial<AetherAgentToolResult> | void> | Partial<AetherAgentToolResult> | void;

function normalizeDetails(
  event: AgentToolResultMiddlewareEvent,
  current: AetherAgentToolResult,
): unknown {
  if (
...
export function createTokenjuiceAgentToolResultMiddleware(): AgentToolResultMiddleware {
  ...
  createTokenjuiceAetherEmbeddedExtension()({
    on(event, handler) {
