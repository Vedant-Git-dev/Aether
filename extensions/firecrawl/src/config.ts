...
import { resolvePositiveTimeoutSeconds } from "aether/plugin-sdk/provider-web-fetch";
...
function resolveFirecrawlSearchConfig(cfg?: AetherConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: AetherConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
function resolveConfiguredSecret(value: unknown, path: string, cfg?: AetherConfig) {
...
export function resolveFirecrawlApiKey(cfg?: AetherConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: AetherConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
...
export function resolveFirecrawlOnlyMainContent(cfg?: AetherConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: AetherConfig, override?: number): number {
