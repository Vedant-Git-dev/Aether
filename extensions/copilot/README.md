# GitHub Copilot agent runtime (Aether plugin)

External Aether plugin that registers a `copilot` agent harness backed by `@github/copilot-sdk` and the GitHub Copilot CLI.

## Install

```bash
aether plugins install @aether/copilot
```
...
entry; `auto` never picks it. Aether remains the default embedded runtime.
