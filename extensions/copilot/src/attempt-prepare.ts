...
import type { SandboxContext } from "aether/plugin-sdk/agent-harness-runtime";
import {
...
export function prepareCopilotAttemptContext(
  ...
  const hostSystemAgentActive =
    deps.isHostScopedToolActive?.("aether") ?? isHostScopedAgentToolActive("aether");
  const ringZeroSystemAgentRun =
    hostSystemAgentActive &&
    input.toolsAllow?.length === 1 &&
    input.toolsAllow[0]?.trim().toLowerCase() === "aether";
