...
import { isBlockedHostnameOrIp } from "aether/plugin-sdk/ssrf-runtime";
import {
  filterStringRecord,
...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support Aether provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by Aether SSRF policy";
...
/**
 * Maps Aether's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; Aether only supplies
