...
import { readNonBlankString } from "aether/plugin-sdk/string-coerce-runtime";
...
// Filtering them out of the Aether bootstrap injection avoids
...
/**
 * Loads Aether workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .aether conventions live) onto the
...
function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "Aether loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
