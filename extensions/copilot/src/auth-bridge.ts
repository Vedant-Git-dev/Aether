...
import { readNonEmptyStringPreservingWhitespace as readNonEmptyString } from "aether/plugin-sdk/string-coerce-runtime";
 ...
 *     use cases.
 *   - Falls back to AETHER_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<aetherHome>/.aether/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `AETHER_HOME` for the home directory root.
 ...
 *   4. AETHER_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with Aether's own files
  ...
  const aetherHome = readNonEmptyString(args.env.AETHER_HOME);
  const rootHome = aetherHome ? resolve(aetherHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <aetherHome>/.aether/agents/<agentId>/copilot
  return resolve(join(rootHome, ".aether", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // AETHER_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "AETHER_GITHUB_TOKEN", value: readNonEmptyString(env.AETHER_GITHUB_TOKEN) },
