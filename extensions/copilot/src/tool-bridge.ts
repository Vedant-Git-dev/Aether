...
import { createAgentHarnessToolSurfaceRuntime } from "aether/plugin-sdk/agent-harness-tool-runtime";
import { toStringifiedError as toCopilotToolError } from "aether/plugin-sdk/error-runtime";
import { isRawCopilotModelRun } from "./attempt-mode.js";

type CreateAetherCodingTools =
  (typeof import("aether/plugin-sdk/agent-harness"))["createAetherCodingTools"];
type AetherCodingToolsOptions = NonNullable<Parameters<CreateAetherCodingTools>[0]>;
type CreateAetherCodingToolsForBridge = (
  options?: AetherCodingToolsOptions,
) => ReturnType<CreateAetherCodingTools> | Promise<ReturnType<CreateAetherCodingTools>>;
 ...
 * authoritative `createAetherCodingTools({...})` call shape).
 *
 * Declared from `EmbeddedRunAttemptParamsV2` (imported from the
 * `aether/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createAetherCodingTools` so the
  ...
  createAetherCodingTools?: CreateAetherCodingToolsForBridge;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createAetherCodingTools =
    input.createAetherCodingTools ??
    (await import("aether/plugin-sdk/agent-harness")).createAetherCodingTools;
  ...
  const toolOptions = buildAetherCodingToolsOptions(
    input,
    {
      ...toolPlan,
  ...
  try {
    const constructedTools = await createAetherCodingTools(toolOptions);
    if (!Array.isArray(constructedTools)) {
      throw new Error("createAetherCodingTools must return an array of tools");
    }
    ...
    throw createError(
      `[copilot-tool-bridge] createAetherCodingTools failed: ${toCopilotToolError(error).message}`,
      error,
    );
  }
  ...
  const sdkTools = exposedTools.map((sourceTool) =>
    convertAetherToolToSdkTool(sourceTool, input, scheduleToolExecution),
  );
  return {
 ...
 */
function buildAetherCodingToolsOptions(
  input: CopilotToolBridgeInput,
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as AetherCodingToolsOptions["modelCompat"])
      : undefined;
...
function convertAetherToolToSdkTool(
  sourceTool: AnyAgentTool,
  input: CopilotToolBridgeInput,
  ...
  return {
    ...
    // on Aether's host-bound tools instead of rejecting registration.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Host-bound tools enforce Aether policy and approvals; an SDK custom-tool
