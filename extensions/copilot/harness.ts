...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps Aether session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  async function runHarnessAttempt(
    ...
    const attemptPromise = (async () => {
      ...
      const aetherSessionId = typeof params.sessionId === "string" ? params.sessionId : undefined;

      // Reuse the SDK session across turns within the same Aether session so
      ...
      const compactionCleanupPending =
        aetherSessionId !== undefined && hasPendingDeferredCompactionCleanup(aetherSessionId);
      const replayBlocked =
        aetherSessionId !== undefined &&
        (compactionCleanupPending || resetBlockedStoredSessions.has(aetherSessionId));
      const tracked =
        aetherSessionId && !replayBlocked ? trackedSessions.get(aetherSessionId) : undefined;
      const stored = aetherSessionId
        ? replayBlocked
          ? undefined
          : lookupStoredBinding(options?.sessionStore, aetherSessionId)
        : undefined;
      ...
      const result = await runCopilotAttempt(effectiveParams, {
        ...
        onSessionEstablished:
          operation === "attempt" && aetherSessionId
            ? ({
                ...
                trackedSessions.set(aetherSessionId, {
                  sdkSessionId,
                ...
                registerStoredBinding(options?.sessionStore, aetherSessionId, {
                  schemaVersion: 2,
                  sdkSessionId,
                  compatKey: currentCompatKey,
                  compactKey: currentCompactKey,
                ...
                resetBlockedStoredSessions.delete(aetherSessionId);
              }
            : undefined,
        onDeferredCompaction: aetherSessionId
          ? ({
              abort,
              cleanup,
              sdkSessionId,
              ...
              const trackedBinding = trackedSessions.get(aetherSessionId);
              const storedBinding = lookupStoredBinding(options?.sessionStore, aetherSessionId);
              ...
              resetBlockedStoredSessions.add(aetherSessionId);
              void cleanup.then((outcome) => {
                const currentTracked = trackedSessions.get(aetherSessionId);
                const currentStored = lookupStoredBinding(options?.sessionStore, aetherSessionId);
                ...
                if (stillOwnsTrackedSession) {
                  trackedSessions.delete(aetherSessionId);
                }
                if (stillOwnsStoredSession) {
                  deleteStoredBinding(options?.sessionStore, aetherSessionId);
                }
                if (stillOwnsTrackedSession || stillOwnsStoredSession) {
                  resetBlockedStoredSessions.add(aetherSessionId);
                }
      ...
      if (operation === "attempt" && aetherSessionId) {
        ...
        if (sdkSessionId && trackedSession?.sdkSessionId === sdkSessionId) {
          ...
          trackedSessions.set(aetherSessionId, nextTracked);
          registerStoredBinding(options?.sessionStore, aetherSessionId, {
            schemaVersion: 2,
            ...(attemptResult.journalValidated ? { journalVersion: 1 } : {}),
            sdkSessionId,
  ...
  return {
    ...
    async reset(params: AgentHarnessResetParams): Promise<void> {
      ...
      const tracked = trackedSessions.get(aetherSessionId);
      const stored = lookupStoredBinding(options?.sessionStore, aetherSessionId);
      resetBlockedStoredSessions.add(aetherSessionId);
      await abortDeferredCompactionCleanups(aetherSessionId);
      const currentStored = lookupStoredBinding(options?.sessionStore, aetherSessionId);
      const stillOwnsStoredSession =
        stored !== undefined && currentStored?.sdkSessionId === stored.sdkSessionId;
      if (stillOwnsStoredSession) {
        if (deleteStoredBinding(options?.sessionStore, aetherSessionId)) {
          resetBlockedStoredSessions.delete(aetherSessionId);
        }
      } else {
        resetBlockedStoredSessions.delete(aetherSessionId);
      ...
      if (trackedSessions.get(aetherSessionId)?.sdkSessionId === tracked.sdkSessionId) {
        trackedSessions.delete(aetherSessionId);
      }
      try {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. Aether only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // Aether session/transcript state stays in SQLite, with no marker
      ...
      if (hasPendingDeferredCompactionCleanup(aetherSessionId)) {
        return {
          ok: false,
          compacted: false,
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with Aether hooks/tools, so preserve the
