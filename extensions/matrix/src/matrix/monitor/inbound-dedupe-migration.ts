...
import type { PersistentDedupeEntry } from "aether/plugin-sdk/persistent-dedupe";
import type { PluginDoctorStateMigrationContext } from "aether/plugin-sdk/runtime-doctor-migrations";
...
const STATE_DATABASE_RELATIVE_PATH = path.join("state", "aether.sqlite");
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile()) {
        if (isStorageRoot && entry.name === MATRIX_LEGACY_INBOUND_DEDUPE_FILENAME) {
          jsonRoots.add(dir);
        } else if (depth === 5 && entry.name === "aether.sqlite") {
...
export async function readLegacyInboundDedupeSqliteSource(
  ...
  const { openNodeSqliteDatabase } = await import("aether/plugin-sdk/sqlite-runtime");
...
export async function importNewestInboundDedupeMarkers(params: {
  ...
  const { createPersistentDedupeImportEntry } =
    await import("aether/plugin-sdk/persistent-dedupe");
