...
import type { RuntimeLogger } from "aether/plugin-sdk/plugin-runtime";
...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleAetherDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Aether devices detected for ${params.auth.userId}: ${deviceHealth.staleAetherDevices.map((device) => device.deviceId).join(", ")}. Run 'aether matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
  ...
  try {
    ...
    if (startupVerification.kind === "verified") {
      ...
      params.logger.info(
        "matrix: device not verified — run 'aether matrix verify device <key>' to enable E2EE",
      );
