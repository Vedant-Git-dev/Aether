...
import type { PluginRuntime, RuntimeLogger } from "aether/plugin-sdk/plugin-runtime";
...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'aether matrix verify status --verbose --account ${accountId}' and 'aether matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Aether runtime is using the same account. " +
    `Check 'aether matrix verify status --verbose --account ${accountId}' and 'aether matrix devices list --account ${accountId}'.`
  );
}
