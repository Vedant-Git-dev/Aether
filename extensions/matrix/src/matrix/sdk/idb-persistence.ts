...
// `aether matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.AETHER_STATE_DIR || path.join(process.env.HOME || "/tmp", ".aether");
