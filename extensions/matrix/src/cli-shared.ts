...
import { parseStrictInteger } from "aether/plugin-sdk/number-runtime";
...
import { formatZonedTimestamp } from "aether/plugin-sdk/time-runtime";
import {
...
export function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["aether", "matrix", ...parts];
...
export function printMatrixVerificationSummary(summary: MatrixCliVerificationSummary): void {
  ...
  console.log(`Initiated by Aether: ${summary.initiatedByMe ? "yes" : "no"}`);
...
function buildVerificationGuidance(
  ...
  if (status.serverDeviceKnown === false) {
    nextSteps.add(
      `This Matrix device is no longer listed on the homeserver. Create a new Aether Matrix device with ${formatMatrixCliCommand("account add --homeserver <url> --user-id <@user:server> --password <password> --device-name Aether-Gateway", accountId)}. If you use token auth, create a fresh Matrix access token in your Matrix client or admin UI, then run ${formatMatrixCliCommand("account add --homeserver <url> --access-token <token>", accountId)}.`,
    );
  }
