...
} from "aether/plugin-sdk/approval-reply-runtime";
import {
...
import { retryAsync } from "aether/plugin-sdk/retry-runtime";
import { normalizeAccountId } from "aether/plugin-sdk/routing";
import { normalizeUniqueStringEntries } from "aether/plugin-sdk/string-coerce-runtime";
import {
...
// Aether Matrix custom event content for capable clients; body and reactions remain fallback.
const MATRIX_APPROVAL_METADATA_KEY = "com.aether.approval" as const;
...
function buildPendingApprovalContent(params: {
  ...
  if (params.view.approvalKind === "plugin") {
    ...
    payload = buildApprovalPendingReplyPayload({
      ...
      text: `Aether change requires approval:\n${params.view.operationSummary}`,
...
function buildResolvedApprovalText(view: ResolvedApprovalView): string {
  ...
  return [
    `${view.approvalKind === "system-agent" ? "Aether change" : "Exec approval"}: ${decisionLabel}`,
    "",
