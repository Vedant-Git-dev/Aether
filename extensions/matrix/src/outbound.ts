...
import { chunkTextForOutbound } from "aether/plugin-sdk/text-chunking";
import { sendMessageMatrix, sendPollMatrix } from "./matrix/send.js";
import type { MatrixExtraContentFields } from "./matrix/send/types.js";

const MATRIX_AETHER_PRESENTATION_KEY = "com.aether.presentation" as const;
const MATRIX_AETHER_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_AETHER_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = asOptionalRecord(extraContent?.[MATRIX_AETHER_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_AETHER_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_AETHER_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
export function resolveMatrixExtraContent(
  ...
  return presentation ? { [MATRIX_AETHER_PRESENTATION_KEY]: presentation } : undefined;
}
