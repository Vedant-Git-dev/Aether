...
import { isAetherManagedMatrixDevice } from "./matrix/device-health.js";
  ...
  try {
    ...
    try {
      ...
      deviceHealth = {
        ...
        staleAetherDeviceIds: addedDevices
          .filter((device) => !device.current && isAetherManagedMatrixDevice(device.displayName))
          .map((device) => device.deviceId),
      };
    } catch (err) {
      deviceHealth = {
        currentDeviceId: null,
        staleAetherDeviceIds: [],
        error: formatErrorMessage(err),
      };
    }
      ...
      async (options: {
        ...
        await cli.runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleAetherDeviceIds.length > 0) {
              const staleDeviceIds = result.deviceHealth.staleAetherDeviceIds
                .map((deviceId) => cli.formatMatrixCliText(deviceId))
                .join(", ");
              console.log(
                `Matrix device hygiene warning: stale Aether devices detected (${staleDeviceIds}). Run ${cli.formatMatrixCliCommand("devices prune-stale", result.accountId)}.`,
              );
            }
            ...
            const bindHint = `aether agents bind --agent <id> --bind matrix:${result.accountId}`;
