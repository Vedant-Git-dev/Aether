...
function resolveKimiBaseUrl(kimi?: KimiConfig, aetherConfig?: AetherConfig): string {
  ...
  const moonshotBaseUrl = aetherConfig?.models?.providers?.moonshot?.baseUrl;
...
export async function executeKimiWebSearchProviderTool(
  ctx: { config?: AetherConfig; searchConfig?: SearchConfigRecord },
  ...
  if (!result.grounded) {
    return {
      ...
      provider: "kimi",
      model,
      docs: "https://docs.aether.ai/tools/kimi-search",
