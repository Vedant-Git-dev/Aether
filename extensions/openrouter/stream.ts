...
import { asNonArrayRecord, readStringValue } from "aether/plugin-sdk/string-coerce-runtime";
...
function mergeOpenRouterAuthHeaders(options: Parameters<StreamFn>[2]): Parameters<StreamFn>[2] {
  ...
  if (!headers.has("http-referer")) {
    headers.set("HTTP-Referer", "https://aether.ai");
  }
  if (!headers.has("x-openrouter-title")) {
    headers.set("X-OpenRouter-Title", "Aether");
  }
  return {
    ...options,
