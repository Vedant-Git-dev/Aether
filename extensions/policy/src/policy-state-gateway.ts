...
export function scanPolicyGatewayExposure(
  ...
  entries.push({
    ...
    kind: "bind",
    source: "oc://aether.config/gateway/bind",
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://aether.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://aether.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://aether.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://aether.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "controlUi",
    false,
    "oc://aether.invariant/gateway/controlUi/deviceIdentity",
  );
  pushGatewayBooleanEvidence(
    entries,
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://aether.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://aether.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://aether.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://aether.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://aether.config/gateway/http/endpoints/${endpoint}`;
...
function pushGatewayNodeCommandEvidence(
  ...
  if (Array.isArray(denyCommands)) {
    denyCommands.forEach((command, index) => {
      ...
      entries.push({
        ...
        source: `oc://aether.config/gateway/nodes/commands/deny/#${index}`,
        value: normalized,
        command: normalized,
      });
    });
  ...
  allowCommands.forEach((command, index) => {
    ...
    entries.push({
      ...
      kind: "nodeCommand",
      source: `oc://aether.config/gateway/nodes/commands/allow/#${index}`,
      value: normalized,
      command: normalized,
    });
  });
