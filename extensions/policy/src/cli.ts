...
import { listAgentIds, resolveDefaultAgentId } from "aether/plugin-sdk/agent-scope-runtime";
import {
...
import { defaultRuntime as cliRuntime } from "aether/plugin-sdk/runtime";
...
function resolvePolicyCommandAgentId(
  ...
  if (requestedAgentId) {
    ...
    if (!listAgentIds(cfg).includes(agentId)) {
      throw new Error(
        `Unknown agent id "${requestedAgentId}". Run ${formatCliCommand("aether agents list")} to see configured agents.`,
      );
    }
    return agentId;
  }
