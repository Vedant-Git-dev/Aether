import type { HealthCheckContext, HealthFinding } from "aether/plugin-sdk/health";
...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://aether.config/plugins/entries/policy/config/expectedHash",
  ...
  if (requiredMetadata.size > 0) {
    ...
    if (legacyToolsFile !== null) {
      unmigratedToolsFinding = {
        ...
        severity: "error",
        message:
          "TOOLS.md contains unmigrated governed tool declarations; run `aether doctor --fix` to migrate them into the AGENTS.md `## Tools` section before policy evaluation can pass.",
        source: "policy",
        path: "TOOLS.md",
        target: "oc://TOOLS.md/tools",
        requirement: `oc://${policyFile.ocDocName}/tools/requireMetadata`,
        fixHint:
          "Run `aether doctor --fix` to migrate TOOLS.md into the AGENTS.md `## Tools` section.",
      };
    }
  ...
  return [
    {
      ...
      requirement: "oc://aether.config/plugins/entries/policy/config/expectedAttestationHash",
