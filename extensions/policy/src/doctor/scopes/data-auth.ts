...
export function createPolicyDataAuthChecks(deps: PolicyDoctorCheckDeps): readonly HealthCheck[] {
  return createPolicyScopedChecks(deps, [
    ...
    [
      ...
      "Aether config SecretRefs use configured secret providers when policy requires managed providers.",
    ],
    [
      CHECK_IDS.policySecretsDeniedProviderSource,
      "Aether config secret providers and SecretRefs do not use sources denied by policy.",
    ],
    [
    ...
    [
      ...
      "Aether config auth profiles declare required provider and mode metadata.",
    ],
    [
      CHECK_IDS.policyAuthProfileUnapprovedMode,
      "Aether config auth profile modes stay within the policy allowlist.",
    ],
  ]);
}
