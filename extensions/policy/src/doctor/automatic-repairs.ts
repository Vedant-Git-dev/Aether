...
import { isRecord } from "aether/plugin-sdk/channel-secret-basic-runtime";
import type {
  HealthFinding,
  HealthRepairContext,
  HealthRepairResult,
...
function mergeRequiredDenyTools(
  ...
  for (const finding of findings) {
    ...
    if (
      ...
      finding.ocPath === "oc://aether.config/tools/deny"
    ) {
      warnings.push(
  ...
  return changes.length > 0
    ? { config: next as AetherConfig, changes: uniqueStrings(changes), warnings }
...
function disableElevatedTools(
  ...
  if (
    !findings.some((finding) => finding.ocPath === "oc://aether.config/tools/elevated/enabled")
  ) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
...
function disableInsecureControlUi(
  ...
  const fields = [
    [
      ...
      "oc://aether.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
    ],
    [
      "dangerouslyAllowHostHeaderOriginFallback",
      "oc://aether.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
    ],
  ] as const;
  ...
  return changes.length > 0
    ? { config: next as AetherConfig, changes }
    : { config: cfg, changes };
}
...
function disableRemoteGatewayMode(
  ...
  if (!findings.some((finding) => finding.ocPath === "oc://aether.config/gateway/mode")) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  const gateway = ensureRecord(next, "gateway");
...
function disableTelemetryContentCapture(cfg: AetherConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function cloneConfig(cfg: AetherConfig): ConfigRecord {
...
function skippedUnsafeScopedRepair(cfg: AetherConfig, warning: string): RepairPatch {
...
function isScopedInheritedChannelDefaultFinding(finding: HealthFinding): boolean {
  return (
    hasScopedPolicyRequirement([finding]) &&
    finding.ocPath?.startsWith("oc://aether.config/channels/defaults/") === true
  );
}
