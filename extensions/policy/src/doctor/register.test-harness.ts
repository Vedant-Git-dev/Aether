...
import {
  ...
  type AetherConfig,
} from "aether/plugin-sdk/health";
import { clearHealthChecksForTest } from "aether/plugin-sdk/plugin-test-runtime";
import { registerPolicyDoctorChecks } from "./register.js";

export let workspaceDir: string;

let originalAetherHome: string | undefined;

let originalAetherStateDir: string | undefined;

export function cfgWithPolicy(settings: Record<string, unknown> = {}): AetherConfig {
  return {
    plugins: {
      entries: {
        policy: {
...
type PolicyConfigFixture = AetherConfig & Record<string, unknown>;
...
export async function writePolicyFixture(
  ...
  const [policy] = json;
  const configPath = join(workspaceDir, "aether.jsonc");
...
export function ctx(configPath: string, cfg: AetherConfig = {}): HealthCheckContext {
  return {
    mode: "lint",
    runtime: {
      log() {},
...
export function repairCtx(configPath: string, cfg: AetherConfig = {}): HealthRepairContext {
  return {
    ...ctx(configPath, cfg),
    mode: "fix",
  };
...
export async function runPolicyChecksFixture(
  policy: unknown,
  cfg: AetherConfig = cfgWithPolicy(),
) {
...
export const setupPolicyDoctorTest = async () => {
  ...
  originalAetherHome = process.env.AETHER_HOME;
  originalAetherStateDir = process.env.AETHER_STATE_DIR;
  workspaceDir = await fs.mkdtemp(join(tmpdir(), "policy-doctor-"));
  process.env.AETHER_HOME = workspaceDir;
  delete process.env.AETHER_STATE_DIR;
  await fs.mkdir(join(workspaceDir, ".aether"), { recursive: true });
  try {
    await fs.symlink(
      "../exec-approvals.json",
      join(workspaceDir, ".aether", "exec-approvals.json"),
    );
  } catch (err) {
    if (typeof err !== "object" || err === null || !("code" in err) || err.code !== "EPERM") {
      throw err;
    }
    await fs.rm(join(workspaceDir, ".aether"), { recursive: true, force: true });
    await fs.symlink(workspaceDir, join(workspaceDir, ".aether"), "junction");
  }
};

export const teardownPolicyDoctorTest = async () => {
  if (originalAetherHome === undefined) {
    delete process.env.AETHER_HOME;
  } else {
    process.env.AETHER_HOME = originalAetherHome;
  }
  if (originalAetherStateDir === undefined) {
    delete process.env.AETHER_STATE_DIR;
  } else {
    process.env.AETHER_STATE_DIR = originalAetherStateDir;
  }
