...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://aether.config/gateway/bind" &&
    finding.ocPath !== "oc://aether.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://aether.config/gateway/nodes/commands/deny"
  ) {
    return [];
  }
  return [
