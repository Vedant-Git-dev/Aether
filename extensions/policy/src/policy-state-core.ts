...
import { normalizeProviderId } from "aether/plugin-sdk/provider-model-shared";
import { asNonArrayRecord, isRecord } from "aether/plugin-sdk/string-coerce-runtime";
import {
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://aether.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://aether.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://aether.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://aether.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://aether.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://aether.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://aether.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://aether.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://aether.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://aether.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://aether.config/agents/defaults/models",
    );
  }
