...
import { coerceSecretRef } from "aether/plugin-sdk/secret-input";
import {
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://aether.config/auth/profiles/${ocPathSegment(id)}`,
...
export function scanPolicyDataHandling(
  ...
  // `aether policy check` evidence and the attestation, since no doctor check can fail.
  entries.push({
    id: "logging-redaction",
    kind: "sensitiveLoggingRedaction",
    source: "oc://aether.invariant/logging/redaction",
    scope: "global",
    value: true,
    explicit: true,
  });
  ...
  entries.push({
    ...
    source: "oc://aether.config/diagnostics/otel/captureContent",
    scope: "global",
  ...
  entries.push({
    ...
    source: "oc://aether.config/session/maintenance/mode",
    scope: "global",
    value: retentionMode,
    ...
    entries.push({
      ...
      source:
        ...
        readBoolean(defaultExperimental.sessionMemory) !== undefined
          ? "oc://aether.config/memory/search/experimental/sessionMemory"
          : "oc://aether.config/memory/search/rememberAcrossConversations",
      scope: "global",
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://aether.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://aether.config/${path.map(ocPathSegment).join("/")}`;
}
