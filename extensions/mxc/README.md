...
Official MXC sandbox execution plugin for Aether.

This plugin lets Aether run tool execution through MXC on Windows hosts with
ProcessContainer support.

## Install

```bash
aether plugins install @aether/mxc-sandbox
```
...
- Plugin id: `mxc`
- Package: `@aether/mxc-sandbox`
- Minimum Aether host: `2026.6.11`
...
Any other key is rejected. `aether.plugin.json` publishes the same schema
(enums, `minimum`/`maximum` bounds) so `aether config` validation and CLI
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    Aether skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the Aether workspace directory
    passed to MXC.
...
- Aether passes per-run command, environment, and filesystem config to the
...
Use an already configured Aether installation with MXC installed and enabled
...
[`aether config patch --stdin`](https://docs.aether.ai/cli/config#config-patch)
...
```json
{
  "filesystem": {
    ...
    "additionalReadonlyPaths": ["C:\\Tools\\Aether\\shared-readonly"],
    "additionalReadwritePaths": ["D:\\AetherScratch"]
  },
  "process": {
    "timeoutSeconds": 120
  }
...
When multiple configured policy files exist, Aether layers them
...
The filesystem bridge keeps protected Aether skill overlays read-only. For
...
```powershell
aether tui --session "agent:${mxcAgent}:main"
```

For local embedded testing without a Gateway:

```powershell
aether tui --local --session "agent:${mxcAgent}:main"
```
...
aether agents delete $mxcAgent --force
