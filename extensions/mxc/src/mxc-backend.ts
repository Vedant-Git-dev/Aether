...
import { resolvePreferredAetherTmpDir } from "aether/plugin-sdk/sandbox";
import type {
...
function createLauncherPayloadFile(
  ...
  const payloadDir = mkdtempSync(
    path.join(resolvePreferredAetherTmpDir(), "aether-mxc-payload-"),
  );
...
function createSandboxTempDir(hostEnv: BaselineHostEnv): string {
  return mkdtempSync(path.join(resolveSandboxTempDir(hostEnv), "aether-mxc-sandbox-"));
}
...
/** Manager for `aether sandbox list` and `aether sandbox remove`. */
