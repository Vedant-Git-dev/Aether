...
import type { CreateSandboxBackendParams, SandboxBackendHandle } from "aether/plugin-sdk/sandbox";
...
function sanitizeRuntimeId(value: string): string {
  if (/:workspace:[a-f0-9]{32}$/i.test(value.trim())) {
    ...
    return `aether-mxc-workspace-${hash}`;
  }
  const slug = value
    .toLowerCase()
    .replace(/[^a-z0-9_.-]+/g, "-")
  ...
  return `aether-mxc-${slug || "sandbox"}-${hash}`;
}

/** Factory function called by Aether when sandbox.backend=mxc. */
