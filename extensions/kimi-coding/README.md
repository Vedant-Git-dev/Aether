# Aether Kimi Coding Provider

Official Aether provider plugin for Kimi Coding.

Install from Aether:

```bash
aether plugins install @aether/kimi-provider
aether gateway restart
```

See <https://docs.aether.ai/providers/moonshot> for setup and configuration.

## Quota errors

Kimi can return HTTP 403 when a weekly usage limit is exhausted. Aether treats
...
Model rows live in `aether.plugin.json` under `modelCatalog.providers.kimi`.
