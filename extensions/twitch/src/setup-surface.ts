...
import { getChatChannelMeta, type ChannelPlugin } from "aether/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: AetherConfig, requestedAccountId?: string): string {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.AETHER_TWITCH_ACCESS_TOKEN?.trim();
