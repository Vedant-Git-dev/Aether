...
import { coerceErrorMessage as errorMessage } from "aether/plugin-sdk/error-runtime";
...
function resolveAetherHome() {
  const explicit = process.env.AETHER_HOME?.trim();
  if (!explicit) {
...
function resolveStateDir() {
  const override = process.env.AETHER_STATE_DIR?.trim();
  if (override) {
    if (override === "~" || override.startsWith("~/") || override.startsWith("~\\")) {
      return path.resolve(override.replace(/^~(?=$|[\\/])/u, () => resolveAetherHome()));
    }
    return path.resolve(override);
  }
  const home = resolveAetherHome();
  const profile = process.env.AETHER_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    ...
    if (!/^[A-Za-z0-9_-]+$/u.test(profile)) {
      throw new Error("invalid Aether profile name");
    }
    return path.join(home, `.aether-${profile}`);
  }
  const current = path.join(home, ".aether");
  const legacy = path.join(home, ".aether");
