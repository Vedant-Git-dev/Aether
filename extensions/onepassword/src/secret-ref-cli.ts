...
} from "aether/plugin-sdk/secret-file-runtime";
...
const onePasswordSecretRefSetupCli: PluginSecretRefSetupCli = createPluginSecretRefSetupCli({
  ...
  defaultPlanPath: () =>
    path.join(resolvePreferredAetherTmpDir(), `aether-1password-secrets-${randomUUID()}.json`),
  beforeApplyCommands: [
    "aether plugins enable onepassword",
    "aether onepassword secretref status",
  ],
});
