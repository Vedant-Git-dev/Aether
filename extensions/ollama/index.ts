// Ollama plugin entrypoint registers its Aether integration.
import { collectConfiguredModelRefValues } from "@aether/model-catalog-core/configured-model-refs";
import { findNormalizedProviderKey } from "@aether/model-catalog-core/provider-id";
...
import type { MemoryEmbeddingProviderAdapter } from "aether/plugin-sdk/memory-core-host-engine-embeddings";
...
import { runLiveProviderCatalog } from "aether/plugin-sdk/provider-catalog-live-runtime";
import { createProviderApiKeyAuthMethod } from "aether/plugin-sdk/provider-entry";
import type {
  ModelDefinitionConfig,
  ModelProviderConfig,
} from "aether/plugin-sdk/provider-model-shared";
import { buildOpenAICompatibleReplayPolicy } from "aether/plugin-sdk/provider-model-shared";
import { buildProviderToolCompatFamilyHooks } from "aether/plugin-sdk/provider-tools";
import { resolveConfiguredSecretInputString } from "aether/plugin-sdk/secret-input-runtime";
import {
...
async function checkWsl2CrashLoopRiskLazily(api: AetherPluginApi): Promise<void> {
  try {
    const { isWSL2Sync } = await import("aether/plugin-sdk/runtime-env");
    if (!isWSL2Sync()) {
      return;
    }
...
const createOllamaSharedProviderHooks = (api: AetherPluginApi) =>
  ({
...
export default definePluginEntry({
  ...
  register(api: AetherPluginApi) {
    ...
    const resolveCurrentPluginConfig = (config?: AetherConfig): OllamaPluginConfig => {
    ...
    api.registerProvider({
      ...
      buildUnknownModelHint: () =>
        ...
        'Set OLLAMA_API_KEY or run "aether onboard --auth-choice ollama-cloud". ' +
        "See: https://docs.aether.ai/providers/ollama",
    });
    api.registerProvider({
      id: OLLAMA_PROVIDER_ID,
      label: "Ollama",
      ...
      buildUnknownModelHint: () =>
        ...
        'Set OLLAMA_API_KEY="ollama-local" (any value works) or run "aether configure". ' +
        "See: https://docs.aether.ai/providers/ollama",
    });
  },
});
