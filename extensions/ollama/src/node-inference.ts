import { jsonResult } from "aether/plugin-sdk/channel-actions";
import { formatErrorMessage as errorMessage } from "aether/plugin-sdk/error-runtime";
...
import {
  ...
  readStringParam,
} from "aether/plugin-sdk/param-readers";
import type {
  AnyAgentTool,
  AetherPluginApi,
...
import { asFiniteNumber, asNullableRecord } from "aether/plugin-sdk/string-coerce-runtime";
...
type NodeSummary = Awaited<
  ReturnType<AetherPluginApi["runtime"]["nodes"]["list"]>
>["nodes"][number];
...
export function createOllamaNodeInferenceTool(api: AetherPluginApi): AnyAgentTool {
  return {
    ...ollamaNodeInferenceToolDefinition,
    execute: async (_toolCallId, args, signal) => {
      ...
      if (action === "discover") {
        ...
        return jsonResult({
          nodes,
          ...(modelNodes.length === 0 && {
            hint: "No connected node advertises Ollama inference. Start Ollama and `aether node run` on the target machine, then approve any request shown by `aether nodes pending`.",
          }),
        });
      }
