...
import type { ProviderAuthMethod } from "aether/plugin-sdk/plugin-entry";
import type {
  AetherConfig,
  SecretInput,
  SecretInputMode,
...
import { applyAgentDefaultModelPrimary } from "aether/plugin-sdk/provider-onboard";
...
function buildOllamaUnreachableLines(baseUrl: string, retry: boolean): string[] {
  return [
    ...
    ...(retry ? ["", "Continue when it is running. Aether will retry this address."] : []),
  ];
}
