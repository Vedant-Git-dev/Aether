...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'models.providers.ollama-cloud.baseUrl="https://ai.ollama.com" is retired; use "https://ollama.com". Run "aether doctor --fix".',
    match: (value) => findRetiredOllamaCloudBaseUrl(value) !== null,
  },
  {
    path: ["models", "providers", OLLAMA_PROVIDER_ID],
    message:
      'Legacy local Ollama authentication markers must be migrated. Run "aether doctor --fix".',
    match: isLegacyOllamaLocalConfig,
  },
];

function cloneProviderConfig(config: AetherConfig, providerId: string) {
  const nextConfig = structuredClone(config);
  const nextModels = asObjectRecord(nextConfig.models) ?? {};
  nextConfig.models = nextModels as AetherConfig["models"];
...
function migrateLegacyOllamaLocalConfig(config: AetherConfig): {
  config: AetherConfig;
  changes: string[];
} | null {
...
function migrateOllamaCloudRetiredBaseUrl(config: AetherConfig): {
  config: AetherConfig;
  changes: string[];
} | null {
