import type { WorkboardCard } from "@aether/workboard-contract";
import { resolveGlobalSingleton } from "aether/plugin-sdk/global-singleton";
import { isCronSessionKey } from "aether/plugin-sdk/routing";
import { isRecord } from "aether/plugin-sdk/string-coerce-runtime";
import type { AetherPluginApi, AetherPluginService } from "../api.js";
...
type WorkboardAutomationNudgeService = AetherPluginService & {
  stop: () => void;
...
type WorkboardAutomationNudgeState = {
  owner?: object;
  logger?: Parameters<AetherPluginService["start"]>[0]["logger"];
  pendingByBoard: Map<string, PendingBoardNudge>;
};

const WORKBOARD_AUTOMATION_NUDGE_STATE_KEY = Symbol.for("aether.workboard.automationNudgeState");
...
export function createWorkboardAutomationNudgeService(params: {
  ...
  gateway: Pick<AetherPluginApi["runtime"]["gateway"], "request">;
