...
const workboardLifecycleGatewayState = resolveGlobalSingleton(
  Symbol.for("aether.workboard.lifecycleGatewayState"),
  () => ({ ready: false }),
  (state) => {
    state.ready = false;
  },
...
type WorkboardLifecycleService = AetherPluginService & {
  stop: () => void;
  onGatewayStart: () => void;
  onGatewayStop: () => void;
};
...
export async function readWorkboardLifecycleSessions(
  gateway: Pick<AetherPluginApi["runtime"]["gateway"], "isAvailable" | "request">,
