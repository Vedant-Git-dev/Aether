...
import { resolvePositiveTimeoutSeconds } from "aether/plugin-sdk/provider-web-search";
...
function resolveTavilySearchConfig(cfg?: AetherConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: AetherConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: AetherConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
