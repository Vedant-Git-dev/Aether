...
import { removeProviderAuthProfilesWithLock } from "aether/plugin-sdk/provider-auth-runtime";
...
import { withAgentModelAliases } from "aether/plugin-sdk/provider-onboard";
import {
...
} from "aether/plugin-sdk/provider-setup";
import { isTruthyEnvValue } from "aether/plugin-sdk/runtime-env";
import { WizardCancelledError, type WizardPrompter } from "aether/plugin-sdk/setup";
...
function resolveLmstudioSetupDefaultBaseUrl(env: NodeJS.ProcessEnv = process.env): string {
  return isTruthyEnvValue(env.AETHER_DOCKER_SETUP)
    ? LMSTUDIO_DOCKER_HOST_BASE_URL
    : LMSTUDIO_DEFAULT_BASE_URL;
}
...
function stripLmstudioStoredAuthConfig(cfg: AetherConfig): AetherConfig {
...
function mergeDiscoveredLmstudioAllowlistEntries(params: {
  existing?: NonNullable<NonNullable<AetherConfig["agents"]>["defaults"]>["models"];
