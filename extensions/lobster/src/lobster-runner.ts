...
import { toErrorObject as toLintErrorObject } from "aether/plugin-sdk/error-runtime";
...
function normalizeEnvelope(
  ...
  if (envelope.status === "needs_input") {
    throw new Error("Lobster input requests are not supported by the Aether Lobster tool yet");
  }
...
async function loadEmbeddedToolRuntimeFromPackage(): Promise<EmbeddedToolRuntime> {
  ...
  // @aether/lobster/core; the plugin's declared @aether/lobster dependency
  // provides it at runtime, so it is a used direct dependency.
  const coreSpecifier = ["@aether", "lobster", "core"].join("/");
