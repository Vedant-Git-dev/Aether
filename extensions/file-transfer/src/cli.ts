...
} from "aether/plugin-sdk/config-mutation";
import { createClackPrompter } from "aether/plugin-sdk/setup-runtime";
...
async function runApprovalMigration(options: MigrationOptions): Promise<void> {
  ...
  if (!prepared.snapshot.valid) {
    throw new Error("Aether config is invalid; fix it before migrating file-transfer approvals");
  }
  ...
  if (options.json || !process.stdin.isTTY) {
    const result = {
      ...
      changed: false,
      items,
      command: "aether file-transfer approvals migrate",
    };
    if (options.json) {
    ...
    throw new Error(
      "File-transfer permissions need interactive review. Run `aether file-transfer approvals migrate` in a terminal.",
    );
  }
  ...
  await prompt.note(
    "Older Aether versions cannot read the migrated format. To downgrade, restore the adjacent config backup shown after migration before starting the older version.",
    "Downgrade",
  );
  if (options.dryRun) {
