...
import { createSubsystemLogger, isTruthyEnvValue } from "aether/plugin-sdk/runtime-env";
...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "AETHER_EAGER_BROWSER_CONTROL_SERVER";
...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage Aether's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
...
function createLazyBrowserTool(
  ...
  config?: AetherPluginToolContext["runtimeConfig"],
): AnyAgentTool {
  const bindingResult =
...
function createBrowserToolOptions(ctx: AetherPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentId?: string;
...
function createBrowserProxyNodeHostCommand(command: string): AetherPluginNodeHostCommand {
  return {
    command,
    cap: "browser",
    isAvailable: ({ config }) =>
...
export const browserPluginNodeHostCommands: AetherPluginNodeHostCommand[] = [
...
export const browserSecurityAuditCollectors: AetherPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): AetherPluginService {
  let service: AetherPluginService | null = null;
...
export function registerBrowserPlugin(api: AetherPluginApi) {
  ...
  api.registerTool(((ctx: AetherPluginToolContext) => {
  ...
  api.registerHttpRoute({
    ...
    handler: (_req: IncomingMessage, res: ServerResponse) => {
      ...
      res.end("Upgrade Required: connect the Aether Chrome extension over WebSocket.");
    },
