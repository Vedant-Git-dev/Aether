#!/usr/bin/env node
/**
 * Copies the unpacked Aether Chrome extension into the browser plugin dist so
 * `aether browser extension path` resolves a stable location for
...
const srcDir = process.env.AETHER_CHROME_EXT_SRC_DIR ?? path.join(pluginDir, "chrome-extension");
const outDir =
  process.env.AETHER_CHROME_EXT_OUT_DIR ??
...
async function main() {
  if (!(await pathExists(srcDir))) {
    if (
      process.env.AETHER_SPARSE_PROFILE ||
      process.env.AETHER_CHROME_EXT_SKIP_MISSING === "1"
    ) {
      return;
    }
