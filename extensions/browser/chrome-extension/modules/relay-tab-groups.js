import { AETHER_TAB_GROUP_TITLE } from "./relay-core.js";

async function isAetherGroupId(groupId) {
  if (!Number.isInteger(groupId) || groupId < 0) {
    return false;
  }
  try {
    const group = await chrome.tabGroups.get(groupId);
    return group.title === AETHER_TAB_GROUP_TITLE;
  } catch {
    return false;
  }
}
...
export async function isTabSelected(tab) {
  return await isAetherGroupId(tab?.groupId);
}

export async function addTabToAetherGroup(tabId, { chromeApi, getGroupColor, created }) {
  ...
  const groups = await chromeApi.tabGroups
    .query({ title: AETHER_TAB_GROUP_TITLE })
    .catch(() => []);
  assertCurrent();
  ...
  if (!group) {
    ...
    await chromeApi.tabGroups.update(groupId, { title: AETHER_TAB_GROUP_TITLE, color });
    assertCurrent();
  }
}
