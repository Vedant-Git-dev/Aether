...
import { addTabToAetherGroup } from "./relay-tab-groups.js";
...
export function createTabAccessPolicy({ chromeApi = chrome, isSelectedTab, getGroupColor }) {
  ...
  const addTabToGroup = (tabId, created) =>
    addTabToAetherGroup(tabId, { chromeApi, getGroupColor, created });
  ...
  async function requireTab(tabId, epoch = capture(tabId), afterNavigation = false) {
    ...
    if (state.reason === "paused") {
      throw new Error(`tab ${tabId} is paused for Aether`);
    }
    if (state.reason === "not-selected") {
      throw new Error(`tab ${tabId} is not in the Aether tab group`);
    }
    if (state.reason === "incognito") {
      throw new Error(`tab ${tabId} is incognito and unavailable to Aether`);
    }
    throw new Error(`tab ${tabId} is restricted or unavailable to Aether`);
  }
