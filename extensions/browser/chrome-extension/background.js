...
// Aether extension service worker.
//
// Thin transport between the Aether extension relay (loopback WebSocket) and
...
// The Aether tab group is the ACL in selected mode and an ownership marker
// in all-tabs mode.
import {
...
const RELAY_WATCHDOG_ALARM = "aether-relay-watchdog";
const RELAY_OPENING_DEADLINE_ALARM = "aether-relay-opening-deadline";
...
async function removeTabFromAetherGroup(tabId) {
  try {
...
function failRelayAuthentication(ws, error) {
  ...
  relayStatusHint =
    "Relay authentication v2 failed. Update Aether, or re-pair after a relay key rotation.";
  try {
    closeRelaySocket(
      4001,
...
function handleRelayOpeningDeadline() {
  ...
  setBadge("error");
  relayStatusHint = "Relay authentication v2 timed out. Make sure Aether is up to date.";
  scheduleReconnect();
}
...
const handlePopupMessage = createPopupMessageHandler({
  ...
  setBadge,
  detachDebugger,
  removeTabFromAetherGroup,
  addTabToAetherGroup: (tabId) => tabAccessPolicy.addTabToGroup(tabId),
  scheduleTabsSync,
  pauseTab,
});
