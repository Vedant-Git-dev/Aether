...
import { isPathInside } from "aether/plugin-sdk/file-access-runtime";
import {
  asNullableRecord,
...
import { DEFAULT_AETHER_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
import { movePathToTrash } from "./browser/trash.js";
import type { AetherConfig } from "./config/config.js";
import { formatCliCommand, note } from "./sdk-setup-tools.js";
import { CONFIG_DIR, resolveUserPath } from "./utils.js";

const CHROME_MCP_MIN_MAJOR = 144;
const LEGACY_AETHER_BROWSER_PROFILE_NAME = "aether";
...
/** Legacy managed aether profile paths that can be archived by doctor --fix. */
...
function collectChromeMcpProfiles(cfg: AetherConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: AetherConfig): ManagedProfile[] {
  const browser = asNullableRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asNullableRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "aether";
    if (driver !== "existing-session") {
...
function isLegacyAetherProfileConfigured(cfg: AetherConfig, legacyProfileDir: string): boolean {
  const browser = asNullableRecord(cfg.browser);
  if (!browser) {
    return false;
  }
  if (normalizeOptionalString(browser.defaultProfile) === LEGACY_AETHER_BROWSER_PROFILE_NAME) {
    return true;
  }
  ...
  if (Object.hasOwn(configuredProfiles, LEGACY_AETHER_BROWSER_PROFILE_NAME)) {
    return true;
  }
...
/** Detects unmanaged legacy aether browser profile residue on disk. */
export function detectLegacyAetherBrowserProfileResidue(
  cfg: AetherConfig,
  ...
  const legacyProfileDir = resolveManagedBrowserProfileDir(
    configDir,
    LEGACY_AETHER_BROWSER_PROFILE_NAME,
  );
  ...
  if (isLegacyAetherProfileConfigured(cfg, legacyProfileDir)) {
    return null;
  }
  ...
  if (
    resolved.defaultProfile !== DEFAULT_AETHER_BROWSER_PROFILE_NAME ||
  ...
  return {
    ...
    canonicalUserDataDir: resolveManagedBrowserUserDataDir(
      configDir,
      DEFAULT_AETHER_BROWSER_PROFILE_NAME,
    ),
  };
}

function formatLegacyAetherBrowserProfileResidueNote(
  residue: LegacyAetherBrowserProfileResidue,
): string {
  return [
    `- Legacy managed browser profile residue was found at ${residue.legacyProfileDir}.`,
    `- The canonical Aether-managed browser profile is ${residue.canonicalUserDataDir}.`,
    `- If no browser is using the legacy profile, run ${formatCliCommand("aether doctor --fix")} to archive it safely instead of deleting it in place.`,
  ].join("\n");
}
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (fs.existsSync(extensionCopyPath)) {
    noteFn(
      [
        ...
        `- Run ${formatCliCommand("aether browser extension status --json")} to inspect it explicitly; this may request browser profile access.`,
        `- Run ${formatCliCommand("aether browser extension install")} if setup or repair is needed.`,
      ].join("\n"),
      "Browser extension bootstrap",
    );
  }
  const legacyAetherResidue = detectLegacyAetherBrowserProfileResidue(cfg, {
    configDir: deps?.configDir,
    pathExists: deps?.pathExists,
  });
  if (legacyAetherResidue) {
    noteFn(formatLegacyAetherBrowserProfileResidueNote(legacyAetherResidue), "Browser");
  }
  if (platform === "darwin") {
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Aether-managed browser profile(s) are configured: ${managedProfileLabel}.`,
        "- No Chromium-based browser executable was found on this host for Aether-managed launch.",
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Aether-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Aether connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Aether does not bundle Chrome.`,
...
export async function maybeRepairOwnedChromeExtensionNativeHosts(): Promise<{
  ...
  return {
    ...
    changes: [],
    warnings: [
      `Chrome extension native-host repair skipped: Doctor does not inspect personal browser profiles. Run ${formatCliCommand("aether browser extension install")} to repair explicitly.`,
    ],
  };
}

/** Archives legacy aether browser profile residue when doctor --fix is requested. */
export async function maybeArchiveLegacyAetherBrowserProfileResidue(
  cfg: AetherConfig,
  deps?: BrowserDoctorFilesystemDeps,
): Promise<{ changes: string[]; warnings: string[] }> {
  const residue = detectLegacyAetherBrowserProfileResidue(cfg, deps);
  if (!residue) {
  ...
  try {
    ...
    return {
      changes: [
        [
          "Archived legacy aether managed browser profile residue.",
    ...
    return {
      changes: [],
      warnings: [`Legacy aether browser profile residue could not be archived: ${message}`],
    };
  }
}
