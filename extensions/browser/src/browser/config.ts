...
import { parseBrowserHttpUrl, redactCdpUrl } from "aether/plugin-sdk/browser-cdp";
import type {
  BrowserConfig,
  BrowserProfileConfig,
  AetherConfig,
} from "aether/plugin-sdk/config-contracts";
import { resolveGatewayPort } from "aether/plugin-sdk/gateway-config-runtime";
import { mergeSsrFPolicies } from "aether/plugin-sdk/ssrf-policy";
import { isLoopbackHost, type SsrFPolicy } from "aether/plugin-sdk/ssrf-runtime";
import {
  normalizeOptionalString,
  normalizeOptionalTrimmedStringList,
  parseBooleanValue,
} from "aether/plugin-sdk/string-coerce-runtime";
import { resolveUserPath } from "aether/plugin-sdk/text-utility-runtime";
import {
...
import {
  ...
  DEFAULT_AETHER_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  mcpArgs?: string[];
  color: string;
  driver: "aether" | "existing-session" | "extension";
  executablePath?: string;
  headless: boolean;
...
const EXTENSION_RELAY_CDP_USER = "aether-internal";
/** Environment variable that overrides managed Chrome headless mode. */
const BROWSER_HEADLESS_ENV_KEY = "AETHER_BROWSER_HEADLESS";
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AETHER_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AETHER_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_AETHER_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_AETHER_BROWSER_PROFILE_NAME]
        ? DEFAULT_AETHER_BROWSER_PROFILE_NAME
        : "user");
