...
async function runBrowserLiveProbe(profileCtx: ProfileContext, signal: AbortSignal) {
  ...
  try {
    ...
    return {
      ...
      summary: String(err),
      fixHint: "Run aether browser start, then retry with aether browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    params.profileCtx.profile.driver !== "aether" ||
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched aether profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "aether" && driver !== "aether" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "aether", "aether", or "existing-session"`,
      );
    }
