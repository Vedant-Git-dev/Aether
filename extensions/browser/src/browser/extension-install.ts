...
import { resolveStateDir } from "aether/plugin-sdk/state-paths";
import {
...
const OWNED_LAUNCHER_MARKER = "# Aether native messaging bootstrap v1";
...
const NATIVE_HOST_DESCRIPTION = "Aether browser extension bootstrap";
export {
  FOUNDATION_CHROME_WEB_STORE_URL,
...
function resolveInstallConfigPath(deps: ExtensionInstallDeps): string | undefined {
  ...
  return explicit ? resolveStateDir({ ...env, AETHER_STATE_DIR: explicit }) : undefined;
}
...
function expectedExtensionIds(extensionIds: string[]): string[] {
  ...
  // it never proves that an arbitrary extension path is Aether-owned.
...
function parseOwnedLauncherTargets(params: {
  ...
  const pattern = new RegExp(
    `^#!/bin/sh\\n${escapeRegExp(OWNED_LAUNCHER_MARKER)}\\nexport AETHER_STATE_DIR=${quotedValue}\\n(?:export AETHER_CONFIG_PATH=${quotedValue}\\n)?exec ${command} "\\$@"\\n$`,
    "u",
  );
...
async function resolveLauncherInstall(params: {
  ...
  return {
    ...
    content: [
      ...
      `export AETHER_STATE_DIR=${shellQuote(resolveInstallStateDir(params.deps))}`,
      ...(configPath ? [`export AETHER_CONFIG_PATH=${shellQuote(configPath)}`] : []),
...
async function inspectRegistration(
  ...
  try {
    ...
    try {
      ...
      issue =
        "registered native host runtime or entry is unavailable or unsafe; run aether browser extension install";
    }
    return {
      product: root.product,
      browser: root.label,
...
export async function installChromeExtensionBootstrap(params: {
  ...
  for (const root of chromeProductRoots(deps)) {
    ...
    try {
      ...
      if (request) {
        params.onProgress?.(
          `Requested the Aether Store extension for ${root.label}. Restart Chrome if needed, then approve Aether in chrome://extensions.`,
        );
      }
    } catch (error) {
      preRegistrationIssues.push(
        `${root.label}: Store installation request refused (${error instanceof Error ? error.message : String(error)}). Add Aether directly: ${FOUNDATION_CHROME_WEB_STORE_URL}`,
      );
    }
  }
  if (preRegisteredRoots > 0) {
    params.onProgress?.(
      `Native bootstrap is ready. Add Aether from the Chrome Web Store: ${FOUNDATION_CHROME_WEB_STORE_URL}. For development, load unpacked from ${installed}.`,
    );
  } else {
    preRegistrationIssues.push(
  ...
  while (
    ...
    if (!announcedWait) {
      params.onProgress?.("Waiting for Chrome to verify the Aether extension…");
      announcedWait = true;
    }
...
export async function browserExtensionStatus(params: {
  ...
  return {
    ...
    issues: [
      ...(installedCopy.present && !installedCopy.owned
        ? [`Chrome extension copy is not Aether-owned: ${installedPath}`]
        : []),
      ...discovery.issues,
...
/** Remove only registrations and launchers that carry Aether ownership. */
