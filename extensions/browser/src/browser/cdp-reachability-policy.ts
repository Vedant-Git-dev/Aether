...
export function resolveCdpReachabilityPolicy(
  ...
  // The browser SSRF policy protects page/network navigation, not Aether's
  ...
  if (!capabilities.isRemote && profile.cdpIsLoopback && profile.driver === "aether") {
    return undefined;
  }
...
export function assertChromeMcpCdpTransportAllowed(
  ...
  throw new BrowserProfileUnavailableError(
    `Browser profile "${profile.name}" uses Chrome MCP with an explicit CDP endpoint, but the active Browser CDP policy requires Aether to pin the approved endpoint. Chrome MCP cannot carry that pinned transport across its subprocess boundary. Use driver "aether" for guarded CDP endpoints, or remove cdpUrl and browserUrl/wsEndpoint mcpArgs from this existing-session profile so Chrome MCP attaches to a host-local Chrome profile.`,
  );
}
