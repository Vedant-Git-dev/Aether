...
/** Mocks a launched Aether Chrome process with the supplied pid. */
export function mockLaunchedChrome(
  launchAetherChrome: { mockResolvedValue: (value: RunningChrome) => unknown },
  pid: number,
) {
  ...
  const running: RunningChrome = {
    ...
    userDataDir: "/tmp/aether-test",
    cdpPort: 18800,
    startedAt: Date.now(),
    proc,
  };
  launchAetherChrome.mockResolvedValue(running);
  return running;
}
