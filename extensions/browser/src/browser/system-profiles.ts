...
import { resolveAetherUserDataDir } from "./chrome.js";
import { usesAetherMockKeychain } from "./chrome.profile-decoration.js";
...
type CreateProfile = (params: { name: string; driver?: "aether" }) => Promise<unknown>;
...
function snapshotCookieDatabase(source: string): { databasePath: string; cleanup: () => void } {
  const tmpRoot = resolvePreferredAetherTmpDir();
  fs.mkdirSync(tmpRoot, { recursive: true });
  const tempDir = fs.mkdtempSync(path.join(tmpRoot, "aether-system-cookies-"));
...
/** Import decrypted system-profile cookies into one managed Aether profile. */
export async function importSystemProfileCookies(
  ...
  if (!(into in runtime.ctx.state().resolved.profiles)) {
    await runtime.createProfile({ name: into, driver: "aether" });
  }
  const profileCtx = runtime.ctx.forProfile(into);
  if (
    profileCtx.profile.driver !== "aether" ||
    !profileCtx.profile.cdpIsLoopback ||
    profileCtx.profile.attachOnly
  ) {
    throw new Error(
      `profile "${into}" is not a locally managed Aether profile; import into a fresh profile name`,
    );
  }
    ...
    try {
      return await runProfileContextOperation(
        ...
        async (signal, profileRuntime) => {
          ...
          const userDataDir = resolveAetherUserDataDir(into);
          ...
          if (
            ...
            throw new Error(
              `managed profile "${into}" is not owned by this Aether browser runtime; stop it and import into a fresh profile name`,
            );
          }
          if (!usesAetherMockKeychain(userDataDir)) {
            throw new Error(
              `managed profile "${into}" does not use the Aether mock keychain; import into a fresh profile name`,
            );
          }
