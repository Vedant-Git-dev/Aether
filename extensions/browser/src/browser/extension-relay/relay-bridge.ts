 ...
 * on one side and the Aether Chrome extension's chrome.debugger transport on
...
const BROWSER_TARGET_ID = "aether-extension-relay";
/** Playwright requires every attached page target to identify its browser context. */
const BROWSER_CONTEXT_ID = "aether-extension-context";
...
export class ExtensionRelayBridge {
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("Aether Chrome extension is not connected to the relay");
    }
  ...
  private async withAttachedTab<T>(
    ...
    const extension = this.extension;
    if (!tab) {
      throw new Error(`tab ${tabId} is not available to Aether`);
    }
  ...
  private async ensureTabAttached(
    ...
    const attaching = attachment.then(async (response) => {
      ...
      const sessionId = `aether-tab-${tabId}-${this.nextSessionOrdinal++}`;
    ...
    switch (request.method) {
      case "Browser.getVersion": {
        ...
        this.respond(client, request, {
          ...
          revision: "aether-extension-relay",
      ...
      case "Target.attachToBrowserTarget": {
        const sessionId = `aether-browser-${this.nextSessionOrdinal++}`;
      ...
      case "Target.attachToTarget": {
        ...
        await this.withAttachedTab(client, found.tabId, (attached) => {
          ...
          const sessionId = `aether-tab-${found.tabId}-${this.nextSessionOrdinal++}`;
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The Aether extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
