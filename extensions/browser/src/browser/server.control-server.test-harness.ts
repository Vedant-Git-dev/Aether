...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "aether" | "existing-session";
      attachOnly?: boolean;
    }
  >;
  tabUrl: string;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.AETHER_GATEWAY_PORT;
    return;
  }
  process.env.AETHER_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "aether",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/aether" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    aether: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/aether.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/aether.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/aether.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchAetherChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveAetherUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOwnedAetherChrome: vi.fn(async () => false),
  stopAetherChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "aether";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.AETHER_GATEWAY_PORT;
  process.env.AETHER_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.AETHER_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.AETHER_GATEWAY_PASSWORD;
  delete process.env.AETHER_GATEWAY_TOKEN;
  delete process.env.AETHER_GATEWAY_PASSWORD;
}
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.AETHER_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.AETHER_GATEWAY_PASSWORD;
  } else {
    process.env.AETHER_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
