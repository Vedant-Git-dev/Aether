...
export const FOUNDATION_CHROME_WEB_STORE_URL = `https://chromewebstore.google.com/detail/aether/${FOUNDATION_CHROME_WEB_STORE_EXTENSION_ID}`;
...
const OWNED_REQUEST = {
  ...
  aetherOwnership: "browser-store-install-v1",
};
...
async function inspectRequest(root: ChromeProductRoot): Promise<ChromeStoreInstallRequest> {
  ...
  try {
    ...
    if (
      ...
      !("aetherOwnership" in value) ||
      value.aetherOwnership !== OWNED_REQUEST.aetherOwnership
    ) {
      return {
        ...result,
        state: "foreign",
        issue: "Store install registration is not Aether-owned",
      };
    }
