...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "aether"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "Aether Chrome extension is connected"
        : "Aether Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
              "Install the Aether Chrome extension (aether browser extension path), run aether browser extension pair, and paste the pairing string into the extension popup.",
          }),
    });
    ...
    checks.push({
      ...
      ...(mismatch
        ? {
            fixHint:
              "Reload the Aether extension from chrome://extensions. If the versions still differ, fully quit and reopen Chrome.",
          }
        : {}),
    });
  } else {
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set AETHER_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ? {}
        : {
            fixHint: "Run aether browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
