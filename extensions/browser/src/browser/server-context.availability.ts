...
function formatLocalPortOwnershipHint(profile: ResolvedBrowserProfile): string {
  const resetHint =
    `If Aether should own this local profile, run action=reset-profile profile=${profile.name} ` +
  ...
  return (
    ...
    `set browser.profiles.${profile.name}.attachOnly=true so Aether attaches without trying ` +
    "to manage the local process. For Browserless Docker, set EXTERNAL to the same WebSocket " +
    "endpoint Aether can reach via browser.profiles.<name>.cdpUrl."
  );
}
...
export function createProfileAvailability({
  ...
  const stopExactRunning = async (
    ...
    try {
      await stopAetherChrome(running);
  ...
  const formatChromeMcpAttachFailure = (lastError: unknown): string => {
    ...
    if (message.includes("DevToolsActivePort") || message.includes("Could not connect to Chrome")) {
      return (
        ...
        'If you do not need your signed-in browser session, use the managed "aether" profile instead.' +
        detail
      );
    }
    return (
  ...
  const waitForCdpReadyAfterLaunch = async (
    ...
    // launchAetherChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const launchManagedChrome = async (
    ...
    try {
      return await launchAetherChrome(current.resolved, profile, {
        ...launchOptions,
        signal,
      });
    } catch (err) {
  ...
  const ensureBrowserAvailableOnce = async (
    ...
    if (!httpReachable) {
      ...
      // Browser control service can restart while a loopback Aether browser is still
      ...
      if (attachOnly || remoteCdp) {
        if (capabilities.mode === "local-extension") {
          ...
          throw new BrowserProfileUnavailableError(
            `The Aether Chrome extension is not connected for profile "${profile.name}". ` +
    ...
    if (attachOnly || remoteCdp) {
      ...
      if (capabilities.mode === "local-extension") {
        const { EXTENSION_PAIRING_HINT } = await getExtensionRelayModule();
        throw new BrowserProfileUnavailableError(
          `The extension relay for profile "${profile.name}" is running but the Aether Chrome extension is not connected. ${EXTENSION_PAIRING_HINT}`,
        );
      }
    ...
    if (!runtime.running) {
      const detail = await describeCdpFailure(PROFILE_ATTACH_RETRY_TIMEOUT_MS);
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by aether. ` +
  ...
  const stopRunningBrowser = async (): Promise<{ stopped: boolean }> => {
    ...
    const result = await beginProfileTransition({
      ...
      afterCleanup: async () => {
        ...
        stoppedOwnedProcess = await stopOwnedAetherChrome(current.resolved, profile);
      },
    });
    return {
