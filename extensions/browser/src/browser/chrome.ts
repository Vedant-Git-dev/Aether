/**
 * Aether-managed Chrome lifecycle and CDP helpers.
 *
...
} from "aether/plugin-sdk/process-runtime";
...
import { resolvePreferredAetherTmpDir } from "../infra/tmp-aether-dir.js";
...
import {
  decorateAetherProfile,
  ensureProfileCleanExit,
  ensureProfileNetworkPredictionDisabled,
  isProfileDecorated,
  usesAetherMockKeychain,
...
async function ensureManagedChromePortAvailable(
  ...
  // Chromium tries IPv4 loopback first, while Aether polls the configured endpoint.
...
function chromeLaunchHints(params: {
  ...
  if (missingDisplay && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set AETHER_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  ...
  if (singletonInUse) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.aether/browser/${params.profile.name}/user-data.`,
    );
  }
...
/** Resolve the user-data-dir path for a managed Aether Chrome profile. */
export function resolveAetherUserDataDir(profileName = DEFAULT_AETHER_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed Aether browser. */
function buildAetherChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
  ...
  if (platform === "darwin" && params.useMockKeychain) {
    // This is an isolated Aether-owned profile, not the user's Chrome profile.
...
/** Launch or attach to the managed Aether Chrome profile. */
export async function launchAetherChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveAetherUserDataDir(profile.name);
  ...
  const useMockKeychain =
    ...
    (usesAetherMockKeychain(userDataDir) || (profileIsNew && headlessMode.headless));

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_AETHER_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = async (onStderr?: (chunk: Buffer | string) => void) => {
    signal?.throwIfAborted();
    const args = buildAetherChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
      ...headlessOptions,
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredAetherTmpDir(), ".chromium");
  ...
  if (needsDecorate) {
    try {
      decorateAetherProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
        downloadDir: DEFAULT_DOWNLOAD_DIR,
        mockKeychain: useMockKeychain,
      });
      log.info(`🦞 aether browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`aether browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`aether browser network-prediction prefs failed: ${String(err)}`);
  }

  try {
    ensureProfileCleanExit(userDataDir);
  } catch (err) {
    log.warn(`aether browser clean-exit prefs failed: ${String(err)}`);
  }
  signal?.throwIfAborted();
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `🦞 aether browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
...
export async function stopOwnedAetherChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  timeoutMs = CHROME_STOP_TIMEOUT_MS,
): Promise<boolean> {
  if (!profile.cdpIsLoopback || profile.attachOnly || profile.driver !== "aether") {
    return false;
  }
...
export async function stopAetherChrome(
