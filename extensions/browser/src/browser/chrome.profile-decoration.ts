/**
 * Aether-managed Chrome profile decoration.
 *
...
import { loadJsonFile, saveJsonFile } from "aether/plugin-sdk/json-store";
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".aether-profile-decorated");
}
...
export function usesAetherMockKeychain(userDataDir: string): boolean {
  const localState = safeReadJson(path.join(userDataDir, "Local State"));
  return readDefaultProfileInfo(localState)?.aether_mock_keychain === true;
}

/** Disable Chromium network prediction in an Aether-managed Chrome profile. */
 ...
 */
export function decorateAetherProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string; mockKeychain?: boolean },
) {
  const desiredName = opts?.name ?? DEFAULT_AETHER_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AETHER_BROWSER_COLOR).toUpperCase();
  ...
  if (opts?.mockKeychain) {
    ...
    setDeep(localState, ["profile", "info_cache", "Default", "aether_mock_keychain"], true);
  }
