 ...
 */
import { parseBrowserHttpUrl } from "aether/plugin-sdk/browser-cdp";
import {
  extractErrorCode,
  formatErrorMessage,
  toErrorObject,
} from "aether/plugin-sdk/error-runtime";
import { resolveTimerTimeoutMs } from "aether/plugin-sdk/number-runtime";
import { readResponseWithLimit } from "aether/plugin-sdk/response-limit-runtime";
import { formatCliCommand } from "aether/plugin-sdk/setup-tools";
import { fetchWithSsrFGuard, isLoopbackHost } from "aether/plugin-sdk/ssrf-runtime";
import {
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-aether-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-aether-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-aether-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveDispatcherBrowserControlOwnership(url: string): BrowserControlOwnership {
  ...
  try {
    ...
    return profile.driver === "aether" && profile.cdpIsLoopback && !profile.attachOnly
      ? "local-managed"
      : "external-browser";
  } catch {
    return "unknown";
...
function resolveBrowserFetchOperatorHint(
  ...
  if (opts?.ownership === "external-browser") {
    return (
      "The browser profile is external to Aether; make sure its browser/CDP endpoint " +
      "is running and reachable. Restarting the Aether gateway will not launch it."
    );
  }
  const isLocal = !isAbsoluteHttp(url);
  return isLocal
    ? `Restart the Aether gateway (Aether.app menubar, or \`${formatCliCommand("aether gateway")}\`).`
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (kind === "timeout") {
    return new Error(
      `Can't reach the Aether browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
  ...
  if (kind === "transient-network") {
    return new Error(
      `Can't reach the Aether browser control service. ${operatorHint} (${msg}) ${BROWSER_TOOL_TRANSIENT_MODEL_HINT}`,
      err instanceof Error ? { cause: err } : undefined,
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Aether browser control service. ${operatorHint} (${msg})`,
