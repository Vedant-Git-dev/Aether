...
import { CONFIG_DIR } from "aether/plugin-sdk/text-utility-runtime";
...
export async function movePathToTrash(targetPath: string): Promise<string> {
  const { movePathToTrash: movePathToTrashWithAllowedRoots } =
    await import("aether/plugin-sdk/browser-config");
  return await movePathToTrashWithAllowedRoots(targetPath, {
    // Managed browser data follows AETHER_STATE_DIR/AETHER_CONFIG_PATH, which
