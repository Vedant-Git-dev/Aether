...
export function describeBrowserTool(opts: {
  ...
  const lines = [
    `Control the browser via Aether's browser control server. Available actions: ${opts.capabilities.actions.join(", ")}.`,
    ...(actions.has("profiles")
      ? [
          "Browser choice: omit profile to use the configured default (normally the isolated Aether-managed `aether` browser).",
