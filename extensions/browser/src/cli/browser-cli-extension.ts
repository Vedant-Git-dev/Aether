/**
 * `aether browser extension` CLI: register the Store and development extension
...
/** Register `aether browser extension` lifecycle and compatibility commands. */
export function registerBrowserExtensionCommands(
  ...
  const extension = browser
    .command("extension")
    .description("Install and inspect the Aether Chrome extension bootstrap");
    ...
    .action(async (opts, command) => {
      await runCommandWithRuntime(
        ...
        async () => {
          ...
          if (!json) {
            defaultRuntime.log(info("Preparing the Aether Chrome extension…"));
          }
                ...
                ? theme.warn(
                    status.platformSupport === "manual_required"
                      ...
                      : status.storeInstallRequests.some((entry) => entry.state === "requested")
                        ? `Store installation requested. Enable Aether in chrome://extensions and approve Chrome's prompt. If it has not appeared, restart Chrome when convenient or add it from ${FOUNDATION_CHROME_WEB_STORE_URL}. Run extension status to check setup again.`
                        : `Setup needs attention. Add Aether from ${FOUNDATION_CHROME_WEB_STORE_URL} after native registration succeeds. For development, load the printed unpacked path. If the extension attempted setup before the native host existed, restart Chrome once.`,
                  )
                : info(
  ...
  extension
    ...
    .description(
      "Remove Aether-owned Store install requests; Chrome may remove the extension on restart",
    )
  ...
  extension
    ...
    .description("Remove only Aether-owned Chrome native-host registrations")
    .option("--json", "Print a machine-readable removal report")
    .action(async (opts, command) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const json = opts.json === true || parentOpts(command).json === true;
        ...
        async () => {
          ...
          defaultRuntime.log(
            [
              ...
              info("2. Open the Aether popup and paste this pairing string:"),
              "",
