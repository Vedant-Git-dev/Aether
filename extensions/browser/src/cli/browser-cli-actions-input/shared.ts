...
import { FsSafeError, readRegularFile } from "aether/plugin-sdk/security-runtime";
import { asRecord, readStringField } from "aether/plugin-sdk/string-coerce-runtime";
  ...
  if (!printBrowserJsonResult(parent, result)) {
    ...
    if (result.blockedByDialog) {
      ...
      const ids = Array.isArray(pending)
        ...
        : [];
      defaultRuntime.log(
        `Action blocked by a modal dialog${ids.length ? ` (${ids.join(", ")})` : ""}. Use aether browser dialog --accept or --dismiss${ids.length ? " --dialog-id <id>" : ""} to continue.`,
      );
...
function createActionsInputTooLargeError(source: string, cause?: unknown): FsSafeError {
  return new FsSafeError(
    "too-large",
    `${source} exceeds ${ACTIONS_INPUT_MAX_BYTES} bytes. Split the batch plan into smaller files or run multiple aether browser batch commands.`,
    { cause },
  );
}
