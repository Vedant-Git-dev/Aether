...
import { redactCdpUrl } from "aether/plugin-sdk/browser-cdp";
...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "aether";
...
function formatBrowserDoctorGatewayError(error: unknown): string {
  ...
  return "Gateway auth SecretRef is unavailable in this command path; browser doctor cannot reach the admin-scoped browser.request endpoint. Set AETHER_GATEWAY_TOKEN or AETHER_GATEWAY_PASSWORD, then retry.";
}
...
async function runBrowserDoctor(parent: BrowserParentOpts, profile?: string, deep?: boolean) {
  ...
  checks.push({
    name: "profile",
    ok: true,
    detail: `${status.profile ?? "aether"} (${usesChromeMcpTransport(status) ? "chrome-mcp" : (status.transport ?? "cdp")})`,
  });
  checks.push({
    name: "browser",
    ok: status.running,
    detail: status.running
      ...
      : "not running; run `aether browser start`",
  });
...
type BrowserProfileDriver = "aether" | "existing-session" | "extension";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "aether" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (aether|existing-session). Default: aether")
    .action(
      async (
        opts: {
          name: string;
        ...
        await runBrowserCommand(async () => {
          if (
            ...
            throw new Error("--driver must be aether or existing-session");
          }
