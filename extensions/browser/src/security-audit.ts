 ...
 */
import type { AetherPluginSecurityAuditContext } from "aether/plugin-sdk/plugin-entry";
...
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "aether/plugin-sdk/ssrf-policy";
import {
  hasNonEmptyString,
...
export function collectBrowserSecurityAuditFindings(ctx: AetherPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("aether security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.AETHER_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.AETHER_GATEWAY_PASSWORD) ||
