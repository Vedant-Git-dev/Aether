...
export function useBuzzBusLifecycleFixture() {
  beforeEach(() => {
    previousStateDir = process.env.AETHER_STATE_DIR;
    // aether-temp-dir: allow extension tests cannot import root test helpers.
    stateDir = mkdtempSync(path.join(tmpdir(), "aether-buzz-dedupe-"));
