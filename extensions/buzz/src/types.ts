...
import { createAccountListHelpers, mergeAccountConfig } from "aether/plugin-sdk/account-helpers";
import {
  DEFAULT_ACCOUNT_ID,
  normalizeAccountId,
  normalizeOptionalAccountId,
} from "aether/plugin-sdk/account-id";
import { assertSecretOwnerAvailable } from "aether/plugin-sdk/channel-secret-owner-runtime";
...
function resolveChannelConfig(cfg: AetherConfig): BuzzConfigInput | undefined {
...
export function resolveBuzzAccount(params: {
  ...
  return {
    accountId,
    name: normalizeOptionalString(config.name) ?? "Aether",
