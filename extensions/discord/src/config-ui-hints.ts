import { createChannelConfigUiHints } from "aether/plugin-sdk/channel-core";
import type { ChannelConfigUiHint } from "aether/plugin-sdk/channel-core";
...
export const discordChannelConfigUiHints = {
  ...
  ...createChannelConfigUiHints({
    ...
    nativeCommands: true,
    streaming: {
      "": 'Discord preview streaming is off by default. Set mode to "partial", "block", or "progress" to opt in. Run aether doctor --fix to migrate legacy keys.',
  ...
  "voice.mode": {
    ...
    help: "Conversation mode: agent-proxy (default) uses realtime voice as the microphone/speaker for the routed Aether agent, stt-tts uses batch speech-to-text plus TTS, and bidi lets the realtime provider converse directly with the Aether consult tool.",
  },
  "voice.agentSession": {
    label: "Discord Voice Agent Session",
    help: 'Controls which Aether conversation receives voice turns. Leave unset for the voice channel session, or set mode="target" with a Discord target such as channel:123 to make voice an extension of an existing text channel session.',
  },
  ...
  "voice.realtime.model": {
    ...
    help: "Provider realtime session model, such as gpt-realtime-2.1. This is separate from voice.model, which remains the Aether agent brain model.",
  },
  ...
  "voice.realtime.toolPolicy": {
    ...
    help: "Tool policy for the Aether agent consult tool in realtime voice modes: safe-read-only, owner, or none. Default is owner for agent-proxy and safe-read-only for bidi.",
  },
  "voice.realtime.consultPolicy": {
    label: "Discord Realtime Consult Policy",
    help: "Use always to strongly prefer the Aether agent brain for substantive realtime turns. agent-proxy defaults to always.",
  },
  ...
  "voice.autoJoin.*.whenOccupied": {
    ...
    help: "Join and remain in this auto-managed voice channel only while at least one human is present. The Aether bot and other bots do not count. Default: false.",
  },
  ...
  "voice.reconnectGraceMs": {
    ...
    help: "Grace period for a disconnected Discord voice session to enter Signalling or Connecting before Aether destroys it. Default: 15000.",
  },
  "voice.captureSilenceGraceMs": {
    label: "Discord Voice Capture Silence Grace (ms)",
    help: "Silence window after Discord reports a speaker ended before Aether finalizes the audio segment for transcription. Default: 2000.",
  },
  "voice.tts": {
