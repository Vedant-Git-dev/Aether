...
import { resolveConfiguredFromCredentialStatuses } from "aether/plugin-sdk/channel-status";
import type {
  DiscordAccountConfig,
  DiscordActionConfig,
  AetherConfig,
} from "aether/plugin-sdk/config-contracts";
import { resolveAccountEntry } from "aether/plugin-sdk/routing";
...
function inspectDiscordRuntimeAvailability(account: ResolvedDiscordAccount, cfg: AetherConfig) {
...
export function listEnabledDiscordAccounts(cfg: AetherConfig): ResolvedDiscordAccount[] {
  return listDiscordAccountIds(cfg)
    .map((accountId) => resolveDiscordAccount({ cfg, accountId }))
    .filter((account) => isDiscordAccountEnabledForRuntime(account, cfg));
}

export function listDiscordStartupAccountIds(cfg: AetherConfig): string[] {
