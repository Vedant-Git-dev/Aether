...
import { sleepWithAbort } from "aether/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "aether/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: AetherConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Aether adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
