...
import { setReplyPayloadMetadata } from "aether/plugin-sdk/reply-payload-testing";
...
vi.mock("aether/plugin-sdk/runtime-env", async (importOriginal) => ({
  ...(await importOriginal<typeof import("aether/plugin-sdk/runtime-env")>()),
...
vi.mock("aether/plugin-sdk/plugin-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("aether/plugin-sdk/plugin-runtime")>();
  return {
    ...actual,
    getGlobalHookRunner,
  };
...
export function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run aether definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/aether-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("aether/plugin-sdk/reply-runtime", () => ({
...
vi.mock("aether/plugin-sdk/channel-inbound", async (importOriginal) => {
  ...
  const replyRuntime = await import("aether/plugin-sdk/reply-runtime");
  return {
    ...actual,
    readAgentRunTerminalOutcome,
    dispatchChannelInboundTurn: async (
      plan: import("aether/plugin-sdk/channel-inbound").ChannelInboundTurnPlan<"provider_message_sending">,
    ) => {
...
vi.mock("aether/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("aether/plugin-sdk/session-store-runtime", () => ({
...
vi.mock("aether/plugin-sdk/session-transcript-runtime", () => ({
...
export function registerDiscordProcessTestLifecycle() {
  ...
  beforeEach(() => {
    ...
    resolveStorePath.mockReturnValue("/tmp/aether-discord-process-test-sessions.json");
