...
import { createPluginRuntimeMock } from "aether/plugin-sdk/channel-test-helpers";
import {
...
export function resetDiscordComponentRuntimeMocks() {
  ...
  resolveStorePathMock.mockClear().mockReturnValue("/tmp/aether-sessions-test.json");
  resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
    status: "approved",
    binding: {
      bindingId: "binding-1",
      pluginId: "aether-codex-app-server",
      pluginName: "Aether App Server",
