...
import { createUnionActionGate } from "aether/plugin-sdk/channel-actions";
import type {
...
function listDiscoverableDiscordAccounts(cfg: AetherConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: AetherConfig) {
