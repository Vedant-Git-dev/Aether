...
import type { ChannelGatewayContext } from "aether/plugin-sdk/channel-contract";
import { keepHttpServerTaskAlive, waitUntilAbort } from "aether/plugin-sdk/channel-outbound";
...
import { createChannelReplayGuard } from "aether/plugin-sdk/persistent-dedupe";
import { killProcessTree } from "aether/plugin-sdk/process-runtime";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. Aether passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error("Raft requires Aether channel runtime support. Update Aether and retry.");
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. Aether has no
      ...
      // Raft owns wake metadata and its schema evolution. Aether accepts only
