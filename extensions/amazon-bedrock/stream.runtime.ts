/**
 * Amazon Bedrock Converse streaming runtime. It maps Aether messages/tools,
...
} from "aether/plugin-sdk/provider-transport-runtime";
...
// Aether synthesizes these caps when the provider's real output limit is unknown.
// Keep them out of Bedrock adaptive requests so Bedrock can use its native default.
const AETHER_FALLBACK_MODEL_MAX_TOKENS = new Set([4096, 8192, 16_384]);
...
function resolveAdaptiveBedrockMaxTokens(
  ...
  return AETHER_FALLBACK_MODEL_MAX_TOKENS.has(model.maxTokens) ? undefined : model.maxTokens;
}
...
/** Stream a Bedrock Converse request from the generic Aether stream options. */
...
function resolveCacheRetention(
  ...
  if (typeof process !== "undefined" && process.env.AETHER_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}
