...
// Nodes without the Aether app (plain `aether node host run`) advertise
...
export async function handleLogbookSnapshot(rawParams: unknown): Promise<LogbookSnapshotPayload> {
  ...
  // keeps captures out of the broader Aether temp namespace.
  const captureDir = path.join(resolvePreferredAetherTmpDir(), "logbook");
