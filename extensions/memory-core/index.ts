...
// Memory Core plugin entrypoint registers its Aether integration.
import {
  jsonResult,
  type MemoryPluginRuntime,
  type AetherConfig,
} from "aether/plugin-sdk/memory-core-host-runtime-core";
import { resolveMemoryBackendConfig } from "aether/plugin-sdk/memory-core-host-runtime-files";
import {
  definePluginEntry,
  type AnyAgentTool,
  ...
  register(api) {
    ...
    api.registerMemoryCapability({
      ...
      promptBuilder: (params) => {
        ...
        const context = resolveMemoryToolContext({
          ...
          config: liveConfig as AetherConfig,
    ...
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      try {
        ...
        const config = (api.runtime.config?.current?.() ?? api.config) as AetherConfig;
        const agentId = resolveSessionAgentIdStrict({
          sessionKey: ctx.sessionKey,
          config,
          agentId: ctx.agentId,
