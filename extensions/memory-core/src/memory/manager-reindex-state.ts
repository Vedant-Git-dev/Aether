...
export type { MemoryIndexIdentityState } from "aether/plugin-sdk/memory-core-host-engine-storage";
...
function aetherIndexMismatch(
  code: "provenance_version" | "chunking_version",
  reason: string,
): MemoryIndexIdentityState {
  return { status: "mismatched", reason, code, owner: "aether" };
}
...
export function resolveMemoryIndexIdentityState(params: {
  ...
  if (meta.provenanceVersion !== MEMORY_INDEX_PROVENANCE_VERSION) {
    return aetherIndexMismatch("provenance_version", "index provenance classifier changed");
  }
  if (meta.chunkingVersion !== MEMORY_CHUNKING_VERSION) {
    return aetherIndexMismatch("chunking_version", "index chunking implementation changed");
  }
  const expectedModel =
