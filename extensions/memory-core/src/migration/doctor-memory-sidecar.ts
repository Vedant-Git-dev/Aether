...
import { reclaimDefinitelyStaleFileLock } from "aether/plugin-sdk/file-lock";
import { resolveUserPath } from "aether/plugin-sdk/memory-core-host-engine-fs";
...
import { normalizeAgentId } from "aether/plugin-sdk/routing";
import {
...
import { asOptionalObjectRecord as readLegacyObjectRecord } from "aether/plugin-sdk/string-coerce-runtime";
import {
...
async function collectLegacyMemorySidecarSources(params: {
  ...
  const migrationEnv = { ...params.env, AETHER_STATE_DIR: params.stateDir };
  ...
  async function addSource(agentId: string, legacyPath: string): Promise<void> {
    ...
    const { resolveAetherAgentSqlitePath } = await import("aether/plugin-sdk/sqlite-runtime");
    const agentDatabasePath = resolveAetherAgentSqlitePath({
      agentId,
      env: migrationEnv,
    });
    if (
...
async function migrateLegacyMemorySidecarSource(params: {
  ...
  const { ensureMemoryIndexSchema, loadSqliteVecExtension } =
    await import("aether/plugin-sdk/memory-core-host-engine-schema");
  const { ensureAetherAgentDatabaseSchema, openNodeSqliteDatabase } =
    await import("aether/plugin-sdk/sqlite-runtime");
  // Aether itself can leave a zero-byte placeholder at the legacy sidecar
  ...
  try {
    const migrationEnv = {
      ...params.env,
      AETHER_STATE_DIR: params.source.stateDir,
    };
    ensureAetherAgentDatabaseSchema(db, {
  ...
  async migrateLegacyState(params) {
    ...
    for (const home of await collectRetiredQmdWorkspaceHomes(params.stateDir)) {
      try {
        ...
        warnings.push(
          `Skipped retired Memory Core QMD workspace cleanup. Run aether doctor --fix to retry. ${home}: ${String(err)}`,
        );
      }
    }
    return {
