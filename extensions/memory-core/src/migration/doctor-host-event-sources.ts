...
import { root } from "aether/plugin-sdk/memory-core-host-engine-fs";
...
export async function collectLegacyMemoryHostEventSources(
  ...
  const { resolveMemoryHostEventLogPath } = await import("aether/plugin-sdk/memory-host-events");
  ...
  for (const workspaceDir of await resolveConfiguredWorkspaces(config, env)) {
    ...
    try {
      ...
      sources.push({
        ...
        workspaceDir: canonicalWorkspaceDir,
        filePath,
        reason: `Skipped unsafe Memory Core host event source ${filePath}: ${String(error)}. Check permissions and use regular files and directories inside the workspace, then rerun aether doctor --fix. For shared notes, use canonical paths in memory.search.extraPaths; this does not migrate legacy events.`,
      });
    }
  }
  return sources;
