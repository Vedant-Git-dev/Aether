...
import type { normalizeMemoryHostEventRecordForStorage } from "aether/plugin-sdk/memory-host-events";
import type {
...
async function migrateLegacyMemoryHostEventSource(params: {
  ...
  const { normalizeMemoryHostEventRecordForStorage, resolveMemoryHostEventLogPath } =
    await import("aether/plugin-sdk/memory-host-events");
  ...
  try {
    ...
    if (source.storage !== "archive" && (await source.root.exists(activeRelativePath))) {
      params.warnings.push(
        "An old writer recreated the Memory Core host event source; rerun aether doctor --fix to import the retained rows",
      );
    }
    return "completed";
  } finally {
