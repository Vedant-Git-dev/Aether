...
import {
  ...
  type AetherConfig,
} from "aether/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
  listSessionTranscriptCorpusEntriesForAgent,
  parseUsageCountedSessionIdFromFileName,
  resolveMemorySessionTargets,
} from "aether/plugin-sdk/memory-core-host-engine-sessions";
import {
  isFileMissingError,
  listMemoryFiles,
  loadSqliteVecExtension,
} from "aether/plugin-sdk/memory-core-host-engine-storage";
import { listMemoryArtifactProvenance } from "aether/plugin-sdk/memory-core-host-runtime-core";
import { resolveStorePath } from "aether/plugin-sdk/session-store-paths";
import {
...
} from "aether/plugin-sdk/sqlite-runtime";
...
const PROMOTION_MARKER = /^\s*<!--\s*aether-memory-promotion:([^\n]*?)\s*-->\s*$/u;
const LINEAGE_MARKER = /^\s*<!--\s*aether-memory-lineage:[^\n]*?-->\s*$/u;
...
async function planMemoryIndex(params: {
  ...
  if (result.value.hasVectorTable && result.value.chunks.length > 0) {
    ...
    const vectorResult = withAetherAgentDatabaseReadOnly(
      ({ db }) => {
...
async function forgetWorkspaceMemory(
  ...
  const { db, release } = borrowAetherAgentDatabase({ agentId: params.agentId });
  try {
    const kysely = getNodeSqliteKysely<ForgetDatabase>(db);
