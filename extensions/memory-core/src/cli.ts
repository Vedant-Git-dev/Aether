...
import {
  ...
  formatHelpExamples,
  theme,
} from "aether/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aether memory status", "Show index and provider status."],
          [
            "aether memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["aether memory status --deep", "Probe embedding provider readiness."],
          ["aether memory index --force", "Force a full reindex."],
          ['aether memory search "meeting notes"', "Quick search using positional query."],
          [
            'aether memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            "aether memory forget --hook-source gmail --dry-run",
            "Preview deletion of memories derived from matching sessions.",
          ],
          [
            `aether memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "aether memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'aether memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "aether memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "aether memory rem-backfill --path ./memory",
            "Write grounded historical REM entries into DREAMS.md for UI review.",
          ],
          [
            "aether memory rem-backfill --path ./memory --stage-short-term",
            "Also seed durable grounded candidates into the live short-term promotion store.",
          ],
          [
            "aether memory session-backfill --agent main --from 2026-01-01",
            "Preview trusted candidates from retained session history.",
          ],
          ["aether memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.aether.ai/cli/memory")}\n`,
    );
