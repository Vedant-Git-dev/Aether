...
import { resolveMemoryDreamingConfig } from "aether/plugin-sdk/memory-core-host-status";
import type { AetherPluginApi, PluginCommandContext } from "aether/plugin-sdk/plugin-entry";
import {
  asNullableRecord,
  normalizeLowercaseStringOrEmpty,
} from "aether/plugin-sdk/string-coerce-runtime";

function resolveDreamingPluginConfig(cfg: AetherConfig): Record<string, unknown> {
  const entry = asNullableRecord(cfg.plugins?.entries?.["memory-core"]);
  return asNullableRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: AetherConfig, enabled: boolean): AetherConfig {
...
function formatStatus(cfg: AetherConfig): string {
...
export async function handleDreamingCommand(api: AetherPluginApi, ctx: PluginCommandContext) {
