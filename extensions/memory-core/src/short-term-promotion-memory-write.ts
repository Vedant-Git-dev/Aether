...
export function buildPromotionMarker(candidateKey: string): string {
  return `<!-- aether-memory-promotion:${candidateKey} -->`;
}
...
export function extractPromotionKeys(content: string): string[] {
  ...
  return [...content.matchAll(/<!--\s*aether-memory-promotion:([^\n]*?)\s*-->/giu)]
  ...
  try {
    await replaceFileAtomic({
      ...
      beforeRename: async () => {
        ...
        // Aether writers are serialized. The recoverable preimage covers the
