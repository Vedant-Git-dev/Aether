...
export async function runMemoryReset(opts: MemoryResetCommandOptions): Promise<void> {
  ...
  for (const agentId of agentIds) {
    const dbPath = resolveAetherAgentSqlitePath({ agentId });
    ...
    try {
      assertAetherAgentDatabaseForMaintenance(db, { agentId, pathname: dbPath });
      ...
      defaultRuntime.log(
        changed
          ? `Memory index reset (${agentId}). Sessions preserved. Rebuild with: aether memory index --agent ${agentId}`
          : `No memory index to reset (${agentId}).`,
      );
      defaultRuntime.log(
        `Reset does not shrink the database file. To reclaim space, back up data and stop the Gateway and other writers, then run: aether doctor --session-sqlite compact --session-sqlite-agent ${agentId}`,
      );
    } finally {
