...
import { formatByteSize } from "aether/plugin-sdk/number-runtime";
...
function formatDreamingSummary(cfg: AetherConfig): string {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        if (audit.issues.some((issue) => issue.fixable)) {
          lines.push(`  ${muted(`Fix: aether memory status --fix --agent ${agentId}`)}`);
        }
      }
    }
