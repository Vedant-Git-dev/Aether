...
export function resolveMemoryPluginConfig(cfg: AetherConfig): Record<string, unknown> {
...
export function resolveMemoryAgent(cfg: AetherConfig, agent?: string) {
...
export function resolveMemoryAgentIds(cfg: AetherConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (agent !== undefined && !trimmed) {
    throw new Error("--agent must not be blank");
  }
...
export async function withMemoryCommand(params: {
  ...
  run: (context: { manager: MemoryManager; cfg: AetherConfig; agentId: string }) => Promise<void>;
