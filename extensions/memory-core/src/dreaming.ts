...
import { resolveMemoryDreamingPluginConfig } from "aether/plugin-sdk/memory-core-host-runtime-core";
import {
...
import { peekSystemEventEntries } from "aether/plugin-sdk/system-event-runtime";
...
async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: AetherPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: AetherPluginApi): void {
  ...
  const resolveCurrentConfig = (): AetherConfig =>
