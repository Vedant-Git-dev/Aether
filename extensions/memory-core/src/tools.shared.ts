...
import type {
  AnyAgentTool,
  AetherConfig,
} from "aether/plugin-sdk/memory-core-host-runtime-core";
...
const SESSION_CANONICAL_KEY_MIGRATION_ACTION =
  "Stop the Gateway and run aether doctor --fix, then restart the Gateway and retry memory_search.";
...
export function buildMemorySearchUnavailableResult(
  ...
  const deadlineAction = overrides?.agentId
    ? `Retry memory_search after a short wait: a memory-corpus timeout pauses retries for up to a minute. If memory-corpus timeouts persist, run: aether memory status --deep --agent ${overrides.agentId}, and rebuild with aether memory index --force --agent ${overrides.agentId} only if it reports the index dirty or incomplete`
  ...
  const warning =
    ...
    (overrides?.code === SESSION_CANONICAL_KEY_MIGRATION_REQUIRED
      ...
      : isQuotaError
        ...
        : isMissingNodeSqlite
          ? "Memory search is unavailable because this Aether Node runtime does not provide SQLite support."
          : isSearchDeadline
  ...
  const action =
    ...
    (overrides?.code === SESSION_CANONICAL_KEY_MIGRATION_REQUIRED
      ...
      : isQuotaError
        ...
        : isMissingNodeSqlite
          ? "Run Aether with a Node runtime that includes node:sqlite, then retry memory_search."
          : isSearchDeadline
            ? deadlineAction
