...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__aether_channel_id";
...
type InteractionCallbackConfig = Pick<AetherConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "aether-mattermost-interactions").update(botToken).digest("hex");
}
