...
} from "aether/plugin-sdk/embedding-providers";
...
type AcquireLocalService = AetherPluginApi["runtime"]["llm"]["acquireLocalService"];
type LocalServiceAwareOptions = EmbeddingProviderCreateOptions & {
  acquireLocalService?: AcquireLocalService;
};

const LOCAL_EMBEDDING_RUNTIME_FACTS = Symbol.for("aether.localEmbeddingRuntimeFacts");
...
export const llamaCppEmbeddingProviderAdapter: EmbeddingProviderAdapter = {
  ...
  formatSetupError: (error) =>
    `Managed local embeddings are unavailable. Run \`aether configure\`, choose llama.cpp, and retry. ${error instanceof Error ? error.message : String(error)}`,
