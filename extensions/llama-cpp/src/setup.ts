  ...
  if (recommendation.kind === "recommended") {
    ...
    const recommendationSummary = [
      ...
      !embeddingModelIsDefault
        ...
        : undefined,
      "Aether will check a real tool call before making this your default model.",
    ]
      .filter(Boolean)
      .join("\n");
...
export async function runLlamaCppSetup(ctx: ProviderAuthContext): Promise<ProviderAuthResult> {
  ...
  try {
    ...
    throw new Error(
      `Managed llama.cpp setup failed. Run aether doctor, fix the reported runtime or model issue, then retry. ${detail}`,
      { cause: error },
    );
  }
}
