# @aether/llama-cpp-provider

Official llama.cpp provider for managed and external Aether model servers.

The `llama-cpp` provider either installs a pinned, integrity-verified
`llama-server` under Aether's `localService` supervisor or connects to a
server that you already operate. Both choices use `llama-cpp/<model>` references
and Aether's normal OpenAI-compatible chat transport. Local embeddings require
the managed choice.

## Install

```bash
aether plugins install @aether/llama-cpp-provider
```
...
After explicit consent, Aether installs the matching server build and a
...
declined, Aether offers a separate embedding-only setup. After explicit
...
setup after changing the model so Aether can verify the file and regenerate
the managed router preset.

See the [llama.cpp provider guide](https://docs.aether.ai/plugins/llama-cpp)
...
optional API key. Aether passively discovers single-model and router catalogs.
...
- Package: `@aether/llama-cpp-provider`
- Minimum Aether host: `2026.6.2`
