// Voice Call plugin entrypoint registers its Aether integration.
...
import { ErrorCodes, errorShape } from "aether/plugin-sdk/gateway-runtime";
...
import { jsonResult as json } from "aether/plugin-sdk/tool-results";
...
const VoiceCallToolSchema = Type.Union([
  Type.Object({
    ...
    sessionKey: Type.Optional(Type.String({ description: "Aether session key for the call" })),
...
function isCliOnlyProcess(): boolean {
  return process.env.AETHER_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_COORDINATOR_KEY = Symbol.for("aether.voice-call.runtimeCoordinator");

type VoiceCallRuntimeGeneration = {
  retired: boolean;
  serviceHealth?: Parameters<
    Parameters<AetherPluginApi["registerService"]>[0]["start"]
  >[0]["serviceHealth"];
};
...
export default definePluginEntry({
  ...
  register(api: AetherPluginApi) {
    ...
    const continueOperationStore = createVoiceCallContinueOperationStore({
      config,
      coreConfig: api.config as AetherConfig,
    });
