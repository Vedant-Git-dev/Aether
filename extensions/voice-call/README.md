...
Official Voice Call plugin for **Aether**.
...
Docs: `https://docs.aether.ai/plugins/voice-call`
Plugin system: `https://docs.aether.ai/tools/plugin`

## Install

```bash
aether plugins install @aether/voice-call
```
...
- Runtime accepts canonical config only. If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `aether doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.aether.ai/plugins/voice-call`
...
- `realtime.consultThinkingLevel` is optional. When set, it overrides the thinking level used by the model behind realtime `aether_agent_consult` calls.
- `realtime.consultFastMode` is optional. When set, it toggles fast mode for realtime `aether_agent_consult` calls.

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.aether.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.aether.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
aether voicecall call --to "+15555550123" --message "Hello from Aether"
aether voicecall continue --call-id <id> --message "Any questions?"
aether voicecall speak --call-id <id> --message "One moment"
aether voicecall end --call-id <id>
aether voicecall status --json
aether voicecall status --call-id <id>
aether voicecall tail
aether voicecall expose --mode funnel
```
