...
function getVoiceCallConfigStore(config: PluginDoctorStateMigrationParams["config"]): string {
  for (const pluginId of ["voice-call", "@aether/voice-call"]) {
...
export function resolveSessionStoreAgentIds(params: { cfg: AetherConfig }): string[] {
...
function resolveVoiceCallStateDatabaseEnv(
  ...
  return {
    ...params.env,
    AETHER_STATE_DIR: resolveVoiceCallStorePath(params),
  };
}

function describeVoiceCallSchemaMigration(migration: AetherStateDatabaseSchemaMigration): string {
...
export const stateMigrations: PluginDoctorStateMigration[] = [
  {
    ...
    async detectLegacyState(params) {
      ...
      const schemaMigrations = detectAetherStateDatabaseSchemaMigrations({
    ...
    async migrateLegacyState(params) {
      ...
      if (schemaMigrations.length > 0) {
        const repaired = repairAetherStateDatabaseSchema({ env: stateDatabaseEnv });
