...
import { mergeDeep } from "aether/plugin-sdk/plugin-config-runtime";
import { REALTIME_VOICE_AGENT_CONSULT_TOOL_POLICIES } from "aether/plugin-sdk/realtime-voice";
...
import { resolveSpeechProviderApiKey } from "aether/plugin-sdk/speech-core";
...
const VoiceCallRealtimeConfigSchema = z
  .object({
    ...
    /** Tool policy for the shared Aether agent consult tool. */
    toolPolicy: VoiceCallRealtimeToolPolicySchema.default("safe-read-only"),
    /** Guidance for when the realtime model should call the Aether agent consult tool. */
