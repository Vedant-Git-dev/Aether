...
import { isLoopbackHost } from "aether/plugin-sdk/gateway-runtime";
...
} from "aether/plugin-sdk/realtime-voice";
...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Aether agent receiving delegated requests from a live phone voice bridge.",
...
function resolveVoiceCallConsultSessionKey(call: {
  config: VoiceCallConfig;
  coreSession?: AetherConfig["session"];
  sessionKey?: string;
  from?: string;
  to?: string;
...
function listRealtimeAgentIds(config: VoiceCallConfig, coreConfig: AetherConfig): string[] {
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as AetherConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as AetherConfig),
    agentRuntime,
    log,
  );
