...
import { normalizeWebhookPath } from "aether/plugin-sdk/webhook-ingress";
...
const FORCED_CONSULT_REASON = "provider_final_transcript_without_aether_agent_consult";
...
function buildForcedConsultSpeechPrompt(result: string): string {
  ...
  return [
    "Internal Aether consult result is ready.",
...
export class RealtimeCallHandler {
  ...
  constructor(
    ...
    private readonly coreConfig?: AetherConfig,
  ) {}
    ...
    if (name === REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME) {
      ...
      if (forcedMatch.kind === "already_delivered" && coordinator.isCancelled(forcedMatch.handle)) {
        ...
        await submitFinalToolResult({
          status: "cancelled",
          message: "Aether cancelled this consult before completion. Do not restart it.",
        });
        return;
      }
      if (forcedConsult) {
        if (forcedConsult.completedAt || forcedMatch.kind === "already_delivered") {
          await submitFinalToolResult({
            ...
            message: "Aether already delivered this consult result internally. Do not repeat it.",
          });
          return;
        }
        forcedConsult.sendSpeechPrompt = false;
