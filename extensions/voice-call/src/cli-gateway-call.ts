...
function gatewayOperationalError(err: unknown): Error {
  ...
  const detail = (() => {
    if (isGatewayClientRequestError(err)) {
      return `Gateway responded but voicecall failed: ${message}\nThe running Gateway owns the voice-call runtime; check \`aether gateway status\` or restart it.`;
    }
    if (isGatewayCredentialFailure(err)) {
      return `Gateway requires credentials: ${message}\nConfigure gateway.auth or pair this device with \`aether devices approve --latest\`.`;
    }
...
async function ensureStandaloneRuntime(params: {
  ...
  try {
    ...
    if (err instanceof Error && "code" in err && err.code === "EADDRINUSE") {
      throw new Error(
        `Voice-call webhook port ${params.config.serve.port} is already in use. A running Gateway probably already serves it; operational commands route through that Gateway. Check \`aether gateway status\` and retry.`,
        { cause: err },
      );
    }
    throw err;
