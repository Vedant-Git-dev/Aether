...
import { canonicalPathFromExistingAncestor } from "aether/plugin-sdk/security-runtime";
...
const MATERIALIZED_SKILLS_REMOTE_PARTS = [".aether", "sandbox-skills"] as const;
...
class OpenShellSandboxBackendImpl {
  ...
  private buildLegacyRuntimeUnavailableError(detail: string): Error {
    const recreateCommand = `aether sandbox recreate --session ${shellEscape(this.params.createParams.scopeKey)}`;
    return new Error(
      [
        `Registered legacy OpenShell sandbox "${this.params.execContext.sandboxName}" is not usable.`,
        detail,
        `Aether will not recreate this retired runtime name. Run \`${recreateCommand}\` to migrate this scope to the current naming format.`,
      ]
        .filter(Boolean)
        .join(" "),
    );
  ...
  private async syncWorkspaceFromRemote(): Promise<void> {
    ...
    for (const [index, root] of roots.entries()) {
      ...
      await withTempWorkspace(
        { rootDir: resolveOpenShellTmpRoot(), prefix: "aether-openshell-sync-" },
        async ({ dir: tmpDir }) => {
          const result = await runOpenShellCli({
            context: this.params.execContext,
  ...
  private async uploadPathToRemote(localPath: string, remotePath: string): Promise<void> {
    await withTempWorkspace(
      { rootDir: resolveOpenShellTmpRoot(), prefix: "aether-openshell-upload-" },
      async ({ dir: tmpDir }) => {
...
function buildLegacyOpenShellSandboxName(scopeKey: string): string {
  ...
  return `aether-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
...
async function moveLocalShadowAside(params: {
  ...
  const preserveRoot = await fs.mkdtemp(
    path.join(path.dirname(params.tmpDir), "aether-openshell-preserve-"),
  );
...
function resolveOpenShellTmpRoot(): string {
  return path.resolve(resolvePreferredAetherTmpDir());
}
