...
import { resolveAcpSessionAvailability } from "aether/plugin-sdk/acp-runtime";
...
import { resolveNodeHostExecutable } from "aether/plugin-sdk/node-host";
...
} from "aether/plugin-sdk/session-catalog";
...
function currentOpenCodeCatalogConfig(api: AetherPluginApi): AetherConfig {
  return (api.runtime.config?.current?.() ?? api.config ?? {}) as AetherConfig;
}
...
function createOpenCodeNodeHostBindings(api: AetherPluginApi) {
...
export function registerOpenCodeSessionCatalog(api: AetherPluginApi): void {
