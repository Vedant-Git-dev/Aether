...
export class VisitorAccessService {
      ...
      response = await this.fetcher(
        `https://api.github.com/users/${encodeURIComponent(input.github)}`,
        {
          headers: {
            ...
            "User-Agent": "Aether-visitor-access",
          },
          redirect: "error",
          signal: this.signal
  ...
  invite(raw: unknown, invitedVia?: string): Promise<string> {
    return this.serialize(async () => {
      ...
      return `Invited ${who}. Expires: ${expiryText(grant.expiresAt)}. Log in at https://team.aether.ai with your GitHub account using this verified email.`;
    });
  }
