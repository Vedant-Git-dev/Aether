...
async function authorizeTool(
  ...
  if (!turn || signal.aborted || typeof toolName !== "string" || !isRecord(input)) {
    return {
      behavior: "deny",
      message: "The Aether run or native tool input is no longer valid.",
    };
  }
  ...
  try {
    ...
    if (activeTurn(session) !== turn || abortSignal.aborted) {
      return { behavior: "deny", message: "The Aether run is no longer active." };
    }
    return decision;
  } catch {
    return { behavior: "deny", message: "Aether could not authorize this tool call." };
  }
}
...
async function handleRequest(
  ...
  if (request.callback_id === "PreToolUse" && input.hook_event_name === "PreToolUse") {
    if (
      ...
      input.tool_name.startsWith("mcp__aether__")
    ) {
      ...
      session.transport = createClaudeCliTransport({
        ...
        onRequest: async (request, signal) => {
          ...
          return () => {
            ...
            const message = "The Aether run is no longer active.";
            if (request.subtype === "can_use_tool") {
