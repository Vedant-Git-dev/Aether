...
export function prepareClaudeCliTransportArgs(context: CliBackendExecuteContext) {
  ...
  for (let index = 0; index < context.args.length; index += 1) {
    ...
    if (flag === "--tools") {
      ...
      allowedTools.push(...names.filter((name) => name.startsWith("mcp__aether__")));
    }
  }
  let approvedTools = [...new Set(allowedTools)];
  if (context.toolAvailability) {
    tools = [...context.toolAvailability.native];
    approvedTools = context.toolAvailability.aether
      .map((name) => `mcp__aether__${name}`)
      .filter((name) => allowedTools.includes(name) || allowedTools.includes("mcp__aether__*"));
  }
  args.push(
    "--print",
    "--input-format",
