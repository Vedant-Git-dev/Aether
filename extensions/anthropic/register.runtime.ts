 ...
 */
import { createLazyRuntimeMethod, createLazyRuntimeModule } from "aether/plugin-sdk/lazy-runtime";
import type {
  AetherPluginApi,
  ProviderAuthContext,
...
} from "aether/plugin-sdk/provider-catalog-shared";
...
const buildOpenAICompatibleProviderCatalog = createLazyRuntimeMethod(
  createLazyRuntimeModule(() => import("aether/plugin-sdk/provider-catalog-live-runtime")),
 ...
 * shaping is selected by version predicates in `@aether/llm-core`, so such an
...
export function registerAnthropicPlugin(api: AetherPluginApi): void {
