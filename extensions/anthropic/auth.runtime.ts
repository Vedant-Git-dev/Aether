...
import { formatCliCommand, parseDurationMs } from "aether/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "aether/plugin-sdk/number-runtime";
import type {
  ProviderAuthContext,
  ProviderAuthMethod,
...
import {
  ...
  type AetherConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  validateAnthropicSetupToken,
} from "aether/plugin-sdk/provider-auth";
import { upsertAuthProfileWithLockOrThrow } from "aether/plugin-sdk/provider-auth-api-key";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  "Anthropic setup-token auth is supported in Aether.",
  "Aether prefers the native Claude CLI runtime when it is available on the host.",
  "Anthropic staff told us this Aether path is allowed again.",
  `If you want a direct API billing path instead, use ${formatCliCommand("aether models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("aether models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
export function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("aether doctor --yes")}"`,
  ].join("\n");
}
