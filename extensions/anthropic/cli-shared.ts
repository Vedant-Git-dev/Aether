...
import { requiresClaudeMandatoryAdaptiveThinking } from "aether/plugin-sdk/claude-model-runtime";
/**
...
import { resolveExecModePolicy } from "aether/plugin-sdk/exec-approvals-runtime";
...
/** Map Aether's effective context budget to Claude Code's native compactor. */
...
/**
 * Map Aether's fixed thinking levels to Claude Code's per-process budget.
 *
 ...
 * These fixed budgets match Aether's canonical provider defaults in
...
function isAetherRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
function resolveClaudeCliRestrictedExecutionArgs(
  ...
  if (availability.aether.length > 0) {
    normalized.push(
      CLAUDE_ALLOWED_TOOLS_ARG,
      availability.aether.map((toolName) => `${AETHER_MCP_TOOL_PREFIX}${toolName}`).join(","),
    );
  }
  const denials = [
    ...new Set([
      ...preservedDenials.map((entry) => entry.trim()).filter(Boolean),
      ...(availability.aether.length === 0 ? [CLAUDE_DENY_MCP_TOOLS_VALUE] : []),
    ]),
  ].toSorted();
  if (denials.length > 0) {
...
export function normalizeClaudeBackendConfig(
  ...
  const permissionMode = isAetherRequestedYolo(context)
