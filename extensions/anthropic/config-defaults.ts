import { listAgentIds, resolveAgentConfig } from "aether/plugin-sdk/agent-scope-runtime";
/**
...
function usesRetiredClaudeCliProviderEntry(config: AetherConfig): boolean {
...
function usesClaudeCliModelSelection(config: AetherConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: AetherConfig): boolean {
  if (usesRetiredClaudeCliProviderEntry(config)) {
    return true;
  }
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: AetherConfig): string[] {
...
/** Apply Anthropic and Claude CLI defaults to an Aether config object. */
