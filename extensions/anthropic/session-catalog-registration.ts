...
import type { SessionCatalogProvider } from "aether/plugin-sdk/session-catalog";
...
function currentConfig(api: AetherPluginApi): AetherConfig {
  return (api.runtime.config?.current?.() ?? api.config ?? {}) as AetherConfig;
}

function registerClaudeSessionCatalog(api: AetherPluginApi): void {
...
function createClaudeSessionNodeHostCommands(): AetherPluginNodeHostCommand[] {
  return [
    {
...
export function createClaudeSessionNodeInvokePolicies(): AetherPluginNodeInvokePolicy[] {
  return [
    {
      commands: [
        CLAUDE_SESSIONS_LIST_COMMAND,
...
export function registerClaudeSessionDiscovery(api: AetherPluginApi): void {
