...
function resolveClaudeCliAuthInput(
  ...
  // Forwarded OAuth here is Aether-managed material (its refresh path is
  // Aether-owned). Native `claude` logins are never forwarded; the current
  // Claude process reads its own config directory. An expired token here is
  // therefore Aether-managed state that must fail loudly.
  if (credential?.type === "oauth" && "access" in credential) {
    ...
    if (typeof expires !== "number" || !Number.isFinite(expires) || expires <= Date.now()) {
      throw new Error(
        "Selected Claude CLI OAuth credential is expired or invalid. Re-authenticate the selected profile and retry. Aether did not start the run.",
      );
    }
    ...
    prepareExecution: (context) => {
      const prepare = () => {
        ...
        const env = {
          ...
          // the conversation prefix after workspace edits or commits. Aether
