...
/** Retired Aether auth profile replaced by Claude CLI's native login. */
...
/** Non-secret marker telling Aether that the installed Claude CLI owns auth. */
export const CLAUDE_CLI_NATIVE_AUTH_MARKER = ["aether", "claude-cli-native-auth"].join(":");
...
// steer Aether-managed Claude CLI runs toward a different provider,
...
/** Environment variables removed before launching Aether-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  // Re-injected per run from Aether's canonical context budget.
  ...
  // Re-injected per run from Aether's effective thinking level.
  "MAX_THINKING_TOKENS",
