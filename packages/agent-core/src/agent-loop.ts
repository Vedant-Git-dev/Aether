...
const TOOL_LOOP_RECOVERY_TERMINATED_MESSAGE =
  "Aether stopped this run because tool-loop recovery encountered another critical loop. No blocked tool action was executed.";
...
function readTurnTaintMetadata(message: AgentMessage): TurnTaintMetadata | undefined {
  const metadata = Reflect.get(message, "__aether");
...
function withAssistantTurnTaint(message: AssistantMessage, tainted: boolean): AssistantMessage {
  ...
  const taintedMessage = {
    ...message,
    __aether: { ...readTurnTaintMetadata(message), turnTainted: true },
  } satisfies AssistantMessage & { __aether: TurnTaintMetadata };
...
function withToolResultContentSource(
  ...
  return {
    ...message,
    __aether: { ...readTurnTaintMetadata(message), resultContentSource: source },
