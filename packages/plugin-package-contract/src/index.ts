...
export const EXTERNAL_CODE_PLUGIN_REQUIRED_FIELD_PATHS = [
  "aether.compat.pluginApi",
  "aether.build.aetherVersion",
] as const;
...
/** Read Aether package.json blocks without trusting caller input shape. */
function readAetherBlock(packageJson: unknown) {
  const root = isRecord(packageJson) ? packageJson : undefined;
  const aether = isRecord(root?.aether) ? root.aether : undefined;
  const compat = isRecord(aether?.compat) ? aether.compat : undefined;
  const build = isRecord(aether?.build) ? aether.build : undefined;
  const install = isRecord(aether?.install) ? aether.install : undefined;
  return { root, aether, compat, build, install };
}
...
export function normalizeExternalPluginCompatibility(
  ...
  const { root, compat, build, install } = readAetherBlock(packageJson);
  ...
  const builtWithAetherVersion = normalizeOptionalString(build?.aetherVersion) ?? version;
  if (builtWithAetherVersion) {
    compatibility.builtWithAetherVersion = builtWithAetherVersion;
  }
...
export function listMissingExternalCodePluginFieldPaths(packageJson: unknown): string[] {
  const { compat, build } = readAetherBlock(packageJson);
  const missing: string[] = [];
  if (!normalizeOptionalString(compat?.pluginApi)) {
    missing.push("aether.compat.pluginApi");
  }
  if (!normalizeOptionalString(build?.aetherVersion)) {
    missing.push("aether.build.aetherVersion");
  }
  return missing;
}
