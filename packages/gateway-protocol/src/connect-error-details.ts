...
const PAIRING_APPROVAL_REMEDIATION =
  "Run `aether devices approve --latest` to preview the pending request, then rerun the printed " +
  "`aether devices approve <requestId>` command and reconnect (pass the same --url and " +
  "--token/--password flags if you connected with explicit credentials).";
const DEVICE_TOKEN_REMEDIATION =
  "Rotate the paired-device token with `aether devices rotate --device <deviceId> --role operator`, then reconnect.";
const SHARED_TOKEN_REMEDIATION =
  "Verify `gateway.remote.token` matches `gateway.auth.token`. If a paired-device token is stale, " +
  "rotate it with `aether devices rotate --device <deviceId> --role operator`, then reconnect.";
const SCOPE_MISMATCH_REMEDIATION =
  "Review approved scopes with `aether devices list`; if an upgrade is pending, preview it with " +
  "`aether devices approve --latest`, approve the printed request, then reconnect.";
...
const IDENTITY_PROXY_REMEDIATION =
  "An identity-aware proxy rejected the WebSocket upgrade. Configure gateway.remote.edgeAuth for the configured Gateway origin, then reconnect. See https://docs.aether.ai/gateway/remote#gateway-behind-an-identity-aware-proxy.";
