...
- Add Aether SDK package and authenticated iOS background presence beacon (#73330).
...
- Add system-agent conversational onboarding (#99935); rename `crestodian.*` methods to
  `aether.chat` / `aether.setup.*` (2026-07-14, `a6a0716`).
...
- Add paginated `aether.changes.list` with typed change sources and summaries (#111286).
...
- Rename structured-question item `id` to `questionId`, flatten keyed answer arrays, and cap input headers at 12 characters; rename catalog `aetherSessionKey` to `sessionKey`, make cursor maps optional, and type the health snapshot (#111041).
...
- Extend worker RPCs with `worker.sessions.spawn` / `send`, `worker.github.publish`, and `worker.portal`, negotiated by `worker-session-tools-v1`, `worker-github-publication-v1`, and `worker-portal-v1` (#121846, #126306, #130105); add public `/__aether__/worker` ingress and `admission-rejected` close reason (#122578).
