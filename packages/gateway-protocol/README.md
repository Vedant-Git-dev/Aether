...
See the [Gateway protocol specification](https://docs.aether.ai/gateway/protocol)
for transport, authentication, roles, scopes, and complete frame examples.

## Versioning

Package versions follow the Aether calendar release train:
`YYYY.M.PATCH`, with the same prerelease suffix when applicable. A package version
therefore identifies the Aether source release that produced the schemas; it is
...
[installation guide](https://docs.aether.ai/gateway/clients#install-the-packages)
...
`aether` CLI has its own package versions and dist-tags.
