...
import { requireApiKey, resolveApiKeyForProvider } from "./aether-runtime-auth.js";
...
function resolveAetherAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.AETHER_VERSION?.trim() : undefined;
  return {
    originator: "aether",
    ...(version ? { version } : {}),
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    entries.push(...Object.entries(resolveAetherAttributionHeaders()));
  }
