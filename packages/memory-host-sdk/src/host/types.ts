...
export type MemorySessionSyncTarget = {
  /** Owning Aether agent. Omit only when the active manager scope already supplies it. */
  agentId?: string;
...
export function resolveMemoryIndexIdentityDiagnostic(
  ...
  if (
    ...
    identity.owner === "aether"
  ) {
    return { status: "missing", reason, code: "metadata_missing", owner: "aether" };
  }
  ...
  if (
    ...
    return { status: "mismatched", reason, code: identity.code, owner: "aether" };
  }
  if (
...
export function formatMemoryIndexRebuildGuidance(
  ...
  const command = `aether memory status --index${agentId?.trim() ? ` --agent ${agentId.trim()}` : ""}`;
