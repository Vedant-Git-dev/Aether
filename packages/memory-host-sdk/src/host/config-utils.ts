...
import {
  ...
  resolveStateDir,
  resolveUserPath,
} from "./aether-runtime-paths.js";
import type { MemoryExtraPath } from "./types.js";
export { normalizeAgentId };

// Shared Aether config helpers used by memory host and agent context code.
...
/** Narrow Aether config shape consumed by memory host utilities. */
...
export function resolveRememberAcrossConversations(cfg: AetherConfig, agentId: string): boolean {
...
function listAgentEntries(cfg: AetherConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: AetherConfig): string {
...
function resolveAgentConfig(cfg: AetherConfig, agentId: string): AgentConfig | undefined {
