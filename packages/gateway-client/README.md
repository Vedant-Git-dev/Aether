...
[Gateway protocol specification](https://docs.aether.ai/gateway/protocol) for
...
Package versions follow the Aether calendar release train: `YYYY.M.PATCH`,
including the Aether prerelease suffix when applicable. The package version is
...
Test it with the Gateway version you deploy; the root `aether` CLI has its own
...
storage through `GatewayClientHostDeps`; the package does not load Aether's
...
const client = new GatewayClient({
  ...
  token: process.env.AETHER_GATEWAY_TOKEN,
...
[device identity and pairing](https://docs.aether.ai/gateway/protocol#device-identity-and-pairing).
...
[Gateway protocol specification](https://docs.aether.ai/gateway/protocol#client-constants).
