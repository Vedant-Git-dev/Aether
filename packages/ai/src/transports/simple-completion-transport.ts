...
import {
  ...
  createAetherTransportStreamFnForModel,
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "aether-provider-simple:";
const PROVIDER_STREAM_API_PREFIX = "aether-provider-stream:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `aether-anthropic-vertex-simple:${suffix}`;
}
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use Aether's transport so resolved request auth is applied.
  ...
  const streamFn = createAetherTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
