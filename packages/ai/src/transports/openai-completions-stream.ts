...
export async function processCompletionsStream(
  ...
  const beginReasoning = (hasFollowingVisibleText: boolean, forceStrict = false) => {
    if (!output.aetherDelivery?.textPhaseRequiresTerminal) {
      output.aetherDelivery = {
        ...output.aetherDelivery,
  ...
  const guardedStream = withFirstStreamEventTimeout(responseStream as AsyncIterable<unknown>, {
    ...
    hint: "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable AETHER_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  });
