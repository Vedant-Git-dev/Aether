...
import type { AssistantMessage, Context, Model, StreamFn } from "@aether/llm-core";
    ...
    void (async () => {
      ...
      try {
        ...
        const asyncToolExecutionEligible =
          ...
          !responsesOptions?.aetherCodeModeToolSurface;
        const prepareRequest = async (request: ReturnType<typeof config.buildRequest>) => {
          ...
          if (
            (options as { aetherCodeModeToolSurface?: unknown } | undefined)
              ?.aetherCodeModeToolSurface === true
          ) {
            const visibleToolNames = resolveCodeModeResponsesVisibleToolNames(context);
            const allowedHostedToolTypes = responsesOptions?.aetherCodeModeAllowedHostedToolTypes;
            enforceCodeModeResponsesToolSurface(
              params,
              visibleToolNames,
