 ...
 *
 * Routes models that need Aether-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
import type { Api, Model, StreamFn } from "@aether/llm-core";
...
const SIMPLE_TRANSPORT_API_ALIAS: Record<string, Api> = {
  "openai-completions": "aether-openai-completions-transport",
  "anthropic-messages": "aether-anthropic-messages-transport",
  "google-generative-ai": "aether-google-generative-ai-transport",
};
...
function hasAetherTransportRequirement(model: Model): boolean {
  return getAiTransportHost().requiresManagedTransport(model);
}

/** Returns whether Aether has a managed transport implementation for this API. */
...
export function resolveTransportAwareSimpleApi(api: Api): Api | undefined {
  if (OPENAI_RESPONSES_APIS.has(api)) {
    const alias = `aether-${api}-transport` as Api;
...
export function createTransportAwareStreamFnForModel(
  ...
  if (!hasAetherTransportRequirement(model)) {
    return undefined;
  }
...
/** Creates a managed Aether transport stream for explicit fallback/runtime callers. */
export function createAetherTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Aether's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same Aether request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
