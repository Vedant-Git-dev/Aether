...
Aether. The package supports isolated runtime instances; importing it does not
register providers globally.

```ts
import { createLlmRuntime } from "@aether/ai";
import { registerBuiltInApiProviders } from "@aether/ai/providers";
...
`@aether/ai/event-stream`, `@aether/ai/transports`, and
`@aether/ai/validation`. No second Aether runtime package is required.

Provider ids, credentials, model catalogs, retries, and failover remain
application concerns. Aether supplies those policies around this package.
...
The explicit `@aether/ai/internal/anthropic`, `google-model-family`, `openai`,
`openai-responses-payload-policy`, `retry-after`, `runtime`, `shared`, and
`tool-schema` subpaths exist for the Aether application itself.
They carry no semver guarantee and can change or disappear in any release; do
not depend on them outside Aether.
