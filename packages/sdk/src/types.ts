...
import type { GatewayClientRequestOptions } from "@aether/gateway-client";
import type {
...
/** Minimal transport interface consumed by the Aether SDK client. */
export type AetherTransport = {
  request<T = unknown>(
    method: string,
    params?: unknown,
...
export type ConnectableAetherTransport = AetherTransport & {
...
export type RuntimeSelection =
  | "auto"
  | { type: "embedded"; id: "aether" | "codex" | (string & {}) }
...
export type AetherEvent<TData = unknown> = {
  version: 1;
  id: string;
  ts: number;
