...
// High-level Aether SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Aether SDK client. */
...
function resolveGatewayUrl(options: AetherOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `Aether Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current Aether Gateway yet`);
}
...
type RunTerminalSource = { kind: "canonical" } | { kind: "chat"; eventType: AetherEvent["type"] };
...
function readChatProjection(event: AetherEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: AetherEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: AetherEvent): boolean {
  return (
...
export class Aether {
  ...
  private readonly transport: AetherTransport;
  private readonly normalizedEvents = new EventHub<AetherEvent>();
  private readonly replayByRunId = new Map<string, AetherEvent[]>();
  ...
  events(filter?: (event: AetherEvent) => boolean): AsyncIterable<AetherEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("Aether SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: AetherEvent): AetherEvent | undefined => {
    ...
    const matches = (event: AetherEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: AetherEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): AetherEvent[] {
...
export class AgentsNamespace {
  constructor(private readonly client: Aether) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<AetherEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Aether,
